 ...
 * QQ120029121 账户安全
 * ============================================================================
 * 演示地址: http://demo.coolhong.com  开发QQ:1 2 0 0 2 9 1 2 1    309485552
 * ============================================================================
 * $Author: p r i n c e  $
 * $Id: security.php 17217 2015-07-27 06:29:08Z p r i n c e  $
 */
...
function action_password_reset_success ()
{
	...
	if($_CFG['sms_change_password'] == 1){
		...
		if($mobile_phone){
			...
			$templateParam=Array("code"=>date("Ymd H:i", gmtime()));
...
function action_email_validate ()
{
	...
	$sql = "select email from " . $yp->table('users') . " where user_name = '" . $_SESSION['user_name'] . "'";
...
function action_mobile_validate ()
{
	...
	$sql = "select mobile_phone from " . $yp->table('users') . " where user_name = '" . $_SESSION['user_name'] . "'";
...
function action_payment_password_reset ()
{
	...
	$sql = "SELECT is_surplus_open FROM " . $yp->table("users") . " WHERE user_id = '" . $user_id . "' LIMIT 1";
...
function action_do_payment_password_close ()
{
	...
	$sql = "UPDATE " . $yp->table('users') . " SET is_surplus_open = 0, surplus_password = '' WHERE user_id = '" . $user_id . "'";
	$db->query($sql);
	$affected_rows = $db->affected_rows();
...
function action_do_sync_supplier ()
{
	...
	$sql = "UPDATE " . $yp->table('supplier_admin_user') . " SET " . implode(', ', $values) . " WHERE uid = '" . $user_id . "' LIMIT 1";
...
function is_supplier ($user_id)
{
	...
	if($count > 0)
	{
		$sql = "select count(*) from " . $yp->table('supplier_admin_user') . " where uid = " . $user_id;
		$count = $db->getOne($sql);
		if($count > 0)
		{
			return true;
