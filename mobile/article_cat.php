...
if (!$smarty->is_cached('article_cat.dwt', $cache_id))
{
    ...
    $smarty->assign('article_categories',   article_categories_childtree($cat_id)); //文章分类树
    ...
    /* Meta */
    $meta = $db->getRow("SELECT keywords, cat_desc ,cat_name FROM " . $yp->table('article_cat') . " WHERE cat_id = '$cat_id'");
    ...
    $article_cats = $db->getOne("SELECT cat_id, cat_desc ,cat_name FROM " . $yp->table('article_cat') . " WHERE parent_id = '$cat_id'");
