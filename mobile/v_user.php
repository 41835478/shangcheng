...
if (!$smarty->is_cached('v_user.dwt', $cache_id))
{
	...
	$split_money = get_sum_split_money($_SESSION['user_id']); //佣金结余

	$user_info['parent_id'] = $db->getOne("select parent_id from ".$yp->table('users')." where user_id=".$_SESSION['user_id']);
	$user_info['parent_name'] = $db->getOne("select user_name from ".$yp->table('users')." where user_id=".$user_info['parent_id']);
	...
	$total_order_count = get_count_shouyi_order_by_user_id($_SESSION['user_id']);;
