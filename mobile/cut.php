...
$supplier_id = isset($_REQUEST['supplier_id']) ? intval($_REQUEST['supplier_id']) : 0;
 if($supplier_id){
   require_once(ROOT_PATH . 'includes/prince/lib_supplier_cut.php');
 }else{
   require_once(ROOT_PATH . 'includes/prince/lib_cut.php');
}//2014 1114   寒冰  qq  309485552     商家砍价
/*------------------------------------------------------ */
//-- act 操作项的初始化
/*------------------------------------------------------ */
...
{
    /* 取得砍价活动总数 */
    $count = cut_count($supplier_id);//2014 1114   寒冰  qq  309485552     商家砍价
    ...
    if (!$smarty->is_cached('cut_list.dwt', $cache_id))
    {
        ...
        {
            /* 取得当前页的砍价活动 */
            $cut_list = cut_list($size, $page ,$supplier_id);//2014 1114   寒冰  qq  309485552     商家砍价
...
if ($_REQUEST['act'] == 'userlist')
{
    ...
    $now = gmtime();
    $sql = "SELECT COUNT(*) " .
            "FROM " . $GLOBALS['yp']->table('cut') .
            "WHERE act_type = '" . GAT_CUT . "' " .
            "AND user_id='".$_SESSION['user_id']."'";//20170925
...
{    
    ...
    if (!$smarty->is_cached('cut.dwt', $cache_id))
    {
        ...
        $cut['share_url'] =HTTP_TYPE.'://'.$_SERVER['HTTP_HOST']."/mobile/cut.php?act=view&id=".$id."&u=".$_SESSION['user_id']; //20160119 added by PRINCE 120029121
...
{    
    ...
    if (!$smarty->is_cached('cut.dwt', $cache_id))
    {
		...
		/* 判断能否砍价 */	
		$sql = "select count(*) from ". $GLOBALS['yp']->table('cut_log') . " where cut_user='$user_id' and cut_id='$cut_id' ";
        ...
        $cut['share_url'] =HTTP_TYPE.'://'.$_SERVER['HTTP_HOST']."/mobile/cut.php?act=cut_view&cut_id=".$cut_id."&u=".$_SESSION['user_id']; //20160119 added by PRINCE 120029121
        $cut['share_img'] =strpos($cut['share_img'],'ttp')>0?$cut['share_img']:$goods['goods_thumb'];
		
		
        $sql = "SELECT cut_price " .
            "FROM  " . $GLOBALS['yp']->table('cut_log') . " " .
		...
		//砍价记录
        $sql = "SELECT cl.* " .
            "FROM  " . $GLOBALS['yp']->table('cut_log') . " AS cl " .
...
elseif ($_REQUEST['act'] == 'join')
{
    ...
    /* 判断能否参加 */	
    $sql = "select count(*) from ". $GLOBALS['yp']->table('cut') . " where user_id='$user_id' and act_id='$id' ";
	...
	if($join_times>=$cut['join_limit'] && $cut['join_limit']>0 ){
		...
		yp_header("Location: cut.php?act=view&id=$id\n");
		exit;
	}
    ...
    $db->autoExecute($yp->table('cut'), $cut, 'INSERT');	
	$cut_id = $db->insert_id();

    $_SESSION['cut_tips']='恭喜您成功参与活动。快分享给好友帮您砍价吧。';

    /* 参加成功 跳转到活动详情页 */
    yp_header("Location: cut.php?act=cut_view&id=$id&cut_id=$cut_id&u=$user_id\n");
    exit;
}
...
elseif ($_REQUEST['act'] == 'logpage')
{
    ...
    if ($id <= 0 ||  $actuid <=0)
    {
		...
		/* 跳转到活动详情页 */
		yp_header("Location: cut.php?act=view&id=$id&actuid=$actuid&u=$u\n");
		exit;
	}
...
elseif ($_REQUEST['act'] == 'cut')
{
	...
	if($had_cut){
	    $_SESSION['cut_tips']="抱歉！您已经参与过本次砍价";
   		yp_header("Location: cut.php?act=cut_view&cut_id=$cut_id&u=$u\n");
	 }
		
		
     //是否达到砍价次数
	$sql = "select count(*) from ". $GLOBALS['yp']->table('cut_log') . " where act_id='$act_id' and cut_user=".$_SESSION['user_id'];
	...
	if(1){
		...
		/* 取得当前价 */	
		$sql = "select new_price from ". $GLOBALS['yp']->table('cut') . " where cut_id='$cut_id'  ";
		...
		$cut_log = array(
			...
			'cut_user_head'  => $getinfo['headimg'],
			'cut_price' => $cut_price,
			'after_cut_price' => $after_cut_price,
			'cut_time'  => gmtime()
		);
		$db->autoExecute($yp->table('cut_log'), $cut_log, 'INSERT');
		
		/*更新发起者价格*/
		$sql = "UPDATE " . $yp->table('cut') . " SET new_price = '$after_cut_price' WHERE cut_id = '$cut_id' LIMIT 1";
		$db->query($sql);
...
elseif ($_REQUEST['act'] == 'buy')
{
	...
	$sql = "select * from ". $GLOBALS['yp']->table('cut') . " where cut_id='$cut_id'  ";
    ...
    $cart = array(
        ...
        'is_shipping'    =>$cut_info['free_shipping']?:0,1,
