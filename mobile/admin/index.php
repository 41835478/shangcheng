...
elseif ($_REQUEST['act'] == 'main')
{
    ...
    /* 已完成的订单 */
    $order['finished']     = $db->GetOne('SELECT COUNT(*) FROM ' . $yp->table('order_info').
...
//    $today_start = mktime(0,0,0,date('m'),date('d'),date('Y'));
    ...
    if ($_CFG['use_storage'])
    {
        $sql = 'SELECT COUNT(*) FROM ' .$yp->table('goods'). ' WHERE is_delete = 0 AND goods_number <= warn_number AND is_real = 1';
        $goods['warn'] = $db->GetOne($sql);
        $sql = 'SELECT COUNT(*) FROM ' .$yp->table('goods'). ' WHERE is_delete = 0 AND goods_number <= warn_number AND is_real=0 AND extension_code=\'virtual_card\'';
    ...
    /* 访问统计信息 */
    $today  = local_getdate();
    $sql    = 'SELECT COUNT(*) FROM ' .$yp->table('stats').
    ...
    /* 未审核评论 */
    $smarty->assign('comment_number', $db->getOne('SELECT COUNT(*) FROM ' . $yp->table('comment') .
    ...
    /* 缺货登记 */
    $smarty->assign('booking_goods', $db->getOne('SELECT COUNT(*) FROM ' . $yp->table('booking_goods') . ' WHERE is_dispose = 0'));

    /* 退款申请 */
    $smarty->assign('new_repay', $db->getOne('SELECT COUNT(*) FROM ' . $yp->table('user_account') . ' WHERE process_type = ' . SURPLUS_RETURN . ' AND is_paid = 0 '));



    assign_query_info();
    $smarty->assign('yp_version',  VERSION);
    $smarty->assign('yp_release',  RELEASE);
    $smarty->assign('yp_lang',     $_CFG['lang']);
    $smarty->assign('yp_charset',  strtoupper(YP_CHARSET));
...
elseif ($_REQUEST['act'] == 'main_api')
{
    ...
    ' WHERE is_delete = 0 AND is_alone_sale = 1 AND is_real = 1');
        ...
        $yp_template = $db->getOne('SELECT value FROM ' . $yp->table('ypmart_shop_config') . ' WHERE code = \'template\'');
        $style = $db->getOne('SELECT value FROM ' . $yp->table('ypmart_shop_config') . ' WHERE code = \'stylename\'');
        if($style == '')
        {
            $style = '0';
        }
