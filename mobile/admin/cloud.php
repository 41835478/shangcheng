 ...
 * ============================================================================
 * $Author: prince $
 * $Id: cloud.php 17063 2011-07-25 06:35:46Z prince $
*/
...
require(ROOT_PATH . 'includes/wx120029121_json.php');

$data['api_ver'] = '1.0';
$data['version'] = VERSION;
$data['patch'] = file_get_contents(ROOT_PATH.ADMIN_PATH_M."/patch_num");
$data['yp_lang'] = $_CFG['lang'];
$data['release'] = RELEASE;
$data['charset'] = strtoupper(YP_CHARSET);
...
$data['shop_url'] = urlencode($yp->url());
$data['admin_url'] = urlencode($yp->url().ADMIN_PATH_M);
...
$must = array('version','yp_lang','charset','patch','stamp','api_ver');
if($act =='menu_api')
{
    ...
    {
        $t = new transport;
       $apiget = "ver= $data[version] &yp_lang= $data[yp_lang] &charset= $data[charset]&ent_id=$data[ent_id]& certificate_id=$data[certificate_id]";
        $api_comment = $t->request('http://cloud.demo.coolhong.com/menu_api.php', $apiget);
        ...
        if (!empty($api_str))
        {
            ...
            if (!empty($api_arr) && $api_arr['error'] == 0 && md5($api_arr['content']) == $api_arr['hash'])
            {
                ...
                {
                    $api_arr['content'] = yp_iconv('UTF-8',$data['charset'],$api_arr['content']);
                }
...
elseif($act == 'cloud_remind')
{
    ...
    {
        $t = new transport('-1',5);
        $apiget = "ver=$data[version]&yp_lang=$data[yp_lang]&charset=$data[charset]&certificate_id=$data[certificate_id]&ent_id=$data[ent_id]";
        $api_comment = $t->request('http://cloud.demo.coolhong.com/cloud_remind.php', $apiget);
        $api_str=    $api_comment["body"];
        $json = new Services_JSON;
        $api_arr = @$json->decode($api_str,1);
        if(!empty($api_str))
...
elseif($act == 'close_remind')
{
    ...
    $t = new transport('-1',5);
    $apiget = "ver= $data[version] &yp_lang= $data[yp_lang] &charset= $data[charset] &certificate_id=$data[certificate_id]&ent_id=$data[ent_id]&remind_id=$remind_id";
    $api_comment = $t->request('http://cloud.demo.coolhong.com/cloud_remind.php', $apiget);
    ...
    if(!empty($api_str))
    {
        if (!empty($api_arr) && $api_arr['error'] == 0 && md5($api_arr['content']) == $api_arr['hash'])
        {
            ...
            if (admin_priv('all','',false))
            {
                ...
                $result=$t->request('http://cloud.demo.coolhong.com/cloud_remind.php', $apiget);
                $api_str = $result["body"];
                //var_dump($api_str);
                $api_arr = array();
                $api_arr = @$json->decode($api_str,1);
...
else
{
    ...
    if (!empty($_GET['link']))
    {
        ...
        if (!empty($url['host']))
        {
            yp_header("Location: ".$url['scheme']."://".$url['host'].$url['path']."?".$url['query'].$query."\n");
            exit();
        }
    }
    ...
    yp_header("Location: http://cloud.demo.coolhong.com/api.php?act=".$act.$query."\n");
    exit();
}
