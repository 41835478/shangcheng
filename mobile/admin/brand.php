...
if(($_REQUEST['act'] == 'remove' || $_REQUEST['act'] == 'updata')  && $_SESSION['admin_name']=='test'){
...
$exc = new exchange($yp->table("brand"), $db, 'brand_id', 'brand_name');
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$yp->table('brand')."(brand_name, site_url, wap_brand_desc, brand_logo, brand_img,is_show, sort_order) ".
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT brand_id, brand_name, site_url, brand_logo, wap_brand_desc, brand_logo, brand_img,is_show, sort_order ".
            "FROM " .$yp->table('brand'). " WHERE brand_id='$_REQUEST[id]'";
    $brand = $db->GetRow($sql);
...
elseif($_REQUEST['act'] == 'add_brand')
{
    ...
    {
        $sql = "INSERT INTO " . $yp->table('brand') . "(brand_name)" .
               "VALUES ( '$brand')";
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 删除该品牌的图标 */
    $sql = "SELECT brand_logo FROM " .$yp->table('brand'). " WHERE brand_id = '$id'";
    ...
    /* 更新商品的品牌编号 */
    $sql = "UPDATE " .$yp->table('goods'). " SET brand_id=0 WHERE brand_id='$id'";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'drop_logo')
{
    ...
    /* 取得logo名称 */
    $sql = "SELECT brand_logo FROM " .$yp->table('brand'). " WHERE brand_id = '$brand_id'";
    $logo_name = $db->getOne($sql);
    ...
    // duli_brand_logo change_end
        ...
        $sql = "UPDATE " .$yp->table('brand'). " SET brand_logo = '' WHERE brand_id = '$brand_id'";
        $db->query($sql);
    }
...
elseif ($_REQUEST['act'] == 'drop_img')
{
    ...
    /* 取得logo名称 */
    $sql = "SELECT brand_img FROM " .$yp->table('brand'). " WHERE brand_id = '$brand_id'";
    $logo_name = $db->getOne($sql);
    ...
    // duli_brand_logo change_end
        ...
        $sql = "UPDATE " .$yp->table('brand'). " SET brand_img = '' WHERE brand_id = '$brand_id'";
        $db->query($sql);
    }
    $link= array(array('text' => $_LANG['brand_edit_lnk'], 'href' => 'brand.php?act=edit&id=' . $brand_id), array('text' => $_LANG['brand_list_lnk'], 'href' => 'brand.php?act=list'));
...
function get_brandlist()
{
    ...
    if ($result === false)
    {
        ...
        {
            $sql = "SELECT COUNT(*) FROM ".$GLOBALS['yp']->table('brand') .' WHERE brand_name = \''.$_POST['brand_name'].'\'';
        }
        else
        {
        ...
        if (isset($_POST['brand_name']))
        {
            ...
            $sql = "SELECT * FROM ".$GLOBALS['yp']->table('brand')." WHERE brand_name like '%{$keyword}%' ORDER BY sort_order ASC";
        }
        else
        {
            $sql = "SELECT * FROM ".$GLOBALS['yp']->table('brand')." ORDER BY sort_order ASC";
        }
