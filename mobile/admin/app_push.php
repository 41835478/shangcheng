...
elseif($_REQUEST['act'] == 'save_setting')
{
    ...
    if(count($app_names) == count($app_keys) && count($app_keys) == count($app_secrets))
    {
		...
		$sql = 'UPDATE '.$yp->table('shop_config').' SET value= \''.serialize($push_setting).'\' WHERE code="push_setting"';
		if($db->query($sql))
		{
...
{
    global $db,$yp;
    $sql = 'SELECT value FROM '.$yp->table('shop_config').' WHERE code="push_setting"';
    $data = $db->getOne($sql);
