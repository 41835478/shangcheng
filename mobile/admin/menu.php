...
if(($_POST  || $_REQUEST['act'] == 'delete'  )&& $_SESSION['admin_name']=='test'){
	sys_msg('警告，你的权限不足，请勿非法操作！');
}
// 今天 优品 多商户系统 技术支持：热风科技，Q Q：309 485 552、12 00 29 121   2016
...
elseif($_REQUEST['act'] == 'update')
{
	...
	$sql = "update ".$GLOBALS['yp']->table('ypmart_menu')." set ".$con." `menu_url`='$menu_url',`menu_name`='$menu_name',`sort`='$sort' where id = '$id'";
...
{
	$id = $_REQUEST['id'];
	$sql = "delete from ".$GLOBALS['yp']->table('ypmart_menu')." where id = '$id'";
	$num = $GLOBALS['db']->query($sql);
	if($num > 0)
	{
...
elseif($_REQUEST['act'] == 'add_menu')
{
	...
	$sql = "insert into ".$GLOBALS['yp']->table('ypmart_menu')."(`menu_name`,`menu_img`,`menu_url`,`sort`) values('$menu_name','$menu_img','$menu_url','$sort')";
	$num = $GLOBALS['db']->query($sql);
	if($num > 0)
	{
...
function get_all_menu()
{
	 ...
	 $sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['yp']->table('ypmart_menu') ." as s ". $where;
     $filter['record_count'] = $GLOBALS['db']->getOne($sql);

     $filter = page_and_size($filter);

     $arr = array();
	 $sql = "select s.* from ".$GLOBALS['yp']->table('ypmart_menu')." as s  $where ";
...
{
	$sql = "select * from ".$GLOBALS['yp']->table('ypmart_menu')." where id = '$id'";
