...
/**
 * QQ120029121 分销会员列表程序
 * ============================================================================
 * 演示地址: http://demo.coolhong.com；
 * ============================================================================
 * $Author: dqy $
 * $Id: user_grade.php 17217 2017-04-01 06:29:08Z dqy $
*/
...
function get_affiliate_ck($user_id,$level)
{
    ...
    if(!empty($affiliate['on']))
    {
        ...
        {
            //推荐注册分成
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('order_info') . " o".
                    " LEFT JOIN".$GLOBALS['yp']->table('users')." u ON o.user_id = u.user_id".
                    " LEFT JOIN " . $GLOBALS['yp']->table('affiliate_log') . " a ON o.order_id = a.order_id" .
    ...
    if(!empty($affiliate['on']))
    {
        if(empty($separate_by))
        {
            ...
            $sql = "SELECT o.*, a.log_id, a.user_id as suid,  a.user_name as auser, a.money, a.point, a.separate_type,u.parent_id as up FROM " . $GLOBALS['yp']->table('order_info') . " o".
                    " LEFT JOIN".$GLOBALS['yp']->table('users')." u ON o.user_id = u.user_id".
                    " LEFT JOIN " . $GLOBALS['yp']->table('affiliate_log') . " a ON o.order_id = a.order_id" .
                    " WHERE o.user_id > 0 AND (u.parent_id > 0 AND o.is_separate = 0 OR o.is_separate > 0) $sqladd".
	...
	foreach($logdb  as $key=>$value){
		if($value['user_id']==$user_id){
			...
			if(!empty($wxid)){
				$weixinInfo = $GLOBALS['db']->getRow("SELECT nickname, headimg FROM " . $GLOBALS['yp']->table('users') . " WHERE fake_id = '$wxid'");
				$logdbnew[$key]['headimg'] = empty($weixinInfo['headimg']) ? '':$weixinInfo['headimg'];
