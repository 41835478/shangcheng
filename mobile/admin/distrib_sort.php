...
function get_distrib_sort()
{
     ...
     $arr = array();
	 $sql = "SELECT d.*,sum(money) as total_money,u.user_name FROM " .$GLOBALS['yp']->table('affiliate_log') ." as d inner join " . $GLOBALS['yp']->table('users') . " as u on d.user_id = u.user_id where d.separate_type=0 group by d.user_id order by total_money desc";
