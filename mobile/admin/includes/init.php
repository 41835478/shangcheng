...
/* 如果有新版本，升级 */
if (!isset($_CFG['yp_version']))
{
    $_CFG['yp_version'] = 'v7.0';
}

if (preg_replace('/(?:\.|\s+)[a-z]*$/i', '', $_CFG['yp_version']) != preg_replace('/(?:\.|\s+)[a-z]*$/i', '', VERSION)
        && file_exists('../upgrade/index.php'))
{
    // 转到升级文件
    yp_header("Location: ../upgrade/index.php\n");
...
if(isset($_GET['ent_id']) && isset($_GET['ent_ac']) &&  isset($_GET['ent_sign']) && isset($_GET['ent_email']))
{
    ...
    if($token==md5(md5($_CFG['token']).$domain_url.ADMIN_PATH_M))
    {
        ...
        $db->query('UPDATE '.$yp->table('ypmart_shop_config',1) . ' SET value = "'. $ent_id .'" WHERE code = "ent_id"');
        $db->query('UPDATE '.$yp->table('ypmart_shop_config',1) . ' SET value = "'. $ent_ac .'" WHERE code = "ent_ac"');
        $db->query('UPDATE '.$yp->table('ypmart_shop_config',1) . ' SET value = "'. $ent_sign .'" WHERE code = "ent_sign"');
        $db->query('UPDATE '.$yp->table('ypmart_shop_config',1) . ' SET value = "'. $ent_email .'" WHERE code = "ent_email"');
        clear_cache_files();
    ...
    $_REQUEST['act'] != 'forget_pwd' && $_REQUEST['act'] != 'reset_pwd' && $_REQUEST['act'] != 'check_order')
{
	...
	$safecode=$_COOKIE['YPCP']['safecode']?$_COOKIE['YPCP']['safecode']:'0';//20170111 prince
    if (!empty($_COOKIE['YPCP']['admin_id']) && !empty($_COOKIE['YPCP']['admin_pass']) && $safecode==md5(ADMIN_SAFECODE))//20170111 prince
    {
        // 找到了cookie, 验证cookie信息
        $sql = 'SELECT user_id, user_name, password, action_list, last_login ' .
                ' FROM ' .$yp->table('admin_user') .
                " WHERE user_id = '" . intval($_COOKIE['YPCP']['admin_id']) . "'";
        $row = $db->GetRow($sql);

        if (!$row)
        {
            // 没有找到这个记录
            setcookie($_COOKIE['YPCP']['admin_id'],   '', 1);
            setcookie($_COOKIE['YPCP']['admin_pass'], '', 1);
        ...
        {
            // 检查密码是否正确
            if (md5($row['password'] . $_CFG['hash_code']) == $_COOKIE['YPCP']['admin_pass'])
            {
...
if ($_REQUEST['act'] != 'login' && $_REQUEST['act'] != 'signin' &&
    $_REQUEST['act'] != 'forget_pwd' && $_REQUEST['act'] != 'reset_pwd' && $_REQUEST['act'] != 'check_order')
{
    $ADMIN_PATH = preg_replace('/:\d+/', '', $yp->url()) . ADMIN_PATH_M;
	//$s1=$yp->url();
	
	//echo $s1.'--'.preg_replace('/:\d+/', '', $s1);
        ...
        strpos(preg_replace('/:\d+/', '', $_SERVER['HTTP_REFERER']), $ADMIN_PATH) === false)
    {
        ...
        {
			//yp_header("Location: privilege.php?act=login\n");
			$ADMIN_PATH = str_replace('/mobile','',$ADMIN_PATH );
			if (!empty($_SERVER['HTTP_REFERER']) &&
				strpos(preg_replace('/:\d+/', '', $_SERVER['HTTP_REFERER']), $ADMIN_PATH) === false)
			{
				if (!empty($_REQUEST['is_ajax']))
