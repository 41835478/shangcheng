...
function get_affiliate_ck()
{
    ...
    if(isset($_REQUEST['supplier_id'])){   
		if(intval($_REQUEST['supplier_id'])>=0){
			$sqladd .= ' AND o.supplier_id = ' . intval($_REQUEST['supplier_id']); 
		}
	}
	...
	if($GLOBALS['_CFG']['distrib_type'] == 0)
	{
		$sql = "SELECT order_sn,is_separate,order_id,o.user_id,add_time,order_status,o.pay_status,o.shipping_status,o.supplier_id,u.user_name FROM ".$GLOBALS['yp']->table('order_info')." AS o," . $GLOBALS['yp']->table('users') . " as u WHERE o.pay_status = 2 and o.is_separate=0  AND o.user_id = u.user_id $sqladd ORDER BY order_id DESC LIMIT " . $filter['start'] . ",$filter[page_size]";
	}
	else
	{
		$sql = "select order_sn,is_separate,order_id,user_id,add_time,
		order_status,pay_status,shipping_status,supplier_id,user_name from " .
		"(select o.order_id,o.order_sn,o.user_id,o.add_time,o.order_status,o.pay_status,o.shipping_status,".
		"o.supplier_id,sum(b.split_money*goods_number) as total_money," .
