...
else if($act == 'guide_picture')
{
	...
	$guide_picture = $db->getOne('SELECT value FROM '.$yp->table('shop_config').' WHERE code="guide_picture"');
...
else if($act == 'save_guide_picture')
{
	...
	$sql = 'UPDATE '.$yp->table('shop_config').' SET value=\''.serialize($value).'\' WHERE code="guide_picture"';
	if($db->query($sql))
	{
...
function get_menu_list(){
	global $db,$yp;
	$sql = 'SELECT value FROM '.$yp->table('shop_config').' WHERE code="menu_setting" ';
...
function save_menu_list($menu_list){
	global $db,$yp;
	$sql = 'UPDATE '.$yp->table('shop_config').' SET value=\''.serialize($menu_list).'\' WHERE code="menu_setting"';
	if($db->query($sql))
	{
		return true;
	}
...
function get_template_setting(){
	global $db,$yp;
	$sql = 'SELECT value FROM '.$yp->table('shop_config').' WHERE code="template_setting"';
...
function save_template_setting($template_setting){
	...
	$template = $db->getOne('SELECT value FROM '.$yp->table('shop_config').' WHERE code="app_template"');
	if(empty($template)){
		$template = 'default';
	}
	$tmp_root_path = substr(ROOT_PATH,0,-7);
	...
	if((empty($template_content) && $result == 0) || (!empty($template_content) && $result > 0)){
		...
		$sql = 'UPDATE '.$yp->table('shop_config').' SET value=\''.$serialized_setting.'\' WHERE code="template_setting"';
		$result = $db->query($sql);
	}
	return $result;
}
...
function get_basic_setting(){
	global $db,$yp;
	$sql = 'SELECT * FROM '.$yp->table('shop_config').' WHERE parent_id="12" AND type !="manual"';
