 ...
 * ============================================================================
 * $Author: Prince $
 * $Id: mail_template.php 17063 2010-03-25 06:35:46Z Prince $
*/
...
if ($_REQUEST['act'] == 'list')
{
    ...
    /* 获得所有邮件模板 */
    $sql = "SELECT template_id, template_code,template_subject	 FROM " .$yp->table('mail_templates') . " WHERE  type = 'template'";
    $res = $db->query($sql);
    $cur = null;
    ...
    while ($row = $db->FetchRow($res))
    {
		...
		$template_subject=$_LANG[$row['template_code']]?$_LANG[$row['template_code']]:$row['template_subject'];//20170904
		$len = strlen($template_subject);
        $templates[$row['template_id']] = $len < 18 ?
            $template_subject.str_repeat('&nbsp;', (18-$len)/2) ." [$row[template_code]]" :
            $template_subject . " [$row[template_code]]";
    }
...
var_dump($_POST);
	...
	if(strpos($content, 'file_put_contents')===false && strpos($content, 'writ')===false && strpos($content, 'fopen')===false ){
    ...
    $sql = "UPDATE " .$yp->table('mail_templates'). " SET ".
...
function load_template($temp_id)
{
    $sql = "SELECT template_subject, template_content, is_html ".
            "FROM " .$GLOBALS['yp']->table('mail_templates'). " WHERE template_id='$temp_id'";
