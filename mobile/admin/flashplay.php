 ...
 * ==========================================================
 * $Author: prince $
 * $Id: flashplay.php 17217 2017-04-01 06:29:08Z prince $
 */
...
if ($_REQUEST['act']== 'list')
{
    ...
    {
        yp_header("Location: flashplay.php?act=custom_list\n");
        exit;
    }
...
elseif($_REQUEST['act']== 'del')
{
    ...
    yp_header("Location: flashplay.php?act=list\n");
    exit;
}
...
elseif ($_REQUEST['act'] == 'install')
{
    ...
    {
        $sql = "UPDATE " .$GLOBALS['yp']->table('ypmart_shop_config',1). " SET value = '$flash_theme' WHERE code = 'flash_theme'";
...
elseif ($_REQUEST['act'] == 'custom_insert')
{
    ...
    $db->autoExecute($yp->table('ad_custom'), $ad, 'INSERT', '', 'SILENT');
    $ad_id = $db->insert_id();
...
elseif($_REQUEST['act']== 'custom_del')
{
    ...
    $query = $db->query("DELETE FROM " . $yp->table('ad_custom') . " WHERE ad_id = $id");
...
elseif ($_REQUEST['act'] == 'custom_status')
{
    ...
    if (modfiy_ad_status($id, $ad_status))
    {
        ...
        /* 标签初始化 */
        $sql = "SELECT  value FROM " . $yp->table("ypmart_shop_config") . " WHERE id =337";
...
elseif ($_REQUEST['act'] == 'custom_edit')
{
    ...
    /* 查询自定义广告信息 */
    $sql = "SELECT ad_id, ad_type, content, url, ad_status, ad_name FROM " . $GLOBALS['yp']->table("ad_custom") . " WHERE ad_id = $id LIMIT 0, 1";
...
elseif ($_REQUEST['act'] == 'custom_update')
{
    ...
    /* 查询自定义广告信息 */
    $sql = "SELECT ad_id, ad_type, content, url, ad_status, ad_name FROM " . $GLOBALS['yp']->table("ad_custom") . " WHERE ad_id = " . $filter['ad']['id'] ." LIMIT 0, 1";
    ...
    $db->autoExecute($yp->table('ad_custom'), $ad, 'UPDATE', 'ad_id = ' . $ad_info['ad_id'], 'SILENT');
...
function put_flash_xml($flashdb)
{
    if (!empty($flashdb))
    {
        $xml = '<?xml version="1.0" encoding="' . YP_CHARSET . '"?><bcaster>';
        foreach ($flashdb as $key => $val)
        {
...
function set_flash_data($tplname, &$msg)
{
    ...
    {
        $flashdata[] = array(
                                'src' => '/data/afficheimg/flash.jpg',
                                'text' => '今天优品多商户系统',
                                'url' =>'http://demo.coolhong.com/'
                            );
        $flashdata[] = array(
                                'src' => '/data/afficheimg/flash.jpg',
                                'text' => '今天优品多商户系统',
...
function set_flash_uproll($tplname, $flashdata)
{
    ...
    $xmldata = '<?xml version="1.0" encoding="' . YP_CHARSET . '"?><myMenu>';
...
{
    $data_file = ROOT_PATH . DATA_DIR . '/flashdata/' . $tplname . '/data.xml';
    $xmldata = '<?xml version="1.0" encoding="' . YP_CHARSET . '"?><bcaster>';
    foreach ($flashdata as $data)
    {
...
function ad_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT ad_id, CASE WHEN ad_type = 0 THEN '图片'
                ...
                FROM " . $GLOBALS['yp']->table("ad_custom") . "
                $where
 ...
 */
 function modfiy_ad_status($ad_id, $ad_status = 0)
 {
    ...
    /* 查询自定义广告信息 */
    $sql = "SELECT ad_type, content, url, ad_status FROM " . $GLOBALS['yp']->table("ad_custom") . " WHERE ad_id = $ad_id LIMIT 0, 1";
    $ad = $GLOBALS['db']->getRow($sql);
    ...
    if ($ad_status == 1)
    {
        ...
        if ($ad['ad_status'] == 0)
        {
            $sql = "UPDATE " . $GLOBALS['yp']->table("ad_custom") . " SET ad_status = 1 WHERE ad_id = $ad_id";
            $GLOBALS['db']->query($sql);
        }

        /* 关闭 其它自定义广告 */
        $sql = "UPDATE " . $GLOBALS['yp']->table("ad_custom") . " SET ad_status = 0 WHERE ad_id <> $ad_id";
        $GLOBALS['db']->query($sql);

        /* 用户自定义广告开启 */
        $sql = "UPDATE " . $GLOBALS['yp']->table("ypmart_shop_config",1) . " SET value = 'cus' WHERE id =337";
        $GLOBALS['db']->query($sql);
    }
    else
    {
        ...
        if ($ad['ad_status'] == 0)
        {
            $sql = "SELECT COUNT(ad_id) FROM " . $GLOBALS['yp']->table("ad_custom") . " WHERE ad_status = 1";
            $ad_status_1 = $GLOBALS['db']->getOne($sql);
            if (empty($ad_status_1))
            {
                $sql = "UPDATE " . $GLOBALS['yp']->table("ypmart_shop_config",1) . " SET value = 'sys' WHERE id =337";
                $GLOBALS['db']->query($sql);
            }
            else
            {
