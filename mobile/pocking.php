...
}elseif (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'get_pickup_point_list_city')
{
	$city_id = $_REQUEST['city_id'];
	$suppid = intval($_REQUEST['suppid']);
	$sql = 'select * from ' . $GLOBALS['yp']->table('pickup_point') . ' where city_id=' . $city_id.' and supplier_id='.$suppid;
	$pickup_point_list = $GLOBALS['db']->getAll($sql);
	die(json_encode($pickup_point_list));
}elseif (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'get_pickup_point_list_province')
{
	$province_id = $_REQUEST['province_id'];
	$suppid = intval($_REQUEST['suppid']);
	$sql = 'select * from ' . $GLOBALS['yp']->table('pickup_point') . ' where province_id=' . $province_id.' and supplier_id='.$suppid;
	$pickup_point_list = $GLOBALS['db']->getAll($sql);
	die(json_encode($pickup_point_list));
}
/*------------------------------------------------------ */
...
// added by prince 20170102 start
$province = '';
$city =  '';
$district =  '';
...
if(strpos($useragent, 'MicroMessenger')!==false){
	...
	if($user_info['Latitude'] && $user_info['Longitude']){
		...
		$location = $user_info['Latitude'].",".$user_info['Longitude'];
		$url = "http://apis.map.qq.com/ws/geocoder/v1/?location=$location&key=$key&get_poi=1";
		$res = file_get_contents($url);
		$arr = json_decode($res, true);
		$province = str_replace("省","",$arr['result']['address_component']['province']);
		$city =  str_replace("市","",$arr['result']['address_component']['city']);
