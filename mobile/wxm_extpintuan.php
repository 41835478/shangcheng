...
function send_order_message($order_id){
	...
	$sql = "SELECT * FROM " . $GLOBALS['yp']->table('extpintuan_orders') . " WHERE order_id = '$order_id' ";
	...
	$query_sql_1 = "SELECT * FROM " . $GLOBALS['yp']->table('users') . " WHERE user_id = '$act_user'";
	$ret_w_1 = $GLOBALS['db']->getRow($query_sql_1);
	$act_wxid = $ret_w_1['fake_id'];
		
	$query_sql_2 = "SELECT * FROM " . $GLOBALS['yp']->table('users') . " WHERE user_id = '$follow_user'";
	$ret_w_2 = $GLOBALS['db']->getRow($query_sql_2);
	$follow_wxid = $ret_w_2['fake_id'];
	
	$wap_url = $GLOBALS['db'] -> getOne("SELECT `wap_url` FROM ".$GLOBALS['yp']->table('weixin_config')." WHERE `id`=1");
	...
	$extpintuan_open = $GLOBALS['db']->getOne ( "SELECT 	templet_id FROM " . $GLOBALS['yp']->table('weixin_new_remind') . " WHERE `remind_name` = 'extpintuan_open' " );
	$extpintuan_join = $GLOBALS['db']->getOne ( "SELECT 	templet_id FROM " . $GLOBALS['yp']->table('weixin_new_remind') . " WHERE `remind_name` = 'extpintuan_join' " );
	$extpintuan_name = $GLOBALS['db']->getOne ( "SELECT 	act_name FROM " . $GLOBALS['yp']->table('goods_activity') . " WHERE `act_id` = '$act_id' " );
	$extpintuan_price = $GLOBALS['db']->getOne ( "SELECT 	price FROM " . $GLOBALS['yp']->table('extpintuan') . " WHERE `pt_id` = '$pt_id' " );
	$end_time = $GLOBALS['db']->getOne ( "SELECT 	end_time FROM " . $GLOBALS['yp']->table('extpintuan') . " WHERE `pt_id` = '$pt_id' " );
...
function send_status_message($user_id,$order_id,$order_sn,$pt_id,$status){
	$access_token = access_token($db);
	$url = 'https://api.weixin.qq.com/cgi-bin/message/template/send?access_token='.$access_token;
	$query_sql = "SELECT * FROM " . $GLOBALS['yp']->table('users') . " WHERE user_id = '$user_id'";
	...
	$extpintuan_success = $GLOBALS['db']->getOne ( "SELECT 	templet_id FROM " . $GLOBALS['yp']->table('weixin_new_remind') . " WHERE `remind_name` = 'extpintuan_success' " );
	$extpintuan_fail = $GLOBALS['db']->getOne ( "SELECT 	templet_id FROM " . $GLOBALS['yp']->table('weixin_new_remind') . " WHERE `remind_name` = 'extpintuan_fail' " );
	$extpintuan_name = $GLOBALS['db']->getOne ( "SELECT 	act_name FROM " . $GLOBALS['yp']->table('goods_activity') . " WHERE `act_id` = '$act_id' " );
	$goods_amount = $GLOBALS['db']->getOne ( "SELECT 	goods_amount FROM " . $GLOBALS['yp']->table('order_info') . " WHERE `order_id` = '$order_id' " );
	$money_paid = $GLOBALS['db']->getOne ( "SELECT 	money_paid FROM " . $GLOBALS['yp']->table('order_info') . " WHERE `order_id` = '$order_id' " );
...
function send_lucky_message($user_id,$order_id,$order_sn,$pt_id,$lucky_order){
	...
	$extpintuan_result = $GLOBALS['db']->getOne ( "SELECT 	templet_id FROM " . $GLOBALS['yp']->table('weixin_new_remind') . " WHERE `remind_name` = 'extpintuan_result' " );
	$extpintuan_name = $GLOBALS['db']->getOne ( "SELECT 	act_name FROM " . $GLOBALS['yp']->table('goods_activity') . " WHERE `act_id` = '$act_id' " );
	$end_time = $GLOBALS['db']->getOne ( "SELECT 	end_time FROM " . $GLOBALS['yp']->table('goods_activity') . " WHERE `act_id` = '$act_id' " );
...
//ALTER TABLE ".$GLOBALS['yp']->table('weixin_config')." CHANGE `access_token` `access_token` TEXT CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL 
// PRINCE QQ 120029121
?>