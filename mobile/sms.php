...
$count = $db->getOne("SELECT COUNT(id) FROM " . $yp->table('verifycode') ." WHERE getip='" . real_ip() . "' AND dateline>'" . gmtime() ."'-120");
...
//	}
	/* 提交的手机号是否已经注册帐号 */
    $sql = "SELECT COUNT(user_id) FROM " . $yp->table('users') ." WHERE mobile_phone = '$mobile'";
    ...
    $content = '您好，您的验证码'.$verifycode.';【qq120029121】';
	/* 发送注册手机短信验证 */
	$templateParam=Array("code"=>$verifycode);
	$templateCode=$GLOBALS['_CFG']['sms_register_dayu'];
	$ret = sendSMS($mobile, $content,'', '',$templateParam , $templateCode);//已支持阿里大于 QQ 120029121
	
    $db->query("delete from ".$yp->table('verifycode')." where mobile='$mobile'");
	
		//插入获取验证码数据记录
		$sql = "INSERT INTO " . $yp->table('verifycode') . "(mobile, getip, verifycode, dateline) VALUES ('" . $mobile . "', '" . real_ip() . "', '$verifycode', '" . gmtime() ."')";
		$db->query($sql);
