...
function brand_get_goods($brand_id, $cate, $size, $page, $sort, $order, $limit='')
{
	...
	if ($sort=='salenum')
	{
		$sql = 'SELECT SUM(o.goods_number) as salenum, g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
            ...
            'FROM ' . $GLOBALS['yp']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['yp']->table('member_price') . ' AS mp ' .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " . "LEFT JOIN " . $GLOBALS['yp']->table('order_goods') .  " as o ON o.goods_id = g.goods_id " . " WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND g.brand_id = '$brand_id' $cate_where group by g.goods_id ORDER BY $sort $order $limit";
	}
