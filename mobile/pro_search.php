...
function get_pro_search_list($limit){
    ...
    if (!empty($_REQUEST['keywords']))
    {
        ...
        foreach ($arr AS $key => $val)
        {
            ...
            $sql = 'SELECT DISTINCT goods_id FROM ' . $GLOBALS['yp']->table('tag') . " WHERE tag_words LIKE '%$val%' ";
            $res = $db->query($sql);
            ...
            $db->autoReplace($GLOBALS['yp']->table('keywords'), array('date' => local_date('Y-m-d'),
    ...
    {
        $sql = "SELECT goods_id, COUNT(*) AS num FROM " . $GLOBALS['yp']->table("goods_attr") . " WHERE 0 ";
    ...
    {
        /* 从选购中心进入的链接 */
        $sql = "SELECT DISTINCT(goods_id) FROM " . $GLOBALS['yp']->table('goods_attr');
        $col = $db->getCol($sql);
...
function get_wap_group( $position, $num )
{
        ...
        else
        {
			...
			$arr[$row['ad_id']]['ss_time']  = '已结束';
				...
				}//20170603 prince 1 2 0 0 29 121
