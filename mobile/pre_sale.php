...
/**
 * QQ120029121 预售活动前台文件
 * ============================================================================
 * ============================================================================
 * $Author: jtypmall $
 * $Id: pre_sale.php 17217 2015-06-24 10:57:00Z jtypmall $
 */
...
if(empty($_REQUEST['act']))
{
	...
	if($pre_sale_id <= 0)
	{
		yp_header("Location: pre_sale.php\n");
		exit();
	}
	...
	/* 查询：查询规格名称和值，不考虑价格 */
	$attr_list = array();
	$sql = "SELECT a.attr_name, g.attr_value " . "FROM " . $yp->table('goods_attr') . " AS g, " . $yp->table('attribute') . " AS a " . "WHERE g.attr_id = a.attr_id " . "AND g.goods_attr_id " . db_create_in($specs);
	$res = $db->query($sql);
	...
	//zhouhui周辉
    $GLOBALS['db']->query("UPDATE ".$GLOBALS['yp']->table('cart')." set extension_code='pre_sale',rec_type='6' where goods_id=".$goods['goods_id']." AND user_id=".$_SESSION['user_id']);
...
function action_view ()
{
	...
	if(! $smarty->is_cached('pre_sale_goods.dwt', $cache_id) || true)
	{
		...
		// 评价晒单 增加 by demo.coolhong.com
		$rank_num['rank_a'] = $GLOBALS['db']->getOne("SELECT COUNT(*) AS num FROM " . $GLOBALS['yp']->table('comment') . " WHERE id_value = '$goods_id' AND status = 1 AND comment_rank in (5,4)");
		$rank_num['rank_b'] = $GLOBALS['db']->getOne("SELECT COUNT(*) AS num FROM " . $GLOBALS['yp']->table('comment') . " WHERE id_value = '$goods_id' AND status = 1 AND comment_rank in (3,2)");
		$rank_num['rank_c'] = $GLOBALS['db']->getOne("SELECT COUNT(*) AS num FROM " . $GLOBALS['yp']->table('comment') . " WHERE id_value = '$goods_id' AND status = 1 AND comment_rank = 1");
		...
		$rank_num['shaidan_num'] = $GLOBALS['db']->getOne("SELECT COUNT(*) AS num FROM " . $GLOBALS['yp']->table('shaidan') . " WHERE goods_id = '$goods_id' AND status = 1");
		...
		foreach($res as $v)
		{
			$v['tag_num'] = $GLOBALS['db']->getOne("SELECT COUNT(*) AS num FROM " . $GLOBALS['yp']->table('comment') . " WHERE id_value = '$goods_id' AND status = 1 AND FIND_IN_SET($v[tag_id],comment_tag)");
			$tag_arr[] = $v;
		}
		...
		$sql_attr = "SELECT a.attr_id, ga.goods_attr_id FROM " . $GLOBALS['yp']->table('attribute') . " AS a left join " . $GLOBALS['yp']->table('goods_attr') . "  AS ga on a.attr_id=ga.attr_id  WHERE a.is_attr_gallery=1 and ga.goods_id='" . $goods_id . "' order by ga.goods_attr_id ";
		...
		{
			$sql_supplier = "SELECT s.supplier_id,s.supplier_name,s.add_time,sr.rank_name FROM " . $yp->table("supplier") . " as s left join " . $yp->table("supplier_rank") . " as sr ON s.rank_id=sr.rank_id WHERE s.supplier_id=" . $goods[supplier_id] . " AND s.status=1";
	...
	// 更新商品点击次数
	$sql = 'UPDATE ' . $GLOBALS['yp']->table('goods') . ' SET click_count = click_count + 1 ' . "WHERE goods_id = '" . $pre_sale['goods_id'] . "'";
...
function pre_sale_count ()
{
	...
	// $GLOBALS['yp']->table('goods_activity') . "WHERE act_type = '" .
	// GAT_PRE_SALE . "' " . "AND start_time <= '$now' AND is_finished < 3";
	$sql = "SELECT COUNT(*) " . "FROM " . $GLOBALS['yp']->table('goods_activity') . "WHERE act_type = '" . GAT_PRE_SALE . "' " . " AND is_finished < 3";
...
function pre_sale_list ($size, $page)
{
	...
	// g.shop_price " . "FROM " . $GLOBALS['yp']->table('goods_activity') . "
	// AS b " . "LEFT JOIN " . $GLOBALS['yp']->table('goods') . " AS g ON
	...
	$sql = "SELECT b.*, IFNULL(g.goods_thumb, '') AS goods_thumb, b.act_id AS pre_sale_id, " . "b.start_time AS start_date, b.end_time AS end_date, g.shop_price " . "FROM " . $GLOBALS['yp']->table('goods_activity') . " AS b " . "LEFT JOIN " . $GLOBALS['yp']->table('goods') . " AS g ON b.goods_id = g.goods_id " . "WHERE b.act_type = '" . GAT_PRE_SALE . "' " . " AND b.is_finished < '" . PSS_SUCCEED . "' ORDER BY b.is_finished ASC";
...
function cancel_ps_order ($order_id, $user_id = 0)
{
	...
	/* 查询订单信息，检查状态 */
	$sql = "SELECT user_id, order_id, order_sn , surplus , integral , bonus_id, order_status, shipping_status, pay_status FROM " . $GLOBALS['yp']->table('order_info') . " WHERE order_id = '$order_id' and extension_code = '" . pre_sale . "'";
	...
	// 将用户订单设置为取消
	$sql = "UPDATE " . $GLOBALS['yp']->table('order_info') . " SET order_status = '" . OS_CANCELED . "' WHERE order_id = '$order_id'";
...
{
	$sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' . "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, " . 'g.market_price, g.promote_price, g.promote_start_date, g.promote_end_date ' . 'FROM ' . $GLOBALS['yp']->table('link_goods') . ' lg ' . 'LEFT JOIN ' . $GLOBALS['yp']->table('goods') . ' AS g ON g.goods_id = lg.link_goods_id ' . "LEFT JOIN " . $GLOBALS['yp']->table('member_price') . " AS mp " . "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " . "WHERE lg.goods_id = '$goods_id' AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 " . "LIMIT " . $GLOBALS['_CFG']['related_goods_number'];
...
{
	$sql = 'SELECT a.article_id, a.title, a.file_url, a.open_type, a.add_time ' . 'FROM ' . $GLOBALS['yp']->table('goods_article') . ' AS g, ' . $GLOBALS['yp']->table('article') . ' AS a ' . "WHERE g.article_id = a.article_id AND g.goods_id = '$goods_id' AND a.is_open = 1 " . 'ORDER BY a.add_time DESC';
	$res = $GLOBALS['db']->query($sql);
...
{
	$sql = "SELECT rank_id, IFNULL(mp.user_price, r.discount * $shop_price / 100) AS price, r.rank_name, r.discount " . 'FROM ' . $GLOBALS['yp']->table('user_rank') . ' AS r ' . 'LEFT JOIN ' . $GLOBALS['yp']->table('member_price') . " AS mp " . "ON mp.goods_id = '$goods_id' AND mp.user_rank = r.rank_id " . "WHERE r.show_price = 1 OR r.rank_id = '$_SESSION[user_rank]'";
	$res = $GLOBALS['db']->query($sql);
...
{
	$sql = 'SELECT COUNT(b.goods_id ) AS num, g.goods_id, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price, g.promote_price, g.promote_start_date, g.promote_end_date ' . 'FROM ' . $GLOBALS['yp']->table('order_goods') . ' AS a ' . 'LEFT JOIN ' . $GLOBALS['yp']->table('order_goods') . ' AS b ON b.order_id = a.order_id ' . 'LEFT JOIN ' . $GLOBALS['yp']->table('goods') . ' AS g ON g.goods_id = b.goods_id ' . "WHERE a.goods_id = '$goods_id' AND b.goods_id <> '$goods_id' AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 " . 'GROUP BY b.goods_id ' . 'ORDER BY num DESC ' . 'LIMIT ' . $GLOBALS['_CFG']['bought_goods'];
	$res = $GLOBALS['db']->query($sql);
...
function get_goods_rank ($goods_id)
{
	...
	/* 查询该商品销量 */
	$sql = 'SELECT IFNULL(SUM(g.goods_number), 0) ' . 'FROM ' . $GLOBALS['yp']->table('order_info') . ' AS o, ' . $GLOBALS['yp']->table('order_goods') . ' AS g ' . "WHERE o.order_id = g.order_id " . "AND o.order_status = '" . OS_CONFIRMED . "' " . "AND o.shipping_status " . db_create_in(array(
	...
	if($sales_count > 0)
	{
		...
		$sql = 'SELECT DISTINCT SUM(goods_number) AS num ' . 'FROM ' . $GLOBALS['yp']->table('order_info') . ' AS o, ' . $GLOBALS['yp']->table('order_goods') . ' AS g ' . "WHERE o.order_id = g.order_id " . "AND o.order_status = '" . OS_CONFIRMED . "' " . "AND o.shipping_status " . db_create_in(array(
			SS_SHIPPED, SS_RECEIVED
		)) . " AND o.pay_status " . db_create_in(array(
			PS_PAYED, PS_PAYING
...
{
	$sql = "SELECT SUM(attr_price) FROM " . $GLOBALS['yp']->table('goods_attr') . " WHERE goods_id='$goods_id' AND " . db_create_in($attr, 'goods_attr_id');
...
{
	$sql = 'SELECT img_id, img_original, img_url, thumb_url, img_desc' . ' FROM ' . $GLOBALS['yp']->table('goods_gallery') . " WHERE goods_id = '$goods_id' and goods_attr_id='$goods_attr_id' LIMIT " . $GLOBALS['_CFG']['goods_gallery_number'];
	$row = $GLOBALS['db']->getAll($sql);
	if(count($row) == 0)
	{
		$sql = 'SELECT img_id, img_original, img_url, thumb_url, img_desc' . ' FROM ' . $GLOBALS['yp']->table('goods_gallery') . " WHERE goods_id = '$goods_id' and goods_attr_id='0' LIMIT " . $GLOBALS['_CFG']['goods_gallery_number'];
		$row = $GLOBALS['db']->getAll($sql);
	}
	/* 格式化相册图片路径 */
...
function pre_sale_info($pre_sale_id, $current_num = 0)
{
	...
	// 获取商品描述
	$pre_sale['goods_desc'] = $GLOBALS['db']->getOne("select goods_desc from ".$GLOBALS['yp']->table('goods')." where goods_id=".$pre_sale['goods_id']);
	$pre_sale['cur_price'] = $cur_price;
...
{
	$sql = "select count(*) from " . $GLOBALS['yp']->table('comment') . " where id_value = '$goods_id' and status = 0 and parent_id = 0";
