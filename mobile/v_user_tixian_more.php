...
if(isset($_REQUEST['act']) && $_REQUEST['act'] == 'act_save')
{
	...
	if($tixian)
	{
		if($id)
		{
			$GLOBALS['db']->autoExecute($GLOBALS['yp']->table('user_deposit'), $tixian, 'UPDATE','id = '.$id);
			...
			{
				yp_header("Location: v_user_tixian.php\n");
				exit; 
			}
		}
		else
		{
			$GLOBALS['db']->autoExecute($GLOBALS['yp']->table('user_deposit'), $tixian, 'INSERT');
			$error_no = $GLOBALS['db']->errno();
			if ($error_no > 0)
			{
				show_message($GLOBALS['db']->errorMsg());
...
if(isset($_REQUEST['act']) && $_REQUEST['act'] == 'del')
{
	...
	$id = intval($_REQUEST['id']);
	$sql = "DELETE FROM " .$GLOBALS['yp']->table('user_deposit') . " WHERE id = '$id' AND user_id = '" . $_SESSION['user_id'] . "'";
...
if(isset($_REQUEST['act']) && $_REQUEST['act'] == 'act_deposit')
{
	...
	{
		 $sql = "SELECT * FROM " . $GLOBALS['yp']->table('user_deposit') . " WHERE id = '$id' AND user_id = '" . $_SESSION['user_id'] . "'";
		 ...
		 if($rows)
		 {
			 ...
			 $sql = "INSERT INTO " . $GLOBALS['yp']->table('deposit') . 			 "(`deposit_money`,`real_name`,`account_name`,`bank_account`,`phone`,`remark`,`add_time`,`user_id`,`status`) ".
...
{
	 $sql = "SELECT * FROM " . $GLOBALS['yp']->table('user_deposit') . " WHERE id = '$id' AND user_id = '$user_id'";
