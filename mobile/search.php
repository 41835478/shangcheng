...
else
{
    ...
    if(isset($_REQUEST['type']) && $_REQUEST['type'] == '1'){
    	...
    	$sql = "SELECT DISTINCT supplier_id
				FROM ".$yp->table('supplier_shop_config')." AS ssc
				WHERE (
				code = 'shop_name'
    	...
    	$arr = array();
    	if(!empty($suppliers)){
	    	$sql = "select supplier_id,code,value from ".$yp->table('supplier_shop_config')." where supplier_id in(".implode(',',$suppliers).")";
	    	$res = $info = $db->query($sql);
	    	
	    	while ($row = $db->FetchRow($res)){
	    		if(!isset($arr[$row['supplier_id']])){
	    			//获取店铺三个最新商品
	    			$gsql = "select goods_id, goods_name,shop_price, goods_thumb, goods_img from ".$yp->table('goods')." where supplier_id=".$row['supplier_id']." AND is_delete = 0 AND is_on_sale = 1 AND is_alone_sale = 1 AND is_virtual = 0 order by goods_id desc";
    ...
    if (!empty($_REQUEST['keywords']))
    {
        ...
        foreach ($arr AS $key => $val)
        {
            ...
            $db->autoReplace($yp->table('keywords'), array('date' => local_date('Y-m-d'),
                'searchengine' => 'jtypmall', 'keyword' => addslashes(str_replace('%', '', $val)), 'count' => 1), array('count' => 1));
        }
        $keywords .= ')';
	...
	$sort  = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array('goods_id', 'final_price', 'last_update', 'salenum'))) ? trim($_REQUEST['sort'])  : $default_sort_order_type;   /* 代码增加_start  By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121 */
