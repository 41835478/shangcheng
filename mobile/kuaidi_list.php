...
if($order_id > 0)
{
	$sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('order_info') . " WHERE order_id = '$order_id' AND user_id = '$user_id'";
	$count = $GLOBALS['db']->getOne($sql);
        if($count > 0)
	{
//		 $sql = "SELECT order_id,shipping_name, order_sn, shipping_status, invoice_no FROM " . $GLOBALS['yp']->table('order_info') . " WHERE order_id = '$order_id'";
