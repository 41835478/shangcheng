...
$u_id = isset($_REQUEST['u']) ? intval($_REQUEST['u']) : 0;
...
$cache_id = sprintf('%X', crc32($id . '-' . $_CFG['lang']));
...
if (!$smarty->is_cached('tuiguang.dwt', $cache_id))
{
	...
	if(!empty($user_id)){
		...
		$fenxiao_sql = "SELECT is_fenxiao FROM " . $GLOBALS['yp']->table('users') . " WHERE user_id = '$user_id'";
