...
$sql = "SELECT order_id FROM " . $GLOBALS['yp']->table('pay_log') . " WHERE log_id = '$out_trade_no'";
$order_id = $GLOBALS['db']->getOne($sql);

$sql = "SELECT * FROM " . $GLOBALS['yp']->table('pay_log') . " WHERE log_id = '$out_trade_no'";
$pay_log = $GLOBALS['db']->getRow($sql);

$order = $db->getRow("SELECT * FROM " . $yp->table('user_account') . " WHERE id = '$order_id'");

if ($order['amount'] > 0){

	$order['pay_id'] = $GLOBALS['db']->getOne("select pay_id from ".$GLOBALS['yp']->table('payment')." where pay_name = '".$order['payment']."'");
