 ...
 * $Id: virtual_group.php 17217 2016-10-30 06:29:08Z prince $
*/
...
$sql = 'select region_id from '.$GLOBALS['yp']->table("region")." where region_name = '$cityIpArr[city]'";
$city_id = $GLOBALS['db']->getOne($sql);
if($city_id){
    $sql = "select city from ".$GLOBALS['yp']->table("virtual_goods_district")." where city = $city_id";
...
{ 
    ...
    $smarty->assign('nowcityname',$GLOBALS['db']->getOne("select region_name from ".$GLOBALS['yp']->table("region")." where region_id=".$city_id));
...
if ($_REQUEST['act'] == 'setcity'){
	...
	$sql = "select region_id,region_name,parent_id from ". $GLOBALS['yp']->table('region') ." where region_id= ".$id." and region_type=2";
...
function get_city_list(){
     $sql = "select r.* from ".$GLOBALS['yp']->table('virtual_goods_district')." as ssr left join ".$GLOBALS['yp']->table('region')." as r on ssr.city=r.region_id where ssr.city>0 group by ssr.city";
...
function get_county_list($city_id){
	...
	$sql = "select distinct r.region_id,r.region_name from ". $GLOBALS['yp']->table("virtual_goods_district") ." as ssr left join ". $GLOBALS['yp']->table("region") ." as r on ssr.county = r.region_id where ssr.is_show = '1' and ssr.city = '".$city_id."'";
...
function get_district_list($county_id){
	...
	$sql = "select distinct district_id,district_name from ". $GLOBALS['yp']->table("virtual_goods_district")." where county = '".$county_id."' and is_show = '1' order by sort asc";
	$res = $GLOBALS['db'] ->getAll($sql);
	$regions =array();
	foreach($res as $k=>$v){
		$regions[0]['name'] = '全部';
...
//    }
    if($cat_id != 0){
        ...
        $sql = "select distinct cat_id  from ".$GLOBALS['yp']->table("category")." where parent_id=$cat_id";
    ...
    if(empty($goods_ids_array)){
        ...
        $sql = "select count(distinct supplier_id) from ".$GLOBALS['yp']->table("goods")." where is_on_sale='1' and is_delete = '0' and is_real='0' and extension_code = 'virtual_good' ".$wherecat."  and goods_id in (".implode(',',$goods_ids_array).")";
    ...
    //$sql = "select * from ".$GLOBALS['yp']->table("goods")." where is_delete = '0' and is_real='0' and extension_code = 'virtual_good' ".$wherecat."  and goods_id in (".implode(',',$goods_ids_array).") ORDER BY $sort $order";
    $sql = "select distinct g.supplier_id, ss.supplier_name, ss.supplier_title, ss.supplier_desc, ss.supplier_tags, ss.logo from ".$GLOBALS['yp']->table("goods").
            " as g left join ".$GLOBALS['yp']->table('supplier_street')." as ss on g.supplier_id=ss.supplier_id "
    ...
    while($street_list = $GLOBALS['db']->fetchRow($res))
        {
          $sql= "select supplier_id,supplier_name,address from ".$GLOBALS['yp']->table("supplier")." where supplier_id = ".$street_list['supplier_id'];
...
//    }
  //  /今/天/优/品/多/商/户/系/统  mod on 20160801 by prince  qq 1/2/0/0/2/9/1/2/1
    ...
    $sql = "select distinct district_id from ".$GLOBALS['yp']->table("virtual_goods_district")." where 1 ".$where;
    ...
    if(empty($goods_ids_array)){
        ...
        $sql = "select count(*) from ".$GLOBALS['yp']->table("goods")." where is_on_sale='1' and is_delete = '0' and is_real='0' and extension_code = 'virtual_good' ".$wherecat."  and goods_id in (".implode(',',$goods_ids_array).")";
        $count =  $GLOBALS['db'] ->getOne($sql);
    ...
    while($goods_list = $GLOBALS['db']->fetchRow($res))
        {
          ...
          $sql= "select supplier_id,supplier_name from ".$GLOBALS['yp']->table("supplier")." where supplier_id = ".$goods_list['supplier_id'];
          $supplier = $GLOBALS['db'] ->getRow($sql);
...
function get_virtual_category(){
    $sql = "select * from ".$GLOBALS['yp']->table("category")." where parent_id = '0' and is_virtual = '1' and is_show = '1' order by sort_order";
...
function get_virtual_category_chr($id){
    $sql = "select * from ".$GLOBALS['yp']->table("category")." where parent_id = '$id' and is_virtual = '1' and is_show = '1' order by sort_order";
    $category = $GLOBALS['db'] ->getAll($sql);
    $category_list = array();
  //  $category_list[0]['cat_name'] = '全部';
  //  $category_list[0]['cat_id'] = 0;
