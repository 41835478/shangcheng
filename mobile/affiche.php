...
if ($_GET['act'] == 'js')
{
    ...
    $sql = 'SELECT ad.ad_id, ad.ad_name, ad.ad_link, ad.ad_code '.
           'FROM ' . $yp->table('ypmart_ad') . ' AS ad ' .
           'LEFT JOIN ' . $yp->table('ypmart_ad_position') . ' AS p ON ad.position_id = p.position_id '.
...
else
{
    ...
    if ($ad_id == '-1')
    {
        ...
        $db->query($sql);
        //$db->autoReplace($yp->table('adsense'), array('from_ad' => -1, 'referer' => $site_name, 'clicks' => 1), array('clicks' => 1));
    ...
    {
        /* 更新站内广告的点击次数 */
        $db->query('UPDATE ' . $yp->table('ypmart_ad') . " SET click_count = click_count + 1 WHERE ad_id = '$ad_id'");
        ...
        $sql="SELECT * FROM ". $yp->table('ypmart_ad') ." WHERE ad_id = '$ad_id'";
        $ad_info=$db->getRow($sql);
        /* 跳转到广告的链接页面 */
        if (!empty($ad_info['ad_link']))
        {
            $uri = (strpos($ad_info['ad_link'], 'http://') === false && strpos($ad_info['ad_link'], 'https://') === false ) ? $yp->http() . urldecode($ad_info['ad_link']) : urldecode($ad_info['ad_link']);
        }
        else
        {
            $uri = $yp->url();
