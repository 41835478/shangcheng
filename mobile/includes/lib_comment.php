...
function get_my_comments($goods_id, $type = 0, $page = 1, $c_tag='')
{
	...
	if ($type != 4)
	{
		...
		$tag_name_c = $GLOBALS['db']->getOne("select tag_id from " . $GLOBALS['yp']->table('goods_tag') . " where goods_id = " . $goods_id . " and tag_name = '" . $c_tag . "'");
		if ($tag_name_c)
		{
			$where .= " AND FIND_IN_SET('$tag_name_c',comment_tag)";			
		}

		$count = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM ".$GLOBALS['yp']->table('comment')." AS c 
										 LEFT JOIN ".$GLOBALS['yp']->table('shaidan')." AS s ON c.rec_id=s.rec_id
		...
		$sql = "SELECT c.*, u.headimg, s.shaidan_id, s.status AS shaidan_status,s.title,s.message FROM ".$GLOBALS['yp']->table('comment')." AS c 
				LEFT JOIN ".$GLOBALS['yp']->table('users')." AS u ON c.user_id=u.user_id
				LEFT JOIN ".$GLOBALS['yp']->table('shaidan')." AS s ON c.rec_id=s.rec_id
		...
		while ($row = $GLOBALS['db']->fetchRow($res))
		{
			...
			{
				$row['shaidan_imgs'] = $GLOBALS['db']->getAll("SELECT * FROM ".$GLOBALS['yp']->table('shaidan_img')." WHERE shaidan_id = '$row[shaidan_id]'");	
			...
			$parent_res = $GLOBALS['db']->getAll("SELECT * FROM ".$GLOBALS['yp']->table('comment')." WHERE parent_id = '$row[comment_id]'");	
...
//		
	...
	{
		$count = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM ".$GLOBALS['yp']->table('shaidan')." AS s 
										 WHERE s.goods_id = '$goods_id' AND s.status = 1");
		$size  = 10;
		$page_count = ($count > 0) ? intval(ceil($count / $size)) : 1;
	
		$sql = "SELECT s.*, u.user_name, c.comment_tag, c.comment_rank, c.comment_id FROM ".$GLOBALS['yp']->table('shaidan')." AS s 
				LEFT JOIN ".$GLOBALS['yp']->table('users')." AS u ON s.user_id=u.user_id
				LEFT JOIN ".$GLOBALS['yp']->table('comment')." AS c ON c.rec_id=s.rec_id
				WHERE s.goods_id = '$goods_id' AND s.status = 1 ORDER BY s.add_time DESC";
		$res = $GLOBALS['db']->selectLimit($sql, $size, ($page-1) * $size);
		$points_list = array();
		while ($row = $GLOBALS['db']->fetchRow($res))
		{
			$row['buy_time'] = $GLOBALS['db']->getOne("SELECT o.add_time FROM ".$GLOBALS['yp']->table('order_info')." AS o
...
function get_user_rank($user_id)
{
	...
	{
		$infos = $GLOBALS['db']->getRow("select * from ".$GLOBALS['yp']->table('users')." where user_id='$user_id'");
		if ($infos['user_rank'] > 0)
		{
			$sql = "SELECT rank_id, rank_name, discount FROM ".$GLOBALS['yp']->table('user_rank') . " WHERE rank_id = '$infos[user_rank]'";
		}
		else
		{
			$sql = "SELECT rank_id, rank_name, discount, min_points FROM ".$GLOBALS['yp']->table('user_rank') .
