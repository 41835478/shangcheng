...
{//var_dump($order_id);
	if ($supplier_id > 0) {
     ...
     $sql = 'SELECT value FROM ' . $GLOBALS['yp']->table('ypmart_shop_config')." WHERE code = 'affiliate'";
    $affiliatearr = $GLOBALS['db']->getOne($sql);
	}
    ...
    $oid = intval($order_id);
	//获取订单分成金额
	$split_money = get_split_money_by_orderid($oid , $supplier_id);

    $row = $GLOBALS['db']->getRow("SELECT o.order_sn,u.parent_id, o.is_separate,(o.goods_amount - o.discount) AS goods_amount, o.user_id FROM " . $GLOBALS['yp']->table('order_info') . " o"." LEFT JOIN " . $GLOBALS['yp']->table('users') . " u ON o.user_id = u.user_id"." WHERE order_id = '$oid'");
...
//获取某一个订单的分成金额
function get_split_money_by_orderid($order_id , $supplier_id)
{
     if ($supplier_id > 0) {
