 ...
 * ============================================================================
 * $Author: prince $
 * $Id: lib_insert.php 17217 2017-04-01 06:29:08Z prince $
*/
...
function insert_query_info()
{
    ...
    $online_count = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('sessions'));
...
function insert_history()
{
    ...
    if (!empty($_COOKIE['YP']['history']))
    {
        ...
        $sql   = 'SELECT goods_id, goods_name, goods_thumb, shop_price FROM ' . $GLOBALS['yp']->table('goods') .
...
{
$where = db_create_in($_COOKIE['YP']['history'], 'goods_id');
        $sql   = 'SELECT cat_id FROM ' . $GLOBALS['yp']->table('goods') .
...
$sql = "select * from ".$GLOBALS['yp']->table('goods')." where cat_id = ".$catid." and is_best = 1 ";
...
{
$sql = "SELECT count(*) FROM " . $GLOBALS['yp']->table('comment') . " WHERE status=1 and  comment_type =0 and id_value =".$goods_id ;//status=1表示通过了的评论才算  comment_type =0表示针对商品的评价 感谢zhangyh的提醒
...
function insert_history_list()
{
    ...
    {
        $where = db_create_in($_COOKIE['YP']['history'], 'goods_id');
        $sql   = 'SELECT goods_id, goods_name, goods_thumb, shop_price, goods_type FROM ' . $GLOBALS['yp']->table('goods') .
                " WHERE $where AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0";
        $query = $GLOBALS['db']->query($sql);
        $res = array();
		$str .= '<ul>';
...
{ $arr['num'] = intval($arr['num']); $arr['id'] = intval($arr['id']);
    ...
    if (!empty($arr['num']) && $arr['num'] != 1)
    {
        $sql  = 'SELECT a.ad_id, a.position_id, a.media_type, a.ad_link, a.ad_code, a.ad_name, p.ad_width, ' .
                ...
                'FROM ' . $GLOBALS['yp']->table('ypmart_ad') . ' AS a '.
                'LEFT JOIN ' . $GLOBALS['yp']->table('ypmart_ad_position') . ' AS p ON a.position_id = p.position_id ' .
...
function insert_member_info()
{
    ...
    {
        if (!empty($_COOKIE['YP']['username']))
        {
            $GLOBALS['smarty']->assign('yp_username', stripslashes($_COOKIE['YP']['username']));
        }
...
{ $arr['id'] = intval($arr['id']);
    ...
    $sql = 'SELECT u.user_name, og.goods_number, oi.add_time, IF(oi.order_status IN (2, 3, 4), 0, 1) AS order_status ' .
           'FROM ' . $GLOBALS['yp']->table('order_info') . ' AS oi LEFT JOIN ' . $GLOBALS['yp']->table('users') . ' AS u ON oi.user_id = u.user_id, ' . $GLOBALS['yp']->table('order_goods') . ' AS og ' .
...
function insert_apply_supplier($user){
    ...
    $sql = "select * from ".$GLOBALS['yp']->table('supplier')." where user_id = ".$user['id'].' limit 1';
    ...
    if($shownum == 1){
		...
		$sql="select str_id,str_name from ". $GLOBALS['yp']->table('street_category') ." where is_show=1 order by sort_order";
		...
		$GLOBALS['smarty']->assign('supplier_country', $supplier_country);
    	/* 供货商等级 */
		$sql="select rank_name from ". $GLOBALS['yp']->table('supplier_rank') ." where rank_id = ".$supplier['rank_id'];
		$rank_name=$GLOBALS['db']->getOne($sql);
		$supplier['rank_name'] = $rank_name;
		/* 店铺类型 */
		 $sql="select str_name from ". $GLOBALS['yp']->table('street_category') ." where str_id = ".$supplier['type_id'];
		$type_name=$GLOBALS['db']->getOne($sql);
		$supplier['type_name'] = $type_name;
		
		$GLOBALS['smarty']->assign('mydomain', $GLOBALS['yp']->url());
    }
...
function insert_goods_sells($arr)
{
    $sql = 'SELECT SUM(goods_number) AS number ' .
           ' FROM ' . $GLOBALS['yp']->table('order_goods') ." AS og , " . $GLOBALS['yp']->table('order_info') ." AS  o ".
...
function insert_supplier_list(){
	...
	$sql = "SELECT ssc.value,ssc.supplier_id,ssc.code FROM ".$GLOBALS['yp']->table('supplier')." as s,".$GLOBALS['yp']->table('supplier_shop_config')." as ssc WHERE s.`status` = 1 AND ssc.supplier_id = s.supplier_id AND code in('shop_name','shop_logo') limit 36";
    $query = $GLOBALS['db']->query($sql);
    $ret = array();
    while ($row = $GLOBALS['db']->fetch_array($query)){
...
function insert_get_shop_shipping($arr){
	...
	if(count($shipping_list)>0){
		...
		$sql = 'SELECT count(*) FROM ' . $yp->table('cart') . " WHERE $sql_where AND `extension_code` != 'package_buy' AND `is_shipping` = 0 ".$sql_plus; //jx
...
function get_children_tree($cat_id)
{
     if ($cat_id >0 )
    {
        $sql = 'SELECT count(*) FROM ' . $GLOBALS['yp']->table('category') . " WHERE parent_id = '$cat_id'";
        ...
        if ($GLOBALS['db']->getOne($sql))
        {
            ...
            $sql = 'SELECT a.cat_id, a.cat_name, a.sort_order AS parent_order, a.cat_id, ' .
                ...
                'LEFT JOIN ' . $GLOBALS['yp']->table('category') . ' AS b ON b.parent_id = a.cat_id ' .
        ...
        else
        {
            ...
            else
            {
                ...
                $sql = 'SELECT a.cat_id, a.cat_name FROM '
                        . $GLOBALS['yp']->table('category') . ' AS a ' .
...
function insert_add_url_uid(){
    ...
    if($user_id)
    {
        ...
        {
            if(strstr($url, '?')){
                $url = HTTP_TYPE.'://'.str_replace('?', '?u='.$user_id.'&', $url);
            }
            else
            {
                $url = HTTP_TYPE.'://'.$url.'?u='.$user_id;
            }
...
function insert_share(){
	$_CFG=$GLOBALS['_CFG'];//20170606
    $weixin_info = $GLOBALS['db']->getRow("SELECT * FROM ".$GLOBALS['yp']->table('weixin_config'));
    if($weixin_info['title'] && $weixin_info['appid'] && $weixin_info['appsecret']){
        ...
        if($signPackage){
			...
			if(preg_match('/goods.php/i', $_SERVER['REQUEST_URI'])){//商品分享
				...
				$share_title = $row['goods_name'];
				
			    if( $weixin_info['is_thumbnail'] == 0){
				    if(strpos($row['goods_thumb'],'http') === false){
			    		$share_img = $web_url."/".$row['goods_thumb'];
					}else{
			    		$share_img = $row['goods_thumb'];
					}// prince qq 120-029-121 2016115				
        		}else{
					$share_img =  $web_url."/".$weixin_info['weixin_logo'];
				}
			}elseif(preg_match('/tuiguang.php/i', $_SERVER['REQUEST_URI']) || preg_match('/article.php/i', $_SERVER['REQUEST_URI'])){//单品推广/文章分享
			
			    $article_id = intval($_GET['id']);
			
			    $sql = "select * from ". $GLOBALS['yp']->table('article') ." where article_id=".$article_id;
				...
				$share_title = $row['title'];//标题
				
			  
			    if( $weixin_info['is_thumbnail'] == 0){
			    	if(strpos($row['file_url'],'http') === false){
			    		$share_img = $web_url."/".$row['file_url'];
					}else{
			    		$share_img = $row['file_url'];
					}	
			    }else{
					$share_img =  $web_url."/".$weixin_info['weixin_logo']; 
		        }
			...
			}elseif(preg_match('/cut.php/i', $_SERVER['REQUEST_URI'])){
			    $sql = "select value from ". $GLOBALS['yp']->table('ypmart_shop_config') ." where code= 'shop_name'";
	            $share_title = $GLOBALS['db']->getOne($sql);
				$sharemsg =  '全民砍价，0元起，快来参加吧';
				$share_img =  $web_url."/".$weixin_info['weixin_logo'];
			}elseif(preg_match('/extpintuan.php/i', $_SERVER['REQUEST_URI'])){
			    $sql = "select value from ". $GLOBALS['yp']->table('ypmart_shop_config') ." where code= 'shop_name'";
	            $share_title = $GLOBALS['db']->getOne($sql);
				$sharemsg =  '全民拼团，超低价格，快来参加吧';
				$share_img =  $web_url."/".$weixin_info['weixin_logo'];
			}elseif(preg_match('/order_raise.php/i', $_SERVER['REQUEST_URI'])){
			    $sql = "select value from ". $GLOBALS['yp']->table('ypmart_shop_config') ." where code= 'shop_name'";
	            $shop_name = $GLOBALS['db']->getOne($sql);
                $share_title = '您朋友喊您帮Ta付款啦！';
				$sharemsg = '刚刚在'.$shop_name.'购买了几件商品，很是喜欢。可惜兜兜里钱不多，土豪！帮我付点吧？！';
				$share_img =  $web_url."/".$weixin_info['weixin_logo'];
			}elseif(preg_match('/lucky_buy.php/i', $_SERVER['REQUEST_URI'])){
			    $sql = "select value from ". $GLOBALS['yp']->table('ypmart_shop_config') ." where code= 'shop_name'";
	            $share_title = $GLOBALS['db']->getOne($sql);
				$sharemsg =  '只需一元即可获得心仪商品，快来参加吧';
				$share_img =  $web_url."/".$weixin_info['weixin_logo'];
			}else{
			...
			$GLOBALS['smarty']->assign('share_title',           $share_title);
