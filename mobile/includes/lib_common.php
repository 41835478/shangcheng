...
function load_config()
{
    ...
    if ($data === false)
    {
        $sql = 'SELECT code, value FROM ' . $GLOBALS['yp']->table('ypmart_shop_config') . ' WHERE parent_id > 0';
        $res = $GLOBALS['db']->getAll($sql);
...
function get_promotion_info($goods_id = '',$suppid=-1)
{
    ...
    $favourable = array();
    $sql = 'SELECT act_id, act_range, act_range_ext, act_name, start_time, end_time FROM ' . $GLOBALS['yp']->table('favourable_activity') . " WHERE start_time <= '$gmtime' AND end_time >= '$gmtime' $where_suppid";
    if(!empty($goods_id))
    {
    ...
    else
    {
        ...
        foreach ($res as $rows)
        {
            ...
            elseif ($rows['act_range'] == FAR_CATEGORY)
            {
                ...
                /* 代码修改_BY qq120029121 STAR_20151215 商品为店铺商品时优惠活动按照店铺内分类筛选 */
                if($suppid>0){
                        $flag = false;
                        $ids = $rows['act_range_ext'];
                        $sql = "select cat_id from ".$GLOBALS['yp']->table('supplier_goods_cat')." where goods_id = $goods_id and supplier_id = $suppid";
                ...
                /* 代码修改_BY qq120029121 END_20151215 商品为店铺商品时优惠活动按照店铺内分类筛选 */        
                }else{
...
function is_customers($cus_type=0,$suppid=-1){
    $sql="select * from " . $GLOBALS['yp']->table('chat_customer') . " WHERE supp_id=0 and cus_name!='' and user_id!='' and of_username!='' ";
    if($suppid>0)   {
        $sql="select * from " . $GLOBALS['yp']->table('chat_customer') . " WHERE supp_id='$supp_id' and cus_name!='' and user_id!='' and of_username!=''  ";
    }
	$res=$GLOBALS['db']->getAll($sql);
    return $res;//20170905 云旺 add by prince
}
...
function is_guanzhu($supplier_id){
   $sql = "select id from ".$GLOBALS['yp']->table('supplier_guanzhu')." where supplierid = $supplier_id and userid ='".$SESSION['user_id']."'";//20170225 修复非登陆用户打开商家手机端首页报错的问题 prince 120029121
...
function get_pc_url(){
   ...
   $sql = "select value from ".$yp->table('shop_config')." where code = 'shop_url' ";//20170414
   $res = $db->getOne($sql);
   if(!$res){
       $res = './..';
   }else{
...
function get_pro_price($spec,$goods_id,$org_price=0)
{
  ...
  $user_rank = isset($GLOBALS['tongbu_user_rank']) ? $GLOBALS['tongbu_user_rank'] : $_SESSION['user_rank'];//寒冰 20180918   增加货品加个按照会员等级折扣
    if($user_rank){
	...
	if($pro_price > 0){
			return $pro_price*$_SESSION['discount'];
		}else{
			return $org_price;
        }
}
...
// 坐标转换
function txmap_translate($lat, $lng){
	$translate_result=array();
    $key = "75LBZ-H4IWQ-DHP5D-GD7SQ-HTDIQ-67BNZ";
    $location = $lat.",".$lng;
	$type=1;
	$url = "http://apis.map.qq.com/ws/coord/v1/translate?locations=$location&type=$type&key=$key";
    $res = file_get_contents($url);
    $arr = json_decode($res, true);
    $translate_result['lat']= $arr['locations'][0]['lat'];
    $translate_result['lng']= $arr['locations'][0]['lng'];
