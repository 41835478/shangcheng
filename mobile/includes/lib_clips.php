...
function add_message($message)
{
	...
	$message['parent_id']=isset($message['parent_id']) ? intval($message['parent_id']) : 0;
...
function insert_user_account($surplus, $amount)
{
    $sql = 'INSERT INTO ' .$GLOBALS['yp']->table('user_account').
           ' (user_id, admin_user, amount, add_time, paid_time, admin_note, user_note, process_type, payment, is_paid,real_name,account_type,account,mobile_phone)'.
            " VALUES ('$surplus[user_id]', '', '$amount', '".gmtime()."', 0, '', '$surplus[user_note]', '$surplus[process_type]', '$surplus[payment]', 0, '$surplus[real_name]', '$surplus[account_type]', '$surplus[account]', '$surplus[mobile_phone]')";  // mod by prince 20161126
    $GLOBALS['db']->query($sql);
...
function get_account_log($user_id, $num, $start)
{
    ...
    if ($res)
    {
        while ($rows = $GLOBALS['db']->fetchRow($res))
        {
            ...
            $rows['real_name']       = $rows['real_name'];//qq 120 029 121 20161126
            $rows['account_type']       = $rows['account_type'];//qq 120 029 121 20161126
            $rows['account']       = $rows['account'];//qq 120 029 121 20161126
            $rows['mobile_phone']       = $rows['mobile_phone'];//qq 120 029 121 20161126
...
function get_comment_list($user_id, $page_size, $start)
{
    ...
    if ($to_article)
    {
        ...
        foreach ($comments as $key=>$row)
        {
            ...
            /* 获取晒单信息*/
       foreach ($comments as $key=>$row){
           $sql = "SELECT * FROM ".$GLOBALS['yp']->table('shaidan')." where goods_id = ".$row['goods_id'];
           $shaidan = $GLOBALS['db']->getAll($sql);
           foreach($shaidan as $k=>$v){
               $shaidan[$k]['shaidan_img'] = $GLOBALS['db']->getAll("SELECT * FROM ".$GLOBALS['yp']->table('shaidan_img')." WHERE shaidan_id = '$v[shaidan_id]]'"); 
...
function get_user_shu($user_id)
{
	...
	if (!empty($a_comment))
	{
		$sql = "SELECT count(*) FROM " . $GLOBALS['yp']->table('order_info') . " WHERE shipping_status = 2 AND order_id in (" . $a_comment . ") ";
...
{
    $sql = "SELECT SUM(pay_points) FROM " .$GLOBALS['yp']->table('account_log').
           " WHERE user_id = '$user_id'";
 ...
 * @return  int
 */
function get_user_rank_points($user_id)
{
    $sql = "SELECT SUM(rank_points) FROM " .$GLOBALS['yp']->table('account_log').
           " WHERE user_id = '$user_id'";
...
{
    $sql = "SELECT SUM(frozen_money) FROM " .$GLOBALS['yp']->table('account_log').
           " WHERE user_id = '$user_id'";
