...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        if ($data === false)
        {
            $sql = 'SELECT g.goods_id, g.is_best, g.is_new, g.is_hot, g.is_promote, b.brand_name,g.sort_order,g.ghost_count ' .
               ' FROM ' . $GLOBALS['yp']->table('goods') . ' AS g ' .
        ...
        $sql = 'SELECT g.goods_id, g.goods_name,g.click_count, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                ...
                "promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img, RAND() AS rnd,g.ghost_count  " .
                'FROM ' . $GLOBALS['yp']->table('goods') . ' AS g ' .
        ...
        foreach ($result AS $idx => $row)
        {
            ...
            $goods[$idx]['ghost_count']  = $row['ghost_count'];
            ...
            if(empty($row['goods_thumb'])){
                $row['goods_thumb'] = $GLOBALS['db']->getOne("SELECT thumb_url FROM ".$GLOBALS['yp']->table('goods_gallery')." WHERE goods_id=".$row['goods_id']);
            }
...
function get_goods_info($goods_id)
{
    ...
    $sql = 'SELECT g.*, c.measure_unit, b.brand_id, b.brand_name AS goods_brand, m.type_money AS bonus_money, ' .
            ...
            'LEFT JOIN ' . $GLOBALS['yp']->table('bonus_type') . ' AS m ' .
    ...
    if ($row !== false)
    {
		...
		//yyy修改end
        $row['goods_desc']   = preg_replace('/width="[56789]\d+"/', 'width="100%"', $row['goods_desc']);//yxn手机详情
        $row['goods_desc']   = preg_replace('/height="\d+"/', '', $row['goods_desc']);//yxn手机详情
        $row['goods_desc']   = preg_replace('/\sheight:\d+/', ' height:', $row['goods_desc']);//yxn手机详情height="115"width:370.0px;
        $row['goods_desc']   = preg_replace('/width:\d+/', 'width:', $row['goods_desc']);//yxn手机详情height="115"width:370.0px;
        $row['goods_desc']   = preg_replace('/width:\.0px/', '', $row['goods_desc']);//yxn手机详情height="115"width:370.0px;
...
function get_goods_properties($goods_id)
{
    ...
    $sql = "SELECT attr_group ".
            "FROM " . $GLOBALS['yp']->table('goods_type') . " AS gt, " . $GLOBALS['yp']->table('goods') . " AS g ".
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_group, a.is_linked, a.attr_type, ".
            ...
            ", g.attr_img, g.attr_thumb ". //采集 mod on 20160808 by PRINCE  qq 1 2 0 0 2 9 1 2 1修改
    ...
    foreach ($res AS $row)
    {
        ...
        else
        {
            ...
            $arr['spe'][$row['attr_id']]['values'][] = array(
                                                        ...
                                                        'attr_img'        => $row['attr_img'], //采集 mod on 20160808 by PRINCE  qq 1 2 0 0 2 9 1 2 1改
                                                        'attr_thumb'        => $row['attr_thumb'], //采集 mod on 20160808 by PRINCE  qq 1 2 0 0 2 9 1 2 1改
    ...
    if (!empty($attr))
    {
        foreach ($attr['lnk'] AS $key => $val)
        {
            ...
            $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
                    ...
                    'FROM ' . $GLOBALS['yp']->table('goods') . ' AS g ' .
                    'LEFT JOIN ' . $GLOBALS['yp']->table('goods_attr') . ' as a ON g.goods_id = a.goods_id ' .
                    "LEFT JOIN " . $GLOBALS['yp']->table('member_price') . " AS mp ".
                        "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
    ...
    /* 分类信息 */
    $sql = 'SELECT cat_name FROM ' . $GLOBALS['yp']->table('category') . " WHERE cat_id = '$cat_id'";
...
function assign_brand_goods($brand_id, $num = 0, $cat_id = 0,$order_rule = '')
{
    ...
    /* 分类信息 */
    $sql = 'SELECT brand_name FROM ' . $GLOBALS['yp']->table('brand') . " WHERE brand_id = '$brand_id'";
...
function get_extension_goods($cats)
{
    ...
    $sql = 'SELECT goods_id FROM ' . $GLOBALS['yp']->table('goods_cat') . " AS g WHERE $cats";
...
function auction_info($act_id, $config = false)
{
    ...
    /* 查询出价用户数和最后出价 */
    $sql = "SELECT COUNT(DISTINCT bid_user) FROM " . $GLOBALS['yp']->table('auction_log') .
            " WHERE act_id = '$act_id'";
    $auction['bid_user_count'] = $GLOBALS['db']->getOne($sql);
    if ($auction['bid_user_count'] > 0)
    {
        $sql = "SELECT a.*, u.user_name " .
                "FROM " . $GLOBALS['yp']->table('auction_log') . " AS a, " .
...
{
    $sql = "SELECT * FROM " . $GLOBALS['yp']->table('favourable_activity') .
            " WHERE act_id = '$act_id'";
    $row = $GLOBALS['db']->getRow($sql);
    if (!empty($row))
    {
...
{
    $sql = "SELECT * FROM " . $GLOBALS['yp']->table('wholesale') .
            " WHERE act_id = '$act_id'";
    $row = $GLOBALS['db']->getRow($sql);
    if (!empty($row))
    {
...
function get_goods_attr($goods_id)
{
    ...
    $sql = "SELECT a.attr_id, a.attr_name " .
            "FROM " . $GLOBALS['yp']->table('goods') . " AS g, " . $GLOBALS['yp']->table('attribute') . " AS a " .
            "WHERE g.goods_id = '$goods_id' " .
...
function get_goods_fittings($goods_list = array())
{
    ...
    $sql = 'SELECT gg.parent_id, ggg.goods_name AS parent_name, gg.goods_id, gg.goods_price, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
            ...
            'LEFT JOIN ' . $GLOBALS['yp']->table('goods') . 'AS g ON g.goods_id = gg.goods_id ' .
            "LEFT JOIN " . $GLOBALS['yp']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = gg.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            "LEFT JOIN " . $GLOBALS['yp']->table('goods') . " AS ggg ON ggg.goods_id = gg.parent_id ".
...
function get_products_info($goods_id, $spec_goods_attr_id)
{
    ...
    if(isset($goods_attr_array['sort']))
    {
        ...
        $sql = "SELECT * FROM " .$GLOBALS['yp']->table('products'). " WHERE goods_id = '$goods_id' AND goods_attr = '$goods_attr' LIMIT 0, 1";
...
{
    $sql = "SELECT ghost_count FROM ".$GLOBALS['yp']->table('goods')." where goods_id='".$goods_id."'";
    $ghost_count = $GLOBALS['db']->getOne($sql);

     $sql= "select sum(goods_number) as count from ".$GLOBALS['yp']->table('order_goods')."where goods_id ='".$goods_id."'";
...
{
$sql = "SELECT count(*) FROM " . $GLOBALS['yp']->table('comment') . " WHERE status=1 and  comment_type =0 and id_value =".$goods_id ;//status=1表示通过了的评论才算  comment_type =0表示针对商品的评价
...
'FROM ' . $GLOBALS ['yp']->table ( 'goods' ) . ' AS g ' . 
'LEFT JOIN ' . $GLOBALS ['yp']->table ( 'category' ) . ' AS c ON c.cat_id = g.cat_id ' . 
"LEFT JOIN " . $GLOBALS ['yp']->table ( 'member_price' ) . " AS mp " . 
"ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " . 
"Where g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 " . 
 ...
 * @param type $goods_id
 * @return type
 * 代码增加_end  By   demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1    ----20161030
 */
function is_virtual_sale_goods($goods_id){
    $sql = "select goods_id from ". $GLOBALS['yp']->table('goods') ." where is_virtual=1 and goods_id = '$goods_id'";
