 ...
 * @return  int
 */
function cut_count($supplier_id)
{
    $now = gmtime();
    $sql = "SELECT COUNT(*) " .
            "FROM " . $GLOBALS['yp']->table('goods_activity') .
            "WHERE act_type = '" . GAT_CUT . "' " .
            "AND start_time <= '$now' AND supplier_id = '$supplier_id' AND end_time >= '$now' AND is_finished < 2";
 ...
 * @return  array
 */
function cut_list($size, $page ,$supplier_id)
{
    $cut_list = array();
    ...
    $sql = "SELECT a.*,g.*, IFNULL(g.goods_thumb, '') AS goods_thumb " .
            ...
            "AND a.start_time <= '$now' AND a.supplier_id = '$supplier_id' AND a.end_time >= '$now' AND a.is_finished < 2 ORDER BY a.act_id DESC";
    //$res = $GLOBALS['db']->selectLimit($sql, $size, ($page - 1) * $size);
...
function get_cutlist()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('goods_activity') .
               " WHERE supplier_id = ".$supplier_id." AND act_type =" . GAT_CUT . $where;//20161114   寒冰    qq  3094  85552    商家支持砍价
