...
function save_validate_record ($key, $code, $type, $last_send_time, $expired_time, $ext_info = array())
{
	...
	if(! $exist)
	{
		...
		/* insert */
		$GLOBALS['db']->autoExecute($GLOBALS['yp']->table('validate_record'), $record, 'INSERT');
	}
	else
	{
		/* update */
		$GLOBALS['db']->autoExecute($GLOBALS['yp']->table('validate_record'), $record, 'UPDATE', "record_key = '$key'");
	}
}
...
{
	$sql = "select count(*) from " . $GLOBALS['yp']->table('validate_record') . " where record_key = '" . $key . "'";
...
{
	$current_time = time();
	$sql = "delete from " . $GLOBALS['yp']->table('validate_record') . " where expired_time < '$current_time'";
...
{
	$sql = "delete from " . $GLOBALS['yp']->table('validate_record') . " where record_key = '$key'";
	return $GLOBALS['db']->query($sql);
}
...
{
	$sql = "select * from " . $GLOBALS['yp']->table('validate_record') . " where record_key = '$key'";
