...
/**
 * QQ120029121 用户帐号相关函数库
 * ============================================================================
 * 演示地址: http://demo.coolhong.com  开发QQ:120029121    309485552
 * ============================================================================
 * $Author: prince $
 * $Id: lib_passport.php 17217 2017-04-01 06:29:08Z prince $
*/
...
function _register ($username, $password, $email_or_mobile, $other = array(), $register_type = 'email')
{
	...
	else
	{
		...
		if(isset($affiliate['on']) && $affiliate['on'] == 1)
		{
			...
			if($up_uid)
			{
				if(! empty($affiliate['config']['level_register_all']))
				{
					...
					{
						$rank_points = $GLOBALS['db']->getOne("SELECT rank_points FROM " . $GLOBALS['yp']->table('users') . " WHERE user_id = '$up_uid'");
				...
				// 设置推荐人
				$sql = 'UPDATE ' . $GLOBALS['yp']->table('users') . ' SET parent_id = ' . $up_uid . ' WHERE user_id = ' . $_SESSION['user_id'];
		...
		$GLOBALS['db']->autoExecute($GLOBALS['yp']->table('users'), $update_data, 'UPDATE', 'user_id = ' . $_SESSION['user_id']);
...
function register($username, $password, $email, $other = array())
{
    ...
    else
    {
        ...
        {
            //log_account_change($_SESSION['user_id'], 0, 0, $GLOBALS['_CFG']['register_points'], $GLOBALS['_CFG']['register_points'], $GLOBALS['_LANG']['register_points']);
            log_account_change($_SESSION['user_id'], 0, 0, $GLOBALS['_CFG']['register_points'], $GLOBALS['_CFG']['register_points'], '注册送积分'); // q q 12 00 29 121   p r in ce 
        }
...
function check_userinfo($user_name, $email)
{
    ...
    {
        yp_header("Location: user.php?act=get_password\n");
...
function send_pwd_email($uid, $user_name, $email, $code)
{
    ...
    $reset_email = $GLOBALS['yp']->url() . 'user.php?act=get_password&uid=' . $uid . '&code=' . $code;
...
function send_regiter_hash ($user_id)
{
    ...
    $validate_email = $GLOBALS['yp']->url() . 'user.php?act=validate_email&hash=' . $hash;
...
{
    $res = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('admin_user') .
