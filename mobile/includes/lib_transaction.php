...
/**
 * QQ120029121 用户交易相关函数库
 * ============================================================================
 * 演示地址: http://demo.coolhong.com  开发QQ:120029121    309485552
 * ============================================================================
 * $Author: prince $
 * $Id: lib_transaction.php 17217 2017-04-01 06:29:08Z prince $
*/
...
function edit_profile($profile)
{
    ...
    if(!empty($profile['user_name'])){
        $user_id_exist = $GLOBALS['db']->getOne("select user_id from ". $GLOBALS['yp']->table('users') ." where user_id <> '".$profile['user_id']."' AND user_name = '".$profile['user_name']."'");
        if($user_id_exist){
    ...
    $cfg = array();
    $cfg['username'] = $GLOBALS['db']->getOne("SELECT user_name FROM " . $GLOBALS['yp']->table('users') . " WHERE user_id='" . $profile['user_id'] . "'");
    ...
    {
        $GLOBALS['db']->autoExecute($GLOBALS['yp']->table('users'), $profile['other'], 'UPDATE', "user_id = '$profile[user_id]'");
    }
...
function get_profile($user_id)
{
    ...
    $infos = array();
    $sql  = "SELECT * ".
           "FROM " .$GLOBALS['yp']->table('users') . " WHERE user_id = '$user_id'";
    ...
    {
        $sql = "SELECT rank_id, rank_name, discount FROM ".$GLOBALS['yp']->table('user_rank') .
               " WHERE rank_id = '$infos[user_rank]'";
    }
    else
    {
        $sql = "SELECT rank_id, rank_name, discount, min_points".
               " FROM ".$GLOBALS['yp']->table('user_rank') .
    ...
    $sql = "SELECT type_name, type_money ".
           "FROM " .$GLOBALS['yp']->table('bonus_type') . " AS t1, " .$GLOBALS['yp']->table('user_bonus') . " AS t2 ".
...
function add_bonus($user_id, $bouns_sn)
{
    ...
    /* 查询红包序列号是否已经存在 */
    $sql = "SELECT bonus_id, bonus_sn, user_id, bonus_type_id FROM " .$GLOBALS['yp']->table('user_bonus') .
    ...
    if ($row)
    {
        if ($row['user_id'] == 0)
        {
            ...
            $sql = "UPDATE " .$GLOBALS['yp']->table('user_bonus') . " SET user_id = '$user_id' ".
...
function get_user_orders_ajax($user_id, $limit,$where='')
{
    ...
    $sql = "SELECT o.*, ifnull(ssc.value,'网站自营') as shopname, " .
    	   ...
    	   " LEFT JOIN " .$GLOBALS['yp']->table('supplier_shop_config') . 'as ssc '.
    ...
    foreach ($res as $key=>$row)
    {
        ...
        {  
            ...
            if ($row['shipping_status'] == SS_SHIPPED)
            {
				$back_num = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('back_order') . " WHERE order_id = " . $row['order_id'] . " AND status_back < 6 AND status_back != 3");
				if ($back_num > 0)
				{
            ...
            else
            {
                ...
                {
                    @$row['handler'] = "<a href=\"user.php?act=order_detail&order_id=" .$row['order_id']. '">' .$GLOBALS['_LANG']['view_order']. '</a>';
					$zt = $GLOBALS['db']->getRow("SELECT status_back,back_id FROM " . $GLOBALS['yp']->table('back_order') . " WHERE order_id = " . $row['order_id'] . " AND back_type= 4");
				...
				}//寒冰 qq  309485552    20170315   手机端退货退款
                }
	...
	$row['order_status1'] = $row['order_status'];
        ...
        $cod_code = $GLOBALS['db']->getOne("select pay_code from " . $GLOBALS['yp']->table('payment') . " where pay_id=" . $row['pay_id']);
		$weixiu_time = $GLOBALS['db']->getOne("select value from " . $GLOBALS['yp']->table('ypmart_shop_config') . " where code='weixiu_time'");
		...
		$arr[$row['order_id']] = array('order_id'       => $row['order_id'],
						...
						'is_suborder' => $row['parent_order_id'] ? "(子订单)" : "",  //代码增加   By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121
						...
						'tk' =>           $row['tk'], //代码增加   By  demo.coolhong.com 今天优品 多商户系统
...
function get_order_goods($order)
{
    ...
    $sql = "SELECT o.*, IF(o.product_id > 0, p.product_number, g.goods_number) AS storage, o.goods_attr, o.goods_attr_id, g.suppliers_id, IFNULL(b.brand_name, '') AS brand_name, p.product_sn, a.attr_value,g.goods_thumb,g.goods_id 
            FROM " . $GLOBALS['yp']->table('order_goods') . " AS o
                ...
                LEFT JOIN " . $GLOBALS['yp']->table('brand') . " AS b
                    ON g.brand_id = b.brand_id
				LEFT JOIN " . $GLOBALS['yp']->table('goods_attr') . " AS a
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $row['goods_thumb'] = get_image_path($row['goods_id'], $row['goods_thumb'],true);// mod by qq120-029-121 20160815
	...
	{  
		$sql_goods = "select bo.*,bg.product_id from ". $GLOBALS['yp']->table('back_order') . " as bo " .
					" left join " . $GLOBALS['yp']->table('back_goods') . " as bg " .
		...
		$goods_list[$goods_key]['back_id']=$back_order['back_id'];
...
{
    /* 查询订单信息，检查状态 */
    $sql = "SELECT user_id, order_id, order_sn , surplus , integral , bonus_id, order_status, shipping_status, pay_status FROM " .$GLOBALS['yp']->table('order_info') ." WHERE order_id = '$order_id'";
...
{
    /* 查询订单信息，检查状态 */
    $sql = "SELECT user_id, order_sn , order_status, shipping_status, pay_status FROM ".$GLOBALS['yp']->table('order_info') ." WHERE order_id = '$order_id'";
    ...
    {
        $sql = "UPDATE " . $GLOBALS['yp']->table('order_info') . " SET shipping_status = '" . SS_RECEIVED . "',shipping_time_end = '" . gmtime() . "' WHERE order_id = '$order_id'";
        if ($GLOBALS['db']->query($sql))
        {
...
function save_consignee($consignee, $default=false)
{
    ...
    {
        /* 修改地址 */
        $res = $GLOBALS['db']->autoExecute($GLOBALS['yp']->table('user_address'), $consignee, 'UPDATE', 'address_id = ' . $consignee['address_id']." AND `user_id`= '".$_SESSION['user_id']."'");
    }
    else
    {
        /* 添加地址 */
        $res = $GLOBALS['db']->autoExecute($GLOBALS['yp']->table('user_address'), $consignee, 'INSERT');
...
{
    $sql = "SELECT user_id FROM " .$GLOBALS['yp']->table('user_address') . " WHERE address_id = '$id'";
    ...
    {
        $sql = "DELETE FROM " .$GLOBALS['yp']->table('user_address') . " WHERE address_id = '$id'";
        $res = $GLOBALS['db']->query($sql);
...
function update_address($address)
{
    ...
    {
         /* 更新指定记录 */
        $GLOBALS['db']->autoExecute($GLOBALS['yp']->table('user_address'), $address, 'UPDATE', 'address_id = ' .$address_id . ' AND user_id = ' . $address['user_id']);
    }
    else
    {
        /* 插入一条新记录 */
        $GLOBALS['db']->autoExecute($GLOBALS['yp']->table('user_address'), $address, 'INSERT');
...
function get_order_detail($order_id, $user_id = 0)
{
    ...
    {
         $shipping_code = $GLOBALS['db']->GetOne("SELECT shipping_code FROM ".$GLOBALS['yp']->table('shipping') ." WHERE shipping_id = '$order[shipping_id]'");
    ...
    if ($order['pay_status'] != PS_UNPAYED)
    {
        ...
        foreach ($virtual_goods AS $code => $goods_list)
        {
            ...
            if ($code == 'package_buy')
            {
                foreach ($goods_list as $goods)
                {
                    $sql = 'SELECT g.goods_id FROM ' . $GLOBALS['yp']->table('package_goods') . ' AS pg, ' . $GLOBALS['yp']->table('goods') . ' AS g ' .
...
{
    include_once(ROOT_PATH . 'includes/lib_order.php');
    $sql  = "SELECT order_sn FROM ".$GLOBALS['yp']->table('order_info') .
...
function merge_user_order($from_order, $to_order, $user_id = 0)
{
    if ($user_id > 0)
    {
        ...
        {
            $sql = "SELECT user_id FROM " .$GLOBALS['yp']->table('order_info').
...
function return_to_cart($order_id)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        if($row['product_id'])
        {
            ...
            $sql="SELECT product_number from ".$GLOBALS['yp']->table('products')."where product_id='$order_goods_product_id'";
        ...
        if (empty($cart_goods))
        {
            ...
            $GLOBALS['db']->autoExecute($GLOBALS['yp']->table('cart'), $return_goods, 'INSERT');
        }
        else
        {
            // 有相同商品，修改数量
            $sql = "UPDATE " . $GLOBALS['yp']->table('cart') . " SET " .
...
function save_order_address($address, $user_id)
{
    ...
    /* 检查订单状态 */
    $sql = "SELECT user_id, order_status FROM " .$GLOBALS['yp']->table('order_info'). " WHERE order_id = '" .$address['order_id']. "'";
    $row = $GLOBALS['db']->getRow($sql);
    if ($row)
    {
        ...
        $GLOBALS['db']->autoExecute($GLOBALS['yp']->table('order_info'), $address, 'UPDATE', "order_id = '$address[order_id]'");
        return true;
    }
    else
    {
...
function get_user_bouns_list($user_id, $num = 10, $start = 0, $suppid=-1)
{
    $sql = "SELECT u.bonus_sn, u.order_id, u.supplier_id, b.type_name, b.type_money, b.min_goods_amount, b.use_start_date, b.use_end_date ".
           " FROM " .$GLOBALS['yp']->table('user_bonus'). " AS u ,".
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $row['supplier_name'] = $row['supplier_id']>0? $GLOBALS['db']->getOne("select value from ". $GLOBALS['yp']->table('supplier_shop_config')." where code = 'shop_name' and supplier_id = '".$row['supplier_id']."'"):'网站自营';
        $arr[] = $row;
    }
    return $arr;
...
function get_order_handler($order){
    if ($order['order_status'] == OS_UNCONFIRMED)
    ...
    else if ($order['order_status'] == OS_SPLITED || $order['order_status'] == OS_CONFIRMED)
        {
            ...
            {
				$back_num = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('back_order') . " WHERE order_id = " . $order['order_id'] . " AND status_back < 6 AND status_back != 3");
				if ($back_num > 0)
				{
					$back_info = "此单存在正在退货商品，确认收货退货申请将取消。";
				}
...
function get_tel($supplier_id = 0){
    if($supplier_id>0){
        $sql = "select value from ".$GLOBALS['yp']->table('supplier_shop_config')." where code = 'service_phone' and supplier_id = $supplier_id and parent_id = 1";
    }else{
        $sql = "select value from ".$GLOBALS['yp']->table('ypmart_shop_config')." where code = 'service_phone'";
    }
