...
/**
 * QQ120029121 短信模块 之 模型（类库）
 * ============================================================================
 * 演示地址: http://demo.coolhong.com  开发QQ:120029121    309485552
 * ============================================================================
 * $Author: prince $
 * $Id: cls_sms.php 17155 2010-05-06 06:29:05Z prince $
 */
...
require_once(ROOT_PATH . 'includes/wx120029121_json.php');
...
class sms
{
     ...
     */
    var $api_urls   = array(
                            'info'              =>      'http://demo.coolhong.com',
                            'send'              =>      'http://demo.coolhong.com',
                            'servertime'        =>      'http://webdemo.coolhong.com'
    ...
    function send($phones,$msg,$send_date = '', $send_num = 1,$sms_type='',$version='1.0')
    {
        ...
        if(count($contents)>1)
        {
        	...
        	$send_str['license']=$GLOBALS['_CFG']['certificate_id'];
                ...
                $send_str['certi_ac']=$this->make_qq120029121_ac($send_str,SOURCE_TOKEN);
                $sms_url= $this->get_url('send');
    ...
    {
        $sql = 'SELECT `value`
                FROM ' . $this->yp->table('ypmart_shop_config') . "
                WHERE `code` = 'ent_id'";
    ...
    {
        $sql = 'SELECT `email` FROM ' . $this->yp->table('admin_user') . " WHERE `user_id` = '" . $_SESSION['admin_id'] . "'";
    ...
    function getTime(){
        ...
        $Tsend_str['certi_ac'] = $this->make_qq120029121_ac($Tsend_str,'SMS_TIME');
    ...
    //加密算法
    function make_qq120029121_ac($temp_arr,$token)
    {
       ksort($temp_arr);
       $str = '';
