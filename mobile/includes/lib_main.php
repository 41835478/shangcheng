...
function get_user_info($id=0)
{
    ...
    $time = date('Y-m-d');
    $sql  = 'SELECT u.user_id, u.email, u.user_name, u.user_money,u.frozen_money, u.pay_points, u.rank_points,u.reg_time,u.isfollow,u.headimg,u.fake_id '.
            ' FROM ' .$GLOBALS['yp']->table('users'). ' AS u ' .
            " WHERE u.user_id = '$id'";
    ...
    $user['user_points'] = $user['pay_points'];
    $user['pay_points'] = $user['pay_points'];
    $user['rank_points'] = $user['rank_points'];
    $user['user_money']  = price_format($user['user_money'], false);
    $user['frozen_money']  = price_format($user['frozen_money'], false);
...
{
    $sql = 'SELECT id, number, type FROM ' . $GLOBALS['yp']->table('ypmart_template') .
...
function save_searchengine_keyword($domain, $path)
{
    ...
    if (!empty($keywords))
    {
        ...
        $GLOBALS['db']->autoReplace($GLOBALS['yp']->table('keywords'), array('date' => local_date('Y-m-d'), 'searchengine' => $searchengine, 'keyword' => htmlspecialchars(addslashes($keywords)), 'count' => 1), array('count' => 1));
    }
}
...
function get_dyna_libs($theme, $tmp)
{
    ...
    $sql = 'SELECT region, library, sort_order, id, number, type' .
            ' FROM ' . $GLOBALS['yp']->table('ypmart_template') .
...
// add by prince qq 120029121 20161113 start
function upload_file1($upload, $type)
{
    if (!empty($upload['tmp_name']))
    {
        $ftype = check_file_type($upload['tmp_name'], $upload['name'], '|png|jpg|jpeg|gif|doc|xls|txt|zip|ppt|pdf|rar|docx|xlsx|pptx|');
...
// add by prince qq 120029121 20161113 end
...
function assign_comment($id, $type, $page = 1,$comment_level=0)
{
	...
	if($row['user_id'])
	{
		$arr[$row['comment_id']]['headimg'] = $GLOBALS['db']->getOne('select headimg from '. $GLOBALS['yp']->table('users') .' where user_id=\''. $row['user_id'] .'\'');
	}
...
{
    global $smarty;
	$sql="select count(*) from ".$GLOBALS['yp']->table('chat_customer')." where supp_id=0 and cus_name!='' and user_id!='' and of_username!='' ";
	$count_im = $GLOBALS['db']->getOne($sql);//20170906 IM by prince qq 120029121
    ...
    $smarty->assign('wechat',        $GLOBALS['_CFG']['wechat']?explode(',', $GLOBALS['_CFG']['wechat']):'');
    $smarty->assign('wechat_qrcode', $GLOBALS['_CFG']['wechat_qrcode']?$GLOBALS['_CFG']['wechat_qrcode']:'');
    $smarty->assign('had_im', $count_im>0?1:'0');//20170905 IM by prince qq 120029121
...
function get_user_bonus($user_id = 0)
{
    ...
    $sql = "SELECT SUM(bt.type_money) AS bonus_value, COUNT(*) AS bonus_count ".
            ...
            "WHERE ub.user_id = '$user_id' AND ub.bonus_type_id = bt.type_id AND ub.order_id = 0 and bt.use_end_date>'$nowtime' ";
 ...
 * @return  array
 */
function article_categories_childtree($cat_id = 0)
{
...
function get_library_number($library, $template = null)
{
    ...
    if (!isset($lib_list[$template]))
    {
        ...
        $sql = "SELECT library, number FROM " . $GLOBALS['yp']->table('ypmart_template') .
