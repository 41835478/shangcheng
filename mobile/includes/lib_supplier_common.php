...
function get_supplier_goods_count($suppid=0){
	...
	$sql="SELECT count(`goods_id`) FROM ".$GLOBALS['yp']->table('goods')." as g WHERE  g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_virtual = 0 AND g.supplier_id='$suppid'";
...
function assign_template_supplier($ctype = '', $catlist = array())
{
    ...
    $smarty->assign('qq', $GLOBALS['_CFG']['qq']?explode(',', $GLOBALS['_CFG']['qq']):'');
    $smarty->assign('ww', $GLOBALS['_CFG']['ww']?explode(',', $GLOBALS['_CFG']['ww']):'');
...
function get_supplier_fensi_count($suppid=0){
    $suppid = (intval($suppid)>0) ? intval($suppid) : intval($_GET['suppId']);
    return $GLOBALS['db']->getOne("SELECT count(*) FROM " .$GLOBALS['yp']->table('supplier_guanzhu') ." WHERE supplierid=$suppid");
}
/**
 * 获取店铺优惠活动数量
 */
function get_supplier_activity_count($suppid=0){
    $suppid = (intval($suppid)>0) ? intval($suppid) : intval($_GET['suppId']);
    $nowtime = gmtime();
    $sql = "SELECT count(*) FROM ".$GLOBALS['yp']->table('favourable_activity') ." WHERE supplier_id = $suppid AND start_time<=".$nowtime." AND end_time>=".$nowtime;
...
{
    $sql = 'SELECT * FROM '. $GLOBALS['yp']->table('supplier_nav') . '
...
function load_config_supplier()
{
    ...
    if ($data === false)
    {
        $sql = 'SELECT code, value FROM ' . $GLOBALS['yp']->table('supplier_shop_config') . ' WHERE parent_id > 0 AND supplier_id='.$suppid;
...
function get_categories_tree_supplier($cat_id = 0)
{
    if ($cat_id > 0)
    {
        $sql = 'SELECT parent_id FROM ' . $GLOBALS['yp']->table('supplier_category') . " WHERE cat_id = '$cat_id' AND supplier_id=".$_GET['suppId'];
    ...
    /*
     ...
     如果不是取当前分类及其下的子分类
    */
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['yp']->table('supplier_category') . " WHERE parent_id = '$parent_id' AND is_show = 1 AND supplier_id=".$_GET['suppId'];
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        /* 获取当前分类及其子分类 */
        $sql = 'SELECT cat_id,cat_name ,parent_id,is_show ' .
                'FROM ' . $GLOBALS['yp']->table('supplier_category') .
...
{
    $three_arr = array();
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['yp']->table('supplier_category') . " WHERE parent_id = '$tree_id' AND is_show = 1 AND supplier_id=".$_GET['suppId'];
...
function cat_list_supplier($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    ...
    if ($res === NULL)
    {
            ...
            $sql = "SELECT c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order, COUNT(s.cat_id) AS has_children ".
                ...
                "LEFT JOIN " . $GLOBALS['yp']->table('supplier_category') . " AS s ON s.parent_id=c.cat_id  where c.supplier_id = ".$suppid.
                " GROUP BY c.cat_id ".
...
function get_extension_goods_supplier($cats)
{
    ...
    $suppid = (isset($_GET['suppId']) && intval($_GET['suppId'])>0) ? intval($_GET['suppId']) : $_SESSION['supplier_id'];
    $sql = 'SELECT goods_id FROM ' . $GLOBALS['yp']->table('supplier_goods_cat') . " AS sgc WHERE $cats";
