...
function get_boss_by_user_id($user_id)
{
	...
	if($parent_id > 0)
	{
		 $sql = "SELECT user_id,user_name FROM " . $GLOBALS['yp']->table('users') . " WHERE user_id = '$parent_id'";
		 $user = $GLOBALS['db']->getRow($sql);
		 $info = get_user_info_by_user_id($user['user_id']);
		 $user['headimg'] = $info['headimg'];
		 return $user;
	}
}
...
{
	$sql = "SELECT * from " . $GLOBALS['yp']->table('dianpu') . " WHERE user_id = '$user_id'";
...
{
	$sql = "SELECT count(*) FROM " . $GLOBALS['yp']->table('weixin_qcode') . " where `type`='4' and content='$user_id'";
...
{
	$sql = "SELECT * from " . $GLOBALS['yp']->table('weixin_qcode') . " WHERE `type` = 4 AND content = '$user_id'";
	return $GLOBALS['db']->getRow($sql); 
}
...
function get_split_money_by_user_id($user_id,$uid)
{
	...
	$sql = "SELECT sum(al.money) FROM " . 
			$GLOBALS['yp']->table('order_info') . " as o ," . 
			$GLOBALS['yp']->table('affiliate_log') . " as al where o.order_id = al.order_id AND o.user_id = '$uid' and al.user_id = '$user_id' AND al.money>0  and o.is_separate=1 and al.separate_type>=0 ";
...
function get_distrib_user_info($user_id,$level)
{
	...
	if($count)
	{
		 ...
		 foreach($list as $key => $val)
		 {
			  if($call_username == 1)
			  {
				  $arr[$key]['call_username'] = '会员ID：'.$val['user_id'];//  20161015
			  }
			  else
			  {
			  ...
			  $arr[$key]['headimg'] = $info['headimg'];
			  $arr[$key]['nickname'] =$info['nickname']? '微信昵称：'.$info['nickname']:0;// added by prince qq 120029121 20161015
...
function get_affiliate_count_by_user_id($user_id)
{
	...
	$sql = "SELECT COUNT(*) FROM 
			(SELECT SUM(og.split_money) as total_money FROM " . 
			$GLOBALS['yp']->table('order_info') . " as o ," . 
			$GLOBALS['yp']->table('order_goods') . " as og where o.order_id = og.order_id AND o.user_id = '$user_id' AND o.shipping_status = 2 $sql_where group by o.order_id ) as oog $where";*/
			
	$my_user_id=$_SESSION['user_id'] ?$_SESSION['user_id'] :0;
			
	$sql = "SELECT COUNT(*) FROM " . 
			$GLOBALS['yp']->table('order_info') . " as o ," . 
			$GLOBALS['yp']->table('affiliate_log') . " as al where o.order_id = al.order_id AND o.user_id = '$user_id' and al.user_id = '$my_user_id' AND al.money>0  and o.is_separate=1 and al.separate_type>=0 ";
...
function get_affiliate_info_by_user_id($user_id,$page,$size)
{
	...
	$sql = "select order_id,is_separate,supplier_id,total_split_money from " . 
			...
			"SUM(b.split_money) as total_money," .
			"SUM(b.goods_price*b.goods_number) as total_split_money from " .
			$GLOBALS['yp']->table('order_info') . " as a ," . 
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
	{
		...
		if($_SESSION['user_id'] > 0)
		{
			...
			$arr[$row['order_id']]['set_money'] =$arr[$row['order_id']]['set_money']>0?$arr[$row['order_id']]['set_money'] :0;
			//20161015 
		}
		else
		{
...
function get_count_distrib_order_by_user_id($user_id,$is_separate)
{
	...
	if(!empty($all_uid))
	{
		...
		$sql = "SELECT order_id FROM " . $GLOBALS['yp']->table('order_info') . " WHERE user_id in($uids) " . $sql_where;
	...
	if(!empty($oids))
	{
		$sql = "SELECT COUNT(*) FROM 
				(SELECT SUM(og.split_money) as total_money FROM " . 
				$GLOBALS['yp']->table('order_info') . " as o ," . 
				$GLOBALS['yp']->table('order_goods') . " as og where o.order_id = og.order_id AND o.order_id in($oids) AND o.shipping_status = 2 AND is_separate = '$is_separate' group by o.order_id ) as oog $where";
		return $GLOBALS['db']->getOne($sql);
	}
	return 0;
}
...
//获取分销商下所有下线会员有收益的订单数量（含自己下单）
function get_count_shouyi_order_by_user_id($user_id)
{
	$sql = "SELECT COUNT(*) FROM " . 
			$GLOBALS['yp']->table('order_info') . " as o ," . 
			$GLOBALS['yp']->table('affiliate_log') . " as al where o.order_id = al.order_id  AND al.user_id = '$user_id' AND al.money>0  and o.is_separate=1 and al.separate_type>=0 ";
	...
	if(!empty($uids))
	{
		...
		if(!empty($oids))
		{
			$sql = "SELECT  order_id,goods_id,goods_name,user_id," . 
				...
				"SUM(og.split_money) as total_money," .
...
function get_total_money_by_user_id($user_id,$is_separate)
{
	...
	{
		//已分成总额
		$sql = "SELECT SUM(money) FROM " . $GLOBALS['yp']->table('affiliate_log') . " WHERE user_id = '$user_id'";
		$total_money = $GLOBALS['db']->getOne($sql);
	}
	else if($is_separate == 2)
	{
		//撤销分成总金额
		$sql = "SELECT SUM(money) FROM " . $GLOBALS['yp']->table('affiliate_log') . " WHERE user_id = '$user_id' AND money < 0";
		$money = $GLOBALS['db']->getOne($sql);
	...
	else
	{
		...
		if(!empty($uids))
		{
			...
			{
				$sql = "select a.order_id,a.user_id,sum(b.split_money*goods_number) as total_money from " . 
				$GLOBALS['yp']->table('order_info') . " as a ," . 
				$GLOBALS['yp']->table('order_goods') . 
...
function is_distribor($user_id)
{
	...
	$sql = "SELECT is_fenxiao FROM " . $GLOBALS['yp']->table('users') . " WHERE user_id = '$user_id'";
	return $GLOBALS['db']->getOne($sql);
}
...
{
	$sql = "SELECT * FROM " . $GLOBALS['yp']->table('affiliate_log') . " WHERE user_id = '$user_id' order by time desc";
	$list = $GLOBALS['db']->getAll($sql);
	$arr = array();
	foreach($list as $key => $val)
	{
...
{
    $time = gmtime();
    $sql = "INSERT INTO " . $GLOBALS['yp']->table('affiliate_log') . "( order_id, user_id, user_name, time, money, separate_type,change_desc)".
