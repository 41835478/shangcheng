...
if ($_REQUEST['act'] == 'asynclist')
{
    ...
    if (is_array($goodslist)) {
        foreach ($goodslist as $vo) {
			...
			//mod by PRINCE 120029121   20161228
...
/*
  ...
  <div class="otherboderhr">
    ...
    if (is_array($luckylist)) {
        ...
        <div class="lucklist">	
			<div class="dleft">
		         <img src="themesmobile/prince_jtypmall_mobile/images/extpt_icon_index.png">
		     </div>
...
{   
    ...
    if (!$smarty->is_cached('extpintuan_goods.dwt', $cache_id))
    {
        ...
        $extpintuan['share_url'] =HTTP_TYPE.'://'.$_SERVER['HTTP_HOST']."/mobile/extpintuan.php?act=view&act_id=".$extpintuan_id."&u=".$user_id;
       ...
       //$extpintuan['need_follow'].'-'.$weixininfo['isfollow'].'-'.is_wechat_browser_for_extpintuan();
		...
		$rand_price = $GLOBALS['db']->getOne("SELECT price FROM ". $GLOBALS['yp']->table('extpintuan_price') . " WHERE user_id = '$user_id' and act_id='$act_id' and level='$level' and status=0 LIMIT 1 ");
        $smarty->assign('rand_price',    $rand_price);  // 取出价格
        $smarty->assign('show_rand_price',    price_format($rand_price));  // 取出价格
		
		 //下单限制
		$sql = "select count(*) from ". $GLOBALS['yp']->table('extpintuan') . " where  user_id= '$user_id' and act_id='$act_id' and need_people='$level' and create_succeed =1 ";
		...
		if($total>=$extpintuan['orderlimit'] && $extpintuan['orderlimit']>0){
		 ...
		 //是否有待付款订单
		$sql = "select eo.* from ". $GLOBALS['yp']->table('extpintuan_orders') . " AS eo  " .
        " LEFT JOIN " . $GLOBALS['yp']->table('order_info') . " AS o ON eo.order_id    = o.order_id    " .
    ...
    //更新商品点击次数
    $sql = 'UPDATE ' . $yp->table('goods') . ' SET click_count = click_count + 1 '.
...
elseif ($_REQUEST['act'] == 'pt_view')
{
    ...
    if (!$smarty->is_cached('extpintuan_view.dwt', $cache_id))
    {
        ...
        $extpintuan['share_url'] =HTTP_TYPE.'://'.$_SERVER['HTTP_HOST']."/mobile/extpintuan.php?act=pt_view&pt_id=".$extpintuan_id."&u=".$user_id;
       ...
       //echo $extpintuan['need_follow'].'-'.$weixininfo['isfollow'].'-'.is_wechat_browser_for_extpintuan();
        ...
        $smarty->assign('show_rand_price',    price_format($rand_price));  // 取出价格
		 ...
		 //下单限制
		$sql = "select count(*) from ". $GLOBALS['yp']->table('extpintuan_orders') . " AS eo  " .
        " LEFT JOIN " . $GLOBALS['yp']->table('order_info') . " AS o ON eo.order_id    = o.order_id    " .
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'setprice')
{
    ...
    if ($act_id == 0 || $user_id ==0){
		...
		$price = array(
			...
			'level'  		 => $level,
			'create_time'    => gmtime() );
		$db->autoExecute($yp->table('extpintuan_price'), $price, 'INSERT');
		}
    }
...
{    	
    ...
    if (empty($extpintuan))
    {
	 ...
	 /*if($pt_id ==0  &&  $extpintuan_level!=0){
			$sql = "select count(*) from ". $GLOBALS['yp']->table('extpintuan') . " where status=0 and act_id=".$extpintuan_id. " and user_id =".$_SESSION['user_id'];
			$total =$GLOBALS['db']->getOne($sql);
			if ($total>=$extpintuan['open_limit']  && $extpintuan['open_limit']!=0){    
				...
				yp_header("Location: extpintuan.php?act=view&act_id=$extpintuan_id\n");
			}
	 }*/
	 
    /* 查询：检查拼团活动是否是进行中 */
	$chk_create_succeed = $GLOBALS['db']->getOne("SELECT create_succeed FROM ". $GLOBALS['yp']->table('extpintuan') . "  WHERE pt_id = '$pt_id' ");
	...
	//获取该拼团信息 
    $sql = "SELECT * FROM  " . $GLOBALS['yp']->table('extpintuan').
            "  WHERE  pt_id=".$pt_id;
	...
	//获取价格
	/*$rand_price = $GLOBALS['db']->getOne("SELECT price FROM ". $GLOBALS['yp']->table('extpintuan_price') . "  WHERE user_id = '$user_id' and act_id='$extpintuan_id' and level='$extpintuan_level' and status=0 LIMIT 1 ");
	if(empty($rand_price)){
		$rand_price = $minprice;
	}*/
    ...
    $cart = array(
        ...
        'is_shipping'    =>$extpintuan['free_shipping']?:0,1,
	...
	if($extpintuan_level>1){
		...
		$_SESSION['extpintuan_headimg'] =$getinfo['headimg'];
