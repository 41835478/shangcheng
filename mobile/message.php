...
if ($action == 'default')
{
    ...
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['yp']->table('comment')." WHERE STATUS =1 AND comment_type =0 ";
    $record_count = $db->getOne($sql);
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['yp']->table('feedback')." WHERE `msg_area`='1' AND `msg_status` = '1' ";
    $record_count += $db->getOne($sql);
...
function get_msg_list($num, $start)
{
    ...
    if($mysql_ver > '3.2.3')
    {
        ...
        $sql .= " FROM " .$GLOBALS['yp']->table('comment');
        ...
        $sql .= " FROM " .$GLOBALS['yp']->table('feedback');
    ...
    else 
    {
        ...
        $con_sql .= " FROM " .$GLOBALS['yp']->table('comment');
        ...
        $fee_sql .= " FROM " .$GLOBALS['yp']->table('feedback');
        ...
        $sql = "SELECT * FROM  " .$GLOBALS['yp']->table('tmp_table') . " ORDER BY msg_time DESC ";
    }
	...
	if($rows['user_name'])
	{
		$msg[$rows['msg_time']]['headimg'] = $GLOBALS['db']->getOne('select headimg from '. $GLOBALS['yp']->table('users') .' where user_name=\''. $rows['user_name'] .'\'');
	}
            ...
            if($rows['id_value'])
            {
                $sql_goods = "SELECT goods_name FROM ".$GLOBALS['yp']->table('goods');
