...
function action_default ()
{
	...
	/* 代码增加_star By demo.coolhong.com 今天优品多商户系统 qq 120029121 */
	$smarty->assign('sms_register', $_CFG['sms_register']);
	/* 代码增加_end By demo.coolhong.com 今天优品多商户系统 qq 120029121 */
	/* 增加是否关闭注册 */
...
// 		}
		...
		$parent_id = isset($_POST['parent_id']) ? $_POST['parent_id'] : trim($_REQUEST['u']);//推荐人id
		
		
		$up_uid = $db->getRow("SELECT * FROM ". $GLOBALS['yp']->table('users') ." WHERE `user_id` = '$parent_id'");
		
		if(! empty($parent_id) && empty($up_uid))
		{
			show_message('您输入的推荐人ID不是有效用户ID');
		}
		...
		{
			/* 把新注册用户的扩展信息插入数据库 */
			$sql = 'SELECT id FROM ' . $yp->table('reg_fields') . ' WHERE type = 0 AND display = 1 ORDER BY dis_order, id'; // 读出所有自定义扩展字段的id
			...
			/*写入上级 id  寒冰  20160808      QQ   3094  85552*/
				if(! empty($parent_id) && $up_uid)
		         {
			     $sql = 'UPDATE ' . $yp->table('users') . " SET `parent_id`='$parent_id' WHERE `user_id`='" . $_SESSION['user_id'] . "'";
				 $db->query($sql);
		       }
			...
			$sql_b = "SELECT type_id FROM " . $yp->table("bonus_type") . " WHERE send_type='" . SEND_BY_REGISTER . "'  AND send_start_date<=" . $now . " AND send_end_date>=" . $now . $sql_bonus_ext;
			$res_bonus = $db->query($sql_b);
			$kkk_bonus = 0;
			while($row_bonus = $db->fetchRow($res_bonus))
			{
				$sql = "INSERT INTO " . $yp->table('user_bonus') . "(bonus_type_id, bonus_sn, user_id, used_time, order_id, emailed)" . " VALUES('" . $row_bonus['type_id'] . "', 0, '" . $_SESSION['user_id'] . "', 0, 0, 0)";
				$db->query($sql);
			...
			//修改新注册的用户成为普通分销商
			//$GLOBALS['db']->query("UPDATE ".$GLOBALS['yp']->table('users')." SET is_fenxiao = 2 WHERE user_id = '" . $_SESSION['user_id'] . "'");//prince
