...
{
	return $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('deposit') . " WHERE user_id = '$user_id'"); 
}

//获取提现记录
function get_deposit_by_user_id($user_id,$page,$size)
{
	$sql = "SELECT * FROM " . $GLOBALS['yp']->table('deposit') . " WHERE user_id = '$user_id'";
