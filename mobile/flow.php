 ...
 * ============================================================================
 * $Author: prince $
 * $Id: flow.php 17218 2011-01-24 04:10:41Z prince $
 */
...
if(isset($_REQUEST['is_ajax']) && $_REQUEST['step']=='show_choose_attr'){
    ...
    $goods_id = $GLOBALS['db']->getOne("select goods_id from ". $GLOBALS['yp']->table('cart') ." where rec_id = $rec_id");
...
if($_REQUEST['act'] == 'edit_user_mobile'){
    include('includes/cls_json.php');
    $json   = new JSON;
    $sql = "UPDATE ".$GLOBALS['yp']->table('users')." set mobile_phone = '".$_REQUEST['mobile_phone']."' where  user_id =".$_SESSION['user_id'];
...
if(isset($_REQUEST['is_ajax']) && $_REQUEST['step']=='edit_cart_goods'){
    ...
    foreach($attr_id_array as $key=>$value){
        $sql = "select ga.attr_id, ga.attr_value, ga.attr_price, at.attr_name from ". $GLOBALS['yp']->table('goods_attr') ." as ga left join ". $GLOBALS['yp']->table('attribute') ." as at on ga.attr_id = at.attr_id  where ga.goods_attr_id = $value";
    ...
    $GLOBALS['db']->query("update ". $GLOBALS['yp']->table('cart') ." set goods_attr = '$goods_attr', goods_attr_id = '$attr_id', goods_price = '$shop_price' where rec_id = $rec_id");
    $result['err'] = '0';
    }  
    die($json->encode($result));
}

/* 代码增加_start  添加    今-天-优-品 多商户系统 QQ 1200-2912-1 */
...
/* 代码增加_end  添加    今-天-优-品 多商户系统 QQ 1200-2912-1 */
...
if ($_REQUEST['act']=='EditAddress')
{
	...
	if ($address_id)
	{
		$sql="select * from ". $yp->table('user_address') ." where address_id='$address_id' ";
...
elseif ($_REQUEST['act']=='selAddress')
{
	...
	$sql = "update ". $GLOBALS['yp']->table('users') ." set address_id='". $_REQUEST['address_id'] ."' where user_id='".$_SESSION['user_id']."' ";
	$db->query($sql);
	$sql = "SELECT * ".
                    " FROM " . $GLOBALS['yp']->table('user_address') . 
    ...
    $sql = 'SELECT count(*) FROM ' . $yp->table('cart') . " WHERE `session_id` = '" . SESS_ID. "' AND `extension_code` != 'package_buy' AND `is_shipping` = 0";
...
elseif ($_REQUEST['act']=='delAddress')
{
	...
	$consignee_list = get_consignee_list_wx_120029121();
...
elseif ($_REQUEST['act']=='saveAddress')
{
	...
	$address_wx_120029121 = $json->decode($_POST['address']);
	$consignee = array(
            'address_id'    => empty($address_wx_120029121->address_id) ?  '0'  :   intval($address_wx_120029121->address_id),
            'consignee'     => empty($address_wx_120029121->consignee)  ? '' :   compile_str(trim($address_wx_120029121->consignee)),
            'country'       => empty($address_wx_120029121->country)    ? '' :   intval($address_wx_120029121->country),
            'province'      => empty($address_wx_120029121->province)   ? '' :   intval($address_wx_120029121->province),
            'city'          => empty($address_wx_120029121->city)       ? '' :   intval($address_wx_120029121->city),
            'district'      => empty($address_wx_120029121->district)   ? '' :   intval($address_wx_120029121->district),
            'email'         => empty($address_wx_120029121->email)      ? '' :   compile_str($address_wx_120029121->email),
            'address'       => empty($address_wx_120029121->address)    ? '' :   compile_str($address_wx_120029121->address),
            'zipcode'       => empty($address_wx_120029121->zipcode)    ? '' :   compile_str(make_semiangle(trim($address_wx_120029121->zipcode))),
            'tel'           => empty($address_wx_120029121->tel)        ? '' :   compile_str(make_semiangle(trim($address_wx_120029121->tel))),
            'mobile'        => empty($address_wx_120029121->mobile)     ? '' :   compile_str(make_semiangle(trim($address_wx_120029121->mobile))),
     );
	...
	$result = array('error' => 0, 'message' => '', 'content' => '', 'closediv'=>$address_wx_120029121->closediv);    
	$consignee_list = get_consignee_list_wx_120029121();
	$smarty->assign('consignee_list', $consignee_list);
	$result['content'] =  $smarty->fetch("library/address_list.lbi");
	...
	if ($address_wx_120029121->shipping_bian=='0' && $address_wx_120029121->address_id>0 && $address_wx_120029121->address_id==$_SESSION['flow_consignee']['address_id'])
	{
		$sql = "SELECT * ".
                    " FROM " . $GLOBALS['yp']->table('user_address') . 
                    " WHERE  address_id = '".$address_wx_120029121->address_id."' ";
		$consignee = $GLOBALS['db']->getRow($sql);
		$_SESSION['flow_consignee'] = $consignee;
	}
	if ($address_wx_120029121->shipping_bian=='1' || ($address_wx_120029121->shipping_bian=='0' && $address_wx_120029121->address_id>0 && $address_wx_120029121->address_id==$_SESSION['flow_consignee']['address_id']))
	{
	$order = flow_order_info();
	$flow_type = isset($_SESSION['flow_type']) ? intval($_SESSION['flow_type']) : CART_GENERAL_GOODS;
    $region            = array($consignee['country'], $consignee['province'], $consignee['city'], $consignee['district']);
...
function get_consignee_list_wx_120029121()
{
	$consignee = get_consignee($_SESSION['user_id']);
	$sql="SELECT * FROM " . $GLOBALS['yp']->table('user_address') .
            " WHERE user_id = '". $_SESSION['user_id'] ."' order by address_id ";
	$consignee_list_wx_120029121 = $GLOBALS['db']->getAll($sql);
	foreach ($consignee_list_wx_120029121  as $cons_key => $cons_val)
	{
				$consignee_list_wx_120029121[$cons_key]['address_short_name'] = $cons_val['consignee']."<br>";
				$consignee_list_wx_120029121[$cons_key]['address_short_name'] .=  get_region_info($cons_val['province'])."-";
				$consignee_list_wx_120029121[$cons_key]['address_short_name'] .=  get_region_info($cons_val['city'])."-";
				$consignee_list_wx_120029121[$cons_key]['address_short_name'] .=  get_region_info($cons_val['district'])."&nbsp;";
				$consignee_list_wx_120029121[$cons_key]['address_short_name'] .=  sub_str($cons_val['address'],16);
				$consignee_list_wx_120029121[$cons_key]['address_short_name'] .=  $cons_val['zipcode'] ? (",".$cons_val['zipcode']) : "";
				$consignee_list_wx_120029121[$cons_key]['address_short_name'] .=  "<br>".($cons_val['tel'] != '--' ? $cons_val['tel'] : $cons_val['mobile']);
				if ($consignee['address_id'] == $cons_val['address_id'])
				{
					$consignee_list_wx_120029121[$cons_key]['def_addr'] =1;
					$have_def_addr=1;
				}
	}
	if ( count($consignee_list_wx_120029121) && !$have_def_addr){ $consignee_list_wx_120029121[0]['def_addr'] =1; }
	return 	$consignee_list_wx_120029121;	
}
...
{
    if(isset($_SESSION['flow_type'])){
		if($_SESSION['flow_type']>=8){
       	 	clear_cart();
			unset($_SESSION['flow_type']);
		}
	}// add by prince 20161114  QQ  120029121 
    ...
    if(!isset($goods->extCode) || $goods->extCode != 'pre_sale')
    {
    	...
    	//demo.coolhong.com start add 2015-311-23 限购
	$time_xg_now=gmtime();
	$row_xg= $GLOBALS['db']->getRow("select is_buy,buymax, buymax_start_date, buymax_end_date from ". $GLOBALS['yp']->table('goods') ." where goods_id='".$goods->goods_id."' " );
	if ( $row_xg['is_buy'] == 1 && $row_xg['buymax'] >0 && $row_xg['buymax_start_date'] < $time_xg_now  && $row_xg['buymax_end_date'] > $time_xg_now  )
	{
		...
		else
		{
			...
			$num_cart_old_1=$GLOBALS['db']->getOne("select sum(goods_number) from ". $GLOBALS['yp']->table('cart') ." where " . $sql_where . " and goods_id= " . $goods->goods_id );
			$num_cart_old_2=$GLOBALS['db']->getOne("select sum(og.goods_number) from ". $GLOBALS['yp']->table('order_goods') ." AS og , ". $GLOBALS['yp']->table('order_info') ." AS o where o.user_id='$_SESSION[user_id]' and  o.order_id = og.order_id and add_time > ". $row_xg['buymax_start_date'] ." and add_time < ". $row_xg['buymax_end_date'] ."  and og.goods_id = " . $goods->goods_id );
	...
	//demo.coolhong.com end add 2015-11-23 限购
    ...
    if (empty($goods->spec) AND empty($goods->quick))
    {
        ...
        'FROM ' . $GLOBALS['yp']->table('goods_attr') . ' AS g ' .
        'LEFT JOIN ' . $GLOBALS['yp']->table('attribute') . ' AS a ON a.attr_id = g.attr_id ' .
    ...
    {
        //clear_cart();//prince 20170915 立即购买不清购物车
    }
...
$rows = $GLOBALS['db']->getRow("select goods_brief,shop_price,goods_name,goods_thumb from ".$GLOBALS['yp']->table('goods')." where goods_id=".$goods->goods_id);
...
elseif ($_REQUEST['step'] == 'login')
{
	...
	$sql_where = $_SESSION['user_id']>0 ? "user_id='". $_SESSION['user_id'] ."' " : "session_id = '" . SESS_ID . "' AND user_id=0 ";		
        $sql = "SELECT COUNT(*) FROM " . $yp->table('cart') .
    ...
    else
    {
		...
		$sql_where = $_SESSION['user_id']>0 ? "user_id='". $_SESSION['user_id'] ."' " : "session_id = '" . SESS_ID . "' AND user_id=0 ";
                $sql = "SELECT COUNT(*) FROM " . $yp->table('cart') . " WHERE $sql_where ";
                if ($db->getOne($sql) > 0)
                {
...
elseif ($_REQUEST['step'] == 'drop_consignee')
{
    ...
    {
        yp_header("Location: flow.php?step=consignee\n");
        exit;
    }
    else
    {
...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    if ($_SESSION['user_id'] == 0)
    {
        ...
        //yp_header("Location: user.php\n");
        yp_header("Location: user.php?act=login&jump=jszx\n");
        exit;
    }
    /*------------------------------------------------------ */
    //-- 订单确认
  ...
  $flow_type = isset($_SESSION['flow_type']) ? intval($_SESSION['flow_type']) : CART_GENERAL_GOODS;
	...
	/*云购、砍价、拼团商品 */
	elseif ($flow_type == CART_LUCKY_BUY_GOODS || $flow_type == CART_EXTPINTUAN_GOODS || $flow_type == CART_CUT_GOODS )
    {
        $smarty->assign('is_cuxiao_goods', 1);//qq309485552 寒冰  2017.02.21  增加 云购  砍价 平台  限制不能使用  货到付款  
    }
	
    /* 虚拟团购 prince ｑｑ　１２００２９１２１　　　２０１６１０３０ */
    ...
    if($flow_type != CART_EXCHANGE_GOODS){
		...
		if($favourable_list){
			...
			foreach($favourable_list as $fk=>$fv){
				if(!$fv['available']){
					$sql = "select count(rec_id) as num from ". $yp->table('cart') .
					" WHERE $sql_where " .
    ...
    if ($db->getOne($sql) == 0)
    {
		...
		$sql="select c.goods_number,g.goods_id, g.goods_name,g.is_buy, g.buymax, g.buymax_start_date, g.buymax_end_date from ".$yp->table('cart'). " AS c left join ".$yp->table('goods'). " AS g on c.goods_id=g.goods_id where 1 ".$sql_plus;
		$goods_list = $db->getAll($sql);
		foreach($goods_list as $k => $v)
		{
			if($v['is_buy'] == 1 && $v['buymax'] >0 && $v['buymax_start_date'] < $time_xg_now  && $v['buymax_end_date'] > $time_xg_now )
			{
				$num_cart_old=$GLOBALS['db']->getOne("select sum(og.goods_number) from ". $GLOBALS['yp']->table('order_goods') ." AS og , ". $GLOBALS['yp']->table('order_info') ." AS o where o.user_id='$_SESSION[user_id]' and o.order_id = og.order_id and add_time > ". $v['buymax_start_date'] ." and add_time < ". $v['buymax_end_date'] ."  and og.goods_id = " . $v['goods_id'] ); 
    ...
    {
        /* 用户没有登录且没有选定匿名购物，转向到登录页面 */
        yp_header("Location: flow.php?step=login\n");
        exit;
    }
	...
	$sql = "SELECT concat( IFNULL(c.region_name, ' '),   
        ...
        " FROM " . $yp->table('region') . " c, " . $yp->table('region') . " p, " . $yp->table('region') . " t, " . $yp->table('region') . " d  
...
$consignee['region'] = $db->getOne($sql); 
    ...
    {	
		...
		foreach ($consignee_list_wx_120029121  as $cons_key => $cons_val)
		{
			...
			$consignee_list_wx_120029121[$cons_key]['address_short_name'] .=  sub_str($cons_val['address'],16);
			$consignee_list_wx_120029121[$cons_key]['address_short_name'] .=  $cons_val['zipcode'] ? (",".$cons_val['zipcode']) : "";
			$consignee_list_wx_120029121[$cons_key]['address_short_name'] .=  "<br>".$cons_val['tel'];
			if ($consignee['address_id'] == $cons_val['address_id'])
			{
				$consignee_list_wx_120029121[$cons_key]['def_addr'] =1;
				$have_def_addr=1;
   ...
   $smarty->assign('name_of_region',   array($_CFG['name_of_region_1'], $_CFG['name_of_region_2'], $_CFG['name_of_region_3'], $_CFG['name_of_region_4']));
   $smarty->assign('consignee_list', $consignee_list_wx_120029121);
   $smarty->assign('shop_province_list', get_regions(1, $_CFG['shop_country']));
    ...
    $smarty->assign('market_price_desc', sprintf($_LANG['than_market_price'], $total['formated_market_price'], $total['formated_saving'], $total['save_rate']));
     ...
     /* 取得配送列表 */
    $region            = array($consignee['country'], $consignee['province'], $consignee['city'], $consignee['district']);
    $shipping_list     = available_shipping_list($region);//2017.02.21 寒冰 qq3094  8555 2   修复配送方式决定是否支持货到付款
//    $cart_weight_price = cart_weight_price($flow_type);
//    $insure_disabled   = true;
//    $cod_disabled      = true;
//
//    // 查看购物车中是否全为免运费商品，若是则把运费赋为零
//    //$sql = 'SELECT count(*) FROM ' . $yp->table('cart') . " WHERE `session_id` = '" . SESS_ID. "' AND `extension_code` != 'package_buy' AND `is_shipping` = 0";
//    $sql = 'SELECT count(*) FROM ' . $yp->table('cart') . " WHERE $sql_where AND `extension_code` != 'package_buy' AND `is_shipping` = 0";
//    $shipping_count = $db->getOne($sql);
//
   foreach ($shipping_list AS $key => $val)//2017.02.21 寒冰 qq3094  8555 2   修复配送方式决定是否支持货到付款
   {
        $shipping_cfg = unserialize_config($val['configure']);
       $shipping_fee = ($shipping_count == 0 AND $cart_weight_price['free_shipping'] == 1) ? 0 : shipping_fee($val['shipping_code'], unserialize($val['configure']),
        $cart_weight_price['weight'], $cart_weight_price['amount'], $cart_weight_price['number']);
...
//    $smarty->assign('insure_disabled', $insure_disabled);
//    $smarty->assign('cod_disabled',    $cod_disabled);
      $smarty->assign('support_cod',    $val['support_cod']);//2017.02.21 寒冰 qq3094  8555 2   修复配送方式决定是否支持货到付款
    ...
    if(isset($payment_list))
    {
        ...
        {    
            ...
            if ($payment['is_cod'] == '1')
            {
                $payment_list[$key]['format_pay_fee'] = '<span id="YP_CODFEE">' . $payment['format_pay_fee'] . '</span>';
            }
...
$user_bonus[$key];
	...
	$sql = 'SELECT `is_surplus_open`'.
            'FROM '. $GLOBALS['yp']->table('users') .''.
            'WHERE `user_id` = \''.$_SESSION['user_id'].'\''.
            'LIMIT 1';
    $is_surplus_open = $GLOBALS['db']->getOne($sql);
	$smarty->assign('is_surplus_open', $is_surplus_open);
     
	setcookie('one_step_buy',   '', 1,'/');//20170915 立即购买不清购物车 q1 2 0 0 2 9 1 2 1
...
//		$sql = 'select * from ' . $yp->table('pickup_point') .
...
elseif ($_REQUEST['step'] == 'check_surplus_open')
{
    ...
    $sql_paycode = "SELECT pay_code FROM " . $yp->table('ypmart_payment') . " WHERE pay_id = " . $pay_id;
...
elseif ($_REQUEST['step'] == 'change_needinv')
{
    ...
    else
    {
      ...
      $result['type_money'] = $total['tax_formated'];/*20160226  寒  冰  qq   30948   5552    修复用户选择开发票  前台无法实时显示  发票税额*/
        //die($smarty->fetch('library/order_total.lbi'));
     $result['content'] = $smarty->fetch('library/order_total.lbi');
    }

    $json = new JSON();
    die($json->encode($result));/*20160226  寒  冰  qq   30948   5552    修复用户选择开发票  前台无法实时显示  发票税额*/
}
...
{	
	...
	$sql = "SELECT * FROM ".$yp->table('cart')."WHERE $sql_where AND parent_id = 0 AND is_gift > 0 AND rec_type = '$flow_type'";
	$res = $db->getAll($sql);
	foreach($res as $key=>$value)
	{
		$goodsid = $value['goods_id'];
		$sql = "SELECT goods_number FROM ".$yp->table('goods')."WHERE goods_id = $goodsid";
		$rec = $db->getOne($sql);
    ...
    if ($_CFG['use_storage'] == '1' && $_CFG['stock_dec_time'] == SDT_PLACE && $flow_type !=12)//20161228 prince
    {
    ...
    foreach ($cart_goods_new as $ckey=>$cval){
		...
		$order['defaultbank'] = $_POST['qq_120029121_bank'] ? trim($_POST['qq_120029121_bank']) : "";
	    ...
	    if(!isset($_POST['pay_ship'][$ckey])){
	    	...
	    	$shipid = $db->getOne("select shipping_id from ".$yp->table('shipping')." where shipping_id=".$_POST['pay_ship'][$ckey]." and supplier_id=".$ckey);
	    	if($shipid){
	    ...
	    {	
	        ...
	        else
	        {
	            if ($user_id > 0)
	            {
	                $sql = "UPDATE " . $yp->table('user_bonus') . " SET user_id = '$user_id' WHERE bonus_id = '$bonus[bonus_id]' LIMIT 1";
					$db->query($sql);
	            }
	    ...
	    {
	        $sql="SELECT shipping_id FROM " . $yp->table('shipping') . " WHERE shipping_id=".$order['shipping_id'] ." AND enabled =1"; 
	        if(!$db->getOne($sql))
	        {
		...
		//if ($order['order_amount'] > 0)
	    {
	        ...
	        else
	        {
				...
				{
					$sql = "SELECT user_money,mobile_phone FROM " . $GLOBALS['yp']->table('users') . " WHERE user_id = '" . $order['user_id'] . "'";
					$users = $GLOBALS['db']->getRow($sql); //寒冰 qq 30948————5552   20171006  修复客户余额变动短信提示时间错误问题
					$content = sprintf($_CFG['sms_use_balance_reduce_tpl'],date("Y-m-d H:i:s",time()),$order['order_amount'],$users['user_money'],$_CFG['sms_sign']);
    ...
    if(count($order_info)>1){
	    ...
	    do
	    {
	        ...
	        $GLOBALS['db']->autoExecute($GLOBALS['yp']->table('order_info'), $save, 'INSERT');
    ...
    //生成订单
	//$payment_www_com['qq_wx_120029121_alipay_bank'] = $_POST['qq_120029121_bank'] ? trim($_POST['qq_120029121_bank']) : "qq_wx_120029121";
    ...
    foreach($order_info as $ok=>$order){
	    ...
	    do
	    {
	        ...
	        $GLOBALS['db']->autoExecute($GLOBALS['yp']->table('order_info'), $order, 'INSERT');
	    ...
	    /* 插入订单商品 下面这个SQL有修改 by demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1 注意末尾那个字段 */
	    /* 代码增加_start  By demo.coolhong.com 今天优品多商户系统 qq 120029121 */
	    ...
	    {
	        $sql = "UPDATE ". $yp->table('goods_activity') ." SET is_finished='2' WHERE act_id=".$order['extension_id'];
	        $db->query($sql);
	    }
		...
		{
			$sql = "UPDATE ". $yp->table('cut') ." SET `order_times`=`order_times`+1 WHERE cut_id=".$_SESSION['cut_id'];
			$db->query($sql);
		}
		...
		{   
		   ...
		   if($_SESSION['extpintuan_pt_id']==0){
				...
				$extpintuan = array(
					...
					'user_head'  => $_SESSION['extpintuan_headimg'],
				...
				$db->autoExecute($yp->table('extpintuan'), $extpintuan, 'INSERT');	
			...
			$extpintuan_orders = array(
					...
					'follow_user_head'  => $_SESSION['extpintuan_headimg'],
					'order_id' => $new_order_id ,
					'order_sn' => $order['order_sn'] ,
					'follow_time'  => $now_time
				);
			$db->autoExecute($yp->table('extpintuan_orders'), $extpintuan_orders, 'INSERT');	
	...
	//$GLOBALS['db'] -> query("unlock tables");
	    ...
	    if ($order['order_amount'] <= 0)
	    {
	        ...
	        foreach ($res AS $row)
	        {   
                    /* 代码增加_start  By demo.coolhong.com 今天优品多商户系统 qq 120029121 _sunlizhi*/
//                    if($row['extension_code'] == 'virtual_good'){
//                        $virtual_goods_num = $virtual_goods_num+1;
//                    }
                    $sqla = "select valid_date,supplier_id from ".$GLOBALS['yp']->table('goods') ." where goods_id=".$row['goods_id'];
                    $goods_info = $GLOBALS['db']->getRow($sqla);
                    $valid_date = $goods_info['valid_date'];
                    $supplier_id = $goods_info['supplier_id'];
                    /* 代码增加_end  By demo.coolhong.com 今天优品多商户系统 qq 120029121 _sunlizhi*/
	        ...
	        if ($virtual_goods AND $flow_type != CART_GROUP_BUY_GOODS)
	        {
	            ...
	            {         
                        ...
                        if($val['supplier_id']){
                            $supplier_name = $GLOBALS['db']->getOne("select supplier_name from ".$GLOBALS['yp']->table('supplier')." where supplier_id = $val[supplier_id]");
                        }else{
                             $supplier_name = '网站自营';
                        }
                        $card = $GLOBALS['db']->getAll("select card_sn from ".$GLOBALS['yp']->table('virtual_goods_card')." where order_sn='".$order['order_sn']."'");
                        require_once (ROOT_PATH . 'sms/sms.php');
                        foreach($card as $k=>$v){  
                            $card_sn .= $v['card_sn'].", ";
                            }     
                            $content = sprintf($_LANG['mobile_virtual_template'], $supplier_name, $val['goods_name'], $card_sn,local_date('Y-m-d',$val['valid_date']));
							$templateParam=Array("code"=>$card_sn);
							$templateCode=$GLOBALS['_CFG']['sms_register_dayu'];
							$result = sendSMS($_REQUEST['mobile_phone'],$content,'', '',$templateParam , $templateCode);//已支持阿里大于 QQ 12002912
                        }
	                /* 如果没有实体商品，修改发货状态，送积分和红包 */
	                $sql = "SELECT COUNT(*)" .
	                        " FROM " . $yp->table('order_goods') .
    ...
    //删除父订单记录
    if($del_patent_id > 0){
    	$sql="delete from ".$GLOBALS['yp']->table('order_info')." where order_id='$del_patent_id' ";
...
elseif ($_REQUEST['step'] == 'clear')
{
    ...
    $sql = "DELETE FROM " . $yp->table('cart') . " WHERE $sql_where";
    $db->query($sql);
...
elseif ($_REQUEST['step'] == 'drop_to_collect')
{
    ...
    {
        $rec_id = intval($_GET['id']);
        $goods_id = $db->getOne("SELECT  goods_id FROM " .$yp->table('cart'). " WHERE rec_id = '$rec_id' AND session_id = '" . SESS_ID . "' ");
        $count = $db->getOne("SELECT goods_id FROM " . $yp->table('collect_goods') . " WHERE user_id = '$_SESSION[user_id]' AND goods_id = '$goods_id'");
        if (empty($count))
        {
            $time = gmtime();
            $sql = "INSERT INTO " .$GLOBALS['yp']->table('collect_goods'). " (user_id, goods_id, add_time)" .
...
{
	/* 标记购物流程为普通商品 20161030  prince  qq 120029121 */
	...
	unset($_SESSION['flow_order']);//删除之前用户对订单做的选  20161030  prince  qq 120029121
    ...
    {
		$sql_where = $_SESSION['user_id']>0 ? "user_id='". $_SESSION['user_id'] ."' " : "session_id = '" . SESS_ID . "' AND user_id=0 ";
		$sql = "SELECT rec_id FROM " . $yp->table('cart') . 
			" WHERE $sql_where AND rec_type = '" . $flow_type . "'  order by rec_id desc"; //立即购买不清购物车 20170915 qq120029121
...
if($_REQUEST['step']=='update_group_cart')
{
	...
	{
		$time_xg_now=gmtime();
		$row_xg= $GLOBALS['db']->getRow("select is_buy,buymax, buymax_start_date, buymax_end_date from ". $GLOBALS['yp']->table('goods') ." where goods_id='".$goods_id."' " );
		if ( $row_xg['is_buy'] == 1 && $row_xg['buymax'] >0 && $row_xg['buymax_start_date'] < $time_xg_now  && $row_xg['buymax_end_date'] > $time_xg_now  )
		{
			if ($_SESSION['user_id'] == 0 )
			{
			...
			else
			{
				...
				$num_cart_old_1=$GLOBALS['db']->getOne("select sum(goods_number) from ". $GLOBALS['yp']->table('cart') ." where " . $sql_where . " and goods_id= " . $goods_id );
				$num_cart_old_2=$GLOBALS['db']->getOne("select sum(og.goods_number) from ". $GLOBALS['yp']->table('order_goods') ." AS og , ". $GLOBALS['yp']->table('order_info') ." AS o where o.user_id='$_SESSION[user_id]' and  o.order_id = og.order_id and add_time > ". $row_xg['buymax_start_date'] ." and add_time < ". $row_xg['buymax_end_date'] ."  and og.goods_id = " . $goods_id );
				$num_cart_old = $num_cart_old_1 + $num_cart_old_2 ;
	...
	if ($GLOBALS['_CFG']['use_storage'] == 1)
	{
		...
		if ($is_package == 0)
		{
			$pg_ids = $GLOBALS['db']->getAll("select goods_id, goods_number from " . $GLOBALS['yp']->table('cart') . " where extension_code = 'package_buy' and " . $sql_where);
			$pg_num = 0;
			foreach($pg_ids as $pg_id)
			{
				$pg_num += $GLOBALS['db']->getOne("select goods_number from " .$GLOBALS['yp']->table('package_goods') . " where package_id =  " . $pg_id['goods_id'] . " and goods_id = " . $goods_id);
			}
			$goods_number = $GLOBALS['db']->getOne("select goods_number from ".$GLOBALS['yp']->table('goods')." where goods_id='$goods_id'");
			...
			if($number2>$goods_number) //////// jx      库存判断 
			{
				...
				$result['number']=$GLOBALS['db']->getOne("select goods_number from ".$GLOBALS['yp']->table('cart')." where rec_id = '$rec_id'");
				die($json->encode($result));
			}
			//添加判断商品有属性的时候的库存   jx    
			$goods_attr_id = $GLOBALS['db']->getOne("SELECT goods_attr_id FROM ".$GLOBALS['yp']->table('cart')."WHERE rec_id = '$rec_id'");
			if($goods_attr_id)
			{
				$str = explode(',',$goods_attr_id);//把字符串转换成数组
				$goods_attr = implode('|',$str);// 把数组转换成以‘|’的字符传
				$attr_number = $GLOBALS['db']->getOne("select product_number from ".$GLOBALS['yp']->table('products')." where goods_id='$goods_id' AND goods_attr ='$goods_attr'");
				if($number>$attr_number)
				{
					$result['error'] = '1';
					$result['content'] ='对不起,您选择的数量超出库存您最多可购买'.$attr_number."件";
					$result['number']=$GLOBALS['db']->getOne("select product_number from ".$GLOBALS['yp']->table('products')." where goods_id='$goods_id' AND goods_attr ='$goods_attr'");
					die($json->encode($result));
				}
			}
		}
		...
		{
			$goods_infos = $GLOBALS['db']->getAll("select pg.goods_id, pg.goods_number, g.goods_name from " . $GLOBALS['yp']->table('package_goods') . " as pg left join " . $GLOBALS['yp']->table('goods') . " as g on pg.goods_id = g.goods_id where package_id='$goods_id'");
			$is_null_g = 0;
			foreach($goods_infos as $goods_info)
			{
				$one_num = $GLOBALS['db']->getOne("SELECT SUM(goods_number) FROM " . $GLOBALS['yp']->table('cart') . " WHERE goods_id = '$goods_info[goods_id]' and " . $sql_where);
				$number2 = $number * $goods_info['goods_number'] + $one_num;
				$goods_number = $GLOBALS['db']->getOne("select goods_number from ".$GLOBALS['yp']->table('goods')." where goods_id='$goods_info[goods_id]'");
	...
	$sql = "UPDATE " . $GLOBALS['yp']->table('cart') . " SET goods_number = '$number' WHERE rec_id = $rec_id";
	$GLOBALS['db']->query($sql);
	...
	if ($is_package == 0)
	{
		...
		$sql = "UPDATE " . $GLOBALS['yp']->table('cart') . " SET goods_price = '$shop_price' WHERE rec_id = $rec_id";
		$GLOBALS['db']->query($sql);
	}
	else
	{
		$sql_sp = "SELECT goods_price FROM " . $GLOBALS['yp']->table('cart') . " WHERE rec_id= " . $rec_id;
	...
	//$subtotal = $GLOBALS['db']->getONE("select goods_price * goods_number AS subtotal from ".$GLOBALS['yp']->table('cart')." where rec_id = $rec_id");
...
{
	$sql_hot = "select goods_id, goods_name, goods_thumb, shop_price, market_price from " . $GLOBALS['yp']->table('goods'). " where ". $rtype ."=1 order by  goods_id desc limit 0,10 ";
	 ...
	 while ($row_hot = $GLOBALS['db']->fetchRow($res_hot))
	 {
        ...
        $row_hot['goods_thumb'] = get_image_path($row_hot['goods_id'], $row_hot['goods_thumb'], true);//mod by qq120-029-121 20160815
...
function flow_available_points()
{
   ...
   /* 代码修改_end  By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121 */
...
function flow_update_cart($arr)
{
    ...
    foreach ($arr AS $key => $val)
    {
        ...
        //查询：
        $sql = "SELECT `goods_id`, `goods_attr_id`, `product_id`, `extension_code` FROM" .$GLOBALS['yp']->table('cart').
               " WHERE rec_id='$key' AND session_id='" . SESS_ID . "'";
        $goods = $GLOBALS['db']->getRow($sql);

        $sql = "SELECT g.goods_name, g.goods_number ".
                "FROM " .$GLOBALS['yp']->table('goods'). " AS g, ".
                    $GLOBALS['yp']->table('cart'). " AS c ".
        ...
        if (intval($GLOBALS['_CFG']['use_storage']) > 0 && $goods['extension_code'] != 'package_buy')
        {
            ...
            {
                $sql = "SELECT product_number FROM " .$GLOBALS['yp']->table('products'). " WHERE goods_id = '" . $goods['goods_id'] . "' AND product_id = '" . $goods['product_id'] . "'";
        ...
        $sql = "SELECT b.goods_number, b.rec_id
                FROM " .$GLOBALS['yp']->table('cart') . " a, " .$GLOBALS['yp']->table('cart') . " b
        ...
        if ($val > 0)
        {
            ...
            {
                //更新购物车中的商品数量
                $sql = "UPDATE " .$GLOBALS['yp']->table('cart').
    ...
    /* 删除所有赠品 */
    $sql = "DELETE FROM " . $GLOBALS['yp']->table('cart') . " WHERE $sql_where1 AND is_gift <> 0";
    $GLOBALS['db']->query($sql);
}
...
function flow_cart_stock($arr)
{
    foreach ($arr AS $key => $val)
    {
        ...
        $sql_where = $_SESSION['user_id']>0 ? "user_id='". $_SESSION['user_id'] ."' " : "session_id = '" . SESS_ID . "' AND user_id=0 ";
        $sql = "SELECT `goods_id`, `goods_attr_id`, `extension_code` FROM" .$GLOBALS['yp']->table('cart').
        ...
        if (intval($GLOBALS['_CFG']['use_storage']) > 0 && $goods['extension_code'] != 'package_buy')
        {
            ...
            {
                $sql = "SELECT product_number FROM " .$GLOBALS['yp']->table('products'). " WHERE goods_id = '" . $goods['goods_id'] . "' AND product_id = '" . $row['product_id'] . "'";
                $product_number = $GLOBALS['db']->getOne($sql);
                if ($product_number < $val)
                {
                    show_message(sprintf($GLOBALS['_LANG']['stock_insufficiency'], $row['goods_name'],
...
{
    /* 取得商品id */
    $sql = "SELECT * FROM " .$GLOBALS['yp']->table('cart'). " WHERE rec_id = '$id'";
    $row = $GLOBALS['db']->getRow($sql);
    if ($row)
    {
		$sql_where = $_SESSION['user_id']>0 ? "user_id='". $_SESSION['user_id'] ."' " : "session_id = '" . SESS_ID . "' AND user_id=0 ";//添加    今-天-优-品 多商户系统 QQ 1200-2912-1
        //如果是超值礼包
        if ($row['extension_code'] == 'package_buy')
        {
/* 代码修改_start  By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121  将这块替换掉*/
...
/* 代码修改_end  By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121  */
        ...
        elseif ($row['parent_id'] == 0 && $row['is_gift'] == 0)
        {
            ...
            $sql = "SELECT c.rec_id
                    FROM " . $GLOBALS['yp']->table('cart') . " AS c, " . $GLOBALS['yp']->table('group_goods') . " AS gg, " . $GLOBALS['yp']->table('goods'). " AS g
...
{
    /* 查询：购物车中所有不可以单独销售的配件 */
	$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";//添加    今-天-优-品 多商户系统 QQ 1200-2912-1

    $sql = "SELECT c.rec_id, gg.parent_id
            FROM " . $GLOBALS['yp']->table('cart') . " AS c
                LEFT JOIN " . $GLOBALS['yp']->table('group_goods') . " AS gg ON c.goods_id = gg.goods_id
                LEFT JOIN" . $GLOBALS['yp']->table('goods') . " AS g ON c.goods_id = g.goods_id
            WHERE $sql_where 
            AND c.extension_code <> 'package_buy'
            AND gg.parent_id > 0
            AND g.is_alone_sale = 0";
...
function favourable_list($user_rank,$is_have=true)
{
    ...
    while ($favourable = $GLOBALS['db']->fetchRow($res))
    {
        ...
        foreach ($favourable['gift'] as $key => $value)
        {
            ...
            $goods_thumb = $GLOBALS['db']->getOne("SELECT `goods_thumb` FROM " . $GLOBALS['yp']->table('goods') . " WHERE `goods_id`='{$value['id']}'");
            $favourable['gift'][$key]['goods_thumb'] = get_image_path($value['id'], $goods_thumb, true);//mod by qq120-029-121 20160815
        	$sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('goods') . " WHERE is_on_sale = 1 AND goods_id = ".$value['id'];
...
function act_range_desc($favourable)
{
    ...
    {
        $sql = "SELECT brand_name FROM " . $GLOBALS['yp']->table('brand') .
                " WHERE brand_id " . db_create_in($favourable['act_range_ext']);
        return join(',', $GLOBALS['db']->getCol($sql));
    }
    elseif ($favourable['act_range'] == FAR_CATEGORY)
    {
        $sql = "SELECT cat_name FROM " . $GLOBALS['yp']->table('category') .
                " WHERE cat_id " . db_create_in($favourable['act_range_ext']);
        return join(',', $GLOBALS['db']->getCol($sql));
    }
    elseif ($favourable['act_range'] == FAR_GOODS)
    {
        $sql = "SELECT goods_name FROM " . $GLOBALS['yp']->table('goods') .
...
{
    $sql = "INSERT INTO " . $GLOBALS['yp']->table('cart') . " (" .
