<?php
/**
 * QQ120029121 OPEN API统一接口
 * ============================================================================
 * 演示地址: http://demo.coolhong.com  开发QQ:120029121    309485552
 * ============================================================================
 * $Author: jtypmall $
 * $Id: goods.php 15921 2009-05-07 05:35:58Z jtypmall $
*/
...
function search_goods_detail()
{
    ...
    if (!empty($_POST['goods_id']) && is_numeric($_POST['goods_id']))
    {
        ...
        ' FROM ' . $GLOBALS['yp']->table('category') . ' AS c, ' . $GLOBALS['yp']->table('goods') . ' AS g LEFT JOIN ' . $GLOBALS['yp']->table('brand') . ' AS b ON g.brand_id = b.brand_id'.
...
function search_site_info()
{
    ...
    $sql = 'SELECT code, value'.
           ' FROM ' . $GLOBALS['yp']->table('ypmart_shop_config') .
...
function check_auth()
{
    ...
    if (!check_qq120029121_ac($_POST, $license['token']))
    {
    ...
    $certi['app_id'] = 'qq120029121_b2c'; // 说明客户端来源
    ...
    $certi['certi_ac'] = make_qq120029121_ac($certi, $license['token']); // 网店验证字符串
 ...
 */
function check_qq120029121_ac($post_params,$token)
{
    ksort($post_params);
    $str = '';
...
function data_back($info, $msg = '', $post, $result = 'success')
{
    ...
    if ($post == 1)
    {
        ...
        if (class_exists('DOMDocument'))
        {
            ...
            $qq120029121=$doc->createElement('qq120029121');
            $doc->appendChild($qq120029121);

            $result=$doc->createElement('result');
            $qq120029121->appendChild($result);
            $result->appendChild($doc->createCDATASection($data_arr['result']));

            $msg=$doc->createElement('msg');
            $qq120029121->appendChild($msg);
            $msg->appendChild($doc->createCDATASection($data_arr['msg']));

            $info=$doc->createElement('info');
            $qq120029121->appendChild($info);
...
function array2xml($data,$root='qq120029121'){
    $xml='<'.$root.'>';
    _array2xml($data,$xml);
    $xml.='</'.$root.'>';
    return $xml;
