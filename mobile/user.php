 ...
 * ============================================================================
 * $Author: prince $
 * $Id: user.php 17217 2017-04-01 06:29:08Z prince $
 */
...
else
{
	if(strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger'))
	{
		...
		{ 
			//$url = $GLOBALS['yp']->url()."user.php";  // prince qq 1 2 00 29121
...
$sql = "SELECT * FROM " . $GLOBALS['yp']->table('supplier_admin_user') . " WHERE uid='$user_id' LIMIT 1";
$row = $GLOBALS['db']->getRow($sql);
if($row){
	$cookietime = time() + 3600 * 24 * 15;
	$_CFG = $GLOBALS['_CFG'];
	setcookie('YPCP[supplier_id]',   $row['supplier_id'],                            $cookietime,'/mobile/'.SUPPLIER_PATH.'/');
	setcookie('YPCP[supplier_user_id]',   $row['user_id'],                            $cookietime,'/mobile/'.SUPPLIER_PATH.'/');
	setcookie('YPCP[supplier_pass]', md5($row['password'] .$_CFG['hash_code']), $cookietime,'/mobile/'.SUPPLIER_PATH.'/');
}
...
/* 显示页面的action列表 */
$ui_arr = array(
	'login','profile','order_list','order_detail','address_list','back_order','back_order_detail','back_list','back_order_detail_edit','back_order_act','collection_list','message_list','tag_list',
        'get_password','reset_password','booking_list','add_booking','account_raply','account_deposit','account_huafei','act_account_huafei','account_log',
       ...
       'supplier_reg','comment_order','address','set_address','follow_shop','account_manage','my_comment','shaidan_send','user_rank'
);
...
// 			die($_LANG['require_login']);
			show_message($_LANG['require_login'], array('</br>登录', '</br>返回首页'), array('user.php?act=login', $yp->url()), 'error', false);
		}
	}
}
...
if(in_array($action, $ui_arr))
{
	...
	else
	{
	 ...
	 $supplier_sql = "SELECT * FROM  " . $GLOBALS['yp']->table('supplier') . " WHERE `user_id`='$user_id'";
	 $supplier_inof =  $GLOBALS['db'] -> getRow($supplier_sql);
	if($supplier_inof['status'] == 1 && $supplier_inof['applynum'] == 3)
	{
		$is_supplier = 1;
...
if (file_exists("pre_sale.php")) {
   $pre_sale = 1;
	...
	$sql = "SELECT value FROM " . $yp->table('ypmart_shop_config') . " WHERE id = 419";
	$row = $db->getRow($sql);
	$car_off = $row['value'];
...
function action_comment_order(){
	...
	$yp = $GLOBALS['yp'];
        ...
        $goods_id = empty($_REQUEST['goods_id'])? $db->getOne("select goods_id from ".$yp->table('order_goods')." where rec_id = $rec_id") : intval($_REQUEST['goods_id']);
...
function action_account_manage(){
	...
	$surplus_amount=$account_sum['user_money'];
		
	if(empty($surplus_amount))
	{
		$surplus_amount = 0;
	}
	$user_info['user_money']=$surplus_amount;
	$user_info['frozen_money']=price_format($account_sum['frozen_money'], false);
	$user_info['rank_points']=$account_sum['rank_points'];
	$user_info['pay_points']=$account_sum['pay_points'];
...
function action_supplier_reg()
{
	...
	$sql = "select * from " . $yp->table('supplier') . " where user_id='" . $_SESSION['user_id'] . "' ";
...
function action_act_supplier_del()
{
	...
	$sql = "delete from " . $yp->table('supplier') . "  where supplier_id=" . $supplier_id;
	$db->query($sql);
...
function action_default()
{
	...
	/* 全部订单*/
        $order_count['all'] = $db->GetOne('SELECT COUNT(*) FROM ' . $yp->table('order_info') . " WHERE 1 $ex_where");
	/* 已完成的订单 */
	$order_count['finished'] = $db->GetOne('SELECT COUNT(*) FROM ' . $yp->table('order_info') . " WHERE 1 $ex_where " . order_query_sql('finished'));
	$status['finished'] = CS_FINISHED;
	
	/* 待发货的订单： */
	$order_count['await_ship'] = $db->GetOne('SELECT COUNT(*)' . ' FROM ' . $yp->table('order_info') . " WHERE 1 $ex_where " . order_query_sql('await_ship'));
	$status['await_ship'] = CS_AWAIT_SHIP;
	
	/* 待付款的订单： */
	$order_count['await_pay'] = $db->GetOne('SELECT COUNT(*)' . ' FROM ' . $yp->table('order_info') . " WHERE 1 $ex_where " . order_query_sql('await_pay'));
	$status['await_pay'] = CS_AWAIT_PAY;

        /* 待收货的订单　*/
	$order_count['await_receipt'] = $db->GetOne("SELECT COUNT(*) FROM " . $yp->table('order_info') . " WHERE 1 $ex_where " . order_query_sql('await_receipt'));
	$status['await_receipt'] = CS_AWAIT_RECEIPT;
        
	/* “未确认”的订单 */
	$order_count['unconfirmed'] = $db->GetOne('SELECT COUNT(*) FROM ' . $yp->table('order_info') . " WHERE 1 $ex_where " . order_query_sql('unconfirmed'));
	$status['unconfirmed'] = OS_UNCONFIRMED;
        $smarty->assign('order_count', $order_count);
	$collect_count = $GLOBALS['db']->getOne("select count(*) from " . $GLOBALS['yp']->table('collect_goods') . " where user_id = " . $user_id);
	$smarty->assign('collect_count', $collect_count);
	$comment_count = $GLOBALS['db']->getOne("select count(*) from " . $GLOBALS['yp']->table('comment') . " where user_id = " . $user_id);
	$smarty->assign('comment_count', $comment_count);
	include_once (ROOT_PATH . 'includes/lib_clips.php');
	

	$account_sum = get_user_surplus($user_id);
	$rank_points=$account_sum['rank_points'];
	...
	$recomm = $db->getOne("SELECT is_recomm FROM " . $GLOBALS['yp']->table('user_rank') . " r" . " LEFT JOIN" . $GLOBALS['yp']->table('users') . " u ON r.rank_id = u.user_rank" . " WHERE u.user_id = '$user_id'");
...
function action_check_mobile()
{
	...
	if($_REQUEST['mobile'])
	{
		$sql = "select user_id from " . $yp->table('users') . " where mobile_phone = '$_REQUEST[mobile]' ";
...
function action_send_mobile_code ()
{
	...
	$result = sendSMS($mobile_phone, $content,'', '',$templateParam , $templateCode);//已支持阿里大于 QQ 120029121
	if($result)
	{
...
function action_oath()
{
	...
	if($c)
	{
		...
		if($back_act[4] != ':')
			$back_act = $yp->url() . $back_act;
		$open = empty($_REQUEST['open']) ? 0 : intval($_REQUEST['open']);
		
		$url = $c->login($yp->url() . 'user.php?act=oath_login&type=' . $type . '&callblock=' . urlencode($back_act) . '&open=' . $open);
		if(! $url)
		{
			show_message($c->get_error(), '首页', $yp->url(), 'error');
		}
		header('Location: ' . $url);
	}
	else
	{
		show_message('服务器尚未注册该插件！', '首页', $yp->url(), 'error');
	}
}
...
function action_oath_login()
{
	...
	if($c)
	{
		...
		if(! $info)
		{
			show_message($c->get_error(), '首页', $yp->url(), 'error', false);
		}
		if(! $info['user_id'])
			show_message($c->get_error(), '首页', $yp->url(), 'error', false);
		...
		$sql = 'SELECT user_name,password,aite_id FROM ' . $yp->table('users') . ' WHERE aite_id = \'' . $info_user_id . '\' OR aite_id=\'' . $info['user_id'] . '\'';
		...
		if(! $count) // 没有当前数据
		{
			...
			$sql = 'INSERT INTO ' . $yp->table('users') . '(user_name , password, aite_id , sex , reg_time , user_rank , is_validated) VALUES ' . "('$info[name]' , '$user_pass' , '$info_user_id' , '$info[sex]' , '" . gmtime() . "' , '$info[rank_id]' , '1')";
			$db->query($sql);
			$tag = 1; //第一次注册标记
		}
		else
		{
			...
			{
				$sql = 'UPDATE ' . $yp->table('users') . " SET aite_id = '$info_user_id' WHERE aite_id = '$count[aite_id]'";
				$db->query($sql);
			}
			if($info['name'] != $count['user_name']) // 这段可删除
			{
				...
				$sql = 'UPDATE ' . $yp->table('users') . " SET user_name = '$info[name]' WHERE aite_id = '$info_user_id'";
				$db->query($sql);
				$tag = 2;
			}
		}
		...
		{
			yp_header('Location: ' . $_REQUEST['callblock']);
		}
	}
}
...
function action_other_login()
{
	...
	if(empty($info))
	{
		show_message("非法访问或请求超时！", '首页', $yp->url(), 'error', false);
	}
	if(! $info['user_id'])
		show_message("非法访问或访问出错，请联系管理员！", '首页', $yp->url(), 'error', false);
...
function action_validate_email()
{
	...
	if($hash)
	{
		...
		if($id > 0)
		{
			$sql = "UPDATE " . $yp->table('users') . " SET is_validated = 1 WHERE user_id='$id'";
			$db->query($sql);
			$sql = 'SELECT user_name, email FROM ' . $yp->table('users') . " WHERE user_id = '$id'";
			$row = $db->getRow($sql);
...
function action_check_register()
{
	...
	require_once (ROOT_PATH . 'includes/lib_sms.php');//验证手机号
	$json = new JSON();
	...
	{
		$sql = "select count(*) from " . $GLOBALS['yp']->table('users') . " where email = '$username'";
	...
	{
		$sql = "select count(*) from " . $GLOBALS['yp']->table('users') . " where mobile_phone = '$username'";
		$num = $GLOBALS['db']->getOne($sql);
		if($num > 0)
		{
			$result['error'] = 2;
	...
	{
		$sql = "update " . $GLOBALS['yp']->table('goods') . " set goods_name = 'ddd' where goods_id = '32'";
		$GLOBALS['db']->query($sql);
		$sql = "select count(*) from " . $GLOBALS['yp']->table('users') . " where user_name = '$username'";
		$num = $GLOBALS['db']->getOne($sql);
		if($num > 0)
		{
			$result['error'] = 3;
...
function action_login()
{
	...
	/* 取出注册扩展字段 */
	$sql = 'SELECT * FROM ' . $yp->table('reg_fields') . ' WHERE type < 2 AND display = 1 ORDER BY dis_order, id';
...
function action_act_login()
{
	...
	{
		$sql = "select user_name from " . $yp->table('users') . " where email='" . $username . "' AND supplier_id=0 ";
	...
	{
		$sql = "select user_name from " . $yp->table('users') . " where mobile_phone='" . $username . "' AND supplier_id=0 ";
...
function action_logout()
{
	...
	{
		$back_act = strpos($GLOBALS['_SERVER']['HTTP_REFERER'], 'user.php') ? './user.php?act=login' : $GLOBALS['_SERVER']['HTTP_REFERER'];
	}//寒冰  20161117     处理用户退出登录都跳转登录页面，用于用户切换账号登陆
	
	$user->logout();
	$ucdata = empty($user->ucdata) ? "" : $user->ucdata;
	show_message($_LANG['logout'] . $ucdata, array(
		$_LANG['relogin_lnk'],$_LANG['new_register']
	), array(
		$back_act,'register.php'//寒冰  20161117     处理用户退出登录都跳转登录页面，用于用户切换账号登陆
	), 'info');
}
...
function action_profile()
{
	...
	$sql = 'SELECT reg_field_id, content ' . 'FROM ' . $yp->table('reg_extend_info') . " WHERE user_id = $user_id";
...

{
	...
	$sql = 'SELECT id FROM ' . $yp->table('reg_fields') . ' WHERE type = 0 AND display = 1 ORDER BY dis_order, id'; // 读出所有扩展字段的id
...

	{
...

		{
			...
			$sql = 'SELECT * FROM ' . $yp->table('reg_extend_info') . "  WHERE reg_field_id = '$val[id]' AND user_id = '$user_id'";

			if($db->getOne($sql)) // 如果之前没有记录，则插入

			{

				$sql = 'UPDATE ' . $yp->table('reg_extend_info') . " SET content = '$temp_field_content' WHERE reg_field_id = '$val[id]' AND user_id = '$user_id'";
...

			{
				...
				$sql = 'INSERT INTO ' . $yp->table('reg_extend_info') . " (`user_id`, `reg_field_id`, `content`) VALUES ('$user_id', '$val[id]', '$temp_field_content')";
...

	{
		...
		$sql = 'UPDATE ' . $yp->table('users') . " SET `passwd_question`='$sel_question', `passwd_answer`='$passwd_answer'  WHERE `user_id`='" . $_SESSION['user_id'] . "'";
...
// 	);
	...
	if(edit_profile($profile)){
	   ...
	   if(!empty($_FILES['headimg']['name'])){
			...
			if(strpos($_FILES['headimg']['name'], 'php')!==false){
				show_message("您的图片含有非法参数,请重新上传！", $_LANG['profile_lnk'], 'user.php?act=profile', 'info');
			}
			...
			$sql = 'UPDATE ' . $yp->table('users') . " SET `headimg`='$headimg_thumb'  WHERE `user_id`='" . $_SESSION['user_id'] . "'";
			$db->query($sql);	 
...
function action_act_edit_img()
{
	...
	else
	{
		...
		$headimg_thumb = $headimg_thumb ? $headimg_thumb : $headimg_original;
		$sql = 'UPDATE ' . $yp->table('users') . " SET `headimg`='$headimg_original'  WHERE `user_id`='" . $_SESSION['user_id'] . "'";
		$db->query($sql);
...
function action_act_identity()
{
	...
	$sql = "select face_card,back_card from " . $GLOBALS['yp']->table('users') . " where user_id = '" . $_SESSION['user_id'] . "'";
	...
	$sql = 'update ' . $GLOBALS['yp']->table('users') . " set real_name = '$real_name',card='$card',country='$country',province='$province',city='$city',district='$district',address='$address',status = '2'";
	if($face_card != '')
	{
...
function action_update_email()
{
	...
	$sql = "select email from " . $GLOBALS['yp']->table('users') . " where user_id = '" . $_SESSION['user_id'] . "'";
...
function action_act_update_email()
{
	...
	{
		$sql = "select email,user_name from " . $GLOBALS['yp']->table('users') . " where user_id = '" . $_SESSION['user_id'] . "'";
		$rows = $GLOBALS['db']->getRow($sql);
		$tpl = get_mail_template('verify_mail');
		$run = "0123456789abcdefghijklmnopqrstuvwxyz";
		$hash = mc_random(16, $run);
		$email = $GLOBALS['yp']->url() . 'user.php?act=valid_email&hash=' . $hash;
		...
		{
			$add_time = time();
			$sql = "insert into " . $GLOBALS['yp']->table('email') . "(`email`,`hash`,`add_time`,`user_id`) values('" . $rows['email'] . "','$hash','$add_time','" . $_SESSION['user_id'] . "')";
			$GLOBALS['db']->query($sql);
			$smarty->display('user_transaction.dwt');
		}
		else
...
function action_valid_email()
{
	...
	$sql = "select * from " . $GLOBALS['yp']->table('email') . " where hash = '$hash'";
	$row = $GLOBALS['db']->getRow($sql);
	$now_time = time();
	if($now_time - $row['add_time'] > 24 * 60 * 60)
	{
		$sql = "delete from " . $GLOBALS['yp']->table('email') . " where hash = '$hash'";
		$GLOBALS['db']->query($sql);
		show_message('验证邮件已发送超过24小时，请重新验证！');
	}
	else
	{
		$sql = "select count(*) from " . $GLOBALS['yp']->table('email') . " where hash = '$hash'";
		$count = $GLOBALS['db']->getOne($sql);
		if($count > 0)
		{
			yp_header("Location: user.php?act=re_binding_email\n");
			exit();
		}
	}
}
...
function action_act_binding_email()
{
	...
	$sql = "select count(*) from " . $GLOBALS['yp']->table('users') . " where email = '$email'";
	$num = $GLOBALS['db']->getOne($sql);
	if($num > 0)
	{
	...
	else
	{
		...
		$validate_email = $GLOBALS['yp']->url() . 'user.php?act=re_validate_email&hash=' . $hash;
		
		$sql = "select user_name from " . $GLOBALS['yp']->table('users') . " where user_id = '" . $_SESSION['user_id'] . "'";
		...
		if($result == true)
		{
			$sql = "update " . $GLOBALS['yp']->table('users') . " set is_validated = 0,email='$email' where user_id = '" . $_SESSION['user_id'] . "'";
			$GLOBALS['db']->query($sql);
			$sql = "delete from " . $GLOBALS['yp']->table('email') . " where user_id = '" . $_SESSION['user_id'] . "'";
			$GLOBALS['db']->query($sql);
			$add_time = time();
			$sql = "insert into " . $GLOBALS['yp']->table('email') . "(`email`,`hash`,`add_time`,`user_id`) values('$email','$hash','$add_time','" . $_SESSION['user_id'] . "')";
			$GLOBALS['db']->query($sql);
...
function action_re_validate_email()
{
	...
	else
	{
		...
		if($count > 0)
		{
			$sql = "update " . $GLOBALS['yp']->table('users') . " set is_validated = 1 where user_id = '" . $_SESSION['user_id'] . "'";
			$GLOBALS['db']->query($sql);
			$sql = "delete from " . $GLOBALS['yp']->table('email') . " where user_id = '" . $_SESSION['user_id'] . "'";
			$GLOBALS['db']->query($sql);
...
function action_update_phone()
{
	...
	$sql = "select mobile_phone from " . $GLOBALS['yp']->table('users') . " where user_id = '" . $_SESSION['user_id'] . "'";
...
function action_act_update_phone()
{
	...
	else
	{
		if(is_telephone($phone))
		{
			...
			{
				/* 验证手机号验证码和IP */
				$sql = "SELECT COUNT(id) FROM " . $yp->table('verifycode') . " WHERE mobile='$phone' AND verifycode='$verifycode' AND getip='" . real_ip() . "' AND status=1 AND dateline>'" . gmtime() . "'-86400"; // 验证码一天内有效
				...
				{
					yp_header("Location: user.php?act=re_binding\n");
					exit();
				}
			}
		}
...
function action_binding()
{
	...
	else
	{
		if(is_telephone($phone))
		{
			$sql = "SELECT COUNT(user_id) FROM " . $yp->table('users') . " WHERE mobile_phone = '$phone'";
			if($db->getOne($sql) > 0)
			{
			...
			else
			{
				...
				else
				{
					...
					{
						$sql = "update " . $yp->table('users') . " set mobile_phone = '$phone',validated = 1 where user_id = '" . $_SESSION['user_id'] . "'";
						$num = $db->query($sql);
						if($num > 0)
						{
...
function action_get_passwd_question()
{
	...
	// 取出会员密码问题和答案
	$sql = 'SELECT user_id, user_name, passwd_question, passwd_answer FROM ' . $yp->table('users') . " WHERE user_name = '" . $user_name . "'";
...
function action_act_edit_password()
{
	...
	if(($user_info && (! empty($code) && md5($user_info['user_id'] . $_CFG['hash_code'] . $user_info['reg_time']) == $code)) || ($_SESSION['user_id'] > 0 && $_SESSION['user_id'] == $user_id && $user->check_user($_SESSION['user_name'], $old_password)))
	{
		...
		{
			$sql = "UPDATE " . $yp->table('users') . "SET `ec_salt`='0' WHERE user_id= '" . $user_id . "'";
			$db->query($sql);
			$user->logout();
...
function action_del_back_order ()
{
	...
	$sql = "select status_back from " . $yp->table('back_order') . " where back_id='$back_id' ";
	...
	{
		$sql = "update " . $yp->table('back_goods') . " set status_back = 8 where back_id='$back_id' ";
		$db->query($sql);
		$sql = "update " . $yp->table('back_order') . " set status_back = 8 where back_id='$back_id' ";
		$db->query($sql);
...
function action_back_order_detail_edit ()
{
	...
	if($shipping_id)
	{
		$sql = "SELECT shipping_name FROM " . $GLOBALS['yp']->table('shipping') . " where shipping_id='$shipping_id' ";
		$shipping_name = $db->getOne($sql);
	}
	$sql = "update " . $yp->table('back_order') . " set shipping_id='$shipping_id', shipping_name='$shipping_name', invoice_no='$invoice_no' where back_id='$back_id' ";
	$db->query($sql);
...
function action_back_order_detail ()
{
	...
	$sql = 'SELECT shipping_id, shipping_code, shipping_name ' . 'FROM ' . $GLOBALS['yp']->table('shipping') . 'WHERE enabled = 1 and supplier_id = 0   ORDER BY shipping_order';
	$shipping_list = $db->getAll($sql);
    
	$smarty->assign('shipping_list', $shipping_list);
	
	$sql = "SELECT * " . " FROM " . $GLOBALS['yp']->table('back_order') . " WHERE back_id= '$back_id' ";
	$back_shipping = $db->getRow($sql);
	
	$sql_og = "SELECT * FROM " . $GLOBALS['yp']->table('back_goods') . " WHERE back_id = " . $back_id;
	...
	$back_shipping['country_name'] = $db->getOne("SELECT region_name FROM " . $yp->table('region') . " WHERE region_id = '$back_shipping[country]'");
	$back_shipping['province_name'] = $db->getOne("SELECT region_name FROM " . $yp->table('region') . " WHERE region_id = '$back_shipping[province]'");
	$back_shipping['city_name'] = $db->getOne("SELECT region_name FROM " . $yp->table('region') . " WHERE region_id = '$back_shipping[city]'");
	$back_shipping['district_name'] = $db->getOne("SELECT region_name FROM " . $yp->table('region') . " WHERE region_id = '$back_shipping[district]'");
	...
	$sql = "select * from " . $yp->table('back_goods') . " where back_id = '$back_id' order by back_type ";
	...
	/* 回复留言 qq-120029121增加 */
	$res = $db->getAll("SELECT * FROM " . $yp->table('back_replay') . " WHERE back_id = '$back_id' ORDER BY add_time ASC");
...
function action_back_replay ()
{
	...
	$db->query("INSERT INTO " . $yp->table('back_replay') . " (back_id, message, add_time, type) VALUES ('$back_id', '$message', '$add_time', 1)");
...

{
   $user = $GLOBALS['user'];
    ...
    $order=$db->getRow("select * FROM " . $yp->table('order_info') ."where order_id='$rec_id'");
   ...
   if($goods_id){
	...
	$goods = $db->getAll("SELECT o.*,g.* FROM " . $yp->table('order_goods') . " as o left join ".$yp->table('goods')." as g on o.goods_id = g.goods_id WHERE order_id = '$rec_id' and g.goods_id='$goods_id'");

	$order_goods=$db->getRow("select goods_number,goods_price from". $yp->table('order_goods') ."where order_id='$rec_id' and goods_id='$goods_id'");



   }else{

       $goods = $db->getAll("SELECT o.order_id,g.* FROM " . $yp->table('order_goods') . " as o left join ".$yp->table('goods')." as g on o.goods_id = g.goods_id WHERE order_id = '$rec_id'");
...
function action_back_order_act ()
{
   ...
   for($i=0;$i<4;$i++){
	...
	$sql = "select * from " . $yp->table('order_info') . " where order_id='$order_id' ";
	...
	if($back_type == 1)
	{
		...
		$sql = "insert into " . $yp->table('back_goods') . "(back_id, goods_id, goods_name, goods_sn, product_id, goods_attr, back_type, " . "back_goods_number, back_goods_price, status_back ) " . " values('$back_id', '$goods_id', '$goods_name', '$goods_sn', '$_REQUEST[product_id_tui]', '$_REQUEST[goods_attr_tui]', '0', " . " '$tui_goods_number', '$_REQUEST[tui_goods_price]', '5') ";
		$db->query($sql);
	}
	if($back_type == 4)
	{
	...
	if($back_type == 2)
	{
		...
		if($huan_count)
		{
			$sql = "insert into " . $yp->table('back_goods') . "(back_id, goods_id, goods_name, goods_sn, product_id, goods_attr, back_type, status_refund, back_goods_number, status_back) " . " values('$back_id', '$goods_id', '$goods_name', '$goods_sn', '$_REQUEST[product_id_tui]', '$_REQUEST[goods_attr_tui]', '1', '9', '$huan_count', '5') ";
			$db->query($sql);
			$parent_id_huan = $db->insert_id();
			foreach($_POST['product_id_huan'] as $pid_key => $pid_huan)
			{
				$sql = "insert into " . $yp->table('back_goods') . "(back_id, goods_id, goods_name, goods_sn, product_id, goods_attr,  back_type, parent_id, status_refund, back_goods_number, status_back) " . "values('$back_id', '$goods_id', '$goods_name', '$goods_sn',  '$pid_huan', '" . $_POST['goods_attr_huan'][$pid_key] . "', '2', '$parent_id_huan', '9', '1', '5')";
				$db->query($sql);
			}
		}
	}
	if($back_type == 3)
	{
		$have_weixiu = 1;
		$tui_goods_number = $_REQUEST['tui_goods_number'] ? intval($_REQUEST['tui_goods_number']) : 1;
		$sql = "insert into " . $yp->table('back_goods') . "(back_id, goods_id, goods_name, goods_sn, product_id, goods_attr, back_type, " . "back_goods_number, back_goods_price, status_back) " . " values('$back_id', '$goods_id', '$goods_name', '$goods_sn', '$_REQUEST[product_id_tui]', '$_REQUEST[goods_attr_tui]', '3', " . " '$tui_goods_number', '$_REQUEST[tui_goods_price]', '5') ";
		$db->query($sql);
	}
	// }
	...
	if($have_tuikuan)
	{
		...
		$sql = "update " . $yp->table('back_order') . " set refund_money_1= '$price_refund' where back_id='$back_id' ";
		$db->query($sql);
	}
	else
	{
		$sql = "update " . $yp->table('back_order') . " set status_refund= '9' where back_id='$back_id' ";
		$db->query($sql);
	}
	} 
	if($have_tuikuan2)
...
function action_back_list ()
{
	...
	$record_count = $db->getOne("SELECT COUNT(*) FROM " . $yp->table('back_order') . " WHERE user_id = '$user_id'");
...
//	include_once (ROOT_PATH . 'includes/lib_clips.php');
//	
        // 手机端暂时不显示虚拟团购  20161031开始显示  q-１　２　００　２９　１　２１　
	//$ex_where = " and user_id=$user_id and extension_code <> 'virtual_good'" ;
	...
	/* 已发货的订单 */
	$order_count['shipped'] = $db->GetOne("SELECT COUNT(*) FROM " . $yp->table('order_info') . " WHERE 1 $ex_where " . order_query_sql('shipped'));
	...
	/* 已取消的订单 */
	$order_count['canceled'] = $db->GetOne("SELECT COUNT(*) FROM " . $yp->table('order_info') . " WHERE 1 $ex_where " . order_query_sql('canceled'));
	$status['canceled'] = OS_CANCELED;
	
	/* 退款中的订单 */
	$order_count['payback'] = $db->GetOne("SELECT COUNT(*) FROM " . $yp->table('order_info') . " WHERE 1 $ex_where " . order_query_sql('payback'));
	$status['payback'] = PS_PAYBACK;
	
	/* 待评价的订单： */
	$order_count['await_comment'] = $db->GetOne("SELECT COUNT(*)" . " FROM " . $yp->table('order_info') . " WHERE 1 $ex_where " . order_query_sql('await_comment'));
...
function action_ajax_order_list(){
	...
	include_once (ROOT_PATH . 'includes/lib_clips.php');
        ...
        $json   = new JSON; 
        // 过滤虚拟商品
	//$where = " and o.extension_code <> 'virtual_good'";  // 20161031 prince 120029121
	$where = " ";
...
function action_ajax_telquery(){
    ...
    $amount = isset($_REQUEST['amount'])?trim($_REQUEST['amount']):'';
	$appkey = $db->getOne("SELECT appkey FROM " . $yp->table('user_account_huafei_config') . " limit 1 ");
	$openid = $db->getOne("SELECT appid FROM " . $yp->table('user_account_huafei_config') . " limit 1 ");

	include 'api/class.juhe.recharge.php'; //引入文件
	$recharge = new recharge($appkey,$openid);

	//根据手机号码以及面额查询商品信息
	$telQueryRes =$recharge->telquery($mobile_phone,$amount); #可以选择的面额5、10、20、30、50、100、300
	if($telQueryRes['error_code'] == '0'){
	   ...
	   $banlance = $db->getOne("SELECT banlance FROM " . $yp->table('user_account_huafei_config') . " limit 1 ");
	   if($banlance<$proinfo['inprice']){
			$result['error'] = 1;
			$result['message'] = '平台账户余额不足，请稍候再试';  
	   }
	   	die($json->encode($result));
		...
		$result['error'] = 1;
		$result['message'] = $telQueryRes['reason'];
		die($json->encode($result));
	}
	die($json->encode($result));
...
/* 商品评价/晒单 增加 by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1 */
function action_my_comment()
{
    ...
    $page = isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;
        //$where = " AND o.extension_code <> 'virtual_good' "; 20162031
        $where = "  ";
    ...
    $count = $db->getOne("SELECT COUNT(*) FROM " . $yp->table('order_goods') . " AS og 
						  LEFT JOIN " . $yp->table('order_info') . " AS o ON og.order_id=o.order_id ".
                                                 " LEFT JOIN " . $yp->table('comment') . " AS c ON og.rec_id=c.rec_id
    ...
    $page_count = ($count > 0) ? intval(ceil($count / $size)) : 1;
            ...
            $sql = "SELECT og.*, o.add_time, o.shipping_time_end, o.order_id, g.goods_thumb, g.shop_price, s.shaidan_id, s.pay_points AS 	shaidan_points, s.status AS shaidan_status, 
                            ...
                            FROM " . $yp->table('order_goods') . " AS og 
                            LEFT JOIN " . $yp->table('order_info') . " AS o ON og.order_id=o.order_id
                            LEFT JOIN " . $yp->table('goods') . " AS g ON og.goods_id=g.goods_id
                            LEFT JOIN " . $yp->table('shaidan') . " AS s ON og.rec_id=s.rec_id
                            LEFT JOIN " . $yp->table('comment') . " AS c ON og.rec_id=c.rec_id
                            LEFT JOIN " . $yp->table('supplier_shop_config') . " AS ssc ON ssc.supplier_id=g.supplier_id AND ssc.code='shop_name'
       ...
       $res = $db->getAll($sql);
        foreach ($res as $key=>$row) {
            ...
            $row['goods_tags'] = $db->getAll("SELECT * FROM " . $yp->table('goods_tag') . " WHERE goods_id = '$row[goods_id]'");
        ...
        foreach ($item_list as $key => $value) {
            ...
            if ($value['shaidan_id'] > 0) {
                $comment['shaidan'] = $GLOBALS['db']->getRow("SELECT * FROM " . $GLOBALS['yp']->table('shaidan') . " WHERE shaidan_id = '$value[shaidan_id]'");
                $comment['shaidan_img'] = $GLOBALS['db']->getAll("SELECT * FROM " . $GLOBALS['yp']->table('shaidan_img') . " WHERE shaidan_id = '$value[shaidan_id]'");
            ...
            if ($comment['comment_id'] > 0) {
                $parent_res = $GLOBALS['db']->getAll("SELECT * FROM " . $GLOBALS['yp']->table('comment') . " WHERE parent_id = '" . $comment['comment_id'] . "'");
...
function action_my_comment_send()
{
	...
	$rec_id = $_POST['rec_id'];
        //判断是否已经评价
        $res_comment_id = $db->getOne("select comment_id from ".$yp->table('comment') ." where rec_id = $rec_id and user_id = $user_id");
        if($res_comment_id){
	...
	$user_info = $db->getRow("SELECT * FROM " . $yp->table('users') . " WHERE user_id = '$user_id'");
	$comment_type = 0;
...
//	}
	...
	$buy_time = $db->getOne("SELECT o.add_time FROM " . $yp->table('order_info') . " AS o
							 LEFT JOIN " . $yp->table('order_goods') . " AS og ON o.order_id=og.order_id
	...
	if(is_array($_POST['tags_zi']))
	{
		foreach($_POST["tags_zi"] as $tag)
		{
			...
			$db->query("INSERT INTO " . $yp->table('goods_tag') . " (goods_id, tag_name, is_user, state) VALUES ('$id_value', '$tag', 1, '$status')");
			$tags[] = $db->insert_id();
		}
	}
...
//代码增加o_id
	$sql = "INSERT INTO " . $yp->table('comment') . "(comment_type, id_value, email, user_name, content, comment_rank, add_time, ip_address, user_id, status, rec_id, comment_tag, buy_time, hide_username, order_id)" . "VALUES ('$comment_type', '$id_value', '$email', '$user_name', '$content', '$comment_rank', '$add_time', '$ip_address', '$user_id', '$status', '$rec_id', '$comment_tag', '$buy_time', '$hide_username', '$o_id')";

	$db->query($sql);
	$db->query("UPDATE " . $yp->table('order_goods') . " SET comment_state = 1 WHERE rec_id = '$rec_id'");

	//代码增加
	if($o_id)
	{
		$o_sn =  $db->getOne("SELECT order_sn FROM " . $yp->table('order_info') . " 				
							 WHERE order_id = '$o_id'");
		$sql = "INSERT INTO " . $yp->table('shop_grade') . "(user_id, user_name, add_time,  server, send, shipping, order_id, order_sn)" . "VALUES ('$user_id', '$user_name', '$add_time', '$server', '$send', '$shipping', '$o_id', '$o_sn')";
		$db->query($sql);
	}
	//代码增加
...
function action_shaidan_send()
{
	...
	$goods = $db->getRow("SELECT o.rec_id,g.* FROM " . $yp->table('order_goods') . " as o left join ".$yp->table('goods')." as g on o.goods_id = g.goods_id WHERE rec_id = '$rec_id'");
        $goods['goods_thumb'] = get_image_path($goods['goods_id'],$goods['goods_thumb']);//mod by qq120-029-121 20160815
        $smarty->assign('shaidan_img', get_array($simgnum));
        $smarty->assign('goods', $goods);
...
function action_shaidan_save()
{
	...
	$hide_username = intval($_POST['hide_username']);
        $res_shaidan_id = $db->getOne("select shaidan_id from ".$yp->table('shaidan') ." where rec_id = $rec_id and user_id = $user_id");
        if($res_shaidan_id){
            echo "<script>alert('此订单商品您已经发表过晒单！');location='user.php?act=my_comment';</script>";
            exit();
        }else{
        $sql = "INSERT INTO " . $yp->table('shaidan') . "(rec_id, goods_id, user_id, title, message, add_time, status, hide_username)" . "VALUES ('$rec_id', '$goods_id', '$user_id', '$title', '$message', '$add_time', '$status', '$hide_username')";
        $db->query($sql);
             ...
             if (file_exists($path . $v["name"])) 
                ...
                { 
                    if(move_uploaded_file($v["tmp_name"],$path . $v["name"])){
                        ...
                        $sql = "INSERT INTO " . $yp->table('shaidan_img') . "(shaidan_id, `desc`, image, thumb)" . "VALUES ('$shaidan_id', '" . $v["name"] . "', '$path_img', '$thumb')";
                        $db->query($sql);
                    }else{
                            echo "<script>alert('图片上传失败，请重新提交！');self.location='user.php?act=shaidan_send&id=$rec_id';</script>";
                            exit();
	...
	{
		$info = $db->GetRow("SELECT * FROM " . $yp->table('shaidan') . " WHERE shaidan_id='$shaidan_id'");
		// 该商品第几位晒单者
		$res = $db->getAll("SELECT shaidan_id FROM " . $yp->table("shaidan") . " WHERE goods_id = '$info[goods_id]' ORDER BY add_time ASC");
		...
		if($info['is_points'] == 0 && $weizhi <= $_CFG['shaidan_pre_num'] && $imgnum >= $_CFG['shaidan_img_num'])
		{
			...
			$db->query("UPDATE " . $yp->table('shaidan') . " SET pay_points = '$pay_points', is_points = 1 WHERE shaidan_id = '$shaidan_id'");
			$db->query("INSERT INTO " . $yp->table('account_log') . "(user_id, rank_points, pay_points, change_time, change_desc, change_type) " . "VALUES ('$info[user_id]', 0, '" . $pay_points . "', " . gmtime() . ", '晒单获得积分', '99')");
			$log = $db->getRow("SELECT SUM(rank_points) AS rank_points, SUM(pay_points) AS pay_points FROM " . $yp->table("account_log") . " WHERE user_id = '$info[user_id]'");
			$db->query("UPDATE " . $yp->table('users') . " SET rank_points = '" . $log['rank_points'] . "', pay_points = '" . $log['pay_points'] . "' WHERE user_id = '$info[user_id]'");
		}
		
		$msg = '您的信息提交成功！';
            }

            $db->query("UPDATE " . $yp->table('order_goods') . " SET shaidan_state = 1 WHERE rec_id = '$rec_id'"); 
        }
...
function action_order_detail()
{
	...
	foreach($goods_list as $key => $value)
	{
		...
		/* 代码增加_start By demo.coolhong.com 今天优品多商户系统 qq 120029121 20161101 */
		if($value['extension_code'] == 'virtual_good')
		{
			$virtual_goods_card = $db->getAll("select gc.card_sn,gc.end_date,buy_date from " . $yp->table('virtual_goods_card') . " as gc join (select order_id,order_sn from " . $yp->table('order_info') . " where order_id='$order_id') as oi on gc.order_sn = oi.order_sn");
			...
			$goods_list[$key]['virtual_goods_card'] = $virtual_goods_card;
		}
		/* 代码增加_end By demo.coolhong.com 今天优品多商户系统 qq 120029121  20161101*/
	...
	if($order['order_amount'] > 0)
	{
		if($order['order_status'] == OS_UNCONFIRMED || $order['order_status'] == OS_CONFIRMED)
		{
			...
			if(($user['user_money'] + $user['credit_line'] > 0 &&  $_CFG['use_surplus'] == '1')) //20170520 by prince
			{
	...
	if($order['order_amount'] > 0 && $order['pay_status'] == PS_UNPAYED && $order['shipping_status'] == SS_UNSHIPPED)
	{
		...
		if(is_array($payment_list))
		{
			foreach($payment_list as $key => $payment)
			{
				...
				//20171101 add by princeqq120029121 start
				if($payment['pay_code'] == 'weixin'){
					$config_list = unserialize($payment['pay_config']);
					foreach ($config_list AS $config)
					{
						$thepayment[$config['name']] = $config['value'];
					}
					if($thepayment['enabled_h5pay']==1){
						$smarty->assign('iswei',      1); 
					}
				}
				//20171101 add by princeqq120029121 end
			}
		}
...
//	}
        $sql = "SELECT delivery_id,shipping_name,invoice_no  FROM ". $GLOBALS['yp']->table('delivery_order'). " WHERE order_id = '$order_id'AND user_id = '$user_id'";  
...
function action_cancel_order()
{
	...
	{
		yp_header("Location: user.php?act=order_list\n");
		exit();
	}
	else
	{
...
//	}
	...
	/* 获取默认收货ID */
	$address_id = $db->getOne("SELECT address_id FROM " . $yp->table('users') . " WHERE user_id='$user_id'");
...
function action_set_address(){
	...
	$yp = $GLOBALS['yp'];
        $user_id = $GLOBALS['user_id'];
        $address_id = empty($_REQUEST['address_id'])?0:intval($_REQUEST['address_id']);
        if($db->query("UPDATE " . $yp->table('users') . " set address_id = $address_id  WHERE user_id='$user_id'")){ 
...
function action_drop_consignee()
{
	...
	{
		yp_header("Location: user.php?act=address_list\n");
		exit();
	}
	else
	{
...
function action_collection_list()
{
	...
	$record_count = $db->getOne("SELECT COUNT(*) FROM " . $yp->table('collect_goods') . " WHERE user_id='$user_id' ORDER BY add_time DESC");
...
function action_delete_collection()
{
	...
	if($collection_id > 0)
	{
		$res = $db->query('DELETE FROM ' . $yp->table('collect_goods') . " WHERE rec_id='$collection_id' AND user_id ='$user_id'");
	}
        if($res){
            $result['message']="删除成功!";
        }else{
	...
	//yp_header("Location: user.php?act=collection_list\n");
        die($json->encode($result));
	exit();
}
...
function action_add_to_attention()
{
	...
	if($rec_id)
	{
		$db->query('UPDATE ' . $yp->table('collect_goods') . "SET is_attention = 1 WHERE rec_id='$rec_id' AND user_id ='$user_id'");
	}
	yp_header("Location: user.php?act=collection_list\n");
	exit();
}
...
function action_del_attention()
{
	...
	if($rec_id)
	{
		$db->query('UPDATE ' . $yp->table('collect_goods') . "SET is_attention = 0 WHERE rec_id='$rec_id' AND user_id ='$user_id'");
	}
	yp_header("Location: user.php?act=collection_list\n");
	exit();
}
...
function action_follow_shop()
{
	...
	$record_count = $db->getOne("SELECT COUNT(*) FROM " . $yp->table('supplier_guanzhu') . " WHERE userid='$user_id'");
...
function action_del_follow()
{
	...
	if($rec_id)
	{
		$res = $db->query('DELETE FROM ' . $yp->table('supplier_guanzhu') . " WHERE id='$rec_id' AND userid ='$user_id'");
	}
        if($res){
            $result['message'] ="删除成功!";
        }else{
        ...
        die($json->encode($result));
	//yp_header("Location: user.php?act=follow_shop\n");
	exit();
}
/* 显示留言列表 */
function action_message_list()
{
	...
	if($order_id)
	{
		$sql = "SELECT COUNT(*) FROM " . $yp->table('feedback') . " WHERE parent_id = 0 AND order_id = '$order_id' AND user_id = '$user_id'";
		$order_info = $db->getRow("SELECT * FROM " . $yp->table('order_info') . " WHERE order_id = '$order_id' AND user_id = '$user_id'");
		$order_info['url'] = 'user.php?act=order_detail&order_id=' . $order_id;
	}
	else
	{
		$sql = "SELECT COUNT(*) FROM " . $yp->table('feedback') . " WHERE parent_id = 0 AND user_id = '$user_id' AND user_name = '" . $_SESSION['user_name'] . "' AND order_id=0";
	}
...
function action_comment_list()
{
	...
	/* 获取用户留言的数量 */
	$sql = "SELECT COUNT(*) FROM " . $yp->table('comment') . " WHERE parent_id = 0 AND user_id = '$user_id'";
	$record_count = $db->getOne($sql);
	$pager = get_pager('user.php', array(
		'act' => $action
	), $record_count, $page, 5);
...
function action_act_del_tag()
{
	...
	yp_header("Location: user.php?act=tag_list\n");
	exit();
}
...
function action_booking_list()
{
	...
	/* 获取缺货登记的数量 */
	$sql = "SELECT COUNT(*) " . "FROM " . $yp->table('booking_goods') . " AS bg, " . $yp->table('goods') . " AS g " . "WHERE bg.goods_id = g.goods_id AND user_id = '$user_id'";
	$record_count = $db->getOne($sql);
	$pager = get_pager('user.php', array(
		'act' => $action
	), $record_count, $page);
...
function action_add_booking()
{
	...
	if($_GET['spec'] != '')
	{
		...
		$attr_list = array();
		$sql = "SELECT a.attr_name, g.attr_value " . "FROM " . $yp->table('goods_attr') . " AS g, " . $yp->table('attribute') . " AS a " . "WHERE g.attr_id = a.attr_id " . "AND g.goods_attr_id " . db_create_in($goods_attr_id);
		$res = $db->query($sql);
...
function action_act_del_booking()
{
	...
	if($id == 0 || $user_id == 0)
	{
		yp_header("Location: user.php?act=booking_list\n");
		exit();
	}
...
function action_account_raply()
{
	...
	//add by prince 20161126 q-1200-29121 start
	$sql = "SELECT * FROM " .$yp->table('users'). " WHERE user_id = '$user_id'";
    $ret = $db->getRow($sql);
	$user_agent = $_SERVER['HTTP_USER_AGENT'];
	if(!empty($ret['fake_id']) && !(strpos($user_agent, 'MicroMessenger') === false)){
		$smarty->assign('support_weixin','1');
		$smarty->assign('nickname', $ret['nickname']?$ret['nickname']:'此微信号');
	}
	//add by prince 20161126 q-1200-29121 end
	include_once (ROOT_PATH . 'includes/lib_clips.php');
...
/*话费充值界面  20160823*/
function action_account_huafei()
{
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
    ...
    $account_money = $db->getOne ( "SELECT account_money FROM " . $GLOBALS['yp']->table('user_account_huafei_config') . " WHERE `id` = 1" );//取设置金额
	if($user_info['account_money'] < 0 || $user_info['account_money'] >  $account_money){//用户充值限额为负
	
	$GLOBALS['db']->query("UPDATE " . $GLOBALS['yp']->table('users') . " SET account_money = 0   WHERE  user_id = '$user_id'");//充值限额重置为0
	
	}
	 $time = gmtime();
	 
	 if($user_info['account_cycle'] < $time){//充值周期过期
	
	 $GLOBALS['db']->query("UPDATE " . $GLOBALS['yp']->table('users') . " SET account_cycle = 0 , account_money = $account_money  WHERE  user_id = '$user_id'");
	 
	 }
	
	if($user_info['account_money'] == 0 && $user_info['account_cycle'] == 0){
	
	$GLOBALS['db']->query("UPDATE " . $GLOBALS['yp']->table('users') . " SET account_money = '$account_money'   WHERE  user_id = '$user_id'");

	}

   is_huafei($user_id);//根据设置 设置用户是否有充值权限
...
/* 对会员话费充值处理 */
function action_act_account_huafei()
{
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	...
	$mobile_phone = isset($_POST['mobile_phone']) ? floatval($_POST['mobile_phone']) : 0;
	...
	/* $mobile_phone    充值 手机号
	 ...
	 surplus_type =  0    平台余额充值
	 
	 surplus_type = 1   余额提现
	 
	 
	 surplus_type = 3    手机号码话费充值*/
	$sql = "SELECT * FROM " . $GLOBALS['yp']->table('users') . " WHERE user_id = '$user_id'";
    $user_info = $GLOBALS['db']->getRow($sql);//获取用户资料
    $account_money = $db->getOne ( "SELECT account_money FROM " . $GLOBALS['yp']->table('user_account_huafei_config') . " WHERE `id` = 1" );//取设置金额
	 if($user_info['is_huafei'] == 0){
	 
	  show_message('您所在的用户组暂未开放话费充值功能，您可以通过购买商品提升用户等级！！'); 
	...
	if($amount > $user_info['account_money'])
	{
	 echo "<script>alert('您此次充值金额大于您的充值额度，您还可以充值".$user_info['account_money']."元');window.location.href=\"user.php?act=account_huafei\";</script>";
	exit;
	}
	/* 变量初始化 */
	$surplus = array(
		'user_id' => $user_id,'process_type' => isset($_POST['surplus_type']) ? intval($_POST['surplus_type']) : 3,'payment_id' => isset($_POST['payment_id']) ? intval($_POST['payment_id']) : 0,'amount' => $amount,'mobile_phone' => $mobile_phone
	);
		...
		if($telQueryRes['error_code'] == '0'){
		 ...
		 $sql = 'INSERT INTO ' .$GLOBALS['yp']->table('user_account').
           ' (user_id, admin_user, amount, add_time, paid_time, admin_note, user_note, process_type, payment, is_paid, mobile_phone,chongzhifei)'.
            " VALUES ('$surplus[user_id]', '', '$amount', '".gmtime()."', 0, '', '$surplus[user_note]', '$surplus[process_type]', '$surplus[payment]', 0, '$surplus[mobile_phone]', '$chongzhifei')";
		...
		// 计算此次预付款需要支付的总金额
	    $account_zhekou = $db->getOne ( "SELECT account_zhekou FROM " . $GLOBALS['yp']->table('user_account_huafei_config') . " WHERE `id` = 1" );
		$order['order_amount'] = $amount * $account_zhekou / 100  + $payment_info['pay_fee'];
		
		// 记录支付log
		$order['log_id'] = insert_pay_log($surplus['rec_id'], $order['order_amount'], $type = 3, 0);
		...
		$smarty->assign('pay_amount', $order['order_amount']);//实付金额
		$smarty->display('user_transaction.dwt');
	}
/* 会员账目明细界面 */
function action_account_detail()
{
	...
	/* 获取记录条数 */
	$sql = "SELECT COUNT(*) FROM " . $yp->table('account_log') . " WHERE user_id = '$user_id'" . " AND $account_type <> 0 ";
	$record_count = $db->getOne($sql);
	...
	$smarty->assign('user_money', price_format($account_sum['user_money'], false));
	$smarty->assign('frozen_money', price_format($account_sum['frozen_money'], false));
	$smarty->assign('rank_points', abs($account_sum['rank_points']));
	$smarty->assign('pay_points', abs($account_sum['pay_points']));

	// 获取余额记录
	$pager['size']=1000;
	$account_log = array();
	$sql = "SELECT * FROM " . $yp->table('account_log') . " WHERE user_id = '$user_id'" . " AND $account_type <> 0 " . " ORDER BY log_id DESC";
	$res = $GLOBALS['db']->selectLimit($sql, $pager['size'], $pager['start']);
	while($row = $db->fetchRow($res))
	{
		$row['change_time'] = local_date($_CFG['date_format'], $row['change_time']);
		$row['user_money'] = $row['user_money'];
		$row['frozen_money'] = $row['frozen_money'];
		$row['rank_points'] = $row['rank_points'];
...
function action_account_log()
{
	...
	/* 获取记录条数 */
	$sql = "SELECT COUNT(*) FROM " . $yp->table('user_account') . " WHERE user_id = '$user_id'" . " AND process_type " . db_create_in(array(
...
function action_act_account()
{
	...
	/* 变量初始化 */
	$surplus = array(
		'user_id' => $user_id,'rec_id' => ! empty($_POST['rec_id']) ? intval($_POST['rec_id']) : 0,'process_type' => isset($_POST['surplus_type']) ? intval($_POST['surplus_type']) : 0,'payment_id' => isset($_POST['payment_id']) ? intval($_POST['payment_id']) : 0,'user_note' => isset($_POST['user_note']) ? trim($_POST['user_note']) : '',
        'real_name'    => isset($_POST['real_name'])    ? trim($_POST['real_name'])      : '',//qq 120 029 121 20161126
        'account_type' => isset($_POST['account_type'])    ? trim($_POST['account_type'])      : '',//qq 120 029 121 20161126
        'account'      => isset($_POST['account'])    ? trim($_POST['account'])      : '',//qq 120 029 121 20161123
        'mobile_phone' => isset($_POST['mobile_phone'])    ? trim($_POST['mobile_phone'])      : '',//qq 120 029 121 20161126
		'amount' => $amount
	);
	...
	if($surplus['process_type'] == 1)
	{
		...
		/* 判断是否有足够的余额的进行退款的操作 */
		$account_sum = get_user_surplus($user_id);
		$sur_amount=$account_sum['user_money'];
...
function action_cancel()
{
	...
	if($id == 0 || $user_id == 0)
	{
		yp_header("Location: user.php?act=account_log\n");
		exit();
	}
...
function action_pay()
{
	...
	if($payment_id == 0)
	{
		yp_header("Location: user.php?act=account_deposit&id=" . $surplus_id . "\n");
		exit();
	}
	...
	if(! empty($payment_info))
	{
		...
		$order_amount = $db->getOne("SELECT order_amount FROM " . $yp->table('pay_log') . " WHERE log_id = '$order[log_id]'");
		if($order_amount != $order['order_amount'])
		{
			$db->query("UPDATE " . $yp->table('pay_log') . " SET order_amount = '$order[order_amount]' WHERE log_id = '$order[log_id]'");
		}
...
function action_collect()
{
	...
	{
		/* 检查是否已经存在于用户的收藏夹 */
		$sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('collect_goods') . " WHERE user_id='$_SESSION[user_id]' AND goods_id = '$goods_id'";
		...
		{
			$time = gmtime();
			$sql = "INSERT INTO " . $GLOBALS['yp']->table('collect_goods') . " (user_id, goods_id, add_time)" . "VALUES ('$_SESSION[user_id]', '$goods_id', '$time')";
...
function action_del_msg()
{
	...
	if($id > 0)
	{
		$sql = 'SELECT user_id, message_img FROM ' . $yp->table('feedback') . " WHERE msg_id = '$id' LIMIT 1";
		$row = $db->getRow($sql);
		if($row && $row['user_id'] == $user_id)
		{
			...
			$sql = "DELETE FROM " . $yp->table('feedback') . " WHERE msg_id = '$id' OR parent_id = '$id'";
			$db->query($sql);
		}
	}
	yp_header("Location: user.php?act=message_list&order_id=$order_id\n");
	exit();
}
...
function action_del_cmt()
{
	...
	if($id > 0)
	{
		$sql = "DELETE FROM " . $yp->table('comment') . " WHERE comment_id = '$id' AND user_id = '$user_id'";
		$db->query($sql);
	}
	yp_header("Location: user.php?act=comment_list\n");
	exit();
}
...
function action_merge_order()
{
	...
	$sql = "select supplier_id from " . $yp->table('order_info') . " where order_sn='$from_order' ";
	$supplier_id_from = $db->getOne($sql);
	$sql = "select supplier_id from " . $yp->table('order_info') . " where order_sn='$to_order' ";
...
function action_return_to_cart()
{
	...
	/* 检查订单是否属于该用户 */
	$order_user = $db->getOne("SELECT user_id FROM " . $yp->table('order_info') . " WHERE order_id = '$order_id'");
	if(empty($order_user))
	{
		$result['error'] = 1;
...
function action_act_edit_surplus()
{
	...
	$sql = "UPDATE " . $yp->table('pay_log') . " SET order_amount = '".$order['order_amount']."' WHERE order_id='$order_id'";//20170520
	$db->query($sql);//20170520
	...
	/* 跳转 */
	yp_header('Location: user.php?act=order_detail&order_id=' . $order_id . "\n");
	exit();
}
...
function action_act_edit_payment()
{
	...
	$sql = "SELECT pay_id FROM " . $yp->table('payment') . " WHERE pay_code = '" . $_POST['pay_code'] . "'";
	$row = $db->getRow($sql);
	/* 检查支付方式 */
	$pay_id = $row['pay_id'];
	if($pay_id <= 0)
	...
	{
		yp_header("Location: user.php?act=order_detail&order_id=$order_id&is_pay=$is_pay\n");
		exit();
	}
	...
	$sql = "UPDATE " . $yp->table('order_info') . " SET pay_id='$pay_id', pay_name='$payment_info[pay_name]', pay_fee='$pay_fee', order_amount='$order_amount'" . " WHERE order_id = '$order_id'";
	$db->query($sql);
...
function action_save_order_address()
{
	...
	{
		yp_header('Location: user.php?act=order_detail&order_id=' . $address['order_id'] . "\n");
		exit();
	}
	else
	{
...
function action_bonus()
{
	...
	$page = isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;
	$record_count = $db->getOne("SELECT COUNT(*) FROM " . $yp->table('user_bonus') . " WHERE user_id = '$user_id'");
...
function action_book_goods ()
{
	...
	{
		$sql = "SELECT user_id,goods_id FROM " . $GLOBALS['yp']->table('booking_goods') . " WHERE user_id='$_SESSION[user_id]' AND is_dispose=0 AND goods_id = '$goods_id'";
		...
		{
			$sql = "SELECT email,mobile_phone FROM " . $GLOBALS['yp']->table('users') . " WHERE user_id='$_SESSION[user_id]'";
...
function action_add_book_goods ()
{
	...
	$tel = $_GET['tel'];
	$email = $_GET['em'];
	if(! preg_match("/^1(3|5|8)[0-9]{1}[0-9]{8}$|15[0189]{1}[0-9]{8}$|189[0-9]{8}$|17[0-9]{1}[0-9]{8}$/", $tel))
	{
		$result['error'] = 0;
	...
	{
		$time = gmtime();
		$sql = "INSERT INTO " . $yp->table('booking_goods') . " (user_id,email,tel,goods_id,goods_number,booking_time,link_man) VALUES ('$_SESSION[user_id]','$email','$tel','$goods_id','$number','$time','$_SESSION[user_name]')";
		if($db->query($sql))
		{
			$result['error'] = 2;
			$result['message'] = "登记成功。";
...
function action_affiliate()
{
	...
	if(empty($goodsid))
	{
		...
		$sql = "SELECT value FROM ".$yp->table('shop_config')." WHERE code='affiliate'";
		...
		if(1)
		{
			...
			$sqlcount = "SELECT count(*) FROM " . $yp->table('order_info') . " o" . " LEFT JOIN" . $yp->table('users') . " u ON o.user_id = u.user_id" . " LEFT JOIN " . $yp->table('affiliate_log') . " a ON o.order_id = a.order_id" . " WHERE o.user_id > 0 AND  a.user_id = '$user_id' AND o.is_separate =1 and a.money>0 ";
			
			$sql = "SELECT o.*, a.log_id, a.user_id as suid,  a.user_name as auser, a.money, a.point, a.separate_type FROM " . $yp->table('order_info') . " o" . " LEFT JOIN" . $yp->table('users') . " u ON o.user_id = u.user_id" . " LEFT JOIN " . $yp->table('affiliate_log') . " a ON o.order_id = a.order_id" . " WHERE o.user_id > 0 AND  a.user_id = '$user_id' AND o.is_separate=1 and a.money>0 " . " ORDER BY order_id DESC";
			...
			$affiliate_intro = nl2br(sprintf($_LANG['affiliate_intro'][2]));
...
function action_email_list()
{
	...
	$ck = $db->getRow("SELECT * FROM " . $yp->table('email_list') . " WHERE email = '$email'");
	if($job == 'add')
	{
		if(empty($ck))
		{
			$hash = substr(md5(time()), 1, 10);
			$sql = "INSERT INTO " . $yp->table('email_list') . " (email, stat, hash) VALUES ('$email', 0, '$hash')";
			$db->query($sql);
			$info = $_LANG['email_check'];
			$url = $yp->url() . "user.php?act=email_list&job=add_check&hash=$hash&email=$email";
		...
		{
			$hash = substr(md5(time()), 1, 10);
			$sql = "UPDATE " . $yp->table('email_list') . "SET hash = '$hash' WHERE email = '$email'";
			$db->query($sql);
	...
	elseif($job == 'del')
	{
		...
		elseif($ck['stat'] == 1)
		{
			...
			$db->query($sql);
			$info = $_LANG['email_check'];
			$url = $yp->url() . "user.php?act=email_list&job=del_check&hash=$hash&email=$email";
			send_mail('', $email, $_LANG['check_mail'], sprintf($_LANG['check_mail_content'], $email, $_CFG['shop_name'], $url, $url, $_CFG['shop_name'], local_date('Y-m-d')), 1);
		}
		else
		{
	...
	elseif($job == 'add_check')
	{
		...
		else
		{
			...
			{
				$sql = "UPDATE " . $yp->table('email_list') . "SET stat = 1 WHERE email = '$email'";
				$db->query($sql);
				$info = $_LANG['email_checked'];
			}
			else
	...
	elseif($job == 'del_check')
	{
		...
		elseif($ck['stat'] == 1)
		{
			if($_GET['hash'] == $ck['hash'])
			{
				$sql = "DELETE FROM " . $yp->table('email_list') . "WHERE email = '$email'";
				$db->query($sql);
...
function action_track_packages()
{
	...
	$sql = "SELECT order_id,order_sn,invoice_no,shipping_id FROM " . $yp->table('order_info') . " WHERE user_id = '$user_id' AND shipping_status = '" . SS_SHIPPED . "'";
	$res = $db->query($sql);
	$record_count = 0;
...
function action_order_query()
{
	...
	$sql = "SELECT order_id, order_status, shipping_status, pay_status, " . " shipping_time, shipping_id, invoice_no, user_id " . " FROM " . $yp->table('order_info') . " WHERE order_sn = '$order_sn' LIMIT 1";
	...
	{
		$sql = "SELECT shipping_code FROM " . $yp->table('shipping') . " WHERE shipping_id = '$row[shipping_id]'";
...
function action_transform_points()
{
	...
	$sql = "SELECT user_id, user_name, pay_points, rank_points FROM " . $yp->table('users') . " WHERE user_id='$user_id'";
	$row = $db->getRow($sql);
...
function action_act_transform_points()
{
	...
	/* 取出用户数据 */
	$sql = "SELECT user_name, user_id, pay_points, rank_points FROM " . $yp->table('users') . " WHERE user_id='$user_id'";
	$row = $db->getRow($sql);
...
function action_act_transform_ucenter_points()
{
	...
	if($result === true)
	{
		$sql = "UPDATE " . $yp->table('users') . " SET {$shop_points[$fromcredits]}={$shop_points[$fromcredits]}-'$exchange_amount' WHERE user_id='{$row['user_id']}'";
		$db->query($sql);
		$sql = "INSERT INTO " . $yp->table('account_log') . "(user_id, {$shop_points[$fromcredits]}, change_time, change_desc, change_type)" . " VALUES ('{$row['user_id']}', '-$exchange_amount', '" . gmtime() . "', '" . $cfg['uc_lang']['exchange'] . "', '98')";
		$db->query($sql);
...
function action_vc_login()
{
	...
	$user_info['surplus']=$surplus_amount;
...
function action_vc_login_act()
{
	...
	$sql = "select vc.*, vt.type_money, vt.use_start_date, vt.use_end_date from " . $yp->table('valuecard') . " AS vc " . " left join " . $yp->table('valuecard_type') . " AS vt " . "on vc.vc_type_id = vt.type_id where vc.vc_sn= '$vc_sn' ";
	$vcrow = $db->getRow($sql);
	if(! $vcrow)
	{
	...
	$sql = 'INSERT INTO ' . $GLOBALS['yp']->table('user_account') . ' (user_id, admin_user, amount, add_time, paid_time, admin_note, user_note, process_type, payment, is_paid)' . " VALUES ('$user_id', '', '$vcrow[type_money]', '" . gmtime() . "', '" . gmtime() . "', '', '储值卡充值', '0', '储值卡号：$vc_sn', 1)";
	$GLOBALS['db']->query($sql);
	log_account_change($user_id, $vcrow['type_money'], 0, 0, 0, '储值卡充值，卡号：' . $vc_sn, ACT_OTHER);
	
	$sql = "update " . $yp->table('valuecard') . " set user_id='$user_id', used_time='$nowtime' where vc_id='$vcrow[vc_id]' ";
	$db->query($sql);
...
function action_tg_login_act()
{
	...
	$sql = "select tg.*, tt.type_money, tt.type_money_count, tt.use_start_date, tt.use_end_date from " . $yp->table('takegoods') . " AS tg " . " left join " . $yp->table('takegoods_type') . " AS tt " . "on tg.type_id = tt.type_id where tg.tg_sn= '$tg_sn' ";
	$tgrow = $db->getRow($sql);
	if(! $tgrow)
	{
	...
	yp_header("Location:takegoods.php");
}
...
function action_tg_order()
{
	...
	$record_count = $db->getOne("SELECT COUNT(*) FROM " . $yp->table('takegoods_order') . " WHERE user_id = '$user_id'");
...
function action_tg_order_confirm()
{
	...
	$sql = "update " . $yp->table('takegoods_order') . " set order_status='2' where rec_id= '$id' ";
	$db->query($sql);
...
function action_check_surplus_open()
{
	...
	$sql = 'SELECT `is_surplus_open`' . 'FROM '. $GLOBALS['yp']->table('users') .'' . 'WHERE `user_id` = \'' . $_SESSION['user_id'] . '\'' . 'LIMIT 1';
...
function action_verify_surplus_password()
{
	...
	$sql = 'SELECT COUNT( * )' . 'FROM '. $GLOBALS['yp']->table('users') .'' . 'WHERE `user_id` = \'' . $_SESSION['user_id'] . '\'' . 'AND `surplus_password` = \'' . md5($_GET['surplus_password']) . '\'';
	$count = $GLOBALS['db']->getOne($sql);
	echo $count;
	exit();
}
...
function action_open_surplus_password()
{
	...
	if(isset($_GET['surplus_password']))
	{
		...
		$sql = 'UPDATE ' . $yp->table('users') . ' SET `surplus_password`=\'' . $surplus_password . '\',`is_surplus_open`=\'1\' WHERE `user_id`=\'' . $user_id . '\'';
		$db->query($sql);
...
function action_close_surplus_password()
{
	...
	if(isset($_GET['surplus_password']))
	{
		$surplus_password = trim($_GET['surplus_password']);
		$surplus_password = md5($surplus_password);
		$sql = 'UPDATE ' . $yp->table('users') . ' SET `is_surplus_open` = 0 WHERE `user_id` = \'' . $user_id . '\' AND `surplus_password` = \'' . $surplus_password . '\'';
		$db->query($sql);
		$affected_rows = $db->affected_rows();
		if($affected_rows == 1)
		{
		...
		if($_REQUEST['course'] == 'update')
		{
			if(isset($_POST['prev_surplus_password']) && isset($_POST['new_surplus_password1']) && isset($_POST['new_surplus_password2']))
			{
				...
				if($new_surplus_password1 == $new_surplus_password2)
				{
					...
					{
						$sql = 'UPDATE ' . $GLOBALS['yp']->table('users') . ' SET `surplus_password` = \'' . md5($new_surplus_password1) . '\' WHERE `user_id` = \'' . $user_id . '\' AND `surplus_password` = \'' . md5($prev_surplus_password) . '\' LIMIT 1';
		...
		elseif($_REQUEST['course'] == 'reset')
		{
			if(isset($_REQUEST['verify_method']))
			{
				...
				if($_REQUEST['verify_method'] == 'phone')
				{
					...
					{
						$sql = 'SELECT mobile_phone FROM ' . $GLOBALS['yp']->table('users') . ' WHERE `user_id` = \'' . $user_id . '\' LIMIT 1';
						$mobile_phone = $db->getOne($sql);
						$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['yp']->table('verifycode') . ' WHERE `mobile` = \'' . $mobile_phone . '\' AND `verifycode` = \'' . $v_code . '\' AND `status` = 1' . ' AND `dateline` + \'86400\' > \'' . gmtime() . '\' LIMIT 1';
						if($db->getOne($sql) == 0)
						{
						...
						{
							$sql = 'DELETE FROM ' . $GLOBALS['yp']->table('verifycode') . ' WHERE `mobile` = \'' . $mobile_phone . '\'';
							$db->query($sql);
							$validated = true;
						}
					}
				...
				elseif($_REQUEST['verify_method'] == 'email')
				{
					...
					{
						$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['yp']->table('email') . ' WHERE `hash` = \'' . $hash . '\'' . ' AND `email` = \'' . $_SESSION['email'] . '\'' . ' LIMIT 1';
						if($GLOBALS['db']->getOne($sql) == 0)
						{
							show_message('非法重置余额支付密码操作！');
						}
						...
						{
							$sql = 'DELETE FROM ' . $GLOBALS['yp']->table('email') . ' WHERE `email` = \'' . $_SESSION['email'] . '\'';
							$db->query($sql);
							$validated = true;
						}
					}
				...
				if($validated)
				{
					if(isset($_POST['surplus_password1']) && isset($_POST['surplus_password2']))
					{
						...
						{
							$sql = 'UPDATE ' . $GLOBALS['yp']->table('users') . ' SET `surplus_password` = \'' . md5($surplus_password1) . '\' WHERE `user_id` = \'' . $user_id . '\' LIMIT 1';
							$GLOBALS['db']->query($sql);
							if($GLOBALS['db']->affected_rows() == 1)
							{
								show_message('余额支付密码修改成功！', '返回', 'user.php?act=account_security', 'success');
...
function action_act_forget_surplus_password()
{
	...
	else
	{
		...
		if($verify_method == 'phone')
		{
			...
			$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['yp']->table('verifycode') . ' WHERE `mobile` = \'' . $v_phone . '\' AND `verifycode` = \'' . $v_code . '\' AND `status` = 1' . ' AND dateline + 86400 > \'' . gmtime() . '\'';
			if($GLOBALS['db']->getOne($sql) == 0)
			{
				show_message('手机号和验证码不匹配，请重新输入！');
			}
		...
		elseif($verify_method == 'email')
		{
			...
			{
				$sql = 'SELECT `user_name`,`email` ' . ' FROM ' . $GLOBALS['yp']->table('users') . ' WHERE `user_id` = \'' . $user_id . '\'';
				$row = $GLOBALS['db']->getRow($sql);
				...
				$reset_link = $GLOBALS['yp']->url() . 'user.php?act=verify_reset_surplus_email' . '&hash=' . $hash;
				...
				{
					$add_time = gmtime();
					$sql = 'INSERT INTO ' . $GLOBALS['yp']->table('email') . '(`email`,`hash`,`add_time`,`user_id`)' . 'VALUES(\'' . $v_email . '\',\'' . $hash . '\',\'' . $add_time . '\',\'' . $user_id . '\')';
					$GLOBALS['db']->query($sql);
					if($GLOBALS['db']->affected_rows() == 1)
					{
...
function action_get_verify_code()
{
	...
	$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['yp']->table('users') . ' WHERE `user_id` = \'' . $user_id . '\' AND `mobile_phone` = \'' . $phone . '\'';
	$count = $GLOBALS['db']->getOne($sql);
	...
	else
	{
		...
		$templateCode=$GLOBALS['_CFG']['sms_register_dayu'];
		$ret = sendSMS($phone, $content,'', '',$templateParam , $templateCode);//已支持阿里大于 QQ 120029121
		
		$sql = 'INSERT INTO ' . $yp->table('verifycode') . '(`mobile`, `getip`, `verifycode`, `dateline`) VALUES (\'' . $phone . '\',\'' . real_ip() . '\',\'' . $verifycode . '\',\'' . gmtime() . '\')';
		$db->query($sql);
...
function action_verify_reset_surplus_email()
{
	...
	else
	{
		...
		$sql = 'SELECT `hash`,`add_time` FROM ' . $GLOBALS['yp']->table('email') . ' WHERE `hash` = \'' . $hash . '\'' . ' LIMIT 1';
		$row = $GLOBALS['db']->getRow($sql);
		if($row)
		{
...
function get_takegoods_orders ($user_id, $num = 10, $start = 0)
{
	...
	$sql = "SELECT * " . " FROM " . $GLOBALS['yp']->table('takegoods_order') . " WHERE user_id = '$user_id' ORDER BY rec_id DESC";
	$res = $GLOBALS['db']->SelectLimit($sql, $num, $start);
	
	while($row = $GLOBALS['db']->fetchRow($res))
	{
		$row['country_name'] = $GLOBALS['db']->getOne("select region_name from " . $GLOBALS['yp']->table('region') . " where region_id='$row[country]' ");
		$row['province_name'] = $GLOBALS['db']->getOne("select region_name from " . $GLOBALS['yp']->table('region') . " where region_id='$row[province]' ");
		$row['city_name'] = $GLOBALS['db']->getOne("select region_name from " . $GLOBALS['yp']->table('region') . " where region_id='$row[city]' ");
		$row['district_name'] = $GLOBALS['db']->getOne("select region_name from " . $GLOBALS['yp']->table('region') . " where region_id='$row[district]' ");
...
{
	$chars = "/^13[0-9]{1}[0-9]{8}$|15[0-9]{1}[0-9]{8}$|18[0-9]{1}[0-9]{8}$|17[0-9]{1}[0-9]{8}$|14[0-9]{1}[0-9]{8}$/";
	if(preg_match($chars, $phone))//寒冰 20171006  手机端登陆增加176 号段
	{
		return true;
	}
}
...
function get_goods_info_wap($goods_id){
    $sql = "select * from ".$GLOBALS['yp']->table('goods')." where goods_id=".$goods_id;
    $goods = $GLOBALS['db']->getRow($sql);
    $goods['goods_thumb'] = get_image_path($goods_id,$goods['goods_thumb']);//mod by qq120-029-121 20160815
    if($goods['supplier_id'] == '0'){
        $goods['supplier_name']=" 网站自营";
    }else{
        $sqla = "select supplier_name from ".$GLOBALS['yp']->table('supplier')." where supplier_id = ".$goods['supplier_id'];
...
function get_order_id_wap($rec_id){
    $sql = "select order_id from ".$GLOBALS['yp']->table('order_goods')." where rec_id=".$rec_id;
...
function get_regions_wap($region_id){
    $sql = 'SELECT region_id,region_name FROM ' . $GLOBALS['yp']->table('region') .
...
//判断会员是否可以使用充值
function is_huafei($user_id)
{
   
	$sql = "SELECT user_rank FROM " . $GLOBALS['yp']->table('user_account_huafei_config') . " WHERE id = 1 ";
	$distrib_rank = $GLOBALS['db']->getOne($sql);
	
	if($distrib_rank == -1)
	{
		 
		$GLOBALS['db']->query("UPDATE " . $GLOBALS['yp']->table('users') . " SET is_huafei = 1 WHERE is_huafei = 0");
	}
	else
	{
		 ...
		 $GLOBALS['db']->query("UPDATE " . $GLOBALS['yp']->table('users') . " SET is_huafei = 0 WHERE is_huafei = 1 AND " . "(".$fx_where.")");
		 
		 $GLOBALS['db']->query("UPDATE " . $GLOBALS['yp']->table('users') . " SET is_huafei = 1 WHERE is_huafei = 0 AND " . "(".$ex_where.")");	
	}
	$sql = "SELECT is_huafei FROM " . $GLOBALS['yp']->table('users') . " WHERE user_id = '$user_id'";
	return $GLOBALS['db']->getOne($sql);
}
...
function get_user_backorders ($user_id, $num = 10, $start = 0)
{
	...
	$sql = "SELECT bo.*, g.goods_name " . " FROM " . $GLOBALS['yp']->table('back_order') . " AS bo left join " . $GLOBALS['yp']->table('goods') . " AS g " . " on bo.goods_id=g.goods_id  " . " WHERE user_id = '$user_id' ORDER BY add_time DESC";
	$res = $GLOBALS['db']->SelectLimit($sql, $num, $start);
	...
	while($row = $GLOBALS['db']->fetchRow($res))
	{
		...
		$sql_goods = "SELECT * FROM " . $GLOBALS['yp']->table('back_goods') . " WHERE back_id = " . $row['back_id'];
		...
		foreach($row['goods_list'] as $k=>$v){
			 ...
			 $sql="select goods_thumb from".$GLOBALS['yp']->table('goods') ."where goods_id= $v[goods_id]";
...
function action_user_rank()
{
	...
	$user_rank = $GLOBALS['db']->getAll("select * from " . $GLOBALS['yp']->table('user_rank') );
	foreach($user_rank as  $key=>$value){
		$user_rank[$key]['discount']=round($user_rank[$key]['discount']/10,2);
	}
