...
switch($type) {
	...
	case 'location':
        ...
        $reMsg = get_supplier_city($content['x'], $content['y']);

        $newsData = array();
  	 https://newdemo.coolhong.com/mobile/supplier_near.php?act=map&supplier_id=2
       if($reMsg){
           	
				$newsData[] = array("Title"=>"为您推荐附近商家：","Description"=>"","PicUrl"=>"","Url"=>$pc_url."mobile/supplier_near.php");
                $newsData[] = array("Title"=>$reMsg[0]['supplier_name']."\n地址：".$reMsg[0]['shop_country']."-".$reMsg[0]['shop_province']."-".$reMsg[0]['shop_city']."-".$reMsg[0]['shop_address']."\n距您：".$reMsg[0]['distance_'],"Description"=>"","PicUrl"=>(strpos($reMsg[0]['shop_logo'],'http') !== false ? $reMsg[0]['shop_logo'] : $pc_url.$reMsg[0]['shop_logo']),"Url"=>$pc_url."mobile/supplier_near.php?act=map&supplier_id=".$reMsg[0]['supplier_id']);
                $newsData[] = array("Title"=>$reMsg[1]['supplier_name']."\n地址：".$reMsg[1]['shop_country']."-".$reMsg[1]['shop_province']."-".$reMsg[1]['shop_city']."-".$reMsg[1]['shop_address']."\n距您：".$reMsg[1]['distance_'],"Description"=>"","PicUrl"=>(strpos($reMsg[1]['shop_logo'],'http') !== false ? $reMsg[1]['shop_logo'] : $pc_url.$reMsg[1]['shop_logo']),"Url"=>$pc_url."mobile/supplier_near.php?act=map&supplier_id=".$reMsg[1]['supplier_id']);
...
if ($event['event'] == "CLICK"){
	...
	if(strpos($content,"article_") !== false){
		...
		$newsData[0]['PicUrl'] = (strpos($artInfo['file_url'], 'http') !== false ? $artInfo['file_url'] : $pc_url.$artInfo['file_url']);
...
function get_supplier_city($latitude, $longitude){
		...
		if($latitude && $longitude){
			...
			include_once(ROOT_PATH. "includes/Geohash.php");
			// 获取所有店铺
			$sql="SELECT * FROM ". $GLOBALS['yp']->table("supplier") ." WHERE status=1 AND wx_latitude<>'' AND wx_longitude<>''";//20170101
			...
			$supplier_list = array_slice($supplier_list, 0, 2);
