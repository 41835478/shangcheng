...
/*获取消息模板信息*/
$sql = "SELECT * FROM " .$GLOBALS['yp']->table('weixin_remind'). " WHERE id = 1";
...
if($notice == 1){
	...
	if($wx_user_id > 0 &&  $ret_re['buynotice'] == 1) {
		...
		$query_sql = "SELECT * FROM " . $GLOBALS['yp']->table('users') . " WHERE user_id = '$wx_user_id'";
		...
		$orders = $db->getRow("SELECT * FROM " . $GLOBALS['yp']->table('order_info') . " WHERE `order_id` = '$order_id' ");
	
		$order_goods = $db->getAll("SELECT * FROM " . $GLOBALS['yp']->table('order_goods') . "  WHERE `order_id` = '$order_id'");
...
elseif($notice == 2){
	...
	if($ret_re['buysuppliertice'] == 1) { //支付成功提醒管理员/商家
		 ...
		 $user_id_sql = "SELECT `user_id` FROM  " . $GLOBALS['yp']->table('supplier') . " WHERE `supplier_id`='$supplier_id'";
	
		 $user_id =  $GLOBALS['db'] -> getOne($user_id_sql);
		 
		 $user_wxid_sql = "SELECT `fake_id` FROM " . $GLOBALS['yp']->table('users') . " WHERE user_id = '$user_id'";
		 ...
		 $admin_id_sql = "SELECT `admin_id` FROM  " . $GLOBALS['yp']->table('weixin_config') . " WHERE `id`=1";
	
		 $admin_id =  $GLOBALS['db'] -> getOne($admin_id_sql);
		 
		 $admin_wxid_sql = "SELECT `fake_id` FROM " . $GLOBALS['yp']->table('users') . " WHERE user_id = '$admin_id'";
	...
	$get_sql = "SELECT parent_id,nickname FROM " . $GLOBALS['yp']->table('users') . " WHERE user_id = '$wx_user_id'";;

	$get_info =  $GLOBALS['db'] -> getRow($get_sql);
	
	$up_uid =  $get_info['parent_id'];
	
	$my_wxname =  $get_info['nickname'];
    ...
    if($up_uid > 0 &&  $ret_re['payuptice'] == 1) { //付款成功提醒上级
	  ...
	  if($orders['pay_status'] == 0) {
	    ...
	    for ($i=0; $i < $num; $i++){
	   	...
	   	$query_sql = "SELECT * FROM " . $GLOBALS['yp']->table('users') . " WHERE user_id = '$up_uid'";
	...
	if($ret->errmsg != 'ok' ||  empty($ret->errmsg)) {
		...
		$qu_wxid = "SELECT parent_id FROM " . $GLOBALS['yp']->table('users') . " WHERE user_id = '$up_uid'";;
...
elseif($notice == 5){
	...
	$query_sql = "SELECT user_id " ." FROM  " . $GLOBALS['yp']->table('users') ." WHERE parent_id=".$up_uid."   ";
	
    $user_id = $db->getOne($query_sql);
...
elseif($notice == 6){
	...
	$wx_id = $_GET['is_one_user'];
	
	$order_id = $_GET['order_id'];
	
	if($wx_id) {
	
		$query_sql = "SELECT * FROM " . $GLOBALS['yp']->table('users') . " WHERE fake_id = '$wx_id'";
	   ...
	   $wx_consignee = "收件人:".$orders['consignee']."   电话：".$orders['mobile'];
		...
		$w_title = "亲爱的【".$nickname."】您有一笔订单尚未付款，请您尽快核实完成付款！！";
		$pay_remark = "请您尽快完成付款，我们将第一时间为您安排发货。"	;	
		$post_msg = '{
		   "touser":"'.$wxid.'",
		   "template_id":"'.$ret_re['no_paymsg'].'",
		   "url":"'.$w_url.'",
		   "topcolor":"#FF0000",
			   "data":{
					   "frist": {
