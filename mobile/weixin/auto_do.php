...
$baseurl = $_SERVER['SERVER_NAME'] ? HTTP_TYPE."://".$_SERVER['SERVER_NAME']."/" : HTTP_TYPE."://".$_SERVER['HTTP_HOST']."/";
$t = time();
$type = intval($_GET['type']);
$rs = $db->getRow ( "SELECT * FROM " . $GLOBALS['yp']->table('weixin_corn') . " WHERE `issend` = 0 and `sendtype`={$type} order by sendtime desc" );
if($rs){
	$sql = "SELECT * FROM " . $GLOBALS['yp']->table('weixin_config');
	...
	for($i = 0; $i < count($list); $i++)
	{
		...
		$where_user = (!empty($_GET['is_one_user']) ? " AND user_id='".$_GET['is_one_user']."'" : '');
		if($type == 1){
			if(isset($_GET['is_affiliate']) && $_GET['is_affiliate'] == 1)
			{
				 $list = $db->getAll("SELECT * FROM " . $GLOBALS['yp']->table('weixin_corn') . " WHERE `issend` = 0 and `sendtype`={$type} order by sendtime desc");
				 foreach($list as $val)
				 {
					  ...
					  $db->query("UPDATE " . $GLOBALS['yp']->table('weixin_corn') . " SET issend=2 where id ={$val['id']}");
				 }
			}
			else
			{
				$user = $db->getAll("SELECT fake_id FROM " . $GLOBALS['yp']->table('users') . " WHERE isfollow=1 and access_token!='' and expire_in>'{$t}' ".$where_user." order by expire_in desc");
			}
		}else{
		...
		if($user){
			...
			$db->query("UPDATE " . $GLOBALS['yp']->table('weixin_corn') . " SET issend=2 where id ={$rs['id']}");
		}
		if($_GET['ajax'] == 1){
