...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order, $limit='')
{
    ...
    $sql = "SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, " .
                ...
                "g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.goods_img FROM " . $GLOBALS['yp']->table('ypmart_distrib_goods') . " as dg left join " . 
	$GLOBALS['yp']->table('goods') . " as g on dg.goods_id = g.goods_id ".
    ...
    if ($sort=='salenum')
	{
		 $sql = "SELECT SUM(o.goods_number) as salenum,g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, " .
                ...
                "g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.goods_img FROM " . $GLOBALS['yp']->table('ypmart_distrib_goods') . " as dg left join " . 
	$GLOBALS['yp']->table('goods') . " as g on dg.goods_id = g.goods_id ".
	"LEFT JOIN " . $GLOBALS['yp']->table('order_goods') .  " as o ON o.goods_id = dg.goods_id " . 
...
function get_cagtegory_goods_count($children, $brand = 0, $min = 0, $max = 0, $ext='')
{
	...
	/* 返回商品总数 */
    return $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' . $GLOBALS['yp']->table('ypmart_distrib_goods') . " AS d," . $GLOBALS['yp']->table('goods') . " as g WHERE d.goods_id = g.goods_id $where $ext");
}
...
function get_catids($user_id)
{
	$sql = "SELECT cat_ids FROM " . 
	 		$GLOBALS['yp']->table('on_sales') . " WHERE user_id = '$user_id'";
