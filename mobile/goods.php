...
if(!empty($_REQUEST['act']) && $_REQUEST['act'] == 'get_pickup_list')
{
	...
	$city = $_REQUEST['city'];
	$sql_cn = 'select region_id from ' . $yp->table('region') . " where region_name='$country'";
	$country_id = $db->getOne($sql_cn);
	$sql_p = 'select region_id from ' . $yp->table('region') . " where region_name='$province'";
	$province_id = $db->getOne($sql_p);
	$sql_c = 'select region_id from ' . $yp->table('region') . " where region_name='$city'";
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'pickup_info')
{
	...
	$sql_pinfo = "select * from " . $yp->table('pickup_point') . " where district_id = '" . $district_id . "'";
...
{
	$district_id = $_REQUEST['district_id'];
	//$sql = 'select * from ' . $GLOBALS['yp']->table('pickup_point') . ' where district_id=' . $district_id;
	$suppid = intval($_REQUEST['suppid']);
...
//今/天/优/品/多/商/户/系/统  mod on 20160801 by prince  qq 1/2/0/0/2/9/1/2/1  Start
if(empty($_REQUEST['act']) && $goods_id &&  $_SESSION['user_id']){
		$sql = "SELECT * FROM  " .  $GLOBALS['yp']->table('users') . " WHERE  user_id= '" . $_SESSION['user_id'] . "'";
		$weixininfo =$GLOBALS['db']->getRow($sql);
		if(!$weixininfo['isfollow']){
			$continue_url=HTTP_TYPE."://".$_SERVER["SERVER_NAME"] . $_SERVER["REQUEST_URI"];
			$sql = "UPDATE ".  $GLOBALS['yp']->table('users') . "  SET continue_url = '". $continue_url . "'".
				   " WHERE user_id = '" . $_SESSION['user_id'] . "'";
			$GLOBALS['db']->query($sql);
		}
}
//今/天/优/品/多/商/户/系/统 mod on 20160801 by prince  qq 1/2/0/0/2/9/1/2/1   end
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'price')
{
    ...
    else
    {
        ...
        $exclusive = $GLOBALS['db']->getOne("select exclusive from ".$GLOBALS['yp']->table('goods')." where goods_id = $goods_id");
...
function get_goods_attr_thumb($goods_id,$attr_id=array()){
    $sql = "select goods_thumb from ".$GLOBALS['yp']->table('goods')." where goods_id = $goods_id";
    $goods_attr_thumb = $GLOBALS['db']->getOne($sql);
    if(!empty($attr_id)){
        $attr_id_s = implode(',',$attr_id);
        $sql = "select goods_attr_id from ".$GLOBALS['yp']->table('goods_attr')." as ga join ".$GLOBALS['yp']->table('attribute')." as at on ga.attr_id = at.attr_id where ga.goods_id = $goods_id and at.is_attr_gallery = 1 and ga.goods_attr_id in ($attr_id_s)";
        $goods_attr_id = $GLOBALS['db']->getOne($sql);
        if($goods_attr_id){
            $sql = "select thumb_url from ".$GLOBALS['yp']->table('goods_gallery')." where goods_attr_id = $goods_attr_id";
		...
		{
			$ga_thumb = $GLOBALS['db']->getOne("select attr_img from ".$GLOBALS['yp']->table('goods_attr')."  where goods_id = $goods_id and  attr_img <> '' and goods_attr_id in ($attr_id_s)");
			if ($ga_thumb)
				$goods_attr_thumb=$ga_thumb;
		}
    }
    return get_image_path($goods_id,$goods_attr_thumb);
}
...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
	...
	if($goods['supplier_id']){
    	$wechat = $db->getOne("select value from " . $yp->table('supplier_shop_config') . " where code='wechat' and supplier_id=" . $goods['supplier_id']);
    	$wechat_qrcode = $db->getOne("select value from " . $yp->table('supplier_shop_config') . " where code='wechat_qrcode' and supplier_id=" . $goods['supplier_id']);
		$smarty->assign('good_wechat',   $wechat?explode(',', $wechat):'');
		$smarty->assign('good_wechat_qrcode',   $wechat_qrcode);
	}else{
		$smarty->assign('good_wechat',   $GLOBALS['_CFG']['wechat']?explode(',', $GLOBALS['_CFG']['wechat']):'');
		$smarty->assign('good_wechat_qrcode',   $_CFG['wechat_qrcode']);
	}
    $smarty->assign('is_wechat',        is_wechat()?1:0);

	
	
/* 代码增加_end   By demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1  自提点 */
	$pups = $db->getOne('select * from ' . $yp->table('shipping') . ' where shipping_code="pups"');
    $smarty->assign('pups',    $pups);

    $suppid = intval($_REQUEST['suppid']);
    $ppts = $GLOBALS['db']->getOne(
        'SELECT COUNT(*) FROM ' . $GLOBALS['yp']->table('pickup_point') . ' WHERE supplier_id = ' . $suppid
    );
    $smarty->assign('ppts', $ppts);
 /* 代码增加 By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121 自提点 End */
	...
	if ($goods['supplier_id'] > 0)
	{
		...
		foreach ($properties['spe'] AS $skey_wx_120029121=>$sval_wx_120029121)
		{
			...
			{				
				...
				{
					$goods_attr_id_qq = $ssval_wx_120029121['id'] ;
					...
					$properties['spe'][$skey_wx_120029121]['values'][$sskey_wx_120029121]['goods_attr_thumb'] = $db->getOne($sql_qq_qq87139667);
				}
			}
			if ($hahaha_zhyh)
			{
        ...
        $smarty->assign('goods_rank',          get_goods_rank($goods_id));                       // 商品的销售排名
				//yyy添加start  //商品评价个数
			$count = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('comment') . " where comment_type=0 and id_value ='$goods_id' and status=1");
	...
	$sql="select * from ".$yp->table('comment')." where comment_type=0 and status=1 and comment_rank!=0 and id_value=$goods_id";
...
function get_dianpu_baseinfo($suppid=0,$suppinfo){
    ...
    //获取店铺海报
     $sql = "select logo from ". $GLOBALS['yp']->table('supplier_street') ." where supplier_id = ".$suppid;
	...
	//$sql="SELECT count(`goods_id`) FROM ".$GLOBALS['yp']->table('goods')." as g WHERE  g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.supplier_id='$suppid'";
...
function region_select($type, $parent_id, $start_id = "")
{
	...
	if ($start_id != "")
	{
		$sql_s = "select region_id, region_name from " . $GLOBALS['yp']->table('region') . " where region_id = '" . $start_id . "'";
		$s_info = $GLOBALS['db']->getRow($sql_s);
		$sql_a = "select region_id, region_name from " . $GLOBALS['yp']->table('region') . " where parent_id = '" . $parent_id . "' and region_id <> '" . $start_id . "'";
	...
	{
		$sql_all = "select region_id, region_name from " . $GLOBALS['yp']->table('region') . " where parent_id = '" . $parent_id . "'";
...
function get_zhekou($goods_id)
{
	...
	else
	{
		...
		{
			$zhekou = (number_format(intval($goods['promote_price'])/intval($goods['shop_price']),2))*10;//20161130 prince qq120029121
		}
		else
		{
...
function get_supplier_fensi_count($suppid=0){
    $suppid = (intval($suppid)>0) ? intval($suppid) : intval($_GET['suppId']);
    $sql = "SELECT count(*) FROM " .$GLOBALS['yp']->table('supplier_guanzhu') ." WHERE supplierid=$suppid";
    return $GLOBALS['db']->getOne($sql);
}
