...
if(isset($_REQUEST['act']) && $_REQUEST['act'] == 'insert_dianpu')
{
	...
	$sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('dianpu') . " WHERE user_id = '" . $_SESSION['user_id'] . "'";
	$count = $GLOBALS['db']->getOne($sql);
	if($count == 0)
	{
		$GLOBALS['db']->autoExecute($GLOBALS['yp']->table('dianpu'), $dianpu, 'INSERT');
		...
		{
			yp_header("Location: v_user_dianpu.php\n"); 
		}
	}
	else
	{
		$GLOBALS['db']->autoExecute($GLOBALS['yp']->table('dianpu'), $dianpu, 'UPDATE' , "user_id = '" . $_SESSION['user_id'] . "'");
		$error_no = $GLOBALS['db']->errno();
    	if ($error_no > 0)
    	{
        	show_message($GLOBALS['db']->errorMsg());
