...
function selectShipping_old(obj)
{
  ...
  {
    document.getElementById('YP_NEEDINSURE').checked = false;
    document.getElementById('YP_NEEDINSURE').disabled = true;
  }
  else
  {
    document.getElementById('YP_NEEDINSURE').checked = false;
    document.getElementById('YP_NEEDINSURE').disabled = false;
  }
...
function orderShippingSelectedResponse(result)
{
  if (result.need_insure)
  {
    try
    {
      document.getElementById('YP_NEEDINSURE').checked = true;
    }
    catch (ex)
    {
      alert(ex.message);
  ...
  try
  {
    if (document.getElementById('YP_CODFEE') != undefined)
    {
      document.getElementById('YP_CODFEE').innerHTML = result.cod_fee;
    }
  }
  catch (ex)
  {
...
function orderSelectedResponse(result)
{
  ...
  try
  {
    ...
    var layer = document.getElementById("YP_ORDERTOTAL");
...
function changeSurplusResponse(obj)
{
	if (obj.error) {
		try {
			alert(obj.error);
			document.getElementById('YP_SURPLUS').value = '0';
			document.getElementById('YP_SURPLUS').focus();
		} catch (ex) {
		}
	} else {
		try {
			document.getElementById('YP_SURPLUS').value = obj.surplus;
			if(obj.show){
				//如果余额完全支付订单金额
				$('#pay_div').hide();
...
function changeIntegralResponse(obj)
{
  if (obj.error)
  {
    try
    {
      document.getElementById('YP_INTEGRAL_NOTICE_'+obj.suppid).innerHTML = obj.error;
      document.getElementById('YP_INTEGRAL_'+obj.suppid).value = '0';
      document.getElementById('YP_INTEGRAL_'+obj.suppid).focus();
    }
    catch (ex) { }
  }
  else
  {
    try
    {
      document.getElementById('YP_INTEGRAL_NOTICE_'+obj.suppid).innerHTML = '';
    }
    catch (ex) { }
    orderSelectedResponse(obj.content);
  }
...
function changeBonusResponse(obj)
{
  if (obj.error)
  {
    ...
    try
    {
      document.getElementById('YP_BONUS_'+obj.suppid).value = '0';
    }
    catch (ex) { }
  }
  else
...
if (obj.error)
  ...
  {
    if(document.getElementById('YP_BONUS_'+obj.suppid)){
        var select = document.getElementById('YP_BONUS_'+obj.suppid);  
...
{
  var obj        = document.getElementById('YP_NEEDINV');
  var objType    = document.getElementById('YP_INVTYPE');
  var objPayee   = document.getElementById('YP_INVPAYEE');
  var objContent = document.getElementById('YP_INVCONTENT');
...
function check_before_submit_vir() {  
 	document.getElementById("tijiao_butn").disabled="disabled";/*20161229 prince q-1-2-0-0-2-9-1-2-1*/
  	document.getElementById("tijiao_butn").value="订单正在提交...";/*20161229 prince q-1-2-0-0-2-9-1-2-1*/
...
function checkOrderForm(frm)
{
  ...
  have_alipay=false;
  if(frm.elements['qq_120029121_bank']){
	  for(var iii=0;iii<frm.elements['qq_120029121_bank'].length;iii++)
	  {    
			if(frm.elements['qq_120029121_bank'][iii].checked)
			{
					have_alipay=true;
			}
	  }
...
//			} 
  ...
  // 检查用户输入的余额
  if (document.getElementById("YP_SURPLUS"))
  {
    var surplus = document.getElementById("YP_SURPLUS").value;
    ...
    if (error)
    {
      try
      {
        document.getElementById("YP_SURPLUS_NOTICE").innerHTML = error;
      }
      catch (ex)
      {
      }
  ...
  // 检查用户输入的积分
  if (document.getElementById("YP_INTEGRAL"))
  {
    var integral = document.getElementById("YP_INTEGRAL").value;
    ...
    if (error)
    {
      ...
      try
      {
        document.getElementById("YP_INTEGRAL_NOTICE").innerHTML = error;
      }
      catch (ex)
      {
      }
    ...
    /*检查发票*/
    if(document.getElementById('YP_NEEDINV') && document.getElementById('YP_NEEDINV').checked)
    {
  ...
  document.getElementById("tijiao_butn").disabled="disabled";
  document.getElementById("tijiao_butn").value="订单正在提交...";
