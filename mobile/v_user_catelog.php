...
if(isset($_REQUEST['act']) && $_REQUEST['act'] == 'act_on_sale')
{
	...
	if(!empty($cat_id))
	{
		...
		$sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('on_sales') . " WHERE user_id = '" . $_SESSION['user_id'] . "'";
		$count = $GLOBALS['db']->getOne($sql);
		if($count == 0)
		{
			$sql = "INSERT INTO " . 
					$GLOBALS['yp']->table('on_sales') . "(`user_id`,`cat_ids`) " . 
					"values('" . $_SESSION['user_id'] . "','$cat_ids')";
		}
		else
		{
			 $sql = "UPDATE " . $GLOBALS['yp']->table('on_sales') . 
			 		" SET cat_ids = '$cat_ids' " . 
...
function get_cat_list($user_id)
{
	$sql = "SELECT g.cat_id FROM " . 
		$GLOBALS['yp']->table('ypmart_distrib_goods') . " as dg ," . 
	...
	$sql = "SELECT cat_ids FROM " . $GLOBALS['yp']->table('on_sales') . " WHERE user_id = '$user_id'";
