...
{ 
    ...
    foreach($shop_list['shops'] as $k=>$v){
            $sql1 = "SELECT AVG(comment_rank) FROM " . $GLOBALS['yp']->table('comment') . " c" . " LEFT JOIN " . $GLOBALS['yp']->table('order_info') . " o"." ON o.order_id = c.order_id"." WHERE c.status > 0 AND  o.supplier_id = " .$v['supplier_id'];
            $avg_comment = $GLOBALS['db']->getOne($sql1);
            $avg_comment = number_format(round($avg_comment), 1);	
            $sql2 = "SELECT AVG(server), AVG(shipping) FROM " . $GLOBALS['yp']->table('shop_grade') . " s" . " LEFT JOIN " . $GLOBALS['yp']->table('order_info') . " o"." ON o.order_id = s.order_id"." WHERE s.is_comment > 0 AND  s.server >0 AND o.supplier_id = " .$v['supplier_id'];
    ...
    //获取店铺logo
    foreach($shop_list['shops'] as $k=>$v){
        $sql = "select value from ". $GLOBALS['yp']->table('supplier_shop_config') ." where code = 'shop_logo' and supplier_id = ".$v['supplier_id'];
...
function get_all_category(){
	$sql = "select * from ".$GLOBALS['yp']->table('street_category')." where is_show=1 order by sort_order";
...
function get_tuijian_shop($tuijian){
	...
	$sql = "select * from ".$GLOBALS['yp']->table('supplier_street')." where supplier_type in($types) and is_groom=1 and status=1 order by sort_order";
	$all = $GLOBALS['db']->getAll($sql);
	foreach($all as $k => $v){
...
function get_all_supplier(){
	...
	if($filter['keywords'] && $filter['keywords'] != '请输入关键词'){
		...
		$where .= " and supplier_id in(SELECT DISTINCT supplier_id
				FROM ".$GLOBALS['yp']->table('supplier_shop_config')." AS ssc
				WHERE (
				code = 'shop_name'
	...
	/* 记录总数 */
     $sql = "SELECT COUNT(*) FROM " .$GLOBALS['yp']->table('supplier_street'). " $where";
     $filter['record_count'] = $GLOBALS['db']->getOne($sql);
     $filter['page_count']     = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;
	
	$sql = "SELECT * ".
	           " FROM " . $GLOBALS['yp']->table('supplier_street'). 
	           " $where" .
	...
	foreach($arr as $key=>$val){
		$arr[$key]['address'] = "";//地址
		$shopinfo = $GLOBALS['db']->getAll("select code,value from ".$GLOBALS['yp']->table('supplier_shop_config')." where supplier_id=".$val['supplier_id']." and code in('shop_closed','shop_name','shop_keywords','shop_province','shop_city','shop_address','qq','ww')");
		...
		$arr[$key]['goods_info'] = $goodsInfo['info'];
                //添加联系电话
                $arr[$key]['contacts_phone'] = $GLOBALS['db']->getOne("select contacts_phone from ".$GLOBALS['yp']->table('supplier')." where supplier_id=".$val['supplier_id']);
                $arr[$key]['service_phone'] = $GLOBALS['db']->getOne("select value from ".$GLOBALS['yp']->table('supplier_shop_config')." where code='service_phone' AND supplier_id=".$val['supplier_id']);
	}
...
function get_street_goods_info($suppid){
	...
	if($allnum > 0){
		...
		foreach($goodsInfo as $key=>$row){
			...
			$goodsInfo[$key]['goods_thumb']      = get_image_path($row['goods_id'], $row['goods_thumb'], true);//mod by qq120-029-121 20160815
