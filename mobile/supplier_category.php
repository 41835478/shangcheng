...
function category_get_goods($children, $size, $page)
{
    ...
    $where = "g.is_on_sale = 1 AND g.supplier_status = 1 AND g.is_alone_sale = 1 AND ".
            "g.is_delete = 0 AND ($children)";//2017.02.27  寒冰  qq  309485552   修复  商家未审核 商品显示在mobile前台 bug
...
function get_cagtegory_goods_count($children)
{
    ...
    /* 返回商品总数 */
    $sql = 'SELECT count(g.goods_id) FROM ' . $GLOBALS['yp']->table('supplier_goods_cat') . ' AS sgc LEFT JOIN ' . $GLOBALS['yp']->table('goods') . ' AS g ' .
