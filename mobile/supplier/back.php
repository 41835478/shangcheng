...
elseif ($_REQUEST['act'] == 'back_info')
{
	...
	/* 获取原订单-商品信息 */
//	$sql = "select * from ". $yp->table('order_goods') ." where order_id ='$back_order[order_id]' and goods_id=41";
...
elseif ($_REQUEST['act'] == 'operate_refund')
{
	...
	$action_note    = isset($_REQUEST['action_note']) ? trim($_REQUEST['action_note']) : '';
	$order = back_order_info($back_id);
	$sql = "update ". $yp->table('back_goods') ." set status_refund='$status_refund'  where back_id='$back_id' and back_type='0' ";
	$db->query($sql);
	...
	$sql = "update ". $yp->table('back_order') ." set  status_refund='$status_refund',  refund_money_2='$refund_money_2', refund_type='$_REQUEST[refund_type]', refund_desc='$refund_desc' where back_id='$back_id' ";
	$db->query($sql);
...
function back_order_info($back_id)
{
    ...
    $sql = "SELECT * FROM " . $GLOBALS['yp']->table('back_order') ." $where LIMIT 0,1";
    ...
    if ($back)
    {
        ...
        {
            $back['user_name'] = $db->getOne('SELECT user_name FROM'.$yp->table('users').' WHERE user_id='.$back['user_id']);
        }
		//用户添加的图片
        if(!empty($back['imgs']))
        {
        ...
        $back_reply = $db->getAll("SELECT * FROM ".$yp->table('back_replay')." WHERE back_id = '$back_id' ORDER BY add_time ASC");
...
function update_back($back_id, $status_back, $status_refund )
{
	...
	{
	   $sql="update ". $GLOBALS['yp']->table('back_goods') ." set status_back='$status_back' where back_type in(1,2) and back_id='$back_id' ";
	   $GLOBALS['db']->query($sql);
	}
