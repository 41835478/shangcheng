...
function getPayHoudaofukuan(){
	...
	return $db->getOne('select pay_id from '.$yp->table('payment').' where is_cod=1 and is_pickup=0');
}
...
function getBackOrderByRebate($rid){
	global $yp,$db;
	$sql = "select oi.order_id,bo.back_type from ".$yp->table('order_info')." as oi right join ".$yp->table('back_order')." as bo on oi.order_id=bo.order_id and bo.status_back < 6 where oi.rebate_id=".$rid;
	$query = $db->query($sql);
	$ret = array();
...
function getRebateOrderMoney($rid){
	...
	$sql = "select (" . order_amount_field() . ") AS total_fee,pay_id from ".$yp->table('order_info')." where rebate_id=".$rid." $back_and and rebate_ispay=2";
	$query = $db->query($sql);
	$online = $onout = 0;
	while($row = $db->fetchRow($query)){
...
function writelog($rid,$inout=0){
	...
	//已经不用了
	global $db,$yp;
	$sql = "select order_sn, (" . order_amount_field() . ") AS total_fee,supplier_id from ".$yp->table('order_info')." where rebate_id=".$rid." and rebate_ispay=2";
	$query = $db->query($sql);
	$addtime = gmtime();
	$bs_qian = '';//前缀内容
	$bs_do = '+';//增减
	...
	while($row = $db->fetchRow($query)){
		if($suppid != $row['supplier_id']){
			$nowmoney = $db->getOne("select supplier_money from ".$yp->table('supplier')." where supplier_id=".$row['supplier_id']);
		}
		if($inout>0){
		...
		$db->autoExecute($yp->table('supplier_money_log'), $loginfo, 'INSERT');
	...
	foreach($suppmoney as $k => $v){
		$db->query('update '.$yp->table('supplier')." set supplier_money='".$v."' where supplier_id=".$k);
	}
}
