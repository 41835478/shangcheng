...
elseif ($_REQUEST['act'] == 'view')
{
	...
	else
	{
		...
		//佣金操作日志
		$sql = "select * from ".$yp->table('supplier_rebate_log')." where rebateid=".$rebate['rebate_id']." and type=".REBATE_LOG_LIST." order by logid desc";
		$logs = array();
		$query = $db->query($sql);
...
function rebate_list($act='')
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
	{
		...
		$row['all_money'] = $GLOBALS['db']->getOne("select sum(" . order_amount_field() . ") from ". $GLOBALS['yp']->table('order_info') ." where rebate_id=". $row['rebate_id'] ." and rebate_ispay=2");
