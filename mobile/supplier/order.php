...
//    $smarty->assign('agency_list', $db->getAll($sql));
    ...
    {
        $sql = 'SELECT pp.shop_name,pp.address,pp.phone,pp.contact,pr.region_name AS province_name,cr.region_name AS city_name,dr.region_name AS district_name FROM '.$yp->table('pickup_point').' AS pp LEFT JOIN '.$yp->table('region').' AS pr ON pr.region_id=pp.province_id LEFT JOIN '.$yp->table('region').' AS cr ON cr.region_id=pp.city_id LEFT JOIN '.$yp->table('region').' AS dr ON dr.region_id=pp.district_id WHERE pp.id='.$order['pickup_point'];
        $order['pickup_point_info'] = $db->getRow($sql);
    }
    //增值税发票地址信息
    if($order['inv_type'] == 'vat_invoice')
    {
        $sql = 'SELECT region_name FROM'.$yp->table('region').' WHERE region_id='.$order['inv_consignee_country'];
        $order['inv_consignee_country_name'] = $db->getOne($sql);
        $sql = 'SELECT region_name FROM'.$yp->table('region').' WHERE region_id='.$order['inv_consignee_province'];
        $order['inv_consignee_province_name'] = $db->getOne($sql);
        $sql = 'SELECT region_name FROM'.$yp->table('region').' WHERE region_id='.$order['inv_consignee_city'];
        $order['inv_consignee_city_name'] = $db->getOne($sql);
        $sql = 'SELECT region_name FROM'.$yp->table('region').' WHERE region_id='.$order['inv_consignee_district'];
...
elseif($act == 'edit')
{
    ...
    elseif ('shipping' == $step)
    {
        ...
        foreach ($shipping_list AS $key => $shipping)
        {
            ...
            $shipping_list[$key]['free_money'] = isset($shipping['configure']['free_money'])?price_format($shipping['configure']['free_money']):0;
        }
...
elseif ($act == 'step_post')
{
    ...
    elseif ('consignee' == $step)
    {
        ...
        elseif (isset($_POST['finish']))
        {
            ...
            if ($agency_changed)
            {
                yp_header("Location: order.php?act=order_list\n");
            }
            else
            {
                yp_header("Location: order.php?act=info&order_id=" . $order_id . "\n");
...
elseif($act == 'quick_delivery')
{
    ...
    $sql = 'SELECT COUNT(*) FROM '.$yp->table('order_info').' WHERE supplier_id='.$_SESSION['supplier_id'].order_query_sql('await_ship');
...
elseif($act == 'remove_delivery')
{
    ...
    // 更新：删除发货单
    $sql = "DELETE FROM ".$yp->table('delivery_order'). " WHERE delivery_id = '$delivery_id'";
    $db->query($sql);
...
function quick_delivery($order_id,$invoice_no,$action_note='Wap端一键发货')
{
    ...
    if (!empty($invoice_no))
    {
        ...
        /* 查询：如果管理员属于某个办事处，检查该订单是否也属于这个办事处 */
        $sql = "SELECT agency_id FROM " . $yp->table('supplier_admin_user') . " WHERE user_id = '$_SESSION[supplier_user_id]'";
        $agency_id = $db->getOne($sql);
        if ($agency_id > 0)
        {
            if ($order['agency_id'] != $agency_id)
...
function get_order_goods($order)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $row['goods_thumb'] = get_image_path($row['goods_id'], $row['goods_thumb']);//mod by qq120-029-121 20160815
        $goods_list[] = $row;
    }
