...
function goods_stock()
{
    ...
    if($result === false)
    {
        ...
        $sql = 'SELECT COUNT(*) FROM '.$yp->table('goods').$where;
        ...
        $sql = 'SELECT goods_id,goods_sn,goods_name,goods_number FROM '.$yp->table('goods').$where.$limit;
    ...
    {
        $sql = 'SELECT goods_attr AS goods_attr_id,product_number FROM '.$yp->table('products').' WHERE goods_id='.$goods_val['goods_id'];
        $attr = $db->getAll($sql);
        foreach($attr as $attr_key => $attr_val)
        {
            ...
            $sql = 'SELECT attr_value FROM '.$yp->table('goods_attr').' WHERE goods_attr_id='.$attr_sql;
