...
function changeStatus($rid){
	...
	$sql = "select oi.order_id,oi.order_status,oi.shipping_status,oi.pay_status,IFNULL(bo.back_type,'-1') as backtype from ".$yp->table('order_info')." as oi left join ".$yp->table('back_order')." as bo on oi.order_id=bo.order_id and bo.status_back < 6 where oi.rebate_id=".$rid." and oi.rebate_ispay=1";
	$info = $db->getAll($sql);
	if(empty($info)){
		//全部订单已经返佣
		return true;
