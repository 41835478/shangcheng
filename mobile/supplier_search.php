...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
    ...
    if($children === false){
    	yp_header("Location: supplier.php?suppId=".$_GET['suppId']);
    	exit;
    }
...
function get_cattype_supplier($cat = 0,$keywords='')
{
	if(empty($keywords)){
		...
		//$sql = "select cat_id  from ". $GLOBALS['yp']->table('supplier_cat_recommend') ." where ".$where;
                $sql = "select cat_id  from ". $GLOBALS['yp']->table('supplier_category') ." where ".$where;
...
function category_get_goods($children, $size, $page,$keywords='')
{
    ...
    $where = "g.is_on_sale = 1 AND g.supplier_status = 1 AND g.is_alone_sale = 1 AND ".
            "g.is_delete = 0 AND g.is_virtual=0 AND ($children)";//2017.02.27  寒冰  qq  309485552   修复  商家未审核 商品显示在mobile前台 bug
...
function get_cagtegory_goods_count($children,$keywords='')
{
    ...
    if(empty($keywords)){
    	...
    	$sql = 'SELECT count(DISTINCT g.goods_id) FROM ' . $GLOBALS['yp']->table('supplier_goods_cat') . ' AS sgc LEFT JOIN ' . $GLOBALS['yp']->table('goods') . ' AS g ' .
