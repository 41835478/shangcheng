 ...
 * ============================================================================
 * $Author: PRINCE $
 * $Id: activity.php 17217 2017-04-01 06:29:08Z PRINCE $
 */
...
if($_REQUEST['act'] == 'ajax_list'){
    ...
    $json   = new JSON;
$nowtime = time();
$sql = "SELECT fa.* FROM " . $GLOBALS['yp']->table('favourable_activity'). " AS fa ".
...
foreach ($res as $key=>$row)
{
    ...
    if($row['supplier_id'] > 0){
	    $sql = "select code,value from " . $GLOBALS['yp']->table('supplier_shop_config'). 
...
$res = $GLOBALS['db']->getRow($sql);
    ...
    if ($res['act_range'] != FAR_ALL && !empty($res['act_range_ext']))
    {
        if ($res['act_range'] == FAR_CATEGORY)
        {
            ...
            if($res['supplier_id']){
            $sql = "SELECT cat_id AS id, cat_name AS name FROM " . $GLOBALS['yp']->table('supplier_category') .
                " WHERE cat_id " . db_create_in($res['act_range_ext']);
            }else{
                 $sql = "SELECT cat_id AS id, cat_name AS name FROM " . $GLOBALS['yp']->table('category') .
                " WHERE cat_id " . db_create_in($res['act_range_ext']);
            }
        }
        elseif ($res['act_range'] == FAR_BRAND)
        {
            ...
            $sql = "SELECT brand_id AS id, brand_name AS name FROM " . $GLOBALS['yp']->table('brand') .
        ...
        else
        {
            ...
            $sql = "SELECT goods_id AS id, goods_name AS name, goods_thumb AS thumb FROM " . $GLOBALS['yp']->table('goods') .
    ...
    switch($res['act_type'])
    {
        case 0:
            ...
            if(is_array($res['gift']))
            {
                ...
                {
                    $res['gift'][$k]['thumb'] = get_image_path($v['id'], $GLOBALS['db']->getOne("SELECT goods_thumb FROM " . $GLOBALS['yp']->table('goods') . " WHERE goods_id = '" . $v['id'] . "'"), true);//mod by qq120-029-121 20160815
                }
            }
            break;
        case 1:
    ...
    if($res['supplier_id'] > 0){
        ...
        $res['shop_name'] = '网站自营';
    	$res['shop_logo'] =  get_pc_url().'/themes/prince_jtypmall_pc/images/ziying.jpg';
    }
