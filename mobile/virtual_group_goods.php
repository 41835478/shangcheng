 ...
 * $Id: virtual_group_goods.php 17217 2016-10-30 06:29:08Z prince $
*/

define('IN_PRINCE', true);

require(dirname(__FILE__) . '/includes/init.php');
//今/天/优/品/多/商/户/系/统  mod on 20160801 by prince  qq 1/2/0/0/2/9/1/2/1
...
if(!empty($_REQUEST['act']) && $_REQUEST['act'] == 'get_pickup_info')
{
	...
	if($city_info['province_id']>0 && $city_info['city_id']>0)
	{
		...
		$sql = 'select * from ' . $GLOBALS['yp']->table('pickup_point') . $where;
...
elseif (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'get_area_list')
{
	...
	$sql = 'select region_id, region_name, region_type from ' . $GLOBALS['yp']->table('region'). ' where parent_id=' . $parent_id;
...
elseif (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'get_pickup_point_list')
{
	...
	$sql = 'select * from ' . $GLOBALS['yp']->table('pickup_point') . ' where district_id=' . $district_id.' and supplier_id='.$suppid;
	$pickup_point_list = $GLOBALS['db']->getAll($sql);
...
if($path_name)
{
	$pathrow = $db->getRow("select c.path_name,c.cat_id from ". $yp->table('goods')." AS g left join ". $yp->table('category') ." AS c on g.cat_id=c.cat_id where g.goods_id='$goods_id'" );
...
$sql_attr_demo_coolhong_com="SELECT a.attr_id, ga.goods_attr_id FROM ". $GLOBALS['yp']->table('attribute') . " AS a left join ". $GLOBALS['yp']->table('goods_attr') . "  AS ga on a.attr_id=ga.attr_id  WHERE a.is_attr_gallery=1 and ga.goods_id='" . $goods_id. "' order by ga.goods_attr_id ";
$goods_attr=$GLOBALS['db']->getRow($sql_attr_demo_coolhong_com);
if($goods_attr){
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'get_gallery_attr')
{
	...
	$gallery_list_demo_coolhong_com=get_goods_gallery_attr_demo_coolhong_com($goods_id, $goods_attr_id);
	$gallery_content=array();
	$gallery_content['thumblist'] ='<ul>';
	foreach($gallery_list_demo_coolhong_com as $gkey=>$gval)
	{
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'gotopage')
{
    ...
    if (!empty($goods_id))
    {
        ...
        $sql = 'SELECT u.user_name,oi.tb_nick, og.goods_number, oi.add_time, IF(oi.order_status IN (2, 3, 4), 0, 1) AS order_status ' .
               'FROM ' . $yp->table('order_info') . ' AS oi LEFT JOIN ' . $yp->table('users') . ' AS u ON oi.user_id = u.user_id, ' . $yp->table('order_goods') . ' AS og ' .
...
if (!$smarty->is_cached('virtual_goods.dwt', $cache_id))
{
...
//今/天/优/品/多/商/户/系/统  mod on 20160801 by prince  qq 1/2/0/0/2/9/1/2/1
	...
	$sql = 'select region_id, region_name from ' . $yp->table('region') . ' where parent_id=' . $_CFG['shop_country'];
	$country_list = $GLOBALS['db']->getAll($sql);
	$smarty->assign('country_list',   $country_list);
	$city_id = $country_list[0]['region_id'];
	$smarty->assign('city_id',        $city_id);
	$district_id = $db->getOne('select region_id from ' . $yp->table('region') . ' where parent_id=' . $city_id);
...
//今/天/优/品/多/商/户/系/统  mod on 20160801 by prince  qq 1/2/0/0/2/9/1/2/1
 ...
 WHERE s.supplier_id=".$goods[supplier_id]." AND s.status=1";
			 ...
			 get_dianpu_baseinfo($goods['supplier_id'],$shopuserinfo);
                     ...
                     $opint = $GLOBALS['db']->getOne("SELECT value FROM " .$GLOBALS['yp']->table('shop_config'). " WHERE code = 'shop_opint'");
                    if(empty($opint)){
                    $city = get_city_name($GLOBALS['db']->getOne("SELECT value FROM " .$GLOBALS['yp']->table('shop_config'). " WHERE code = 'shop_city'"));
                    $pointJson = file_get_contents('http://api.map.baidu.com/geocoder/v2/?ak=dDbtTxnNUxSPt68hA2PhrNqS&output=json&address='.$GLOBALS['db']->getOne("SELECT value FROM " .$GLOBALS['yp']->table('shop_config'). " WHERE code = 'shop_address'").'city='.$city);//获取地图坐标
        ...
        {
            $time = gmtime();
            $sql = "SELECT type_money FROM " . $yp->table('bonus_type') .
        ...
        /* 检查是否已经存在于用户的收藏夹 */
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['yp']->table('collect_goods') .
        ...
        assign_template('c', $catlist);
         ...
         /* 上一个商品下一个商品 */
        $prev_gid = $db->getOne("SELECT goods_id FROM " .$yp->table('goods'). " WHERE cat_id=" . $goods['cat_id'] . " AND goods_id > " . $goods['goods_id'] . " AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0 LIMIT 1");
        if (!empty($prev_gid))
        {
        ...
        $next_gid = $db->getOne("SELECT max(goods_id) FROM " . $yp->table('goods') . " WHERE cat_id=".$goods['cat_id']." AND goods_id < ".$goods['goods_id'] . " AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0");
        if (!empty($next_gid))
        {
		...
		$sql_zhyh_qq = "select attr_id from ".$yp->table('attribute')." where cat_id='". $goods['goods_type'] ."' and is_attr_gallery='1' ";
		...
		$results_demo_coolhong_com = $GLOBALS['db']->getAll($sql);
		$return_arr = array();
		foreach ($results_demo_coolhong_com as $value_jtyp)
		{
			$return_arr[$value_jtyp['goods_attr_id']] = $value_jtyp['attr_value'];
		}
		...
		foreach ($properties['spe'] AS $skey_jtyp=>$sval_jtyp)
		{
			$hahaha_zhyh = 0;
			$sskey_demo_coolhong_com = '-1';
			foreach ($sval_jtyp['values'] AS $sskey_jtyp=>$ssval_jtyp)
			{				
				if ( is_exist_prod($selected_first, $ssval_jtyp['id'], $prod_exist_arr))
				{ 
					$hahaha_zhyh = $hahaha_zhyh ? $hahaha_zhyh : $ssval_jtyp['id'];
					$sskey_demo_coolhong_com = ($sskey_demo_coolhong_com != '-1') ? $sskey_demo_coolhong_com : $sskey_jtyp;
				}
				else
				{
					$properties['spe'][$skey_jtyp]['values'][$sskey_jtyp]['disabled'] = "disabled";
				}

				if ($skey_jtyp==$attr_id_gallery)
				{
					$goods_attr_id_qq = $ssval_jtyp['id'] ;
					$sql_qq_qq87139667 = "select  thumb_url from ". $yp->table('goods_gallery'). " where goods_id='$goods_id' and goods_attr_id='$goods_attr_id_qq' and is_attr_image='1' ";
					$properties['spe'][$skey_jtyp]['values'][$sskey_jtyp]['goods_attr_thumb'] = $db->getOne($sql_qq_qq87139667);
				}
			}
			if ($hahaha_zhyh)
			{
				$selected_first[$skey_jtyp] =  $hahaha_zhyh;
			}
			if ($sskey_demo_coolhong_com!='-1')
			{
				$properties['spe'][$skey_jtyp]['values'][$sskey_demo_coolhong_com]['selected_key_jtyp'] = "1";
			}
		}
		...
		$smarty->assign('pictures',            get_goods_gallery_attr_demo_coolhong_com($goods_id, $goods_attr_id)); // 商品相册_修改 By demo.coolhong.com
		$smarty->assign('new_goods',           get_recommend_goods('new'));     // 最新商品  改 By demo.coolhong.com
		...
		$package_goods_list_jtypmall = get_package_goods_list_jtypmall($goods['goods_id']);
        $smarty->assign('package_goods_list_jtypmall',$package_goods_list_jtypmall);    // 获取关联礼包
		...
		$res = $GLOBALS['db']->getAll("SELECT * FROM ".$GLOBALS['yp']->table('goods_tag')." WHERE goods_id = '$goods_id' AND state = 1");
		foreach ($res as $v)
		{
			$v['tag_num'] = $db->getOne("SELECT COUNT(*) AS num FROM ".$yp->table('comment')." WHERE id_value = '$goods_id' AND status = 1 AND FIND_IN_SET($v[tag_id],comment_tag)");
			$tag_arr[] = $v;	
		}
...
{
    $history = explode(',', $_COOKIE['YP']['history']);
    ...
    setcookie('YP[history]', implode(',', $history), gmtime() + 3600 * 24 * 30);
}
else
{
    setcookie('YP[history]', $goods_id, gmtime() + 3600 * 24 * 30);
}
/*寒冰  20161023    虚拟团购商品  qq  30 94   8 5552*/
 $weburl = $_SERVER['SERVER_NAME'] ? "http://".$_SERVER['SERVER_NAME']."/" : "http://".$_SERVER['HTTP_HOST']."/";
 $g_imgsrc = strpos($goods['goods_thumb'],'http') !== false ? $goods['goods_thumb'] : $weburl.$goods['goods_thumb'];
 $smarty->assign('g_imgsrc',$g_imgsrc);
/* 更新点击次数 */
$db->query('UPDATE ' . $yp->table('goods') . " SET click_count = click_count + 1 WHERE goods_id = '$_REQUEST[id]'");

$goods_n=$db->getOne('SELECT goods_number FROM' . $yp->table('goods') . " WHERE goods_id = '$_REQUEST[id]'");
$smarty->assign('goods_n',$goods_n);

$smarty->assign('now_time',  gmtime());           // 当前系统时间
$smarty->display('virtual_goods.dwt',      $cache_id);
...
function get_linked_goods($goods_id)
{
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
            ...
            'FROM ' . $GLOBALS['yp']->table('link_goods') . ' lg ' .
            'LEFT JOIN ' . $GLOBALS['yp']->table('goods') . ' AS g ON g.goods_id = lg.link_goods_id ' .
...
function get_linked_articles($goods_id)
{
    $sql = 'SELECT a.article_id, a.title, a.file_url, a.open_type, a.add_time ' .
            'FROM ' . $GLOBALS['yp']->table('goods_article') . ' AS g, ' .
                $GLOBALS['yp']->table('article') . ' AS a ' .
...
function get_user_rank_prices($goods_id, $shop_price)
{
    $sql = "SELECT rank_id, IFNULL(mp.user_price, r.discount * $shop_price / 100) AS price, r.rank_name, r.discount " .
            'FROM ' . $GLOBALS['yp']->table('user_rank') . ' AS r ' .
            'LEFT JOIN ' . $GLOBALS['yp']->table('member_price') . " AS mp ".
...
function get_also_bought($goods_id)
{
    $sql = 'SELECT COUNT(b.goods_id ) AS num, g.goods_id, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price, g.promote_price, g.promote_start_date, g.promote_end_date ' .
            'FROM ' . $GLOBALS['yp']->table('order_goods') . ' AS a ' .
            'LEFT JOIN ' . $GLOBALS['yp']->table('order_goods') . ' AS b ON b.order_id = a.order_id ' .
            'LEFT JOIN ' . $GLOBALS['yp']->table('goods') . ' AS g ON g.goods_id = b.goods_id ' .
...
{
    $sql = "SELECT SUM(attr_price) FROM " . $GLOBALS['yp']->table('goods_attr') .
...
function get_package_goods_list($goods_id)
{
    ...
    $sql = "SELECT pg.goods_id, ga.act_id, ga.act_name, ga.act_desc, ga.goods_name, ga.start_time,
            ...
            FROM " . $GLOBALS['yp']->table('goods_activity') . " AS ga, " . $GLOBALS['yp']->table('package_goods') . " AS pg
    ...
    foreach ($res as $tempkey => $value)
    {
        ...
        $sql = "SELECT pg.package_id, pg.goods_id, pg.goods_number, pg.admin_id, p.goods_attr, g.goods_sn, g.goods_name, g.market_price, g.goods_thumb, IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS rank_price
                FROM " . $GLOBALS['yp']->table('package_goods') . " AS pg
                    ...
                    LEFT JOIN ". $GLOBALS['yp']->table('products') . " AS p
...
function get_dianpu_baseinfo($suppid=0,$suppinfo){
    ...
    $smarty->assign('region', get_province_city($_goods_attr['shop_province'],$_goods_attr['shop_city']));
        $opint = $GLOBALS['db']->getOne("SELECT value FROM " .$GLOBALS['yp']->table('supplier_shop_config'). " WHERE code = 'shop_opint' and supplier_id = " . $suppid);
        if(empty($opint)){
 ...
 * @return  array
 */
function get_goods_gallery_attr_demo_coolhong_com($goods_id, $goods_attr_id)
{
...
{
	$sql = "select ".$name." from ". $GLOBALS['yp']->table('goods') ." where goods_id=".$goodsid;
	$row = $GLOBALS['db']->getRow($sql);
	return $row;
}
/* 代码增加_start  By demo.coolhong.com */
function get_package_goods_list_jtypmall($goods_id)
{
	$now = gmtime();
...
function get_virtual_price($attr_id){
    $sql = "select attr_price from " .$GLOBALS['yp']->table('goods_attr')." where goods_attr_id =".$attr_id;
...
function get_city_name($regoin_id){
	if($regoin_id){//20170926 prince q120029121
		$sql = "select region_name from ".$GLOBALS['yp']->table('region')." where region_id=".$regoin_id;
		$res = $GLOBALS['db']->getOne($sql);
	}else{
		$res="北京";
	}
...
function get_other_virtual_goods($id){
    ...
    $sql = "select district_id from ".$GLOBALS['yp']->table("virtual_goods_district")." where 1 ".$where;
    ...
    $sql = "select goods_id from ".$GLOBALS['yp']->table("virtual_district")." where district_id in (".implode(',',$district_ids_array).")";   
 ...
 $sql = "select g.goods_id,g.goods_name,g.goods_number,g.supplier_id,g.cat_id,g.add_time,g.valid_date,g.last_update,g.shop_price,g.goods_thumb,ifnull(salenum,0) as salenum from ".$GLOBALS['yp']->table("goods").
            " as g left join (select goods_id,order_id, count(*) as salenum from ".$GLOBALS['yp']->table("order_goods")." group by goods_id) as og on og.goods_id = g.goods_id 
            left join (select order_id from ".$GLOBALS['yp']->table("order_info")." where order_status = '5') as oi on oi.order_id = og.order_id"
