 ...
 * ============================================================================
 * $Author: PRINCE $
 * $Id: comment.php 17217 2017-04-01 06:29:08Z PRINCE $
*/
                ...
                if ($cmt->type == 0 && $factor > 0)
                {
                    ...
                    switch ($factor)
                    {
                        ...
                        case COMMENT_CUSTOM :
                            if ($_SESSION['user_id'] > 0)
                            {
                                $sql = "SELECT o.order_id FROM " . $yp->table('order_info') . " AS o ".
...
else
{
	...
	$cmt->comment_level = !empty($_GET['comment_level']) ? intval($_GET['comment_level'])  : 0;  //代码增加  By demo.coolhong.com
}

if ($result['error'] == 0)
{
    $comments = assign_comment($cmt->id, $cmt->type, $cmt->page, $cmt->comment_level);  //代码修改 增加一个 $cmt->comment_level   By demo.coolhong.com

    $smarty->assign('comment_type', $cmt->type);
	$smarty->assign('comment_level',    $cmt->comment_level);  //代码增加  By demo.coolhong.com
