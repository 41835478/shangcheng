 ...
 * ============================================================================
 * $Author: prince $
 * $Id: article.php 17217 2015-02-07 06:29:08Z prince $
*/
...
/*  同城快递 shopping_id  */ 
$sql = "select shipping_id from ". $yp->table('shipping') ." where shipping_code = 'tc_express' ";
...
if ($action=='get_OrderStatus')
{
	...
	$sql="select order_id from ". $yp->table('kuaidi_order') ." where order_sn='". $order->order_sn ."' ";
	...
	{
			$sql="select * from ". $yp->table('kuaidi_order_status') ." where order_id='$order_id'  order by status_id";
				...
				if ($row_status['status_display']==1)
				{
					switch ($row_status['status_id'])
					{
						...
						case 2 :
							...
							break;
						case 3 :
							$postman_id = $db->getOne("select postman_id from ".$yp->table('kuaidi_order')." where order_sn='".$order->order_sn."'");
...
{  
	...
	if (isset($_POST['yzcode']) && $_POST['yzcode']==$_SESSION['yzcode'])
	{
		...
		$sql="insert into ". $yp->table('kuaidi_order') ."(user_id, send_name, send_tel, send_region_id, send_address, to_name, to_tel,  ".
		...
		foreach ($orderstatus_array AS $okey=>$oval)
		{
			...
			$sql="insert into ".$yp->table('kuaidi_order_status').
...
{ 
	...
	if ($_CFG['shop_city'])
	{  
		$sql = "select * from ". $yp->table('region') ." where parent_id='$_CFG[shop_city]' ";
...
{        
	...
	if ($shop_city)
	{  
		$shop_city_name = $db->getOne("select region_name from ". $yp->table('region') ." where region_id=$shop_city ");
		$sql = "select * from ". $yp->table('region') ." where parent_id='$shop_city' ";
		$district_list = $db->getAll($sql);
		$smarty->assign('district_list', $district_list);
