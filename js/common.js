...
/* JS代码增加_start  By demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1 */
...
/* JS代码增加_end By demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1 */
/*******************************************************************************
 * 添加商品到购物车
...
function addToCart(goodsId, parentId, isnowbuy, extCode) {
	...
	var number = 1;
	var formBuy = document.forms['YP_FORMBUY'];
	var quick = 0;
	var custom_stye_1 = 1;// added by prince 120029121 20160813 
	var custom_stye_2 = 1;// added by prince 120029121 20160813 
	...
	if (formBuy) {
		...
		quick = 1;
		// added by prince 120029121 20160813 
		if (formBuy.elements['custom_stye_1']) {
			custom_stye_1 = formBuy.elements['custom_stye_1'].value;
		}
		if (formBuy.elements['custom_stye_2']) {
			custom_stye_2 = formBuy.elements['custom_stye_2'].value;
		}
		// added by prince 120029121 20160813 
	}else{
	...
	if (isSelectAttr(spec_arr)) {
		...
		goods.number = number;
		goods.custom_stye_1 = custom_stye_1;// added by prince 120029121 20160813 
		goods.custom_stye_2 = custom_stye_2;// added by prince 120029121 20160813 
...
/* 组合购买__添加商品到购物车__Start By demo.coolhong.com 今天优品多商户系统 qq 120029121 */
...
/* 组合购买__添加商品到购物车__End By demo.coolhong.com 今天优品多商户系统 qq 120029121 */
...
function gotoPageResponse_question(result) {
	document.getElementById("YP_QUESTION").innerHTML = result.content;
}
...
/* 代码修改_start 整个替换掉即可 By demo.coolhong.com 今天优品多商户系统 qq 120029121 */
/*******************************************************************************
 * 评论的翻页函数
 */
...
function gotoPageResponse(result) {
	document.getElementById("YP_COMMENT").innerHTML = result.content;
}
...
function gotoBuyPageResponse(result) {
	document.getElementById("YP_BOUGHT").innerHTML = result.result;
}
...
function bidResponse(result) {
	if (result.error == 0) {
		document.getElementById('YP_SNATCH').innerHTML = result.content;
...
function newPriceResponse(result) {
	document.getElementById('YP_PRICE_LIST').innerHTML = result;
}
...
function set_insure_status() {
	...
	if (shippingId > 0) {
		...
		// 设置配送保价，为0隐藏
		if (document.getElementById("yp_insure_cell")) {
			if (insure_fee > 0) {
				document.getElementById("yp_insure_cell").style.display = '';
				setValue(document.getElementById("yp_insure_fee_cell"), getFormatedPrice(insure_fee));
			} else {
				document.getElementById("yp_insure_cell").style.display = "none";
				setValue(document.getElementById("yp_insure_fee_cell"), '');
			}
		}
	}
}
...
function showCatalog(obj) {
	...
	var div = document.getElementById('YP_CATALOG');
...
function orderQueryResponse(result) {
	...
	if (result.error == 0) {
		var div = document.getElementById('YP_ORDER_QUERY');
 ...
 * 添加礼包到购物车
 */
/* 代码修改_start By demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1 这里增加了一个变量 */
	...
	for (var spec = 0; spec < message.length; spec++) {
		...
		if (message[spec]['attr_type'] == 1) {
			...
			for (var val_arr = 0; val_arr < message[spec]['values'].length; val_arr++) {
				if (val_arr == 0) {
					speDD.innerHTML += "<span class='attr-radio curr'><label for='spec_value_"+message[spec]['values'][val_arr]['id']+"'><input type='radio' name='spec_" + message[spec]['attr_id'] + "' value='" + message[spec]['values'][val_arr]['id'] + "' id='spec_value_" + message[spec]['values'][val_arr]['id'] + "' checked />&nbsp;<font>" + message[spec]['values'][val_arr]['label'] + '</font> </font></label></span>';
				} else {
					speDD.innerHTML += "<span class='attr-radio'><label for='spec_value_"+message[spec]['values'][val_arr]['id']+"'><input type='radio' name='spec_" + message[spec]['attr_id'] + "' value='" + message[spec]['values'][val_arr]['id'] + "' id='spec_value_" + message[spec]['values'][val_arr]['id'] + "' />&nbsp;<font>" + message[spec]['values'][val_arr]['label'] + '</font></font></label></span>';
				}
			}
...
function opencartDiv(price, name, pic, goods_brief, goods_id, total, number) {
	...
	// 生成层内内容
	html = '<div class=cardivfloat><span class=cartdivfloattitle>商品已成功添加到购物车！</span><a href=\'javascript:cancel_div()\' style="float:right;padding:0 26px 0 0;background:url(themes/prince_jtypmall_pc/images/ico_closebig1.gif) right center no-repeat;cursor:pointer;color:#ffffff;font-size:12px;" >关闭</a></div><div class="cartpopDiv"><div class="toptitle"><a href="goods.php?id=' + goods_id + '" class="pic"><img src=' + pic + ' width="98" height="98" style="border:#dddddd 1px solid; display:block;"/></a><p><font style="font-weight:bold">' + name + '</font>  <br>' + goods_brief + '<br>购买价格：<font style="color:#cc0000">' + price + '</font><br></p></div>';
	...
	html += "<div class=cartbntfloat ><a href='flow.php'><img src='themes/prince_jtypmall_pc/images/jsico1.gif'></a><a href=\'javascript:cancel_div()\'><img src='themes/prince_jtypmall_pc/images/goon_ico1.gif'></a></div>";
	html += '</div></div>';
