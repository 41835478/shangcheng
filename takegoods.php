...
$tg_id       = $_SESSION['takegoods_id_qq120029121'];
...
{
	$_SESSION['takegoods_id_qq120029121']=0;
	$_SESSION['takegoods_sn_qq120029121']='';
	header("Location:user.php?act=tg_login");
}
...
if ($_REQUEST['act'] == 'list')
{
    ...
    $sql = "select tg.*, tt.type_money_count,tt.type_money from ".$yp->table('takegoods')."AS tg left join ".$yp->table('takegoods_type')."AS tt on tg.type_id = tt.type_id where tg.tg_id='$tg_id' ";
    $rowtg = $db->getRow($sql);
    ...
    $sql = "select goods_ids from ". $yp->table('takegoods_goods') ." where tg_id = '$tg_id' ";
	$goods_ids = $db->getOne($sql);
	if (empty($goods_ids))
	{
		$sql = "select goods_ids from ". $yp->table('takegoods_type_goods') ." where tg_type_id = '$rowtg[type_id]' ";
		$goods_ids = $db->getOne($sql);
	}

	$sql = "select goods_id, goods_name, goods_thumb,shop_price from ". $yp->table('goods') ." where goods_id ". db_create_in($goods_ids);
	$res = $db->query($sql);
...
elseif ($_REQUEST['act'] == 'take')
{
	...
	$takegoods_sn_qq120029121 = $_SESSION['takegoods_sn_qq120029121'];
	$takegoods_id_qq120029121 = isset($_POST['takegoods_id_qq120029121'])  ? intval($_POST['takegoods_id_qq120029121']) : 0;
    if (empty($takegoods_id_qq120029121))
	{
		show_message('错误操作', '返回上一页', 'takegoods.php', 'error');
	}
	else
	{ 
		$sql = "select goods_ids from ". $yp->table('takegoods_goods') ." where tg_id='$takegoods_id_qq120029121' ";
		$goods_ids = $db->getOne($sql);
		if (empty($goods_ids))
		{
			$sql = "select tgtg.goods_ids from ". $yp->table('takegoods') ." AS tg left join ". $yp->table('takegoods_type_goods') .
				" AS tgtg on tgtg.tg_type_id=tg.type_id ".
				" where tg.tg_id='$takegoods_id_qq120029121' ";
			$goods_ids = $db->getOne($sql);
		}
		...
		$sql = "select tg.used_time, tt.type_money_count from ". $yp->table('takegoods') ."AS tg left join ".$yp->table('takegoods_type')."AS tt on tg.type_id = tt.type_id where tg.tg_id='$takegoods_id_qq120029121' ";
	...
	$sql = "insert into ". $yp->table('takegoods_order') . "(tg_id, tg_sn, user_id, goods_id, goods_name, country, province, city, district, address, consignee,mobile,email,add_time  )".
		" value('$takegoods_id_qq120029121', '$takegoods_sn_qq120029121', '$user_id', '$goods_id', '$goods_name', '$country', '$province','$city','$district', '$address', '$consignee' , '$mobile', '$email', '$add_time')";
	$db->query($sql);
	$tg_order_id = $db->insert_id();

	$add_time_sql = "select tg_order_id, used_time from ".$yp->table('takegoods')." where tg_id='$takegoods_id_qq120029121'";
	...
	$sql = "update ". $yp->table('takegoods') ." set tg_order_id='$tg_order_id', used_time='$add_time'  where  tg_id='$takegoods_id_qq120029121' ";
	$db->query($sql);

	if ($_CFG['takegoods_send_email'] =='1' || $_CFG['takegoods_send_sms'] == '1' )
	{
		$take_user_name = $db->getOne("select user_name from ". $yp->table('users') ." where user_id='$user_id' ");		
		...
		{	
			$admin_email = $db->getOne("select email from ". $yp->table('admin_user') ." where user_id=1 ");
			send_mail($_CFG['shop_name'], $admin_email, '卡号：'. $takegoods_sn_qq120029121 .' 提货成功', $content1, 0);
			send_mail($_CFG['shop_name'], $email, '卡号：'. $takegoods_sn_qq120029121 .' 提货成功', $content2, 0);
		}
		if ($_CFG['takegoods_send_sms'] == '1' )
		{
			...
			sendSMS($_CFG['sms_shop_mobile'],$content1,'', '',$templateParam , $templateCode);//暂不支持阿里大于 QQ 120029121
			sendSMS($mobile,$content2,'', '',$templateParam , $templateCode);//暂不支持阿里大于 QQ 120029121
		}
	}    
	
    
	 $country_name = $db->getOne("select region_name from ". $yp->table('region') ." where region_id='$country' "); 
	 $province_name = $db->getOne("select region_name from ". $yp->table('region') ." where region_id='$province' "); 
	 $city_name = $db->getOne("select region_name from ". $yp->table('region') ." where region_id='$city' "); 
	 $district_name = $db->getOne("select region_name from ". $yp->table('region') ." where region_id='$district' "); 
