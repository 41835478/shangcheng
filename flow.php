...
elseif ($_REQUEST['act']=='saveAddress')
{
	...
	$consignee = array(
            ...
            'country'       => empty($address_wx_120029121->country)    ? '1' :   intval($address_wx_120029121->country),
...
function get_consignee_list_wx_120029121()
{
	...
	foreach ($consignee_list_wx_120029121  as $cons_key => $cons_val)
	{
				...
				$consignee_list_wx_120029121[$cons_key]['address_short_name'] .=  "<br>".(empty($cons_val['mobile']) ? $cons_val['tel'] : $cons_val['mobile']);
...
if ($_REQUEST['step'] == 'add_to_cart')
{
	...
	//demo.coolhong.com start add 2015-3-26
	$time_xg_now=gmtime();
	...
	//demo.coolhong.com end add 2015-3-26
    ...
    else
    {
        ...
        // 更新：添加到购物车
        if (addto_cart($goods->goods_id, $goods->number, $goods->spec, $goods->parent,$goods->custom_stye_1,$goods->custom_stye_2))// mod by prince 120029121 20160813
        {
	...
	$rows = $GLOBALS['db']->getRow("select goods_brief,shop_price,goods_name,promote_end_date,promote_start_date,promote_price,goods_thumb from ".$GLOBALS['yp']->table('goods')." where goods_id=".$goods->goods_id);
	$time1 = gmtime();
...
/* 代码增加_end  By demo.coolhong.com 今天优品多商户系统 qq 120029121 */
    ...
    else
    {
        ...
        elseif (!empty($_POST['act']) && $_POST['act'] == 'signup')
        {
            ...
            {
                /* 用户注册成功 */
				/* 四合一修改 start by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1 */
                yp_header("Location: flow.php?step=checkout\n");
				/* 四合一修改 end by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1 */
                exit;
            }
            else
            {
...
elseif ($_REQUEST['step'] == 'checkout')
{
   ...
   // else if($_SESSION['extension_code'] == 'virtual_good')
    ...
    {
        /* 用户没有登录且没有选定匿名购物，转向到登录页面 */
        yp_header("Location: user.php?act=login\n");
        exit;
    }
    ...
    {
        /* 如果不完整则转向到收货人信息填写界面 */
		/* 代码修改_start  By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121  注释或删掉  */
        //yp_header("Location: flow.php?step=consignee\n");   
        //exit;
...
//    $cod_disabled      = true;
    ...
    // 查看购物车中是否全为免运费商品，若是则把运费赋为零
    //$sql = 'SELECT count(*) FROM ' . $yp->table('cart') . " WHERE `session_id` = '" . SESS_ID. "' AND `extension_code` != 'package_buy' AND `is_shipping` = 0";
   /* $sql = 'SELECT count(*) FROM ' . $yp->table('cart') . " WHERE $sql_where AND `extension_code` != 'package_buy' AND `is_shipping` = 0 AND rec_id in (".$_SESSION['sel_cartgoods'].")"; //jx
    $shipping_count = $db->getOne($sql);*/
	...
	*/
	...
	if($consignee['city']){
		$sql = 'select r.region_name, r.region_id from ' . $yp->table('pickup_point') . ' p left join ' . $yp->table('region') .
...
elseif ($_REQUEST['step'] == 'change_surplus')
{
    ...
    else
    {
        ...
        else
        {
            ...
            /* 代码修改 By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121 #1305 Start */
//            if($total['amount_formated'] <= 0){
            if($total['amount'] <= 0)
            {
            /* 代码修改 By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121 #1305 End */
    ...
    /* 代码修改 By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121 #1854 Start */
    $result['surplus'] = round($result['surplus'] ,2);
    /* 代码修改 By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121 #1854 End */
    $json = new JSON();
    die($json->encode($result));
}
...
//                    }
	        ...
	        if ($virtual_goods AND $flow_type != CART_GROUP_BUY_GOODS)
	        {
	            ...
	            {         
                        ...
                        foreach($card as $k=>$v){  
							...
							$result = sendSMS($_REQUEST['mobile_phone'],$content,'', '',$templateParam , $templateCode);//已支持阿里大于 QQ 120029121  
                        }
    ...
    /* 订单信息 */
    $sql = "SELECT pay_id FROM ".$yp->table('payment')." WHERE pay_code='weixin'";
...
elseif ($_REQUEST['step'] == 'update_cart')
{
	...
	{
		$sql="select g.goods_id, g.goods_name, g.is_buy, g.buymax, g.buymax_start_date, g.buymax_end_date from ".$yp->table('cart'). " AS c left join ".$yp->table('goods'). " AS g on c.goods_id=g.goods_id  where c.rec_id=".$key;
		$row=$db->getRow($sql);
		if ($row['is_buy'] == 1 && $row['buymax'] >0 && $row['buymax_start_date'] < $time_xg_now  && $row['buymax_end_date'] > $time_xg_now  )
		{
			$num_cart_old=$GLOBALS['db']->getOne("select sum(og.goods_number) from ". $GLOBALS['yp']->table('order_goods') ." AS og , ". $GLOBALS['yp']->table('order_info') ." AS o where o.user_id='$_SESSION[user_id]' and o.order_id = og.order_id and add_time > ". $row['buymax_start_date'] ." and add_time < ". $row['buymax_end_date'] ."  and og.goods_id = " . $row['goods_id'] );
...
elseif ($_REQUEST['step'] == 'add_package_to_cart')
{
    ...
    {
        /* 添加到购物车 修改 by demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1 增加两个参数 */
...
//线上红包
$sql = "SELECT * FROM " .$GLOBALS['yp']->table('bonus_type') .
    " WHERE send_type = '4'";
...
/* 代码修改_start  By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121 */
