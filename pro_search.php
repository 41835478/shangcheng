...
else
{
	...
	$sql_pcat = "SELECT parent_id FROM " . $yp->table('category') . " WHERE cat_id = " . $category;
...
{
	$now = gmtime();
	$sql_a = "SELECT DISTINCT(cat_id) FROM " . $GLOBALS['yp']->table('goods') . " WHERE is_delete = 0 AND is_on_sale = 1 AND is_alone_sale = 1 AND is_promote = 1 AND promote_start_date <= " . $now . " AND promote_end_date >= " . $now;
...
{
	$pid = $GLOBALS['db']->getOne("SELECT parent_id FROM " . $GLOBALS['yp']->table('category') . " WHERE cat_id = " . $cid);
	if ($pid > 0)
	{
		$cid = get_pid($pid);
	}
