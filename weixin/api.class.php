...
class weixinapi{
	...
	function subscribe($wxid){//适用于首次关注
		...
		include_once('../includes/lib_passport.php');
		$user = $GLOBALS['db']->getOne("SELECT count(1) FROM " . $GLOBALS['yp']->table('users') . " where  fake_id='$wxid' ");
		if($user==0){
	...
	function sendBonus($wxid,$type){
		...
		if($uid){//2017.02.18 修复重复绑定   重复赠送红包问题 qq   3094   85552
		    $row = $GLOBALS['db']->getRow("SELECT * FROM " . $GLOBALS['yp']->table('user_bonus') . " where bonus_type_id={$type} and user_id={$uid}");
			if($row){
				return false;
			}else{
				$sql = "INSERT INTO " . $GLOBALS['yp']->table('user_bonus') . " (bonus_type_id, user_id) VALUES('$type', '$uid')";
				$GLOBALS['db']->query($sql);
				return true;
			}
		}else{
			//关注红包
			$bonus_sn = $GLOBALS['db']->getOne("SELECT bonus_sn FROM " . $GLOBALS['yp']->table('user_bonus') . " where bonus_type_id={$type} and used_time=0 and emailed=0");
			if($bonus_sn){
				$GLOBALS['db']->query("update ".$GLOBALS['yp']->table('user_bonus')." set emailed=1 where bonus_sn='{$bonus_sn}'");
				return $bonus_sn;
			}
		}
		return false;
	...
	function followUser($wxid,$info=array()){
		...
		if($id>0){
			...
			$reg_time = time();
			$createymd = date('Y-m-d');
			$sql = "insert into ".$GLOBALS['yp']->table('users')." (`fake_id`,`is_bind`,`reg_time`,`createymd`,`isfollow`,`nickname`,`sex`,`country`,`province`,`city`,`access_token`,`expire_in`,`headimg`,`from_id`,`froms`) 
				value ('{$wxid}',1,'{$reg_time}','{$createymd}',1,'{$nickname}','{$sex}','{$country}','{$province}','{$city}','{$access_token}','{$expire_in}','{$headimg}',$from_id,'mobile')";
		}
		$GLOBALS['db']->query($sql);
		return true;
	}
	...
	**/
	...
	function gettestinfo($db,$wxid){
		if($wxid=='o86iDs_1zpCrcPjFXpWIQXQnpsoc' || $wxid=='o86iDs8DmXg4VXrStycbxCjH_j24'){
			...
			$text = "演示公众号：今天优品\r\n";
			$text .= "电脑端：http://demo.coolhong.com/ \r\n";
			$text .= "手机端：http://demo.coolhong.com/mobile/ \r\n";
			$text .= "电脑端平台后台：http://demo.coolhong.com/admin \r\n";
			$text .= "手机端平台后台：http://demo.coolhong.com/mobile/admin \r\n";
			$text .= "电脑端商家后台：http://demo.coolhong.com/supplier/privilege.php?act=login \r\n";
			$text .= "手机端商家后台：http://demo.coolhong.com/mobile/supplier/privilege.php?act=login \r\n";
			$text .= "平台/商家后台账号：test\r\n";
			$text .= "密码均为：".$pwd."\r\n";
			$text .= "安全码：020202\r\n";
