...
if ($action == 'reg')
{
	...
	{
		 yp_header("Location: user.php?back_act=supplier_reg.php?act=reg\n");
		 exit;
	}
    
	$sql = "select supplier_id, status from ". $yp->table('supplier') ." where user_id='". $_SESSION['user_id'] ."' ";
	...
	{
		yp_header("Location: user.php?act=supplier_reg\n");
		exit;
	}
