...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
    ...
    $sql = "SELECT b.brand_id, b.brand_name, COUNT(*) AS goods_num ".
            "FROM " . $GLOBALS['yp']->table('brand') . "AS b, ".
                $GLOBALS['yp']->table('goods') . " AS sgc LEFT JOIN ". $GLOBALS['yp']->table('goods_cat') . " AS gc ON sgc.goods_id = gc.goods_id " .
...
{
    return $GLOBALS['db']->getRow('SELECT cat_name, keywords, cat_desc, style, grade, filter_attr, parent_id,is_show,is_show_cat_pic,cat_pic,cat_pic_url FROM ' . $GLOBALS['yp']->table('supplier_category') .
        " WHERE cat_id = '$cat_id'");
}
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT parent_id, cat_id, grade ".
                   " FROM " . $GLOBALS['yp']->table('supplier_category') . "WHERE supplier_id=".$_GET['suppId'];
