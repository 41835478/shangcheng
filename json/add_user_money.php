...
*/
	...
	if(!empty($user_id)&&$user_id!=0){
		$sql="select vc.*, vt.type_money, vt.use_start_date, vt.use_end_date from ". $yp->table('valuecard') ." AS vc ".
				" left join " . $yp->table('valuecard_type')." AS vt ".
		...
		$row = $GLOBALS['db'] -> getRow("SELECT * FROM ".$GLOBALS['yp']->table('users')." WHERE `user_id`='$user_id'");
		$result['code']=1;
		$result['info']=$row;
