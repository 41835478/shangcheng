...
ob_start();
	...
	for($i=0;$i<count($res);$i++)
	{
		...
		$aa = $res[$i]['order_id'];
		$sql ="SELECT g.goods_thumb,s.* FROM ".$yp->table('order_goods')."as s,".$yp->table('goods')." as g WHERE s.order_id='$aa' AND s.goods_id=g.goods_id";
	...
	$sql ="SELECT g.goods_thumb,s.* FROM ".$yp->table('order_goods')."as s,".$yp->table('goods')." as g WHERE s.order_id='$order_id' AND s.goods_id=g.goods_id";
	$resa = $db -> getAll($sql);
