...
function get_position_list()
{
    ...
    $sql = 'SELECT position_id, position_name, ad_width, ad_height '.
           'FROM ' . $GLOBALS['yp']->table('ad_position');
    $res = $GLOBALS['db']->query($sql);
...
{
    $sql = 'SELECT cat_id, cat_name FROM ' . $GLOBALS['yp']->table('goods_type') . ' WHERE enabled = 1';
    $res = $GLOBALS['db']->query($sql);
...
function update_user_info()
{
    ...
    $sql = 'SELECT u.user_money,u.email, u.pay_points, u.user_rank, u.rank_points, '.
            ...
            ' FROM ' .$GLOBALS['yp']->table('users'). ' AS u ' .
            ' LEFT JOIN ' .$GLOBALS['yp']->table('user_bonus'). ' AS ub'.
            ' ON ub.user_id = u.user_id AND ub.used_time = 0 ' .
            ' LEFT JOIN ' .$GLOBALS['yp']->table('bonus_type'). ' AS b'.
    ...
    if ($row = $GLOBALS['db']->getRow($sql))
    {
        ...
        if($row['user_rank'] >0)
        {
            $sql="SELECT special_rank from ".$GLOBALS['yp']->table('user_rank')."where rank_id='$row[user_rank]'";
            if($GLOBALS['db']->getOne($sql)==='0' || $GLOBALS['db']->getOne($sql)===null)
            {   
                $sql="update ".$GLOBALS['yp']->table('users')."set user_rank='0' where user_id='$_SESSION[user_id]'";
                $GLOBALS['db']->query($sql);
                $row['user_rank']=0;
            }
        }
        ...
        if ($row['user_rank'] == 0)
        {
            ...
            $sql = 'SELECT rank_id, discount FROM ' . $GLOBALS['yp']->table('user_rank') . " WHERE special_rank = '0' AND min_points <= " . intval($row['rank_points']) . ' AND max_points > ' . intval($row['rank_points']);
            if ($row = $GLOBALS['db']->getRow($sql))
            {
        ...
        {
            // 特殊等级
            $sql = 'SELECT rank_id, discount FROM ' . $GLOBALS['yp']->table('user_rank') . " WHERE rank_id = '$row[user_rank]'";
            if ($row = $GLOBALS['db']->getRow($sql))
            {
                $_SESSION['user_rank'] = $row['rank_id'];
                $_SESSION['discount']  = $row['discount'] / 100.00;
...
function get_user_info($id=0)
{
	...
	}else
	{
		...
		$sql = "SELECT rank_name FROM ".$GLOBALS['yp']->table('user_rank')."WHERE rank_id='$rank_id'";
...
{
    $sql = 'SELECT id, number, type FROM ' . $GLOBALS['yp']->table('template') .
...
{
    $sql = 'SELECT cat_name FROM ' . $GLOBALS['yp']->table('article_cat') . " WHERE cat_id = '" . $id ."'";
...
function get_shop_help()
{
    $sql = 'SELECT c.cat_id, c.cat_name, c.sort_order, a.article_id, a.title, a.file_url, a.open_type ' .
            ...
            'LEFT JOIN ' .$GLOBALS['yp']->table('article_cat'). ' AS c ' .
...
function get_vote($id = '')
{
    ...
    if ($vote_arr !== false && !empty($vote_arr))
    {
        ...
        $sql_option = 'SELECT v.*, o.option_id, o.vote_id, o.option_name, o.option_count ' .
                      'FROM ' . $GLOBALS['yp']->table('vote') . ' AS v, ' .
                            $GLOBALS['yp']->table('vote_option') . ' AS o ' .
        ...
        /* 总票数 */
        $sql = 'SELECT SUM(option_count) AS all_option FROM ' . $GLOBALS['yp']->table('vote_option') .
...
function is_spider($record = true)
{
    ...
    foreach ($searchengine_bot AS $key => $value)
    {
        if (strpos($spider, $value) !== false)
        {
            ...
            if ($record === true)
            {
                $GLOBALS['db']->autoReplace($GLOBALS['yp']->table('searchengine'), array('date' => local_date('Y-m-d'), 'searchengine' => $spider, 'count' => 1), array('count' => 1));
            }
...
function visit_stats()
{
    ...
    $visit_times = (!empty($_COOKIE['YP']['visit_times'])) ? intval($_COOKIE['YP']['visit_times']) + 1 : 1;
    setcookie('YP[visit_times]', $visit_times, $time + 86400 * 365, '/');
    ...
    $sql = 'INSERT INTO ' . $GLOBALS['yp']->table('stats') . ' ( ' .
...
function save_searchengine_keyword($domain, $path)
{
    ...
    if (!empty($keywords))
    {
        ...
        if (YP_CHARSET == 'utf-8' && in_array($searchengine, $gb_search))
        {
            $keywords = yp_iconv('GBK', 'UTF8', $keywords);
        }
        if (YP_CHARSET == 'gbk' && !in_array($searchengine, $gb_search))
        {
            $keywords = yp_iconv('UTF8', 'GBK', $keywords);
        }

        $GLOBALS['db']->autoReplace($GLOBALS['yp']->table('keywords'), array('date' => local_date('Y-m-d'), 'searchengine' => $searchengine, 'keyword' => addslashes($keywords), 'count' => 1), array('count' => 1));
    }
}
...
function get_dyna_libs($theme, $tmp)
{
	...
	$tmp_arr_qq_wx_120029121 = explode('.', $tmp);
    $ext = end($tmp_arr_qq_wx_120029121);
	/* 代码修改_end  By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121 */

    $tmp = basename($tmp,".$ext");
    $sql = 'SELECT region, library, sort_order, id, number, type' .
            ' FROM ' . $GLOBALS['yp']->table('template') .
...
function recalculate_price()
{
    ...
    foreach ($res AS $row)
    {
        ...
        $goods_sql = "UPDATE " .$GLOBALS['yp']->table('cart'). " SET goods_price = '$goods_price' ".
	...
	$sql = "select rec_id,goods_id,goods_attr,goods_number ".  
				" from ".$GLOBALS['yp']->table('cart')  ." where user_id=0 ".
   ...
   while ($row = $GLOBALS['db']->fetchRow($res))
    {
		$sql = "select rec_id from ".$GLOBALS['yp']->table('cart')." where user_id='". $_SESSION['user_id'] ."' ".
					" AND add_time >='$time1' and add_time<'$time2' ".
					" AND goods_id='$row[goods_id]' and goods_attr= '$row[goods_attr]' ";
		$rec_id = $GLOBALS['db']->getOne($sql);
		if($rec_id)
		{
			$sql = "update ".$GLOBALS['yp']->table('cart')." set goods_number= goods_number + ".$row['goods_number'].
						" where rec_id='$rec_id' ";
			$GLOBALS['db']->query($sql);
			$sql="delete from ".$GLOBALS['yp']->table('cart')." where rec_id='".$row['rec_id']."'";
			$GLOBALS['db']->query($sql);
		}
		else
		{
			$sql = "update ".$GLOBALS['yp']->table('cart')." set user_id='$_SESSION[user_id]' ".
    ...
    /* 删除赠品，重新选择 */
    $GLOBALS['db']->query('DELETE FROM ' . $GLOBALS['yp']->table('cart') .
        " WHERE session_id = '" . SESS_ID . "' AND is_gift > 0");
}

/**
*  将 assign_comment 函数整个替换掉   By demo.coolhong.com 今天优品多商户系统 qq 120029121 * 查询评论内容
 *
 * @access  public
 * @params  integer     $id
 * @params  integer     $type
...
function assign_comment($id, $type, $page = 1,$comment_level=0)
{
    ...
    $count = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' .$GLOBALS['yp']->table('comment').
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		/* 会员级别 */
		$sql_ur = "select user_rank  from " . $GLOBALS['yp']->table('users') . " where user_id = '$row[user_id]' ";
...
function assign_template($ctype = '', $catlist = array())
{
    ...
    $smarty->assign('yp_version',   VERSION);
...
function get_user_bonus($user_id = 0)
{
    ...
    $sql = "SELECT SUM(bt.type_money) AS bonus_value, COUNT(*) AS bonus_count ".
            "FROM " .$GLOBALS['yp']->table('user_bonus'). " AS ub, ".
                $GLOBALS['yp']->table('bonus_type') . " AS bt ".
...
{
    setcookie('jtypmall_affiliate_uid', intval($_GET['u']), gmtime() + 3600 * 24); // 过期时间为 1 天
    ...
    if (!empty($_GET['u']) && $config['on'] == 1)
    {
        if(!empty($config['config']['expire']))
        {
            ...
            setcookie('jtypmall_affiliate_uid', intval($_GET['u']), gmtime() + 3600 * $config['config']['expire'] * $c);
        }
        else
        {
            setcookie('jtypmall_affiliate_uid', intval($_GET['u']), gmtime() + 3600 * 24); // 过期时间为 1 天
...
{
    if (!empty($_COOKIE['jtypmall_affiliate_uid']))
    {
        $uid = intval($_COOKIE['jtypmall_affiliate_uid']);
        if ($GLOBALS['db']->getOne('SELECT user_id FROM ' . $GLOBALS['yp']->table('users') . "WHERE user_id = '$uid'"))
        {
            return $uid;
        }
        else
        {
            setcookie('jtypmall_affiliate_uid', '', 1);
        }
    }
...
function article_categories_tree($cat_id = 0)
{
    if ($cat_id > 0)
    {
        $sql = 'SELECT parent_id FROM ' . $GLOBALS['yp']->table('article_cat') . " WHERE cat_id = '$cat_id'";
    ...
    /*
     ...
     如果不是取当前分类及其下的子分类
    */
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['yp']->table('article_cat') . " WHERE parent_id = '$parent_id'";
    if ($GLOBALS['db']->getOne($sql))
    {
        /* 获取当前分类及其子分类 */
        $sql = 'SELECT a.cat_id, a.cat_name, a.sort_order AS parent_order, a.cat_id, ' .
                    'b.cat_id AS child_id, b.cat_name AS child_name, b.sort_order AS child_order ' .
                'FROM ' . $GLOBALS['yp']->table('article_cat') . ' AS a ' .
                'LEFT JOIN ' . $GLOBALS['yp']->table('article_cat') . ' AS b ON b.parent_id = a.cat_id ' .
...
function get_article_parent_cats($cat)
{
    ...
    $arr = $GLOBALS['db']->GetAll('SELECT cat_id, cat_name, parent_id FROM ' . $GLOBALS['yp']->table('article_cat'));
...
function get_library_number($library, $template = null)
{
    ...
    if (!isset($lib_list[$template]))
    {
        ...
        $sql = "SELECT library, number FROM " . $GLOBALS['yp']->table('template') .
...
{
    $sql = 'SELECT * FROM '. $GLOBALS['yp']->table('nav') . '
...
function license_info()
{
    if($GLOBALS['_CFG']['licensed'] > 0)
    {
        ...
        $license = '<a href="http://demo.coolhong.com/license.php?product=qq120029121_b2c&url=' . urlencode($host) . '" target="_blank"
>&nbsp;&nbsp;Licensed</a>';
        return $license;
    }
    else
...
{
    /* 取得咨询列表 */
    $count = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' .$GLOBALS['yp']->table('question').
           " WHERE id_value = '$id' AND status = 1 AND parent_id = 0 and question_type = '$question_type' ");
    $size  = !empty($GLOBALS['_CFG']['comments_number']) ? $GLOBALS['_CFG']['comments_number'] : 5;

    $page_count = ($count > 0) ? intval(ceil($count / $size)) : 1;

    $sql = 'SELECT * FROM ' . $GLOBALS['yp']->table('question') .
            " WHERE id_value = '$id'  AND status = 1 AND parent_id = 0 AND question_type= '$question_type' ".
