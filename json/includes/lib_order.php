...
function order_fee($order, $goods, $consignee)
{
    ...
    if ($order['shipping_id'] > 0 && $total['real_goods_count'] > 0)
    {
        ...
        if (!empty($shipping_info))
        {
            ...
            $sql = 'SELECT count(*) FROM ' . $GLOBALS['yp']->table('cart') . " WHERE  $sql_where AND `extension_code` != 'package_buy' AND `is_shipping` = 0 ";  //jx  AND rec_id in (".$_SESSION['sel_cartgoods'].")
	...
	foreach ($goods AS $val)
    {
       ...
       $sql = 'SELECT count(*) FROM ' . $GLOBALS['yp']->table('cart') . " AS c left join ". $GLOBALS['yp']->table('goods') ." AS g on c.goods_id=g.goods_id WHERE g.supplier_id = '". $row_supp['supplier_id'] ."' AND $sql_where AND c.extension_code != 'package_buy' AND c.is_shipping = 0 ";  //jx  AND c.rec_id in (".$_SESSION['sel_cartgoods'].")
...
$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";
    $sql = "SELECT c.rec_id, c.user_id, c.goods_id, c.goods_name, c.goods_sn, c.goods_number, " .
            ...
            "AND c.rec_type = '$type'";  //代码修改 By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121  增加一个 $id_ext , package_attr_id	  $id_ext 
...
function compute_discount($supplierid=-1)
{
	...
	//jx  查询用户等级
	$user_id = $_SESSION['user_id'];
	$sql = "SELECT rank_points FROM ".$GLOBALS['yp']->table('users')." WHERE `user_id`='$user_id'";
	$rank_points = $GLOBALS['db']->getOne($sql);
	$sql = "SELECT rank_id FROM " . $GLOBALS['yp']->table('user_rank') . " WHERE max_points > '$rank_points' ORDER BY max_points ASC LIMIT 1";
...
function send_order_bonus($order_id)
{
    ...
    if ($bonus_list)
    {
        ...
        foreach ($bonus_list AS $bonus)
        {
            ...
            /* 修改用户红包 */
            $sql = "INSERT INTO " . $GLOBALS['yp']->table('user_bonus') . " (bonus_type_id, user_id) " .
