...
if($_REQUEST['step']=='update_group_cart')
{
	...
	$sql = "UPDATE " . $GLOBALS['yp']->table('cart') . " SET goods_number = '$number',goods_price = '$min_price' WHERE rec_id = $rec_id";
...
{
	$sql = "SELECT * FROM ".$GLOBALS['yp']->table('cart')." WHERE goods_id = '$goods_id' AND  rec_id = '$rec_id'";
	...
	}else
	{
		...
		foreach ($goods_att as $value)
		{
			if(!empty($value))
			{
				$sql="SELECT `attr_price` FROM " .$GLOBALS['yp']->table('goods_attr')." WHERE goods_id='$goods_id' AND `goods_attr_id`='$value'";
				$atr_price+=$GLOBALS['db']->getOne($sql);
			}
			
		}
	}
	$sql = "SELECT volume_price FROM ".$GLOBALS['yp']->table('volume_price')."WHERE goods_id = '$goods_id' AND volume_number <= '$number'";
	...
	{
		$row = $GLOBALS['db'] -> getOne("SELECT rank_points FROM ".$GLOBALS['yp']->table('users')." WHERE `user_id`='$user_id'");
		$sql = "SELECT rank_id FROM " . $GLOBALS['yp']->table('user_rank') . " WHERE max_points > '$row' ORDER BY max_points ASC LIMIT 1";
