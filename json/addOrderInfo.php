...
*/
	...
	$address_id = $db -> getRow("SELECT address_id,user_money,pay_points FROM ".$GLOBALS['yp']->table('users')." WHERE `user_id`='$user' ");
	$result['user_money']=$address_id['user_money'];
	$result['user_integral']=$address_id['pay_points'];
	$address_id=$address_id['address_id'];
	$address = $db -> getRow("SELECT * FROM ".$GLOBALS['yp']->table('user_address')." WHERE `address_id`='$address_id'");
	if(!empty($address)){
		$region_id_list=array($address['country'], $address['province'], $address['city'], $address['district']);
		
		$region_id=$address['country'];
		$province = $db -> getRow("SELECT * FROM ".$GLOBALS['yp']->table('region')." WHERE region_type ='0' AND `region_id`='$region_id'");
		$address['country']=$province['region_name'];
	
		$region_id=$address['province'];
		$province = $db -> getRow("SELECT * FROM ".$GLOBALS['yp']->table('region')." WHERE region_type ='1' AND `region_id`='$region_id'");
		$address['province']=$province['region_name'];
		
		$region_id=$address['city'];
		$province = $db -> getRow("SELECT * FROM ".$GLOBALS['yp']->table('region')." WHERE region_type ='2' AND `region_id`='$region_id'");
		$address['city']=$province['region_name'];
		
		$region_id=$address['district'];
		$province = $db -> getRow("SELECT * FROM ".$GLOBALS['yp']->table('region')." WHERE region_type ='3' AND `region_id`='$region_id'");
		 ...
		 */
		 ...
		 g.goods_id,g.is_shipping,g.shop_price,g.goods_weight,g.goods_number,g.integral,g.give_integral,g.rank_integral,
			...
			g.promote_end_date,
			g.is_promote
		 FROM ". $GLOBALS['yp']->table('goods') . " as g WHERE g.is_shipping=0 AND g.goods_id IN (".$goods_id.")";
    ...
    $sql="SELECT `user_money` FROM ". $GLOBALS['yp']->table('users') ." WHERE  `user_id`='$user' ";
    $user_money=$db->getOne($sql);
    if($user_money>$total)
    {
        $sql = "SELECT pay_id,pay_code,pay_name " .
        "FROM " . $GLOBALS['yp']->table('payment') . " WHERE (pay_code='alipay' OR pay_code='cod' OR pay_code='balance') AND enabled='1' ";
    }
    else
    {
        $sql = "SELECT pay_id,pay_code,pay_name " .
        "FROM " . $GLOBALS['yp']->table('payment') . " WHERE (pay_code='alipay' OR pay_code='cod') AND enabled='1' ";
        $result['is_balance']=2;
    }
