...
*/
	...
	$result=array();
	/*查找后台配置的库存管理*/
	$sql="SELECT value FROM ".$yp->table('shop_config')." WHERE id='207'";
	$use_storage=$db ->getRow($sql);
	$result['use_storage']=$use_storage;
	
	/*每100积分可抵多少元现金*/
	$sql="SELECT value FROM ".$yp->table('shop_config')." WHERE id='211'";
    ...
    if(empty($act))
    {  
	$sql="SELECT img_url FROM ".$yp->table('goods_gallery')." WHERE goods_id='$goods_id'  LIMIT 0 ,5 ";
	$goods_gallery = $db -> getAll($sql);
	/*获取商品的评论*/
	//$sql="SELECT user_name,content,add_time FROM ".$yp->table('comment')." WHERE id_value='$goods_id'  LIMIT 0 ,5 ";
	//$comment = $db -> getAll($sql);
	
	/*获取商品的类型*/
	
	$sql="SELECT attr.goods_attr_id,attr.attr_value,attribute.attr_name 
	FROM ".$yp->table('goods_attr')." AS attr,".$yp->table('attribute')." AS attribute  
	WHERE attr.goods_id='$goods_id' AND attr.attr_id=attribute.attr_id ;";
	
	$sql="SELECT a.attr_id, a.attr_name, a.attr_group, a.is_linked, a.attr_type, g.goods_attr_id, g.attr_value, g.attr_price FROM ".$yp->table('goods_attr')." AS g LEFT JOIN  ".$yp->table('attribute')."  AS a ON a.attr_id = g.attr_id WHERE a.attr_group='0' AND  g.goods_id = '$goods_id' ORDER BY a.sort_order, g.attr_price, g.goods_attr_id";
	$res = $db -> getAll($sql);
	$goods_attr=array();
	$attr=array();
	$list=array();
	...
	$sql="SELECT click_count FROM ".$yp->table('goods')." WHERE goods_id='$goods_id'";
	$goods=$db ->getRow($sql);
	$click_count=$goods['click_count']+1;
	$db -> query("update ".$yp->table('goods')." set click_count = '$click_count' where goods_id = '$goods_id'");
	...
	FROM ".$yp->table('goods')." AS g LEFT JOIN ".$yp->table('supplier_shop_config')." AS ssc ON g.supplier_id=ssc.supplier_id AND ssc.code='shop_name' WHERE g.goods_id='$goods_id'";
	$goods=$db ->getRow($sql);
	$goods['shop_logo'] = $db ->getOne("select value from ".$yp->table('supplier_shop_config')." where code='shop_logo' and supplier_id=".$goods['supplier_id']);
	$goods['service_phone'] = $db ->getOne("select value from ".$yp->table('supplier_shop_config')." where code='service_phone' and supplier_id=".$goods['supplier_id']);
...
//	$result['linked_goods']=get_linked_goods($goods_id);//获取指定商品的关联商品
  ...
  {
      /*	$sql="SELECT 
		shop_price	FROM ".$yp->table('goods')." WHERE goods_id='$goods_id'";
   ...
   // $goods['shop_price']+=$atr_price;
   	$sql="SELECT 
		...
		promote_end_date,
		is_promote
		FROM ".$yp->table('goods')." WHERE goods_id='$goods_id'";
	   $goods=$db ->getRow($sql);
	...
	//判断所选属性是不是有库存
	$sql = "SELECT product_number FROM ".$yp->table('products')."WHERE goods_id = '$goods_id' and goods_attr = '$good_attr'";
    ...
    foreach ($goods_att as $val)
    {
        ...
        if(!empty($val))
        {
            $sql="SELECT `attr_price` FROM " .$yp->table('goods_attr')." WHERE goods_id='$goods_id' AND `goods_attr_id`='$val'";
...
{
	$user_rank = $GLOBALS['db']->getOne("SELECT user_rank FROM " . $GLOBALS['yp']->table('users') . " WHERE user_id = '$user_id'");
...
function get_rank_info($user_id)
{
	...
	$sql = "SELECT rank_name FROM " . $GLOBALS['yp']->table('user_rank') . " WHERE rank_id = '$user_rank'";
...
{
$sql="SELECT * 
	FROM  ".$GLOBALS['yp']->table('collect_goods')." WHERE user_id='$user_id' and goods_id='$goods_id'";
...
{
	$sql = "SELECT * FROM ".$GLOBALS['yp']->table('volume_price')."WHERE goods_id = '$goods_id' order by volume_number ASC";
