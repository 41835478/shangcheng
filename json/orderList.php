...
$smarty->template_dir = ROOT_PATH . 'json/tpl';//app部分模板所在位置
	...
	switch($orderType){
		case 1:
			$sql="SELECT * 
		FROM  ".$yp->table('order_info')."  WHERE user_id='$uid' AND pay_status='0' AND order_status != 2 ORDER BY order_id DESC
		LIMIT $page,10";
			break;
		case 2:
			$sql="SELECT * 
		FROM  ".$yp->table('order_info')."  WHERE user_id='$uid' AND shipping_status='0' AND order_status != 2 ORDER BY order_id DESC
		LIMIT $page,10";
			break;
		case 3:
			$sql="SELECT * 
		FROM  ".$yp->table('order_info')."  WHERE user_id='$uid'  AND shipping_status='1' AND order_status != 2 ORDER BY order_id DESC
		LIMIT $page,10";
			break;
		case 4:
			$sql="SELECT * 
		FROM  ".$yp->table('order_info')."  WHERE user_id='$uid' ORDER BY order_id DESC
		LIMIT $page,10";
			break;
		default:
			$sql="SELECT * 
