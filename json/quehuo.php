...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'goodsinfo')
{
	...
	$sql = "SELECT bg.rec_id, bg.goods_id, bg.goods_number, bg.goods_desc, bg.booking_time, bg.dispose_note,g.goods_id, g.goods_name ,g.goods_thumb,g.supplier_id".
           " FROM " .$GLOBALS['yp']->table('booking_goods')." AS bg , " .$GLOBALS['yp']->table('goods')." AS g WHERE bg.goods_id = g.goods_id AND bg.user_id = '$user_id' ORDER BY bg.booking_time DESC LIMIT ".($page-1)*$size.",".$size;
	$res = $GLOBALS['db']->query($sql);
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		{
			$row['supplier_name'] = $GLOBALS['db']->getOne("select supplier_name from ".$GLOBALS['yp']->table('supplier')." where supplier_id = ".$row['supplier_id']);
		}
