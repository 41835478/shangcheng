...
if ($_POST['act'] == 'signin' && $_POST['acr'] == 'app')
{
    ...
    if ($user->login($username, $password))
    {
        ...
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['yp']->table('order_info')."WHERE user_id = '$user_id' AND pay_status = 0 AND order_status != 2 ";
        $user['payment'] = $GLOBALS['db']->getOne($sql);
        /*查找代发货的数据   jx*/
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['yp']->table('order_info')."WHERE user_id = '$user_id' AND shipping_status = 0 AND order_status != 2";
        $user['deliver'] = $GLOBALS['db']->getOne($sql);
        /*查找代收货的数据   jx*/
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['yp']->table('order_info')."WHERE user_id = '$user_id' AND shipping_status = 1 AND order_status != 2";
        $user['receipt'] = $GLOBALS['db']->getOne($sql);
        /*查找全部订单数据   jx*/
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['yp']->table('order_info')."WHERE user_id = '$user_id'";
...
//	file_put_contents('1.txt','weibo:'.var_export($_REQUEST,true));
	...
	$count = $db->getOne('SELECT COUNT(*) FROM '.$yp->table('users').' WHERE aite_id="'.$info['aite_id'].'"');
	
	if($count == 0)
	{
		$sql = 'INSERT INTO '.$yp->table('users').'(user_name,password,aite_id,sex,alias,reg_time,froms) VALUES("'.$type.'_'.rand().'","'.MD5($info['aite_id']).'","'.$info['aite_id'].'","'.$info['sex'].'","'.$info['alias'].'","'.time().'","app")';
		$try = 0;
	...
	{
		$user_id = $db->getOne('SELECT user_id FROM '.$yp->table('users').' WHERE aite_id="'.$aite_id.'"');
		$_SESSION['user_id'] = $user_id;
	}
	else
	{
