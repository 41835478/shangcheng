...
*/
	...
	//获取用户的收货地址
	$address = $db -> getRow("SELECT * FROM ".$yp->table('user_address')."  WHERE  `address_id`='$address_id';");
	
	$sql = "SELECT t.type_id, t.type_name, t.type_money, b.bonus_id " .
            "FROM " . $yp->table('bonus_type') . " AS t," .
                $yp->table('user_bonus') . " AS b " .
	...
	$sql="SELECT `pay_code` FROM ".$yp->table('payment') ." WHERE `pay_id`=".$order['pay_id'];
    $pay_code=$db->getOne($sql);
	
	if($pay_code=='balance')
    {
        $sql="SELECT `user_money` FROM ".$yp->table('users') ." WHERE `user_id`='$uid'";
        $user_money=$db->getOne($sql);
        if($user_money>$order_amount)
        {
            $sql="UPDATE ".$yp->table('users') . " SET `user_money`=`user_money`-$order_amount WHERE `user_id`='$uid'";
            $db->query($sql);
	...
	$db->autoExecute($yp->table('order_info'), $order, 'INSERT');
	$new_order_id = $db->insert_id();
	if($new_order_id){//使用余额支付，减去用户余额
		if($_REQUEST['user_meny']>0){
			$surplus=$_REQUEST['user_meny'];
			$sql="UPDATE ".$yp->table('users') . " SET `user_money`=`user_money`-$surplus WHERE `user_id`='$uid'";
            $db->query($sql);
		}
	}
	if($_REQUEST['integral']>0){//使用积分支付，减去用户积分
		...
		$sql="UPDATE ".$yp->table('users') . " SET `pay_points`=`pay_points`-$integral WHERE `user_id`='$uid'";
		$db->query($sql);
       ...
       $db->autoExecute($yp->table('account_log'), $order_log, 'INSERT');  
		
	}
	/*修改红包状态*/
	$sql="UPDATE  " .$yp->table('user_bonus') . " SET  `used_time` =  '".gmtime()."' , `order_id` =  '$new_order_id' WHERE  bonus_id ='$bonus_id'";
	...
	for($i=0;$i<$length;$i++){
		...
		$db -> query("update ".$yp->table('goods')." set goods_number = '$update_number' where goods_id = '$goods_id'");
	...
	if($rank_points>0){
       ...
       $order_log['change_time'] = gmtime();
       $db->autoExecute($yp->table('account_log'), $order_log, 'INSERT');
	   $sql="UPDATE ".$yp->table('users') . " SET `rank_points`=`rank_points`+$rank_points WHERE `user_id`='$uid'";
		$db->query($sql);
	}
	if($pay_points>0){
		$order_log=array();
       ...
       $order_log['change_time'] = gmtime();
       $db->autoExecute($yp->table('account_log'), $order_log, 'INSERT');
	   $sql="UPDATE ".$yp->table('users') . " SET `pay_points`=`pay_points`+$pay_points WHERE `user_id`='$uid'";
		$db->query($sql);
	}
