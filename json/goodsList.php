...
*/
	...
	$order=$_GET['order'];
	  // 获得分类的相关信息
	$sql = "SELECT * FROM " .$yp->table('category'). " WHERE cat_id='$cat' LIMIT 1";
	...
	if(!empty($cat)){
		...
		$sql = "SELECT * FROM " .$yp->table('category'). " WHERE parent_id='$cat' ";
		$cat_List=$db->getAll($sql);
		for($i=0;$i<count($cat_List);$i++){
			$cat.=",".$cat_List[$i]['cat_id'];
			$cat_id1=$cat_List[$i]['cat_id'];
			$sql = "SELECT * FROM " .$yp->table('category'). " WHERE parent_id ='$cat_id1' ";
		...
		$sql = "SELECT goods_id FROM " .$yp->table('goods_cat'). " WHERE cat_id IN ($cat) ";
	...
	if($field=="goods_number"){
		if(!empty($cat)){
			if(	$cat==0){
					...
					FROM  ".$yp->table('goods')." AS g LEFT JOIN ".$yp->table('order_goods')." AS og 
				...
				$sql="SELECT g.goods_id,g.goods_name,g.goods_brief,g.shop_price,g.goods_thumb,g.promote_price,g.is_promote,promote_end_date,promote_start_date,g.is_hot,g.is_new,g.is_best,g.click_count,SUM( og.goods_number ) AS goods_number 
				FROM  ".$yp->table('goods')." AS g LEFT JOIN ".$yp->table('order_goods')." AS og ON  og.goods_id = g.goods_id where g.is_delete = '0' AND g.is_on_sale = '1' AND (g.cat_id IN ($cat) OR g.goods_id IN ($cat_goods_ids) )  GROUP BY g.goods_id order by  goods_number DESC, g.shop_price $order   LIMIT $page,10";
			}
		}
		if(!empty($brand)){
			$sql="SELECT g.goods_id,g.goods_name,g.goods_brief,g.shop_price,g.goods_thumb,g.promote_price,g.is_promote,promote_end_date,promote_start_date,g.is_hot,g.is_new,g.is_best,g.click_count,SUM( og.goods_number ) AS goods_number 
			FROM  ".$yp->table('goods')." AS g  LEFT JOIN ".$yp->table('order_goods')." AS og ON  og.goods_id = g.goods_id where g.is_delete = '0' AND g.is_on_sale = '1' AND g.brand_id='$brand' AND  GROUP BY g.goods_id order by goods_number DESC , g.shop_price $order   LIMIT $page,10";
		...
		if(!empty($cat)){
			if(	$cat==0){
					$sql="SELECT g.add_time,g.goods_id,g.goods_name,g.goods_brief,g.shop_price,g.goods_thumb,g.promote_price,g.is_promote,promote_end_date,promote_start_date,g.is_hot,g.is_new,g.is_best,g.click_count FROM  ".$yp->table('goods')." AS g WHERE is_delete = '0' AND is_on_sale = '1' order by   g.$field $order  LIMIT $page,10 ";
			}else{
