...
*/
   ...
   $row = $db -> query("update ".$yp->table('order_info')." set shipping_status = '2' where order_sn = '$order_sn' AND `user_id`='$user_id'");
	...
	$sql="SELECT * FROM  ".$yp->table('order_info')." WHERE order_sn='$order_sn' ";
	...
	$sql="SELECT goods_name, goods_price,goods_number,goods_attr
		FROM  ".$yp->table('order_goods')." WHERE order_id='$order_id' ";
	//print_r($sql);
	$res = $db -> getAll($sql);
