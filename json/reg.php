...
if($_POST['acr'] == 'app')
{
	if($_POST['act'] == 'reg')
	{
		...
		$result=array();
		$row = $db -> getRow("SELECT * FROM ".$yp->table('users')."  WHERE  `user_name`='$user'");
		if(!empty($row)){
			$result['code']=2;
		...
		$row = $db -> getRow("SELECT * FROM ".$yp->table('users')."  WHERE  `email`='$email'");
		if(!empty($row)){
			$result['code']=3;
			$result['info']="该邮箱已经存在";
			print_r(json_encode($result));
		...
		$set=$db->autoExecute($yp->table('users'), $add, 'INSERT');
		if($set){
			/*注册赠送积分*/
			$sql="SELECT value FROM ".$yp->table('shop_config')." WHERE id='220'";
			...
			if($shop_config_integral>0){
			   ...
			   $db->autoExecute($yp->table('account_log'), $user_log, 'INSERT');
			   $sql="UPDATE ".$yp->table('users') . " SET `pay_points`=`pay_points`+$shop_config_integral WHERE `user_id`='$uid'";
				$db->query($sql);
			}
			if ($extend_field_str)      //插入注册扩展数据
			{
				...
				$sql = 'INSERT INTO '. $yp->table('reg_extend_info') . ' (`user_id`, `reg_field_id`, `content`) VALUES' . $extend_list;
				$db->query($sql);
			}
