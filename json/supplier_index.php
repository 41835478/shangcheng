...
if(!empty($GLOBALS['_CFG']['service_phone'])){
	...
	//jx  轮播图片正则匹配和获取
	$aa = $suppinfo['user_id'];
	$sql = "SELECT user_name FROM ".$GLOBALS['yp']->table('users')." WHERE user_id = '$aa'";
...
function get_supplier_goods($gtype=0,$limit=10){
	...
	$sql = "SELECT DISTINCT g.goods_id,g.* FROM ". $GLOBALS['yp']->table('goods') ." AS g, ". $GLOBALS['yp']->table('supplier_goods_cat') ." AS gc, ". $GLOBALS['yp']->table('supplier_cat_recommend') ." AS cr 
