...
/*优化了分类，去掉了之前开发的分类
if ($_REQUEST['act'] == 'get_catsel_qq120029121')
{
...
$sqlc = "select cat_id, cat_name, parent_id from ". $yp->table('category') ." where parent_id='$parentid' ";
$resc = $db->query($sqlc);
$option_temp ='';
$j=0;
...
$divid=$level+1;
$divid_next = $divid+1;
$arr['optionshtml'] = '<select name="cat_id_'. $divid .'" size=8 style="float:left;display:inline;margin-left:3px;" onchange="catsel_qq120029121(this, '. $divid .')"><option value="0" style="background:#dce6f0;">请选择&nbsp;&nbsp;&nbsp;&nbsp;</option>'. $option_temp .'</select><div id="catsel'. $divid_next .'" style="float:left;display:inline;margin-left:3px;"></div>';
$arr['divid']    = $divid;
$arr['count'] =$j;
...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
	...
	elseif ($_REQUEST['act'] == 'list' && $_REQUEST['supplier_status']=='2')
	{
		...
		$smarty->assign('supplier_status', 2);
	}
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
	...
	if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'copy'){//复制或者添加新商品    寒   冰      qq3095  8 5552 

        //商品总数     寒冰   2016.09.09     商家商品数量限制      qq   30 94 8 5552
        $goods_total = $db->getOne('SELECT COUNT(*) FROM '.$yp->table('goods').' WHERE is_delete=0 AND is_real=1 AND is_alone_sale=1 AND  supplier_id='.$_SESSION['supplier_id']);
		//店铺等级id
		$supplier_rank_id = $db->getOne('SELECT rank_id FROM '.$yp->table('supplier').' WHERE  supplier_id='.$_SESSION['supplier_id']);
		//店铺等级配置
		$supplier_rank = $db->getRow('SELECT * FROM '.$yp->table('supplier_rank').' WHERE  rank_id = '.$supplier_rank_id);
...
/*	优化了分类，去掉了之前开发的分类
	$pcats=array();
	$sqlc = "select cat_id, cat_name from ". $yp->table('category')." where  parent_id = 0 ";
	$resc= $db->query($sqlc);
	while ($rowc=$db->fetchRow($resc))
	{
	...
	$goods['buymax_start_date'] = local_date('Y-m-d H:i:s', $goods['buymax_start_date']);
	$goods['buymax_end_date'] = local_date('Y-m-d H:i:s', $goods['buymax_end_date']);
...
/*		优化了分类，去掉了之前开发的分类
		...
		$catoptions = array();
		$catoptions[1] = $db->getAll("select * from ". $yp->table('category') ." where parent_id=0");
		$kkkk=2;
		foreach ($catlist as $cattemp)
		{
			...
			$catoptions[$kkkk] = $db->getAll("select * from ". $yp->table('category') ." where parent_id=$cattemp");
			$kkkk++;
		}
...
*/		
	...
	/*$cate=array();
	$sqlc = "select cat_id, parent_id, cat_name from ". $yp->table('supplier_category') ." where supplier_id='". $_SESSION['supplier_id'] ."' ";
	$resc = $db->query($sqlc);
	while ($rowc = $db->fetchRow($resc))
	{
	...
	$smarty->assign('catstr',$catstr);*/ //20170818
	$smarty->assign('supplier_cat_id', $goods['supplier_cat_id']);
	$smarty->assign('catstr',cat_list_supplier(0, $goods['supplier_cat_id']));
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
	...
	$need_approve=$db->getOne("SELECT need_approve FROM " . $yp->table('supplier') . " WHERE supplier_id = '$supplier_id'");
	$supplier_status=$need_approve?0:1;
    $ghost_count = isset($_POST['ghost_count']) ? intval($_POST['ghost_count']) : '0';
    $click_count = isset($_POST['click_count']) ? intval($_POST['click_count']) : '0';
	//手机专享价格 app   jx 
    ...
    $supplier_cat_id = empty($_POST['supplier_cat_id']) ? '' : intval($_POST['supplier_cat_id']);
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $yp->table('goods') . " (goods_name, goods_name_style, ghost_count, click_count, goods_sn, " .
                    "cat_id,supplier_cat_id, cost_price, brand_id, shop_price, market_price, is_promote, zhekou, promote_price, " .
                ...
                "VALUES ('$_POST[goods_name]', '$goods_name_style','$ghost_count', '$click_count', '$goods_sn', '$catgory_id','$supplier_cat_id', '$cost_price', " .
                    ...
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral','$exclusive', '$supplier_id', '$supplier_status')";
					$is_distrib_goods = 1;
        ...
        else
        {
            $sql = "INSERT INTO " . $yp->table('goods') . " (goods_name, goods_name_style, ghost_count, click_count, goods_sn, " .
                ...
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$ghost_count','$click_count','$goods_sn', '$catgory_id', '$cost_price', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote', '$zhekou', '$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$is_buy','$buymax','$buymax_start_date','$buymax_end_date','$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code','$exclusive', '$rank_integral','$supplier_status')";
					$is_distrib_goods = 1;
        }
    }
    else
...
//今/天/优/品/多/商/户/系/统-采集 mod on 20160808 by prince  qq 1/2/0/0/2/9/1/2/1-begin
    $click_url = trim($_POST['click_url']) ;//今/天/优/品/多/商/户/系/统-采集 mod on 20160808 by prince  qq 1/2/0/0/2/9/1/2/1添加
	$sql = "UPDATE " . $yp->table('goods') . " SET " .
           "click_url = '$click_url' " .
...
/*编辑商品分成金额成为分销商品*/ 
    ...
    {
        handle_other_cat2($goods_id, intval($_POST['supplier_cat_id']));//20170818
    }
    ...
    {
        $link[0] = array('href' => 'goods.php?act=product_list&supplier_status='. $_REQUEST['supplier_status'] .'&goods_id=' . $goods_id, 'text' => '设置货品/规格价格');
    }
    if ($code == 'virtual_card')
    {
	...
	if($is_insert){	
	    if($need_approve){
			...
			$link[3] = array('href' => 'goods.php?act=list&supplier_status=1' , 'text' => '返回商品列表');
		}
	 }else{
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    {
        $smarty->assign('is_addbest', $_CFG['supplier_addbest']);
    }//20170907 mod by prince qq 120029121
    /* 排序标记 */
    $sort_flag  = sort_flag($goods_list['filter']);
    $smarty->assign($sort_flag['tag'], $sort_flag['img']);
...
elseif ($_REQUEST['act'] == 'export')
{
    ...
    $sql = "SELECT "
		...
		. "LEFT JOIN " . $GLOBALS['yp']->table('supplier_goods_cat') . " AS sg "
...
elseif ($_REQUEST['act'] == 'drop_goods')
{
	...
	$sql = "DELETE FROM " . $yp->table('supplier_goods_cat') . " WHERE goods_id = '$goods_id' and supplier_id = '$supplier_id'";
    $db->query($sql);
    if (empty($goods))
    {
...
elseif ($_REQUEST['act'] == 'product_add_execute')
{
    ...
    foreach($product['product_sn'] as $key => $value)
    {
        ...
        /* 插入货品表 */
        $sql = "INSERT INTO " . $GLOBALS['yp']->table('products') . " (goods_id, goods_attr, product_sn, product_number, pro_price)  VALUES ('" . $product['goods_id'] . "', '$goods_attr', '$value', '" . $product['product_number'][$key] . "','". $product['pro_price'][$key] . "')";//采集 mod on 20160808 by PRINCE  qq 1 2 0 0 2 9 1 2 1保存货品价格
    ...
    if ($insert)
    {
         ...
         $link[] = array('href' => 'goods.php?act=product_list&supplier_status='. $_REQUEST['supplier_status'] .'&goods_id=' . $product['goods_id'], 'text' => '设置货品/规格价格');
    }
    else
    {
...
//			}
 ...
 function handle_other_cat2($goods_id, $cat_id)//20170818
{


    /* 删除分类 */
    $sql = "DELETE FROM " . $GLOBALS['yp']->table('supplier_goods_cat') .
                " WHERE goods_id = '$goods_id' AND supplier_id= '$_SESSION[supplier_id]' " ;    
    $GLOBALS['db']->query($sql);
	
	
    /* 查询所有父分类 */
	$supplier_parent_cats=get_supplier_parent_cats($cat_id);

    /* 添加新加的分类 */
    foreach ($supplier_parent_cats AS $value)
    {
        // 插入记录
        $sql = "INSERT INTO " . $GLOBALS['yp']->table('supplier_goods_cat') .
                " (goods_id, cat_id, supplier_id) " .
                "VALUES ('$goods_id', '$value[cat_id]', '$_SESSION[supplier_id]')";
        $GLOBALS['db']->query($sql);
    }
}
...
function get_parent_cats($cat)
{
    ...
    $arr = $GLOBALS['db']->GetAll('SELECT cat_id, cat_name, parent_id FROM ' . $GLOBALS['yp']->table('category'));
...
//查出父分类 20170818
function get_supplier_parent_cats($cat){
    if ($cat == 0){
        return array();
    }

    $arr = $GLOBALS['db']->GetAll('SELECT cat_id, cat_name, parent_id FROM ' . $GLOBALS['yp']->table('supplier_category')." where supplier_id= '".$_SESSION['supplier_id']."'");
