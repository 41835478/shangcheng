...
if ($_REQUEST['act'] == 'insert')
{
    ...
    /* 检查类型是否有重复 */
    $sql = "SELECT COUNT(*) FROM " .$yp->table('bonus_type'). " WHERE supplier_id = ".$_SESSION['supplier_id']." AND type_name='$type_name'";
    if ($db->getOne($sql) > 0)
    {
    ...
    /* 插入数据库。 */
    $sql = "INSERT INTO ".$yp->table('bonus_type')." (type_name, type_money,send_start_date,send_end_date,use_start_date,use_end_date,send_type,min_amount,min_goods_amount,user_bonus_max,supplier_id)
    VALUES ('$type_name',
            '$_POST[type_money]',
            '$send_startdate',
            '$send_enddate',
...
if ($_REQUEST['act'] == 'send_by_print')
{
    ...
    for ($i = 0, $j = 0; $i < $bonus_sum; $i++)
    {
        ...
        $db->query("INSERT INTO ".$yp->table('user_bonus')." (bonus_type_id, bonus_sn, supplier_id) VALUES('$bonus_typeid', '$bonus_sn', '$_SESSION[supplier_id]')");
...
function get_type_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['yp']->table('bonus_type'). " WHERE supplier_id = ".$_SESSION['supplier_id'];
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        /* 分页大小 */
        $filter = page_and_size($filter);

        $sql = "SELECT * FROM " .$GLOBALS['yp']->table('bonus_type'). " WHERE supplier_id = ".$_SESSION['supplier_id']." ORDER BY $filter[sort_by] $filter[sort_order]";
...
function add_to_maillist($username, $email, $subject, $content, $is_html)
{
    ...
    $sql = "INSERT INTO "  . $GLOBALS['yp']->table('email_sendlist') . " ( email, template_id, email_content, pri, last_send, supplier_id) VALUES ('$email', $template_id, '$content', 1, '$time', '$_SESSION[supplier_id]')";
    $GLOBALS['db']->query($sql);
    return true;
}
