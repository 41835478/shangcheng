...
/**
 * QQ120029121 广告位置管理程序
 * ============================================================================
 * 演示地址: http://demo.coolhong.com  开发QQ:120029121    309485552
 * ============================================================================
 * $Author: prince $
 * $Id: ad_position.php 17217 2017-04-01 06:29:08Z prince $
*/
...
$exc = new exchange($yp->table("ypmart_ad_position"), $db, 'position_id', 'position_name');
...
if ($_REQUEST['act'] == 'list')
{
	...
	if($position_list['record_count']==0){
        create_ad_and_position($supplier_id);
    	$position_list = ad_position_list();
	}
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    {
        /* 将广告位置的信息插入数据表 */
        $sql = 'INSERT INTO '.$yp->table('ypmart_ad_position').' (supplier_id,position_name, ad_width, ad_height, position_desc, position_style) '.
               "VALUES ('$supplier_id','$position_name', '$ad_width', '$ad_height', '$position_desc', '$_POST[position_style]')";
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    /* 获取广告位数据 */
    $sql = 'SELECT * FROM ' .$yp->table('ypmart_ad_position'). " WHERE position_id='$id'";
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    /* 查看广告位是否与其它有重复 */
    $sql = 'SELECT COUNT(*) FROM ' .$yp->table('ypmart_ad_position').
           " WHERE position_name = '$position_name' AND position_id <> '$position_id'";
    if ($db->getOne($sql) == 0)
    {
        $sql = "UPDATE " .$yp->table('ypmart_ad_position'). " SET ".
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['yp']->table('ypmart_ad_position'). " WHERE position_id = '$id' and 	parent_position_id>0";
    if ($db->getOne($sql) > 0)
    {
        make_json_error('系统保留广告位，请勿删除！');
    }

    /* 查询广告位下是否有广告存在 */
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['yp']->table('ypmart_ad'). " WHERE position_id = '$id'";
...
function ad_position_list()
{
    ...
    /* 记录总数以及页数 */
    $sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['yp']->table('ypmart_ad_position'). " where supplier_id='$supplier_id' ";
    ...
    /* 查询数据 */
    $arr = array();
    $sql = 'SELECT * FROM ' .$GLOBALS['yp']->table('ypmart_ad_position'). " where supplier_id='$supplier_id'  ORDER BY position_id DESC";
