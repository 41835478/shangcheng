...
/**
 * QQ120029121 管理员信息以及权限管理程序
 * ============================================================================
 * 演示地址: http://demo.coolhong.com  开发QQ:120029121    309485552
 * ============================================================================
 * $Author: PRINCE $
 * $Id: privilege.php 17217 2017-04-01 06:29:08Z PRINCE $
 */
...
/* 初始化 $exc 对象 */
$exc = new exchange($yp->table("supplier_admin_user"), $db, 'user_id', 'user_name');
...
function action_logout ()
{
	...
	setcookie('YPCP[supplier_id]', '', 1,'/mobile/'.SUPPLIER_PATH.'/');
	setcookie('YPCP[supplier_user_id]', '', 1,'/mobile/'.SUPPLIER_PATH.'/');
	setcookie('YPCP[supplier_pass]', '', 1,'/mobile/'.SUPPLIER_PATH.'/');
...
function action_signin ()
{
	...
	if($row)
	{
		...
		/*if(empty($row['ec_salt']))
		{
			...
			$sql="UPDATE " . $yp->table('supplier_admin_user') . " SET ec_salt='" . $ec_salt . "', password='" . $new_possword . "'" . " WHERE user_id='$_SESSION[supplier_id]'";
			$db->query($sql);
		}*/
		...
		// 更新最后登录时间和IP
		$db->query("UPDATE " . $yp->table('supplier_admin_user') . " SET last_login='" . gmtime() . "', last_ip='" . real_ip() . "'" . " WHERE user_id='$_SESSION[supplier_user_id]'");
		
		if(isset($_POST['remember']))
		{
			setcookie('YPCP[supplier_id]', $row['supplier_id'], $time);
			setcookie('YPCP[supplier_user_id]', $row['user_id'], $time);
			setcookie('YPCP[supplier_pass]', md5($new_possword . $_CFG['hash_code']), $time);

		}
		
			$time = gmtime() + 3600 * 24 * 365;
			setcookie('YPCP[supplier_id]', $row['supplier_id'], $time,'/mobile/'.SUPPLIER_PATH.'/');
			setcookie('YPCP[supplier_user_id]', $row['user_id'], $time,'/mobile/'.SUPPLIER_PATH.'/');
			setcookie('YPCP[supplier_pass]', md5($new_possword . $_CFG['hash_code']), $time,'/mobile/'.SUPPLIER_PATH.'/');
	...
	/*
	 *
	 * $sql="SELECT `ec_salt` FROM ". $yp->table('users') ."WHERE user_name =
	 * '" . $_POST['username']."'";
	 * $ec_salt =$db->getOne($sql);
	 * if(!empty($ec_salt))
	 * {
	 ...
	 * $yp->table("supplier") ." where status='1' and user_id=" .
	 * $row['user_id']);
	 ...
	 * {
	 * $time = gmtime() + 3600 * 24 * 365;
	 * setcookie('YPCP[supplier_id]', $supplier_id, $time);
	 * setcookie('YPCP[supplier_user_id]', $row['user_id'], $time);
	 * setcookie('YPCP[supplier_pass]', md5($row['password'] .
	 * $_CFG['hash_code']), $time);
	 * }
	 *
	 * // 清除购物车中过期的数据
...
function action_insert ()
{
	...
	{
		$sql = "SELECT action_list FROM " . $yp->table('role') . " WHERE role_id = '" . $_POST['select_role'] . "'";
		$row = $db->getRow($sql);
		$action_list = $row['action_list'];
		$role_id = $_POST['select_role'];
	}
	
	$sql = "SELECT nav_list FROM " . $yp->table('supplier_admin_user') . " WHERE action_list = 'all' AND supplier_id=" . $_SESSION['supplier_id'];
	$row = $db->getRow($sql);
	
	$sql = "INSERT INTO " . $yp->table('supplier_admin_user') . " (user_name, email, password, add_time, nav_list, action_list, role_id, supplier_id, checked) " . "VALUES ('" . trim($_POST['user_name']) . "', '" . trim($_POST['email']) . "', '$password', '$add_time', '$row[nav_list]', '$action_list', '$role_id', '$_SESSION[supplier_id]', 1)";
...
function action_edit ()
{
	...
	/* 获取管理员信息 */
	$sql = "SELECT user_id, user_name, email, password, agency_id, role_id, supplier_id FROM " . $yp->table('supplier_admin_user') . " WHERE user_id = '" . $_REQUEST['id'] . "' AND supplier_id=" . $_SESSION['supplier_id'];
	$user_info = $db->getRow($sql);
	
	/* 取得该管理员负责的办事处名称 */
	if($user_info['agency_id'] > 0)
	{
		$sql = "SELECT agency_name FROM " . $yp->table('agency') . " WHERE agency_id = '$user_info[agency_id]'";
	...
	/* 获得该管理员的权限 */
	$priv_str = $db->getOne("SELECT action_list FROM " . $yp->table('supplier_admin_user') . " WHERE user_id = '$_GET[id]' AND supplier_id = '$_SESSION[supplier_id]'");
...
function action_update ()
{
    ...
    $action = ! empty($_REQUEST['act']) ? trim($_REQUEST['act']) : 'login';//20161224 prince
	...
	$sql = "select uid from " . $yp->table('supplier_admin_user') . " where user_id = '$admin_id' AND supplier_id=" . $_SESSION['supplier_id'];//20161224 prince
	$uid = $db->getOne($sql);//20161224 prince
	...
	if(! empty($admin_name))
	{
		...
		//20161224 prince start
		$sql = "select * from " . $yp->table('users') . " where user_name='" . $admin_name . "' and user_id<>'$uid' ";
	    $had_exist = $db->getRow($sql);
		if($had_exist)
		{
			sys_msg(sprintf($_LANG['user_name_exist'], stripslashes($admin_name)), 1);
		}
	...
	if(! empty($admin_email))
	{
		...
		//20161224 prince start
		$sql = "select * from " . $yp->table('users') . " where email='" . $admin_email . "' and user_id<>'$uid' ";
	    $had_exist = $db->getRow($sql);
		if($had_exist)
		{
			sys_msg(sprintf($_LANG['email_exist'], stripslashes($admin_email)), 1);
	...
	{
		/* 查询旧密码并与输入的旧密码比较是否相同 */
		$sql = "SELECT password FROM " . $yp->table('supplier_admin_user') . " WHERE user_id = '$admin_id' AND supplier_id=" . $_SESSION['supplier_id'];
		$old_password = $db->getOne($sql);
		$sql = "SELECT ec_salt FROM " . $yp->table('supplier_admin_user') . " WHERE user_id = '$admin_id' AND supplier_id=" . $_SESSION['supplier_id'];
	...
	if($pwd_modified)
	{
		$sql = "UPDATE " . $yp->table('supplier_admin_user') . " SET " . "user_name = '$admin_name', " . "email = '$admin_email', " . "ec_salt = '$ec_salt' " . $action_list . $role_id . $password . $nav_list . "WHERE user_id = '$admin_id' AND supplier_id=" . $_SESSION['supplier_id'];
	}
	else
	{
		$sql = "UPDATE " . $yp->table('supplier_admin_user') . " SET " . "user_name = '$admin_name', " . "email = '$admin_email' " . $action_list . $role_id . $nav_list . "WHERE user_id = '$admin_id' AND supplier_id=" . $_SESSION['supplier_id'];
	}
	
	$db->query($sql);
	
	// 同步会员信息 start 20161224 prince 120029121
	$passwd_weixin=$_POST['new_password'];
	if($pwd_modified)
	{
		$sql = "UPDATE " . $yp->table('users') . " SET " . "user_name = '$admin_name', " . "email = '$admin_email', " . "ec_salt = '$ec_salt' " .  $password .  ", passwd_weixin='$passwd_weixin' WHERE user_id = '$uid' " ;
	}
	else
	{
		$sql = "UPDATE " . $yp->table('users') . " SET " . "user_name = '$admin_name', " . "email = '$admin_email' WHERE user_id = '$uid' " ;
	}
	
	$db->query($sql);
	// 同步会员信息 end 20161224 prince 120029121
...
function action_modif ()
{
	...
	/* 包含插件菜单语言项 */
	$sql = "SELECT code FROM " . $yp->table('plugins');
	$rs = $db->query($sql);
	...
	/* 获得当前管理员数据信息 */
	$sql = "SELECT u.user_id, u.user_name, u.email, s.nav_list " . "FROM " . $yp->table('users') . " AS u, " . $yp->table('supplier') . "AS s WHERE u.user_id=s.user_id AND u.user_id = '" . $_SESSION['supplier_user_id'] . "'";
	$user_info = $db->getRow($sql);
...
function action_allot ()
{
	...
	/* 获取商家超级管理员 id*/
	$min_supplier_id_user_id = $db->getOne("SELECT user_id FROM " . $yp->table('supplier_admin_user') . " WHERE supplier_id = '$_SESSION[supplier_id]' order by user_id asc limit 1 ");
	
	/* 如果被编辑的管理员拥有了all这个权限或者是商家超级管理员，将不能编辑 prince qq 10260820 */
	if($priv_str == 'all' || $min_supplier_id_user_id==$_GET['id'])
	{
		$link[] = array(
	...
	$priv_str_prince = $db->getOne("SELECT action_list FROM " . $yp->table('supplier_admin_user') . " WHERE  user_id = '$_SESSION[supplier_user_id]' order by user_id asc limit 1 ");// ＰＲＩＮＣＥ　Ｑ　Ｑ　　１　２００２９１２１　　20160820

	/* 获取权限的分组数据 */
	$sql_query = "SELECT action_id, parent_id, action_code,relevance FROM " . $yp->table('supplier_admin_action') . " WHERE parent_id = 0";
	$res = $db->query($sql_query);
	while($rows = $db->FetchRow($res))
	{
		//if(strpos('prince'.$priv_str_prince.'qq120029121',$rows['action_code'])>=0){ // ＰＲＩＮＣＥ　Ｑ　Ｑ　　１　２００２９１２１　　20160820
			$priv_arr[$rows['action_id']] = $rows;
		//}
	}
	
	/* 按权限组查询底级的权限名称 */
	$sql = "SELECT action_id, parent_id, action_code,relevance FROM " . $yp->table('supplier_admin_action') . " WHERE parent_id " . db_create_in(array_keys($priv_arr));
	$result = $db->query($sql);
	
	while($priv = $db->FetchRow($result))
	{
		if($priv_str_prince='all' || strpos('prince'.$priv_str_prince.'qq120029121',$priv['action_code'])>0){ // ＰＲＩＮＣＥ　Ｑ　Ｑ　　１　２００２９１２１　　20170908
...
function action_update_allot ()
{
	...
	/* 取得当前管理员用户名 */
	$admin_name = $db->getOne("SELECT user_name FROM " . $yp->table('supplier_admin_user') . " WHERE user_id = '$_POST[id]' AND supplier_id = '$_SESSION[supplier_id]'");
	
	/* 更新管理员的权限 */
	$act_list = @join(",", $_POST['action_code']);
	$sql = "UPDATE " . $yp->table('supplier_admin_user') . " SET action_list = '$act_list', role_id = '' " . "WHERE user_id = '$_POST[id]' AND supplier_id = '$_SESSION[supplier_id]'";
...
{
	if($_SESSION['supplier_name']=='test'){
		make_json_error('对不起，您的权限不足，无法操作！');
	}
	// 全局变量
	// $user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	...
	/* 获得管理员用户名 */
	$action_list = $db->getOne('SELECT action_list FROM ' . $yp->table('supplier_admin_user') . " WHERE user_id='$id' AND supplier_id='$_SESSION[supplier_id]'");
...
{
	$list = array();
	$sql = 'SELECT user_id, user_name, email, add_time, last_login, supplier_id ' . 'FROM ' . $GLOBALS['yp']->table('supplier_admin_user') . ' where checked=1 and supplier_id = ' . $_SESSION['supplier_id'] . ' ORDER BY user_id DESC';
...
{
	/* 取得有效的session */
	$sql = "SELECT DISTINCT session_id " . "FROM " . $GLOBALS['yp']->table('cart') . " AS c, " . $GLOBALS['yp']->table('sessions') . " AS s " . "WHERE c.session_id = s.sesskey ";
	$valid_sess = $GLOBALS['db']->getCol($sql);
	
	// 删除cart中无效的数据
	$sql = "DELETE FROM " . $GLOBALS['yp']->table('cart') . " WHERE session_id NOT " . db_create_in($valid_sess);
...
function get_role_list ()
{
	return '';
	$list = array();
	$sql = 'SELECT role_id, role_name, action_list ' . 'FROM ' . $GLOBALS['yp']->table('role');
	$list = $GLOBALS['db']->getAll($sql);
	return $list;
}
