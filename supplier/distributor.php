 ...
 * ============================================================================
 * $Author: p r i n c e $
 * $Id: distributor.php 17217 2017-04-01 06:29:08Z p r i n c e $
*/
...
$yp = $GLOBALS['yp'];
$db = $GLOBALS['db'];
$exc = new exchange($yp->table('users'), $db, 'user_id', 'user_name');
...
if ($_REQUEST['act'] == 'list')
{
	...
	{
		 //所有注册会员都是分销商
		$GLOBALS['db']->query("UPDATE " . $GLOBALS['yp']->table('users') . " SET is_fenxiao = 1 WHERE is_fenxiao = 0");
	}
	else
	{
		 ...
		 //没达到条件的所有会员变为普通会员
		 $GLOBALS['db']->query("UPDATE " . $GLOBALS['yp']->table('users') . " SET is_fenxiao = 2 WHERE is_fenxiao <> 0 AND " . "(".$fx_where.")");
		 //达到条件的所有会员晋级为分销商
		 $GLOBALS['db']->query("UPDATE " . $GLOBALS['yp']->table('users') . " SET is_fenxiao = 1 WHERE is_fenxiao <> 0 AND " . "(".$ex_where.")");	
	}*/
     //所有注册会员都是分销商
     $supplier_id = $_SESSION['supplier_id'];
	 $GLOBALS['db']->query("UPDATE " . $GLOBALS['yp']->table('users') . " SET is_fenxiao = 1 WHERE is_fenxiao = 0 AND supplier_id = '$supplier_id'");
    $user_list = user_list();
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT u.user_name, u.sex, u.birthday, u.pay_points, u.rank_points, u.user_rank , u.user_money, u.frozen_money, u.credit_line, u.parent_id, u2.user_name as parent_username, u.qq, u.msn, u.office_phone, u.home_phone, u.mobile_phone".
        " FROM " .$yp->table('users'). " u LEFT JOIN " . $yp->table('users') . " u2 ON u.parent_id = u2.user_id WHERE u.user_id='$_GET[id]' AND u.supplier_id = '$supplier_id'";
...
elseif ($_REQUEST['act'] == 'update')
{
	...
	elseif($parent_id > 0)
	{
		$sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('users') . " WHERE user_id = '$parent_id'";
		if($db->getOne($sql) == 0)
		{
		...
		else
		{
			 ...
			 {
				 $sql = "SELECT parent_id FROM " . $yp->table('users') . " WHERE user_id = '$parent_id'";
				 $pid = $db->getOne($sql);
				 if($pid == $uid)
				 {
	...
	$sql = "update ".$yp->table('users')." set `real_name`='$real_name',`card`='$card',`country`='$country',`province`='$province',`city`='$city',`district`='$district',`address`='$address',`status`='$status' ".$con." where user_name = '".$username."'";
	$db->query($sql);
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $sql = "SELECT user_name FROM " . $yp->table('users') . " WHERE user_id = '" . $_GET['id'] . "' AND supplier_id = '$supplier_id'";
    $username = $db->getOne($sql);
    ...
    sys_msg(sprintf($_LANG['remove_success'], $username), 0, $link);
     ...
     $link[] = array('text' => $_LANG['go_back'], 'href'=>'distributor.php?act=list');
      sys_msg(sprintf('操作失败！', $username), 0, $link);
...
elseif ($_REQUEST['act'] == 'address_list')
{
    ...
    $sql = "SELECT a.*, c.region_name AS country_name, p.region_name AS province, ct.region_name AS city_name, d.region_name AS district_name ".
           " FROM " .$yp->table('user_address'). " as a ".
           " LEFT JOIN " . $yp->table('region') . " AS c ON c.region_id = a.country " .
           " LEFT JOIN " . $yp->table('region') . " AS p ON p.region_id = a.province " .
           " LEFT JOIN " . $yp->table('region') . " AS ct ON ct.region_id = a.city " .
           " LEFT JOIN " . $yp->table('region') . " AS d ON d.region_id = a.district " .
           " WHERE user_id='$id'";
...
elseif ($_REQUEST['act'] == 'remove_parent')
{
    ...
    $sql = "UPDATE " . $yp->table('users') . " SET parent_id = 0 WHERE user_id = '" . $_GET['id'] . "' AND supplier_id = '$supplier_id'";
    ...
    for ($i = 1; $i<=$num; $i++)
    {
        ...
        if ($count)
        {
            $sql = "SELECT user_id, user_name, '$i' AS level, email, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time ".
                    " FROM " . $GLOBALS['yp']->table('users') . " WHERE user_id IN($up_uid)" .
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        $filter['sort_order'] = empty($_REQUEST['sort_order']) ? 'DESC'     : trim($_REQUEST['sort_order']);
         ...
         $supplier_id = $_SESSION['supplier_id'];
		$ex_where = " WHERE 1 AND supplier_id = '$supplier_id'";
		...
		$sql = "SELECT user_id, user_name, email, is_validated, validated,status,user_money, frozen_money, rank_points, pay_points, reg_time,is_fenxiao,parent_id ".    
                " FROM " . $GLOBALS['yp']->table('users') . $ex_where .
    ...
    for ($i=0; $i<$count; $i++)
    {
		...
		$user_list[$i]['split_money'] = get_sum_split_money($user_list[$i]['user_id']); //分成结余
...
function get_user_count($user_id,$level)
{
    ...
    for ($i = 1; $i<=$level; $i++)
    {
        ...
        if ($up_uid)
        {
            $sql = "SELECT user_id FROM " . $GLOBALS['yp']->table('users') . " WHERE parent_id IN($up_uid)";
	...
	if($count)
	{
		$sql = "SELECT count(*) FROM " . $GLOBALS['yp']->table('users') . " WHERE user_id IN($up_uid)";
...
{
	 $sql = "SELECT user_name FROM " . $GLOBALS['yp']->table('users') . " WHERE user_id = '$user_id'";
...
{
	$sql = "SELECT user_id FROM " . $GLOBALS['yp']->table('users') . " WHERE user_name = '$username'";
	return $GLOBALS['db']->getOne($sql); 
}


function get_sum_split_money($user_id)
{
	 $sql = "SELECT sum(money) FROM " . 
	 		$GLOBALS['yp']->table('affiliate_log') . 
