 ...
 * ============================================================================
 * $Author: PRINCE $
 * $Id: virtual_card.php 17217 2017-04-01 06:29:08Z PRINCE $
 */
...
if($_REQUEST['act']=='verification_info'){
    ...
    if(!empty($card_sn)){
        $sql = "select v.*,g.goods_name,g.goods_thumb,shop_price from ".$yp->table('virtual_goods_card')."as v left join ".$yp->table('goods')." as g on v.goods_id= g.goods_id where v.card_sn = '$card_sn' and v.is_saled = 1 and v.supplier_id = $supplier_id";
        $res = $db -> getRow($sql);
        if($res){
...
if($_REQUEST['act']=='verification'){
    ...
    $now = gmtime();
    if(!empty($card_sn)){
        $sql = "select * from ".$yp->table('virtual_goods_card')." where card_sn = '$card_sn' and is_saled = 1 and supplier_id = $_SESSION[supplier_id]";
        $card = $db -> getRow($sql);
        if($card){
   ...
   if($result['msg'] == '1'){
        $sql1 = "update ".$yp->table('virtual_goods_card')." set is_verification = 1 , buy_date = ".gmtime()." where card_sn = '$card_sn'";
        $db -> query($sql1);

         /* 改变订单状态 */
        if($card){
             $sql = "select count(*) from ".$yp->table('virtual_goods_card')." where card_sn = '$card_sn' and  supplier_id = $_SESSION[supplier_id] and is_verification=1";
            if($db->getOne($sql)){
                $sql = "update ".$yp->table('order_info')." set order_status = '5', shipping_status = '2' , shipping_time_end = ".gmtime()." where order_sn = '$card[order_sn]'";
            }else{
                $sql = "update ".$yp->table('order_info')." set shipping_status = '2' , shipping_time_end = ".gmtime()." where order_sn = '$card[order_sn]'";
            }
            $db -> query($sql);
    ...
    $goods = $db -> getRow("select goods_name,goods_thumb,shop_price from ".$yp->table('goods')." where goods_id = $card[goods_id]");
...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    $sql = "SELECT MAX(order_id) FROM " . $yp->table('order_info') . " as o WHERE supplier_id = '$order[supplier_id]' and order_id < '$order[order_id]' and extension_code = 'virtual_good'";
    if ($agency_id > 0)
    {
    ...
    $sql = "SELECT MIN(order_id) FROM " . $yp->table('order_info') . " as o WHERE supplier_id = '$order[supplier_id]' and order_id > '$order[order_id]' and extension_code = 'virtual_good'";
...
function get_virtual_order_list(){    
  ...
  //  $where .= ($filter['is_verification'] !='') ? " AND is_verification = '" . $filter['is_verification'] . "' " : '';
    ...
    $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('order_info') . "  WHERE 1 and  supplier_id = ".$_SESSION['supplier_id']." AND extension_code = 'virtual_good'  and order_status <> '".OS_INVALID."' " .$where;
...
function get_orderid_by_ordersn($order_sn){
    $sql = "select a.order_id, b.goods_price from ". $GLOBALS['yp']->table('order_info') .
            "as a left join ". $GLOBALS['yp']->table('order_goods') ." as b on a.order_id = b.order_id".
