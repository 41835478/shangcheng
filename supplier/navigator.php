 ...
 * ==========================================================
 * $Author: PRINCE $
 * $Id: navigator.php 17217 2017-04-01 06:29:08Z PRINCE $
 */
...
$exc = new exchange($yp->table("supplier_nav"), $db, 'id', 'name');
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    elseif ($_REQUEST['step'] == 2)
    {
        ...
        $vieworder = $db->getOne("SELECT max(vieworder) FROM ". $yp->table('supplier_nav') . " WHERE type = '". $item_type ."' and supplier_id=".$_SESSION['supplier_id']);
        ...
        if($item_ifshow == 1 && $item_type == 'middle')
        {
            ...
            if($arr)
            {
                ...
                $sql = "INSERT INTO " . $GLOBALS['yp']->table('supplier_nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,type,supplier_id) VALUES('$item_name','".$arr['type']."','".$arr['id']."','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_type',".$_SESSION['supplier_id'].")";
            }
        }

        if(empty($sql))
        {
            $sql = "INSERT INTO " . $GLOBALS['yp']->table('supplier_nav') . " (name,ifshow,vieworder,opennew,url,type,supplier_id) VALUES('$item_name','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_type',".$_SESSION['supplier_id'].")";
        }
        $db->query($sql);
        clear_cache_files();
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    if (empty($_REQUEST['step']))
    {
        ...
        $row = $db->getRow("SELECT * FROM " . $GLOBALS['yp']->table('supplier_nav') . " WHERE id='$id'");
    ...
    elseif ($_REQUEST['step'] == 2)
    {
        ...
        $row = $db->getRow("SELECT ctype,cid,ifshow,type FROM " . $GLOBALS['yp']->table('supplier_nav') . " WHERE id = '$id'");
        ...
        if($arr)
        {
            ...
            $sql = "UPDATE " . $GLOBALS['yp']->table('supplier_nav') .
...
{
    $id = (int)$_GET['id'];
    $row = $db->getRow("SELECT ctype,cid,type FROM " . $GLOBALS['yp']->table('supplier_nav') . " WHERE id = '$id' AND supplier_id=".$_SESSION['supplier_id']." LIMIT 1");
    ...
    $sql = " DELETE FROM " . $GLOBALS['yp']->table('supplier_nav') . " WHERE id='$id' AND supplier_id=".$_SESSION['supplier_id']." LIMIT 1";
    $db->query($sql);
    clear_cache_files();
    yp_header("Location: navigator.php?act=list\n");
    exit;
}
...
if ($_REQUEST['act'] == 'toggle_ifshow')
{
    ...
    $row = $db->getRow("SELECT type,ctype,cid FROM " . $GLOBALS['yp']->table('supplier_nav') . " WHERE id = '$id' LIMIT 1");
...
function get_nav()
{
    ...
    if($result === false)
    {
        ...
        $sql = "SELECT count(*) FROM " .$GLOBALS['yp']->table('supplier_nav'). " WHERE supplier_id=".$_SESSION['supplier_id'];
        ...
        $sql = "SELECT id, name, ifshow, vieworder, opennew, url, type".
               " FROM ".$GLOBALS['yp']->table('supplier_nav')." where supplier_id=".$_SESSION['supplier_id'].
...
function nav_update($id, $args)
{
    ...
    return $GLOBALS['db']->autoExecute($GLOBALS['yp']->table('supplier_nav'), $args, 'update', "id='$id'");
}
...
function is_show_in_nav($type, $id)
{
    if($type == 'c')
    {
        $tablename = $GLOBALS['yp']->table('supplier_category');
    }
    else
    {//目前没用到
        $tablename = $GLOBALS['yp']->table('article_cat');
    }
