...
/**
 * QQ120029121 管理中心批发管理
 * ============================================================================
 * 演示地址: http://demo.coolhong.com  开发QQ:120029121    309485552
 * ============================================================================
 * $Author: PRINCE $
 * $Id: wholesale.php 17217 2017-04-01 06:29:08Z PRINCE $
 */
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 删除记录 */
    $sql = "DELETE FROM " . $yp->table('wholesale') .
...
elseif ($_REQUEST['act'] == 'batch_add_insert')
{
    ...
    $sql = "SELECT goods_name, goods_id FROM " . $yp->table('goods') .
    ...
    if (isset($_POST['rank_id']))
    {
        ...
        {
            $sql = "SELECT COUNT(act_id) AS num, goods_id FROM " . $yp->table('wholesale') .
                    " WHERE goods_id IN ($dst_goods) " .
    ...
    foreach ($_POST['dst_goods_lists'] as $goods_value)
    {
        ...
        /* 保存数据 */
        $db->autoExecute($yp->table('wholesale'), $_wholesale, 'INSERT');
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if (isset($_POST['rank_id']))
    {
        foreach ($_POST['rank_id'] as $rank_id)
        {
            $sql = "SELECT COUNT(*) FROM " . $yp->table('wholesale') .
                    " WHERE goods_id = '$goods_id' " .
                    " AND CONCAT(',', rank_ids, ',') LIKE CONCAT('%,', '$rank_id', ',%')";
            if (!$is_add)
            {
    ...
    $sql = "SELECT a.attr_id " .
            "FROM " . $yp->table('goods') . " AS g, " . $yp->table('attribute') . " AS a " .
    ...
    foreach ($key_list as $key)
    {
        ...
        if (!empty($attr))
        {
            ...
            $sql = "SELECT product_id FROM " . $yp->table('products') . " WHERE goods_attr = '$goods_attr' AND goods_id = '$goods_id'";
            if (!$db->getOne($sql))
            {
                $attr_error = true;
                continue;
    ...
    if ($is_add)
    {
        $db->autoExecute($yp->table('wholesale'), $wholesale, 'INSERT');
        $wholesale['act_id'] = $db->insert_id();
    }
    else
    {
        $db->autoExecute($yp->table('wholesale'), $wholesale, 'UPDATE', "act_id = '$wholesale[act_id]'");
    }
...
function wholesale_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('wholesale') .
                " WHERE 1 $where";
        ...
        /* 查询 */
        $sql = "SELECT * ".
                "FROM " . $GLOBALS['yp']->table('wholesale') .
                " WHERE 1 $where ".
