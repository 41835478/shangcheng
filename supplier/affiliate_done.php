...
function get_affiliate_ck()
{
   ...
   $sql = "SELECT value FROM " . $GLOBALS['yp']->table('supplier_shop_config') . " WHERE code = 'affiliate' AND supplier_id = '$supplier_id'";
	...
	$sql = "SELECT order_sn,is_separate,o.split_money,order_id,o.user_id,add_time,order_status,o.pay_status,o.shipping_status,u.user_name FROM ".$GLOBALS['yp']->table('order_info')." AS o," . $GLOBALS['yp']->table('users') . " as u WHERE o.pay_status = 2 and o.is_separate>0 AND o.user_id = u.user_id $sqladd ORDER BY order_id DESC LIMIT " . $filter['start'] . ",$filter[page_size]";
