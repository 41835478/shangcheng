...
if(!$id){
	...
	if($content){
		...
		if($_GET['do'] == 1){
			...
			$db->query("update {$yp->table('goods')} set 
				goods_desc='{$content}',goods_thumb='{$imgArr[0]['thumb']}',goods_img='{$imgArr[0]['goods']}',original_img='{$imgArr[0]['source']}'{$uptitle} 
				where goods_id={$gid}");
			foreach($imgArr as $v){
				$db->query("insert into {$yp->table('goods_gallery')} (goods_id,img_url,thumb_url,img_original) value ($gid,'{$v[goods]}','{$v[thumb]}','{$v[source]}')");
			}
			if($iscomment == 1 && $comment_list){
				$t = time();
				foreach($comment_list as $k=>$c){
					$t = $t-832*$k;
					$db->query("insert into {$yp->table('comment')} (id_value,content,comment_rank,add_time,user_name,status) 
...
function createOrder($gid,$t,$nick){
	...
	$goods = $GLOBALS['db']->getRow("select * from ".$GLOBALS['yp']->table('goods')." where goods_id=$gid");
	$sql = "INSERT INTO " . $GLOBALS['yp']->table('order_goods') . "( " .
