...
elseif ($_REQUEST['act'] == 'top')
{
    // 获得管理员设置的菜单
    $lst = array();
    $nav = $db->GetOne('SELECT nav_list FROM ' . $yp->table('supplier') . " WHERE user_id = '" . $_SESSION['supplier_user_id'] . "'");
...
{
	/* 供货商文章 */
	$sql = "select cat_id from ". $yp->table('article_cat') ." where cat_type='99' ";
	$article_cat_id = $db->getOne($sql);
	$sql = "select article_id, title, add_time  from ". $yp->table('article') ." where cat_id = '$article_cat_id' ";
	$sres = $db->query($sql);
    ...
    /*后台管理起始页_添加_START_demo.coolhong.com*/
...
//    $today['money'] = $db->GetOne('SELECT SUM(money_paid) AS money FROM ' .$yp->table('order_info'). ' WHERE `add_time` BETWEEN '.$today_start.' AND '.$today_end.' AND supplier_id='.$_SESSION['supplier_id'].order_query_sql('finished'));
    $today['money'] = $db->GetOne('SELECT SUM(goods_amount) AS money FROM ' .$yp->table('order_info'). ' WHERE `add_time` BETWEEN '.$today_start.' AND '.$today_end.' AND supplier_id='.$_SESSION['supplier_id'].order_query_sql('finished'));
    /* 代码修改 By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121 End */
    $today['money'] = price_format($today['money']);
    //今日订单数
    $today['order'] = $db->GetOne('SELECT COUNT(*) FROM ' .$yp->table('order_info'). ' WHERE `confirm_time` BETWEEN '.$today_start.' AND '.$today_end.' AND supplier_id='.$_SESSION['supplier_id']);
    $smarty->assign('today',$today);
    //待处理事务
    $task = array();
    //待审核商品
    $task['goods'] = $db->getOne('SELECT COUNT(*) FROM '.$yp->table('goods').' WHERE supplier_status=0 AND supplier_id='.$_SESSION['supplier_id']);
    //待处理佣金
    //$task['commission'] = $db->getOne('SELECT COUNT(*) FROM '.$yp->table('supplier_rebate').' WHERE is_pay_ok=0 '.' AND supplier_id='.$_SESSION['supplier_id']);
    //未查看平台文章
    //$sql = "SELECT cat_id FROM ". $yp->table('article_cat') ." WHERE cat_type='99' ";
	//$article_cat_id = $db->getOne($sql);
    $task['article'] = $db->getOne("SELECT COUNT(*)  FROM ". $yp->table('supplier_article') ." WHERE supplier_id = ".$_SESSION['supplier_id']);
    $task['total'] = $task['goods'] + $task['commission'] + $task['article'];
    $smarty->assign('task',$task);
    
    //商品相关
    //商品总数
    $goods['total'] = $db->getOne('SELECT COUNT(*) FROM '.$yp->table('goods').' WHERE is_delete=0 AND is_real=1 AND is_alone_sale=1 AND supplier_id='.$_SESSION['supplier_id']);
    //审核通过商品数
    $goods['pass'] = $db->getOne('SELECT COUNT(*) FROM '.$yp->table('goods').' WHERE is_delete=0 AND is_real=1 AND is_alone_sale=1 AND supplier_status=1 AND supplier_id='.$_SESSION['supplier_id']);
    //审核未通过商品数
    $goods['impass'] = $db->getOne('SELECT COUNT(*) FROM '.$yp->table('goods').' WHERE is_delete=0 AND is_real=1 AND is_alone_sale=1 AND supplier_status=-1 AND supplier_id='.$_SESSION['supplier_id']);
    //库存警告
    if ($_CFG['use_storage'])
    {
        $sql = 'SELECT COUNT(*) FROM ' .$yp->table('goods'). ' WHERE is_delete = 0 AND goods_number <= warn_number AND is_real = 1 AND supplier_id='.$_SESSION['supplier_id'];
    ...
    //已下架商品总数
    $goods['deleted'] = $db->getOne(' SELECT COUNT(*) FROM '.$yp->table('goods').' WHERE is_on_sale = 0 AND is_real=1 AND is_delete=0 AND is_alone_sale = 1 AND supplier_id='.$_SESSION['supplier_id']);
    $smarty->assign('goods',$goods);
    
    //订单相关
    //订单总数
    $order['total'] = $db->GetOne('SELECT COUNT(*) FROM '.$yp->table('order_info'). ' WHERE 1'.' AND supplier_id='.$_SESSION['supplier_id']);
    //待发货订单
    ...
    //待确认订单
    $order['unconfirmed']  = $db->GetOne('SELECT COUNT(*) FROM ' .$yp->table('order_info').
	...
	//退款申请
	//$sql = 'SELECT COUNT(*) FROM ' . $yp->table('back_order') . ' WHERE status_back=5 AND supplier_id="'.$_SESSION['supplier_id'].'"';
    //$order['new_repay'] = $db->getOne($sql );
    $order['new_repay'] = $db->getOne('SELECT COUNT(*) FROM '.$yp->table('back_order').' WHERE status_back < 6 AND status_back != 3 AND back_type = 4 AND supplier_id='.$_SESSION['supplier_id'].'');// 代码修改   By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121 End
    //退货申请
    //$order['returns'] = $db->getOne('SELECT COUNT(*) FROM '.$yp->table('back_order').' WHERE status_back=5 AND supplier_id="'.$_SESSION['supplier_id'].'"');
    $order['returns'] = $db->getOne('SELECT COUNT(*) FROM '.$yp->table('back_order').' WHERE status_back < 6 AND status_back != 3 AND back_type = 1 AND supplier_id="'.$_SESSION['supplier_id'].'"');
    //缺货登记
    $order['booking_goods'] = $db->getOne('SELECT COUNT(*) FROM ' . $yp->table('booking_goods') . ' AS bg LEFT JOIN '.$yp->table('goods').' AS g ON bg.goods_id=g.goods_id WHERE g.supplier_id="'.$_SESSION['supplier_id'].'" AND is_dispose=0');

   
    //已完成订单总数
    $order['finished'] = $db->GetOne('SELECT COUNT(*) FROM '.$yp->table('order_info'). ' WHERE 1'.order_query_sql('finished').' AND supplier_id='.$_SESSION['supplier_id']);
    ...
    $sql = 'SELECT `froms`, count(*) AS `count` FROM '.$yp->table('order_info').' WHERE `confirm_time` BETWEEN '.$month_start.' AND '.$month_end. ' AND supplier_id='.$_SESSION['supplier_id'].' GROUP BY `froms` ORDER BY `count` DESC';
    $result = $db->query($sql);
    ...
    $result = $db->query('SELECT DATE_FORMAT(FROM_UNIXTIME(`confirm_time`),"%d") AS day,COUNT(*) AS count,SUM(money_paid) AS money FROM '.$yp->table('order_info').' WHERE `confirm_time` BETWEEN '.$month_start.' AND '.$month_end.' '.order_query_sql('finished').'  AND supplier_id='.$_SESSION['supplier_id'].' GROUP BY day ORDER BY day ASC ');
    ...
    /*后台管理起始页_添加_END_demo.coolhong.com*/
...
elseif ($_REQUEST['act'] == 'main_api')
{
    ...
    ' WHERE is_delete = 0 AND is_alone_sale = 1 AND is_real = 1');
        ...
        $apiget = "ver= $yp_version &lang= $yp_lang &release= $yp_release &php_ver= $php_ver &mysql_ver= $mysql_ver &ocount= $ocount &oamount= $oamount &gcount= $gcount &charset= $yp_charset &usecount= $yp_user &template= $yp_template &style= $yp_style &url= $shop_url &patch= $patch_file ";

        $t = new transport;
        //$api_comment = $t->request('http://api.demo.coolhong.com/checkver.php', $apiget);
