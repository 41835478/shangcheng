...
if ($_REQUEST['act'] == 'list')
{
    ...
    for ($i = 0; $i < count($modules); $i++)
    {
        ...
        /* 检查该插件是否已经安装 */
        $sql = "SELECT shipping_id, shipping_name, shipping_desc, insure, support_cod,shipping_order,is_default_show FROM " .$yp->table('shipping'). " WHERE shipping_code='" .$modules[$i]['code']. "' and supplier_id=".$_SESSION['supplier_id']." ORDER BY shipping_order";
        $row = $db->GetRow($sql);
...
elseif ($_REQUEST['act'] == 'install')
{
    ...
    /* 检查该配送方式是否已经安装 */
    $sql = "SELECT shipping_id FROM " .$yp->table('shipping'). " WHERE shipping_code = '$_GET[code]' and supplier_id=".$_SESSION['supplier_id'];
    $id = $db->GetOne($sql);

    if ($id > 0)
    {
        /* 该配送方式已经安装过, 将该配送方式的状态设置为 enable */
        $db->query("UPDATE " .$yp->table('shipping'). " SET enabled = 1 WHERE shipping_code = '$_GET[code]' and supplier_id=".$_SESSION['supplier_id']." LIMIT 1");
    }
    else
    {
...
elseif ($_REQUEST['act'] == 'uninstall')
{
    ...
    /* 获得该配送方式的ID */
    $row = $db->GetRow("SELECT shipping_id,shipping_code, shipping_name, print_bg FROM " .$yp->table('shipping'). " WHERE shipping_code='$_GET[code]' and supplier_id=".$_SESSION['supplier_id']);
...
elseif ($_REQUEST['act'] == 'is_default_show')
{
	...
	$sql = "select shipping_id,shipping_name from ".$yp->table("shipping")." where is_default_show=0 and supplier_id=".$_SESSION['supplier_id']." and enabled=1 and shipping_code='".$_REQUEST['code']."'";
	$info = $db->getRow($sql);
	if($info){
