...
/**
 * SZY QQ120029121 预售管理
 * ============================================================================
 * 演示地址: http://demo.coolhong.com；
 * ============================================================================
 * $Author: prince $
 * $Id: presale.php 17217 2017-04-01 06:29:08Z niqingyang $
 */
...
function action_insert_update ()
{
	...
	if(isset($_POST['finish']))
	{
		...
		/* 结束预售活动，修改结束时间为当前时间 */
		$sql = "UPDATE " . $yp->table('goods_activity') . " SET end_time = '" . gmtime() . "' " . "WHERE act_id = '$pre_sale_id' LIMIT 1";
		$db->query($sql);
	...
	elseif(isset($_POST['succeed']))
	{
		...
		if($pre_sale['total_order'] > 0)
		{
			...
			$sql = "SELECT order_id " . "FROM " . $yp->table('order_info') . " WHERE extension_code = '" . PRE_SALE_CODE . "' " . "AND extension_id = '$pre_sale_id' " . "AND (order_status = '" . OS_CONFIRMED . "' or order_status = '" . OS_UNCONFIRMED . "')";
			$order_id_list = $db->getCol($sql);
			
			/* 更新订单商品价 */
			$final_price = $pre_sale['trans_price'];
			$sql = "UPDATE " . $yp->table('order_goods') . " SET goods_price = '$final_price' " . "WHERE order_id " . db_create_in($order_id_list);
			$db->query($sql);
			
			/* 查询订单商品总额 */
			$sql = "SELECT order_id, SUM(goods_number * goods_price) AS goods_amount " . "FROM " . $yp->table('order_goods') . " WHERE order_id " . db_create_in($order_id_list) . " GROUP BY order_id";
			$res = $db->query($sql);
		...
		/* 修改预售活动状态为成功 */
		$sql = "UPDATE " . $yp->table('goods_activity') . " SET is_finished = '" . PSS_SUCCEED . "' " . "WHERE act_id = '$pre_sale_id' LIMIT 1";
		$db->query($sql);
	...
	elseif(isset($_POST['fail']))
	{
		...
		{
			/* 查找未确认或已确认的订单 */
			$sql = "SELECT * " . "FROM " . $yp->table('order_info') . " WHERE extension_code = '" . PRE_SALE_CODE . "' " . "AND extension_id = '$pre_sale_id' " . "AND (order_status = '" . OS_CONFIRMED . "' OR order_status = '" . OS_UNCONFIRMED . "') ";
			$res = $db->query($sql);
		...
		$sql = "UPDATE " . $yp->table('goods_activity') . " SET is_finished = '" . PSS_FAIL . "', " . "act_desc = '$_POST[act_desc]' " . "WHERE act_id = '$pre_sale_id' LIMIT 1";
		$db->query($sql);
	...
	elseif(isset($_POST['mail']))
	{
		...
		/* 取得有效订单 */
		$sql = "SELECT o.consignee, o.add_time, g.goods_number, o.order_sn, " . "o.order_amount, o.order_id, o.email " . "FROM " . $yp->table('order_info') . " AS o, " . $yp->table('order_goods') . " AS g " . "WHERE o.order_id = g.order_id " . "AND o.extension_code = '" . PRE_SALE_CODE . "' " . "AND o.extension_id = '$pre_sale_id' " . "AND o.order_status = '" . OS_CONFIRMED . "'";
		$res = $db->query($sql);
		while($order = $db->fetchRow($res))
		{
			/* 邮件模板赋值 */
			...
			$smarty->assign('shop_url', $yp->url() . 'user.php?act=order_detail&order_id=' . $order['order_id']);
	...
	else
	{
		...
		{
			/* update */
			$db->autoExecute($yp->table('goods_activity'), $pre_sale, 'UPDATE', "act_id = '$pre_sale_id'");
		...
		{
			/* insert */
			$db->autoExecute($yp->table('goods_activity'), $pre_sale, 'INSERT');
...
function action_batch_drop ()
{
	...
	if(isset($_POST['checkboxes']))
	{
		...
		foreach($_POST['checkboxes'] as $key => $id)
		{
			...
			{
				/* 删除预售活动 */
				$sql = "DELETE FROM " . $GLOBALS['yp']->table('goods_activity') . " WHERE act_id = '$id' LIMIT 1";
...
function pre_sale_list ()
{
	...
	if($result === false)
	{
		...
		$sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('goods_activity') . " WHERE act_type = '" . GAT_PRE_SALE . "' $where";
		...
		/* 查询 */
		$sql = "SELECT * " . "FROM " . $GLOBALS['yp']->table('goods_activity') . " WHERE act_type = '" . GAT_PRE_SALE . "' $where " . " ORDER BY $filter[sort_by] $filter[sort_order] " . " LIMIT " . $filter['start'] . ", $filter[page_size]";
...
{
	$sql = "SELECT * FROM " . $GLOBALS['yp']->table('goods_activity') . " WHERE goods_id = '$goods_id' " . " AND act_type = '" . GAT_PRE_SALE . "'" . " AND start_time <= " . gmtime() . " AND end_time >= " . gmtime();
