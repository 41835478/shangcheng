...
$exc = new exchange($yp->table("supplier_category"), $db, 'cat_id', 'cat_name');
...
if ($_REQUEST['act'] == 'add')
{
	...
	$goods_list = get_goods_options($_SESSION['supplier_id'],0);
	$cat_list = cat_list_supplier(0, 0, true);
	$page_list = get_page_options();
	$article_list = get_articles_options($_SESSION['supplier_id'],0);
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    $cat['xcx_cat_name']     = !empty($_POST['xcx_cat_name'])     ? trim($_POST['xcx_cat_name'])     : '';//新增分类小程序缩略名 prince
    ...
    //$cat['cat_pic_xcxurl'] = !empty($_POST['cat_pic_xcxurl'])  ? htmlspecialchars($_POST['cat_pic_xcxurl']): '';
	...
	if(!empty($catpic['cat_pic'])){
		$cat['cat_pic'] = $catpic['cat_pic'];
	}
	if(!empty($catpic['cat_logo'])){
		$cat['cat_logo'] = $catpic['cat_logo'];
	}
    ...
    /* 入库的操作 */
    if ($db->autoExecute($yp->table('supplier_category'), $cat) !== false)
    {
        $cat_id = $db->insert_id();
		...
		*/
        ...
        {
            $vieworder = $db->getOne("SELECT max(vieworder) FROM ". $yp->table('supplier_nav') . " WHERE type = 'middle' AND supplier_id=".$_SESSION['supplier_id']);
            $vieworder += 2;
            //显示在自定义导航栏中
            $sql = "INSERT INTO " . $yp->table('supplier_nav') .
...
if ($_REQUEST['act'] == 'edit')
{
   admin_priv('cat_manage');
	...
	$goods_list = get_goods_options($_SESSION['supplier_id'],$selected);
	$cat_list = cat_list_supplier(0, $selected, true);
	$page_list = get_page_options($selected);
	$article_list = get_articles_options($_SESSION['supplier_id'],$selected);
    ...
    if ($cat_info['filter_attr'])
    {
        ...
        {
            $attr_cat_id = $db->getOne("SELECT cat_id FROM " . $yp->table('attribute') . " WHERE attr_id = '" . intval($v) . "'");
    ...
    //分类是否存在首页推荐
    $res = $db->getAll("SELECT recommend_type FROM " . $yp->table("supplier_cat_recommend") . " WHERE cat_id=" . $cat_id . " AND supplier_id=".$_SESSION['supplier_id']);
    if (!empty($res))
    {
        $cat_recommend = array();
        foreach($res as $data)
...
elseif($_REQUEST['act'] == 'add_category')
{
    ...
    {
        $sql = "INSERT INTO " . $yp->table('supplier_category') . "(cat_name, parent_id, is_show, supplier_id)" .
...
if ($_REQUEST['act'] == 'update')
{
    ...
    $dat = $db->getRow("SELECT cat_name, show_in_nav FROM ". $yp->table('supplier_category') . " WHERE cat_id = '$cat_id'");

    if ($db->autoExecute($yp->table('supplier_category'), $cat, 'UPDATE', "cat_id='$cat_id'"))
    {
        if($cat['cat_name'] != $dat['cat_name'])
        {
            //如果分类名称发生了改变
            $sql = "UPDATE " . $yp->table('supplier_nav') . " SET name = '" . $cat['cat_name'] . "' WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle' AND supplier_id=".$_SESSION['supplier_id'];
            $db->query($sql);
        }
		...
		*/
        ...
        if($cat['show_in_nav'] != $dat['show_in_nav'])
        {
            ...
            {
                //显示
                $nid = $db->getOne("SELECT id FROM ". $yp->table('supplier_nav') . " WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle' AND supplier_id=".$_SESSION['supplier_id']);
                if(empty($nid))
                {
                    //不存在
                    $vieworder = $db->getOne("SELECT max(vieworder) FROM ". $yp->table('supplier_nav') . " WHERE type = 'middle' AND supplier_id=".$_SESSION['supplier_id']);
                    ...
                    $sql = "INSERT INTO " . $yp->table('supplier_nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,type,supplier_id) VALUES('" . $cat['cat_name'] . "', 'c', '$cat_id','1','$vieworder','0', '" . $uri . "','middle',".$_SESSION['supplier_id'].")";
                }
                else
                {
                    $sql = "UPDATE " . $yp->table('supplier_nav') . " SET ifshow = 1 WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle' AND supplier_id=".$_SESSION['supplier_id'];
                }
                $db->query($sql);
            }
            else
            {
                //去除
                $db->query("UPDATE " . $yp->table('supplier_nav') . " SET ifshow = 0 WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle' AND supplier_id=".$_SESSION['supplier_id']);
            }
        }
...
if ($_REQUEST['act'] == 'move_cat')
{
    ...
    /* 更新商品分类 */
    //$sql = "UPDATE " .$yp->table('goods'). " SET cat_id = '$target_cat_id' ".
          // "WHERE cat_id = '$cat_id'";
    $sql = "UPDATE " .$yp->table('supplier_goods_cat'). " SET cat_id = '$target_cat_id' ".
...
if ($_REQUEST['act'] == 'toggle_show_in_nav')
{
    ...
    if (cat_update($id, array('show_in_nav' => $val)) != false)
    {
        if($val == 1)
        {
            ...
            $vieworder = $db->getOne("SELECT max(vieworder) FROM ". $yp->table('supplier_nav') . " WHERE type = 'middle' AND supplier_id=".$_SESSION['supplier_id']);
            $vieworder += 2;
            $catname = $db->getOne("SELECT cat_name FROM ". $yp->table('supplier_category') . " WHERE cat_id = '$id'");
            //显示在自定义导航栏中
            $_CFG['rewrite'] = 0;
            $uri = build_uri('supplier', array('go'=>'category','suppid'=>$_SESSION['supplier_id'],'cid'=> $id), $catname);

            $nid = $db->getOne("SELECT id FROM ". $yp->table('supplier_nav') . " WHERE ctype = 'c' AND cid = '" . $id . "' AND type = 'middle' AND supplier_id=".$_SESSION['supplier_id']);
            if(empty($nid))
            {
                //不存在
                $sql = "INSERT INTO " . $yp->table('supplier_nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,type,supplier_id) VALUES('" . $catname . "', 'c', '$id','1','$vieworder','0', '" . $uri . "','middle',".$_SESSION['supplier_id'].")";
            }
            else
            {
                $sql = "UPDATE " . $yp->table('supplier_nav') . " SET ifshow = 1 WHERE ctype = 'c' AND cid = '" . $id . "' AND type = 'middle' AND supplier_id=".$_SESSION['supplier_id'];
            }
            $db->query($sql);
        }
        else
        {
            //去除
            $db->query("UPDATE " . $yp->table('supplier_nav') . "SET ifshow = 0 WHERE ctype = 'c' AND cid = '" . $id . "' AND type = 'middle' AND supplier_id=".$_SESSION['supplier_id']);
        }
        clear_cache_files();
        make_json_result($val);
    }
...

{
     ...
     $sqls = "UPDATE " . $GLOBALS['yp']->table('supplier_category') . "
...
if ($_REQUEST['act'] == 'remove')
{
    ...
    $cat_name = $db->getOne('SELECT cat_name FROM ' .$yp->table('supplier_category'). " WHERE cat_id='$cat_id'");

    /* 当前分类下是否有子分类 */
    $cat_count = $db->getOne('SELECT COUNT(*) FROM ' .$yp->table('supplier_category'). " WHERE parent_id='$cat_id' AND supplier_id=".$_SESSION['supplier_id']);

    /* 当前分类下是否存在商品 */
    //$goods_count = $db->getOne('SELECT COUNT(*) FROM ' .$yp->table('goods'). " WHERE cat_id='$cat_id' AND supplier_id = ".$_SESSION['supplier_id']);
    $goods_count = $db->getOne('SELECT COUNT(*) FROM ' .$yp->table('supplier_goods_cat'). " WHERE cat_id='$cat_id' AND supplier_id = ".$_SESSION['supplier_id']);

    /* 如果不存在下级子分类和商品，则删除之 */
    if ($cat_count == 0 && $goods_count == 0)
    {
        /* 删除分类 */
        $sql = 'DELETE FROM ' .$yp->table('supplier_category'). " WHERE cat_id = '$cat_id'";
        if ($db->query($sql))
        {
            $db->query("DELETE FROM " . $yp->table('nav') . "WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'");//20170909 prince
            clear_cache_files();
...
{
    $sql = "SELECT * FROM " .$GLOBALS['yp']->table('supplier_category'). " WHERE cat_id='$cat_id' LIMIT 1";
...
function cat_update($cat_id, $args)
{
    ...
    return $GLOBALS['db']->autoExecute($GLOBALS['yp']->table('supplier_category'), $args, 'update', "cat_id='$cat_id'");
}
...
function get_attr_list()
{
    $sql = "SELECT a.attr_id, a.cat_id, a.attr_name ".
           ...
           $GLOBALS['yp']->table('goods_type') . " AS c ".
...
function update_category_pic($catid,$pic){
	$sql = "UPDATE " . $yp->table('supplier_shop_config') . " SET cat_pic = '$pic' WHERE cat_id = '$catid' AND supplier_id=".$_SESSION['supplier_id'];
...
function insert_cat_recommend($recommend_type, $cat_id)
{
    ...
    {
        //取得之前的分类
        $recommend_res = $GLOBALS['db']->getAll("SELECT recommend_type FROM " . $GLOBALS['yp']->table("supplier_cat_recommend") . " WHERE cat_id=" . $cat_id . " AND supplier_id=".$_SESSION['supplier_id']);
        if (empty($recommend_res))
        {
            foreach($recommend_type as $data)
            {
                $data = intval($data);
                $GLOBALS['db']->query("INSERT INTO " . $GLOBALS['yp']->table("supplier_cat_recommend") . "(cat_id, recommend_type, supplier_id) VALUES ('$cat_id', '$data', ".$_SESSION['supplier_id'].")");
            }
        }
        else
        {
            ...
            {
                $GLOBALS['db']->query("DELETE FROM " . $GLOBALS['yp']->table("supplier_cat_recommend") . " WHERE supplier_id=".$_SESSION['supplier_id']." AND cat_id=$cat_id AND recommend_type " . db_create_in($delete_array));
            }
    ...
    {
        $GLOBALS['db']->query("DELETE FROM ". $GLOBALS['yp']->table("supplier_cat_recommend") . " WHERE cat_id=" . $cat_id. " AND supplier_id=".$_SESSION['supplier_id']);
    }
}
        ...
        if ((isset($file['error']) && $file['error'] == 0) || (!isset($file['error']) && $file['tmp_name'] != 'none'))
        {
            ...
            else
            {
                ...
                if($code == 'cat_pic')
                {
                	...
                	$file_name .= $_SESSION['supplier_id'].'cpic'.time() . '.' . $ext;
                    if($catid>0){
                ...
                }elseif($code == 'cat_logo')
                {
                	$ext = array_pop(explode('.', $file['name']));
                	$file_name .= $_SESSION['supplier_id'].'clogo'.time() . '.' . $ext;
                    if($catid>0){
                    	$catpic = get_cat_info($catid);
	                    if (file_exists($catpic['cat_pic']))
	                    {
                ...
                {
                   //return $file_name;//20170818
				   if($code == 'cat_pic'){
				   		$name['cat_pic']=$file_name;
				   }elseif($code == 'cat_logo'){
				   		$name['cat_logo']=$file_name;
				   }
                }
                else
                {
...
function get_goods_options($supplier_id,$selected=0){
	$sql="SELECT * FROM ".$GLOBALS['yp']->table('goods')."WHERE supplier_id='$supplier_id' AND is_on_sale=1 AND is_delete=0 AND supplier_status=1 ORDER BY goods_id desc ";
...
function get_articles_options($supplier_id,$selected=0){
	$sql="SELECT * FROM ".$GLOBALS['yp']->table('supplier_article')."WHERE supplier_id='$supplier_id' AND is_open=1 ORDER BY article_id desc ";
	$result = $GLOBALS['db']->getAll($sql);
	$options = '';
	foreach($result as $val)
	{
