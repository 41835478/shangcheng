...
require_once(ROOT_PATH . 'mobile/includes/prince/lib_supplier_extpintuan.php');
...
{  
    ...
    $list = extpintuan_list_adm($supplier_id);
...
elseif ($_REQUEST['act'] =='insert_update')
{
    ...
    if (isset($_POST['finish']))
    {
        ...
        /* 结束拼团活动，修改结束时间为当前时间 */
        $sql = "UPDATE " . $yp->table('goods_activity') .
                " SET end_time = '" . gmtime() . "' " .
    ...
    else
    {
        ...
        $had_finished = $db->getOne("SELECT count(*) FROM " . $yp->table('extpintuan') . " WHERE status = 4 and act_id = '$extpintuan_id' ");
        if ($had_finished)
        {
            sys_msg('活动已开奖，禁止编辑');
        }
        ...
        {
            /* update */
            $db->autoExecute($yp->table('goods_activity'), $extpintuan, 'UPDATE', "act_id = '$extpintuan_id'");
        ...
        {
            /* insert */
            $db->autoExecute($yp->table('goods_activity'), $extpintuan, 'INSERT');
...
elseif ($_REQUEST['act'] == 'detail_view')
{
		...
		$extpintuan_list = get_extpintuan_detail($supplier_id);
...
elseif ($_REQUEST['act'] == 'batch_drop')
{
    if (isset($_POST['checkboxes']))
    {
        ...
        foreach ($_POST['checkboxes'] AS $key => $id)
        {
            ...
            {
                /* 删除拼团活动 */
                $sql = "DELETE FROM " . $GLOBALS['yp']->table('goods_activity') .
...
elseif ($_REQUEST['act'] == 'edit_deposit')
{
    ...
    $sql = "SELECT ext_info FROM " . $yp->table('goods_activity') .
            " WHERE supplier_id = ".$supplier_id." AND act_id = '$id' AND act_type = '" . GAT_EXTPINTUAN . "'";
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 删除拼团活动 */
    $sql = "DELETE FROM " . $yp->table('goods_activity') . " WHERE act_id = '$id' LIMIT 1";
    $db->query($sql);
    ...
    clear_cache_files();
     ...
     $links[] = array('text' => $_LANG['back_list'], 'href'=>'extpintuan.php?act=list');
     sys_msg($_LANG['batch_drop_success'], 0, $links);
}
 ...
 * @return   array
 */
function extpintuan_list_adm($supplier_id)
{
    $result = get_filter();
    if ($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('goods_activity') .
                " WHERE supplier_id = ".$supplier_id." AND act_type = '" . GAT_EXTPINTUAN . "' $where";
