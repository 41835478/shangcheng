...
/**
 * QQ120029121 商品类型管理程序
 * ============================================================================
 * 演示地址: http://demo.coolhong.com  开发QQ:120029121    309485552
 * ============================================================================
 * $Author: PRINCE $
 * $Id: goods_type.php 17217 2017-04-01 06:29:08Z PRINCE $
*/
...
$exc = new exchange($yp->table("goods_type"), $db, 'cat_id', 'cat_name');
...
if ($_REQUEST['act'] == 'manage')
{
    ...
    $query = $db->query("SELECT a.cat_id FROM " . $yp->table('attribute') . " AS a RIGHT JOIN " . $yp->table('goods_attr') . " AS g ON g.attr_id = a.attr_id GROUP BY a.cat_id");
...
elseif ($_REQUEST['act'] == 'edit_type_name')
{
    ...
    $goods_type = get_goodstype_info($type_id);

    if ($goods_type['supplier_id'] != $_SESSION['supplier_id'] ) {
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $goods_type['supplier_id']    = $_SESSION['supplier_id'];

    if ($db->autoExecute($yp->table('goods_type'), $goods_type) !== false)
    {
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if ($db->autoExecute($yp->table('goods_type'), $goods_type, 'UPDATE', "cat_id='$cat_id'") !== false)
    {
        /* 对比原来的分组 */
...
elseif ($_REQUEST['act'] == 'remove')
{
    check_authz_json('goods_type');
     ...
     $id = intval($_GET['id']);
    $goods_type = get_goodstype_info($id);
    ...
    if ($exc->drop($id))
    {
        ...
        /* 清除该类型下的所有属性 */
        $sql = "SELECT attr_id FROM " .$yp->table('attribute'). " WHERE cat_id = '$id'";
        $arr = $db->getCol($sql);

        $GLOBALS['db']->query("DELETE FROM " .$yp->table('attribute'). " WHERE attr_id " . db_create_in($arr));
        $GLOBALS['db']->query("DELETE FROM " .$yp->table('goods_attr'). " WHERE attr_id " . db_create_in($arr));
...
function get_goodstype()
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数以及页数 */
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['yp']->table('goods_type'). " WHERE  supplier_id IN (0,'$supplier_id')";
        ...
        $sql = "SELECT t.*, COUNT(a.cat_id) AS attr_count ".
               "FROM ". $GLOBALS['yp']->table('goods_type'). " AS t ".
               "LEFT JOIN ". $GLOBALS['yp']->table('attribute'). " AS a ON a.cat_id=t.cat_id ".
               "WHERE t.supplier_id IN (0,'$supplier_id') GROUP BY t.cat_id " .
...
{
    $sql = "SELECT * FROM " .$GLOBALS['yp']->table('goods_type'). " WHERE cat_id='$cat_id'";
...
{
    $sql = "UPDATE " . $GLOBALS['yp']->table('attribute') .
