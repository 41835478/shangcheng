...
$exc   = new exchange($yp->table("pickup_point"), $db, 'id', 'shop_name');
/*------------------------------------------------------ */
//-- 自提点列表
/*------------------------------------------------------ */
if ($_REQUEST['act'] == 'list')
{
	...
	$sql = 'select * from ' . $yp->table('region') . ' where parent_id=' . $GLOBALS['_CFG']['shop_country'];
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$yp->table('pickup_point')."(shop_name, address, contact, phone, province_id, ".
...
{
    /* 取自提点数据 */
    $sql = "SELECT * FROM " .$yp->table('pickup_point'). " WHERE id='$_REQUEST[id]'";
    $pickup_point = $db->GetRow($sql);
	
	$sql = 'select * from ' . $yp->table('region') . ' where parent_id=' . $GLOBALS['_CFG']['shop_country'];
	$province_list = $db->getAll($sql);
	$sql = 'select * from ' . $yp->table('region') . ' where parent_id=' . $pickup_point['province_id'];
	$city_list = $db->getAll($sql);
	$sql = 'select * from ' . $yp->table('region') . ' where parent_id=' . $pickup_point['city_id'];
...
elseif ($_REQUEST['act'] == 'download')
{
    ...
    if (isset($_LANG['upload_pickup_point']))
    {
        ...
        if ($_GET['charset'] == 'zh_cn' || $_GET['charset'] == 'zh_tw')
        {
            ...
            echo yp_iconv(YP_CHARSET, $to_charset, join(',', $_LANG['upload_pickup_point']));
        }
        else
        {
...
elseif ($_REQUEST['act'] == 'batch_insert')
{
	if (isset($_POST['checked']))
    {
        ...
        /* 获取商品good id */
        $max_id = $db->getOne("SELECT MAX(id) + 1 FROM ".$yp->table('pickup_point'));
        ...
        foreach ($_POST['checked'] AS $key => $value)
        {
            ...
            $db->autoExecute($yp->table('pickup_point'), $field_arr, 'INSERT');
...
function get_pickup_point_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 自提点总数 */
        $sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['yp']->table('pickup_point') .
        ...
        /* 获取自提点数据 */
        $sql = 'SELECT * '.
               'FROM ' .$GLOBALS['yp']->table('pickup_point') .
...
{
	$sql = 'select region_name from ' . $GLOBALS['yp']->table('region') . ' where region_id=' . $city_id;
