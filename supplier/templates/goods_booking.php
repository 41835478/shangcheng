...
/**
 * QQ120029121 缺货处理管理程序
 * ============================================================================
 * 演示地址: http://demo.coolhong.com  开发QQ:120029121    309485552
 * ============================================================================
 * $Author: PRINCE $
 * $Id: goods_booking.php 17217 2017-04-01 06:29:08Z PRINCE $
*/
...
if ($_REQUEST['act'] == 'remove')
{
    ...
    $db->query("DELETE FROM " .$yp->table('booking_goods'). " WHERE rec_id='$id'");
...
if ($_REQUEST['act'] =='update')
{
    ...
    $sql = "UPDATE  ".$yp->table('booking_goods').
    ...
    if (!empty($_POST['send_email_notice']) or isset($_POST['remail']))
    {
        ...
        $sql = 'SELECT bg.email, bg.link_man, bg.goods_id, g.goods_name ' .
               'FROM ' .$yp->table('booking_goods'). ' AS bg, ' .$yp->table('goods'). ' AS g ' .
        ...
        $goods_link = $yp->url() . 'goods.php?id=' . $booking_info['goods_id'];
    ...
    yp_header("Location: ?act=detail&id=".$_REQUEST['rec_id']."&send_ok=$send_ok\n");
    exit;
}
...
function get_bookinglist()
{
    ...
    $sql = 'SELECT bg.rec_id, bg.link_man, g.goods_id, g.goods_name, bg.goods_number, bg.booking_time, bg.is_dispose '.
            'FROM ' .$GLOBALS['yp']->table('booking_goods'). ' AS bg, ' .$GLOBALS['yp']->table('goods'). ' AS g '.
            "WHERE bg.goods_id = g.goods_id $where " .
...
function get_booking_info($id)
{
    ...
    $sql ="SELECT bg.rec_id, bg.user_id, IFNULL(u.user_name, '$_LANG[guest_user]') AS user_name, ".
            ...
            "FROM " . $yp->table('booking_goods')." AS bg ".
            "LEFT JOIN " . $yp->table('goods') . " AS g ON g.goods_id=bg.goods_id ".
            "LEFT JOIN " . $yp->table('users') . " AS u ON u.user_id=bg.user_id ".
            "WHERE bg.rec_id ='$id'";
