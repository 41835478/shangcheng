...
require_once(ROOT_PATH . 'mobile/includes/prince/lib_supplier_cut.php');
$exc = new exchange($yp->table("goods_activity"), $db, 'act_id', 'act_name');
$supplier_id = $_SESSION['supplier_id'];//20161114   寒冰    qq  3094  85552    商家支持砍价
/*------------------------------------------------------ */
//-- 添加活动
/*------------------------------------------------------ */
if ($_REQUEST['act'] == 'add')
{
    ...
    $cut     = array('start_price'=>'1.00','free_shipping'=>'1','end_price'=>'5.00','max_price'=>'0','orders_limit'=>'1', 'need_follow'=>'1','start_time' => $start_time,'end_time' => $end_time,'option'=>'<option value="0">'.$_LANG['make_option'].'</option>');
...
elseif ($_REQUEST['act'] =='insert')
{
    ...
    /* 检查商品是否存在 */
    $sql = "SELECT goods_name FROM ".$yp->table('goods')." WHERE goods_id = '$_POST[goods_id]'";
    ...
    $record = array('act_name'=>$_POST['cut_name'], 'act_desc'=>$_POST['act_desc'],
                    ...
                    'is_finished'=>0, 'free_shipping'=>$_POST['free_shipping'],'ext_info'=>serialize($info));

    $db->AutoExecute($yp->table('goods_activity'),$record,'INSERT');
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $sql = "select count(*) FROM " . $GLOBALS['yp']->table('cut') . " WHERE act_id = '$id' ";
...
elseif ($_REQUEST['act'] =='update')
{
    ...
    {
        $_POST['goods_name'] = $db->getOne("SELECT goods_name FROM " . $yp->table('goods') . "WHERE goods_id= '$_POST[goods_id]'");
    }
    if (empty($_POST['price']))
    {
        $_POST['price'] = 0;
    ...
    $record = array('act_name' => $_POST['cut_name'], 'goods_id' => $_POST['goods_id'],
                    ...
                    'product_id'=>$_POST['product_id'],'free_shipping'=>$_POST['free_shipping'],
                    'ext_info'=>serialize($info));
    $db->autoExecute($yp->table('goods_activity'), $record, 'UPDATE', "act_id = '" . $_POST['id'] . "' AND act_type = " . GAT_CUT );
