...
elseif ($_REQUEST['act'] == 'check')
{
   ...
   $supplier_id = $_SESSION['supplier_id'];
    ...
    $sql = "SELECT * FROM " .$GLOBALS['yp']->table('user_account') ." as a inner join " . $GLOBALS['yp']->table('users') . " as u on a.user_id = u.user_id where u.supplier_id='$supplier_id' AND a.id = '$id'";

      
        $attr = $db->getRow($sql);

        if (empty($attr)) {
            $links[] = array('text' => $_LANG['09_user_account'], 'href' => 'user_account.php?act=list');
...
elseif ($_REQUEST['act'] == 'action')
{
    ...
    $user_id = $db->getOne("SELECT user_id FROM " .$yp->table('users'). " WHERE supplier_id='$supplier_id'  user_id = '$id'");
...
function account_list()
{
    ...
    if ($result === false)
    {
        ...
        $supplier_id = $_SESSION['supplier_id'];
        $where = " WHERE 1 AND supplier_id = '$supplier_id'";
        //$where = " WHERE 1 ";
        if ($filter['user_id'] > 0)
        {
