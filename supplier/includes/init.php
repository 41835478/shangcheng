...
define('OLD_ADMIN_PATH','supplier');//优化采集 20170907 add by prince 此项禁止修改，否则影响采集
    ...
    $_REQUEST['act'] != 'forget_pwd' && $_REQUEST['act'] != 'reset_pwd' && $_REQUEST['act'] != 'check_order')
{
    ...
    if (!empty($_COOKIE['YPCP']['supplier_id']) && !empty($_COOKIE['YPCP']['supplier_user_id']) && !empty($_COOKIE['YPCP']['supplier_pass']))
    {
        ...
        //        ' FROM ' .$yp->table('users') . " AS u ,  ". $yp->table('supplier') ." AS s ".
        //        " WHERE u.user_id =s.user_id and s.supplier_id= '" . intval($_COOKIE['YPCP']['supplier_id']) . "'";
        //$row = $db->GetRow($sql);
        ...
        else
        {
            ...
            else
            {
                ...
                {   //$test=md5($row['password'] . $_CFG['hash_code']).'-'.$_COOKIE['YPCP']['supplier_pass'].'-'.$row['password'].'-'. $_CFG['hash_code'] ;
                    yp_header("Location: privilege.php?act=login&num=$test\n");
                }
...
if ($_REQUEST['act'] != 'login' && $_REQUEST['act'] != 'signin' &&
    $_REQUEST['act'] != 'forget_pwd' && $_REQUEST['act'] != 'reset_pwd' && $_REQUEST['act'] != 'check_order' && $_REQUEST['from'] != 'weixin')
{
