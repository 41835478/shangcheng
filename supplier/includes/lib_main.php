...
/**
 * QQ120029121 管理中心公用函数库
 * ============================================================================
 * 演示地址: http://demo.coolhong.com  开发QQ:120029121    309485552
 * ============================================================================
 * $Author: PRINCE $
 * $Id: lib_main.php 17217 2017-04-01 06:29:08Z PRINCE $
*/
...
function admin_log($sn = '', $action, $content)
{
    ...
    $sql = 'INSERT INTO ' . $GLOBALS['yp']->table('admin_log') . ' (log_time, user_id, log_info, ip_address) ' .
...
function insert_config($parent, $code, $value)
{
    ...
    $sql = 'SELECT id FROM ' . $yp->table('shop_config') . " WHERE code = '$parent' AND type = 1";
    $parent_id = $db->getOne($sql);

    $sql = 'INSERT INTO ' . $yp->table('shop_config') . ' (parent_id, code, value) ' .
...
{
    $bonus = array();
    $sql = 'SELECT type_id, type_name, type_money FROM ' . $GLOBALS['yp']->table('bonus_type') .
...
{
    $rank_list = array();
    $sql = 'SELECT rank_id, rank_name, min_points FROM ' . $GLOBALS['yp']->table('user_rank');
    if ($is_special)
    {
...
{
    $user_list = array();
    $sql = 'SELECT user_id, user_name FROM ' . $GLOBALS['yp']->table('users') . $where.
 ...
 */
function get_position_list($supplier_id=0)
{
    $position_list = array();
    $sql = 'SELECT position_id, position_name, ad_width, ad_height '.
           'FROM ' . $GLOBALS['yp']->table('ypmart_ad_position')." where supplier_id='$supplier_id' ";
    $res = $GLOBALS['db']->query($sql);
...
function get_goods_list($filter)
{
   ...
   // $sql = 'SELECT goods_id, goods_name, shop_price '.
    ...
    $sql = 'select g.goods_id,g.goods_name,g.shop_price '.
           'from ' . $GLOBALS['yp']->table('supplier_goods_cat') . 'as sgc '.
           'left join ' . $GLOBALS['yp']->table('goods') . 'as g on sgc.goods_id = g.goods_id' . $where.
           'LIMIT 50';
...
function get_article_list($filter)
{
    ...
    $sql = 'SELECT a.article_id, a.title '.
           'FROM ' .$GLOBALS['yp']->table('article'). ' AS a, ' .$GLOBALS['yp']->table('article_cat'). ' AS c ' . $where;
    $res = $GLOBALS['db']->query($sql);
...
function area_list($region_id)
{
    ...
    $sql = 'SELECT * FROM ' . $GLOBALS['yp']->table('region').
...
function goods_type_list($selected)
{
    ...
    $sql = "SELECT cat_id, cat_name FROM " . $GLOBALS['yp']->table('goods_type') . " WHERE enabled = 1 AND supplier_id IN (0,'$supplier_id')";
    $res = $GLOBALS['db']->query($sql);
...
function get_pay_ids()
{
    ...
    $sql = 'SELECT pay_id, is_cod FROM ' .$GLOBALS['yp']->table('payment'). ' WHERE enabled = 1';
    $res = $GLOBALS['db']->query($sql);
...
{
    $sql = 'TRUNCATE TABLE ' .$GLOBALS['yp']->table($table_name);
...
{
    $sql = "SELECT attr_group FROM " . $GLOBALS['yp']->table('goods_type') . " WHERE cat_id='$cat_id'";
...
function set_filter($filter, $sql, $param_str = '')
{
    ...
    setcookie('YPCP[lastfilterfile]', sprintf('%X', crc32($filterfile)), time() + 600);
    setcookie('YPCP[lastfilter]',     urlencode(serialize($filter)), time() + 600);
    setcookie('YPCP[lastfiltersql]',  base64_encode($sql), time() + 600);
}
...
function get_filter($param_str = '')
{
    ...
    if (isset($_GET['uselastfilter']) && isset($_COOKIE['YPCP']['lastfilterfile'])
        && $_COOKIE['YPCP']['lastfilterfile'] == sprintf('%X', crc32($filterfile)))
    {
        return array(
            'filter' => unserialize(urldecode($_COOKIE['YPCP']['lastfilter'])),
            'sql'    => base64_decode($_COOKIE['YPCP']['lastfiltersql'])
        );
    }
    else
    {
...
{
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['yp']->table('supplier_category').
...
{
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['yp']->table('category').
...
{
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['yp']->table('brand').
...
function admin_info()
{
    $sql = "SELECT * FROM ". $GLOBALS['yp']->table('admin_user')."
...
function cat_list_2($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    ...
    if ($res === NULL)
    {
            ...
            $sql = "SELECT c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order, COUNT(s.cat_id) AS has_children ".
                'FROM ' . $GLOBALS['yp']->table('supplier_category') . " AS c ".
                "LEFT JOIN " . $GLOBALS['yp']->table('supplier_category') . " AS s ON s.parent_id=c.cat_id  where c.supplier_id = ".$_SESSION['supplier_id'].
                " GROUP BY c.cat_id ".
            ...
            $sql = "SELECT gc.cat_id, COUNT(*) AS goods_num " .
                    " FROM " . $GLOBALS['yp']->table('supplier_goods_cat') . " AS gc , " . $GLOBALS['yp']->table('goods') . " AS g " .
                    //" WHERE gc.supplier_id = ".$_SESSION['supplier_id']." AND g.goods_id = gc.goods_id AND g.is_delete = 0 AND g.is_on_sale = 1 " .
                    " WHERE gc.supplier_id = ".$_SESSION['supplier_id']." AND g.goods_id = gc.goods_id AND g.is_delete = 0  " .
...
function create_shop_settiongs()
{
	...
	$sql = "SELECT count(id) FROM " . $yp->table('supplier_shop_config') ." WHERE supplier_id=".$_SESSION['supplier_id'];
	$num = $db->getOne($sql);

	if($num>0){
		return;
	}else{
		$insql = "INSERT INTO ". $yp->table('supplier_shop_config') ." (`id`, `parent_id`, `code`, `type`, `store_range`, `store_dir`, `value`, `sort_order`, `supplier_id`) VALUES
				(1, 0, 'shop_info', 'group', '', '', '', 1, ".$_SESSION['supplier_id']."),
				(2, 0, 'hidden', 'hidden', '', '', '', 1, ".$_SESSION['supplier_id']."),
				(8, 0, 'sms', 'group', '', '', '', 1, ".$_SESSION['supplier_id']."),
				(9, 0, 'printer', 'group', '', '', '', 1, ".$_SESSION['supplier_id']."),
				(10, 0, 'use_supplier_url', 'group', '', '', '', 5, ".$_SESSION['supplier_id']."),
                (11, 0, 'distrib', 'group', '', '', '', 4, ".$_SESSION['supplier_id']."),
				...
				(108, 1, 'shop_district', 'manual', '', '', '0', 1, ".$_SESSION['supplier_id']."),
				(109, 1, 'shop_address', 'text', '', '', '', 1, ".$_SESSION['supplier_id']."),
				(110, 1, 'qq', 'text', '', '', '', 1, ".$_SESSION['supplier_id']."),
				(111, 1, 'wechat', 'text', '', '', '', 1, ".$_SESSION['supplier_id']."),
				(112, 1, 'wechat_qrcode', 'file', '', '../images/qrcode/', '', 1, ".$_SESSION['supplier_id']."),
				(113, 1, 'ww', 'text', '', '', '', 1, ".$_SESSION['supplier_id']."),
				(114, 1, 'skype', 'hidden', '', '', '', 1, ".$_SESSION['supplier_id']."),
				(115, 1, 'ym', 'hidden', '', '', '', 1, ".$_SESSION['supplier_id']."),
				(116, 1, 'msn', 'hidden', '', '', '', 1, ".$_SESSION['supplier_id']."),
				(117, 1, 'service_email', 'text', '', '', '', 1, ".$_SESSION['supplier_id']."),
				(118, 1, 'service_phone', 'text', '', '', '', 1, ".$_SESSION['supplier_id']."),
				(119, 1, 'shop_closed', 'select', '0,1', '', '0', 1, ".$_SESSION['supplier_id']."),
				(120, 1, 'close_comment', 'textarea', '', '', '该店铺正在装修', 1, ".$_SESSION['supplier_id']."),
				(121, 1, 'shop_logo', 'file', '', '../themes/".'{$template}'."/images/', '', 1, ".$_SESSION['supplier_id']."),
				(122, 1, 'licensed', 'hidden', '0,1', '', '1', 1, ".$_SESSION['supplier_id']."),
				(123, 1, 'user_notice', 'hidden', '', '', '用户中心公告！', 1, ".$_SESSION['supplier_id']."),
				(124, 1, 'shop_notice', 'textarea', '', '', '欢迎使用多商户系统,我们的宗旨：诚信经营、服务客户！\r\n<MARQUEE onmouseover=this.stop() onmouseout=this.start() \r\nscrollAmount=3><U><FONT color=red>\r\n<P>技术客服QQ：120-029-121、309-485-552</P></FONT></U></MARQUEE>', 1, ".$_SESSION['supplier_id']."),
				(125, 1, 'shop_reg_closed', 'hidden', '1,0', '', '0', 1, ".$_SESSION['supplier_id']."),
				(126, 1, 'shop_index_num', 'textarea', '', '', '8\r\n6\r\n4', 1, ".$_SESSION['supplier_id']."),
				(127, 1, 'shop_search_price', 'textarea', '', '', '0-1000元\r\n1000-2000元\r\n2000-4000元', 1, ".$_SESSION['supplier_id']."),
				(128, 1, 'jingdu', 'jwd', '', '', '0', 1, ".$_SESSION['supplier_id']."),
				(129, 1, 'weidu', 'jwd', '', '', '0', 1, ".$_SESSION['supplier_id']."),
				...
				(206, 2, 'mobile_shop_header_color', 'hidden', '', '', '#E4368F', 1, ".$_SESSION['supplier_id']."),
				(207, 2, 'mobile_header_picture', 'hidden', '', '', '请上传logo和banner', 1, ".$_SESSION['supplier_id']."),
				(901, 9, 'printer_open', 'select', '1,0', '', '0', 1, ".$_SESSION['supplier_id']."),
				(902, 9, 'printer_user', 'text', '', '', '', 1, ".$_SESSION['supplier_id']."),
				(903, 9, 'printer_key', 'text', '', '', '', 1, ".$_SESSION['supplier_id']."),
				(904, 9, 'printer_sn', 'text', '', '', '', 1, ".$_SESSION['supplier_id']."),
				(905, 9, 'custom_lang', 'text', '', '', '', 1, ".$_SESSION['supplier_id']."),
				(1001, 10, 'use_supplier_url', 'text', '', '', '', 5, ".$_SESSION['supplier_id']."),
                ...
                (1100, 11, 'is_distrib', 'select', '1,0', '', '0', 1, ".$_SESSION['supplier_id']."),
                (1101, 11, 'distrib_type', 'select', '1,0', '', '1', 2, ".$_SESSION['supplier_id']."),
                (1102, 11, 'distrib_percent', 'text', '', '', '10', 3, ".$_SESSION['supplier_id']."),
                (1103, 11, 'fenxiao_deposit_least_money', 'text', '', '', '10', 4, ".$_SESSION['supplier_id']."),
                (1104, 11, 'reserve_money', 'text', '', '', '10', 5, ".$_SESSION['supplier_id']."),
                (1105, 11, 'search_goods_count', 'text', '', '', '50', 6, ".$_SESSION['supplier_id']."),
                (1106, 11, 'distrib_style', 'select', '1,0', '', '1', 7, ".$_SESSION['supplier_id']."),
                (1200, 11, 'affiliate', 'hidden', '', '', '', 1, ".$_SESSION['supplier_id'].")";
...
//创建广告及广告位 prince
function create_ad_and_position($supplier_id=0){
		global $db, $yp, $_LANG;
		$sysad=array(200,201,202,203,204,205);
		foreach($sysad as $key=>$value){
			$sql="insert into ".$yp->table('ypmart_ad_position')."(supplier_id,parent_position_id,position_name, ad_width, ad_height, position_desc, position_style) SELECT $supplier_id,parent_position_id,position_name, ad_width, ad_height, position_desc, position_style FROM ".$yp->table('ypmart_ad_position')." where position_id='$value' ";
			$db->query($sql);
			$position_id=$db->insert_id();
			if($value==200){
				for($i=1; $i<=3; $i++){
					$ad_name='小程序轮播广告'.$i;
					$ad_code='https://newdemo.coolhong.com/data/afficheimg/'.$value.'-'.$i.'.jpg';
					$sql="insert into ".$yp->table('ypmart_ad').			
					"(supplier_id,position_id,parent_position_id,media_type,ad_name,ad_link,ad_code,start_time,end_time,enabled)".
					"values($supplier_id,$position_id,$value,0,'$ad_name','pages/goods/grouplist/list','$ad_code',1504950636,1798732800,1)";
					$db->query($sql);
				}
			}else{
				$ad_name='小程序首页静态图片广告'.$i;
				$ad_code='https://newdemo.coolhong.com/data/afficheimg/'.$value.'.jpg';
				$sql="insert into ".$yp->table('ypmart_ad').			
				"(supplier_id,position_id,parent_position_id,media_type,ad_name,ad_link,ad_code,start_time,end_time,enabled)".
				"values($supplier_id,$position_id,$value,0,'$ad_name','pages/goods/grouplist/list','$ad_code',1504950636,1798732800,1)";
				$db->query($sql);
...
//创建菜单 prince
function create_xcx_menu($supplier_id=0){
		global $db, $yp, $_LANG;
		$sql="insert into ".$yp->table('weixin_xcx_menu')."(`supplier_id`,`menu_name`,`menu_img`,`menu_url`,`sort`,`is_tabbar`) SELECT $supplier_id,menu_name,menu_img,menu_url,sort,is_tabbar FROM ".$yp->table('weixin_xcx_menu')." where supplier_id='0' limit 8 ";
		$db->query($sql);
}
