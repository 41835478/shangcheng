...
function delete_goods($goods_id)
{
    ...
    $res = $GLOBALS['db']->query($sql);
	  ...
	  */
    $sql = "SELECT img_url, thumb_url, img_original " .
            "FROM " . $GLOBALS['yp']->table('bar_code') .
            " WHERE goods_id " . db_create_in($goods_id);
     $GLOBALS['db']->query($sql);
	/*删除条形码结束*/
    while ($row = $GLOBALS['db']->fetchRow($res))
...
function build_attr_html($cat_id, $goods_id = 0 , $bar_code = 0)
{
    ...
    foreach ($attr AS $key => $val)
    {
        ...
        $html .= ($val['attr_type'] == 1 || $val['attr_type'] == 2) ?
            ...
            ' <input type="hidden" name="attr_price_list[]" value="0" />';//20170818 prince 
...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
	...
	$filter['suppliers_id'] = !empty($_REQUEST['suppliers_id']) ? (empty($_REQUEST['suppliers_id']) ? $_SESSION['suppliers_id'] : trim($_REQUEST['suppliers_id'])) : '';
		...
		/* 虚拟团购代码添加 by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1 start */
		if($filter['city']){
		...
		/* 虚拟团购代码添加 by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1 end */
        ...
        /* 记录总数 */
            ...
            if(!$real_goods && $filter['extension_code'] == 'virtual_good'){
                $sql = "SELECT COUNT(distinct(g.goods_id)) FROM " .$GLOBALS['yp']->table('goods'). " AS g,"
                        .$GLOBALS['yp']->table('supplier_goods_cat')." as sgc, ".$GLOBALS['yp']->table('virtual_district')." as dis, ".
						...
						//file_put_contents('./31113.txt',$sql);//prince 
            }else{
                $sql = "SELECT COUNT(distinct(g.goods_id)) FROM " .$GLOBALS['yp']->table('goods'). " AS g,".$GLOBALS['yp']->table('supplier_goods_cat')." as sgc WHERE sgc.goods_id=g.goods_id AND is_delete='$is_delete' AND sgc.supplier_id='". $_SESSION['supplier_id'] ."' $where";
				//file_put_contents('./322223.txt',$sql);//prince
            }
	 /* 代码增加_虚拟团购_END  demo.coolhong.com */	
...
/* 代码增加_虚拟团购_START  demo.coolhong.com */
         if(!$real_goods && $filter['extension_code'] == 'virtual_good'){
             $sql = "SELECT distinct(g.goods_id), goods_name, goods_type, exclusive, goods_sn, shop_price,cost_price, is_on_sale, is_best, is_new, is_hot, sort_order,supplier_status, goods_number, integral, " .
//今/天/优/品/多/商/户/系/统-采集 mod on 20160808 by prince  qq 1/2/0/0/2/9/1/2/1-begin
...
//今/天/优/品/多/商/户/系/统-采集 mod on 20160808 by prince  qq 1/2/0/0/2/9/1/2/1-end
                    ...
                    " FROM " . $GLOBALS['yp']->table('goods') . " AS g,".$GLOBALS['yp']->table('supplier_goods_cat')." as sgc,".$GLOBALS['yp']->table('virtual_district')." as dis, ".
                        $GLOBALS['yp']->table('virtual_goods_district')." as dig".
                        " WHERE dis.supplier_id = ".$_SESSION['supplier_id']." AND sgc.goods_id=g.goods_id and dis.goods_id = g.goods_id and dis.district_id = dig.district_id  AND is_delete='$is_delete' AND sgc.supplier_id='". $_SESSION['supplier_id'] ."' $where" .
...
//今/天/优/品/多/商/户/系/统-采集 mod on 20160808 by prince  qq 1/2/0/0/2/9/1/2/1-end
                    " (promote_price > 0 AND promote_start_date <= '$today' AND promote_end_date >= '$today') AS is_promote ".
                    " FROM " . $GLOBALS['yp']->table('goods') . " AS g,".$GLOBALS['yp']->table('supplier_goods_cat')." as sgc WHERE sgc.goods_id=g.goods_id AND is_delete='$is_delete' AND sgc.supplier_id='". $_SESSION['supplier_id'] ."' $where" .
                    " ORDER BY $filter[sort_by] $filter[sort_order] ".
                    " LIMIT " . $filter['start'] . ",$filter[page_size]";
         }
          /* 代码增加_虚拟团购_END  demo.coolhong.com */
...
function product_list($goods_id, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        //采集 mod on 20160808 by PRINCE  qq 1 2 0 0 2 9 1 2 1增加：属性价格pro_price//
