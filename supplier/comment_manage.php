...
if ($_REQUEST['act']=='reply')
{
    ...
    /* 获取管理员的用户名和Email地址 */
    $sql = "SELECT user_name, email FROM ". $yp->table('supplier_admin_user').
...
function get_comment_list()
{
    ...
    $sql = "SELECT count(*) FROM " .$GLOBALS['yp']->table('comment'). " AS c left join ". $GLOBALS['yp']->table('goods') .
    ...
    /* 获取评论数据 */
    $arr = array();
    $sql  = "SELECT c.*, g.goods_name AS title FROM " .$GLOBALS['yp']->table('comment'). " AS c left join ". $GLOBALS['yp']->table('goods') .
			" AS g on c.id_value = g.goods_id WHERE c.parent_id = 0 $where " .
