...
/**
 * PRINCE QQ120029121 在线聊天客服管理
 * ============================================================================
 * 演示地址: http://demo.coolhong.com；
 * ============================================================================
 * $Author: jtypmall $
 * $Id: customer.php 17217 2017-07-07 06:29:08Z PRINCE $
 */
...
function action_list ()
{
	...
	$sql = "SELECT count(*) " . "FROM " . $GLOBALS['yp']->table('chat_customer') . " WHERE supp_id='$supplier_id' ";
	$count=$GLOBALS['db']->getOne($sql);

	if($count>1){
		$limit=$count-1;
		$sql = "delete from " . $yp->table('chat_customer') . " where supp_id='$supplier_id' limit $limit ";
		$GLOBALS['db']->query($sql);
	}elseif($count==0){
		$customer['add_time'] = gmtime();
		$customer['supp_id'] =$supplier_id;
		$GLOBALS['db']->autoExecute($yp->table('chat_customer'), $customer, 'INSERT');
	}
	$sql = "SELECT * " . "FROM " . $GLOBALS['yp']->table('chat_customer') . " WHERE supp_id='$supplier_id' ";
