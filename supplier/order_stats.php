 ...
 * ============================================================================
 * $Author: jtypmall $
 * $Id: order_stats.php 2015-10-21 14:00:08Z jtypmall $
 */
...
if ($_REQUEST['act'] == 'list')
{
    ...
    $order_money = $db->getOne(
        ...
        . $yp->table('order_info') . $where
    );
    $order_money = empty($order_money) ? 0 : $order_money;
    // 下单会员数
    $order_member = $db->getOne(
        'SELECT COUNT(*) FROM (SELECT DISTINCT user_id FROM ' . $yp->table('order_info') . $where. ') o'
    );
    // 下单量
    $order_count = $db->getOne(
        'SELECT COUNT(*) FROM ' . $yp->table('order_info') . $where
    );
    // 下单商品数
    $order_goods_number = $db->getOne(
        'SELECT COUNT(*) FROM ' . $yp->table('order_goods') . ' WHERE order_id IN ( '
        . 'SELECT order_id FROM ' . $yp->table('order_info') . $where . ')'
    );
    ...
    $goods_save = $db->getOne(
        'SELECT COUNT(*) FROM ' . $yp->table('collect_goods') . 'WHERE goods_id IN ('
        . 'SELECT goods_id FROM ' . $yp->table('goods') . 'WHERE supplier_id = '
        . $_SESSION['supplier_id'] . ')'
    );
    // 商品总数
    $goods_number = $db->getOne(
        'SELECT COUNT(*) FROM ' . $yp->table('goods') . 'WHERE supplier_id = ' . $_SESSION['supplier_id']
    );
    // 店铺收藏量
    $shop_save = $db->getOne(
        'SELECT COUNT(*) FROM ' . $yp->table('supplier_guanzhu') . ' WHERE supplierid = ' . $_SESSION['supplier_id']
    );
    ...
    $result = $db->getAll(
        ...
        . $yp->table('order_info') . $where . ' GROUP BY date ORDER BY date'
    );
