...
elseif ($_REQUEST['act'] == 'info')
{
   ...
   /*付款方式_添加_START_demo.coolhong.com*/
   $payment_sql = 
   		"SELECT pay_code FROM " . $GLOBALS['yp']->table('payment') .
   		" WHERE pay_id = " . $order['pay_id'];
    $order['pay_code'] = $GLOBALS['db']->getOne($payment_sql);
    /*付款方式_添加_end_demo.coolhong.com*/
    ...
    $sql = "SELECT MAX(order_id) FROM " . $yp->table('order_info') . " as o WHERE supplier_id = '$order[supplier_id]' and order_id < '$order[order_id]'";
    if ($agency_id > 0)
    {
    ...
    $sql = "SELECT MIN(order_id) FROM " . $yp->table('order_info') . " as o WHERE supplier_id = '$order[supplier_id]' and order_id > '$order[order_id]'";
...
{
	admin_priv('edit_order_print');
	sys_msg('对不起，你的权限不足！');// 禁止供应商编辑订单打印模板 20160904
    /* 更新模板文件的内容 */
...
elseif ($_REQUEST['act'] == 'search_users')
{
    ...
    if ($id_name != '')
    {
        $sql = "SELECT user_id, user_name FROM " . $GLOBALS['yp']->table('users') .
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT agency_id FROM " . $GLOBALS['yp']->table('supplier_admin_user') . " WHERE user_id = '$_SESSION[admin_id]'";
        ...
        {
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('order_info') . " AS o ". $where;
        }
...
function delivery_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('delivery_order') . "AS d left join  ". $GLOBALS['yp']->table('order_info')." AS o on d.order_id=o.order_id ". $where;
        $filter['record_count']   = $GLOBALS['db']->getOne($sql);
        $filter['page_count']     = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;
        ...
        $sql = "SELECT d.delivery_id, d.delivery_sn, d.order_sn, d.order_id, d.add_time, d.action_user, d.consignee, d.country,
                ...
                FROM " . $GLOBALS['yp']->table("delivery_order") . " AS d left join ". $GLOBALS['yp']->table("order_info") . " AS o on d.order_id=o.order_id 
                $where
...
function back_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('back_order') . " AS b left join  ". $GLOBALS['yp']->table("order_info") ." AS  o on b.order_id=o.order_id ". $where;
        $filter['record_count']   = $GLOBALS['db']->getOne($sql);
        $filter['page_count']     = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;
        ...
        $sql = "SELECT b.back_id, b.delivery_sn, b.order_sn, b.order_id, b.add_time, b.action_user, b.consignee, b.country,
                ...
                FROM " . $GLOBALS['yp']->table("back_order") . " AS b left join ". $GLOBALS['yp']->table("order_info") ." AS o on b.order_id=o.order_id 
                $where
