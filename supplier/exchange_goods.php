...
/*初始化数据交换对象 */
$exc   = new exchange($yp->table("exchange_goods"), $db, 'goods_id', 'exchange_integral');
//$image = new cls_image();
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$yp->table('exchange_goods')."(goods_id, exchange_integral, is_exchange, is_hot) ".
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT eg.goods_id, eg.exchange_integral,eg.is_exchange, eg.is_hot, g.goods_name ".
           " FROM " . $yp->table('exchange_goods') . " AS eg ".
           "  LEFT JOIN " . $yp->table('goods') . " AS g ON g.goods_id = eg.goods_id ".
...
//        $where = '';
        $where = " AND g.is_delete = 0 AND g.is_on_sale = 1  AND g.supplier_id = '$supplier_id' AND g.supplier_status = 1";
