...
}elseif ($_REQUEST['act'] == 'saveinfo')
{
	...
	$status = $db->getOne('SELECT status FROM ' .$yp->table('supplier'). ' WHERE supplier_id='.$save['supplier_id']);
	if($status != 1){
		$link[] = array('text' => $_LANG['go_back'], 'href' => 'javascript:history.back(-1)');
	...
	$sql = "INSERT INTO " .$yp->table('supplier_street'). " (supplier_id,supplier_type,supplier_name,supplier_title,supplier_desc,supplier_tags,add_time,is_show,status)VALUES ".
	...
	if($_FILES['logo']['size']){
		...
		$pic_sql = "update " .$yp->table('supplier_street'). " set logo='".$save['logo']."' where supplier_id=".$save['supplier_id'];
		$db->query($pic_sql);
	...
	if($_REQUEST['code'] == 'logo'){
		$pic_sql = "update " .$yp->table('supplier_street'). " set logo='' where supplier_id=".$_SESSION['supplier_id'];
		$db->query($pic_sql);
