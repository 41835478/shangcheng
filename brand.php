...
{
    $sql = 'SELECT * FROM ' . $GLOBALS['yp']->table('brand') . " WHERE brand_id = '$id'";
...
function brand_get_goods($brand_id, $cate, $size, $page, $sort, $order)
{
	...
	if ($sort=='salenum')
	{
		$sql = 'SELECT SUM(o.goods_number) as salenum, g.goods_id, g.goods_name, g.click_count, g.goods_number, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
            ...
            'FROM ' . $GLOBALS['yp']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['yp']->table('member_price') . ' AS mp ' .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " . "LEFT JOIN " . $GLOBALS['yp']->table('order_goods') .  " as o ON o.goods_id = g.goods_id " . " where g.brand_id = '$brand_id' $ext group by g.goods_id ORDER BY $sort $order";
...
function brand_related_cat($brand)
{
    ...
    $sql = "SELECT c.cat_id, c.cat_name, COUNT(g.goods_id) AS goods_count FROM ".
            $GLOBALS['yp']->table('category'). " AS c, ".
