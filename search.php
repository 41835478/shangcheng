 ...
 * ============================================================================
 * $Author: PRINCE $
 * $Id: search.php 17217 2017-04-01 06:29:08Z PRINCE $
*/

define('IN_PRINCE', true);
/* 代码添加_START  添加    今-天-优-品-多-商-户-系-统 Q-Q 120 029 121 */
...
/* 代码添加_END  添加    今-天-优-品-多-商-户-系-统 Q-Q 120 029 121 */
...
else
{
    ...
    {
        /* 代码修改_START   By    demo.coolhong.com   */
		include_once('includes/lib_splitword_qq120029121.php');
		$Recordkw = str_replace(array("\'"), array(''), trim($_REQUEST['keywords']));
		$cfg_soft_lang_qq_120029121 = 'utf-8';
        $sp_qq_120029121 = new SplitWord($cfg_soft_lang_qq_120029121, $cfg_soft_lang_qq_120029121);
        $sp_qq_120029121->SetSource($Recordkw, $cfg_soft_lang_qq_120029121, $cfg_soft_lang_qq_120029121);
        $sp_qq_120029121->SetResultType(1);
        $sp_qq_120029121->StartAnalysis(TRUE);
        $word_qq_120029121 = $sp_qq_120029121->GetFinallyResult(' '); 
		//echo  $word_qq_120029121;
        $word_qq_120029121 = preg_replace("/[ ]{1,}/", " ", trim($word_qq_120029121));
        $replacef_qq_120029121 = explode(' ', $word_qq_120029121);


        $keywords = 'AND (';
        $goods_ids = array();
        foreach ($replacef_qq_120029121 AS $key => $val)
        {
            if ($key > 0 && $key < count($replacef_qq_120029121) && count($replacef_qq_120029121) > 1)
            {
                $keywords .= " OR ";
            }
            ...
            $sql = 'SELECT DISTINCT goods_id FROM ' . $yp->table('tag') . " WHERE tag_words LIKE '%$val%' ";
            $res = $db->query($sql);
		...
		//$db->autoReplace($yp->table('keywords'), array('date' => local_date('Y-m-d'),
        //'searchengine' => 'jtypmall', 'keyword' => htmlspecialchars_decode(str_replace('%', '', $Recordkw)), 'count' => 1), array('count' => 1));

		/* 代码修改_END   By    demo.coolhong.com   */
        $keywords .= ')';
    ...
    {
        $sql = "SELECT goods_id, COUNT(*) AS num FROM " . $yp->table("goods_attr") . " WHERE 0 ";
    ...
    {
        /* 从选购中心进入的链接 */
        $sql = "SELECT DISTINCT(goods_id) FROM " . $yp->table('goods_attr');
        $col = $db->getCol($sql);
   ...
   /* fulltext_search_add_END_demo.coolhong.com */
    /* 获得符合条件的商品总数 */
    $sql   = "SELECT COUNT(*) FROM " .$yp->table('goods'). " AS g ".
    ...
    $count = $db->getOne($sql);
	/* 代码添加_START   By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121  */	
    if ($page == 1 && $Recordkw && $count && !$category) Recordkeyword($Recordkw, $count, 'jtypmall');  // 代码修改  添加    今-天-优-品-多-商-户-系-统 Q-Q 120 029 121
	if($count==0)
	{
		if (preg_match('/^[a-zA-z]+$/i', $_REQUEST['keywords']))
		{
			$sql_qq_120029121 = "select keyword from ". $yp->table('keyword') ." where letter='". $_REQUEST['keywords'] ."' order by items desc limit 0,1";
			$keyword_qq_120029121 = $db->getOne($sql_qq_120029121);
			if($keyword_qq_120029121 && $keyword_qq_120029121!=$_REQUEST['keywords'])
			{
				header('Location: search.php?keywords='.$keyword_qq_120029121.'&keyword_zm='.$_REQUEST['keywords']);
				exit;
			}
		}
	}
	...
	{
		$smarty->assign('beizhuxinxi_qq_120029121', '关键词<font color=#cc0000>'.$_REQUEST['keyword_zm'].'</font>搜索结果为零，<br>但是我们为您匹配到了相关关键词<font color=#cc0000>'.$_REQUEST['keywords'].'</font>，下面是它的查询结果！');
	}
	/* 代码添加_END   By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121  */
...
//	$ts_string_array = array("*","/");
    ...
    if($_CFG['fulltext_search'] == '1'){
		...
		if(empty($idarray)){
			$idarray = array();
				foreach($replacef_qq_120029121 as $key => $value){
    ...
    $count = $db->getOne($sql);
    if ($page == 1 && $Recordkw && $count && !$category) Recordkeyword($Recordkw, $count, 'jtypmall');
	if($count==0)
	{
		if (preg_match('/^[a-zA-z]+$/i', $_REQUEST['keywords']))
		{
			$sql = "select keyword from ". $yp->table('keyword') ." where letter='". $_REQUEST['keywords'] ."' order by items desc limit 0,1";
    ...
    if ($page > $max_page)
    {
        ...
        //$sqls = "select * from goods where goods_id in ({$ids})";
        $sql = "SELECT g.goods_id, g.goods_name, g.market_price, g.click_count, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ".
            ...
            //如果服务关闭则关闭全文搜索功能
            $sql = "update ".$yp->table('shop_config')." set value = 0 where code = 'fulltext_search'";
            $db -> query($sql);
            clear_cache_files(); 
    ...
    while ($row = $db->FetchRow($res))
    {
		...
		$arr[$row['goods_id']]['goods_name'] = $row['goods_name'];
		/* 代码添加_START   By    demo.coolhong.com */
		$arr[$row['goods_id']]['goods_name_qq_120029121'] =  $arr[$row['goods_id']]['goods_name'];
		foreach($replacef_qq_120029121 as $key_qq_120029121 =>$temp_qq)
		{
				//$replacet[$key_qq_120029121]=  '<strong style="color:#cc0000;">'. $temp_qq .'</strong>';;
				
			if(trim($temp_qq) != '+'){
				if(in_array($temp_qq,$ts_string_array)){
					$arr[$row['goods_id']]['goods_name_qq_120029121'] = preg_replace('/(?!<[^>]*)\\'.$temp_qq.'(?![^<]*>)/i', '<strong style="color:#cc0000;">'. $temp_qq .'</strong>', $arr[$row['goods_id']]['goods_name_qq_120029121']);
				}else{
					$arr[$row['goods_id']]['goods_name_qq_120029121'] = preg_replace('/(?!<[^>]*)'.$temp_qq.'(?![^<]*>)/i', '<strong style="color:#cc0000;">'. $temp_qq .'</strong>', $arr[$row['goods_id']]['goods_name_qq_120029121']);
				}
			}
    ...
    $smarty->assign('promotion_info', get_promotion_info());
	/* demo.coolhong.com */
	$sql= "select g.cat_id, count(*) AS cat_count from ".$yp->table('goods')." AS g ".
	...
	//扩展分类
	/*
	$sql_k =  "select gc.cat_id, count(*) AS cat_count from ".$yp->table('goods_cat')." AS gc ".
				" left join ". $yp->table('goods') ." AS g  on g.goods_id= gc.goods_id ".
	...
	*/
	...
	if (strlen($Recordkw)>1)
	{
		...
		if(!empty($last)){
		    $sql_r = "SELECT w_id FROM " .$GLOBALS['yp']->table('keyword'). " WHERE searchengine='jtypmall' AND word='$Recordkw'  ";
			$w_id = $db->getOne($sql_r);
			if($w_id){
				$sql_r = "UPDATE " . $yp->table('keyword') . " SET " .
...
function get_seachable_attributes($cat_id = 0)
{
    ...
    /* 获得可用的商品类型 */
    $sql = "SELECT t.cat_id, cat_name FROM " .$GLOBALS['yp']->table('goods_type'). " AS t, ".
