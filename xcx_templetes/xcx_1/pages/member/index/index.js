...
Page({
  ...
  getFenxiaomenu: function () {
    var that = this;
    var user_id = getApp().globalData.userInfo.user_id
    server.getJSON('/User/getFenxiaomenu/user_id/' + user_id, function (res) {
      var is_distrib = res.data.result;
      that.setData({
...
wx.navigateTo({
  ...
  complete: function(res) {
    ...
    return ;
       ...
       wx.request({
         url: 'https://newdemo.coolhong.com/index.php/Api/User/logoutWX/openid/' + app.globalData.openid,
         data: {},
  ...
  getnum: function (e) {
    ...
    if (parseInt(that.data.phoneNum).toString().length == 11) {
      ...
      return ;
      wx.request({
        url: 'https://newdemo.coolhong.com/index.php/Api/Shipper/getCaptcha.html',
  ...
  quick_login_phone: function (e) {
    ...
    if (parseInt(this.data.num).toString().length == 4) {
      ...
      console.log('https://newdemo.coolhong.com/index.php/Api/User/validate?phone=' + this.data.phoneNum + "&num=" + this.data.num + "&openid=" + app.globalData.openid);
      wx.request({
        url: 'https://newdemo.coolhong.com/index.php/Api/User/validate?phone=' + this.data.phoneNum + "&num=" + this.data.num + "&openid=" + app.globalData.openid,
...
timeout = setTimeout(function doHandler()
  ...
  getMainUserInfo: function () {
    var that = this;
    var app = getApp();
    var syskey = app.globalData.syskey
    var user_id = app.globalData.userInfo.user_id

    server.getJSON('/User/getMainUserInfo/user_id/' + user_id + '/syskey/' + syskey, function (res) {
      if (res.data.status == 1) {
        if (res.data.result.mobile_phone){
          that.setData({
            mobile_phone: res.data.result.mobile_phone
          });
        }
      }
    });
  ...
  getPhoneNumber: function (e) {
    ...
    server.postJSON('/User/setPhoneNumber/user_id/' + user_id, { user_id: user_id, syskey: syskey, session_key: session_key, iv: iv, encryptedData: encryptedData }, function (res) {
      if (res.data.status == 1) {
