...
Page({
  ...
  useCoupon: function () {
    ...
    var newValue = parseInt(money * 100);
    money = newValue / 100.0;//转换 by prince

    var totalObj = this.data.totalPrice;
    totalObj.coupon_money = money;//prince
    console.log(1);
    console.log(totalObj);
    totalObj.total_fee = (parseInt(totalObj.goods_amount * 100) + parseInt(totalObj.shipping_fee * 100) - parseInt(totalObj.coupon_money * 100) - parseInt(totalObj.user_money * 100)) / 100.0;//prince 
  ...
  use: function () {
    ...
    if (use_money == "" || use_money == undefined) {
      wx.showToast({
        title: '请输入余额',
        duration: 1000
      });
    ...
    //var totalPrice = m - use_money;//prince

    var totalObj = this.data.totalPrice;
    var newValue = parseInt(use_money * 100);
    use_money = newValue / 100.0;//转换 by prince
    totalObj.user_money = use_money;
    console.log(2);
    console.log(totalObj);
    totalObj.total_fee = (parseInt(totalObj.goods_amount * 100) + parseInt(totalObj.shipping_fee * 100) - parseInt(totalObj.coupon_money * 100) - parseInt(totalObj.user_money*100))/100.0;//prince 
    if (totalObj.total_fee < 0){
  ...
  onShow: function () {
    ...
    this.setData({ cartIds: cartIds, amount: amount, use_money: 0, "coupon": [], cv: '请选择优惠劵', cpos: -1});//prince
  ...
  formSubmit: function (e) {
    ...
    var coupon_id = 0;
    if (this.data.cpos != -1) {
      coupon_id = this.data.couponList[this.data.cpos].bonus_id;//prince 
    }
    var couponCode = this.data.couponCode;

    server.getJSON('/Cart/cart3/act/submit_order/user_id/' + user_id + "/address_id/" + address_id + "/user_money/" + use_money + "/pay_points/" + pay_points + "/couponTypeSelect/" + couponTypeSelect + "/coupon_id/" + coupon_id + "/couponCode/" + couponCode + "/syskey/" + syskey, function (res) {
  ...
  getCarts: function (cartIds) {
    ...
    server.getJSON('/Cart/cart2/user_id/' + user_id + '/syskey/' + syskey, function (res) {
      ...
      var enable_coupon = res.data.result.enable_coupon
      var enable_balance = res.data.result.enable_balance
      var enable_point = res.data.result.enable_point
      ...
      that.setData({ address: address, cartList: cartList, userInfo: userInfo, totalPrice: totalPrice, enable_coupon: enable_coupon, enable_balance: enable_balance, enable_point: enable_point  });
