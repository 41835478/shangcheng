...
{
	case 'goods':
		$pathrow = $db->getRow("select c.path_name,c.cat_id from ". $yp->table('goods')." AS g left join ". $yp->table('category') ." AS c on g.cat_id=c.cat_id where g.goods_id='$id'" );
	...
	case 'category':
		$pathrow = $db->getRow("select path_name, cat_id from ".  $yp->table('category') ."  where cat_id = '$id' " );
		$pathrow['path_name'] = $pathrow['path_name'] ? $pathrow['path_name'] : ("cat".$pathrow['cat_id']);
		$pathrow['path_name'] = PREFIX_CATEGORY ."-".$pathrow['path_name'];
	...
	case 'article':
		$pathrow = $db->getRow("select c.path_name,c.cat_id from ". $yp->table('article')." AS a left join ". $yp->table('article_cat') ." AS c on a.cat_id=c.cat_id where a.article_id='$id'" );
		$pathrow['path_name'] = $pathrow['path_name'] ? $pathrow['path_name'] : ("cat".$pathrow['cat_id']);
	...
	case 'article_cat':
		...
		$pathrow = $db->getRow("select path_name, cat_id from ".  $yp->table('article_cat') ."  where cat_id='$id'" );
		$pathrow['path_name'] = $pathrow['path_name'] ? $pathrow['path_name'] : ("cat".$pathrow['cat_id']);
		$pathrow['path_name'] = PREFIX_ARTICLECAT ."-".$pathrow['path_name'];
