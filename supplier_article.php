...
if (!$smarty->is_cached('article_pro.dwt', $cache_id))
{
    ...
    /* 上一篇下一篇文章 */
    $next_article = $db->getRow("SELECT article_id, title FROM " .$yp->table('supplier_article'). " WHERE article_id > $article_id AND cat_id=$article[cat_id] AND is_open=1 AND supplier_id=".$_GET['suppId']." LIMIT 1");
    ...
    $prev_aid = $db->getOne("SELECT max(article_id) FROM " . $yp->table('supplier_article') . " WHERE article_id < $article_id AND cat_id=$article[cat_id] AND is_open=1 AND supplier_id=".$_GET['suppId']);
    if (!empty($prev_aid))
    {
        $prev_article = $db->getRow("SELECT article_id, title FROM " .$yp->table('supplier_article'). " WHERE article_id = $prev_aid");
