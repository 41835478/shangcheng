...
{
	$sql = "select cat_id, cat_name from " . $GLOBALS['yp']->table('category') . " where parent_id= '$cat_id' and is_show=1 ";
...
{
	$sql = "select * from " . $GLOBALS['yp']->table("cat_flashimg") . " where cat_id='$cat_id' order by sort_order";
...
{
	$sql = "select cat_name , cat_nameimg, cat_id  from " . $GLOBALS['yp']->table('category') . " where parent_id=$pcat_id and is_show =1 and show_in_index=1 order by sort_order  limit 0,15 ";
	$res = $GLOBALS['db']->query($sql);
	$arr = array();
	while($row = $GLOBALS['db']->fetchRow($res))
	{
		...
		$sql = "select cat_adimg_1, cat_adurl_1, cat_adimg_2, cat_adurl_2 from " . $GLOBALS['yp']->table("category") . " where cat_id='" . $row['cat_id'] . "' ";
		...
		$sql = "select goods_id,goods_thumb,goods_name,shop_price,promote_price,market_price,promote_start_date,promote_end_date from " . $GLOBALS['yp']->table('goods') . " AS g where g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 and g.is_catindex = 1 and ($children OR " . get_extension_goods($children) . ") order by goods_id desc limit 0, $limit_num";
...
{
	$topcat_info = $GLOBALS['db']->getRow('SELECT cat_name, cat_index_rightad FROM ' . $GLOBALS['yp']->table('category') . " WHERE cat_id = '$cat_id'");
...
function get_pro_top10 ($cats)
{
	...
	$sql = 'SELECT g.goods_id, g.goods_name,g.goods_brief, g.shop_price, g.goods_thumb, SUM(og.goods_number) as goods_number ' . 'FROM ' . $GLOBALS['yp']->table('goods') . ' AS g, ' . $GLOBALS['yp']->table('order_info') . ' AS o, ' . $GLOBALS['yp']->table('order_goods') . ' AS og ' . "WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 $where $top10_time ";
	// �ж��Ƿ����ÿ�棬������Ƿ����0
