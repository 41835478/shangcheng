function get_gallery_attr(goods_id, goods_attr_id)
	...
	{		
		...
		*   By demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1
		*/		
...
function changeAtt(t,goods_id) {
	...
	changePrice();    // By demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1
...
function show_attr_status(theid, goods_id, attr_id_qq_wx_120029121)
{
	...
	var qq_wx_120029121= theid.parentNode.id.replace('catt_', '');
	document.getElementById('spec_attr_type_'+qq_wx_120029121).value=selected_first[0];

	//if(attr_selected()){
	//	return true;
	//}
	if(change_classname(theid,qq_wx_120029121)){
		//var formBuy = document.forms['YP_FORMBUY'];
		//spec_arr = getSelectedAttributes(formBuy);
		//Ajax.call('goods.php?act=get_products_info', 'id=' + spec_arr+ '&goods_id=' + goods_id, shows_number, 'GET', 'JSON');
		changePriceAll();
	...
	for(zzz=0; zzz<mylist.length; zzz++)
	{
		if(mylist[zzz].className!='wuxiao')
		{
				mylist[zzz].onclick=function(){
					show_attr_status(this, goods_id, attr_id_qq_wx_120029121);
				}
		}
	...
	document.getElementById(the_input_id).checked="checked";
	 ...
	 {
	     selid_qq_wx_120029121=0;
	     if (spec_attr_type[iii].value != qq_wx_120029121)
	     {
		...
		{	
			...
			{					
				if (selid_qq_wx_120029121)
				{
					if (s1_list[jjj].className == 'cattsel')
					{
						selid_qq_wx_120029121 = s1_a_id;
					}
				}
				else
				{
		...
		document.getElementById('spec_value_' + selid_qq_wx_120029121).checked = "checked";
		selected_first.push(selid_qq_wx_120029121);
		document.getElementById('xuan_a_'+selid_qq_wx_120029121).className='cattsel';
		//document.getElementById('xuan_a_'+selid_qq_wx_120029121).onclick=function(){}		
	     }
	}
	changeAtt(theid, goods_id);
	if (theid.parentNode.id.replace('catt_','')==attr_id_qq_wx_120029121)
	{
 ...
 function is_exist_prod(selected_first, id, prod_exist_arr)
 {
	...
	var qq_wx_120029121_selected = selected_first.slice(0);
	qq_wx_120029121_selected.push(id);	
	var all_valid =0;
	for (var i in prod_exist_arr)
	{
		var first_exist=1;
		for (var j in qq_wx_120029121_selected)
		{
			if ( prod_exist_arr[i].indexOf("|" + qq_wx_120029121_selected[j] + "|") =='-1')
			{
				first_exist=0;
				break;
			}
