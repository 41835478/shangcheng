...
{
		$sql = "SELECT a.article_id, a.title, a.file_url, a.open_type, a.add_time, a.content FROM ".$GLOBALS['yp']->table( "goods_article" )." AS g, ".$GLOBALS['yp']->table( "article" )." AS a ".( "WHERE g.article_id = a.article_id AND g.goods_id = '".$goods_id."' AND a.is_open = 1 " )."ORDER BY a.add_time DESC";
...
{
		$row = $GLOBALS['db']->getRow( "SELECT cat_name FROM ".$GLOBALS['yp']->table( "article_cat" ).( " WHERE cat_id = '".$cat_id."'" ) );
		return $row['cat_name'];
}

function get_goods_info_ex( $goods_id )
{
		$sql = "SELECT g.market_price, g.shop_price, g.promote_price, g.is_promote, g.promote_start_date, g.promote_end_date  FROM ".$GLOBALS['yp']->table( "goods" )." AS g ".( "WHERE g.goods_id = '".$goods_id."' AND g.is_delete = 0 " );
...
function index_get_group_buyex( )
{
		...
		if ( 0 < $limit )
		{
				$sql = "SELECT gb.act_id AS group_buy_id, gb.goods_id, gb.ext_info, gb.goods_name,gb.end_time, g.goods_thumb,g.market_price, g.goods_img, gb.act_desc FROM ".$GLOBALS['yp']->table( "goods_activity" )." AS gb, ".$GLOBALS['yp']->table( "goods" )." AS g WHERE gb.act_type = '".GAT_GROUP_BUY."' AND g.goods_id = gb.goods_id AND gb.start_time <= '".$time."' AND gb.end_time >= '".$time."' AND g.is_delete = 0 ORDER BY gb.act_id DESC ".( "LIMIT ".$limit );
				$res = $GLOBALS['db']->query( $sql );
				while ( $row = $GLOBALS['db']->fetchRow( $res ) )
				{
...
{
		$sql = "select rc.recommend_type from ".$GLOBALS['yp']->table( "category" )." as c left join ".$GLOBALS['yp']->table( "cat_recommend" ).( " as rc on c.cat_id = rc.cat_id where c.cat_id=".$cid );
...
function get_cart_goodsex( )
{
		...
		$sql = "SELECT *, IF(parent_id, parent_id, goods_id) AS pid  FROM ".$GLOBALS['yp']->table( "cart" )."  WHERE session_id = '".SESS_ID."' AND rec_type = '".CART_GENERAL_GOODS."' ORDER BY pid, parent_id";
		$res = $GLOBALS['db']->query( $sql );
		...
		while ( $row = $GLOBALS['db']->fetchRow( $res ) )
		{
				...
				{
						$sql = "SELECT attr_value FROM ".$GLOBALS['yp']->table( "goods_attr" )." WHERE goods_attr_id ".db_create_in( $row['goods_attr'] );
...
function get_categories_tree_ex( $cat_id = 0 )
{
		if ( 0 < $cat_id )
		{
				$sql = "SELECT parent_id FROM ".$GLOBALS['yp']->table( "category" ).( " WHERE cat_id = '".$cat_id."'" );
		...
		$sql = "SELECT count(*) FROM ".$GLOBALS['yp']->table( "category" ).( " WHERE parent_id = '".$parent_id."' AND is_show = 1 " );
		if ( $GLOBALS['db']->getOne( $sql ) || $parent_id == 0 )
		{
				$sql = "SELECT cat_id,cat_name ,parent_id,is_show FROM ".$GLOBALS['yp']->table( "category" ).( "WHERE parent_id = '".$parent_id."' AND is_show = 1 ORDER BY sort_order ASC, cat_id ASC" );
...
{
		$three_arr = array( );
		$sql = "SELECT count(*) FROM ".$GLOBALS['yp']->table( "category" ).( " WHERE parent_id = '".$tree_id."' AND is_show = 1 " );
		if ( $GLOBALS['db']->getOne( $sql ) || $tree_id == 0 )
		{
				$child_sql = "SELECT cat_id, cat_name, parent_id, is_show FROM ".$GLOBALS['yp']->table( "category" ).( "WHERE parent_id = '".$tree_id."' AND is_show = 1 ORDER BY sort_order ASC, cat_id ASC" );
...
function get_cagtegory_goods_count_ex( $children, $brand = 0, $min = 0, $max = 0, $ext = "" )
{
		...
		return $GLOBALS['db']->getOne( "SELECT COUNT(*) FROM ".$GLOBALS['yp']->table( "goods" ).( " AS g WHERE ".$where." {$ext}" ) );
}
...
{
		$sql = "SELECT b.brand_id, b.brand_name, b.brand_logo, b.brand_desc, COUNT(*) AS goods_num, IF(b.brand_logo > '', '1', '0') AS tag FROM ".$GLOBALS['yp']->table( "brand" )."AS b, ".$GLOBALS['yp']->table( "goods" )." AS g ".( "WHERE g.brand_id = b.brand_id ".$children." AND is_show = 1 " )." AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 GROUP BY b.brand_id HAVING goods_num > 0 ORDER BY  CONVERT( brand_name USING gbk ) COLLATE gbk_chinese_ci ASC";
...
{
		$sql = "SELECT c.cat_id, c.cat_name, c.sort_order, a.article_id, a.title, a.file_url, a.open_type FROM ".$GLOBALS['yp']->table( "article" )." AS a LEFT JOIN ".$GLOBALS['yp']->table( "article_cat" )." AS c ON a.cat_id = c.cat_id WHERE c.cat_type = 5 AND a.is_open = 1 ORDER BY c.sort_order ASC, a.article_id";
		$res = $GLOBALS['db']->getAll( $sql );
		$arr = array( );
...
function get_cat_new_arts( $cat_id, $num )
{
		...
		$sql = "SELECT a.article_id, a.title, ac.cat_name, a.add_time, a.file_url, a.open_type, ac.cat_id, ac.cat_name  FROM ".$GLOBALS['yp']->table( "article" )." AS a, ".$GLOBALS['yp']->table( "article_cat" )." AS ac WHERE a.is_open = 1 and a.article_type = 0 AND a.cat_id = ac.cat_id AND ac.cat_type = 1 AND a.".$cat_str." ORDER BY a.add_time DESC LIMIT ".$num;
		$res = $GLOBALS['db']->getAll( $sql );
		$arr = array( );
...
{
		$sql = "SELECT * FROM ".$GLOBALS['yp']->table( "article" ).( " WHERE is_open = 1 AND article_id = '".$art_id."'" );
		$row = $GLOBALS['db']->getRow( $sql );
		return $row;
}
...
{
		$sql = "select parent_id from ".$GLOBALS['yp']->table( "category" )." where cat_id = ".$nid."";
...
function get_comment_count( $id, $type, $flag = 0 )
{
		...
		$sql = "SELECT COUNT(*) FROM ".$GLOBALS['yp']->table( "comment" ).( " WHERE id_value = '".$id."' AND comment_type = '{$type}' AND status = 1 AND parent_id = 0 " ).$where;
...
function assign_commentex( $id, $type, $page = 1, $flag = 0 )
{
		...
		$count = $GLOBALS['db']->getOne( "SELECT COUNT(*) FROM ".$GLOBALS['yp']->table( "comment" ).( " WHERE id_value = '".$id."' AND comment_type = '{$type}' {$ip_address} AND status = 1 AND parent_id = 0" ) );
		$size = !empty( $GLOBALS['_CFG']['comments_number'] ) ? $GLOBALS['_CFG']['comments_number'] : 5;
		$page_count = 0 < $count ? intval( ceil( $count / $size ) ) : 1;
		$sql = "SELECT * FROM ".$GLOBALS['yp']->table( "comment" ).( " WHERE id_value = '".$id."' AND comment_type = '{$type}' {$ip_address} AND status = 1 AND parent_id = 0" )." ORDER BY comment_id DESC";
		$res = $GLOBALS['db']->selectLimit( $sql, $size, ( $page - 1 ) * $size );
		$arr = array( );
		$ids = "";
		while ( ( $row = $GLOBALS['db']->fetchRow( $res ) ) ) // 修改 By  demo.coolhong.com 今天优品多商户系统 Q-Q：12 00 29 12 1
		{
				$ids .= $row['comment_id'];
		...
		if ( $ids )
		{
				$sql = "SELECT * FROM ".$GLOBALS['yp']->table( "comment" ).( " WHERE parent_id IN( ".$ids." )" );
				$res = $GLOBALS['db']->query( $sql );
...
function get_cat_recommend_goods( $type = "", $cats = "", $cat_num = 0, $brand = 0, $min = 0, $max = 0, $ext = "" )
{
		...
		$sql = "SELECT g.goods_id, g.goods_name, g.goods_name_style,g.goods_sn,  g.market_price, g.shop_price AS org_price, g.promote_price, ".( "IFNULL(mp.user_price, g.shop_price * '".$_SESSION['discount']."') AS shop_price, " )."(select AVG(r.comment_rank) from ".$GLOBALS['yp']->table( "comment" )." as r where r.id_value = g.goods_id AND r.comment_type = 0 AND r.parent_id = 0 AND r.status = 1) AS comment_rank, (select IFNULL(sum(og.goods_number), 0) from ".$GLOBALS['yp']->table( "order_goods" )." as og where og.goods_id = g.goods_id) AS sell_number, promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, goods_img, b.brand_name, b.brand_id, g.is_best, g.is_new, g.is_hot, g.is_promote FROM ".$GLOBALS['yp']->table( "goods" )." AS g LEFT JOIN ".$GLOBALS['yp']->table( "brand" )." AS b ON b.brand_id = g.brand_id LEFT JOIN ".$GLOBALS['yp']->table( "member_price" )." AS mp ".( "ON mp.goods_id = g.goods_id AND mp.user_rank = '".$_SESSION['user_rank']."' " )."WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ".$brand_where.$price_where.$ext;
		$num = 0;
...
function get_child_cat( $tree_id = 0, $num = 0 )
{
		...
		if ( $GLOBALS['db']->getOne( $sql ) || $tree_id == 0 )
		{
				$child_sql = "SELECT cat_id, cat_name, parent_id, is_show FROM ".$GLOBALS['yp']->table( "category" ).( "WHERE parent_id = '".$tree_id."' AND is_show = 1 ORDER BY sort_order ASC, cat_id ASC {$where}" );
				$res = $GLOBALS['db']->getAll( $child_sql );
				foreach ( $res as $row )
				{
						if ( $row['is_show'] )
...
{
		$sql = "select * from ".$GLOBALS['yp']->table( "category" )." where cat_id = ".$cid."";
		return $GLOBALS['db']->getRow( $sql );
}

function get_cat_name_ex( $cid )
{
		$sql = "select cat_name from ".$GLOBALS['yp']->table( "category" )." where cat_id = ".$cid."";
		return $GLOBALS['db']->getOne( $sql );
}

function get_brand_info_ex( $bid )
{
		$sql = "select * from ".$GLOBALS['yp']->table( "brand" )." where brand_id = ".$bid."";
		return $GLOBALS['db']->getRow( $sql );
}
...
function get_cat_brands( $cat, $num = 0, $app = "category" )
{
		...
		$sql = "SELECT b.brand_id, b.brand_name, b.brand_logo, COUNT(g.goods_id) AS goods_num, IF(b.brand_logo > '', '1', '0') AS tag FROM ".$GLOBALS['yp']->table( "brand" )."AS b, ".$GLOBALS['yp']->table( "goods" )." AS g ".( "WHERE g.brand_id = b.brand_id ".$children." " )."GROUP BY b.brand_id HAVING goods_num > 0 ORDER BY tag DESC, b.sort_order ASC ".$where;
...
//add by prince 120029121 s
function get_cat_supplier( $cat, $num = 0, $app = "category" )
{
		$where = "";
		if ( $num != 0 )
		{
		...
		$children = 0 < $cat ? " AND ".get_children( $cat ) : "";
		$sql = "SELECT b.supplier_id, b.supplier_name, COUNT(g.goods_id) AS goods_num FROM ".$GLOBALS['yp']->table( "supplier" )."AS b, ".$GLOBALS['yp']->table( "goods" )." AS g ".( "WHERE g.supplier_id = b.supplier_id ".$children." " )." and b.status=1 GROUP BY b.supplier_id HAVING goods_num > 0 ORDER BY b.rank_id DESC ".$where;
		$row = $GLOBALS['db']->getAll( $sql );
		foreach ( $row as $key => $val )
		{
				$row[$key]['id'] = $val['supplier_id'];
				$row[$key]['name'] = $val['supplier_name'];
				$row[$key]['logo'] = 'logo_supplier'.$val['supplier_id'].'.jpg';
				$row[$key]['url'] = 'supplier.php?suppId='.$val['supplier_id'];
		}
		return $row;
}
//add by prince 120029121 e

function get_goods_gallerys( $goods_id )
{
		$sql = "SELECT img_id, img_url, thumb_url,img_original, img_desc FROM ".$GLOBALS['yp']->table( "goods_gallery" ).( " WHERE goods_id = '".$goods_id."' LIMIT " ).$GLOBALS['_CFG']['goods_gallery_number'];
		$row = $GLOBALS['db']->getAll( $sql );
...
{
		$str = "";
		if ( !empty( $_COOKIE['YP']['history'] ) )
		{
				$where = db_create_in( $_COOKIE['YP']['history'], "goods_id" );
				$sql = "SELECT goods_id, goods_name, goods_thumb, shop_price, market_price FROM ".$GLOBALS['yp']->table( "goods" ).( " WHERE ".$where." AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0" );
...
function get_hotcate_tree( $rtype = "hot" )
{
		...
		if ( $GLOBALS['db']->getOne( $sql ) || $parent_id == 0 )
		{
				$sql = "select c.* from ".$GLOBALS['yp']->table( "category" )." as c left join ".$GLOBALS['yp']->table( "cat_recommend" ).( " as rc on c.cat_id = rc.cat_id where rc.recommend_type=3 and c.parent_id=".$parent_id." order by c.sort_order asc, c.cat_id asc" );
				$res = $GLOBALS['db']->getAll( $sql );
				foreach ( $res as $row )
				{
						if ( $row['is_show'] )
...
function get_top_parentid( $id = 0, $type = "" )
{
		if ( 0 < $id && $type != "" )
		{
				if ( $type == "goods" )
				{
						$sql = "SELECT cat_id FROM ".$GLOBALS['yp']->table( "goods" )." WHERE goods_id=".$id;
						$id = $GLOBALS['db']->getOne( $sql );
						$res['goods_parentid'] = $id;
				}
				while ( $id )
				{
						$sql = "SELECT\tcat_id,parent_id FROM ".$GLOBALS['yp']->table( "category" )." WHERE cat_id=".$id;
...
{
		$sql = "SELECT cat_id,cat_name,parent_id FROM ".$GLOBALS['yp']->table( "category" )." WHERE cat_id in (SELECT DISTINCT cat_id FROM ".$GLOBALS['yp']->table( "cat_recommend" )." WHERE is_show=1 ";
		if ( $rtype == "best" )
		{
...
{
		$sql = "SELECT SUM(goods_number) AS number, SUM(goods_price * goods_number) AS amount FROM ".$GLOBALS['yp']->table( "cart" )." WHERE session_id = '".SESS_ID."' AND rec_type = '".CART_GENERAL_GOODS."'";
		$row = $GLOBALS['db']->GetRow( $sql );
		if ( $row )
		{
...
{
		$arr = array( );
		$sql = "select c.* from ".$GLOBALS['yp']->table( "category" )." as c left join ".$GLOBALS['yp']->table( "cat_recommend" ).( " as rc on c.cat_id = rc.cat_id where rc.recommend_type=".$rec." order by c.sort_order asc, c.cat_id asc limit {$num}" );
		$res = $GLOBALS['db']->getAll( $sql );
		foreach ( $res as $row )
		{
				$arr[$row['cat_id']]['id'] = $row['cat_id'];
...
{
		$arr = array( );
		$sql = "select c.*, rc.recommend_type from ".$GLOBALS['yp']->table( "category" )." as c left join ".$GLOBALS['yp']->table( "cat_recommend" ).( " as rc on c.cat_id = rc.cat_id where c.parent_id=".$pid." order by c.sort_order asc, c.cat_id asc" );
		$res = $GLOBALS['db']->getAll( $sql );
		foreach ( $res as $row )
		{
...
{
		$arr = array( );
		$sql = "select c.*, rc.recommend_type from ".$GLOBALS['yp']->table( "category" )." as c left join ".$GLOBALS['yp']->table( "cat_recommend" ).( " as rc on c.cat_id = rc.cat_id where rc.recommend_type=".$rec." and c.parent_id={$pid} order by c.sort_order asc, c.cat_id asc" );
		$res = $GLOBALS['db']->getAll( $sql );
		foreach ( $res as $row )
		{
				$arr[$row['cat_id']]['id'] = $row['cat_id'];
...
{
		$arr = array( );
		$sql = "select c.*, g.goods_id, g.goods_thumb, g.goods_name from ".$GLOBALS['yp']->table( "comment" )." AS c  LEFT JOIN ".$GLOBALS['yp']->table( "goods" )." AS g ON c.id_value = g.goods_id where c.comment_type = ".$type." and c.status=1 order by c.add_time desc limit ".$count;
		$res = $GLOBALS['db']->getAll( $sql );
		foreach ( $res as $idx => $row )
		{
...
{
	$sql = 'SELECT id, number, type, sort_order, ext_info FROM ' . $GLOBALS['yp']->table('template') .
...
function get_child_tree_best($tree_id = 0)
{
    ...
    if ($GLOBALS['db']->getOne($sql) || $tree_id == 0)
    {
        $child_sql = 'SELECT c.cat_id, c.cat_name, c.parent_id, c.is_show, r.recommend_type ' .
                ...
                'INNER JOIN ' . $GLOBALS['yp']->table('cat_recommend') . ' AS r ' .
