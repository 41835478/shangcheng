...
{
	$sql = "SELECT ap.ad_width, ap.ad_height, ad.ad_name, ad.ad_code, ad.ad_link FROM " . $GLOBALS['yp']->table('ad_position') .           " AS ap LEFT JOIN " . $GLOBALS['yp']->table('ad') . " AS ad ON ad.position_id = ap.position_id WHERE ad.ad_name";
	if($getlike==true)
	{
		$sql .= " LIKE '" . $name . "%'";
	}
...
{
	$sql = "select cat_id,cat_name from ".$GLOBALS['yp']->table('category')." where parent_id=".$pid;
...
{
	$sql = "select cat.cat_id,cat.cat_name from ".$GLOBALS['yp']->table('category')." as cat where cat.cat_id in (select distinct cat_id from ".$GLOBALS['yp']->table('goods')." where brand_id = ".$bid.")";
	$res = $GLOBALS['db']->getAll($sql);
	return $res;
}
...
{
	$sql = "select ".$field_name." from ".$GLOBALS['yp']->table('goods')." where goods_id='".$pid."'";
...
function get_subcate_byurl($url)
{
	...
	if($rs!==false)
	{
		...
		$cid = $matches[0];
		$cat_arr = array();
		$sql = "select * from ".$GLOBALS['yp']->table('category')." where parent_id=".$cid." and is_show=1";
		$res = $GLOBALS['db']->getAll($sql);
...
{
   //开始获取子分类
    $sql_sub = "select * from ".$GLOBALS['yp']->table('category')." where parent_id=".$pid." and is_show=1";
...
{
	$sql = "select * from ".$GLOBALS['yp']->table('comment')." where comment_type = ".$type." and status=1 order by add_time desc limit ".$count;
	$res = $GLOBALS['db']->getAll($sql);
	return $res;
}
...
{
	$sql = "SELECT * FROM " . $GLOBALS['yp']->table('brand') . " WHERE is_show=1";
	$sql .= " ORDER BY sort_order ASC";
	if($num>0)
	{
		$sql .= " LIMIT " . $num;
...
{
	$sql = "SELECT cat_id,cat_name,parent_id FROM " . $GLOBALS['yp']->table('category') . " WHERE cat_id in (SELECT DISTINCT cat_id FROM " . $GLOBALS['yp']->table('cat_recommend') . " WHERE is_show=1 AND recommend_type=3) ";
	$sql .= " ORDER BY sort_order ASC";
	if($num>0)
	{
		$sql .= " LIMIT " . $num;
...
function get_comments($num)
 {
    $sql = 'SELECT a.*,b.goods_id,b.goods_thumb,b.goods_name FROM '. $GLOBALS['yp']->table('comment') .
             ' AS a,'. $GLOBALS['yp']->table('goods') .'AS b WHERE a.status = 1 AND a.parent_id = 0 and a.comment_type=0 and a.id_value=b.goods_id '.
