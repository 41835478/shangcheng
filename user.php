...
if(in_array($action, $ui_arr))
{
	...
	$sql = "SELECT value FROM " . $yp->table('shop_config') . " WHERE id = 419";
	$row = $db->getRow($sql);
	$car_off = $row['value'];
...
/* 代码增加_Start By demo.coolhong.com 今天优品多商户系统 qq 120029121  用户中心获取会员信息*/
...
if($rank = get_rank_info())
{
	...
	$rn = $rank['rank_name'];
	$recomm = $db->getOne("SELECT is_recomm FROM " . $GLOBALS['yp']->table('user_rank') . " WHERE rank_name= '$rn'");
...
function action_default ()
{
	...
	$yp = $GLOBALS['yp'];
	$user_id = $_SESSION['user_id'];
	/* 代码注释 by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1 _star 路由前已调用会员信息，去掉重复调用*/
	$rank = get_rank_info();
	/* 	include_once (ROOT_PATH . 'includes/lib_clips.php');
	 if($rank = get_rank_info())
	 {
	...
	/* 代码注释 by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1 _end */
	...
	$num_comment = $db->getOne("SELECT COUNT(*) AS num FROM " . $yp->table('order_goods') . " AS og
	...
	/* 代码增加--cb--推荐分成-- by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1 _star */
	$rn = $rank['rank_name'];
	$recomm = $db->getOne("SELECT is_recomm FROM " . $GLOBALS['yp']->table('user_rank') . " WHERE rank_name= '$rn'");

    $smarty->assign('recomm', $recomm); // 获取当前用户是否是分成用户判断是否显示我的推荐
	/* 代码增加--cb--推荐分成-- by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1 _end */
...
//    $smarty->assign('user_notice', $_CFG['user_notice']);
    $user_notice = $db->getOne(
        'SELECT value FROM ' . $yp->table('shop_config') . " WHERE code = 'user_notice'"
    );
...
function action_getverifycode ()
{
	...
	require_once (ROOT_PATH . 'sms/sms.php');//20170803
	$phone = trim($_GET['mobile']);
	$u_name = trim($_GET['u_name']);
	
	/* 获取验证码请求是否获取过 */
	$sql = "SELECT COUNT(id) FROM " . $yp->table('verifycode') . " WHERE status=1 AND getip='" . real_ip() . "' AND dateline>'" . gmtime() . "'-" . "60";
	...
	$sql = "select count(*) from " . $GLOBALS['yp']->table('users') . " where user_name = '$u_name' and mobile_phone = '$phone'";
	...
	$content = '您的验证码为' . $verifycode . '【qq120029121】';
	/* 发送注册手机短信验证 */
	...
	$db->query("delete from " . $yp->table('verifycode') . " where mobile='$phone'");
...
function action_oath ()
{
	...
	if($c)
	{
		...
		$url = $yp->url() . 'user.php?act=oath_login&type=' . $type . '&callblock=' . urlencode($back_act) . '&open=' . $open;
...
function action_check_username ()
{
    ...
    $sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['yp']->table('users') . " WHERE user_name = '$username' AND user_id <> '$_SESSION[user_id]'";
...
// 代码增加--qq120029121--侧边栏登录 判断登录是否开启验证码
...
function action_ch_email ()
{
	...
	else
	{
		...
		$v_email = $GLOBALS['yp']->url() . 'user.php?act=ck_email&hash=' . $hash;
		...
		{
			$add_time = time();
			$sql = "insert into " . $GLOBALS['yp']->table('email') . "(`email`,`hash`,`add_time`,`user_id`) values('$email','$hash','$add_time',0)";
...
function action_act_edit_profile ()
{
	...
	if($username != $u_name)
	{
		...
		{//寒冰   qq   3094 8  555    2           20160909    取消 用户修改用户名不能为中文限制
...
function action_act_edit_password ()
{
	...
	/* 代码添加_WX_1200-29121_20150729_STAR */
	$confirm_password = isset($_POST['confirm_password']) ? trim($_POST['confirm_password']) : '';
	/* 代码添加_WX_1200-29121_20150729_END */
	...
	if(($user_info && (! empty($code) && md5($user_info['user_id'] . $_CFG['hash_code'] . $user_info['reg_time']) == $code)) || ($_SESSION['user_id'] > 0 && $_SESSION['user_id'] == $user_id && $user->check_user($_SESSION['user_name'], $old_password)))
	{
		...
		), empty($code) ? 0 : 1))
		{
			...
			if($_CFG['sms_change_password'] == 1)
			{
				...
				$sql = "SELECT mobile_phone FROM " . $GLOBALS['yp']->table('users') . " WHERE user_id = '$user_id'";
				$mobile_phone = $GLOBALS['db']->getOne($sql);
				if($mobile_phone)
				{
					//include_once ('send.php');
					require_once (ROOT_PATH . 'sms/sms.php');//20170803
					$templateParam=Array("code"=>date("Y-m-d H:i:s", gmtime()));
					$templateCode=$GLOBALS['_CFG']['sms_change_password_dayu'];
					sendSMS($mobile_phone, $content,'', '',$templateParam , $templateCode);//已支持阿里大于 QQ 120029121
				}
			}
			$user->logout();
...
function action_order_list ()
{
	...
	$order_count['stats'] = $db->getRow('SELECT COUNT(*) AS oCount, IFNULL(SUM(order_amount), 0) AS oAmount' . ' FROM ' . $yp->table('order_info'));
	...
	$record_count = $db->getOne("SELECT COUNT(*) FROM " . $yp->table('order_info') . " WHERE user_id = '$user_id'");

	/* 代码添加_WX_1200-29121_20150909_STAR */
	// 未确认
    ...
    /* 代码添加_WX_1200-29121_20150909_END */
	...
	foreach($orders as $k_kuaidi => $v_kuaidi)
	{
		...
		{
			$kos_order_id = $db->getOne("select order_id from " . $yp->table('kuaidi_order') . " where order_sn='" . $v_kuaidi['invoice_no'] . "'");
			$sql = "select * from " . $yp->table('kuaidi_order_status') . " where order_id='" . $kos_order_id . "'  order by status_id desc";
				...
				if($row_status['status_display'] == 1)
				{
					switch($row_status['status_id'])
					{
						...
						case 2:
							...
							break;
						case 3:
							$postman_id = $db->getOne("select postman_id from " . $yp->table('kuaidi_order') . " where order_sn='" . $orders[$k_kuaidi]['invoice_no'] . "'");
							$postman_info = $db->getRow("select postman_name, mobile from " . $yp->table('postman') . " where postman_id=" . $postman_id);
...
function action_order_detail ()
{
	...
	$sql_invoices = "SELECT invoice_no,shipping_name FROM ".$GLOBALS['yp']->table('delivery_order')." WHERE order_id = ".$order['order_id'];
	$order['invoices'] = $GLOBALS['db']->getAll($sql_invoices);
        
	/* 退换货插件 demo.coolhong.com添加    今-天-优-品-多-商-户-系-统 Q-Q 120 029 121 */
	$shipping_time = $db->getOne("SELECT shipping_time FROM " . $yp->table('order_info') . " WHERE order_id = '$order_id'");
	$now_time = gmtime();
	$not_back = 0;
	...
	if($db->getOne("select shipping_code from " . $yp->table('shipping') . " where shipping_id=" . $order['shipping_id']) == "tc_express")
	{
 
		$order['tc_express'] = 1;
		
		$ko_order_sn = $db->getOne("select invoice_no from " . $yp->table('delivery_order') . " where order_id=" . $order_id);
		if($ko_order_sn)
		{
			$kos_order_id = $db->getOne("select order_id from " . $yp->table('kuaidi_order') . " where order_sn='" . $ko_order_sn . "'");
		}
		
		$sql = "select * from " . $yp->table('kuaidi_order_status') . " where order_id='$kos_order_id'  order by status_id";
			...
			if($row_status['status_display'] == 1)
			{
				switch($row_status['status_id'])
				{
					...
					case 2:
						...
						break;
					case 3:
						$postman_id = $db->getOne("select postman_id from " . $yp->table('kuaidi_order') . " where order_sn='" . $order['invoice_no'] . "'");
						$postman_info = $db->getRow("select postman_name, mobile from " . $yp->table('postman') . " where postman_id=" . $postman_id);
						$shipping_info .= "您的快件正在派送，快递员：" . $postman_info['postman_name'] . "，电话：" . $postman_info['mobile'] . " (" . local_date('Y-m-d H:i:s', $row_status['status_time']) . ")";
						break;
					case 4:
	...
	foreach($goods_list as $key => $value)
	{
		...
		$sql_back = "SELECT bg.*, bo.back_type FROM " . $yp->table('back_goods') . " AS bg " . " LEFT JOIN " . $yp->table('back_order') . " AS bo " . " ON bg.back_id = bo.back_id " . " WHERE bo.order_id = " . $order_id . " AND bg.goods_id = " . $value['goods_id'] . " AND bg.product_id = " . $value['product_id'] . " AND bg.status_back < 6";
...
//	$order['shipping_status'] = $_LANG['ss'][$order['shipping_status']];
	...
	/* 代码增加_start By demo.coolhong.com 今天优品多商户系统 qq 120029121 */
	$smarty->assign('mobile_phone', $GLOBALS['db']->getOne("select mobile_phone from " . $GLOBALS['yp']->table('users') . " where user_id='$_SESSION[user_id]'"));
	foreach($goods_list as $goods_key => $goods_val)
	{
		$sql_goods = "select count(*) from " . $yp->table('back_order') . " where order_id='$order[order_id]' and goods_id='$goods_val[goods_id]'";
		$back_order_count = $db->getOne($sql_goods);
		$goods_list[$goods_key]['back_can'] = $back_order_count ? '0' : '1';
		/* 代码增加_start By demo.coolhong.com 今天优品多商户系统 qq 120029121 _20150804 */
		if($goods_val['extension_code'] == 'virtual_good')
		{
			$virtual_goods_card = $db->getAll("select gc.card_sn,gc.end_date,buy_date from " . $yp->table('virtual_goods_card') . " as gc join (select order_id,order_sn from " . $yp->table('order_info') . " where order_id='$goods_val[order_id]') as oi on gc.order_sn = oi.order_sn");
		...
		/* 代码增加_end By demo.coolhong.com 今天优品多商户系统 qq 120029121 _20150804 */
...
function action_back_order_detail ()
{
	...
	/* 回复留言 demo.coolhong.com添加    今-天-优-品-多-商-户-系-统 Q-Q 120 029 121 */
...
function action_back_order ()
{
	...
	if(!$_REQUEST['order_all'])
	{
		...
		$sql = "select og.goods_id, og.goods_name, og.goods_sn, og.goods_number, og.goods_price, og.product_id, og.goods_attr, o.order_id, o.order_sn, o.user_id, o.shipping_time_end " . " from " . $GLOBALS['yp']->table('order_info') . " AS o left join " . $GLOBALS['yp']->table('order_goods') . " AS og " . " on o.order_id=og.order_id where og.goods_id='$goods_id' and og.order_id='$order_id' and og.product_id='$product_id'";
	...
	{
		$sql_oi = "SELECT * FROM " . $GLOBALS['yp']->table('order_info') . " WHERE order_id = " . $order_id;
	...
	// 收货地址 demo.coolhong.com添加    今-天-优-品-多-商-户-系-统 Q-Q 120 029 121
	include_once (ROOT_PATH . 'includes/lib_transaction.php');
	$order = $db->getRow("SELECT * FROM " . $yp->table('order_info') . " WHERE order_id='$order_id'");
	$smarty->assign('order', $order);
...
function action_back_order_act ()
{
	...
	if($have_tuikuan)
	{
		if ($_POST['order_all'])
		{
			$price_refund = $GLOBALS['db']->getOne("SELECT money_paid FROM " . $GLOBALS['yp']->table('order_info') . " WHERE order_id = " . $order_id);
		}
		else
		{
...
function action_add_huan_goods ()
{
	...
	$goods_name = $db->getOne("select goods_name from " . $yp->table('goods') . " where goods_id='$goods_id' ");
...
function action_delete_collection ()
{
	...
	if($collection_id > 0)
	{
		$db->query('DELETE FROM ' . $yp->table('collect_goods') . " WHERE rec_id='$collection_id' AND user_id ='$user_id'");
	}
...
function action_del_follow ()
{
	...
	if($rec_id)
	{
		$db->query('DELETE FROM ' . $yp->table('supplier_guanzhu') . " WHERE id='$rec_id' AND userid ='$user_id'");
	}
...
function action_booking_list ()
{
	...
	foreach($booking as $key => $value)
	{
		...
		{
			$sql = "SELECT value FROM" . $yp->table('shop_config') . "WHERE code='shop_name'";
			$booking[$key]['supplier_id'] = $value['supplier_id'];
			$booking[$key]['supplier_name'] = $db->getOne($sql);
		}
		else
		{
			$sql = "SELECT supplier_name FROM " . $yp->table('supplier') . "WHERE supplier_id='" . $value['supplier_id'] . "'";
			$booking[$key]['supplier_id'] = $value['supplier_id'];
			$booking[$key]['supplier_name'] = $db->getOne($sql);
		}
	}
...
function action_affirm_received ()
{
	...
	{//20161118   寒冰增加 pc端收货   不给分成问题
	
	 /* $distrib_style = $db->getOne("SELECT value FROM " . $GLOBALS['yp']->table('ypmart_shop_config') .
            			" WHERE `code` = 'distrib_style'");
		if($distrib_style == 0){
			include_once(ROOT_PATH . 'mobile/includes/lib_fencheng.php');
...
function action_act_account ()
{
	...
	$surplus = array(
        ...
        'real_name'    => isset($_POST['real_name'])    ? trim($_POST['real_name'])      : '',//qq 120 029 121 20161123
        'account_type' => isset($_POST['account_type'])    ? trim($_POST['account_type'])      : '',//qq 120 029 121 20161123
        'account'      => isset($_POST['account'])    ? trim($_POST['account'])      : '',//qq 120 029 121 20161123
        'mobile_phone' => isset($_POST['mobile_phone'])    ? trim($_POST['mobile_phone'])      : '',//qq 120 029 121 20161123
		'amount' => $amount
	);
	...
	{
		$surplus['defaultbank'] = isset($_POST['qq_120029121_bank']) ? trim($_POST['qq_120029121_bank']) : '';
	}
...
function action_query_pay_status(){
    ...
    $sql = 'SELECT is_paid FROM ' . $GLOBALS['yp']->table('pay_log') . " WHERE log_id = '$log_id'";
    $is_paid = $GLOBALS['db']->getOne($sql);
	
    $order_id=isset($_REQUEST['order_id'])?$_REQUEST['order_id']:0;
    $sql = 'SELECT pay_status FROM ' . $GLOBALS['yp']->table('order_info') . " WHERE `order_id` =  '$order_id' ";
...
function action_user_bonus ()
{
	...
	{
		$sql = "SELECT COUNT(bonus_type_id) FROM " . $GLOBALS['yp']->table('user_bonus') . " WHERE user_id='$_SESSION[user_id]' AND bonus_type_id = '$type_id'";
		$u_bonus = $GLOBALS['db']->GetOne($sql);
		
		$sql1 = "SELECT user_bonus_max FROM " . $GLOBALS['yp']->table('bonus_type') . " WHERE type_id = '$type_id'";
		...
		{
			$sql = "INSERT INTO " . $GLOBALS['yp']->table('user_bonus') . " (user_id,bonus_type_id)" . "VALUES ('$_SESSION[user_id]', '$type_id')";
...
function action_re_collect ()
{
	...
	else
	{
		...
		$sql = "SELECT goods_id FROM " . $GLOBALS['yp']->table('collect_goods') . " WHERE user_id='$_SESSION[user_id]'";
...
function action_act_edit_payment ()
{
	...
	{
		$defaultbank = $_POST['qq_120029121_bank'];
		$sql = "UPDATE " . $yp->table('order_info') . " SET pay_id='$pay_id', pay_name='$payment_info[pay_name]', pay_fee='$pay_fee', order_amount='$order_amount', defaultbank='$defaultbank'" . " WHERE order_id = '$order_id'";
	}
	else
	{
...
function action_bonus ()
{
	...
	$sql = "SELECT ub.supplier_id, ifnull( ssc.value, '网站自营' ) as suppname
			FROM " . $yp->table('user_bonus') . " AS ub
			LEFT JOIN " . $yp->table('supplier_shop_config') . " AS ssc ON ub.supplier_id = ssc.supplier_id
	...
	$record_count = $db->getOne("SELECT COUNT(*) FROM " . $yp->table('user_bonus') . " WHERE user_id = '$user_id'" . $where_suppid);
...
function action_affiliate ()
{
	...
	if(empty($goodsid))
	{
		...
		if(1){
			...
			$affiliate_intro = nl2br($_LANG['affiliate_intro'][$affiliate['config']['separate_by']]);
		}
	...
	$affiliate['on'] = 1;
	$recomm = 1;
    //file_put_contents('./33$affiliate.txt',var_export($affiliate,true));//prince 
    //file_put_contents('./33$recomm.txt',var_export($recomm,true));  //prince 
...
function action_vc_login_act ()
{
	...
	{
		$sql = "SELECT user_money,mobile_phone FROM " . $GLOBALS['yp']->table('users') . " WHERE user_id = '$user_id'";
...
function action_tg_login_act ()
{
	...
	$_SESSION['takegoods_sn_qq120029121'] = $tg_sn;
	$_SESSION['takegoods_id_qq120029121'] = $tgrow['tg_id'];
...
function action_my_comment ()
{
	...
	// 统计信息
	$num['x'] = $db->getOne("SELECT COUNT(*) AS num FROM " . $yp->table('order_goods') . " AS og 
							LEFT JOIN " . $yp->table('order_info') . " AS o ON og.order_id=o.order_id
							WHERE o.user_id = '$user_id' AND og.is_back = 0 AND og.comment_state = 0 AND o.shipping_time_end > $min_time");
	$num['y'] = $db->getOne("SELECT COUNT(*) AS num FROM " . $yp->table('order_goods') . " AS og 
							LEFT JOIN " . $yp->table('order_info') . " AS o ON og.order_id=o.order_id
...
/* 余额额支付密码_添加_START_demo.coolhong.com */
...
function action_shaidan_send ()
{
	...
	$goods = $db->getRow("SELECT * FROM " . $yp->table('order_goods') . " WHERE rec_id = '$rec_id'");
	
	$min_time = gmtime() - 86400 * $_CFG['comment_youxiaoqi'];
	$pan_1 = $db->getOne("select shipping_time_end from " . $yp->table('order_info') . " where order_id = " . $goods['order_id']);
	$pan_1 = ($pan_1 > $min_time) ? 1 : 0;
	$smarty->assign('pan_1', $pan_1);
	
	$pan_2 = $db->getOne("select rec_id from " . $yp->table('shaidan') . " where rec_id = '$rec_id'");
	$pan_2 = ! empty($pan_2) ? 1 : 0;
	$smarty->assign('pan_2', $pan_2);
	
	$s_user = $db->getOne("select user_id from " . $yp->table('order_info') . " where order_id = " . $goods['order_id']);
...
function action_shaidan_save ()
{
	...
	if(is_array($img_srcs))
	{
		foreach($img_srcs as $i => $src)
		{
			...
			$sql = "INSERT INTO " . $yp->table('shaidan_img') . "(shaidan_id, `desc`, image, thumb)" . "VALUES ('$shaidan_id', '" . $img_names[$i] . "', '$src', '$thumb')";
			$db->query($sql);
		}
	}
...
function action_get_tc_express ()
{
	...
	$kos_order_id = $db->getOne("select order_id from ".$yp->table('kuaidi_order')." where order_sn='".$_REQUEST['expressno']."'");
		...
		{				
			switch ($row_status['status_id'])
			{
				...
				case 2 :
					...
					break;
				case 3 :
					$postman_id = $db->getOne("select postman_id from ".$yp->table('kuaidi_order')." where order_sn='".$_REQUEST['expressno']."'");
					$postman_info = $db->getRow("select postman_name, mobile from ".$yp->table('postman')." where postman_id=".$postman_id);
					$shipping_info .= "您的快件正在派送，快递员：".$postman_info['postman_name']."，电话：".$postman_info['mobile']." (".local_date('Y-m-d H:i:s', $row_status['status_time']).")";
					break;
				case 4 :
...
/* 账户安全_end_demo.coolhong.com */
/* 余额额支付密码_添加_END_demo.coolhong.com */
...
{
    $sql = "SELECT SUM(user_money) FROM " .$GLOBALS['yp']->table('account_log').
...
{
	$sql = "SELECT user_money FROM " . $GLOBALS['yp']->table('users') . "WHERE user_id = '$user_id'";
...
function get_inv_complete_region ($order_id, $inv_type)
{
	if(! empty($order_id))
	{
		...
		{
			$sql = "SELECT concat(IFNULL(c.region_name, ''), '  ', IFNULL(p.region_name, ''), " . "'  ', IFNULL(t.region_name, ''), '  ', IFNULL(d.region_name, '')) AS region " . "FROM " . $GLOBALS['yp']->table('order_info') . " AS o " . "LEFT JOIN " . $GLOBALS['yp']->table('region') . " AS c ON o.country = c.region_id " . "LEFT JOIN " . $GLOBALS['yp']->table('region') . " AS p ON o.province = p.region_id " . "LEFT JOIN " . $GLOBALS['yp']->table('region') . " AS t ON o.city = t.region_id " . "LEFT JOIN " . $GLOBALS['yp']->table('region') . " AS d ON o.district = d.region_id " . "WHERE o.order_id = '$order_id'";
			return $GLOBALS['db']->getOne($sql);
		}
		elseif($inv_type == 'vat_invoice')
		{
			$sql = "SELECT concat(IFNULL(p.region_name, ''), " . "'  ', IFNULL(t.region_name, ''), '  ', IFNULL(d.region_name, '')) AS region " . "FROM " . $GLOBALS['yp']->table('order_info') . " AS o " . "LEFT JOIN " . $GLOBALS['yp']->table('region') . " AS p ON o.inv_consignee_province = p.region_id " . "LEFT JOIN " . $GLOBALS['yp']->table('region') . " AS t ON o.inv_consignee_city = t.region_id " . "LEFT JOIN " . $GLOBALS['yp']->table('region') . " AS d ON o.inv_consignee_district = d.region_id " . "WHERE o.order_id = '$order_id'";
			return $GLOBALS['db']->getOne($sql);
		}
		else
		{
