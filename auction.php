 ...
 * ============================================================================
 * $Author: PRINCE $
 * $Id: auction.php 17217 2017-04-01 06:29:08Z PRINCE $
 */
...
elseif ($_REQUEST['act'] == 'view')
{
        ...
        && $auction['order_count'] == 0)
    {
		...
		$sql_order = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('order_info') . " WHERE extension_id = " . $id . " AND order_status NOT IN (2,3)";
...
elseif ($_REQUEST['act'] == 'bid')
{
    ...
    $db->autoExecute($yp->table('auction_log'), $auction_log, 'INSERT');
	$act_count = $_POST['act_count'] + 1;
	$db->query("UPDATE " . $yp->table('goods_activity') . " SET act_count = " . $act_count . " WHERE act_id = " . $id);

    /* 出价是否等于一口价 */
    if ($bid_price == $auction['end_price'])
    {
        /* 结束拍卖活动 */
        $sql = "UPDATE " . $yp->table('goods_activity') . " SET is_finished = 1 WHERE act_id = '$id' LIMIT 1";
        $db->query($sql);
    }

    /* 跳转到活动详情页 */
    yp_header("Location: auction.php?act=view&id=$id\n");
    exit;
}
...
//    $sql = "SELECT a.*,g.original_img, IFNULL(g.goods_thumb, '') AS goods_thumb " .
    ...
    /* 代码修改 By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121 End */
        "FROM " . $GLOBALS['yp']->table('goods_activity') . " AS a " .
            ...
            'LEFT JOIN ' . $GLOBALS['yp']->table('supplier') . ' s ON s.supplier_id = a.supplier_id ' .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('auction_log') .
				" WHERE act_id = " . $auction['act_id'];
		$auction['bid_user_count'] = $GLOBALS['db']->getOne($sql);
		if ($auction['bid_user_count'] > 0)
		{
			$auction['formated_bid_price'] = $GLOBALS['db']->getOne("select bid_price from " . $GLOBALS['yp']->table('auction_log') . " where act_id = '" . $auction['act_id'] . "' order by bid_price desc limit 0,1");
		}
