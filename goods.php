...
/* 代码增加 by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1 strat */
...
if(!empty($_REQUEST['act']) && $_REQUEST['act'] == 'get_pickup_info')
{
	...
	if($city_info['province_id']>0 )
	{   
	    if($city){//20170118 q-1200-29121
			$where .= ' and province_id=' . $city_info['province_id'] . ' and city_id=' . $city_info['city_id'];
		}else{
			$where .= ' and province_id=' . $city_info['province_id'];
		}//20170118 q-1200-29121
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'price')
{
    ...
    $json   = new JSON;
    $res    = array('err_msg' => '', 'result' => '', 'qty' => 1, 'custom_stye_1' => 0, 'custom_stye_2' => 0);

    //$attr_id    = isset($_REQUEST['attr']) ? explode(',', $_REQUEST['attr']) : array();
    $attr_id    = isset($_REQUEST['attr']) ? $_REQUEST['attr'] : array();
    $number     = (isset($_REQUEST['number'])) ? intval($_REQUEST['number']) : 1;
	
	$custom_stye_1  = $_REQUEST['custom_stye_1'] ? $_REQUEST['custom_stye_1'] : 1;
	$custom_stye_2  = $_REQUEST['custom_stye_2'] ? $_REQUEST['custom_stye_2'] : 1;
    //$number		= 1;
...
// 		$number = 1;
      ...
      $res['result'] = price_format($shop_price * $number * $custom_stye_1 * $custom_stye_2);
		
		//$res['result'] = price_format($shop_price * $number);
        $res['result2'] = price_format($shop_price );//今/天/优/品/多/商/户/系/统-采集 mod on 20160808 by prince  qq 1/2/0/0/2/9/1/2/1
...
/* 判断是否为虚拟商品 */
$is_virtual = $GLOBALS['db']->getOne("select is_virtual from ".$GLOBALS['yp']->table('goods')." where goods_id=$goods_id");
if($is_virtual){
    //直接跳到虚拟商品页面
    yp_header("Location: virtual_group_goods.php?id=".$goods_id."\n");
    exit;
}
...
/* 代码增加_end   By demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1 */
 ...
 WHERE s.supplier_id=".$goods[supplier_id]." AND s.status=1";
        ...
        //收藏个数统计
        $sql = "SELECT COUNT(rec_id) FROM " . $GLOBALS['yp']->table('collect_goods') ." WHERE goods_id = '$goods_id'";
		...
		if($_REQUEST['u']){
		   ...
		   $url = HTTP_TYPE .'://'.str_replace('//', '/', $url);//20171010  寒冰  qq、 30958——--5552   修复商品详情也评价 售后切换刷新
		   if($_REQUEST['u'] != $_SESSION['user_id']){
		   $url = str_replace('u='.$_REQUEST['u'],'u='.$u,$url); 
		   }
		   $smarty->assign('url', $url);
			...
			$url = $_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
			 $url = HTTP_TYPE .'://'.str_replace('//', '/', $url);//20171010  寒冰  qq、 30958——--5552   修复商品详情也评价 售后切换刷新
			$u=$_SESSION['user_id']?$_SESSION['user_id']:0;
			$smarty->assign('url', $url);//寒冰  20161124  qq  309485552
		}// prince 20160704
		...
		$smarty->assign('new_goods',           get_recommend_goods('new',$goods['supplier_id']));     // 最新商品  改 By demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1
		...
		$rank_num['rank_pc'] = ($rank_num['rank_c'] > 0) ? round(($rank_num['rank_c'] / $rank_num['rank_total']) * 100,1) : 0;//寒冰  20161201  修复无评价状态 商品评价条满红状态
...
//代码增加 
    ...
    $_goods_attr['wechat'] =  $_goods_attr['wechat']?explode(',', $_goods_attr['wechat']):'';
	
	
	
    //prince 判断是否入住商品再分析显示联系微信
    if($suppinfo['supplier_name']){
	    $smarty->assign('s_servicewechat', $_goods_attr['wechat']?$_goods_attr['wechat']:'');
	    $smarty->assign('s_wechat_qrcode', $_goods_attr['wechat_qrcode']?$_goods_attr['wechat_qrcode']:'');
	}
	else
	{
	    $smarty->assign('servicewechat',  $GLOBALS['_CFG']['wechat']?explode(',', $GLOBALS['_CFG']['wechat']):'');
	    $smarty->assign('wechat_qrcode',  $GLOBALS['_CFG']['wechat_qrcode']);
	}
            ...
            if ($val['goods_attr'] != '')
            {
                ...
                foreach ($goods_attr_array as $_attr)
                {
                    $goods_attr_price += $GLOBALS['db']->getOne(
                        'SELECT attr_price FROM ' . $GLOBALS['yp']->table('goods_attr') . ' WHERE goods_attr_id = ' . $_attr
                    );
                    $goods_attr[] = $_goods_attr[$_attr];
                }
