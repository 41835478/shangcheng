...
if (isset ( $set_modules ) && $set_modules == TRUE) {
	...
	$modules [$i] ['config'] = array (
			...
			array (
					...
					'type' => 'select',
					'value' => '0' 
			) //20171101 add by prince qq120029121
...
class weixin {
	...
	function get_code($order, $payment) {
		$return_url = HTTP_TYPE.'://' . $_SERVER ['HTTP_HOST'].'/respond.php';
		...
		$selfUrl = HTTP_TYPE.'://' . $_SERVER ['HTTP_HOST'] . $_SERVER ['PHP_SELF'] . '?' . $_SERVER ['QUERY_STRING'];
	...
	function respond() {
		...
		$this->logResult("pclog::respond::xml:".$xml);  // PRINCE 
		$notify->saveData ( $xml );
		...
		if ($notify->checkSign () == TRUE) {
			if ($notify->data ["return_code"] == "FAIL") {
				...
				order_paid ($order_sn, 2,'',$notify->data['out_trade_no'],$notify->data['transaction_id']);
				echo 'success';exit;
			}
		}else{
	...
	function addLog($other = array(), $type = 1) {
		...
		$log ['other'] = $other;
		$log = serialize ( $log );
		return $GLOBALS['db']->query( "INSERT INTO " . $GLOBALS['yp']->table('weixin_paylog') . " (`log`,`type`) VALUES ('$log','$type')" );
	}
    ...
    function logResult($word = '',$var=array()) {
        ...
        file_put_contents($log_path.$output_date."_wx.txt", $output, FILE_APPEND | LOCK_EX);
    }
	...
	function natpayHtml($order) {
		if (! strpos ( $_SERVER ['HTTP_USER_AGENT'], 'MicroMessenger' )) {
			...
			if ($unifiedOrderResult["return_code"] == "FAIL") {
				...
				$log_id = $GLOBALS ['db']->getOne ( "SELECT log_id FROM " . $GLOBALS ['yp']->table ( 'pay_log' ) . "where order_id='{$order ['order_id']}' and is_paid=0 order by log_id desc" );
				if($log_id > 0 && $unifiedOrderResult['err_code'] == 'ORDERPAID'){
					order_paid ( $log_id, 2,'',$unifiedOrderResult['out_trade_no'],$unifiedOrderResult['transaction_id'] );
				}
			...
			return "<img src='erweima_wx.php?text=" . $product_url . "' alt='扫描进行支付'><iframe src='weixin_order_check.php?oid={$order['order_id']}' style='display:none'></iframe>";
		}
	}
}
?>