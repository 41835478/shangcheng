 ...
 * ============================================================================
 * $Author: prince $
 * $Id: qq120029121.php 17217 2017-04-01 06:29:08Z prince $
 */
...
if(isset($set_modules) && $set_modules == TRUE)
{
	...
	$modules[$i]['code'] = 'qq120029121';
	
	/* 被整合的第三方程序的名称 */
	$modules[$i]['name'] = '今天优品多商户系统';
	
	/* 被整合的第三方程序的版本 */
	$modules[$i]['version'] = '2.0';
	
	/* 插件的作者 */
	$modules[$i]['author'] = 'PRINCE';
...
class qq120029121 extends integrate
{
	...
	function check_user ($username, $password = null)
	{
		...
		{
			$post_username = yp_iconv('UTF8', $this->charset, $username);
		}
		else
		{
		...
		{
			$sql = "SELECT user_id, password, salt,ec_salt " . " FROM " . $this->table($this->user_table) . " WHERE supplier_id=0 AND  user_name='$post_username'";
	...
	function edit_user ($cfg)
	{
		...
		if((! empty($cfg['md5password'])) && $this->field_pass != 'NULL')
		{
			...
			$GLOBALS['db']->query("UPDATE ".$GLOBALS['yp']->table('users')." SET `passwd_weixin` =  '$pwd' WHERE user_name = '$user_name'");
		}
		...
		if((! empty($cfg['mobile_phone'])) && $this->field_mobile_phone != 'NULL')
		{
			...
			//file_put_contents(ROOT_PATH."test.txt", "POST:".$sql."\n", FILE_APPEND | LOCK_EX);
