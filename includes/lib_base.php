 ...
 * ============================================================================
 * $Author: PRINCE $
 * $Id: lib_base.php 17217 2017-04-01 06:29:08Z PRINCE $
*/
...
{
    /* 如果邮件编码不是YP_CHARSET，创建字符集转换对象，转换编码 */
    if ($GLOBALS['_CFG']['mail_charset'] != YP_CHARSET)
    {
        $name      = yp_iconv(YP_CHARSET, $GLOBALS['_CFG']['mail_charset'], $name);
        $subject   = yp_iconv(YP_CHARSET, $GLOBALS['_CFG']['mail_charset'], $subject);
        $content   = yp_iconv(YP_CHARSET, $GLOBALS['_CFG']['mail_charset'], $content);
        $shop_name = yp_iconv(YP_CHARSET, $GLOBALS['_CFG']['mail_charset'], $GLOBALS['_CFG']['shop_name']);
    }
 ...
 * @return  void
 **/
function yp_header($string, $replace = true, $http_response_code = 0)
{
...
function yp_iconv($source_lang, $target_lang, $source_string = '')
{
    static $chs = NULL;
...
function json_str_iconv($str)
{
    if (YP_CHARSET != 'utf-8')
    {
        if (is_string($str))
        {
            return addslashes(stripslashes(yp_iconv('utf-8', YP_CHARSET, $str)));
        }
        elseif (is_array($str))
        {
...
function to_utf8_iconv($str)
{
    if (YP_CHARSET != 'utf-8')
    {
        if (is_string($str))
        {
            return yp_iconv(YP_CHARSET, 'utf-8', $str);
        }
        elseif (is_array($str))
        {
            foreach ($str as $key => $value)
...
function write_static_cache($cache_name, $caches)
{
    ...
    $content .= "?>";
    //20170905 prince file_put_contents($cache_file_path, $content, LOCK_EX);
}
