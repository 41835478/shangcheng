 ...
 * ============================================================================
 * $Author: jtypmall $
 * $Id: lib_payment.php 17218 2011-01-24 04:10:41Z jtypmall $
 */
        ...
        if ($pay_log && $pay_log['is_paid'] == 0)
        {
            ...
            if ($pay_log['order_type'] == PAY_ORDER)
            {
               ...
               foreach($orderinfo as $key => $order){
                    ...
                    $mobile_phone = $GLOBALS['db']->getOne("select mobile_phone from ".$GLOBALS['yp']->table('users')." where user_id = ".$order['user_id']);                
	                ...
	                if (!empty($virtual_goods))
	                {
	                    ...
	                    if (virtual_goods_ship($virtual_goods, $msg, $order_sn, true))
	                    {
                            foreach($virtual_goods['virtual_good'] as $key=>$val){
                                ...
                                $card = $GLOBALS['db']->getAll("select card_sn from ".$GLOBALS['yp']->table('virtual_goods_card')." where order_sn='$order_sn'");
                                ...
                                foreach($card as $k=>$v){  
                                    ...
                                    sendSMS($mobile_phone,$content,'', '',$templateParam , $templateCode);//已支持阿里大于 QQ 120029121 暂时使用普通验证码模板  
                            }
