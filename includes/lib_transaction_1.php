...
function get_user_orders_1($user_id, $num = 10, $start = 0,$where='')
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$sql_invoices = "SELECT invoice_no,shipping_name FROM ".$GLOBALS['yp']->table('delivery_order')." WHERE order_id = ".$row['order_id']." AND status = 0";
		...
		$weixiu_time = $GLOBALS['db']->getOne("select value from " . $GLOBALS['yp']->table('shop_config') . " where code='weixiu_time'");
		...
		$back_info_num = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('back_order') .
