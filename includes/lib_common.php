 ...
 * ============================================================================
 * $Author: PRINCE $
 * $Id: lib_common.php 17217 2017-04-01 06:29:08Z PRINCE $
*/
...
{
    /* 获得配置信息 */
    $sql = 'SELECT configure FROM ' . $GLOBALS['yp']->table('shipping_area') . " WHERE shipping_area_id = '$area_id'";
...
function cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show,c.is_xcx_show,  c.show_in_nav, c.grade, c.sort_order, COUNT(s.cat_id) AS has_children ".
                'FROM ' . $GLOBALS['yp']->table('category') . " AS c ".
                "LEFT JOIN " . $GLOBALS['yp']->table('category') . " AS s ON s.parent_id=c.cat_id ".
            ...
            $sql = "SELECT gc.cat_id, COUNT(*) AS goods_num " .
                    " FROM " . $GLOBALS['yp']->table('goods_cat') . " AS gc , " . $GLOBALS['yp']->table('goods') . " AS g " .
...
function cat_list1($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show,c.is_xcx_show, c.show_in_nav, c.grade, c.sort_order, COUNT(s.cat_id) AS has_children,c.is_virtual ".
                'FROM ' . $GLOBALS['yp']->table('category') . " AS c ".
                "LEFT JOIN " . $GLOBALS['yp']->table('category') . " AS s ON s.parent_id=c.cat_id ".
...
function load_config()
{
    ...
    if ($data === false)
    {
        $sql = 'SELECT code, value FROM ' . $GLOBALS['yp']->table('shop_config') . ' WHERE parent_id > 0';
        $res = $GLOBALS['db']->getAll($sql);
        ...
        // 代码修改_start demo.coolhong.com
//         $arr['qq']                   = !empty($arr['qq']) ? $arr['qq'] : '';
//         $arr['ww']                   = !empty($arr['ww']) ? $arr['ww'] : '';
        $arr['qq'] = '';
        $arr['ww'] = '';
        $qq = $GLOBALS['db']->getAll("SELECT cus_no FROM " . $GLOBALS['yp']->table('chat_third_customer') . " WHERE is_master = 1 AND cus_type = 0 AND supplier_id = 0");
        $ww = $GLOBALS['db']->getAll("SELECT cus_no FROM " . $GLOBALS['yp']->table('chat_third_customer') . " WHERE is_master = 1 AND cus_type = 1 AND supplier_id = 0");
        foreach ($qq as $k => $v)
        {
	...
	$arr['fulltext_search']            = isset($arr['fulltext_search']) ? $arr['fulltext_search'] : 0;  
        ...
        if (!isset($GLOBALS['_CFG']['yp_version']))
        {
            /* 如果没有版本号则默认为2.0.5 */
            $GLOBALS['_CFG']['yp_version'] = 'v7.0';
        }
        ...
        {
            $arr['integrate_code'] = 'qq120029121'; // 默认的会员整合插件
        }
...
{
    $sql = 'SELECT brand_id, brand_name FROM ' . $GLOBALS['yp']->table('brand') . ' ORDER BY sort_order';
    $res = $GLOBALS['db']->getAll($sql);
...
function get_promotion_info($goods_id = '',$suppid=-1)
{
    ...
    $sql = 'SELECT act_id, act_name, act_type, start_time, end_time FROM ' . $GLOBALS['yp']->table('goods_activity') . " WHERE is_finished=0 AND start_time <= '$gmtime' AND end_time >= '$gmtime'";
    if(!empty($goods_id))
    {
    ...
    $favourable = array();
    $sql = 'SELECT act_id, act_range,act_type, act_range_ext, act_name,gift, start_time, end_time FROM ' . $GLOBALS['yp']->table('favourable_activity') . " WHERE start_time <= '$gmtime' AND end_time >= '$gmtime' $where_suppid";
    if(!empty($goods_id))
    {
        ...
        foreach ($res as $rows)
        {
			...
			$bb = unserialize($rows['gift']);
				if(is_array($bb))
            	{
					foreach($bb as $k=>$v)
					{
						$bb[$k]['thumb'] = get_image_path($v['id'], $GLOBALS['db']->getOne("SELECT goods_thumb FROM " . $GLOBALS['yp']->table('goods') . "WHERE goods_id = '" . $v['id'] . "'"), true);
					}
				}
    ...
    {
        $sql = "SELECT cat_id, brand_id FROM " . $GLOBALS['yp']->table('goods') .
...
{
    $sql = 'SELECT template_subject, is_html, template_content FROM ' . $GLOBALS['yp']->table('mail_templates') . " WHERE template_code = '$tpl_name'";
...
function order_action($order_sn, $order_status, $shipping_status, $pay_status, $note = '', $username = null, $place = 0)
{
    ...
    $sql = 'INSERT INTO ' . $GLOBALS['yp']->table('order_action') .
            ...
            'FROM ' . $GLOBALS['yp']->table('order_info') . " WHERE order_sn = '$order_sn'";
...
function price_format($price, $change_price = true)
{
    ...
    if ($change_price && defined('YP_ADMIN') === false)
    {
...
function get_virtual_goods($order_id, $shipping = false)
{
    ...
    foreach ($res AS $row)
    {
        $goods_info = $GLOBALS['db']->getRow("select valid_date,supplier_id from ".$GLOBALS['yp']->table('goods')." where goods_id = ".$row['goods_id']);
...
function virtual_card_shipping ($goods, $order_sn, &$msg, $process = 'other')
{
    ...
    /* 检查有没有缺货 */
    $sql = "SELECT COUNT(*) FROM ".$GLOBALS['yp']->table('virtual_card')." WHERE goods_id = '$goods[goods_id]' AND is_saled = 0 ";
    ...
    if ($num < $goods['num'])
    {
     ...
     /* 取出卡片信息 */
     $sql = "SELECT card_id, card_sn, card_password, end_date, crc32 FROM ".$GLOBALS['yp']->table('virtual_card')." WHERE goods_id = '$goods[goods_id]' AND is_saled = 0  LIMIT " . $goods['num'];
    ...
    /* 更新库存 */
    $sql = "UPDATE ".$GLOBALS['yp']->table('goods'). " SET goods_number = goods_number - '$goods[num]' WHERE goods_id = '$goods[goods_id]'";
    $GLOBALS['db']->query($sql);
...
{
/* 代码增加_虚拟团购_START  demo.coolhong.com */
    $add_date = gmtime();
    for($i=0;$i<$goods['num'];$i++){
        ...
        $sql = "INSERT INTO ".$GLOBALS['yp']->table('virtual_goods_card')." (goods_id, card_sn, end_date, add_date, supplier_id, is_verification) ".
                   "VALUES ('$goods[goods_id]', '$coded_card_sn', '$end_date', '$add_date', '$supplier_id', '0')";
        $GLOBALS['db']->query($sql);
    }

/* 代码增加_虚拟团购_END  demo.coolhong.com */

 /* 取出卡片信息 */
     $sql = "SELECT card_id, card_sn, end_date,buy_date,supplier_id,is_verification  FROM ".$GLOBALS['yp']->table('virtual_goods_card')." WHERE goods_id = '$goods[goods_id]' AND is_saled = 0  LIMIT " . $goods['num'];
     $arr = $GLOBALS['db']->getAll($sql);
     ...
     /* 标记已经取出的卡片 */
    $sql = "UPDATE ".$GLOBALS['yp']->table('virtual_goods_card')." SET ".
           "is_saled = 1 ,".
           "order_sn = '$order_sn' ".
           "WHERE " . db_create_in($card_ids, 'card_id');
    if (!$GLOBALS['db']->query($sql, 'SILENT'))
    ...
    if(empty($goods_attr_id)){
        ...
        $sql ="UPDATE ".$GLOBALS['yp']->table('products')."set product_number = product_number - '$goods[num]'  where goods_id = '$goods[goods_id]' and goods_attr='$goods_attr_id'";
    }
    $GLOBALS['db']->query($sql);
    if (true)
    {
...
//        send***SMS($mobile_phone,$content);//废弃
//    }
...
function virtual_card_result($order_sn, $goods)
{
    ...
    /* 获取已经发送的卡片数据 */
    $sql = "SELECT card_sn, card_password, end_date, crc32 FROM ".$GLOBALS['yp']->table('virtual_card')." WHERE goods_id= '$goods[goods_id]' AND order_sn = '$order_sn' ";
...
function virtual_goods_result($order_sn, $goods)
{
    ...
    /* 获取已经发送的卡片数据 */
    $sql = "SELECT card_sn,  end_date, is_verification FROM ".$GLOBALS['yp']->table('virtual_goods_card')." WHERE goods_id= '$goods[goods_id]' AND order_sn = '$order_sn' ";
    $res= $GLOBALS['db']->query($sql);
    $cards = array();
...
function get_snatch_result($id)
{
    $sql = 'SELECT u.user_id, u.user_name, u.email, lg.bid_price, lg.bid_time, count(*) as num' .
            ' FROM ' . $GLOBALS['yp']->table('snatch_log') . ' AS lg '.
            ' LEFT JOIN ' . $GLOBALS['yp']->table('users') . ' AS u ON lg.user_id = u.user_id'.
...
/**
 *  清除手机指定后缀的模板缓存或编译文件
 *  wei2 增加 start by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1
 * @access  public
 * @param  bool       $is_cache  是否清除缓存还是清出编译文件
 * @param  string     $ext       需要删除的文件名，不包含后缀
 *
...
/* wei2 增加 end by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1 */
/**
 * 清除模版编译文件
 *
 * @access  public
...
//                    }
        ...
        case 'goods':
            ...
            else
            {
				...
				{	
					$uri =  'goods-' . $gid;
					/*$pathrow = $GLOBALS['db']->getRow("select c.path_name,c.cat_id from ". $GLOBALS['yp']->table('goods')." AS g left join ". $GLOBALS['yp']->table('category') ." AS c on g.cat_id=c.cat_id where g.goods_id='$gid'" );
        ...
        case 'article':
            ...
            else
            {
				...
				{	
					$uri = 'article-' . $aid;
					/*$pathrow = $GLOBALS['db']->getRow("select c.path_name,c.cat_id from ". $GLOBALS['yp']->table('article')." AS a left join ". $GLOBALS['yp']->table('article_cat') ." AS c on a.cat_id=c.cat_id where a.article_id='$aid'" );
					$pathrow['path_name'] = $pathrow['path_name'] ? $pathrow['path_name'] : ("cat". $pathrow['cat_id']);
    ...
    if (($rewrite == 2) && (strpos(strtolower(YP_CHARSET), 'utf') !== 0))
    {
        $uri = urlencode($uri);
    }
    return $uri;
...
function log_account_change($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER)
{
    ...
    $GLOBALS['db']->autoExecute($GLOBALS['yp']->table('account_log'), $account_log, 'INSERT');
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT c.*, COUNT(s.cat_id) AS has_children, COUNT(a.article_id) AS aricle_num ".
               ' FROM ' . $GLOBALS['yp']->table('article_cat') . " AS c".
               " LEFT JOIN " . $GLOBALS['yp']->table('article_cat') . " AS s ON s.parent_id=c.cat_id".
               " LEFT JOIN " . $GLOBALS['yp']->table('article') . " AS a ON a.cat_id=c.cat_id".
               " GROUP BY c.cat_id ".
...
function get_volume_price_list($goods_id, $price_type = '1')
{
    ...
    $sql = "SELECT `volume_number` , `volume_price`".
           " FROM " .$GLOBALS['yp']->table('volume_price'). "".
...
function get_final_price($goods_id, $goods_num = '1', $is_spec_price = false, $spec = array())
{
    ...
    if(!empty($_REQUEST['pre_sale_id']))
    {
    	...
    	if(!empty($pre_sale)){
            ...
            //采集 mod on 20160808 by PRINCE  qq 1 2 0 0 2 9 1 2 1 调用属性价格
    ...
    if ($is_spec_price)
    {
        if (!empty($spec))
        {
            $spec_price   = spec_price($spec);
            $final_price += $spec_price;
            //采集 mod on 20160808 by PRINCE  qq 1 2 0 0 2 9 1 2 1调用属性价格
            $final_price=get_pro_price($spec,$goods_id,$final_price);
        }
    }
  //$final_price = 1000;
...
function get_pro_price($spec,$goods_id,$org_price=0)
{
	...
	$pro_price = $GLOBALS['db']->getOne("SELECT pro_price FROM " .$GLOBALS['yp']->table('products'). " WHERE goods_attr='".$goods_attr."' and goods_id='".$goods_id."'");

     }   

    $user_rank = isset($GLOBALS['tongbu_user_rank']) ? $GLOBALS['tongbu_user_rank'] : $_SESSION['user_rank'];
    if($user_rank){//寒冰 20180918   增加货品加个按照会员等级折扣
      $rank_discount = $GLOBALS['db']->getOne("SELECT discount FROM " .$GLOBALS['yp']->table('user_rank'). " WHERE rank_id = $user_rank");
      $pro_price = $pro_price * $rank_discount / 100;
    }
	if($pro_price > 0){
		return $pro_price;
    }else{
...
function sort_goods_attr_id_array($goods_attr_id_array, $sort = 'asc')
{
    ...
    $sql = "SELECT a.attr_type, v.attr_value, v.goods_attr_id
            ...
            LEFT JOIN " .$GLOBALS['yp']->table('goods_attr'). " AS v
...
function get_product_supplier($goods_id){
	$sql = "SELECT s.supplier_name 
            FROM " . $GLOBALS['yp']->table('goods') . " AS g
                LEFT JOIN " . $GLOBALS['yp']->table('supplier') . " AS s ON s.supplier_id = g.supplier_id
 ...
 * @return  array
 */
/* 修改 by demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1 增加一个参数 */
function get_package_goods($package_id, $package_attr_id='')
{

	//增加 By demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1
	if ($package_attr_id)
	{
	 ...
	 // 下面SQL语句增加两个字段，注意逗号 , g.goods_thumb, g.shop_price   By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121
    $sql = "SELECT pg.goods_id, g.goods_name, pg.goods_number, p.goods_attr, p.product_number, p.product_id, g.goods_thumb, g.shop_price, ga.attr_price 
            FROM " . $GLOBALS['yp']->table('package_goods') . " AS pg
				...
				LEFT JOIN " . $GLOBALS['yp']->table('goods_attr') . " AS ga ON ga.goods_attr_id = p.goods_attr
            WHERE pg.package_id = '$package_id' $sql_package_attr_id "; //有修改 by demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1 注意最后那个 $sql_package_attr_id
    if ($package_id == 0)
    {
    ...
    if ($good_product_str != '')
    {
        $sql = "SELECT goods_attr_id, attr_value FROM " .$GLOBALS['yp']->table('goods_attr'). " WHERE goods_id IN ($good_product_str)";
...
function get_good_products($goods_id, $conditions = '')
{
    ...
    /* 取货品 */
    $sql = "SELECT * FROM " .$GLOBALS['yp']->table('products'). " WHERE $_goods_id $conditions";
    $result_products = $GLOBALS['db']->getAll($sql);

    /* 取商品属性 */
    $sql = "SELECT goods_attr_id, attr_value FROM " .$GLOBALS['yp']->table('goods_attr'). " WHERE $_goods_id";
    $result_goods_attr = $GLOBALS['db']->getAll($sql);
...
{ 
	if(!$text) return '';
	if(YP_CHARSET != 'gbk') $text = yp_iconv(YP_CHARSET, 'gbk', $text);
	$data = array();
...
function Recordkeyword($word_qq_120029121, $items = 0, $searchengine = 'jtypmall')
{
	if(strlen($word_qq_120029121) < 3 || strlen($word_qq_120029121) > 30 || strpos($word_qq_120029121, ' ') !== false) return;
	$sql_qq_120029121 = "SELECT * FROM " .$GLOBALS['yp']->table('keyword'). " WHERE searchengine='jtypmall' AND word='$word_qq_120029121'";
	$r = $GLOBALS['db']->getRow($sql_qq_120029121);
	if($r)
	{
		$items = intval($items) ;   //demo.coolhong.com
        ...
        $sql_qq_120029121 = "UPDATE " . $GLOBALS['yp']->table('keyword') . " SET " .
                "items = '$items', " .
                "updatetime = '".gmtime()."', " .
				...
				"WHERE w_id = '".$r['w_id']."'";
		$GLOBALS['db']->query($sql_qq_120029121);
		$w_id = $r['w_id'];
	}
	else
	{
		$letter_qq_120029121   = gb2py($word_qq_120029121);
		$sql_qq_120029121 = "INSERT INTO " . $GLOBALS['yp']->table('keyword') . " (searchengine, word, keyword, letter, items, updatetime, total_search, ".
		"month_search, week_search, today_search, status) " .
                " VALUES ('$searchengine', '$word_qq_120029121', '$word_qq_120029121', '$letter_qq_120029121', '$items', '".gmtime()."', '1', '1', '1', '1', '1')";
        $GLOBALS['db']->query($sql_qq_120029121);
		$w_id = $GLOBALS['db']->insert_id();
	}
	if (!empty($w_id))
	{
    	$ip_qq_120029121       = real_ip();
    	$area_qq_120029121     = yp_geoip($ip);
    	$sql_qq_120029121 = 'INSERT INTO ' . $GLOBALS['yp']->table('keyword_area') . ' ( ' .
                'w_id, access_time, ip_address, area) VALUES (' .
                "'$w_id', '".gmtime()."', '$ip_qq_120029121', '$area_qq_120029121')";
    	$GLOBALS['db']->query($sql_qq_120029121);
	}
}
/* 代码增加_END    by   demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121 */
...
function send_sms($supplierinfo='',$content='',$position=1){
	...
	if(array_search(0,$supplier_ids) !== false){//发给平台的短信接收者
		if ($position == 1){
			if ($_CFG['sms_order_placed'] == '1' && $_CFG['sms_shop_mobile'] != '')
		    {
				...
				$templateParam=Array("code"=>$supplierinfo[0]);
				$templateCode=$GLOBALS['_CFG']['sms_order_placed_dayu'];
		    	foreach($phones as $phone){
		    		sendSMS($phone,$content1,'', '',$templateParam , $templateCode);//已支持阿里大于 QQ 120029121
		    	}
		    }
		}elseif ($position == 2){
			if ($_CFG['sms_order_payed'] == '1' && $_CFG['sms_shop_mobile'] != '')
		    {
				...
				$content1 = sprintf($content,$supplierinfo[0],$_CFG['sms_sign']);
				$templateParam=Array("code"=>$supplierinfo[0]);
				$templateCode=$GLOBALS['_CFG']['sms_order_payed_dayu'];
		    	foreach($phones as $phone){
		    		sendSMS($phone,$content1,'', '',$templateParam , $templateCode);//已支持阿里大于 QQ 120029121
		    	}
		    }
	...
	foreach($supplier_ids as $val){
		...
		if ($position == 1){
			if ($info['sms_order_placed'] == '1' && $info['sms_shop_mobile'] != '')
		    {
				...
				$templateParam=Array("code"=>$supplierinfo[$val]);
				$templateCode=$GLOBALS['_CFG']['sms_order_placed_dayu'];
		    	foreach($phones as $phone){
		    		sendSMS($phone,$content1,'', '',$templateParam , $templateCode);//已支持阿里大于 QQ 120029121
		    	}
...
function get_supplier_info($suppid){
	$sql = "select code,value,supplier_id from ". $GLOBALS['yp']->table('supplier_shop_config'). " where supplier_id=".$suppid." and parent_id=8";
 	$result = $GLOBALS['db']->getAll($sql);
...
= 1" );
	...
	$user = $GLOBALS['db']->getRow("select * from " . $GLOBALS['yp']->table('users') . "  where user_id='{$ecuid}'");
...
{
	$sql = 'select region_id from ' . $GLOBALS['yp']->table('region') . " where region_name='$province'";
	$province_id = $GLOBALS['db']->getOne($sql);
	
	if($province_id > 0)
	{
		$sql = 'select region_id from ' . $GLOBALS['yp']->table('region') .
...
function make_html()
{
	...
	$thisurl= $GLOBALS['yp']->get_domain().$_SERVER['REQUEST_URI'];
	$thisroot=$GLOBALS['yp']->url();
...
function get_dir($type, $id)
{
	...
	if ($type == 'category')
	{
		$sql = "select path_name, cat_id from ". $GLOBALS['yp']->table('category') ."  where cat_id= '$id' ";
	...
	{
		$sql = "select path_name, cat_id from ". $GLOBALS['yp']->table('article_cat') ." where cat_id= '$id' ";
		$path_row = $GLOBALS['db'] ->getRow($sql);
		$path_row['path_name'] = $path_row['path_name'] ? $path_row['path_name'] : "cat".$path_row['cat_id'];	
