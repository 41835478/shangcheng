 ...
 * ============================================================================
 * $Author: Prince $
 * $Id: cls_template.php 17063 2010-03-25 06:35:46Z Prince $
 */
...
//                        }
		...
		if(HTTP_TYPE=='https'){
			$out=str_replace("src=\"http://","src=\"https://",$out);
			$out=str_replace("src='http://","src='https://",$out);
			if(strpos($out,'tbcdn.cn') ){
				$out=str_replace("src=\"https://img1.tbcdn.cn","src=\"http://img1.tbcdn.cn",$out);//20170826 解决选择性采集问题
				$out=str_replace("src=\"https://img2.tbcdn.cn","src=\"http://img2.tbcdn.cn",$out);//20170826 解决选择性采集问题
				$out=str_replace("src=\"https://img3.tbcdn.cn","src=\"http://img3.tbcdn.cn",$out);//20170826 解决选择性采集问题
				$out=str_replace("src=\"https://img4.tbcdn.cn","src=\"http://img4.tbcdn.cn",$out);//20170826 解决选择性采集问题
				$out=str_replace("src=\"https://img5.tbcdn.cn","src=\"http://img5.tbcdn.cn",$out);//20170826 解决选择性采集问题
				$out=str_replace("src=\"https://img6.tbcdn.cn","src=\"http://img6.tbcdn.cn",$out);//20170826 解决选择性采集问题
			}
	    }//20170618
        return $out; // 返回html数据
    }
    ...
    function make_compiled($filename)
    {
        ...
        if ($this->force_compile || $filestat['mtime'] > $expires)
        {
            ...
            } ///prince 20170905 mod on 20171024
    ...
    function select($tag)
    {
        ...
        else
        {
			...
			$tag_arr_qq_wx_120029121 = explode(' ', $tag);
			$tag_sel = array_shift($tag_arr_qq_wx_120029121);
            ...
            switch ($tag_sel)
            {
                ...
                case 'insert' :
                    ...
                    $out = "<?php \n" . '$k = ' . preg_replace_callback("/(\'\\$[^,]+)/" , function ($matches){return stripcslashes(trim($matches[1],'\''));}, var_export($t, true)) . ";\n";
                ...
                switch ($s[0])
                {
                    case 'escape':
                        ...
                        elseif ($s[1] == 'u8_url')
                        {
                            if (YP_CHARSET != 'utf-8')
                            {
                                $p = 'urlencode(yp_iconv("' . YP_CHARSET . '", "utf-8",' . $p . '))';
                            }
                            else
                            {
                                $p = 'urlencode(' . $p . ')';
    ...
    function smarty_prefilter_preCompile($source)
    {
        ...
        if ($file_type == '.dwt')
        {
			...
			/*  今天 优品多商户系统 Mod by PRINCE QQ 120 029 121 20160720 */
			if(!empty($GLOBALS['_CFG']['shop_url'])){
				//$source = preg_replace('/<head>/i', "<head>\r\n<base href=\"". $GLOBALS['yp']->url() ."\" />",  $source);
				$source = preg_replace('/<head>/i', "<head>\r\n<base href=\"". $GLOBALS['_CFG']['shop_url'] ."\" />",  $source);

			}
			/*  今天优品多商户系统 Mod by PRINCE QQ 120029121  20160720 */
