 ...
 * ============================================================================
 * $Author: Prince $
 * $Id: lib_goods.php 17113 2010-04-16 03:44:19Z Prince $
*/
    ...
    if (empty($type_goods[$type]))
    {
        ...
        if ($data === false)
        {
            ...
            $sql = "SELECT g.goods_id, g.is_best, g.is_new,g.goods_brief, g.is_hot, g.is_promote, b.brand_name,g.sort_order " .
               ...
               " WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND (g.is_best = 1 OR g.is_new =1 OR g.is_hot = 1) AND g.supplier_id ='$cats'".
...
function get_goods_properties($goods_id)
{
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_group, a.is_linked, a.attr_type, ".
            ...
            ", g.attr_img, g.attr_thumb ". //采集 mod on 20160808 by PRINCE  qq 1 2 0 0 2 9 1 2 1 修改
    ...
    foreach ($res AS $row)
    {
        ...
        else
        {
        	...
        	$sql_1 = "select thumb_url from ". $GLOBALS['yp']->table('goods_gallery'). " where goods_id='$goods_id' and goods_attr_id='$goods_attr_id' and is_attr_image='1' ";
...
function group_buy_info($group_buy_id, $current_num = 0)
{
	...
	//yyy start
$group_buy['goods_desc'] = $GLOBALS['db']->getOne("select goods_desc from ".$GLOBALS['yp']->table('goods')." where goods_id=".$group_buy['goods_id']);
//yyy end
...
{
	/*$sql = "select sum(goods_number) from " . $GLOBALS['yp']->table('order_goods') . " AS g ,".$GLOBALS['yp']->table('order_info') . " AS o WHERE o.pay_status = 2 AND o.order_status >= 1 and o.order_id=g.order_id and g.goods_id = " . $goods_id    ;注意订单状态*/
	
    $sql = "SELECT ghost_count FROM ".$GLOBALS['yp']->table('goods')." where goods_id=".$goods_id;
    $ghost_count = $GLOBALS['db']->getOne($sql);

    $sql = "select sum(goods_number) from ". $GLOBALS['yp']->table('order_goods') ." where goods_id=".$goods_id;
...
function get_goods_by_txm($info){
	...
	$sql = "select bc.*,g.* from ".$yp->table('bar_code')." as bc left join ".$yp->table('goods')." as g on bc.goods_id=g.goods_id where bc.bar_code in(".implode(',',$info).")";
