...
function order_info($order_id, $order_sn = '')
{
    ...
    if ($order_id > 0)
    {
       $sql = "SELECT o.*, p.shop_name, p.address as zt_address, p.phone, p.contact, " . $total_fee . " FROM " . $GLOBALS['yp']->table('order_info') .
               " as o left join " . $GLOBALS['yp']->table('pickup_point') . " as p on o.pickup_point = p.id WHERE o.order_id = '$order_id'";
    }
    else
    {
	   $sql = "SELECT o.*, p.shop_name, p.address as zt_address, p.phone, p.contact, " . $total_fee . " FROM " . $GLOBALS['yp']->table('order_info') .
               " as o left join " . $GLOBALS['yp']->table('pickup_point')." as p on o.pickup_point = p.id WHERE o.order_sn = '$order_sn'";
    }
    ...
    if ($order)
    {
		...
		$sql_invoices = "SELECT invoice_no,shipping_name FROM ".$GLOBALS['yp']->table('delivery_order')." WHERE order_id = ".$order['order_id']." AND status = 0";
		$order['invoices'] = $GLOBALS['db']->getAll($sql_invoices);
		
        $sql = "select region_id, region_name from " . $GLOBALS['yp']->table('region') . " where region_id in (" . $order['country'] . "," . $order['province'] . "," . $order['city'] . "," . $order['district'] . ")";
...
function order_goods($order_id)
{
	...
	$sql = "SELECT rec_id, og.goods_id, og.product_id,og.goods_name, g.goods_thumb, og.goods_sn, og.market_price,og.exclusive, og.goods_number, " .
            ...
            "FROM " . $GLOBALS['yp']->table('order_goods') . "as og right join" . $GLOBALS['yp']->table('goods') .
			"as g on og.goods_id = g.goods_id" .
            " WHERE order_id = '$order_id'";// 修改 by demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1 增加 package_attr_id 字段
...
function order_amount($order_id, $include_gift = true)
{
   ...
   /* $sql = "SELECT SUM(goods_price * goods_number) " .
	...
	$sql = "SELECT goods_price,goods_number,exclusive FROM ". $GLOBALS['yp']->table('order_goods') ." WHERE order_id = '$order_id'";
    if (!$include_gift)
    {
        $sql .= " AND is_gift = 0";
    }
...
function order_fee($order, $goods, $consignee)
{
	...
	if (count($order['shipping_pay']) > 0 && $total['real_goods_count'] > 0){
		...
		foreach ($goods AS $val)
		{
			...
			{
				$sql_supp = "select g.supplier_id, IF(g.supplier_id='0', '本网站', s.supplier_name) AS supplier_name2 from ".$GLOBALS['yp']->table('goods_activity').
				" AS g left join ".$GLOBALS['yp']->table('supplier')." AS s on g.supplier_id=s.supplier_id where g.act_id='". $val['goods_id'] ."' ";
			}
			else
			{
           ...
           if ($val['extension_code'] == 'package_buy')
		   {
			   $sql = 'SELECT count(*) FROM ' . $GLOBALS['yp']->table('cart') . " AS c left join ". $GLOBALS['yp']->table('goods_activity') ." AS g on c.goods_id=g.act_id WHERE g.supplier_id = '". $row_supp['supplier_id'] ."' AND $sql_where AND c.extension_code != 'package_buy' AND c.is_shipping = 0 ".$sql_plus;  //jx
		   }
		   else
		   {
...
$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";
    ...
    foreach ($arr as $key => $value)
    {
        ...
        {
            $arr[$key]['package_goods_list'] = get_package_goods($value['goods_id'], $value['package_attr_id']); //修改 by demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1
        }
    }
    return $arr;
 ...
 * @return  boolean
 */
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0,$custom_stye_1=1,$custom_stye_2=1)// mod prince 20160813
{
    $GLOBALS['err']->clean();
    $_parent_id = $parent;
    ...
    $parent = array(
		...
		'cost_price'	=> $goods['cost_price'],//分销金额
		'split_money'	=> get_split_money_by_id($goods_id,$num,$spec), //分销金额
        'is_real'       => $goods['is_real'],
        'extension_code'=> $goods['extension_code'],
        'is_gift'       => 0,
		'exclusive'     => $goods['exclusive'],//手机专享价格  app  jx
        'is_shipping'   => $goods['is_shipping'],
		'add_time'   => gmtime(),   //代码增加   By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121
    ...
    if ($num > 0)
    {
        ...
        $sql = "SELECT goods_number FROM " .$GLOBALS['yp']->table('cart').
                ...
                " AND parent_id = 0 AND goods_attr = '" .get_goods_attr_info($spec). "' " .
                " AND extension_code <> 'package_buy' " .
				" AND user_id= '".$_SESSION['user_id']."'". //代码增加  By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121
        ...
        if($row) //如果购物车已经有此物品，则更新
        {
            ...
            {
                $goods_price = get_final_price($goods_id, $num, true, $spec);
				if($custom_stye_1 && $custom_stye_2){// add by prince 20170309
					$goods_price = $goods_price*$custom_stye_1*$custom_stye_2;// add by prince 20160813
				}// add by prince 120029121  20170309
        ...
        else //购物车没有此物品，则插入
        {
			...
			if($custom_stye_1 && $custom_stye_2){// add by prince 20161227
				$goods_price = $goods_price*$custom_stye_1*$custom_stye_2;// add by prince 20160813
			}// add by prince 20161227
...
/* 重写方法_start  By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121 */
/**
 * 清空购物车
...
function user_info($user_id)
{
	...
	$sql = "SELECT u.*,ifnull(s.supplier_name,'') as supplier_name FROM " . $GLOBALS['yp']->table('users') .
            " as u left join ".$GLOBALS['yp']->table('supplier')." as s on u.user_id=s.user_id WHERE u.user_id = '$user_id'";
...
/* 重构方法_start  By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121 */
/**
 * 获得购物车中的商品
 *
 * @access  public
...
function get_cart_goods($other='')
{
    ...
    /* 代码增加_start    添加    今-天-优-品-多-商-户-系-统 Q-Q 120 029 121 */
	$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";
...
function get_consignee($user_id)
{
    ...
    {		
			...
			if(!isset($_SESSION['flow_consignee']['province']))
			{
				...
				{
					/* 取默认地址 */
					$sql = "SELECT * FROM ".$GLOBALS['yp']->table('user_address')." WHERE user_id='$user_id'";
...
function send_order_bonus($order_id,$supplier_id)
{
    ...
    if ($bonus_list)
    {
        ...
        {	
            ...
            /* 修改用户红包 */
            $sql = "INSERT INTO " . $GLOBALS['yp']->table('user_bonus') . " (bonus_type_id, user_id, supplier_id) " .
...
function compute_discount_amount($suppid=-1)
{
	...
	if($suppid>-1)
	{
		$sql = "SELECT c.goods_id, c.goods_price * c.goods_number AS subtotal, g.cat_id, g.brand_id, " .
			...
			" LEFT JOIN " . $GLOBALS['yp']->table('goods') . " AS ga " .
 ...
 * @return  boolean
 */
/* 代码修改_start By demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1  提示：增加了两个参数 */
...
//    }
    ...
    $parent = array(
        ...
        'market_price'  => $package_pricea[0] ? $package_pricea[0] :  $package['market_package'], //修改 by demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1
        'goods_price'   => $package_pricea[1] ? $package_pricea[1] :  $package['package_price'], //修改 by demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1
		'package_attr_id' =>$package_attr_id, //增加 by demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1
        'goods_number'  => $num,
        'goods_attr'    => '',
        'goods_attr_id' => '',
    ...
    if ($num > 0)
    {
        ...
        $sql = "SELECT goods_number FROM " .$GLOBALS['yp']->table('cart').
                ...
                " AND package_attr_id = '$package_attr_id'  AND rec_type = '" . CART_GENERAL_GOODS . "'";   //修改 by demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1 增加一条件
...
function cart_weight_price2($type = CART_GENERAL_GOODS, $supplier_id)
{
	...
	$sql = 'SELECT goods_id, goods_number, goods_price FROM ' . $GLOBALS['yp']->table('cart') . " WHERE extension_code = 'package_buy' AND ".$sql_where." ".$sql_plus;
    $row = $GLOBALS['db']->getAll($sql);
...
function is_cansel($goods_id, $product_id, $package_buy)
{
	...
	$sql = "select is_on_sale, is_delete, goods_number from ". $GLOBALS['yp']->table('goods') ." where goods_id='$goods_id' ";
	$row = $GLOBALS['db']->getRow($sql);
...
function put_virtual_cart($order_id){
    ...
    $order = $GLOBALS['db']->getRow("select * from ".$GLOBALS['yp']->table('order_info')." where order_id=$order_id");
    $goods = $GLOBALS['db']->getRow("select o.goods_id, o.goods_number,g.valid_date from ".$GLOBALS['yp']->table('order_goods')." as o left join ".$GLOBALS['yp']->table('goods')." as g on o.goods_id = g.goods_id where order_id=$order_id");
    for($i=0;$i<$goods['goods_number'];$i++){
        $coded_card_sn   = (rand(1000,9999).$i.gmtime());
        $add_date = gmtime();
        $end_date = $goods['valid_date'];
        $supplier_id = $order['supplier_id'];
        $sql = "INSERT INTO ".$GLOBALS['yp']->table('virtual_goods_card')." (goods_id, card_sn, end_date, add_date, is_saled, order_sn, supplier_id, is_verification) ".
...
//分成后，推送到各个上级分销商微信
function push_fencheng_user_msg($ecuid,$order_sn,$split_money){
	$type = 1;
 ...
 * 获取首页订单信息汇总
 * @param void
 * @retrun array $result 返回结果结合
*/
function get_index_total()
{
    global $db;
    global $yp;
    $day_time = local_strtotime(local_date("Y-m-d 00:00:00"));
    $day_date = local_date("Y-m-d H:i:s",$day_time);
    $result = array('day' => 0, 'day3' => 0, 'month' => 0, 'comment' => 0, 'goods_number' => 0);
    $day = $db->getOne("select count(*) from ".$yp->table('order_info')." where add_time >= $day_time and order_status in(1,4,5,6)");
    $result['day'] = $day;
    
    $day3_time  = local_strtotime("$day_date -3 day");
    $day3 = $db->getOne("select count(*) from ".$yp->table('order_info')." where add_time >= $day3_time and order_status in(1,4,5,6)");
    $result['day3'] = $day3;
    
    $month_time  = local_strtotime("$day_date -1 month");
    $month = $db->getOne("select count(*) from ".$yp->table('order_info')." where add_time >= $month_time and order_status in(1,4,5,6)");
    $result['month'] = $month;
    
    $comment = $db->getOne("select count(*) from ".$yp->table('comment')." where status=1 and parent_id=0");
    $result['comment'] = $comment;

    $result['goods_number'] = $db->getOne("select count(*) from ".$yp->table('goods')."");
    return $result;
}
 ...
 * @retrun array $result 返回结果结合
*/
function get_index_order_list($limit = 20)
{
    global $db;
    global $yp;
    
    $list = $db->getAll("select order_sn,pay_name,consignee,add_time,pay_status,shipping_status,
             (goods_amount + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee + tax - discount) AS total_fee
             from ".$yp->table('order_info')." where  order_status in(0,1,4,5,6) order by add_time desc limit $limit");
    foreach ($list as $k => $row) {
        $list[$k]['total_fee'] = price_format($row['total_fee'], false);
        $list[$k]['add_time'] = local_date("m-d H:i:s", $row['add_time']);
        $list[$k]['pay_status'] = $GLOBALS['_LANG']['ps'][$row['pay_status']];
        $list[$k]['shipping_status'] = $GLOBALS['_LANG']['ss'][$row['shipping_status']];
        $list[$k]['consignee'] = sub_str($row['consignee'],1,false)."**";
    }
    return $list;
}

//获取商品分成金额
function get_split_money_by_id($goods_id,$num,$spec)/*分成金额*/
{
