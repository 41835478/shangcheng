...
function get_user_orders($user_id, $num = 10, $start = 0)
{
    ...
    $sql = "SELECT order_id, order_sn, order_status, shipping_status, pay_status, add_time, shipping_time," .
			" supplier_id, rebate_id, parent_order_id,	  ". //代码增加  By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121
    ...
    else
    {
        ...
        if ($GLOBALS['db']->query($sql))
        {
			if(get_cod_id($order_id)){
				...
				$GLOBALS['db']->query("UPDATE " . $GLOBALS['yp']->table('order_info') . " SET rebate_ispay = 2 WHERE order_id = ".$order_id);
			}
			$sql_2 = "SELECT back_id FROM " . $GLOBALS['yp']->table('back_order') . " WHERE order_id = '$order_id' AND status_back < 6 AND status_back != 3";
			$re_2  = $GLOBALS['db']->getCol($sql_2);
			if (count($re_2) > 0)
			{
				$sql_3 = "UPDATE " . $GLOBALS['yp']->table('back_goods') . " SET status_back = 8 WHERE back_id in (" . implode(',', $re_2) . ")";
				$GLOBALS['db']->query($sql_3);
			}
			
			$sql_4 = "UPDATE " . $GLOBALS['yp']->table('back_order') . " SET status_back = 8 WHERE order_id = '$order_id' AND status_back < 6 AND status_back != 3";
...
function get_order_detail($order_id, $user_id = 0)
{
        ...
        $order['order_status'] == OS_CONFIRMED))
    {
        ...
        else
        {
			...
			{		
				$payment['qq_wx_120029121_alipay_bank'] = $order['defaultbank'];
			}
   ...
   /* $sql = "SELECT u.bonus_sn,u.supplier_id, u.order_id, b.type_name, b.type_money, b.min_goods_amount, b.use_start_date, b.use_end_date ".
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		else
		{
			...
			$sql = "SELECT * FROM ".$GLOBALS['yp']->table('supplier_shop_config')."WHERE supplier_id = '$supplierid' AND code = 'shop_name'";
