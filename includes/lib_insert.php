 ...
 * ============================================================================
 * $Author: Prince $
 * $Id: lib_insert.php 17063 2010-03-25 06:35:46Z Prince $
*/
...
$sql = "select * from ".$GLOBALS['yp']->table('goods')." where cat_id = ".$catid." and is_best = 1 AND is_delete = 0";
...
function insert_history_list()
{
    ...
    {
        $where = db_create_in($_COOKIE['YP']['history'], 'goods_id');
        $sql   = 'SELECT goods_id, goods_name, goods_thumb, shop_price,promote_price,market_price, goods_type FROM ' . $GLOBALS['yp']->table('goods') .
                " WHERE $where AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0";
        $query = $GLOBALS['db']->query($sql);
        $res = array();
		$str .= '<ul>';
...
{
	$sql = 'SELECT * FROM '. $GLOBALS['yp']->table('chat_third_customer') . ' WHERE supplier_id = 0 ORDER BY cus_type, cus_name';
	$row = $GLOBALS['db']->GetAll($sql);
	$arr = array();
	foreach($row AS $k=>$v)
	{
	...
	$GLOBALS['smarty']->assign('wechat', $GLOBALS['_CFG']['wechat']?explode(',', $GLOBALS['_CFG']['wechat']):'');
	$GLOBALS['smarty']->assign('wechat_qrcode',$GLOBALS['_CFG']['wechat_qrcode']? $GLOBALS['_CFG']['wechat_qrcode']:'');
...
{ $arr['num'] = intval($arr['num']); $arr['id'] = intval($arr['id']);
    ...
    if (!empty($arr['num']) && $arr['num'] != 1)
    {
        $sql  = 'SELECT a.ad_id, a.position_id, a.media_type, a.ad_link, a.ad_code, a.ad_name, p.ad_width, ' .
                ...
                'FROM ' . $GLOBALS['yp']->table('ad') . ' AS a '.
                'LEFT JOIN ' . $GLOBALS['yp']->table('ad_position') . ' AS p ON a.position_id = p.position_id ' .
...
function insert_rs_member_info()
{
    $need_cache = $GLOBALS['smarty']->caching;
    $GLOBALS['smarty']->caching = false;
...
function insert_supplier_list($arr){
	...
	$tag_sql = "select * from ".$GLOBALS['yp']->table('supplier_tag')." where is_groom=1 order by sort_order";
	...
	if(empty($tag_ret)){//没有标签
		$sql = "SELECT ssc.value,ssc.supplier_id,ssc.code FROM ".$GLOBALS['yp']->table('supplier_street')." as ss,".$GLOBALS['yp']->table('supplier_shop_config')." as ssc WHERE ss.`status` = 1 AND ssc.supplier_id = ss.supplier_id AND code in('shop_name','shop_logo') order by ss.sort_order limit 48";
		...
		$sql = " select ssc.value,ssc.supplier_id,ssc.code from ".$GLOBALS['yp']->table('supplier_tag_map')." as stm,".
			   $GLOBALS['yp']->table('supplier_shop_config')." as ssc,".
			   $GLOBALS['yp']->table('supplier_street')." as ss ".
...
function insert_goods_collect($arr)
 {
         $sql='select count(user_id) as ids '.'from '. $GLOBALS['yp']->table('collect_goods')."as co "."where co.goods_id=".$arr['goods_id'];
         $row=$GLOBALS['db']->GetRow($sql);
         if($row)
         {
                 $ids = intval($row['ids']);
