    ...
    if ($row = $GLOBALS['db']->getRow($sql))
    {
        ...
        if ($row['user_rank'] == 0)
        {
            ...
            if ($row = $GLOBALS['db']->getRow($sql))
            {
                ...
                $sql="SELECT rank_name from ".$GLOBALS['yp']->table('user_rank')."where rank_id= ".$row['rank_id'];
...
function recalculate_price()
{
    ...
    foreach ($res AS $row)
    {
        ...
        $goods_sql = "UPDATE " .$GLOBALS['yp']->table('cart'). " SET market_price = '".$row['market_price']."', goods_price = '$goods_price' ".
...
function assign_comment($id, $type, $page = 1,$comment_level=0)
{
    ...
    $sql = 'SELECT c.*, u.headimg FROM ' . $GLOBALS['yp']->table('comment') . " AS c " .
			" LEFT JOIN " . $GLOBALS['yp']->table('users') . " AS u ON c.user_name = u.user_name " .
...
{
	$arr = "";
	$cat_all = $GLOBALS['db']->getAll("SELECT cat_id, cat_name FROM " . $GLOBALS['yp']->table('category') . " WHERE parent_id = " . $p);
...
/**
 * jtypmall 实现其他页面(商品详情页、商品分类页、团购页、优惠活动页、积分商城)
...
function pc_to_mobile()
{
    ...
    if($is_mobile){
         ...
         */
        $mobile_url = $GLOBALS['yp']->url().'mobile';
        ...
        if ($filename != 'index')
        {
               ...
               /* 分类页 */
                elseif ('category' == $filename)
                {
                    $Loaction = $mobile_url .'/category.php?&id='.$_GET['id'];
                }
				 /* 分类页 */
                elseif ('brand' == $filename)
                {
                    $Loaction = $mobile_url .'/brand.php?&id='.$_GET['id'];
                }
            /* 跳转到手机对应页面 */
            if (!empty($Loaction)){
