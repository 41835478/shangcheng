...
function register_hash ($operation, $key)
{
	...
	{
		$user_id = intval($key);
		$sql = "SELECT reg_time " . " FROM " . $GLOBALS['yp']->table('users') . " WHERE user_id = '$user_id' LIMIT 1";
...
{
	$res = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('admin_user') . " WHERE user_name = '$adminname'");
	return $res;
}
