...
function get_my_comments($goods_id, $type = 0, $page = 1, $c_tag='')
{
	...
	if ($type != 4)
	{
		...
		$sql = "SELECT c.*, u.headimg, s.shaidan_id, s.status AS shaidan_status FROM ".$GLOBALS['yp']->table('comment')." AS c 
	...
	else
	{
		...
		$sql = "SELECT s.*, u.user_name, u.headimg, c.comment_tag, c.comment_rank, c.comment_id FROM ".$GLOBALS['yp']->table('shaidan')." AS s 
