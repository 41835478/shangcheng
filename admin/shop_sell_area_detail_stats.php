 ...
 * ============================================================================
 * $Author: jtypmall $
 * $Id: shop_sell_area_detail_stats.php 17217 2015-10-30 13:06:08Z jtypmall $
 */
...
function get_result_list ($end_date, $area_type, $province, $city, $district)
{
    ...
    if($result === false)
    {
        ...
        $sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['yp']->table('supplier') . ' s ' . $where;
        // 按省统计
        ...
        $sql = "SELECT s.supplier_name, sa.user_name, r.rank_name, s.add_time FROM "
            . $GLOBALS['yp']->table('supplier') . ' s LEFT JOIN ' . $GLOBALS['yp']->table('supplier_admin_user')
            . ' sa ON s.supplier_id = sa.supplier_id LEFT JOIN ' . $GLOBALS['yp']->table('supplier_rank')
