...
elseif($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if($is_insert)
    {
        $sql = 'INSERT INTO ' . $yp->table('tag') . '(tag_id, goods_id, tag_words)' .
...
elseif ($_REQUEST['act'] == 'batch_drop')
{
    ...
    if (isset($_POST['checkboxes']))
    {
        ...
        {
            $sql = "DELETE FROM " .$yp->table('tag'). " WHERE tag_id='$id'";
            $db->query($sql);
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 鑾峰彇鍒犻櫎鐨勬爣绛剧殑鍚嶇О */
    $tag_name = $db->getOne("SELECT tag_words FROM " .$yp->table('tag'). " WHERE tag_id = '$id'");
...
function tag_is_only($name, $tag_id, $goods_id = '')
{
    ...
    {
        $db = $GLOBALS['db'];
        $sql = 'SELECT goods_id FROM ' . $GLOBALS['yp']->table('tag') . " WHERE tag_id = '$tag_id'";
        $row = $GLOBALS['db']->getRow($sql);
        $goods_id = $row['goods_id'];
    }

    $sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['yp']->table('tag') . " WHERE tag_words = '$name'" .
...
{
    $db = $GLOBALS['db'];
    $sql = 'UPDATE ' . $GLOBALS['yp']->table('tag') . " SET tag_words = '$name'";
    if(!empty($goods_id))
    {
...
function get_tag_list()
{
    ...
    $sql = "SELECT t.tag_id, u.user_name, t.goods_id, g.goods_name, t.tag_words ".
            "FROM " .$GLOBALS['yp']->table('tag'). " AS t ".
            "LEFT JOIN " .$GLOBALS['yp']->table('users'). " AS u ON u.user_id=t.user_id ".
            "LEFT JOIN " .$GLOBALS['yp']->table('goods'). " AS g ON g.goods_id=t.goods_id ".
...
{
    $sql = 'SELECT t.tag_id, t.tag_words, t.goods_id, g.goods_name FROM ' . $GLOBALS['yp']->table('tag') . ' AS t' .
           ' LEFT JOIN ' . $GLOBALS['yp']->table('goods') . ' AS g ON t.goods_id=g.goods_id' .
           " WHERE tag_id = '$tag_id'";
    $row = $GLOBALS['db']->getRow($sql);
