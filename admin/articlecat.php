...
$exc = new exchange($yp->table("article_cat"), $db, 'cat_id', 'cat_name');
/* act操作项的初始化 */
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    {
        $sql = "SELECT cat_type FROM " . $yp->table('article_cat') . " WHERE cat_id = '$_POST[parent_id]'";
    ...
    $sql = "INSERT INTO ".$yp->table('article_cat')."(cat_name, cat_type, cat_desc,keywords, parent_id, sort_order, show_in_nav, path_name)
           VALUES ('$_POST[cat_name]', '$cat_type',  '$_POST[cat_desc]','$_POST[keywords]', '$_POST[parent_id]', '$_POST[sort_order]', '$_POST[show_in_nav]', '$_POST[path_name]')";//代码修改 By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121 增加新字段 path_name 和  $_POST[path_name]
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT cat_id, cat_name, cat_type, cat_desc, show_in_nav, keywords, parent_id,sort_order, path_name FROM ".
           $yp->table('article_cat'). " WHERE cat_id='$_REQUEST[id]'";//代码修改 By demo.coolhong.com 今天优品多商户系统 qq 120029121 增加一个path_name 字段
    $cat = $db->GetRow($sql);
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    $row = $db->getRow("SELECT cat_type, parent_id FROM " . $yp->table('article_cat') . " WHERE cat_id='$_POST[id]'");
    ...
    $dat = $db->getOne("SELECT cat_name, show_in_nav FROM ". $yp->table('article_cat') . " WHERE cat_id = '" . $_POST['id'] . "'");
    if ($exc->edit("cat_name = '$_POST[cat_name]', cat_desc ='$_POST[cat_desc]', keywords='$_POST[keywords]',parent_id = '$_POST[parent_id]', cat_type='$cat_type', sort_order='$_POST[sort_order]', show_in_nav = '$_POST[show_in_nav]'",  $_POST['id']))
    {
        ...
        {
            //如果分类名称发生了改变
            $sql = "UPDATE " . $yp->table('nav') . " SET name = '" . $_POST['cat_name'] . "' WHERE ctype = 'a' AND cid = '" . $_POST['id'] . "' AND type = 'middle'";
            $db->query($sql);
        }
        if($_POST['show_in_nav'] != $dat['show_in_nav'])
        {
            ...
            {
                //显示
                $nid = $db->getOne("SELECT id FROM ". $yp->table('nav') . " WHERE ctype = 'a' AND cid = '" . $_POST['id'] . "' AND type = 'middle'");
                if(empty($nid))
                {
                    $vieworder = $db->getOne("SELECT max(vieworder) FROM ". $yp->table('nav') . " WHERE type = 'middle'");
                    $vieworder += 2;
                ...
                {
                    $sql = "UPDATE " . $yp->table('nav') . " SET ifshow = 1 WHERE ctype = 'a' AND cid = '" . $_POST['id'] . "' AND type = 'middle'";
                }
                $db->query($sql);
            }
            else
            {
                //去除
                $db->query("UPDATE " . $yp->table('nav') . " SET ifshow = 0 WHERE ctype = 'a' AND cid = '" . $_POST['id'] . "' AND type = 'middle'");
            }
        }
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $sql = "SELECT cat_type FROM " . $yp->table('article_cat') . " WHERE cat_id = '$id'";
    ...
    $sql = "SELECT COUNT(*) FROM " . $yp->table('article_cat') . " WHERE parent_id = '$id'";
    if ($db->getOne($sql) > 0)
    {
        /* 还有子分类，不能删除 */
    ...
    /* 非空的分类不允许删除 */
    $sql = "SELECT COUNT(*) FROM ".$yp->table('article')." WHERE cat_id = '$id'";
    if ($db->getOne($sql) > 0)
    {
    ...
    {
        $exc->drop($id);
        $db->query("DELETE FROM " . $yp->table('nav') . "WHERE  ctype = 'a' AND cid = '$id' AND type = 'middle'");
        clear_cache_files();
...
function cat_update($cat_id, $args)
{
    ...
    return $GLOBALS['db']->autoExecute($GLOBALS['yp']->table('article_cat'), $args, 'update', "cat_id='$cat_id'");
}
?>
