...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$yp->table('brand')."(brand_name, site_url, brand_desc, brand_logo, is_show, sort_order) ".
...
elseif ($_REQUEST['act'] == 'export')
{
    ...
    if (isset($_REQUEST['keyword']) && !empty($_REQUEST['keyword']))
    {
        $sql = "SELECT brand_name, brand_logo, site_url, brand_desc, sort_order, is_show FROM "
            .$GLOBALS['yp']->table('brand')." WHERE brand_name LIKE '%{$_REQUEST['keyword']}%' ORDER BY sort_order ASC";
    }
    // 不存在搜索关键字时搜索全部
    else
    {
        $sql = "SELECT brand_name, brand_logo, site_url, brand_desc, sort_order, is_show FROM "
            .$GLOBALS['yp']->table('brand')." ORDER BY sort_order ASC";
    }
    // 取得导出数据
