...
if ($_REQUEST['act'] == 'setup')
{
    ...
    $sql = "SELECT region, library, sort_order, id, number, type, ext_info FROM ".$yp->table('template') ." ".
    ...
    if (isset($_POST['regions']['cat_goods']))
    {
        foreach ($_POST['regions']['cat_goods'] AS $key => $val)
        {
            ...
            if ($_POST['categories']['cat_goods'][$key] != '' && intval($_POST['categories']['cat_goods'][$key]) > 0)
            {
            	...
            	/* insert */
            	$db->autoExecute($yp->table('template'), $template, 'INSERT');
...
if ($_REQUEST['act'] == 'install')
{
    ...
    $sql = "UPDATE " .$GLOBALS['yp']->table('shop_config'). " SET value = '$tpl_name' WHERE code = 'template'";
    $step_one = $db->query($sql, 'SILENT');
    $sql = "UPDATE " .$GLOBALS['yp']->table('shop_config'). " SET value = '$tpl_fg' WHERE code = 'stylename'";
