 ...
 * ============================================================================
 * $Author: PRINCE $
 * $Id: users.php 17217 2017-04-01 06:29:08Z PRINCE $
 */
...
function action_list ()
{
	...
	$sql = "SELECT rank_id, rank_name, min_points FROM " . $yp->table('user_rank') . " ORDER BY min_points ASC ";
	$rs = $db->query($sql);
...
function action_add ()
{
	...
	$user = array(
		'rank_points' => $_CFG['register_points'],'pay_points' => $_CFG['register_points'],'sex' => 0,'credit_line' => 0,'supplier_id' => 0
	);//寒冰  20170926  修复平台添加会员提示  非平台会员无法编辑问题
	/* 取出注册扩展字段 */
	$sql = 'SELECT * FROM ' . $yp->table('reg_fields') . ' WHERE type < 2 AND display = 1 AND id != 6 ORDER BY dis_order, id';
...
function action_insert ()
{
	...
	{
		$sql = 'INSERT INTO ' . $yp->table('reg_extend_info') . ' (`user_id`, `reg_field_id`, `content`) VALUES' . $extend_field_str;
		$db->query($sql);
	}
	...
	$db->autoExecute($yp->table('users'), $other, 'UPDATE', "user_name = '$username'");
	/* 代码增加2014-12-23 by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1 _star */
	...
	$sql = "update " . $yp->table('users') . " set  mobile_phone='$mobile_phone' , `real_name`='$real_name',`card`='$card',`country`='$country',`province`='$province',`city`='$city',`district`='$district',`address`='$address',`status`='$status' where user_name = '" . $username . "'";
	$db->query($sql);
	
	if($face_card != '')
	{
		$sql = "update " . $yp->table('users') . " set `face_card` = '$face_card' where user_name = '" . $username . "'";
		$db->query($sql);
	}
	if($back_card != '')
	{
		$sql = "update " . $yp->table('users') . " set `back_card` = '$back_card' where user_name = '" . $username . "'";
		$db->query($sql);
	}
	/* 代码增加2014-12-23 by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1 _end */
	/* 记录管理员操作 */
...
function action_edit ()
{
	...
	$sql = "SELECT u.user_name, u.sex, u.birthday, u.pay_points, u.rank_points, u.user_rank , u.user_money, u.frozen_money, u.credit_line, u.parent_id,u.supplier_id, u2.user_name as parent_username, u.qq, u.msn, u.office_phone, u.home_phone, u.mobile_phone" . " FROM " . $yp->table('users') . " u LEFT JOIN " . $yp->table('users') . " u2 ON u.parent_id = u2.user_id WHERE u.user_id='$_GET[id]'";
	...
	/* 代码增加2014-12-23 by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1 _star */
	$sql = "SELECT u.user_id, u.sex, u.birthday, u.pay_points, u.rank_points, u.user_rank , u.user_money, u.frozen_money, u.credit_line, u.parent_id,u.supplier_id, u2.user_name as parent_username, u.qq, u.msn,
    u.office_phone, u.home_phone, u.mobile_phone,u.real_name,u.card,u.face_card,u.back_card,u.country,u.province,u.city,u.district,u.address,u.status " . " FROM " . $yp->table('users') . " u LEFT JOIN " . $yp->table('users') . " u2 ON u.parent_id = u2.user_id WHERE u.user_id='$_GET[id]'";
	/* 代码增加2014-12-23 by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1 _end */
	...
	if($row)
	{
		...
		/* 代码增加2014-12-23 by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1 _star */
		$user['supplier_id'] = $row['supplier_id'];
	...
	$sql = 'SELECT reg_field_id, content ' . 'FROM ' . $yp->table('reg_extend_info') . " WHERE user_id = $user[user_id]";
...
function action_update ()
{
	...
	if(! empty($password))
	{
		$sql = "UPDATE " . $yp->table('users') . "SET `ec_salt`='0' WHERE user_name= '" . $username . "'";
		$db->query($sql);
	}
	/* 代码增加2014-12-23 by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1 _star */
	if(isset($_FILES['face_card']) && $_FILES['face_card']['tmp_name'] != '')
	...
	$sql = "update " . $yp->table('users') . " set `real_name`='$real_name',`card`='$card',`country`='$country',`province`='$province',`city`='$city',`district`='$district',`address`='$address',`status`='$status' where user_name = '" . $username . "'";
	$db->query($sql);
	...
	/** 去掉此处，此处会将手机号码设置为未验证
	$sql = "select mobile_phone from " . $GLOBALS['yp']->table('users') . " where user_name = '$username'";
	if($GLOBALS['db']->getOne($sql) != $other['mobile_phone'])
	{
		$sql = "UPDATE " . $GLOBALS['yp']->table('users') . " SET validated = 0 where user_name = '$username'";
...
function action_batch_remove ()
{
	...
	{
		$sql = "SELECT user_name FROM " . $yp->table('users') . " WHERE user_id " . db_create_in($_POST['checkboxes']);
		$col = $db->getCol($sql);
...
function action_edit_username ()
{
	...
	{
		if($_CFG['integrate_code'] != 'jtypmall')
		{
			/* 更新商城会员表 */
			$db->query('UPDATE ' . $yp->table('users') . " SET user_name = '$username' WHERE user_id = '$id'");
		}
...
function action_edit_email ()
{
	...
	$sql = "SELECT user_name FROM " . $yp->table('users') . " WHERE user_id = '$id'";
...
function action_remove ()
{
	...
	$sql=" SELECT COUNT(*) FROM " . $yp->table('supplier') . " WHERE user_id='" . $_GET['id'] . "'";
	...
	if($issupplier>0){
		...
		sys_msg(sprintf('该会员已申请或正在申请小程序，不能删除！'), 0, $link);
	}else{
		$sql = "SELECT user_name FROM " . $yp->table('users') . " WHERE user_id = '" . $_GET['id'] . "'";
		$username = $db->getOne($sql);
		/* 通过插件来删除用户 */
		$users = & init_users();
...
function action_address_list ()
{
	...
	$sql = "SELECT a.*, c.region_name AS country_name, p.region_name AS province, ct.region_name AS city_name, d.region_name AS district_name " . " FROM " . $yp->table('user_address') . " as a " . " LEFT JOIN " . $yp->table('region') . " AS c ON c.region_id = a.country " . " LEFT JOIN " . $yp->table('region') . " AS p ON p.region_id = a.province " . " LEFT JOIN " . $yp->table('region') . " AS ct ON ct.region_id = a.city " . " LEFT JOIN " . $yp->table('region') . " AS d ON d.region_id = a.district " . " WHERE user_id='$id'";
...
function action_remove_parent ()
{
	...
	$sql = "UPDATE " . $yp->table('users') . " SET parent_id = 0 WHERE user_id = '" . $_GET['id'] . "'";
	$db->query($sql);
...
function action_aff_list ()
{
	...
	for($i = 1; $i <= $num; $i ++)
	{
		...
		if($count)
		{
			$sql = "SELECT user_id, user_name, '$i' AS level, email, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time " . " FROM " . $GLOBALS['yp']->table('users') . " WHERE user_id IN($up_uid)" . " ORDER by level, user_id";
...
function user_list ()
{
	...
	if($result === false)
	{
		...
		$filter['supplier_id'] = empty($_REQUEST['suppid']) ? 0 : intval($_REQUEST['suppid']);
        ...
        if ($filter['supplier_id']) {
			...
			$ex_where .= " AND supplier_id = $filter[supplier_id] "; //20160926 by prince 
       }
		if($filter['keywords'])
		{
			$ex_where .= " AND user_name LIKE '%" . mysql_like_quote($filter['keywords']) . "%' or email like  '%" . mysql_like_quote($filter['keywords']) . "%' or mobile_phone like  '%" . mysql_like_quote($filter['keywords']) . "%' or nickname like  '%" . mysql_like_quote($filter['keywords']) . "%' "; //20160926 by prince 
		}
		if($filter['rank'])
		{
			$sql = "SELECT min_points, max_points, special_rank FROM " . $GLOBALS['yp']->table('user_rank') . " WHERE rank_id = '$filter[rank]'";
		...
		$filter['record_count'] = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('users'). $ex_where);
		...
		// reg_time ".
		$sql = "SELECT user_id, user_name, email, mobile_phone, is_validated, supplier_id, validated, user_money, frozen_money, rank_points, pay_points, status, reg_time, froms,headimg,nickname ".  //20160926 by prince 
		        /* 代码增加2014-12-23 by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1  _end  */
                " FROM " . $GLOBALS['yp']->table('users') ."AS u ".$ex_where . " ORDER by " . $filter['sort_by'] . ' ' . $filter['sort_order'] . " LIMIT " . $filter['start'] . ',' . $filter['page_size'];//20160926 by prince 
	...
	$sql = "select * from " . $GLOBALS['yp']->table('user_rank') . " ";
	...
	for($i = 0; $i < $count; $i ++)
	{
		if(!empty($user_list[$i]['headimg']) && strpos($user_list[$i]['headimg'],'http') === false){
			$user_list[$i]['headimg'] = './../'.$user_list[$i]['headimg'];
		}
		if($user_list[$i]['supplier_id']){
        $sql = "select supplier_name from " . $GLOBALS['yp']->table('supplier') . "WHERE supplier_id = ".$user_list[$i]['supplier_id'];
        $user_list[$i]['supplier_name'] = $GLOBALS['db']->getOne($sql);//会员归属商家
      }else{

        $user_list[$i]['supplier_name']= '平台自有';
