...
/**
 * QQ120029121 会员统计-会员规模统计
 * ============================================================================
 * 演示地址: http://demo.coolhong.com/
 * ============================================================================
 * $Author: jtypmall $
 * $Id: user_analysis_scale.php 2015-10-27 18:48:08Z jtypmall $
 */
...
function get_result_list ($start_date, $end_date)
{
    ...
    if($result === false)
    {
        ...
        $sql = 'SELECT us.user_name, SUM(oi.goods_amount) order_amount, us.amount user_amount, us.pay_points FROM '
            . $GLOBALS['yp']->table('order_info') . ' oi LEFT JOIN(SELECT u.user_id, u.user_name,'
            . ' IFNULL(SUM(ua.amount),0) amount, IFNULL(SUM(u.pay_points),0) pay_points FROM '
            . $GLOBALS['yp']->table('users') . ' u LEFT JOIN ' . $GLOBALS['yp']->table('user_account')
