 ...
 * ==========================================================
 * $Author: Prince $
 * $Id: keyword.php 17063 2010-03-25 06:35:46Z Prince $
 */
...
{
    $id = intval($_GET['id']);
	$sql = "SELECT * FROM " .$yp->table('keyword'). " WHERE searchengine='jtypmall' AND w_id='$id'";
	$kw = $db->getRow($sql);
	$area_xml  = '';
    ...
    $sql = "SELECT COUNT(*) AS access_count, area FROM " . $yp->table('keyword_area') .
                " WHERE w_id = '$id' " .
...
elseif ($_REQUEST['act'] == 'toggle_show')
{
    ...
    $sql = "UPDATE " . $yp->table('keyword') . " SET status = '$val' WHERE w_id = '$id'";
	$db->query($sql);
...
elseif ($_REQUEST['act'] == 'batch_drop')
{
    if (isset($_POST['checkboxes']))
    {
        ...
        {
            $db->query("DELETE FROM " .$yp->table('keyword'). " WHERE w_id='$id'");
            $db->query("DELETE FROM " .$yp->table('keyword_area'). " WHERE w_id='$id'");
            $count++;
        }
...
elseif ($_REQUEST['act'] == 'edit_word')
{
    ...
    {
		if (YP_CHARSET != 'utf-8') $word = yp_iconv(YP_CHARSET, 'utf-8', $word);
		$letter = gb2py($word);
    	$sql = 'UPDATE ' . $yp->table('keyword') . " SET word = '$word', letter = '$letter' WHERE w_id = '$id'";
    	$db->query($sql);
...
elseif ($_REQUEST['act'] == 'edit_letter')
{
    ...
    $sql = "UPDATE " . $yp->table('keyword') . " SET letter = '$val' WHERE w_id = '$id'";
	$db->query($sql);
...
elseif ($_REQUEST['act'] == 'edit_items')
{
    ...
    {
		$sql = "UPDATE " . $yp->table('keyword') . " SET items = '$val' WHERE w_id = '$id'";
		$db->query($sql);
        make_json_result(stripslashes($val));
    }
}
elseif ($_REQUEST['act'] == 'edit_total_search')
{
    ...
    {
		$sql = "UPDATE " . $yp->table('keyword') . " SET total_search = '$val' WHERE w_id = '$id'";
		$db->query($sql);
        make_json_result(stripslashes($val));
    }
}
elseif ($_REQUEST['act'] == 'edit_month_search')
{
    ...
    {
		$sql = "UPDATE " . $yp->table('keyword') . " SET month_search = '$val' WHERE w_id = '$id'";
		$db->query($sql);
        make_json_result(stripslashes($val));
    }
}
elseif ($_REQUEST['act'] == 'edit_week_search')
{
    ...
    {
		$sql = "UPDATE " . $yp->table('keyword') . " SET week_search = '$val' WHERE w_id = '$id'";
		$db->query($sql);
        make_json_result(stripslashes($val));
    }
}
elseif ($_REQUEST['act'] == 'edit_today_search')
{
    ...
    {
		$sql = "UPDATE " . $yp->table('keyword') . " SET today_search = '$val' WHERE w_id = '$id'";
		$db->query($sql);
        make_json_result(stripslashes($val));
    }
}
...
{
    $sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['yp']->table('keyword') . " WHERE word = '$word'" .
           " AND w_id != '$w_id'";
...
function get_keyword()
{
    ...
    if ($result === false)
    {
    	...
    	$sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('keyword') . " WHERE  searchengine='jtypmall' $where";
        ...
        /* 查询数据 */
    	$sql = "SELECT * FROM " . $GLOBALS['yp']->table('keyword') .
           " WHERE  searchengine='jtypmall' $where" .
