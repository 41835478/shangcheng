 ...
 * ============================================================================
 * $Author: PRINCE $
 * $Id: topic.php 17217 2017-04-01 06:29:08Z PRINCE $
*/
...
if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    if (!$isadd)
    {
        $sql = "SELECT * FROM " . $yp->table('topic') . " WHERE topic_id = '$topic_id'";
        $topic = $db->getRow($sql);
        $topic['start_time'] = local_date('Y-m-d', $topic['start_time']);
        $topic['end_time']   = local_date('Y-m-d', $topic['end_time']);

        create_html_editor('topic_intro', htmlspecialchars($topic['intro'])); /* 修改 by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1 百度编辑器 */
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    else
    {
        ...
        if (isset($_POST['drop']))
        {
            ...
            {
                /* 删除记录 */
                $sql = "DELETE FROM " . $yp->table('topic') .
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($is_insert)
    {
        $sql = "INSERT INTO " . $yp->table('topic') . " (title,start_time,end_time,data,intro,template,css,topic_img,title_pic,base_style, htmls,keywords,description)" .
...
elseif ($_REQUEST["act"] == "delete")
{
    ...
    $sql = "DELETE FROM " . $yp->table('topic') . " WHERE ";
...
function get_topic_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['yp']->table('topic');
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        /* 分页大小 */
        $filter = page_and_size($filter);

        $sql = "SELECT * FROM " .$GLOBALS['yp']->table('topic'). " ORDER BY $filter[sort_by] $filter[sort_order]";
    ...
    while($topic = $GLOBALS['db']->fetch_array($query)){
        $topic['start_time'] = local_date('Y-m-d',$topic['start_time']);
        $topic['end_time']   = local_date('Y-m-d',$topic['end_time']);
        $topic['url']        = $GLOBALS['yp']->url() . 'topic.php?topic_id=' . $topic['topic_id'];
        $res[] = $topic;
    }
