...
if($_REQUEST['act'] == 'edit_district' || $_REQUEST['act'] == 'add_district'){
   ...
   $sql = "select a.district_id, a.district_name,a.province,a.city,a.county,a.sort,a.is_show, b.region_name as city_name, c.region_name as county_name from " .$GLOBALS['yp']->table('virtual_goods_district').
           " as a left join ".$GLOBALS['yp']->table('region')." as b on b.region_id = a.city".
           " left join ".$GLOBALS['yp']->table('region')." as c on c.region_id = a.county  where district_id = $district_id";
...
if($_REQUEST['act'] == 'insert_district'){
    ...
    if(!$district_id) {
      $sql = "INSERT INTO ".$GLOBALS['yp']->table('virtual_goods_district')." (district_name, province, city,county,sort,is_show)  VALUES ('$district_name','$province','$city','$county','$sort','$is_show') ";
    }else{
      $sql = "UPDATE ".$GLOBALS['yp']->table('virtual_goods_district')."  set district_name = '$district_name', province = '$province',city ='$city',county='$county', sort='$sort',is_show='$is_show' where district_id = $district_id";
    }
...
if($_REQUEST['act'] == 'get_region_list'){
    ...
    $sql = "select region_id, region_name from ".$GLOBALS['yp']->table('region')." where parent_id = '$region_id'";
...
if($_REQUEST['act'] == 'selectCounty'){
    $sql = "select distinct county,region_name from ".$GLOBALS['yp']->table('virtual_goods_district'). "as d left join (select region_id,region_name from" .$GLOBALS['yp']->table('region'). ") as r on r.region_id = d.county where city = '{$_REQUEST['city']}'";
...
if($_REQUEST['act']=='toggle_is_show'){
    ...
    $sql = "update ".$GLOBALS['yp']->table('virtual_goods_district')." set is_show = '$is_show' where district_id = $district_id";
...
if($_REQUEST['act']=='edit_district_name'){
    ...
    $sql = "update ".$GLOBALS['yp']->table('virtual_goods_district')." set district_name = '$district_name' where district_id = $district_id";
    if( $GLOBALS['db'] -> query($sql)){
...
if($_REQUEST['act']=='edit_sort'){
    ...
    $sql = "update ".$GLOBALS['yp']->table('virtual_goods_district')." set sort = '$sort' where district_id = $district_id";
    if( $GLOBALS['db'] -> query($sql)){
...
if($_REQUEST['act']=='remove_district'){
    ...
    $sql = "delete from ".$GLOBALS['yp']->table('virtual_goods_district')." where district_id = $district_id";
    $GLOBALS['db'] -> query($sql);
if ($GLOBALS['db'] ->errno() == 0)
    {
...
if($_REQUEST['act']=='batch_district'){
       if (!isset($_POST['checkboxes']) || !is_array($_POST['checkboxes']))
            ...
            {
                $sql = "delete from ".$GLOBALS['yp']->table('virtual_goods_district')." where district_id = $id";
                $GLOBALS['db'] -> query($sql);
            }
...
if($_REQUEST['act'] == 'get_district_tree'){
    $sql = "select distinct city from ".$GLOBALS['yp']->table('virtual_goods_district');
    $city = $GLOBALS['db']->getAll($sql);  
    $arr = array();
    
    foreach($city as $k=>$v){
        $sql = "select region_id, parent_id, region_name from ".$GLOBALS['yp']->table('region')." where region_id = $v[city]";
    ...
    $sql = "select distinct county from ".$GLOBALS['yp']->table('virtual_goods_district');
    $county = $GLOBALS['db']->getAll($sql);  
    foreach($county as $k=>$v){
        $sql = "select region_id, parent_id, region_name from ".$GLOBALS['yp']->table('region')." where region_id = $v[county]";
        $a =  $GLOBALS['db']->getRow($sql);  
...
if($_REQUEST['act'] == 'validate_district'){
        ...
        $sql = "select district_id from ".$GLOBALS['yp']->table('virtual_goods_district')." where province = '$province' and city = '$city' and county='$county' and district_name='$district_name'";
...
function get_district_list()
{
    ...
    $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('virtual_goods_district') . " WHERE 1 $where";
    ...
    /* 查询 */
    $sql = "SELECT *".
            " FROM ".$GLOBALS['yp']->table('virtual_goods_district').
            " WHERE 1 ".$where.
...
{	
	...
	else
	{
		...
		$sql_cats_zhyh="select * from ". $yp->table('supplier_goods_cat') ." where goods_id='$goods[goods_id]' ";
        ...
        /* 获取区域列表 city*/
        $sql = "select distinct city,region_name from " . $yp->table('virtual_goods_district') ." as d left join (select region_id,region_name from ". $yp->table('region') ." ) as r on r.region_id = d.city";
        $city = $db->getAll($sql);
        $smarty->assign('city',$city);
        
            /* 获取商圈 */
        $sql = "select d.district_id,v.district_name from ".$GLOBALS['yp']->table('virtual_district')." as d 
            left join ".$GLOBALS['yp']->table('goods')." as g on  d.goods_id = g.goods_id
            left join ".$GLOBALS['yp']->table('virtual_goods_district')." as v on v.district_id = d.district_id
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($is_insert)
    {
        $sql = "INSERT INTO " . $yp->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
   ...
   $sql = "delete from ".$yp->table('virtual_district')." where goods_id = '$goods_id' and supplier_id = '$_REQUEST[supplier_id]'";
   $db->query($sql);   //先删除对应goods_id下的商圈   

   foreach($district_list_ids_arr as $k=>$v){
        $sql = "INSERT INTO " . $yp->table('virtual_district') . " (district_id, goods_id, supplier_id)" .
...
elseif ($_REQUEST['act'] == 'product_add_execute')
{
    ...
    foreach($product['product_sn'] as $key => $value)
    {
        ...
        /* 插入货品表 */
        $sql = "INSERT INTO " . $GLOBALS['yp']->table('products') . " (goods_id, goods_attr, product_sn, product_number)  VALUES ('" . $product['goods_id'] . "', '$goods_attr', '$value', '" . $product['product_number'][$key] . "')";
        if (!$GLOBALS['db']->query($sql))
        {
            continue;
...
if($_REQUEST['act']=='getCounty'){
    $city_id = $_REQUEST['id'];
    $sql = "select distinct county,region_name from ".$yp->table('virtual_goods_district')." as d left join (select region_id,region_name from ".$yp->table('region').") as r on r.region_id = d.county where city = '{$city_id}'";
...
if($_REQUEST['act']=='getDistrict'){
    ...
    $sql = "select distinct district_id,district_name from ".$yp->table('virtual_goods_district')." where city = '{$city_id}' and county = '{$county_id}'";
    $district = $db -> getAll($sql);
...
if($_REQUEST['act'] == 'selectDistrict'){
    $sql = "select district_id, district_name from ".$GLOBALS['yp']->table('virtual_goods_district'). " where county = '{$_REQUEST['county']}'";
    $area = $GLOBALS['db'] -> getAll($sql);
    echo json_encode($area);
}
?>