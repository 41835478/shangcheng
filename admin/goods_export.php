 ...
 *
 * $Author: PRINCE $
 * $Id: goods_export.php 17217 2017-04-01 06:29:08Z PRINCE $
*/
...
elseif ($_REQUEST['act'] == 'act_export_taobao')
{
    ...
    {
        $sql = "SELECT region_id,  region_name FROM " . $yp->table('region') . " WHERE region_id IN ('$_CFG[shop_province]',  '$_CFG[shop_city]')";
        $arr = $db->getAll($sql);
    ...
    " FROM " . $yp->table('goods') . " AS g ". $where;
    ...
    if (YP_CHARSET != 'utf-8')
    {
        $content = yp_iconv(YP_CHARSET, 'utf-8', $content);
    }
...
elseif($_REQUEST['act'] == 'act_export_jtypmall')
{
    /* 检查权限 */
    admin_priv('goods_export');
    ...
    $sql = "SELECT g.*, b.brand_name as brandname " .
           " FROM " . $yp->table('goods') . " AS g LEFT JOIN " . $yp->table('brand') . " AS b " .
    ...
    if ($charset == 'UTF8')
	{
		$content = implode("\t", $_LANG['jtypmall']) . "\n";
	}
	else
	{
		$content = '"' . implode('","', $_LANG['jtypmall']) . "\"\n";
    ...
    if ($charset == 'UTF8')
	{
		$zip->add_file("\xFF\xFE" . utf82u2(yp_iconv(YP_CHARSET, $charset, $content)), 'goods_list.csv');
	
	}
	else
	{
		$zip->add_file(yp_iconv(YP_CHARSET, $charset, $content), 'goods_list.csv');
	}
...
elseif ($_REQUEST['act'] == 'act_export_paipai')
{
    ...
    if (YP_CHARSET == 'utf-8')
    {
        $zip->add_file(yp_iconv('UTF8', 'GB2312', $content), 'goods_list.csv');
    }
    else
    {
...
elseif ($_REQUEST['act'] == 'act_export_custom')
{
    ...
    while ($row = $db->fetchRow($res))
    {
        ...
        $sql = "SELECT `attr_id`, `attr_value` FROM " . $yp->table('goods_attr') . " WHERE `goods_id` = '" . $row['goods_id'] . "'";
        $query = $db->query($sql);
...
{
    $sql = "SELECT `attr_id`, `cat_id`, `attr_name` FROM " . $GLOBALS['yp']->table('attribute') . " ";
    if (!empty($cat_id))
    {
...
function set_goods_field_name($array, $lang)
{
    ...
    foreach ($array as $key => $value)
    {
        ...
        {
            $tmp_fields[$key] = $GLOBALS['db']->getOne("SELECT `attr_name` FROM " . $GLOBALS['yp']->table('attribute') . " WHERE `attr_id` = '" . intval($value) . "'");
        }
    }
    return $tmp_fields;
}
