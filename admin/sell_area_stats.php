<?php
/**
 * QQ120029121 销售报告：区域分布
 * ============================================================================
 * 演示地址: http://demo.coolhong.com/
 * ============================================================================
 * $Author: jtypmall $
 * $Id: sell_area_stats.php 2015-10-23 13:00:08Z jtypmall $
 */
...
$sql_supplier = "SELECT supplier_id, supplier_name FROM "
    . $GLOBALS['yp']->table("supplier") . " WHERE status = '1' ORDER BY supplier_id";
...
if ($_REQUEST['act'] == 'list')
{
    ...
    if ($select_type == 'province')
    {
        $order_users = $db->getAll(
            ...
            . $yp->table('order_info') . ' o ' . $where . ' GROUP BY user_id) so, '
            . $yp->table('region') . ' r WHERE so.province = r.region_id GROUP BY province ORDER BY count DESC'
        );
    }
    elseif($select_type == 'city')
    {
        $order_users = $db->getAll(
            'SELECT r.region_name, COUNT(*) count FROM (SELECT * FROM '
            . $yp->table('order_info') . ' o ' . $where . ' GROUP BY user_id) so, '
            . $yp->table('region') . ' r WHERE so.city = r.region_id GROUP BY city ORDER BY count DESC'
        );
    }
    elseif($select_type == 'district')
    {
        $order_users = $db->getAll(
            'SELECT r.region_name, COUNT(*) count FROM (SELECT * FROM '
            . $yp->table('order_info') . ' o ' . $where . ' GROUP BY user_id) so, '
            . $yp->table('region') . ' r WHERE so.district = r.region_id GROUP BY district ORDER BY count DESC'
        );
    }
    ...
    {
        $order_users = $db->getAll(
            'SELECT r.region_name, SUM(o.goods_amount) amount FROM ' . $yp->table('order_info') . ' o, '
            . $yp->table('region') . ' r ' . $where . ' AND o.province = r.region_id GROUP BY province ORDER BY amount DESC'
        );
    }
    elseif($select_type == 'city')
    {
        $order_users = $db->getAll(
            'SELECT r.region_name, SUM(o.goods_amount) amount FROM ' . $yp->table('order_info') . ' o, '
            . $yp->table('region') . ' r ' . $where . ' AND o.city = r.region_id GROUP BY city ORDER BY amount DESC'
        );
    }
    elseif($select_type == 'district')
    {
        $order_users = $db->getAll(
            'SELECT r.region_name, SUM(o.goods_amount) amount FROM ' . $yp->table('order_info') . ' o, '
            . $yp->table('region') . ' r ' . $where . ' AND o.district = r.region_id GROUP BY district ORDER BY amount DESC'
        );
    }
    ...
    {
        $order_users = $db->getAll(
            'SELECT r.region_name, COUNT(*) count FROM ' . $yp->table('order_info') . ' o, '
            . $yp->table('region') . ' r ' . $where . ' AND o.province = r.region_id GROUP BY province ORDER BY count DESC'
        );
    }
    elseif($select_type == 'city')
    {
        $order_users = $db->getAll(
            'SELECT r.region_name, COUNT(*) count FROM ' . $yp->table('order_info') . ' o, '
            . $yp->table('region') . ' r ' . $where . ' AND o.city = r.region_id GROUP BY city ORDER BY count DESC'
        );
    }
    elseif($select_type == 'district')
    {
        $order_users = $db->getAll(
            'SELECT r.region_name, COUNT(*) count FROM ' . $yp->table('order_info') . ' o, '
            . $yp->table('region') . ' r ' . $where . ' AND o.district = r.region_id GROUP BY district ORDER BY count DESC'
        );
    }
    ...
    // 地域下拉框选项
    $sql = 'select * from ' . $GLOBALS['yp']->table('region') . ' where parent_id=' . $GLOBALS['_CFG']['shop_country'];
