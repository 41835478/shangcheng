 ...
 * ============================================================================
 * $Author: jtypmall $
 * $Id: order_stats.php 2015-10-28 08:45:08Z jtypmall $
 */
...
if ($_REQUEST['act'] == 'list')
{
    ...
    $goods_save = $db->getOne(
        ...
        . 'SELECT goods_id FROM ' . $yp->table('goods') . $where1 . ')'
    );
    // 商品总数
    $goods_number = $db->getOne(
        'SELECT COUNT(*) FROM ' . $yp->table('goods') . $where1
    );

    // 店铺收藏量
    $shop_save = $db->getOne(
        'SELECT COUNT(*) FROM ' . $yp->table('supplier_guanzhu') . $where2
    );
