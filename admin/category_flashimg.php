...
if ($_REQUEST['act'] == 'insert')
{
    ...
    /* 入库的操作 */
    if ($db->autoExecute($yp->table('cat_flashimg'), $flashimg) !== false)
    {
...
if ($_REQUEST['act'] == 'update')
{
    ...
    if ($db->autoExecute($yp->table('cat_flashimg'), $img, 'UPDATE', "img_id='$img_id'"))
    {
        /* 更新分类信息成功 */
        clear_cache_files(); // 清除缓存
...
if ($_REQUEST['act'] == 'remove')
{
   /* 权限检查 */
	...
	$img_id=!empty($_REQUEST['id']) ? intval($_REQUEST['id']) : 0;
	$sql="select img_url from " .$yp->table("cat_flashimg"). " where img_id='$img_id' ";
	...
	if (!empty($img_url))
    {
     ...
     /* 删除分类 */
     $sql = 'DELETE FROM ' .$yp->table('cat_flashimg'). " WHERE img_id = '$img_id'";
     if ($db->query($sql))
     {
          clear_cache_files();
    }
...
{
    $sql = "SELECT * FROM " .$GLOBALS['yp']->table('cat_flashimg'). " WHERE img_id='$img_id' LIMIT 1";
...
{
    $sql = "SELECT * ".
           " FROM " . $GLOBALS['yp']->table('cat_flashimg').
           " WHERE  cat_id = '$cat_id' ".
           " ORDER BY sort_order";
