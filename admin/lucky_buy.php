...
elseif ($_REQUEST['act'] =='insert_update')
{
    ...
    $act_id = intval($_POST['act_id']);
        ...
        {
            /* update */
            $db->autoExecute($yp->table('goods_activity'), $lucky_buy, 'UPDATE', "act_id = '$act_id'");
        ...
        {
            /* insert */
            $db->autoExecute($yp->table('goods_activity'), $lucky_buy, 'INSERT');
...
elseif ($_REQUEST['act'] == 'edit_handl_status')
{
	...
	$sql = 'UPDATE ' . $yp->table('lucky_buy') . " SET `handl_status` =" . $val. " WHERE lucky_buy_id = '$id' ";
	$db->query($sql);
	if($val==1){
		$val='已处理';
	}else{
