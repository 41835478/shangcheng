...
/**
 * QQ120029121 站点地图生成程序
 * ============================================================================
 * 演示地址: http://demo.coolhong.com  开发QQ:120029121    309485552
 * ============================================================================
 * $Author: PRINCE $
 * $Id: sitemap.php 17217 2017-04-01 06:29:08Z PRINCE $
*/
...
else
{
    ...
    $db->query("UPDATE " .$yp->table('shop_config'). " SET VALUE='$config' WHERE code='sitemap'");
    ...
    /* 商品 */
    $sql = "SELECT goods_id, goods_name FROM " .$yp->table('goods'). " WHERE is_delete = 0";
    $res = $db->query($sql);
    ...
    /* 文章 */
    $sql = "SELECT article_id,title,file_url,open_type FROM " .$yp->table('article'). " WHERE is_open=1";
    $res = $db->query($sql);
    ...
    if ($sm->build($sm_file))
    {
        sys_msg(sprintf($_LANG['generate_success'], $yp->url()."sitemaps.xml"));
    }
    else
    {
        ...
        if ($sm->build($sm_file))
        {
            sys_msg(sprintf($_LANG['generate_success'], $yp->url(). DATA_DIR . '/sitemaps.xml'));
        }
        else
        {
