 ...
 * ============================================================================
 * $Author: prince $
 * $Id: takegoods.php 17217 2015-02-04 06:29:08Z prince $
 */
...
/* 初始化$exc对象 */
$exc = new exchange($yp->table('takegoods_type'), $db, 'type_id', 'type_name');
...
if ($_REQUEST['act'] == 'remove')
{
    ...
    $sql="select count(*) from ". $yp->table('takegoods') ." where type_id='$id' ";
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    /* 检查类型是否有重复 */
    $sql = "SELECT COUNT(*) FROM " .$yp->table('takegoods_type'). " WHERE type_name='$type_name'";
    if ($db->getOne($sql) > 0)
    {
    ...
    /* 插入数据库。 */
    $sql = "INSERT INTO ".$yp->table('takegoods_type')." (type_name, type_money, type_money_count, use_start_date, use_end_date)
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    $vtype_arr = $db->getRow("SELECT * FROM " .$yp->table('takegoods_type'). " WHERE type_id = '$type_id'");
...
if ($_REQUEST['act'] == 'update')
{
    ...
    $sql = "UPDATE " .$yp->table('takegoods_type'). " SET ".
        "type_name       = '$type_name', ".
...
elseif ( $_REQUEST['act'] == 'add_goods' )
{
    ...
    $sql="select goods_ids from ". $yp->table('takegoods_type_goods') ." where tg_type_id='$type_id' ";
    $goods_ids = $db->getOne($sql);
    if($goods_ids)
    {
        $goods_list_array=array();
        $sql= "select goods_id,goods_name,shop_price from ". $yp->table('goods') ." where goods_id ".db_create_in($goods_ids);
        $res = $db->query($sql);
...
elseif ( $_REQUEST['act'] == 'add_goods_update' )
{
    ...
    $sql = "select type_money from ". $yp->table('takegoods_type') ." where type_id='$type_id' ";
    ...
    if ($_POST['add_type'] == 'tglist')
    {
        ...
        if ($tg_ids)
        {
            ...
            {
                $sql="replace into ".$yp->table('takegoods_goods')."(tg_id, goods_ids) values('$tg_id', '$goods_ids')";
                $db->query($sql);
            }
        }
    }
    ...
    {
        //针对类型进行配置
        $sql="replace into ".$yp->table('takegoods_type_goods')."(tg_type_id, goods_ids) values('$type_id', '$goods_ids')";
        $db->query($sql);
    }
...
if ($_REQUEST['act'] == 'send_by_print')
{
    ...
    $sql="select tg_sn from ".$yp->table('takegoods')." where type_id='$type_id' order by tg_sn desc limit 0,1";
...
//		$tg_sn .=  str_pad($tg_sn_num, 4, '0', STR_PAD_LEFT);
        ...
        $db->query("INSERT INTO ".$yp->table('takegoods')." (type_id, tg_sn, tg_pwd, add_time) VALUES('$type_id', '$tg_sn', '$tg_pwd', '$add_time')");
        $j++;
        $tg_sn_num++;
    }
...
if ($_REQUEST['act'] == 'gen_excel')
{
    ...
    $type_name = $db->getOne("SELECT type_name FROM ".$yp->table('takegoods_type')." WHERE type_id = '$type_id'");

    /* 文件名称 */
    $takegoods_filename = $type_name .'_takegoods_list';
    if (YP_CHARSET != 'gbk')
    {
        $takegoods_filename = yp_iconv('UTF8', 'GB2312',$takegoods_filename);
    }
    ...
    if (YP_CHARSET != 'gbk')
    {
        echo yp_iconv('UTF8', 'GB2312', $type_name."提货券列表") . "\t\n";
        /* 红包序列号, 红包金额, 类型名称(红包名称), 使用结束日期 */
        echo yp_iconv('UTF8', 'GB2312', $_LANG['tg_sn']) ."\t";
        echo yp_iconv('UTF8', 'GB2312', $_LANG['tg_pwd']) ."\t";
        echo yp_iconv('UTF8', 'GB2312', $_LANG['tg_type_name']) ."\t";
        echo yp_iconv('UTF8', 'GB2312', $_LANG['type_money']) ."\t";
        echo yp_iconv('UTF8', 'GB2312', $_LANG['type_money_count']) ."\t";
        echo yp_iconv('UTF8', 'GB2312', $_LANG['type_money_all']) ."\t";
        echo yp_iconv('UTF8', 'GB2312', $_LANG['use_date_valid']) ."\t\n";
    }
    else
    {
    ...
    $sql = "SELECT tg.tg_id, tg.type_id, tg.tg_sn, tg.tg_pwd,  tt.type_name, tt.type_money, tt.type_money_count, tt.use_start_date, tt.use_end_date ".
        "FROM ".$yp->table('takegoods')." AS tg, ".$yp->table('takegoods_type')." AS tt ".
...
if ($_REQUEST['act'] == 'order_send')
{
    ...
    $sql = "select * from ". $yp->table('takegoods_order') ."  where rec_id='$id' ";
    $order = $db->getRow($sql);
    if (empty($order))
    {
    ...
    $nowtime=gmtime();
    $sql = "update ". $yp->table('takegoods_order') .
...
if ($_REQUEST['act'] == 'order_view')
{
    ...
    $sql = "SELECT MAX(rec_id) FROM " . $yp->table('takegoods_order') . " WHERE rec_id < '$order[rec_id]'";
    $smarty->assign('prev_id', $db->getOne($sql));
    $sql = "SELECT MIN(rec_id) FROM " . $yp->table('takegoods_order') . " WHERE rec_id > '$order[rec_id]'";
...
elseif ($_REQUEST['act'] == 'remove_order')
{
    ...
    if (is_array($ids))
    {
        $sql="DELETE FROM " .$yp->table('takegoods_order'). " WHERE rec_id ". db_create_in($ids);
    }
    else
    {
        $sql="DELETE FROM " .$yp->table('takegoods_order'). " WHERE rec_id='$id'";
    }
    $db->query($sql);

    if (!empty($_GET['tgid']))
    {
        yp_header("Location: takegoods.php?act=order_list&tg_type=$_GET[tg_type]&is_used=$_GET[is_used]&tgid=$_GET[tgid]\n");
    }
    else
    {
        yp_header("Location: $url\n");
...
elseif ($_REQUEST['act'] == 'remove_bonus')
{
    ...
    $db->query("DELETE FROM " .$yp->table('takegoods'). " WHERE tg_id='$id'");
...
if ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['checkboxes']))
    {
        ...
        if (isset($_POST['drop']))
        {
            $sql = "DELETE FROM " . $yp->table('takegoods'). " WHERE tg_id " . db_create_in($tg_id_list);
            $db->query($sql);
        ...
        elseif (isset($_POST['add_goods']))
        {
            ...
            //yp_header("Location:takegoods.php?act=add_goods\n");
        }
...
function get_type_list()
{
    ...
    $sql = "SELECT type_id, COUNT(*) AS sent_count".
        " FROM " .$GLOBALS['yp']->table('takegoods') .
        " GROUP BY type_id";
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['yp']->table('takegoods_type');
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        /* 分页大小 */
        $filter = page_and_size($filter);

        $sql = "SELECT * FROM " .$GLOBALS['yp']->table('takegoods_type'). " ORDER BY $filter[sort_by] $filter[sort_order]";
...
function get_takegoods_order($tgid)
{
    ...
    $sql = "SELECT COUNT(*) FROM ".$GLOBALS['yp']->table('takegoods_order'). ' AS tgo '.$where;
    $filter['record_count'] = $GLOBALS['db']->getOne($sql);
    ...
    $sql = "SELECT tgo.*, u.user_name ".
        " FROM " .$GLOBALS['yp']->table('takegoods_order'). " AS tgo ".
        " LEFT JOIN ". $GLOBALS['yp']->table('users') ." AS u on tgo.user_id = u.user_id  ".
...
function get_takegoods_list()
{
    ...
    $sql = "SELECT COUNT(*) FROM ".$GLOBALS['yp']->table('takegoods'). ' AS tg '.$where;
    $filter['record_count'] = $GLOBALS['db']->getOne($sql);
    ...
    $sql = "SELECT tg.*, tgg.goods_ids ".
        " FROM ".$GLOBALS['yp']->table('takegoods'). " AS tg ".
        " LEFT JOIN " .$GLOBALS['yp']->table('takegoods_order'). " AS tgo ON tgo.rec_id = tg.tg_order_id   ".
        " LEFT JOIN ". $GLOBALS['yp']->table('takegoods_goods') ." AS tgg on tg.tg_id =tgg.tg_id  ".
        " $where ORDER BY ".$filter['sort_by']." ".$filter['sort_order'].
        " LIMIT ". $filter['start'] .", $filter[page_size]";
    $row = $GLOBALS['db']->getAll($sql);
...
{
    $sql = "SELECT * FROM " . $GLOBALS['yp']->table('takegoods_type') .
