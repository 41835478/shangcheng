...
/**
 * QQ120029121 行业分析：行业排行：行业店铺
 * ============================================================================
 * 演示地址: http://demo.coolhong.com；
 * ============================================================================
 * $Author: jtypmall $
 * $Id: industry_best_shop_stats.php 17217 2015-10-27 08:35:08Z jtypmall $
 */
...
if ($_REQUEST['act'] == 'list')
{
    ...
    $sql = "SELECT IFNULL(s.supplier_name, '平台自营') shop_name, COUNT(*) goods_count FROM "
            . $yp->table('order_goods') . ' og LEFT JOIN '
            . $yp->table('order_info') . ' oi ON og.order_id = oi.order_id LEFT JOIN '
            . $yp->table('goods') . ' g ON og.goods_id = g.goods_id LEFT JOIN '
            . $yp->table('supplier') . ' s ON s.supplier_id = g.supplier_id ' . $where
