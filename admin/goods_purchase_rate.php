<?php
/**
 * QQ120029121 商品分析：访问购买率
 * ============================================================================
 * 演示地址: http://demo.coolhong.com/
 * ============================================================================
 * $Author: jtypmall $
 * $Id: goods_purchase_rate.php 2015-10-23 10:23:08Z jtypmall $
 */
...
elseif ($_REQUEST['act'] == 'export')
{
    ...
    $cnt = $GLOBALS['db']->getOne(
        'SELECT COUNT(*) FROM ' . $GLOBALS['yp']->table('goods') . ' g ' . $where2
    );

    // 查询
    $result = $db->getAll(
        'SELECT g.goods_name, g.click_count, (SELECT COUNT(*) FROM ' . $GLOBALS['yp']->table('order_goods')
        . ' og LEFT JOIN ' . $GLOBALS['yp']->table('order_info') . ' i ON og.order_id = i.order_id ' . $where1
        ...
        . $GLOBALS['yp']->table('goods') . ' g LEFT JOIN '
        . $GLOBALS['yp']->table('brand') . ' b ON b.brand_id = g.brand_id ' . $where2
...
function get_purchase_rate($start_date, $end_date, $cat_id, $brand_id, $sel_shop, $supplier_id)
{
    ...
    if($result === false)
    {
        ...
        /* 查询 */
        $sql = 'SELECT g.goods_name, g.click_count, (SELECT COUNT(*) FROM ' . $GLOBALS['yp']->table('order_goods')
            . ' og LEFT JOIN ' . $GLOBALS['yp']->table('order_info') . ' i ON og.order_id = i.order_id ' . $where1
            . ' AND og.goods_id = g.goods_id ) purchase_number, IFNULL(FORMAT((SELECT COUNT(*) FROM '
            . $GLOBALS['yp']->table('order_goods')
            . ' og LEFT JOIN ' . $GLOBALS['yp']->table('order_info') . ' i ON og.order_id = i.order_id ' . $where1
