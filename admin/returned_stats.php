<?php
/**
 * QQ120029121 售后统计：退货统计
 * ============================================================================
 * 演示地址: http://demo.coolhong.com/
 * ============================================================================
 * $Author: jtypmall $
 * $Id: returned_stats.php 2015-10-25 16:00:08Z jtypmall $
 */
...
if ($_REQUEST['act'] == 'list')
{
    ...
    $sql = "SELECT FLOOR((b.add_time - $start_date) / (24 * 3600)) date, SUM(b.refund_money_2) refund_money FROM "
        . $yp->table('back_order') . ' b ' . $where . ' AND b.back_type = 1 GROUP BY date ORDER BY date';
    // 退款金额统计
...
elseif ($_REQUEST['act'] == 'export')
{
    ...
    $result = $db->getAll(
        ...
        . ' b.refund_money_2, b.status_back, b.back_type, b.status_refund FROM ' . $GLOBALS['yp']->table('order_info') . ' o, '
        . $GLOBALS['yp']->table('users') . ' u, ' . $GLOBALS['yp']->table('back_order') . ' b ' . $where
...
function get_result_list($start_date, $end_date, $sel_shop, $supplier_id)
{
    ...
    if($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('back_order') . ' b ' . $where . ' AND b.back_type = 1';
