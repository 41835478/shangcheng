...
/**
 * QQ120029121 会员统计：区域分布
 * ============================================================================
 * 演示地址: http://demo.coolhong.com；
 * ============================================================================
 * $Author: jtypmall $
 * $Id: user_area_stats.php 17217 2015-11-04 20:45:08Z jtypmall $
 */
...
elseif ($_REQUEST['act'] == 'export')
{
    ...
    if($area_type == 0)
    {
        $sql = "SELECT COUNT(*) order_count, SUM(o.goods_amount) goods_amount, o.province, 0 city, 0 district, (SELECT r.region_name FROM "
            . $GLOBALS['yp']->table('region') . ' r WHERE r.region_id = o.province) province_name FROM '
            . $GLOBALS['yp']->table('order_info') . ' o ' . $where . ' GROUP BY o.province ORDER BY order_count DESC';
    }
    // 按市统计
    elseif($area_type == 1)
    {
        $sql = "SELECT COUNT(*) order_count, SUM(o.goods_amount) goods_amount, o.province, o.city, 0 district, (SELECT r.region_name FROM "
            . $GLOBALS['yp']->table('region') . ' r WHERE r.region_id = o.province) province_name, (SELECT r.region_name FROM '
            . $GLOBALS['yp']->table('region') . ' r WHERE r.region_id = o.city) city_name FROM '
            . $GLOBALS['yp']->table('order_info') . ' o ' . $where
    ...
    else
    {
        $sql = 'SELECT COUNT(*) order_count, SUM(o.goods_amount) goods_amount, o.province, o.city, o.district, (SELECT r.region_name FROM '
            . $GLOBALS['yp']->table('region') . ' r WHERE r.region_id = o.province) province_name, (SELECT r.region_name FROM '
            . $GLOBALS['yp']->table('region') . ' r WHERE r.region_id = o.city) city_name, (SELECT r.region_name FROM '
            . $GLOBALS['yp']->table('region') . ' r WHERE r.region_id = o.district) district_name FROM '
            . $GLOBALS['yp']->table('order_info') . ' o ' . $where
    ...
    foreach($result as $key => $value)
    {
        ...
        if($filter['area_type'] == 0)
        {
            $user_count = $GLOBALS['db']->getOne(
                'SELECT COUNT(*) FROM (SELECT DISTINCT o.user_id FROM ' . $GLOBALS['yp']->table('order_info') . ' o '
...
function get_result_list ($area_type, $province, $city)
{
    ...
    if($result === false)
    {
        ...
        $where = ' WHERE (o.pay_id = 6 AND o.shipping_status = 2) OR (o.pay_id <> 6 AND o.pay_status = 2) ';
        $sql = 'SELECT COUNT(*) FROM (SELECT * FROM ' . $GLOBALS['yp']->table('order_info');
        // 按省统计
        if($filter['area_type'] == 0)
        {
