...
$exc = new exchange($yp->table("ad_position"), $db, 'position_id', 'position_name');
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    {
        /* 将广告位置的信息插入数据表 */
        $sql = 'INSERT INTO '.$yp->table('ad_position').' (position_name, ad_width, ad_height, position_desc, position_style) '.
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    /* 获取广告位数据 */
    $sql = 'SELECT * FROM ' .$yp->table('ad_position'). " WHERE position_id='$id'";
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    /* 查看广告位是否与其它有重复 */
    $sql = 'SELECT COUNT(*) FROM ' .$yp->table('ad_position').
           " WHERE position_name = '$position_name' AND position_id <> '$position_id'";
    if ($db->getOne($sql) == 0)
    {
        $sql = "UPDATE " .$yp->table('ad_position'). " SET ".
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 查询广告位下是否有广告存在 */
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['yp']->table('ad'). " WHERE position_id = '$id'";
...
{	 
    ...
    if ($result === false)
    {
        ...
        if (isset($_POST['keyword']))
        {
            ...
            {
				$sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('ad_position') . " WHERE position_name like '%" . $_POST['keyword'] . "%' ";
			}
			else
			{
				$sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('ad_position') . " WHERE position_id like '%" . $_POST['keyword'] . "%' ";
			}
        }
        else
        {
            $sql = "SELECT COUNT(*) FROM ".$GLOBALS['yp']->table('ad_position');
        }
        ...
        if (isset($_POST['keyword']))
        {
			if(isset($_POST['select']) && $_POST['select'] == "按广告位名称")
			{
            	...
            	$sql = "SELECT * FROM ".$GLOBALS['yp']->table('ad_position')." WHERE position_name like '%". $keyword ."%' ORDER BY position_id ASC";
			}
			else
			{
				if(strtoupper(YP_CHARSET) == 'GBK')
            	...
            	$sql = "SELECT * FROM ".$GLOBALS['yp']->table('ad_position')." WHERE position_id like '%". $keyword ."%' ORDER BY position_id ASC";
			}
        }
        else
        {
            $sql = "SELECT * FROM ".$GLOBALS['yp']->table('ad_position')." ORDER BY position_id ASC";
        }
