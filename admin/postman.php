 ...
 * ============================================================================
 * $Author:prince $
 * $Id: takegoods.php 17217 2015-02-07 06:29:08Z prince $
*/
...
/* 初始化$exc对象 */
//$exc = new exchange($yp->table('postman'), $db, 'postman_id', 'postman_name');
...
{  
    ...
    /* 检查快递员是否有重复 */
    $sql = "SELECT COUNT(*) FROM " .$yp->table('postman'). " WHERE  region_id='$region_id' and postman_name='$postman_name'";
    if ($db->getOne($sql) > 0)
    {
    ...
    /* 插入数据库。 */
    $sql = "INSERT INTO ".$yp->table('postman')." (postman_name, region_id, mobile)
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    $postman = $db->getRow("SELECT * FROM " .$yp->table('postman'). " WHERE postman_id = '$postman_id'");
...
if ($_REQUEST['act'] == 'update')
{
    ...
    $sql = "UPDATE " .$yp->table('postman'). " SET ".
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    if (is_array($ids))
	{
		$sql="DELETE FROM " .$yp->table('postman'). " WHERE postman_id ". db_create_in($ids);		
	}
	else
	{
		$sql="DELETE FROM " .$yp->table('postman'). " WHERE postman_id='$id'";
	}
    $db->query($sql);
...
function get_postman_list()
{
    ...
    $sql = "SELECT COUNT(*) FROM ".$GLOBALS['yp']->table('postman')." AS p ". $where;
    ...
    $sql = "SELECT p.*, r.region_name".
          " FROM ".$GLOBALS['yp']->table('postman'). " AS p ".
		" left join ". $GLOBALS['yp']->table('region') ." AS r on p.region_id=r.region_id ".
