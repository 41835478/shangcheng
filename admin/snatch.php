...
elseif ($_REQUEST['act'] =='update')
{
    ...
    $db->autoExecute($yp->table('goods_activity'), $record, 'UPDATE', "act_id = '" . $_POST['id'] . "' AND act_type = " . GAT_SNATCH );
...
function get_snatch_detail()
{
    ...
    /* 获得记录总数以及总页数 */
    $sql = "SELECT count(*) FROM ".$GLOBALS['yp']->table('snatch_log'). $where;
    $filter['record_count'] = $GLOBALS['db']->getOne($sql);
    ...
    $sql = "SELECT s.log_id, u.user_name, s.bid_price, s.bid_time ".
            " FROM ".$GLOBALS['yp']->table('snatch_log')." AS s ".
            " LEFT JOIN ".$GLOBALS['yp']->table('users')." AS u ON s.user_id = u.user_id  ". $where.
