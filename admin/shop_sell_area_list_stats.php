...
/**
 * QQ120029121 店铺统计：区域分布
 * ============================================================================
 * 演示地址: http://demo.coolhong.com；
 * ============================================================================
 * $Author: jtypmall $
 * $Id: shop_sell_area_list_stats.php 17217 2015-10-29 16:14:08Z jtypmall $
 */
...
function get_result_list ($end_date, $area_type, $province, $city)
{
    ...
    if($result === false)
    {
        ...
        $sql = 'SELECT COUNT(*) FROM (SELECT * FROM ' . $GLOBALS['yp']->table('supplier');
        // 按省统计
        ...
        if($filter['area_type'] == 0)
        {
            $sql = "SELECT COUNT(*) count, s.province, 0 city, 0 district, (SELECT r.region_name FROM "
                . $GLOBALS['yp']->table('region') . ' r WHERE r.region_id = s.province) province_name FROM '
                . $GLOBALS['yp']->table('supplier') . ' s ' . $where . ' GROUP BY s.province ORDER BY count DESC';
        }
        // 按市统计
        elseif($filter['area_type'] == 1)
        {
            $sql = "SELECT COUNT(*) count, s.province, s.city, 0 district, (SELECT r.region_name FROM "
                . $GLOBALS['yp']->table('region') . ' r WHERE r.region_id = s.province) province_name, (SELECT r.region_name FROM '
                . $GLOBALS['yp']->table('region') . ' r WHERE r.region_id = s.city) city_name FROM '
                . $GLOBALS['yp']->table('supplier') . ' s ' . $where
        ...
        else
        {
            $sql = 'SELECT COUNT(*) count, s.province, s.city, s.district, (SELECT r.region_name FROM '
                . $GLOBALS['yp']->table('region') . ' r WHERE r.region_id = s.province) province_name, (SELECT r.region_name FROM '
                . $GLOBALS['yp']->table('region') . ' r WHERE r.region_id = s.city) city_name, (SELECT r.region_name FROM '
                . $GLOBALS['yp']->table('region') . ' r WHERE r.region_id = s.district) district_name FROM '
                . $GLOBALS['yp']->table('supplier') . ' s ' . $where
