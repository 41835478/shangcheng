...
$auth = local_mktime(); //代码修改  By demo.coolhong.com 今天优品多商户系统 qq 120029121
...
elseif ($_REQUEST['act'] == 'post')
{
    ...
    $arr = array();
    $sql = 'SELECT id, value FROM ' . $yp->table('shop_config');
    $res= $db->query($sql);
    ...
    foreach ($_POST['value'] AS $key => $val)
    {
        if($arr[$key] != $val)
        {
            $sql = "UPDATE " . $yp->table('shop_config') . " SET value = '" . trim($val) . "' WHERE id = '" . $key . "'";
            $db->query($sql);
        }
    }
    ...
    /* 处理上传文件 */
    $file_var_list = array();
    $sql = "SELECT * FROM " . $yp->table('shop_config') . " WHERE parent_id > 0 AND type = 'file'";
    $res = $db->query($sql);
    while ($row = $db->fetchRow($res))
    {
    ...
    foreach ($_FILES AS $code => $file)
    {
        ...
        if ((isset($file['error']) && $file['error'] == 0) || (!isset($file['error']) && $file['tmp_name'] != 'none'))
        {
            ...
            else
            {
                ...
                {
                    $sql = "UPDATE " . $yp->table('shop_config') . " SET value = '$file_name' WHERE code = '$code'";
                    $db->query($sql);
                }
                else
                {
              	...
              	/*生成商品默认图片缩略图  修改 start by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1 */
                ...
                if ($_CFG['thumb_width'] != 0 || $_CFG['thumb_height'] != 0)
                {
                    ...
                    if ($goods_thumb === false)
                    {
                        ...
                        $sql = "UPDATE " . $yp->table('shop_config') . " SET store_range = '$goods_thumb' WHERE code = '$code'";
                        $db->query($sql);
						if($code == 'wechat_qrcode'){
							$sql = "UPDATE " . $yp->table('shop_config') . " SET value = '$goods_thumb' WHERE code = '$code'";
							$db->query($sql);
						}
                    }
                }
                /*  修改 end by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1 */
...
function update_configure($key, $val='')
{
    if (!empty($key))
    {
        $sql = "UPDATE " . $GLOBALS['yp']->table('shop_config') . " SET value='$val' WHERE code='$key'";
...
function get_settings($groups=null, $excludes=null, $excludeCodes=null)
{
    ...
    /* 取出全部数据：分组和变量 */
    $sql = "SELECT * FROM " . $yp->table('shop_config') .
