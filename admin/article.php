...
/*初始化数据交换对象 */
$exc   = new exchange($yp->table("article"), $db, 'article_id', 'title');
//$image = new cls_image();
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$yp->table('article')."(title, cat_id, article_type, is_open, author, ".
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    /* 取文章数据 */
    $sql = "SELECT * FROM " .$yp->table('article'). " WHERE article_id='$_REQUEST[id]'";
...
if ($_REQUEST['act'] =='update')
{
    ...
    $sql = "SELECT file_url FROM " . $yp->table('article') . " WHERE article_id = '$_POST[id]'";
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 删除原来的文件 */
    $sql = "SELECT file_url FROM " . $yp->table('article') . " WHERE article_id = '$id'";
    $old_url = $db->getOne($sql);
...
function get_articleslist()
{
    ...
    if ($result === false)
    {
        ...
        /* 文章总数 */
        $sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['yp']->table('article'). ' AS a '.
               'LEFT JOIN ' .$GLOBALS['yp']->table('article_cat'). ' AS ac ON ac.cat_id = a.cat_id '.
               'WHERE 1 ' .$where;
