<?php
/**
 * QQ120029121 商品分析：商品销售明细
 * ============================================================================
 * 演示地址: http://demo.coolhong.com/
 * ============================================================================
 * $Author: jtypmall $
 * $Id: goods_sell_detail.php 2015-10-22 16:30:08Z jtypmall $
 */
...
elseif ($_REQUEST['act'] == 'export')
{
    ...
    $sql = "SELECT og.goods_name, og.goods_attr, i.order_sn, og.goods_number sells_count, og.goods_price, i.add_time FROM "
        . $GLOBALS['yp']->table('order_goods') . ' og LEFT JOIN ' . $GLOBALS['yp']->table('order_info')
        . ' i ON og.order_id = i.order_id LEFT JOIN ' . $GLOBALS['yp']->table('goods')
        . ' g ON g.goods_id = og.goods_id LEFT JOIN ' . $GLOBALS['yp']->table('brand')
...
function get_sell_detail($start_date, $end_date, $cat_id, $brand_id, $sel_shop, $supplier_id)
{
    ...
    if($result === false)
    {
        ...
        $filter['record_count'] = $GLOBALS['db']->getOne(
            'SELECT COUNT(*) FROM (SELECT og.order_id FROM ' . $GLOBALS['yp']->table('order_goods')
            . ' og LEFT JOIN '. $GLOBALS['yp']->table('order_info') . ' i ON og.order_id = i.order_id LEFT JOIN '
            . $GLOBALS['yp']->table('goods') . ' g ON g.goods_id = og.goods_id LEFT JOIN '
            . $GLOBALS['yp']->table('brand') . ' b ON g.brand_id = b.brand_id ' . $where
