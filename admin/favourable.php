...
/**
 * QQ120029121 管理中心优惠活动管理
 * ============================================================================
 * 演示地址: http://demo.coolhong.com  开发QQ:120029121    309485552
 * ============================================================================
 * $Author: PRINCE $
 * $Id: favourable.php 17217 2017-04-01 06:29:08Z PRINCE $
 */
...
$exc = new exchange($yp->table('favourable_activity'), $db, 'act_id', 'act_name');
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    else
    {
        ...
        {
            /* 删除记录 */
            $sql = "DELETE FROM " . $yp->table('favourable_activity') .
...
elseif ($_REQUEST['act'] == 'edit_sort_order')
{
    ...
    $sql = "UPDATE " . $yp->table('favourable_activity') .
            " SET sort_order = '$val'" .
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    if ($favourable['act_range'] != FAR_ALL && !empty($favourable['act_range_ext']))
    {
        ...
        {
            $sql = "SELECT cat_id AS id, cat_name AS name FROM " . $yp->table('category') .
        ...
        {
            $sql = "SELECT goods_id AS id, goods_name AS name FROM " . $yp->table('goods') .
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($is_add)
    {
        $db->autoExecute($yp->table('favourable_activity'), $favourable, 'INSERT');
        $favourable['act_id'] = $db->insert_id();
    }
    else
    {
        $db->autoExecute($yp->table('favourable_activity'), $favourable, 'UPDATE', "act_id = '$favourable[act_id]'");
    }
	...
	if($_FILES['logo']['size']){
		...
		$pic_sql = "update " .$yp->table('favourable_activity'). " set logo='".$save['logo']."' where act_id=".$favourable['act_id'];
		$db->query($pic_sql);
	}
...
function favourable_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('favourable_activity') .
