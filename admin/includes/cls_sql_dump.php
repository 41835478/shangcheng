 ...
 * ============================================================================
 * $Author: PRINCE $
 * $Id: cls_sql_dump.php 17217 2017-04-01 06:29:08Z PRINCE $
*/
...
function dump_escape_string($str)
{
	...
	/* 代码修改_star  By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121 */
}
...
class cls_sql_dump
{
    ...
    function get_table_df($table, $add_drop = false)
    {
        ...
        {
            $table_df .= $tmp_sql . " ENGINE=MyISAM DEFAULT CHARSET=" . str_replace('-', '', YP_CHARSET) . ";\r\n";
        }
        else
        {
    ...
    function dump_table($path, $vol)
    {
        ...
        $this->dump_sql .= '-- END jtypmall v2.x SQL Dump Program ';
     ...
     */
    public static function get_head($path)  //代码修改   By    demo.coolhong.com   增加 public static
    {
        /* 获取sql文件头部信息 */
        $sql_info = array('date'=>'', 'mysql_ver'=> '', 'php_ver'=>0, 'yp_ver'=>'', 'vol'=>0);
        $fp = fopen($path,'rb');
        $str = fread($fp, 250);
        fclose($fp);
        $arr = explode("\n", $str);
        ...
        foreach ($arr AS $val)
        {
            ...
            if ($pos > 0)
            {
                ...
                elseif ($type == 'JTYPmall VERSION')
                {
                    $sql_info['yp_ver'] = $value;
                }
                elseif ($type == 'Vol')
                {
                    $sql_info['vol'] = $value;
     ...
     */
    public static function get_random_name()  //代码修改   By    demo.coolhong.com
    {
        $str = date('Ymd');
