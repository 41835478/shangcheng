...
/**
 * QQ120029121 管理中心商品相关函数
 * ============================================================================
 * 演示地址: http://demo.coolhong.com  开发QQ:120029121    309485552
 * ============================================================================
 * $Author: PRINCE $
 * $Id: lib_goods.php 17217 2017-04-01 06:29:08Z PRINCE $
 */
...
function handle_goods_attr($goods_id, $id_list, $is_spec_list, $value_price_list)
{
    ...
    foreach ($id_list AS $key => $id)
    {
        ...
        // 插入或更新记录
        $sql = "SELECT goods_attr_id FROM " . $GLOBALS['yp']->table('goods_attr') . " WHERE goods_id = '$goods_id' AND attr_id = '$id' AND attr_value = '$value' LIMIT 0, 1";
        $result_id = $GLOBALS['db']->getOne($sql);
        if (!empty($result_id))
        {
            $sql = "UPDATE " . $GLOBALS['yp']->table('goods_attr') . "
        ...
        {
            $sql = "INSERT INTO " . $GLOBALS['yp']->table('goods_attr') . " (goods_id, attr_id, attr_value, attr_price) " .
...
function handle_member_price($goods_id, $rank_list, $price_list)
{
    ...
    foreach ($rank_list AS $key => $rank)
    {
        ...
        // 插入或更新记录
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('member_price') .
        ...
        if ($GLOBALS['db']->getOne($sql) > 0)
        {
            ...
            if ($price < 0)
            {
                $sql = "DELETE FROM " . $GLOBALS['yp']->table('member_price') .
                       " WHERE goods_id = '$goods_id' AND user_rank = '$rank' LIMIT 1";
            }
            else
            {
                $sql = "UPDATE " . $GLOBALS['yp']->table('member_price') .
        ...
        else
        {
            ...
            {
                $sql = "INSERT INTO " . $GLOBALS['yp']->table('member_price') . " (goods_id, user_rank, user_price) " .
...
{
    /* 查询现有的扩展分类 */
    $sql = "SELECT cat_id FROM " . $GLOBALS['yp']->table('goods_cat') .
            " WHERE goods_id = '$goods_id'";
    ...
    if ($delete_list)
    {
        $sql = "DELETE FROM " . $GLOBALS['yp']->table('goods_cat') .
                " WHERE goods_id = '$goods_id' " .
    ...
    {
        // 插入记录
        $sql = "INSERT INTO " . $GLOBALS['yp']->table('goods_cat') .
                " (goods_id, cat_id) " .
...
{
    $sql = "UPDATE " . $GLOBALS['yp']->table('link_goods') . " SET " .
            " goods_id = '$goods_id' " .
            " WHERE goods_id = '0'" .
...
{
    $sql = "UPDATE " . $GLOBALS['yp']->table('group_goods') . " SET " .
            " parent_id = '$goods_id' " .
            " WHERE parent_id = '0'" .
            " AND admin_id = '$_SESSION[admin_id]'";
    $GLOBALS['db']->query($sql);
...
{
    $sql = "UPDATE " . $GLOBALS['yp']->table('goods_article') . " SET " .
            " goods_id = '$goods_id' " .
            " WHERE goods_id = '0'" .
            " AND admin_id = '$_SESSION[admin_id]'";
    $GLOBALS['db']->query($sql);
...
function handle_gallery_image($goods_id, $image_files, $image_descs, $image_urls)
{
    ...
    foreach ($image_descs AS $key => $img_desc)
    {
        ...
        if ($flag)
        {
            ...
            $sql = "INSERT INTO " . $GLOBALS['yp']->table('goods_gallery') . " (goods_id, img_url, img_desc, thumb_url, img_original) " .
                    "VALUES ('$goods_id', '$img_url', '$img_desc', '$thumb_url', '$img_original')";
            $GLOBALS['db']->query($sql);
            /* 不保留商品原图的时候删除原图 */
            if ($proc_thumb && !$GLOBALS['_CFG']['retain_original_img'] && !empty($img_original))
            {
                $GLOBALS['db']->query("UPDATE " . $GLOBALS['yp']->table('goods_gallery') . " SET img_original='' WHERE `goods_id`='{$goods_id}'");
                @unlink('../' . $img_original);
            }
        }
...
/**
 * 删除代码中的图片    寒冰  qq  309485552    20161207    删除商品同步删除详情图片
...
function delete_goods($goods_id)
{
    ...
    /* 取得有效商品id */
    $sql = "SELECT DISTINCT goods_id FROM " . $GLOBALS['yp']->table('goods') .
    ...
    {   
	     /* 删除内容中的图片 */
        delete_goods_img($goods['goods_desc']);//寒冰   20161207    qq    309485552     删除商品 同步删除图片文件
    ...
    /* 删除商品 */
    $sql = "DELETE FROM " . $GLOBALS['yp']->table('goods') .
            " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);

    /* 删除商品的货品记录 */
    $sql = "DELETE FROM " . $GLOBALS['yp']->table('products') .
            " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    ...
    /* 删除商品相册 */
    $sql = "DELETE FROM " . $GLOBALS['yp']->table('goods_gallery') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);

    /* 删除相关表记录 */
    $sql = "DELETE FROM " . $GLOBALS['yp']->table('collect_goods') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['yp']->table('goods_article') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['yp']->table('goods_attr') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['yp']->table('goods_cat') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['yp']->table('member_price') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['yp']->table('group_goods') . " WHERE parent_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['yp']->table('group_goods') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['yp']->table('link_goods') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['yp']->table('link_goods') . " WHERE link_goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['yp']->table('tag') . " WHERE goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['yp']->table('comment') . " WHERE comment_type = 0 AND id_value " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);

    /* 删除相应虚拟商品记录 */
    $sql = "DELETE FROM " . $GLOBALS['yp']->table('virtual_card') . " WHERE goods_id " . db_create_in($goods_id);
...
function generate_goods_sn($goods_id)
{
    ...
    $sql = "SELECT goods_sn FROM " . $GLOBALS['yp']->table('goods') .
...
function check_goods_sn_exist($goods_sn, $goods_id = 0)
{
    ...
    if (empty($goods_id))
    {
        $sql = "SELECT goods_id FROM " . $GLOBALS['yp']->table('goods') ."
...
function get_goods_type_specifications()
{
    ...
    $sql = "SELECT DISTINCT cat_id
            FROM " .$GLOBALS['yp']->table('attribute'). "
            WHERE attr_type = 1";
    $row = $GLOBALS['db']->GetAll($sql);
...
function build_attr_html($cat_id, $goods_id = 0 , $bar_code = 0)
{
    ...
    foreach ($attr AS $key => $val)
    {
        ...
        $html .= ($val['attr_type'] == 1 || $val['attr_type'] == 2) ?
            ...
            ' <input type="hidden" name="attr_price_list[]" value="0" />';//20170907 qq 120029121
...
function get_linked_goods($goods_id)
{
    $sql = "SELECT lg.link_goods_id AS goods_id, g.goods_name, lg.is_double " .
            "FROM " . $GLOBALS['yp']->table('link_goods') . " AS lg, " .
...
function get_group_goods($goods_id)
{
    $sql = "SELECT gg.goods_id, CONCAT(g.goods_name, ' -- [', gg.goods_price, ']') AS goods_name " .
            "FROM " . $GLOBALS['yp']->table('group_goods') . " AS gg, " .
                $GLOBALS['yp']->table('goods') . " AS g " .
...
//        $filter['keyword']          = empty($_REQUEST['keyword']) ? '' : trim($_REQUEST['keyword']);
      ...
      if ($filter['is_morning_market'])
       ...
       if ($real_goods > -1)
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['yp']->table('goods'). " AS g WHERE is_delete='$is_delete' $where";
        ...
        if(intval($_REQUEST['supp'])>0){
        	$sql = "SELECT goods_id, goods_name, goods_type,is_morning_market, goods_sn, shop_price,cost_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number,exclusive, integral, " .
                    //今/天/优/品/多/商/户/系/统-采集 mod on 20160808 by prince  qq 1/2/0/0/2/9/1/2/1-begin
...
//今/天/优/品/多/商/户/系/统-采集 mod on 20160808 by prince  qq 1/2/0/0/2/9/1/2/1-end
        	...
        	$sql = "SELECT goods_id, goods_name, goods_type, goods_sn,is_morning_market, shop_price,cost_price,  is_on_sale, is_best, is_new, is_hot, sort_order, goods_number,exclusive, integral, " .
                    //今/天/优/品/多/商/户/系/统-采集 mod on 20160808 by prince  qq 1/2/0/0/2/9/1/2/1-begin
" num_iid, goods_thumb, click_url, original_img, commission_num, is_taobao, shop_title, nick, goods_weight, NOT LOCATE('http',original_img) as r_img," .
//今/天/优/品/多/商/户/系/统-采集 mod on 20160808 by prince  qq 1/2/0/0/2/9/1/2/1-end
                    " (promote_price > 0 AND promote_start_date <= '$today' AND promote_end_date >= '$today') AS is_promote ".
					", supplier_status, supplier_id ".	//代码增加   By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121
                    " FROM " . $GLOBALS['yp']->table('goods') . " AS g WHERE is_delete='$is_delete' $where" .
                    " ORDER BY $filter[sort_by] $filter[sort_order] ".
                    " LIMIT " . $filter['start'] . ",$filter[page_size]";
        }
	...
	$sql_fav = "SELECT gift FROM " . $GLOBALS['yp']->table('favourable_activity') . " WHERE act_type = 0";
    ...
    {
        // 查询商品是否参加促销
        $row[$k]['is_pro_sale'] = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' . $GLOBALS['yp']->table('goods_activity') . ' WHERE goods_id = ' . $v['goods_id']);
        // 查询商品是否参加优惠活动
...
function virtual_goods_list($is_delete, $real_goods=1, $conditions = '')
{
	...
	if($filter['city']){
       ...
       if ($real_goods > -1)
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['yp']->table('goods'). " AS g, "
                   .$GLOBALS['yp']->table('virtual_district')." as dis, ".
                        $GLOBALS['yp']->table('virtual_goods_district')." as dig"
...
//今/天/优/品/多/商/户/系/统-采集 mod on 20160808 by prince  qq 1/2/0/0/2/9/1/2/1-end
                    ...
                    " FROM " . $GLOBALS['yp']->table('goods') . " AS g ".
                    " LEFT JOIN " . $GLOBALS['yp']->table('supplier') . " AS s ON s.supplier_id = g.supplier_id ".
                     "left join ". $GLOBALS['yp']->table('virtual_district') ." as dis on dis.goods_id=g.goods_id ".
                   "left join ".$GLOBALS['yp']->table('virtual_goods_district')."  as dig on dis.district_id = dig.district_id".
                    " WHERE is_delete='$is_delete' $where" .
                    " ORDER BY $filter[sort_by] $filter[sort_order] ".
                    " LIMIT " . $filter['start'] . ",$filter[page_size]";
        }else{
...
//今/天/优/品/多/商/户/系/统-采集 mod on 20160808 by prince  qq 1/2/0/0/2/9/1/2/1-end
                    " (promote_price > 0 AND promote_start_date <= '$today' AND promote_end_date >= '$today') AS is_promote ".
					", supplier_status, g.supplier_id ".	//代码增加   By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121
                    " FROM " . $GLOBALS['yp']->table('goods') . " AS g ".
                    "left join ". $GLOBALS['yp']->table('virtual_district') ." as dis on dis.goods_id=g.goods_id ".
                   "left join ".$GLOBALS['yp']->table('virtual_goods_district')."  as dig on dis.district_id = dig.district_id ".
                     " WHERE is_delete='$is_delete' $where" .
...
function get_goods_specifications_list($goods_id)
{
    ...
    $sql = "SELECT g.goods_attr_id, g.attr_value, g.attr_id, a.attr_name
            FROM " . $GLOBALS['yp']->table('goods_attr') . " AS g
                LEFT JOIN " . $GLOBALS['yp']->table('attribute') . " AS a
...
function product_list($goods_id, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['yp']->table('products'). " AS p WHERE goods_id = $goods_id $where";
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        ////采集 mod on 20160808 by PRINCE  qq 1 2 0 0 2 9 1 2 1 增加：属性价格pro_price//
        $sql = "SELECT product_id, pro_price, goods_id, goods_attr, product_sn, product_number
                FROM " . $GLOBALS['yp']->table('products') . " AS g
...
function get_product_info($product_id, $filed = '')
{
    ...
    $sql = "SELECT $filed FROM  " . $GLOBALS['yp']->table('products') . " WHERE product_id = '$product_id'";
...
function check_goods_specifications_exist($goods_id)
{
    ...
    $sql = "SELECT COUNT(a.attr_id)
            FROM " .$GLOBALS['yp']->table('attribute'). " AS a, " .$GLOBALS['yp']->table('goods'). " AS g
...
function check_goods_attr_exist($goods_attr, $goods_id, $product_id = 0)
{
    ...
    if (empty($product_id))
    {
        $sql = "SELECT product_id FROM " . $GLOBALS['yp']->table('products') ."
...
function check_product_sn_exist($product_sn, $product_id = 0)
{
    ...
    $sql="SELECT goods_id FROM ". $GLOBALS['yp']->table('goods')."WHERE goods_sn='$product_sn'";
...
function spec_price($spec)
{
    if (!empty($spec))
    {
        ...
        $sql = 'SELECT SUM(attr_price) AS attr_price FROM ' . $GLOBALS['yp']->table('goods_attr') . " WHERE $where";
