<?php
/**
 * QQ120029121 销售报告：销售统计
 * ============================================================================
 * 演示地址: http://demo.coolhong.com/
 * ============================================================================
 * $Author: jtypmall $
 * $Id: sell_stats.php 2015-10-23 13:00:08Z jtypmall $
 */
...
if ($_REQUEST['act'] == 'list')
{
    ...
    $order_money = $db->getOne(
        ...
        . $yp->table('order_info') . ' o ' . $where
    );
    $order_money = empty($order_money) ? 0 : $order_money;

    // 下单量
    $order_count = $db->getOne(
        'SELECT COUNT(*) FROM ' . $yp->table('order_info') . ' o ' . $where
    );
    ...
    $order_money_list = $db->getAll(
        ...
        . $yp->table('order_info') . ' o '  . $where . ' GROUP BY date ORDER BY date'
    );
...
function export($start_date, $end_date, $sel_shop, $supplier_id, $_LANG)
{
    ...
    $result = $GLOBALS['db']->getAll(
        ...
        . $GLOBALS['yp']->table('order_info') . ' o, ' . $GLOBALS['yp']->table('users') . ' u '
...
function get_result_list($start_date, $end_date, $sel_shop, $supplier_id)
{
    ...
    if($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('order_info') . ' o ' . $where;
