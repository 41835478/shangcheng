 ...
 * ==========================================================
 * $Author: jtypmall $
 * $Id: webcollect.php 16131 2009-05-31 08:21:41Z jtypmall $
 */
...
if (!empty($license['certificate_id']) && !empty($license['token']) && !empty($license['certi']))
{
    ...
    $certi_login['app_id'] = 'qq120029121_b2c'; // 说明客户端来源
    $certi_login['app_instance_id'] = 'cert_auth'; // 应用服务ID
    $certi_login['version'] = VERSION . '#' .  RELEASE; // 网店软件版本号
    $certi_login['certi_url'] = sprintf($GLOBALS['yp']->url()); // 网店URL
    $certi_login['certi_session'] = $GLOBALS['sess']->get_session_id(); // 网店SESSION标识
    $certi_login['certi_validate_url'] = sprintf($GLOBALS['yp']->url() . 'certi.php'); // 网店提供于官方反查接口
    $certi_login['format'] = 'json'; // 官方返回数据格式
    $certi_login['certificate_id'] = $license['certificate_id']; // 网店证书ID
    $certi_login['certi_ac'] = make_qq120029121_ac($certi_login, $license['token']); // 网店验证字符串
