...
elseif ($_REQUEST['act'] == 'operate1')
{
	...
	//获取所有可以结算的订单
	$sql = "update " . $GLOBALS['yp']->table('order_info') . " set rebate_ispay=2 where rebate_id=".$rebid." and order_sn in(".$_REQUEST['order_id'].")";
	...
	if($GLOBALS['db']->query($sql)){
		...
		$db->autoExecute($yp->table('supplier_rebate_log'), $rebate_order, 'INSERT');
		if(changeStatus($rebid)){
			//记录用户资金日志
			
			//修改佣金状态
			$db->query("update ".$yp->table('supplier_rebate')." set status=1 where rebate_id=".$rebid);
			//修改佣金信息状态记录
			$rebate_list = array(
				'rebateid' => $rebid,
				'username' => '平台方:'.$_SESSION['user_name'],
			...
			$db->autoExecute($yp->table('supplier_rebate_log'), $rebate_list, 'INSERT');
		}
...
elseif ($_REQUEST['act'] == 'operate2')
{
	...
	//入驻商资金添加日志
	writelog($rebid,1);
	$sql = "update " . $GLOBALS['yp']->table('order_info') . " set rebate_ispay=1 where rebate_id=".$rebid." and rebate_ispay=2";
	$links[] = array('href' => 'supplier_rebate.php?act=list', 'text' => '查看本期佣金列表');
	if($GLOBALS['db']->query($sql)){
		...
		$db->query("update ".$yp->table('supplier_rebate')." set status=0 where rebate_id=".$rebid);
		//修改佣金信息状态记录
		$rebate_list = array(
				'rebateid' => $rebid,
				'username' => '平台方:'.$_SESSION['user_name'],
...
function getOkOrder(){
    ...
    if ($result === false)
    {
		...
		//总数
		$sql = "select count(order_id) from ".$yp->table('order_info')." where ".$and;
		...
		$sql = "select order_id, order_sn, add_time, order_status, shipping_status, order_amount, money_paid,".
			...
			"from ".$yp->table('order_info')." where ".$and." LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ",$filter[page_size]";
		//echo $sql;
...
function getBackHuanOrder(){
    ...
    if ($result === false)
    {
		...
		$sql = "select count(oi.order_id) " .
			"from ".$yp->table('order_info')." as oi,".$yp->table('back_order')." as bo where ".$and;
		$filter['record_count']   = $GLOBALS['db']->getOne($sql);
		$filter['page_count']     = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;
		...
		$sql = "select oi.order_id, oi.order_sn, oi.add_time, oi.order_status, oi.shipping_status, oi.order_amount, oi.money_paid,".
			...
			"from ".$yp->table('order_info')." as oi,".$yp->table('back_order')." as bo where ".$and." LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ",$filter[page_size]";
		//echo $sql;
		set_filter($filter, $sql);
	}
    else
...
function getBackOrderMoney(){
	...
	$sql = "select bo.status_refund,(" . order_amount_field('oi.') . ") AS total_fee,oi.pay_id " .
			"from ".$yp->table('order_info')." as oi,".$yp->table('back_order')." as bo where oi.rebate_id=".$rid." and bo.back_type!=3 and bo.status_back < 5 and oi.order_id=bo.order_id";
	$query = $db->query($sql);
...
function changeStatus($rid){
	...
	$sql = "select oi.order_id,oi.order_status,oi.shipping_status,oi.pay_status,oi.shipping_time,IFNULL(bo.back_type,'-1') as backtype from ".$yp->table('order_info')." as oi left join ".$yp->table('back_order')." as bo on oi.order_id=bo.order_id and bo.status_back < 6 where oi.rebate_id=".$rid." and oi.rebate_ispay=1";
	$info = $db->getAll($sql);
	if(empty($info)){
		//全部订单已经返佣
		return true;
