...
function action_default ()
{
	...
	// 检查shop_config
	$sql = "select * from " . $yp->table("shop_config") . " where code = 'chat'";
	
	$row = $db->getRow($sql, true);
	
	if($row == false)
	{
		$sql = "select max(parent_id) from " . $yp->table("shop_config") . "";
		$parent_id = $db->getOne($sql) + 1;
		$chat = array(
			"id" => $parent_id, "code" => "chat", "parent_id" => 0, "type" => "group", "value" => ""
		);
		$db->autoExecute($yp->table('shop_config'), $chat, 'INSERT');
	}
	else
	{
		$parent_id = $row['id'];
	...
	$sql = "select * from " . $yp->table("shop_config") . " where parent_id = '" . $parent_id . "'";
	$rows = $db->getAll($sql);
...
function action_post ()
{
	...
	foreach($chat as $key => $value)
	{
		...
		if(isset($records[$key]))
		{
			...
			{
				$db->autoExecute($yp->table('shop_config'), $record, 'UPDATE', "id = '$id'");
			}
		}
		else
		{
			$db->autoExecute($yp->table('shop_config'), $record, 'INSERT', "parent_id = '$parent_id'");
		}
	}
