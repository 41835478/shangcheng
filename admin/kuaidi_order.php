 ...
 * ============================================================================
 * $Author:prince  $
 * $Id: takegoods.php 17217 2017-04-01 06:29:08Z prince $
*/
...
/* 初始化$exc对象 */
$exc = new exchange($yp->table('kuaidi_order'), $db, 'order_id', 'order_sn');
...
if ($_REQUEST['act'] == 'set_postman')
{
	...
	$sql="select * from ". $yp->table('kuaidi_order') ." where order_id='$order_id' ";
	$order = $db->getRow($sql);

	if (empty($order['send_name']))
	{
		$shop_name    = $db->getRow("select value from ".$yp->table('shop_config')." where code='shop_name'");
		$shop_city_id = $db->getRow("select value from ".$yp->table('shop_config')." where code='shop_city'");
		$shop_city    = $db->getRow("select region_name from ".$yp->table('region')." where region_id=".$shop_city_id['value']);
		$shop_address = $db->getRow("select value from ".$yp->table('shop_config')." where code='shop_address'");
		$shop_tel     = $db->getRow("select value from ".$yp->table('shop_config')." where code='service_phone'");
		...
		$best_time_info = $db->getRow("select best_time from ".$yp->table('delivery_order')." where delivery_id=".$order['goods_name']);
...
if ($_REQUEST['act'] == 'set_postman_save')
{
	...
	$sql = "update ". $yp->table('kuaidi_order') ." set postman_id= '$postman_id', order_status='2' where order_id='$order_id' ";
	$db->query($sql);
	$sql = "update ". $yp->table('kuaidi_order_status')." set status_display='1' where order_id='$order_id' AND status_id in (1,2) ";
	$db->query($sql);
	$sql2 = "update ". $yp->table('kuaidi_order_status')." set status_time='".gmtime()."' where order_id='$order_id' AND status_id=2 ";
	$db->query($sql2);	
    /*短信通知快递员*/
   $sql = "SELECT `mobile` FROM ". $yp->table('postman')." WHERE `postman_id`= '$postman_id'";
   $postman_mobile =  $GLOBALS['db'] -> getOne($sql);
if($postman_mobile){//发送短信通知
	   /* 获取快递单数据 */
    $order = $db->getRow("SELECT k.*, u.user_name, p.postman_name FROM " .$yp->table('kuaidi_order'). " AS k ".
											"left join ". $yp->table('users') ." AS u on k.user_id=u.user_id ".
											"left join ". $yp->table('postman') ." AS p on k.postman_id=p.postman_id ".
	...
	$content = $order['send_name'].'提醒您：发货商家：'.$order['send_name'].'），收件人：'.$order['to_name'].'，地址：'.$order['to_region_name'].'，电话（'.$order['to_tel'].'）。寄件人信息：区域（'.$order['send_region_name'].'），包裹数（'.$order['package_num'].'），重量（'.$order['goods_weight'].'），'.$order['goods_type'].'，联系人（'.$order['send_name'].'），联系电话（'.$order['send_tel'].'）。期望送货时间：'.$order['send_time'].'。请您尽快取件派送！【'.$_CFG['sms_sign'].'】';
	sendSMS($postman_mobile,$content,'', '',$templateParam , $templateCode);//暂不支持阿里大于 QQ 120029121
	}
...
if ($_REQUEST['act'] == 'change_PostmanBox')
{
	...
	$sql="select * from ". $yp->table('postman') ." where region_id='$region_id' ";
...
if ($_REQUEST['act'] == 'list')
{
    ...
    if ($list === false){//20170920  寒冰 qq309485552   修复后台快递单列表 mysql 语句报错
      /* 提示信息 */
     $link[] = array('text' => '返回商店设置', 'href' => 'shop_config.php?act=list_edit');
     sys_msg('出错！请先设置商店所在城市。', 1, $link);
...
if ($_REQUEST['act'] == 'view')
{
	...
	$sql = "SELECT MAX(order_id) FROM " . $yp->table('kuaidi_order') . " WHERE order_id < '$order_id' ";
    $smarty->assign('prev_id', $db->getOne($sql));
    $sql = "SELECT MIN(order_id) FROM " . $yp->table('kuaidi_order') . "  WHERE order_id > '$order_id' ";
...
if ($_REQUEST['act'] == 'update')
{
    ...
    $sql = "UPDATE " .$yp->table('kuaidi_order'). " SET ".
...
if ($_REQUEST['act'] == 'batch')
{
    ...
    if (is_array($ids))
	{
				...
				{
						$sql="select * FROM " .$yp->table('kuaidi_order'). " AS k ".
								" left join ". $yp->table('users') ." AS u on k.user_id= u.user_id ".
								 " left join ". $yp->table('postman') ." AS p on k.postman_id=p.postman_id ".
						...
						while ($row_kd = $db->fetchRow($res_kd))
						{
							...
							if (empty($row_kd['send_name']))
							{
								...
								$best_time_info = $db->getRow("select best_time from ".$yp->table('delivery_order')." where delivery_id=".$row_kd['goods_name']);
				...
				{
						$id_list_nocancel ="";
						$sql="select order_id,order_status from ".$yp->table('kuaidi_order')." WHERE order_id ". db_create_in($ids);	
						$res = $db->query($sql);
						...
						$sql="update  " .$yp->table('kuaidi_order'). " set order_status='7' WHERE order_id ". db_create_in($ids);	
						$db->query($sql);
						foreach ($ids AS $o_id)
					    {
				...
				{
						$sql="DELETE FROM " .$yp->table('kuaidi_order'). " WHERE order_id ". db_create_in($ids);	
						$db->query($sql);
						$sql="DELETE FROM " .$yp->table('kuaidi_order_status'). " WHERE order_id ". db_create_in($ids);	
						$db->query($sql);
						$link[] = array('text' => $_LANG['back_order_list'], 'href' => 'kuaidi_order.php?act=list&' . list_link_postfix());
						sys_msg($_LANG['attradd_succed'], 0, $link);
				}
				...
				{
						$id_list_nocancel ="";
						$sql="select order_id,postman_id from ".$yp->table('kuaidi_order')." WHERE order_id ". db_create_in($ids);	
						$res = $db->query($sql);
						while($row=$db->fetchRow($res))
						{
								if (!$row['postman_id']) 
						...
						$sql="update  " .$yp->table('kuaidi_order'). " set order_status='$order_status' WHERE order_id ". db_create_in($ids);	
						$db->query($sql);
						foreach ($ids AS $o_id)
					    {
...
function get_district_list()
{
	...
	{
		$sql="select * from ". $GLOBALS['yp']->table('region') ." where parent_id= '" . $GLOBALS['_CFG']['shop_city'] . "' ";
...
//    $filter['sort_order'] = empty($_REQUEST['sort_order']) ? 'DESC' : trim($_REQUEST['sort_order']);
    ...
    $sql = "SELECT COUNT(*) FROM ".$GLOBALS['yp']->table('kuaidi_order')." AS k ".
				" left join ". $GLOBALS['yp']->table('postman') ." AS p on k.postman_id=p.postman_id ".
		         $where;
    ...
    $sql = "SELECT k.*, p.postman_name, u.user_name ".
          " FROM ".$GLOBALS['yp']->table('kuaidi_order'). " AS k ".
		" left join ". $GLOBALS['yp']->table('users') ." AS u on k.user_id= u.user_id ".
		" left join ". $GLOBALS['yp']->table('postman') ." AS p on k.postman_id = p.postman_id ".
	...
	foreach($row AS $row_key =>$row_val)
	{
		...
		{
			$shop_name    = $GLOBALS['db']->getRow("select value from ".$GLOBALS['yp']->table('shop_config')." where code='shop_name'");
			$shop_city_id = $GLOBALS['db']->getRow("select value from ".$GLOBALS['yp']->table('shop_config')." where code='shop_city'");

			if (empty($shop_city_id['value'])) {//20170920  寒冰 qq309485552   修复后台快递单列表 mysql 语句报错
				return false;
				exit();
			}
			
			$shop_city    = $GLOBALS['db']->getRow("select region_name from ".$GLOBALS['yp']->table('region')." where region_id=".$shop_city_id['value']);
			$shop_address = $GLOBALS['db']->getRow("select value from ".$GLOBALS['yp']->table('shop_config')." where code='shop_address'");
			$shop_tel     = $GLOBALS['db']->getRow("select value from ".$GLOBALS['yp']->table('shop_config')." where code='service_phone'");
			...
			$best_time_info = $GLOBALS['db']->getRow("select best_time from ".$GLOBALS['yp']->table('delivery_order')." where delivery_id=".$row_val['goods_name']);
...
{
	$region_info = $GLOBALS['db']->getRow('select region_name, parent_id from '. $GLOBALS['yp']->table('region') .' where region_id='. $region_id);
	return $GLOBALS['db']->getOne('select region_name from '. $GLOBALS['yp']->table('region') .' where region_id='. $region_info['parent_id'])." ".$region_info['region_name'];
}

function set_order_status($o_id, $o_status)
{
		if (!$o_status ) {return false;}
		$sql = "update ". $GLOBALS['yp']->table('kuaidi_order_status'). " set  status_display = '0'  where   order_id='$o_id' ";
		...
		if ($o_type == '0' || $o_type == '1')
		{
				...
				{
						$sql = "update ". $GLOBALS['yp']->table('kuaidi_order_status')." set  status_display='1'  where  status_id = '$i' and order_id='$o_id' ";
						$GLOBALS['db']->query($sql);
				}
				$sql = "update ". $GLOBALS['yp']->table('kuaidi_order_status')." set status_time='".gmtime()."' where  status_id = '$i' and order_id='$o_id' ";
				$GLOBALS['db']->query($sql);
		...
		else if($o_type == '2')
		{
				for($i=1;$i<=$o_status; $i++)
				{
					...
					$sql = "update ". $GLOBALS['yp']->table('kuaidi_order_status')." set  status_display='$display_val'  where  status_id = '$i' and order_id='$o_id' ";
					$GLOBALS['db']->query($sql);
				}
				$sql = "update ". $GLOBALS['yp']->table('kuaidi_order_status')." set status_time='".gmtime()."'  where  status_id = '$i' and order_id='$o_id' ";
				$GLOBALS['db']->query($sql);
		...
		{				
			$sql = "update ". $GLOBALS['yp']->table('kuaidi_order_status')." set  status_display='1',status_time='".gmtime()."'  where status_id = '7' and order_id='$o_id' ";
			$GLOBALS['db']->query($sql);				
		}
}
?>