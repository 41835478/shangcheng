...
/**
 * QQ120029121 管理中心信息管理
 * ============================================================================
 * 演示地址: http://demo.coolhong.com  开发QQ:120029121    309485552
 * ============================================================================
 * $Author: PRINCE $
 * $Id: brand.php 17217 2017-04-01 06:29:08Z PRINCE $
*/

define('IN_PRINCE', true);

require(dirname(__FILE__) . '/includes/init.php');
$exc = new exchange($yp->table("shaidan"), $db, 'shaidan_id', 'title');
...
elseif ($_REQUEST['act'] == 'edit')
{
	...
	$goods = $db->GetRow("SELECT * FROM " .$yp->table('order_goods'). " WHERE rec_id='$info[rec_id]'");
...
elseif ($_REQUEST['act'] == 'updata')
{
	...
	if ($get_points == 1 && $pay_points > 0)
	{
		...
		$db->query("INSERT INTO ".$yp->table('account_log')."(user_id, rank_points, pay_points, change_time, change_desc, change_type) ".
	...
	$db->query("UPDATE ".$yp->table('shaidan')." SET status = '$status' WHERE shaidan_id = '$shaidan_id'");
	
    $link[0]['text'] = '返回列表';
    $link[0]['href'] = 'shaidan.php?act=list';
    sys_msg('操作成功',0, $link);
}
elseif ($_REQUEST['act'] == 'del')//删除   20170116   寒冰  qq   309485552  
{
    $shaidan_id = isset($_REQUEST['id']) ? trim($_REQUEST['id']) : 0;
	if ($shaidan_id)
	{
		$db->query("DELETE FROM".$yp->table('shaidan')." WHERE shaidan_id = '$shaidan_id'");
		$db->query("DELETE FROM".$yp->table('shaidan_img')." WHERE shaidan_id = '$shaidan_id'");
		$link[0]['text'] = '返回列表';
        $link[0]['href'] = 'shaidan.php?act=list';
        sys_msg('操作成功',0, $link);
	}else{
...
function get_shaidanlist()
{
    ...
    if ($result === false)
    {
        ...
        $sql = 'SELECT a.* , u.user_name, og.goods_name '.
               'FROM ' .$GLOBALS['yp']->table('shaidan'). ' AS a '.
               'LEFT JOIN ' .$GLOBALS['yp']->table('users'). ' AS u ON u.user_id = a.user_id '.
			   'LEFT JOIN ' .$GLOBALS['yp']->table('order_goods'). ' AS og ON og.rec_id = a.rec_id '.
