...
if ($_REQUEST['act'] == 'replenish')
{
    ...
    {
        $goods_name = $db->GetOne("SELECT goods_name From ".$yp->table('goods')." WHERE goods_id='".$_REQUEST['goods_id']."' AND is_real = 0 AND extension_code='virtual_card' ");
        if (empty($goods_name))
        {
...
elseif ($_REQUEST['act'] == 'edit_replenish')
{
    ...
    $sql = "SELECT T1.card_id, T1.goods_id, T2.goods_name,T1.card_sn,  T1.end_date  FROM ".
            $yp->table('virtual_goods_card')." AS T1, ".$yp->table('goods')." AS T2 ".
...
elseif ($_REQUEST['act'] == 'action')
{
    ...
    admin_priv('virtual_card_list');
        ...
        $sql = "UPDATE ".$yp->table('virtual_goods_card')." SET end_date='$end_date' ".
...
if($_REQUEST['act']=='verification_info'){
    ...
    if(!empty($card_sn)){
        $sql = "select v.*,g.goods_name,g.goods_thumb,g.shop_price from ".$yp->table('virtual_goods_card')."as v left join ".$yp->table('goods')." as g on v.goods_id= g.goods_id where v.card_sn = '$card_sn' and v.is_saled = 1 and v.supplier_id = $supplier_id";
        $res = $db -> getRow($sql);
        if($res){
...
if($_REQUEST['act']=='verification'){
    ...
    if(!empty($card_sn)){
        ...
        $sql = "select * from ".$yp->table('virtual_goods_card')." where card_sn = '$card_sn' and is_saled = 1 and supplier_id = $supplier_id";
        $res = $db -> getRow($sql);
        ...
        if($res){
            if($res['is_verification'] == '0'){
                ...
                if($end_date >= $now){
                    ...
                    $sql = "update ".$yp->table('virtual_goods_card')." set is_verification = 1 , buy_date = ".gmtime()." where card_sn = '$card_sn'";
                    $db -> query($sql);
                    /* 改变订单状态 */
                    $sql = "select count(*) from ".$yp->table('virtual_goods_card')." where card_sn = '$card_sn' and  supplier_id = $supplier_id and is_verification=1";
                    if($db->getOne($sql)){
                        $sql = "update ".$yp->table('order_info')." set order_status = '5', shipping_status = '2' , shipping_time_end = ".gmtime()." where order_sn = '$res[order_sn]'";
                    }else{
                        $sql = "update ".$yp->table('order_info')." set shipping_status = '2' , shipping_time_end = ".gmtime()." where order_sn = '$res[order_sn]'";
                    }
                    $db -> query($sql);
                }else{
                    $result['msg'] = '3';
    ...
    $goods = $db -> getRow("select goods_name,goods_thumb,shop_price from ".$yp->table('goods')." where goods_id = $res[goods_id]");
...
elseif ($_REQUEST['act'] == 'batch_drop_card')
{
    ...
    $sql = "DELETE FROM ".$yp->table('virtual_goods_card')." WHERE card_id ".db_create_in(implode(',',$_POST['checkboxes']));
    if ($db->query($sql))
    {
        /* 商品数量减$num */
...
elseif ($_REQUEST['act'] == 'del_virtual_order')
{
    ...
    foreach($order_ids as $order_id){
         $sql = " update ".$yp->table('order_info')." set order_status = ".OS_INVALID." WHERE order_id = $order_id";
         $db->query($sql); 
         $order_sn = $db->getOne("select order_sn from ".$yp->table('order_info')." where order_id = $order_id");
         $db->query("update ".$yp->table('virtual_goods_card')." set is_saled = 0 where order_sn = '$order_sn'"); 
    }
        //设置验证码为未出售
...
elseif ($_REQUEST['act'] == 'batch_insert')
{
    ...
    foreach ($_POST['checked'] as $key)
    {
        ...
        $db->AutoExecute($yp->table('virtual_goods_card'), $rec, 'INSERT');
        $i++;
    }
...
elseif ($_REQUEST['act'] == 'submit_change')
{
    ...
    if (isset($_POST['old_string']) && isset($_POST['new_string']))
    {
        ...
        $sql = "SELECT card_id, card_sn, card_password FROM " . $yp->table('virtual_goods_card') . " WHERE crc32 = '$old_crc32'";
        $res = $db->query($sql);
        while ($row = $db->fetchRow($res))
        {
            ...
            $row['crc32'] = $new_crc32;
            $db->autoExecute($yp->table('virtual_goods_card'), $row, 'UPDATE', 'card_id = ' . $row['card_id']);
        }
...
elseif ($_REQUEST['act'] == 'toggle_sold')
{
    ...
    $sql = "UPDATE ".$yp->table('virtual_goods_card')." SET is_saled= '$val' WHERE card_id='$id'";

    if ($db->query($sql, 'SILENT'))
    {
        /* 修改商品库存 */
        $sql = "SELECT goods_id FROM " . $yp->table('virtual_goods_card') . " WHERE card_id = '$id' LIMIT 1";
...
elseif ($_REQUEST['act'] == 'remove_card')
{
    ...
    $row = $db->GetRow('SELECT card_sn, goods_id FROM ' . $yp->table('virtual_goods_card') . " WHERE card_id = '$id'");

    $sql = 'DELETE FROM ' . $yp->table('virtual_goods_card') . " WHERE card_id = '$id'";
    if ($db->query($sql, 'SILENT'))
    {
        /* 修改商品数量 */
...
elseif ($_REQUEST['act'] == 'start_change')
{
    ...
    $sql = "SELECT crc32, count(*) AS cnt FROM " . $yp->table('virtual_goods_card') . " GROUP BY crc32";
...
elseif ($_REQUEST['act'] == 'on_change')
{
   ...
   $sql = "SELECT card_id, card_sn, card_password ".
            " FROM " . $yp->table('virtual_goods_card') .
   ...
   while ($row = $db->fetchRow($res))
   {
       ...
       if (!$db->autoExecute($yp->table('virtual_goods_card'), $row, 'UPDATE', 'card_id = ' . $row['card_id']))
       {
    ...
    // 查询是否还有未更新的
    $sql      = "SELECT COUNT(*) FROM " . $yp->table('virtual_goods_card') . " WHERE crc32 = '$old_crc32' ";
    ...
    else
    {
        ...
        $sql = "SELECT crc32, count(*) AS cnt FROM " . $GLOBALS['yp']->table('virtual_goods_card') . " GROUP BY crc32";
        $res = $GLOBALS['db']->query($sql);
        while ($row = $db->fetchRow($res))
        {
...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    $sql = "SELECT MAX(order_id) FROM " . $yp->table('order_info') . " as o WHERE order_id < '$order[order_id]' and extension_code='virtual_good'";
    if ($agency_id > 0)
    {
    ...
    $sql = "SELECT MIN(order_id) FROM " . $yp->table('order_info') . " as o WHERE order_id > '$order[order_id]' and extension_code='virtual_good'";
    if ($agency_id > 0)
    {
        $sql .= " AND agency_id = '$agency_id'";
    }
...
function get_virtual_order_list(){    
  ...
  //  $where .= ($filter['is_verification'] !='') ? " AND is_verification = '" . $filter['is_verification'] . "' " : '';
    ...
    $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('order_info') . " as a  WHERE 1  AND extension_code = 'virtual_good'  and order_status <> '".OS_INVALID."' " .$where;
    ...
    /* 查询 */
       $sql = "select a.order_id, a.order_sn, a.order_status, a.pay_status,(". order_amount_field('a.') . ") AS total_fee, b.goods_price, d.supplier_id, c.supplier_name, f.user_name  from ". $GLOBALS['yp']->table('order_info') .
            "as a left join ". $GLOBALS['yp']->table('order_goods') ." as b on a.order_id = b.order_id".
            " left join ". $GLOBALS['yp']->table('goods')." as d on d.goods_id = b.goods_id".
            " left join ".$GLOBALS['yp']->table('supplier')." as c on c.supplier_id = d.supplier_id".
            " left join ".$GLOBALS['yp']->table('users')." as f on f.user_id = a.user_id".
...
function get_replenish_list()
{
    ...
    $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('virtual_goods_card') . " WHERE 1 $where";
    $filter['record_count'] = $GLOBALS['db']->getOne($sql);
    ...
    $sql = "SELECT card_id, goods_id, card_sn, add_date, end_date, buy_date, is_saled, order_sn, is_verification".
            " FROM ".$GLOBALS['yp']->table('virtual_goods_card').
            " WHERE 1 ".$where.
...
{
    $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('virtual_goods_card') . " WHERE goods_id = '$goods_id' AND is_saled = 0";
    $goods_number = $GLOBALS['db']->getOne($sql);

    $sql = "UPDATE " . $GLOBALS['yp']->table('goods') . " SET goods_number = '$goods_number' WHERE goods_id = '$goods_id' AND extension_code='virtual_card'";
...
function get_orderid_by_ordersn($order_sn){
    $sql = "select a.order_id, b.goods_price, d.supplier_id, c.supplier_name, f.user_name  from ". $GLOBALS['yp']->table('order_info') .
            "as a left join ". $GLOBALS['yp']->table('order_goods') ." as b on a.order_id = b.order_id".
            " left join ". $GLOBALS['yp']->table('goods')." as d on d.goods_id = b.goods_id".
            " left join ".$GLOBALS['yp']->table('supplier')." as c on c.supplier_id = d.supplier_id".
            " left join ".$GLOBALS['yp']->table('order_info')." as e on e.order_id = a.order_id".
            " left join ".$GLOBALS['yp']->table('users')." as f on f.user_id = e.user_id".
...
function get_supplier_list(){
    $sql = "select supplier_id, supplier_name from ". $GLOBALS['yp']->table('supplier')." where status=1";
...
function get_order_card($order_sn){
    $sql = "SELECT card_id, goods_id, card_sn, add_date, end_date, buy_date, is_saled, is_verification".
          " FROM ".$GLOBALS['yp']->table('virtual_goods_card')." where order_sn = '".$order_sn."'";
