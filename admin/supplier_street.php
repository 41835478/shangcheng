...
$exc = new exchange($yp->table("supplier_street"), $db, 'supplier_id', 'supplier_name');
...
elseif ($_REQUEST['act'] == 'edit_info')
{
	...
	$info = $db->getRow("select * from ".$yp->table('supplier_street')." where supplier_id=".$suppid);
...
elseif($_REQUEST['act'] == 'saveinfo')
{
	...
	if ($db->autoExecute($yp->table('supplier_street'), $save, 'UPDATE', "supplier_id='$suppid'")){
...
elseif ($_REQUEST['act'] == 'remove_show')
{
	admin_priv('supplier_manage');
        ...
        /* 删除退货单 */
            $sql = "UPDATE ".$yp->table('supplier_street'). " set is_show = 0 WHERE supplier_id in ($supp_id)";
            $db->query($sql);    
...
elseif ($_REQUEST['act'] == 'remove_supplier')
{
	admin_priv('supplier_manage');
        ...
        if(is_array($supp_id))
        {
            ...
            $sql = "DELETE FROM ".$yp->table('supplier_street'). " WHERE supplier_id in ($supp_id_list)";
            $db->query($sql);    
        }
        else
        {
            $sql = "DELETE FROM ".$yp->table('supplier_street'). " WHERE supplier_id in($supp_id)";			
            $db->query($sql);
        }
		//echo $sql;
...
if ($_REQUEST['act'] == 'toggle_tag')
{
	...
	if($val>0){
		...
		$sql = "INSERT INTO ".$yp->table('supplier_tag_map')." (tag_id,supplier_id) VALUES (".$tid.",".$sid.") ON DUPLICATE KEY UPDATE tag_id=".$tid.",supplier_id=".$sid;
	}else{
		//删除店铺的对应标签记录
		$sql = "delete from ".$yp->table('supplier_tag_map')." where tag_id=".$tid." and supplier_id=".$sid;
	}
...
function str_update($cat_id, $args)
{
    ...
    return $GLOBALS['db']->autoExecute($GLOBALS['yp']->table('supplier_street'), $args, 'update', "supplier_id='$cat_id'");
}
...
{
    $sql = "SELECT * FROM " .$GLOBALS['yp']->table('street_category'). " WHERE str_id='$cat_id' LIMIT 1";
...
function get_street_list()
{
    ...
    if ($result === false)
    {
	    ...
	    if($_REQUEST['check'] !== false){
	     ...
	     /* 记录总数 */
	     $sql = "SELECT COUNT(*) FROM " .$GLOBALS['yp']->table('supplier_street'). " as ss $where";
	     $filter['record_count'] = $GLOBALS['db']->getOne($sql);
	     $filter['page_count']     = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;
	        
	    $sql = "SELECT ss.*,sc.str_name ".
	           " FROM " . $GLOBALS['yp']->table('supplier_street'). " AS ss ".
	           " LEFT JOIN" . $GLOBALS['yp']->table('street_category'). " AS sc ".
...
function get_tag(){
	global $db,$yp;
	$ret = array();
	$sql = "select tag_id,tag_name from ".$yp->table('supplier_tag')." where is_groom=1 order by sort_order";
	$query = $db->query($sql);
...
function get_tag_map($suppid){
	...
	$ret = array();
	$tag_info = get_tag();
	$sql = "select stm.tag_id,st.tag_name from ".$yp->table('supplier_tag_map')." as stm left join ".$yp->table('supplier_tag')." as st on stm.tag_id=st.tag_id where supplier_id=".$suppid;
	$info = $db->getAll($sql);
	if(empty($info)){
		return $tag_info;
	}else{
