...
/**
 * QQ120029121 销售明细列表程序
 * ============================================================================
 * 演示地址: http://demo.coolhong.com  开发QQ:120029121    309485552
 * ============================================================================
 * $Author: PRINCE $
 * $Id: sale_list.php 17217 2017-04-01 06:29:08Z PRINCE $
*/
...
if (isset($_REQUEST['act']) && ($_REQUEST['act'] == 'query' ||  $_REQUEST['act'] == 'download'))
{
    ...
    if ($_REQUEST['act'] == 'download')
    {
        ...
        /* 文件标题 */
        echo yp_iconv(YP_CHARSET, 'GB2312', $_REQUEST['start_date']. $_LANG['to'] .$_REQUEST['end_date']. $_LANG['sales_list']) . "\t\n";

        /* 商品名称,订单号,商品数量,销售价格,销售日期 */
        echo yp_iconv(YP_CHARSET, 'GB2312', $_LANG['goods_name']) . "\t";
        echo yp_iconv(YP_CHARSET, 'GB2312', $_LANG['order_sn']) . "\t";
        echo yp_iconv(YP_CHARSET, 'GB2312', $_LANG['amount']) . "\t";
        echo yp_iconv(YP_CHARSET, 'GB2312', $_LANG['sell_price']) . "\t";
        echo yp_iconv(YP_CHARSET, 'GB2312', $_LANG['sell_date']) . "\t\n";

        foreach ($goods_sales_list['sale_list_data'] AS $key => $value)
        {
            echo yp_iconv(YP_CHARSET, 'GB2312', $value['goods_name']) . "\t";
            echo yp_iconv(YP_CHARSET, 'GB2312', '[ ' . $value['order_sn'] . ' ]') . "\t";
            echo yp_iconv(YP_CHARSET, 'GB2312', $value['goods_num']) . "\t";
            echo yp_iconv(YP_CHARSET, 'GB2312', $value['sales_price']) . "\t";
            echo yp_iconv(YP_CHARSET, 'GB2312', $value['sales_time']) . "\t";
            echo "\n";
        }
        exit;
    }
...
function get_sale_list($is_pagination = true){
    ...
    $sql = 'SELECT og.goods_id, og.goods_sn, og.goods_name, og.goods_number AS goods_num, og.goods_price '.
           ...
           "FROM " . $GLOBALS['yp']->table('order_goods')." AS og, ".$GLOBALS['yp']->table('order_info')." AS oi ".
