...
$exc   = new exchange($yp->table("ad"), $db, 'ad_id', 'ad_name');
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    /* 查看广告名称是否有重复 */
    $sql = "SELECT COUNT(*) FROM " .$yp->table('ad'). " WHERE ad_name = '$ad_name'";
    if ($db->getOne($sql) > 0)
    {
    ...
    /* 插入数据 */
    $sql = "INSERT INTO ".$yp->table('ad'). " (position_id,media_type,ad_name,ad_link,ad_code,start_time,end_time,link_man,link_email,link_phone,click_count,enabled)
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    /* 获取广告数据 */
    $sql = "SELECT * FROM " .$yp->table('ad'). " WHERE ad_id='".intval($_REQUEST['id'])."'";
...
function get_adslist()
{
    ...
    /* 获得总记录数据 */
    $sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['yp']->table('ad'). ' AS ad ' . $where;
    ...
    $sql = 'SELECT ad.*, COUNT(o.order_id) AS ad_stats, p.position_name '.
            'FROM ' .$GLOBALS['yp']->table('ad'). 'AS ad ' .
            'LEFT JOIN ' . $GLOBALS['yp']->table('ad_position'). ' AS p ON p.position_id = ad.position_id '.
