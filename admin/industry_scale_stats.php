...
/**
 * QQ120029121 行业分析：行业规模
 * ============================================================================
 * 演示地址: http://demo.coolhong.com；
 * ============================================================================
 * $Author: jtypmall $
 * $Id: industry_scale_stats.php 17217 2015-10-26 11:06:08Z jtypmall $
 */
...
if ($_REQUEST['act'] == 'list')
{
    ...
    if(isset($_REQUEST['cat_id']) && !empty($_REQUEST['cat_id']))
    {
        ...
        // 取得所有一级分类
        $cat_list = $db->getAll(
            'SELECT cat_id id, cat_name name FROM ' . $yp->table('category') . ' WHERE parent_id = 0 AND is_virtual = 0 ORDER BY cat_id'
        );
    }
    ...
    foreach($cat_list as $value)
    {
        ...
        $cat_name = $value['name'];
        // 查询分类下下单金额
        $sql = 'SELECT IFNULL(SUM(og.goods_price), 0) goods_price FROM ' . $yp->table('order_info') . ' oi, '
            . $yp->table('order_goods') . ' og, ' . $yp->table('goods') . ' g ' . $where
        ...
        // 查询分类下下单商品数
        $sql = 'SELECT SUM(og.goods_number) goods_count FROM ' . $yp->table('order_info') . ' oi, '
            . $yp->table('order_goods') . ' og, ' . $yp->table('goods') . ' g ' . $where
            . ' AND og.goods_id = g.goods_id AND og.order_id = oi.order_id AND ' . get_children($cat_id);
        // 取得下单商品数
        ...
        // 查询分类下下单量
        $sql = 'SELECT COUNT(*) goods_count FROM ' . $yp->table('order_info') . ' oi, '
            . $yp->table('order_goods') . ' og, ' . $yp->table('goods') . ' g ' . $where
            . ' AND og.goods_id = g.goods_id AND og.order_id = oi.order_id AND ' . get_children($cat_id);
        // 取得下单量
