...
/**
 * QQ120029121 地区列表管理文件
 * ============================================================================
 * 演示地址: http://demo.coolhong.com  开发QQ:120029121    309485552
 * ============================================================================
 * $Author: PRINCE $
 * $Id: area_manage.php 17217 2017-04-01 06:29:08Z PRINCE $
*/

define('IN_PRINCE', true);

require(dirname(__FILE__) . '/includes/init.php');
$exc = new exchange($yp->table('region'), $db, 'region_id', 'region_name');
...
elseif ($_REQUEST['act'] == 'add_area')
{
    ...
    $sql = "INSERT INTO " . $yp->table('region') . " (parent_id, region_name, region_type) ".
...
elseif ($_REQUEST['act'] == 'drop_area')
{
    ...
    $sql = "SELECT * FROM " . $yp->table('region') . " WHERE region_id = '$id'";
    $region = $db->getRow($sql);

//    /* 如果底下有下级区域,不能删除 */
//    $sql = "SELECT COUNT(*) FROM " . $yp->table('region') . " WHERE parent_id = '$id'";
//    if ($db->getOne($sql) > 0)
//    {
...
//    }
    ...
    $sql="DELETE FROM ". $yp->table("region")."WHERE region_id".db_create_in($delete_region);
     $db->query($sql);
    if ($exc->drop($id))
    {
...
function new_region_id($region_id)
{
    ...
    $sql="SELECT region_id FROM ". $GLOBALS['yp']->table("region")."WHERE parent_id ".db_create_in($region_id);
