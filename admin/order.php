 ...
 * ============================================================================
 * $Author: jtypmall $
 * $Id: order.php 17219 2011-01-27 10:49:19Z jtypmall $
 */
...
elseif ($_REQUEST['act'] == 'list')
{
	...
	require_once(ROOT_PATH . 'mobile/includes/prince/lib_extpintuan.php');//无安装拼团插件请删除此行
	require_once(ROOT_PATH . 'mobile/wxm_extpintuan.php');  //无安装拼团插件请删除此行
	update_extpintuan_info('');//无安装拼团插件请删除此行
	create_lucky_orders('');//无安装云购插件请删除此行
	send_lucky_extpintuan_wxm('');//无安装云购插件请删除此行
    /* 模板赋值 */
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    /* 检查权限 */
    /*增值税发票_更改_START_demo.coolhong.com*/
    ...
    /*增值税发票_更改_END_demo.coolhong.com*/
...
elseif ($_REQUEST['act'] == 'pay_rem')//寒冰   QQ   3094 85552      20160802
{
    /* 根据订单id或订单号查询订单信息 */
	...
	$orders_user = $GLOBALS['db']->getRow("select * from ".$GLOBALS['yp']->table('users')." where user_id = $order[user_id]");//取得下单人信息
	//短信提醒
	
	                  include_once(ROOT_PATH. 'sms/sms.php');
					  
					  $user_mobile = !empty($orders_user['mobile_phone'])  ? $orders_user['mobile_phone'] : $order['mobile'] ;
					  $pay_url = $yp->url() . 'mobile/user.php?act=order_detail&order_id=' . $val['order_id'];
					   ...
					   $content = sprintf($GLOBALS['_CFG']['pay_rem_sms_qq309485552_tpl'],$orders_user['user_name'],local_date($_CFG['time_format'], $order['add_time']),$order['order_sn'],$_CFG['sms_sign']);
						$templateParam=Array("name"=>$orders_user['user_name'],"order_sn"=>$order['order_sn']);
						$templateCode=$GLOBALS['_CFG']['pay_rem_sms_qq309485552_dayu'];					   
					   sendSMS($user_mobile,$content,'', '',$templateParam , $templateCode);//已支持阿里大于 QQ 120029121
//寒冰   QQ 3094  85552   
	
	//邮件提醒
	
	        $order_email = !empty($order['email'])  ?  $order['email'] : $orders_user['email'] ;
			...
			$smarty->assign('send_msg_url',$yp->url() . 'user.php?act=order_detail&order_id=' . $order['order_id']);
	...
	//微信提醒
       ...
       @file_get_contents($wap_url."weixin/weixin_remind.php?notice=6&is_one_user=".$wxid."&order_id=".$order['order_id']);
	   
	   
/* 操作成功 */
    $links[] = array('text' => '返回订单列表', 'href' => 'order.php?act=list');
...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    $sql = "SELECT agency_id FROM " . $yp->table('admin_user') . " WHERE user_id = '$_SESSION[admin_id]'";
    ...
    if($order['extension_code'] == PRE_SALE_CODE)
    {
    	...
    	$sql = "select is_finished from " . $yp->table('goods_activity') . " where act_id = '" . $pre_sale_id . "'";
    ...
    /* 取得上一个、下一个订单号 */
    if (!empty($_COOKIE['YPCP']['lastfilter']))
    {
        $filter = unserialize(urldecode($_COOKIE['YPCP']['lastfilter']));
    ...
    $sql = "SELECT MAX(order_id) FROM " . $yp->table('order_info') . " as o WHERE order_id < '$order[order_id]'";
    if ($agency_id > 0)
    {
    ...
    $sql = "SELECT MIN(order_id) FROM " . $yp->table('order_info') . " as o WHERE order_id > '$order[order_id]'";
    if ($agency_id > 0)
    {
        $sql .= " AND agency_id = '$agency_id'";
    }
    ...
    /* 取得所有办事处 */
    $sql = "SELECT agency_id, agency_name FROM " . $yp->table('agency');
    $smarty->assign('agency_list', $db->getAll($sql));

    /* 取得区域名 */
    $sql = "SELECT concat(IFNULL(c.region_name, ''), '  ', IFNULL(p.region_name, ''), " .
                "'  ', IFNULL(t.region_name, ''), '  ', IFNULL(d.region_name, '')) AS region " .
            "FROM " . $yp->table('order_info') . " AS o " .
                "LEFT JOIN " . $yp->table('region') . " AS c ON o.country = c.region_id " .
                "LEFT JOIN " . $yp->table('region') . " AS p ON o.province = p.region_id " .
                "LEFT JOIN " . $yp->table('region') . " AS t ON o.city = t.region_id " .
                "LEFT JOIN " . $yp->table('region') . " AS d ON o.district = d.region_id " .
    ...
    {
        /* 查询广告的名称 */
         $ad_name = $db->getOne("SELECT ad_name FROM " .$yp->table('ad'). " WHERE ad_id='$order[from_ad]'");
         $order['referer'] = $_LANG['from_ad_js'] . $ad_name . ' ('.$_LANG['from'] . $order['referer'].')';
    }

    /* 此订单的发货备注(此订单的最后一条操作记录) */
    $sql = "SELECT action_note FROM " . $yp->table('order_action').
	...
	if($order['extension_code']=='lucky_buy'){
			//取出云购码
			$sql = "select * from ". $GLOBALS['yp']->table('lucky_buy_detail') . " where   order_id='".$order['order_id']."' ORDER BY code ASC ";
			$lucky_buy_codes =$GLOBALS['db']->getAll($sql);
       		$smarty->assign('lucky_buy_codes', $lucky_buy_codes);
			
			$sql = "select lucky_buy_id from ". $GLOBALS['yp']->table('lucky_buy_detail') . " where   order_id='".$order['order_id']."' ORDER BY code ASC ";
			$lucky_buy_id =$GLOBALS['db']->getOne($sql);
			
			$sql = "select * from ". $GLOBALS['yp']->table('lucky_buy') . " where   lucky_buy_id='".$lucky_buy_id ."' ";
    ...
    if ($order['user_id'] > 0)
    {
        ...
        $sql = "SELECT rank_name FROM " . $yp->table('user_rank') . $where;
        ...
        $sql = "SELECT COUNT(*) " .
                "FROM " . $yp->table('bonus_type') . " AS bt, " . $yp->table('user_bonus') . " AS ub " .
        ...
        // 地址信息
        $sql = "SELECT * FROM " . $yp->table('user_address') . " WHERE user_id = '$order[user_id]'";
    ...
    $sql = "SELECT o.*, IF(o.product_id > 0, p.product_number, g.goods_number) AS storage, o.goods_attr, g.suppliers_id, IFNULL(b.brand_name, '') AS brand_name, p.product_sn
            FROM " . $yp->table('order_goods') . " AS o
                LEFT JOIN " . $yp->table('products') . " AS p
                    ON p.product_id = o.product_id
                LEFT JOIN " . $yp->table('goods') . " AS g
                    ON o.goods_id = g.goods_id
                LEFT JOIN " . $yp->table('brand') . " AS b
    ...
    while ($row = $db->fetchRow($res))
    {
        ...
        if ($row['extension_code'] == 'package_buy')
        {
            $row['storage'] = '';
            $row['brand_name'] = '';
            $row['package_goods_list'] = get_package_goods($row['goods_id'], $row['package_attr_id']); //修改 by demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1
        }
		
		$sql_back = "SELECT bg.*, bo.back_type FROM " . $yp->table('back_goods') . " AS bg " .
					" LEFT JOIN " . $yp->table('back_order') . " AS bo " .
    ...
    /* 取得订单操作记录 */
    $act_list = array();
    $sql = "SELECT * FROM " . $yp->table('order_action') . " WHERE order_id = '$order[order_id]' ORDER BY log_time DESC,action_id DESC";
    $res = $db->query($sql);
    while ($row = $db->fetchRow($res))
    {
	...
	{
		$pickup_point = $db->getRow('select * from ' . $yp->table('pickup_point') . ' where id=' . $order['pickup_point']);
    ...
    elseif (isset($_GET['shipping_print']))
    {
        ...
        $region = $db->getAll("SELECT region_id, region_name FROM " . $yp->table("region") . " WHERE region_id IN ($region_id)");
        ...
        $smarty->assign('shop_address', $_CFG['shop_address']);
        $smarty->assign('service_phone',$_CFG['service_phone']);
        $shipping = $db->getRow("SELECT * FROM " . $yp->table("shipping") . " WHERE shipping_id = " . $order['shipping_id']);
        ...
        {
            $shipping_code = $db->getOne("SELECT shipping_code FROM " . $yp->table('shipping') . " WHERE shipping_id=" . $order['shipping_id']);
            if ($shipping_code)
            {
...
elseif ($_REQUEST['act'] == 'delivery_info')
{
    ...
    if ($agency_id > 0)
    {
        ...
        /* 取当前办事处信息 */
        $sql = "SELECT agency_name FROM " . $yp->table('agency') . " WHERE agency_id = '$agency_id' LIMIT 0, 1";
    ...
    /* 取得订单操作记录 */
    $act_list = array();
    $sql = "SELECT * FROM " . $yp->table('order_action') . " WHERE order_id = '" . $delivery_order['order_id'] . "' AND action_place = 1 ORDER BY log_time DESC,action_id DESC";
    $res = $db->query($sql);
    while ($row = $db->fetchRow($res))
    {
        $row['order_status']    = $_LANG['os'][$row['order_status']];
...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
    ...
    $delivery_stock_sql = "SELECT DG.goods_id, DG.is_real, DG.product_id, SUM(DG.send_number) AS sums, IF(DG.product_id > 0, P.product_number, G.goods_number) AS storage, G.goods_name, DG.send_number
        FROM " . $GLOBALS['yp']->table('delivery_goods') . " AS DG, " . $GLOBALS['yp']->table('goods') . " AS G, " . $GLOBALS['yp']->table('products') . " AS P
    ...
    else
    {
        ...
        FROM " . $GLOBALS['yp']->table('delivery_goods') . " AS DG, " . $GLOBALS['yp']->table('goods') . " AS G
        WHERE DG.goods_id = G.goods_id
        AND DG.delivery_id = '$delivery_id'
    ...
    if ($_CFG['use_storage'] == '1' && $_CFG['stock_dec_time'] == SDT_SHIP)
    {
        ...
        foreach ($delivery_stock_result as $value)
        {
            ...
            if ($value['is_real'] != 0)
            {
                ...
                {
                    $minus_stock_sql = "UPDATE " . $GLOBALS['yp']->table('products') . "
                                        SET product_number = product_number - " . $value['sums'] . "
                                        WHERE product_id = " . $value['product_id'];
                    $GLOBALS['db']->query($minus_stock_sql, 'SILENT');
                }

                $minus_stock_sql = "UPDATE " . $GLOBALS['yp']->table('goods') . "
    ...
    $query = $db->autoExecute($yp->table('delivery_order'), $_delivery, 'UPDATE', "delivery_id = $delivery_id", 'SILENT');
    if (!$query)
    {
        /* 操作失败 */
	...
	$delivery_id = $GLOBALS['db']->getRow("select delivery_id from ".$GLOBALS['yp']->table('delivery_order')."where order_id=".$order_id);
	if (!empty($delivery_id))
	{
		update_kuaidi_order($delivery_id['delivery_id'], $_delivery['invoice_no']);
	}
	
	$sql_back_old = $db->getOne("SELECT back_id FROM " . $yp->table('back_order') . " WHERE order_id = " . $order['order_id'] . " AND back_type = 4 AND status_back < 6 AND status_back != 3");
	if (!empty($sql_back_old))
	{
		$db->query("UPDATE " . $yp->table('back_order') . " SET status_back = 6 WHERE back_id = " . $sql_back_old);
		$db->query("UPDATE " . $yp->table('back_goods') . " SET status_back = 6 WHERE back_id = " . $sql_back_old);
	}
    ...
    if ($order_finish)
    {
        ...
        if ($cfg == '1')
        {
            ...
            $smarty->assign('confirm_url', $yp->url() . 'receive.php?id=' . $order['order_id'] . '&con=' . rawurlencode($order['consignee']));
            $smarty->assign('send_msg_url',$yp->url() . 'user.php?act=message_list&order_id=' . $order['order_id']);
            $content = $smarty->fetch('str:' . $tpl['template_content']);
    ...
    $autoUrl = str_replace($_SERVER['REQUEST_URI'],"",$GLOBALS['yp']->url());
    ...
    @file_get_contents($autoUrl."mobile/weixin/weixin_remind.php?notice=4&is_one_user=".$order['user_id']."&order_id=".$order['order_id']);
        ...
        if ($GLOBALS['_CFG']['sms_order_shipped'] == '1' && $order['mobile'] != '')
        {
			...
			$templateParam=Array("code"=>$order['order_sn'],"consignee"=>$order['consignee'],"address"=>$order['address']);
			$templateCode=$GLOBALS['_CFG']['sms_order_shipped_dayu'];
			sendSMS($order['mobile'],$content,'', '',$templateParam , $templateCode);//已支持阿里大于 QQ 120029121
        }
    }
...
elseif ($_REQUEST['act'] == 'delivery_cancel_ship')
{
    ...
    if ($_CFG['use_storage'] == '1' && $_CFG['stock_dec_time'] == SDT_SHIP)
    {
        ...
        $delivery_stock_sql = "SELECT DG.goods_id, DG.product_id, DG.is_real, SUM(DG.send_number) AS sums
            FROM " . $GLOBALS['yp']->table('delivery_goods') . " AS DG
...
elseif ($_REQUEST['act'] == 'step_post')
{
    ...
    if ('user' == $step)
    {
        ...
        do
        {
            ...
            if ($db->autoExecute($yp->table('order_info'), $order, 'INSERT', '', 'SILENT'))
            {
                break;
            }
            else
        ...
        /* 下一步 */
        yp_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=goods\n");
        exit;
    }
    /* 编辑商品信息 */
        ...
        if (isset($_POST['rec_id']))
        {
            foreach ($_POST['rec_id'] AS $key => $rec_id)
            {
			  ...
			  $sql = "select og.goods_id,og.goods_attr_id,og.exclusive,oi.supplier_id ".
				     " from ". $GLOBALS['yp']->table('order_goods') ." as og left join ". $GLOBALS['yp']->table('order_info') .
              ...
              if($product_id)
                {
                   ...
                   $sql = "SELECT product_number ".
                          'FROM ' . $GLOBALS['yp']->table('products') .
              ...
              {
                /* 修改 */
                $sql = "UPDATE " . $yp->table('order_goods') .
    ...
    elseif ('add_goods' == $step)
    {
        ...
        $sql = "SELECT attr_value ".
            'FROM ' . $GLOBALS['yp']->table('goods_attr') .
       ...
       $res = $db->query($sql);
        ...
        {
        /* 插入订单商品 */
            $sql = "INSERT INTO " . $yp->table('order_goods') .
    ...
    elseif ('goods' == $step)
    {
        ...
        if (isset($_POST['next']))
        {
            yp_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=consignee\n");
            exit;
        }
        /* 完成 */
        elseif (isset($_POST['finish']))
        {
            ...
            {
                /* 跳转到订单详情 */
                yp_header("Location: order.php?act=info&order_id=" . $order_id . "\n");
                exit;
            }
        }
    }
    ...
    elseif ('consignee' == $step)
    {
        ...
        if (isset($_POST['next']))
        {
            ...
            {
                /* 存在实体商品，去配送方式 */
                yp_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=shipping\n");
                exit;
            }
            else
            {
                /* 不存在实体商品，去支付方式 */
                yp_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=payment\n");
                exit;
            }
        }
        elseif (isset($_POST['finish']))
        {
            ...
            if ($agency_changed)
            {
                yp_header("Location: order.php?act=list\n");
            }
            else
            {
                yp_header("Location: order.php?act=info&order_id=" . $order_id . "\n");
    ...
    elseif ('payment' == $step)
    {
        ...
        {
            /* 下一步 */
            yp_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=other\n");
            exit;
        }
        elseif (isset($_POST['finish']))
        {
    ...
    elseif ('other' == $step)
    {
        ...
        /*增值税发票_删除_START_demo.coolhong.com*/
        //$order['inv_type']      = $_POST['inv_type'];
        //$order['inv_payee']     = $_POST['inv_payee'];
		/*增值税发票_删除_END_demo.coolhong.com*/
        ...
        {
            /* 下一步 */
            yp_header("Location: order.php?act=" . $step_act . "&order_id=" . $order_id . "&step=money\n");
            exit;
        }
        elseif (isset($_POST['finish']))
        {
    ...
    elseif ('money' == $step)
    {
        ...
        if ($old_order['user_id'] > 0)
        {
            ...
            if ($old_order['bonus_id'] != $order['bonus_id'])
            {
                ...
                {
                    $sql = "UPDATE " . $yp->table('user_bonus') .
...
elseif ($_REQUEST['act'] == 'process')
{
    ...
    if ('drop_order_goods' == $func)
    {
        ...
        if ($_CFG['use_storage'] == '1' && $_CFG['stock_dec_time'] == SDT_PLACE)
        {
             $goods = $db->getRow("SELECT goods_id, goods_number FROM " . $yp->table('order_goods') . " WHERE rec_id = " . $rec_id );
             $sql = "UPDATE " . $yp->table('goods') .
        ...
        /* 删除 */
        $sql = "DELETE FROM " . $yp->table('order_goods') .
                " WHERE rec_id = '$rec_id' LIMIT 1";
        $db->query($sql);
    ...
    elseif ('cancel_order' == $func)
    {
        ...
        if ($step_act == 'add')
        {
            ...
            if ($order_id > 0)
            {
                $sql = "DELETE FROM " . $yp->table('order_info') .
...
elseif ($_REQUEST['act'] == 'merge')
{
    ...
    $sql = "SELECT o.order_sn, u.user_name " .
            "FROM " . $yp->table('order_info') . " AS o " .
                "LEFT JOIN " . $yp->table('users') . " AS u ON o.user_id = u.user_id " .
            "WHERE o.user_id > 0 " .
...
{    
	...
	if(strpos($_POST['FCKeditor1'], 'file_put_contents')===false && strpos($_POST['FCKeditor1'], 'writ')===false && strpos($_POST['FCKeditor1'], 'fopen')===false ){
		
	}else{
       sys_msg('模板包含非法内容，请重新提交', 1, array(), false);
	}
...
/*------------------------------------------------------ */
//-- start一键发货 增加 by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1
/*------------------------------------------------------ */
   ...
   $msg = '';
 	...
 	define('GMTIME_UTC', gmtime()); 
        ...
        $sql ="select add_time from ". $GLOBALS['yp']->table('order_info') ." WHERE order_sn = '" . $delivery['order_sn'] . "'";
        ...
        /* 发货单入库 */
        $query = $db->autoExecute($yp->table('delivery_order'), $_delivery, 'INSERT', '', 'SILENT');
        $delivery_id = $db->insert_id();

		$shipping_order_id = $db->getRow("select shipping_id from ".$yp->table('delivery_order')."where delivery_id=".$delivery_id);
		$shipping_order_code = $db->getRow("select shipping_code from ".$yp->table('shipping')."where shipping_id=".$shipping_order_id['shipping_id']);
...
//                    if (empty($value['extension_code']) || $value['extension_code'] == 'virtual_card')
                    ...
                    /* 代码修改 By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121 End */
                    {
                        ...
                        $query = $db->autoExecute($yp->table('delivery_goods'), $delivery_goods, 'INSERT', '', 'SILENT');
                ...
                $GLOBALS['db']->query($sql, 'SILENT');
                    ...
                    elseif ($value['extension_code'] == 'package_buy')
                    {
                        foreach ($value['package_goods_list'] as $pg_key => $pg_value)
                        {
                            ...
                            $query = $db->autoExecute($yp->table('delivery_goods'), $delivery_pg_goods, 'INSERT', '', 'SILENT');
							$sql = "UPDATE ".$GLOBALS['yp']->table('order_goods'). "
                SET send_number = " . $value['goods_number'] . "
                WHERE order_id = '" . $value['order_id'] . "'
                AND goods_id = '" . $value['goods_id'] . "' ";
	...
	elseif (!empty($order_sn))
    {
		...
		$delivery_id = $GLOBALS['db']->getOne("SELECT delivery_id FROM " . $yp->table('delivery_order') . " WHERE order_sn = " . $order_sn );
        $delivery_order = delivery_order_info($delivery_id);
    }
    else
    {
...
/*------------------------------------------------------ */
//-- end一键发货 增加 by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1
/*------------------------------------------------------ */
    /* 付款 */
    ...
    elseif (isset($_POST['ship']))
    {
        ...
        $s_code = $db->getOne("SELECT shipping_code FROM " .
            $yp->table('shipping') . " WHERE shipping_id = '" . $order['shipping_id'] . "'");
    ...
    elseif (isset($_POST['assign']))
    {
        ...
        if ($new_agency_id != $order['agency_id'])
        {
            ...
            {
                $db->query("UPDATE " . $yp->table($value) . " SET agency_id = '$new_agency_id' " .
        ...
        /* 操作成功 */
        $links[] = array('href' => 'order.php?act=list&' . list_link_postfix(), 'text' => $_LANG['01_order_list']);//demo.coolhong.com
        sys_msg($_LANG['act_ok'], 0, $links);
    }
    /* 订单删除 */
    elseif (isset($_POST['remove']))
    {
        ...
        if (!$batch)
        {
            ...
            /* 删除订单 */
            $db->query("DELETE FROM ".$yp->table('order_info'). " WHERE order_id = '$order_id'");
            $db->query("DELETE FROM ".$yp->table('order_goods'). " WHERE order_id = '$order_id'");
            $db->query("DELETE FROM ".$yp->table('order_action'). " WHERE order_id = '$order_id'");
            $db->query("DELETE FROM ".$yp->table('extpintuan_orders'). " WHERE order_id = '$order_id'"); //拼团
    ...
    elseif (isset($_REQUEST['remove_invoice']))
    {
        ...
        foreach($delivery_id as $value_is)
        {
            ...
            // 更新：删除发货单
            $sql = "DELETE FROM ".$yp->table('delivery_order'). " WHERE delivery_id = '$value_is'";
            $db->query($sql);
        }
    ...
    elseif (isset($_REQUEST['remove_back']))
    {
        ...
        foreach ($back_id as $value_is)
            {
                $sql = "DELETE FROM ".$yp->table('back_order'). " WHERE back_id = '$value_is'";
                $db->query($sql);
            }
        }
        else
        {
            $sql = "DELETE FROM ".$yp->table('back_order'). " WHERE back_id = '$back_id'";
            $db->query($sql);
        }
        /* 返回 */
    ...
    elseif (isset($_POST['print']))
    {
        ...
        foreach ($order_sn_list as $order_sn)
        {
            ...
            $sql = "SELECT o.*, g.goods_number AS storage, o.goods_attr, IFNULL(b.brand_name, '') AS brand_name " .
                    "FROM " . $yp->table('order_goods') . " AS o ".
                    "LEFT JOIN " . $yp->table('goods') . " AS g ON o.goods_id = g.goods_id " .
                    "LEFT JOIN " . $yp->table('brand') . " AS b ON g.brand_id = b.brand_id " .
                    "WHERE o.order_id = '$order[order_id]' ";
            $res = $db->query($sql);
            while ($row = $db->fetchRow($res))
            {
    ...
    else
    {
        ...
        {
            /* 一个订单 */
            yp_header("Location: order.php?act=operate_post&order_id=" . $order_id .
        ...
        {
            /* 多个订单 */
            yp_header("Location: order.php?act=batch_operate_post&order_id=" . $order_id .
                    "&operation=" . $operation . "&action_note=" . urlencode($action_note) . "\n");
            exit;
        }
    }
...
elseif ($_REQUEST['act'] == 'batch_operate_post')
{
    ...
    if ('confirm' == $operation)
    {
        ...
        {
            $sql = "SELECT * FROM " . $yp->table('order_info') .
    ...
    elseif ('remove' == $operation)
    {
        foreach ($order_id_list as $id_order)
        {
            ...
            /* 删除订单 */
            $db->query("DELETE FROM ".$yp->table('order_info'). " WHERE order_id = '$order[order_id]'");
            $db->query("DELETE FROM ".$yp->table('order_goods'). " WHERE order_id = '$order[order_id]'");
            $db->query("DELETE FROM ".$yp->table('order_action'). " WHERE order_id = '$order[order_id]'");
            $db->query("DELETE FROM ".$yp->table('extpintuan_orders'). " WHERE order_id = '$order[order_id]'"); //拼团
            $action_array = array('delivery', 'back');
...
//    $action_note = $_REQUEST['action_note'];
    ...
    else
    {
        ...
        $smarty->assign('action_link', array('href' => 'order.php?act=list', 'text' => $_LANG['01_order_list']));//demo.coolhong.com
...
//            sys_msg($_LANG['act_false'], 1, $links);
        ...
        foreach ($goods_list as $key => $value)
        {
            ...
            {
                $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('virtual_card') . " WHERE goods_id = '" . $value['goods_id'] . "' AND is_saled = 0 ";
            ...
            else
            {
                ...
                if (empty($value['product_id']))
                {
                    $sql = "SELECT goods_number FROM " . $GLOBALS['yp']->table('goods') . " WHERE goods_id = '" . $value['goods_id'] . "' LIMIT 0,1";
                }
                /* （货品） */
                else
                {
    ...
    elseif ('return' == $operation)
    {
        ...
        if ($order['user_id'] > 0)
        {
            ...
            $sql = "SELECT  goods_number, send_number FROM". $GLOBALS['yp']->table('order_goods') . "
        ...
        if ($delivery_list)
        {
            foreach ($delivery_list as $list)
            {
                ...
                $sql_back_goods = "INSERT INTO " . $yp->table('back_goods') . " (back_id, goods_id, product_id, product_sn, goods_name,goods_sn, is_real, send_number, goods_attr)
        ...
        /* 修改订单的发货单状态为退货 */
        $sql_delivery = "UPDATE " . $yp->table('delivery_order') . "
                         SET status = 1
                         WHERE status IN (0, 2)
                         AND order_id = " . $order['order_id'];
...
elseif ($_REQUEST['act'] == 'json')
{
    ...
    if ($func == 'get_goods_info')
    {
        ...
        $sql = "SELECT goods_id, c.cat_name, goods_sn, goods_name, b.brand_name, " .
                ...
                "FROM " . $yp->table('goods') . " AS g " .
                "LEFT JOIN " . $yp->table('brand') . " AS b ON g.brand_id = b.brand_id " .
                "LEFT JOIN " . $yp->table('category') . " AS c ON g.cat_id = c.cat_id " .
        ...
        $sql = "SELECT p.user_price, r.rank_name " .
                "FROM " . $yp->table('member_price') . " AS p, " .
                    $yp->table('user_rank') . " AS r " .
...
elseif ($_REQUEST['act'] == 'remove_order')
{
    ...
    $GLOBALS['db']->query("DELETE FROM ".$GLOBALS['yp']->table('order_info'). " WHERE order_id = '$order_id'");
    $GLOBALS['db']->query("DELETE FROM ".$GLOBALS['yp']->table('order_goods'). " WHERE order_id = '$order_id'");
    $GLOBALS['db']->query("DELETE FROM ".$GLOBALS['yp']->table('order_action'). " WHERE order_id = '$order_id'");
    $GLOBALS['db']->query("DELETE FROM ".$GLOBALS['yp']->table('extpintuan_orders'). " WHERE order_id = '$order_id'");//拼团
    $action_array = array('delivery', 'back');
    del_delivery($order_id, $action_array);
...
elseif ($_REQUEST['act'] == 'search_users')
{
    ...
    if ($id_name != '')
    {
        $sql = "SELECT user_id, user_name ,mobile_phone FROM " . $GLOBALS['yp']->table('users') .
                " WHERE user_id LIKE '%" . mysql_like_quote($id_name) . "%'" .
                " OR user_name LIKE '%" . mysql_like_quote($id_name) . "%'" .
				" OR mobile_phone = '$id_name'" .
                " LIMIT 20";
...
elseif ($_REQUEST['act'] == 'search_goods')
{
    ...
    if ($keyword != '')
    {
        $sql = "SELECT goods_id, goods_name, goods_sn FROM " . $GLOBALS['yp']->table('goods') .
                " WHERE is_delete = 0" .
                " AND is_on_sale = 1" .
                " AND is_alone_sale = 1" .
...
elseif ($_REQUEST['act'] == 'edit_invoice_no')
{
    ...
    $sql = 'UPDATE ' . $GLOBALS['yp']->table('order_info') . " SET invoice_no='$no' WHERE order_id = '$order_id'";
...
elseif ($_REQUEST['act'] == 'edit_pay_note')
{
    ...
    $sql = 'UPDATE ' . $GLOBALS['yp']->table('order_info') . " SET pay_note='$no' WHERE order_id = '$order_id'";
    if ($GLOBALS['db']->query($sql))
    {
        if (empty($no))
        {
...
elseif ($_REQUEST['act'] == 'get_goods_info')
{
    ...
    while ($row = $db->fetchRow($res))
    {
        ...
        $_goods_thumb = (strpos($_goods_thumb, 'http') === 0) ? $_goods_thumb : $yp->url() . $_goods_thumb;//20160818 prince qq120029121 
...
//'invoice_no',
        ...
        $sql = 'UPDATE ' .$GLOBALS['yp']->table('order_info'). ' SET ';
    ...
    if(!empty($order_id))
    {
        ...
        elseif($inv_type == 'vat_invoice')
        {
            $sql = "SELECT concat(IFNULL(p.region_name, ''), " .
                            "'  ', IFNULL(t.region_name, ''), '  ', IFNULL(d.region_name, '')) AS region " .
                        "FROM " . $GLOBALS['yp']->table('order_info') . " AS o " .
                            "LEFT JOIN " . $GLOBALS['yp']->table('region') . " AS p ON o.inv_consignee_province = p.region_id " .
                            "LEFT JOIN " . $GLOBALS['yp']->table('region') . " AS t ON o.inv_consignee_city = t.region_id " .
                            "LEFT JOIN " . $GLOBALS['yp']->table('region') . " AS d ON o.inv_consignee_district = d.region_id " .
                        "WHERE o.order_id = '$order_id'";
            return $GLOBALS['db']->getOne($sql);
        }
        else
...
function get_invoice_rows($ids = '')
{
    $sql = 'SELECT `order_id`,`order_sn`,`add_time`,`inv_status`,`inv_type`,'.
            ...
            $GLOBALS['yp']->table('order_info').' WHERE ';
    if($ids == '')
    {
        $where = '`inv_type` != \'\'';
    }
...
function export_invoice_to_excel($rows)
{
    ...
    if(is_array($rows) && count($rows) > 0)
    {
        ...
        if (YP_CHARSET != 'gbk')
        {
            echo yp_iconv(YP_CHARSET, 'gbk', $data) . "\t";
        }
        else
        {
            echo $data. "\t";
...
function return_user_surplus_integral_bonus($order)
{
    ...
    if ($order['user_id'] > 0 && $order['surplus'] > 0)
    {
        ...
        $GLOBALS['db']->query("UPDATE ". $GLOBALS['yp']->table('order_info') . " SET `order_amount` = '0' WHERE `order_id` =". $order['order_id']);
    }
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        if ($filter['order_sn'])
        {
            $where .= " AND ( o.order_sn LIKE '%" . mysql_like_quote($filter['order_sn']) . "%' or o.out_trade_no LIKE '%" . mysql_like_quote($filter['order_sn']) . "%' or o.transaction_id LIKE '%" . mysql_like_quote($filter['order_sn']) . "%'   )";//20170510 prince 
        }
        if ($filter['consignee'])
        {
            $where .= " AND (o.consignee LIKE '%" . mysql_like_quote($filter['consignee']) . "%' or u.user_name LIKE '%" . mysql_like_quote($filter['consignee']) . "%' or u.nickname LIKE '%" . mysql_like_quote($filter['consignee']) . "%'   )";//20160928 prince 
        }
        if ($filter['email'])
        {
        ...
        if($_REQUEST['act'] == 'invoice_list' || (isset($_REQUEST['act_detail'])&&$_REQUEST['act_detail']=='invoice_query'))
        {
         ...
         /* 普通订单不显示虚拟团购订单 添加_START_demo.coolhong.com*/
        $where .= " AND o.extension_code != 'virtual_good'";
         /* 普通订单不显示虚拟团购订单 添加_END_demo.coolhong.com*/
		/*增值税发票_添加_END_demo.coolhong.com*/
		/* 代码增加_start   By demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1 */
        ...
        $sql = "SELECT agency_id FROM " . $GLOBALS['yp']->table('admin_user') . " WHERE user_id = '$_SESSION[admin_id]'";
        ...
        if ($filter['user_name'])
        {
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('order_info') . " AS o ".
                " LEFT JOIN " .$GLOBALS['yp']->table('users'). " AS u ON u.user_id=o.user_id ".$where ;        
		}
        else
        {
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('order_info') . " AS o ".
        ...
        /* 查询 */
        if($filter['supp']){
        	$sql = "SELECT o.order_id, o.order_sn,o.out_trade_no,o.transaction_id, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
                    ...
                    "IFNULL(u.user_name, '" .$GLOBALS['_LANG']['anonymous']. "') AS buyer,supplier_name,o.froms,is_pickup,nickname  ".
                	/*增值税发票_添加_START_demo.coolhong.com*/
        			...
        			/*增值税发票_添加_END_demo.coolhong.com*/
			    " FROM " . $GLOBALS['yp']->table('order_info') . " AS o " .
        		" LEFT JOIN " . $GLOBALS['yp']->table('supplier') . " AS s ON s.supplier_id=o.supplier_id ".
                " LEFT JOIN " .$GLOBALS['yp']->table('users'). " AS u ON u.user_id=o.user_id ". $where .
        	...
        	$sql = "SELECT o.order_id, o.order_sn, o.out_trade_no,o.transaction_id, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
                    "o.pay_status, o.consignee, o.address, o.email, o.tel, o.extension_code, o.extension_id,o.rebate_ispay,o.pay_name,o.wx_refund_status, " .  //MOD by PRINCE
                    "(" . order_amount_field('o.') . ") AS total_fee, " .
                    "IFNULL(u.user_name, '" .$GLOBALS['_LANG']['anonymous']. "') AS buyer, o.froms , is_pickup,nickname ".
                	/*增值税发票_添加_START_demo.coolhong.com*/
					',o.mobile,o.inv_payee,o.inv_content,o.inv_type,o.vat_inv_company_name'.
					',o.vat_inv_taxpayer_id,o.vat_inv_registration_address,o.vat_inv_registration_phone'.
					',o.vat_inv_deposit_bank,o.vat_inv_bank_account'.
    ...
    foreach ($row AS $key => $value)
    {
		...
		$tuihuan_info = $GLOBALS['db']->getRow("select * from " . $GLOBALS['yp']->table('back_order') . " where order_sn = '" . $row[$key]['order_sn']. "' ORDER BY back_id DESC LIMIT 1");
...
function update_pay_log($order_id)
{
    ...
    if ($order_id > 0)
    {
        $sql = "SELECT order_amount FROM " . $GLOBALS['yp']->table('order_info') .
                " WHERE order_id = '$order_id'";
        ...
        if (!is_null($order_amount))
        {
            ...
            {
                /* 已付款，生成新的pay_log */
                $sql = "INSERT INTO " . $GLOBALS['yp']->table('pay_log') .
...
{
    $sql = 'SELECT *
            FROM ' . $GLOBALS['yp']->table('suppliers') . '
            WHERE is_check = 1
...
function get_order_goods($order)
{
    ...
    $sql = "SELECT o.*, g.suppliers_id AS suppliers_id,IF(o.product_id > 0, p.product_number, g.goods_number) AS storage, o.goods_attr, IFNULL(b.brand_name, '') AS brand_name, p.product_sn " .
            ...
            "LEFT JOIN " . $GLOBALS['yp']->table('products') . " AS p ON o.product_id = p.product_id " .
            "LEFT JOIN " . $GLOBALS['yp']->table('goods') . " AS g ON o.goods_id = g.goods_id " .
...
function get_package_goods_list($package_id)
{
    $sql = "SELECT pg.goods_id, g.goods_name, (CASE WHEN pg.product_id > 0 THEN p.product_number ELSE g.goods_number END) AS goods_number, p.goods_attr, p.product_id, pg.goods_number AS
            ...
            FROM " . $GLOBALS['yp']->table('package_goods') . " AS pg
                LEFT JOIN " .$GLOBALS['yp']->table('goods') . " AS g ON pg.goods_id = g.goods_id
                LEFT JOIN " . $GLOBALS['yp']->table('products') . " AS p ON pg.product_id = p.product_id
    ...
    if ($good_product_str != '')
    {
        $sql = "SELECT ga.goods_attr_id, ga.attr_value, ga.attr_price, a.attr_name
                FROM " .$GLOBALS['yp']->table('goods_attr'). " AS ga, " .$GLOBALS['yp']->table('attribute'). " AS a
...
function order_delivery_num($order_id, $goods_id, $product_id = 0)
{
    $sql = 'SELECT SUM(G.send_number) AS sums
            FROM ' . $GLOBALS['yp']->table('delivery_goods') . ' AS G, ' . $GLOBALS['yp']->table('delivery_order') . ' AS O
...
function order_deliveryed($order_id)
{
    ...
    $sql = 'SELECT COUNT(delivery_id)
            FROM ' . $GLOBALS['yp']->table('delivery_order') . '
            WHERE order_id = \''. $order_id . '\'
            AND status = 0';
    $sum = $GLOBALS['db']->getOne($sql);
...
function delivery_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('delivery_order') . $where;
        $filter['record_count']   = $GLOBALS['db']->getOne($sql);
        $filter['page_count']     = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;
...
function back_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('back_order') . $where;
        $filter['record_count']   = $GLOBALS['db']->getOne($sql);
        $filter['page_count']     = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;
        ...
        $sql = "SELECT back_id, delivery_sn, order_sn, order_id, add_time, action_user, consignee, country,
                ...
                FROM " . $GLOBALS['yp']->table("back_order") . "
                $where
                ORDER BY " . $filter['sort_by'] . " " . $filter['sort_order']. "
                LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ", " . $filter['page_size'] . " ";
...
function delivery_order_info($delivery_id, $delivery_sn = '')
{
    ...
    $sql = "SELECT * FROM " . $GLOBALS['yp']->table('delivery_order');
    if ($delivery_id > 0)
    {
...
function back_order_info($back_id)
{
    ...
    $sql = "SELECT * FROM " . $GLOBALS['yp']->table('back_order') . "
...
function package_sended($package_id, $goods_id, $order_id, $extension_code, $product_id = 0)
{
    ...
    $sql = "SELECT SUM(DG.send_number)
            FROM " . $GLOBALS['yp']->table('delivery_goods') . " AS DG, " . $GLOBALS['yp']->table('delivery_order') . " AS o
...
function package_virtual_card_shipping($goods, $order_sn)
{
    ...
    foreach ($goods as $virtual_goods_key => $virtual_goods_value)
    {
        ...
        $sql = "SELECT card_id, card_sn, card_password, end_date, crc32
                FROM ".$GLOBALS['yp']->table('virtual_card')."
        ...
        /* 标记已经取出的卡片 */
        $sql = "UPDATE ".$GLOBALS['yp']->table('virtual_card')." SET ".
           "is_saled = 1 ,".
           "order_sn = '$order_sn' ".
        ...
        /* 获取订单信息 */
        $sql = "SELECT order_id, order_sn, consignee, email FROM ".$GLOBALS['yp']->table('order_info'). " WHERE order_sn = '$order_sn'";
...
function delivery_return_goods($delivery_id, $delivery_order)
{
    /* 查询：取得发货单商品 */
    $goods_sql = "SELECT *
                 FROM " . $GLOBALS['yp']->table('delivery_goods') . "
                 WHERE delivery_id = " . $delivery_order['delivery_id'];
    $goods_list = $GLOBALS['db']->getAll($goods_sql);
    /* 更新： */
...
{
	$delivery_order = $GLOBALS['db']->getRow("select * from ".$GLOBALS['yp']->table('delivery_order')." where delivery_id = ".$delivery_id);
	...
	$sql = "insert into ".$GLOBALS['yp']->table('kuaidi_order')." (user_id, to_name, to_tel, to_region_id, to_address, goods_name, package_num, start_time, end_time, add_time) values ('$delivery_order[user_id]', '$delivery_order[consignee]', '$to_tel', '$delivery_order[district]', '$delivery_order[address]', '$delivery_id', '1', '0', '0', '".gmtime()."')";
	$GLOBALS['db']->query($sql);
	...
	foreach ($orderstatus_array AS $okey=>$oval)
	{
		...
		$sql="insert into ".$GLOBALS['yp']->table('kuaidi_order_status').
...
{
	$kuaidi_order_id = $GLOBALS['db']->getRow("select order_id from ".$GLOBALS['yp']->table('kuaidi_order')." where goods_name='".$delivery_id."' and send_name=''");

	if ($kuaidi_order_id['order_id']) {
		$GLOBALS['db']->query("delete from ".$GLOBALS['yp']->table('kuaidi_order')." where order_id=".$kuaidi_order_id['order_id']);
		$GLOBALS['db']->query("delete from ".$GLOBALS['yp']->table('kuaidi_order_status')." where order_id=".$kuaidi_order_id['order_id']);
	}
}

function update_kuaidi_order($delivery_id, $order_sn = "")
{
	$GLOBALS['db']->query("update ".$GLOBALS['yp']->table('kuaidi_order')." set order_sn='".$order_sn."' where goods_name='".$delivery_id."' and send_name=''");
}
...

{
	...
	$sql = "SELECT * " .
				...
				" LEFT JOIN " . $GLOBALS['yp']->table('extpintuan') . " AS pt ON pt.pt_id  = pto.pt_id  " .
