 ...
 * ============================================================================
 * $Author: zhangyh $
 * $Id: takegoods.php 17217 2017-04-01 06:29:08Z zhangyh $
*/
...
if ($_REQUEST['act'] == 'update')
{
	...
	$sql = "select * from ". $yp->table('pricecut') ."  where pricecut_id='$id' ";
...
{  
    ...
    $id = isset($_REQUEST['id']) ? intval($_REQUEST['id']) : 0;
	$sql = "select p.*, g.goods_name from ". $yp->table('pricecut') ." AS p left join ".$yp->table('goods') ." AS g on p.goods_id=g.goods_id  where p.pricecut_id='$id' ";
    $notice = $db->getRow($sql);   
	if ($notice)
	{
...
elseif ($_REQUEST['act'] == 'remove')
{
	...
	{
		$sql="DELETE FROM " .$yp->table('pricecut'). " WHERE pricecut_id='$id'";
	}
    $db->query($sql);
...
function get_pricecut_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['yp']->table('pricecut')." AS p $where ";
        $filter['record_count'] = $GLOBALS['db']->getOne($sql); 

        /* 分页大小 */
        $filter = page_and_size($filter);

        $sql = "SELECT p.*, g.goods_name FROM " .$GLOBALS['yp']->table('pricecut'). " AS p  ".
					" left join ". $GLOBALS['yp']->table('goods') ." AS g on p.goods_id=g.goods_id $where".
...
function get_min_price($goods_id)
{
     $sql="SELECT g.goods_type, g.promote_price, g.promote_start_date, g.promote_end_date, g.shop_price, a.attr_id,a.attr_name ".
				...
				" AS g left join ". $GLOBALS['yp']->table('attribute') ." AS a on g.goods_type=a.cat_id ".
