<?php
/**
 * QQ120029121 商品分析：商品销售排行
 * ============================================================================
 * 演示地址: http://demo.coolhong.com/
 * ============================================================================
 * $Author: jtypmall $
 * $Id: goods_stats.php 2015-10-28 11:02:08Z jtypmall $
 */
...
elseif ($_REQUEST['act'] == 'export')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_sn, g.goods_name, g.goods_attr, SUM(g.goods_number) sales_volume, SUM(g.goods_price*g.goods_number) sales_money, '
        ...
        . $GLOBALS['yp']->table('order_goods') . ' g, ' . $GLOBALS['yp']->table('order_info') . ' i ' . $where
...
function get_sell_list($start_date, $end_date, $sel_shop, $supplier_id)
{
    ...
    if($result === false)
    {
        ...
        $cnt = $GLOBALS['db']->getOne(
            'SELECT COUNT(*) FROM (SELECT DISTINCT goods_id FROM '. $GLOBALS['yp']->table('order_goods')
            .' WHERE order_id IN (SELECT order_id FROM ' . $GLOBALS['yp']->table('order_info') . ' i ' . $where . ')) t'
        );
