...
/**
 * QQ120029121 站外JS投放的统计程序
 * ============================================================================
 * 演示地址: http://demo.coolhong.com  开发QQ:120029121    309485552
 * ============================================================================
 * $Author: PRINCE $
 * $Id: adsense.php 17217 2017-04-01 06:29:08Z PRINCE $
*/
...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'download')
{
    ...
    $sql = "SELECT a.ad_id, a.ad_name, b.* ".
           "FROM " .$yp->table('ad'). " AS a, " .$yp->table('adsense'). " AS b ".
    ...
    while ($rows = $db->fetchRow($res))
    {
		...
		/* 获取当前广告所产生的订单总数 */
        $sql2 = 'SELECT COUNT(order_id) FROM ' .$yp->table('order_info'). " WHERE from_ad='$rows[ad_id]' AND referer='$rows[referer]'";
        $rows['order_num'] = $db->getOne($sql2);

        /* 当前广告所产生的已完成的有效订单 */
        $sql3 = "SELECT COUNT(order_id) FROM " .$yp->table('order_info').
    ...
    /* 站外JS投放商品的统计数据 */
    $goods_stats    = array();
    $goods_sql      = "SELECT from_ad, referer, clicks FROM " .$yp->table('adsense').
    ...
    while ($rows2 = $db->fetchRow($goods_res))
    {
		...
		/* 获取当前广告所产生的订单总数 */
        $rows2['order_num'] = $db->getOne("SELECT COUNT(order_id) FROM " .$yp->table('order_info'). " WHERE referer='$rows2[referer]'");

        /* 当前广告所产生的已完成的有效订单 */
        $sql = "SELECT COUNT(order_id) FROM " .$yp->table('order_info').
    ...
    if ($_REQUEST['act'] == 'download')
    {
        ...
        echo yp_iconv(YP_CHARSET, 'GB2312', $data);
        exit;
    }
