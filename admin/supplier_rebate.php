...
elseif ($_REQUEST['act'] == 'export_supps')
{
	...
	if (YP_CHARSET != 'gbk')
    {
        echo yp_iconv(YP_CHARSET, 'gbk', $export) . "\t";
    }
    else
    {
        echo $export. "\t";
...
function supplier_rebate_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(sr.supplier_id) FROM " . $GLOBALS['yp']->table('supplier_rebate_log') ." AS sr  " . $where . " group by sr.supplier_id ";
...
{
	global $db,$yp;
	$sql = "select sum(all_money) as all_money,sum(rebate_money) as rebate_money,sum(result_money) as result_money ,sum(split_money) as split_money from ".$yp->table('supplier_rebate_log');
...
{
	global $db,$yp;
	$sql = "select srl.supplier_id, s.supplier_name from ".$yp->table('supplier_rebate_log')." as srl left join ".$yp->table('supplier')." as s on srl.supplier_id=s.supplier_id group by srl.supplier_id";
