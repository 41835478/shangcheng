...
elseif ($_REQUEST['act'] == 'back_info')
{
	...
	if($back_order)
	{
		$base_order = $db->getRow("select * from ". $yp->table('order_info') ." where order_id='$back_order[order_id]' ");
		if($base_order)
		{
	...
	$sql = "select * from ". $yp->table('order_goods') . $where;
    ...
    /* 取得退换货商品 */
    $goods_sql = "SELECT * FROM " . $yp->table('back_goods') . 
	...
	$back_order['country_name'] = $db->getOne("SELECT region_name FROM ".$yp->table('region')." WHERE region_id = '$back_order[country]'");
	$back_order['province_name'] = $db->getOne("SELECT region_name FROM ".$yp->table('region')." WHERE region_id = '$back_order[province]'");
	$back_order['city_name'] = $db->getOne("SELECT region_name FROM ".$yp->table('region')." WHERE region_id = '$back_order[city]'");
	$back_order['district_name'] = $db->getOne("SELECT region_name FROM ".$yp->table('region')." WHERE region_id = '$back_order[district]'");
	...
	/* 取得订单操作记录 */
    $act_list = array();
    $sql = "SELECT * FROM " . $yp->table('back_action') . " WHERE back_id = '$back_id' ORDER BY log_time DESC,action_id DESC";
	...
	/* 回复留言图片 demo.coolhong.com添加    今-天-优-品-多-商-户-系-统 Q-Q 120 029 121 */
...
elseif ($_REQUEST['act'] == 'operate')
{
    ...
    elseif (isset($_POST['refund']))
    {
		...
		$sql="select * from ".$yp->table('back_order')." where back_id='$back_id' ";
...
elseif ($_REQUEST['act'] == 'operate_refund')
{
	...
	$action_note    = isset($_REQUEST['action_note']) ? trim($_REQUEST['action_note']) : '';
	$order = back_order_info($back_id);
	$sql = "update ". $yp->table('back_goods') ." set status_refund='$status_refund'  where back_id='$back_id' and (back_type='0' or back_type='4') ";
	$db->query($sql);
	...
	$sql2 = "update ". $yp->table('back_order') ." set  status_refund='$status_refund',  refund_money_2='$refund_money_2', refund_type='$_REQUEST[refund_type]', refund_desc='$refund_desc' where back_id='$back_id' ";
	$db->query($sql2);
...
elseif ($_REQUEST['act'] == 'remove_back')
{
        ...
        if(is_array($back_id))
        {
            ...
            $sql = "DELETE FROM ".$yp->table('back_order'). " WHERE back_id in ($back_id_list)";
            $db->query($sql);    
			$sql = "DELETE FROM ".$yp->table('back_goods'). " WHERE back_id in ($back_id_list)";
            $db->query($sql);
        }
        else
        {
            ...
            $db->query($sql);
			$sql = "DELETE FROM ".$yp->table('back_goods'). " WHERE back_id = '$back_id'";			
            $db->query($sql);
        }
		//echo $sql;
...
elseif ($_REQUEST['act'] == 'replay')
{
		...
		$db->query("INSERT INTO ".$yp->table('back_replay')." (back_id, message, add_time) VALUES ('$back_id', '$message', '$add_time')");
...
function back_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('back_order') ." AS b ". $where;
        $filter['record_count']   = $GLOBALS['db']->getOne($sql);
        $filter['page_count']     = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;

        /* 查询 */
        if($filter['supp']){
        	$sql = "SELECT b.*,s.supplier_name FROM " . $GLOBALS['yp']->table("back_order") . " AS b LEFT JOIN ". $GLOBALS['yp']->table("supplier") ." AS s ON b.supplier_id=s.supplier_id ".
			    "  $where   ORDER BY " . $filter['sort_by'] . " " . $filter['sort_order']. 
				" LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ", " . $filter['page_size'] . " ";
        }else{
        	$sql = "SELECT * FROM " . $GLOBALS['yp']->table("back_order") . " AS b ".
			    "  $where   ORDER BY " . $filter['sort_by'] . " " . $filter['sort_order']. 
				" LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ", " . $filter['page_size'] . " ";
        }
    ...
    foreach ($row AS $key => $value)
    {
		...
		$sql_og = "SELECT * FROM " . $GLOBALS['yp']->table('back_goods') . " WHERE back_id = " . $value['back_id'];
...
function back_order_info($back_id)
{
    ...
    if ($back)
    {
		...
		if ($back['back_type'] == 4)
		{
			$back['money_paid'] = $GLOBALS['db']->getOne("SELECT money_paid FROM " . $GLOBALS['yp']->table('order_info') . " WHERE order_id = " . $back['order_id']);
		}
...
function update_back($back_id, $status_back, $status_refund )
{
	...
	$sql = "update ". $GLOBALS['yp']->table('back_order') ." set  $setsql where back_id='$back_id' ";
	$GLOBALS['db']->query($sql);

	if($status_back =='5') //通过申请
	{
	   $status_b = $GLOBALS['db']->getOne("select back_type from " . $GLOBALS['yp']->table('back_order') . " where back_id='$back_id'");
	   $status_b = ($status_b == 4) ? 4 : 0;
	   $status_bo = $GLOBALS['db']->getOne("select order_sn from " . $GLOBALS['yp']->table('back_order') . " where back_id='$back_id'");
	   $close_order = $GLOBALS['db']->getOne("select shipping_status from " . $GLOBALS['yp']->table('order_info') . " where order_sn = '" . $status_bo . "'");
	   if ($close_order < 1)
	   {
		   $sql3="update ". $GLOBALS['yp']->table('order_info') ." set order_status='2', to_buyer='用户对订单内的部分或全部商品申请退款并取消订单' where order_sn = '" . $status_bo . "'";
		   $GLOBALS['db']->query($sql3);
	   }
	   
	   $sql="update ". $GLOBALS['yp']->table('back_goods') ." set status_back='$status_b' where back_id='$back_id' ";
	   $GLOBALS['db']->query($sql);
	   $sql2="update ". $GLOBALS['yp']->table('back_order') ." set status_back='$status_b' where back_id='$back_id' ";
	   $GLOBALS['db']->query($sql2);
	}
	if($status_back =='6') //拒绝申请
	{
	   $sql="update ". $GLOBALS['yp']->table('back_goods') ." set status_back='$status_back' where back_id='$back_id' ";
	   $GLOBALS['db']->query($sql);
	   $sql2="update ". $GLOBALS['yp']->table('back_order') ." set status_back='$status_back' where back_id='$back_id' ";
	   $GLOBALS['db']->query($sql2);
	}
	...
	if($status_back =='1' or $status_back =='3') //收到退换回的货物，完成退换货
	{
	   ...
	   $get_order_id = $GLOBALS['db']->getOne("SELECT order_id FROM " . $GLOBALS['yp']->table('back_order') . " WHERE back_id = '" . $back_id . "'");
	   $get_goods_id = $GLOBALS['db']->getCol("SELECT goods_id FROM " . $GLOBALS['yp']->table('back_order') . " WHERE order_id = '" . $get_order_id . "' AND status_back = '3' AND back_type <> '3'");
	   if (count($get_goods_id) > 0)
	   {
    	   $get_goods_id_c =  (count($get_goods_id) == 1 ? ("<> '" . implode(',', $get_goods_id) . "'") : ("NOT IN (" . implode(',', $get_goods_id) . ")"));
    	   $no_back = $GLOBALS['db']->getOne("SELECT COUNT(rec_id) FROM " . $GLOBALS['yp']->table('order_goods') . " WHERE order_id = '" . $get_order_id . "' AND goods_id " . $get_goods_id_c);
    	   if ($no_back == 0)
    	   {
        	   $sql3="UPDATE ". $GLOBALS['yp']->table('order_info') ." SET order_status='2' WHERE order_id='" . $get_order_id . "' ";
        	   $GLOBALS['db']->query($sql3);
    	   }
	   }
	   $get_goods_info = $GLOBALS['db']->getRow("SELECT goods_id, back_type FROM " . $GLOBALS['yp']->table('back_goods') . " WHERE back_id = '" . $back_id . "'");
	   if ($status_back == '3' && $get_goods_info['back_type'] != '3') // 退款退货完成时，改变订单中商品的is_back值
	   {
	       $sql4 = "UPDATE " .$GLOBALS['yp']->table('order_goods') . " SET is_back = 1 WHERE goods_id = '" . $get_goods_info['goods_id'] . "' AND order_id = '" . $get_order_id . "'";
	       $GLOBALS['db']->query($sql4);
		   
		   //退款完成后，进行返库
		   $back_type = $GLOBALS['db']->getOne("SELECT back_type FROM " . $GLOBALS['yp']->table('back_order') . " WHERE back_id = '" . $back_id . "'");
		   $stock_dec_time = $GLOBALS['db']->getOne("SELECT value FROM " . $GLOBALS['yp']->table('shop_config') . " WHERE code =  'stock_dec_time'");
		   if ($back_type == 4 && $stock_dec_time == 1)
		   {
			   $back_go = $GLOBALS['db']->getAll("SELECT * FROM " . $GLOBALS['yp']->table('order_goods') . " WHERE order_id = " . $get_order_id);
			   foreach($back_go as $back_g)
			   {
				   if ($back_g['product_id'] > 0)
				   {
					   $GLOBALS['db']->query("UPDATE " . $GLOBALS['yp']->table('products') . " SET product_number = product_number + " . $back_g['goods_number'] . " WHERE product_id = " . $back_g['product_id']);
				   }
					$GLOBALS['db']->query("UPDATE " . $GLOBALS['yp']->table('goods') . " SET goods_number = goods_number + " . $back_g['goods_number'] . " WHERE goods_id = " . $back_g['goods_id']);
			   }
		   }
	   }
	}
	...
	{
	   $sql="update ". $GLOBALS['yp']->table('back_goods') ." set status_back='$status_back' where back_type in(1,2,3) and back_id='$back_id' ";
	   $GLOBALS['db']->query($sql);
	}
	if($status_refund=='1') //退款
	{
	   $sql="update ". $GLOBALS['yp']->table('back_goods') ." set status_refund='$status_refund' where back_type ='0' and back_id='$back_id' ";
	   $GLOBALS['db']->query($sql);
	}
}
