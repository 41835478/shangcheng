...
$exc = new exchange($yp->table("street_category"), $db, 'str_id', 'str_name');
...
if ($_REQUEST['act'] == 'insert')
{
 ...
 foreach($arrCatName as $arrCatNameValue)
 {
  ...
  /* 入库的操作 */
  if ($db->autoExecute($yp->table('street_category'), $cat) !== false)
  {
   $cat_id = $db->insert_id();
  }
 }
...
if ($_REQUEST['act'] == 'remove')
{
    ...
    $cat_name = $db->getOne('SELECT str_name FROM ' .$yp->table('street_category'). " WHERE str_id='$cat_id'");

    /* 当前分类下是否存在店铺 */
    $shop_count = $db->getOne('SELECT COUNT(*) FROM ' .$yp->table('supplier_street'). " WHERE supplier_type='$cat_id'");

    /* 如果不存在店铺，则删除之 */
    if ($shop_count == 0)
    {
        /* 删除分类 */
        $sql = 'DELETE FROM ' .$yp->table('street_category'). " WHERE str_id = '$cat_id'";
        $db->query($sql);
    }
    else
    {
...
{
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['yp']->table('street_category').
...
function str_update($cat_id, $args)
{
    ...
    return $GLOBALS['db']->autoExecute($GLOBALS['yp']->table('street_category'), $args, 'update', "str_id='$cat_id'");
}
...
{
	$sql = "SELECT supplier_type,count( supplier_type ) as num FROM " . $GLOBALS['yp']->table('supplier_street') . " GROUP BY supplier_type";
	$rows = $GLOBALS['db']->getAll($sql);
    $sql = "SELECT * ".
           " FROM " . $GLOBALS['yp']->table('street_category'). 
           " ORDER BY sort_order";
