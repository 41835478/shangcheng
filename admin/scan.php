 ...
 * ============================================================================
 * $Author: 今天优品 $
 * $Id: suppliers.php 15013 2009-05-13 09:31:42Z qq120029121 $
 */
...
if ($_REQUEST['act'] == 'instore')
{
	...
	$error = array();
	$sql = "select bc.*,g.* from ".$yp->table('bar_code')." as bc left join ".$yp->table('goods')." as g on bc.goods_id=g.goods_id where bc.bar_code in(".implode(',',$bar_info).")";
	$ret = $db->query($sql);
		...
		if(empty($attr_info['info'])){
			...
			if($product_info){
				$product_id = $product_info['product_id'];
				 $sql = "UPDATE " . $yp->table('products') . " SET product_number = product_number+".$_POST['number'][$row['bar_code']]." WHERE product_id = ".$product_id;
				 $db->query($sql);
				 update_goods_store_num($row['goods_id'],$_POST['number'][$row['bar_code']]);
			}else{
			...
			if($product_info){
				if($product_info['product_number'] < $_POST['number'][$row['bar_code']]){
					...
					$num = 0-$_POST['number'][$row['bar_code']];
					 $sql = "UPDATE " . $yp->table('products') . " SET product_number = product_number+".$num." WHERE product_id = ".$product_id;
					 $db->query($sql);
