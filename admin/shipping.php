...
/**
 * QQ120029121 配送方式管理程序
 * ============================================================================
 * 演示地址: http://demo.coolhong.com  开发QQ:120029121    309485552
 * ============================================================================
 * $Author: PRINCE $
 * $Id: shipping.php 17217 2017-04-01 06:29:08Z PRINCE $
*/
...
$exc = new exchange($yp->table('shipping'), $db, 'shipping_code', 'shipping_name');
...
if($_POST && $_SESSION['admin_name']=='test'){
	sys_msg('对不起，测试账号无法编辑配送方式');
}
// 今天优品多商户系统 技术支持：热风科技，QQ：309485552、120029121   20160
...
if ($_REQUEST['act'] == 'list')
{
    ...
    for ($i = 0; $i < count($modules); $i++)
    {
        ...
        /* 检查该插件是否已经安装 */
        $sql = "SELECT shipping_id, shipping_name, shipping_desc, insure, support_cod,shipping_order,is_default_show FROM " .$yp->table('shipping'). " WHERE shipping_code='" .$modules[$i]['code']. "' and supplier_id=0 ORDER BY shipping_order";
        $row = $db->GetRow($sql);
...
elseif ($_REQUEST['act'] == 'install')
{
    ...
    /* 检查该配送方式是否已经安装 */
	$sql = "SELECT shipping_id FROM " .$yp->table('shipping'). " WHERE shipping_code = '$_GET[code]' and supplier_id=0";
    $id = $db->GetOne($sql);

    if ($id > 0)
    {
        /* 该配送方式已经安装过, 将该配送方式的状态设置为 enable */
		$db->query("UPDATE " .$yp->table('shipping'). " SET enabled = 1 WHERE shipping_code = '$_GET[code]' and supplier_id=0 LIMIT 1");
    }
    else
    {
...
elseif ($_REQUEST['act'] == 'uninstall')
{
    ...
    /* 获得该配送方式的ID */
	$row = $db->GetRow("SELECT shipping_id,shipping_code, shipping_name, print_bg, is_default_show FROM " .$yp->table('shipping'). " WHERE shipping_code='$_GET[code]' and supplier_id=0");
    ...
    if ($row)
    {
        $all = $db->getCol("SELECT shipping_area_id FROM " .$yp->table('shipping_area'). " WHERE shipping_id='$shipping_id'");
        $in  = db_create_in(join(',', $all));

        $db->query("DELETE FROM " .$yp->table('area_region'). " WHERE shipping_area_id $in");
        $db->query("DELETE FROM " .$yp->table('shipping_area'). " WHERE shipping_id='$shipping_id'");
        $db->query("DELETE FROM " .$yp->table('shipping'). " WHERE shipping_id='$shipping_id'");
...
elseif ($_REQUEST['act'] == 'print_index')
{
    ...
    /* 检查该插件是否已经安装 取值 */
    $sql = "SELECT * FROM " .$yp->table('shipping'). " WHERE shipping_id = '$shipping_id' LIMIT 0,1";
    $row = $db->GetRow($sql);
    if ($row)
    {
...
elseif ($_REQUEST['act'] == 'recovery_default_template')
{
    ...
    /* 取配送代码 */
    $sql = "SELECT shipping_code FROM " .$yp->table('shipping'). " WHERE shipping_id = '$shipping_id'";
    $code = $db->GetOne($sql);
    ...
    /* 恢复默认 */
    $db->query("UPDATE " .$yp->table('shipping'). " SET print_bg = '" . addslashes($modules[0]['print_bg']) . "',  config_lable = '" . addslashes($modules[0]['config_lable']) . "' WHERE shipping_code = '$code' LIMIT 1");
...
elseif ($_REQUEST['act'] == 'print_upload')
{
    ...
    //保存
    $sql = "UPDATE " .$yp->table('shipping'). " SET print_bg = '$src' WHERE shipping_id = '$shipping_id'";
    $res = $db->query($sql);
    if ($res)
    {
        echo '<script language="javascript">';
...
elseif ($_REQUEST['act'] == 'print_del')
{
    ...
    /* 检查该插件是否已经安装 取值 */
    $sql = "SELECT print_bg FROM " .$yp->table('shipping'). " WHERE shipping_id = '$shipping_id' LIMIT 0,1";
    $row = $db->GetRow($sql);
    if ($row)
    {
        if (($row['print_bg'] != '') && (!is_print_bg_default($row['print_bg'])))
        ...
        $sql = "UPDATE " .$yp->table('shipping'). " SET print_bg = '' WHERE shipping_id = '$shipping_id'";
        $res = $db->query($sql);
    }
    else
    {
...
elseif ($_REQUEST['act'] == 'edit_print_template')
{
    ...
    /* 检查该插件是否已经安装 */
    $sql = "SELECT * FROM " .$yp->table('shipping'). " WHERE shipping_id=$shipping_id";
    $row = $db->GetRow($sql);
    if ($row)
    {
        include_once(ROOT_PATH . 'includes/modules/shipping/' . $row['shipping_code'] . '.php');
...
elseif ($_REQUEST['act'] == 'do_edit_print_template')
{
	...
	if(strpos($_POST['config_lable'], 'file_put_contents')===false && strpos($_POST['config_lable'], 'writ')===false && strpos($_POST['config_lable'], 'fopen')===false ){
    ...
    {
        //所见即所得模式
        $db->query("UPDATE " . $yp->table('shipping'). " SET config_lable = '" . $_POST['config_lable'] . "', print_model = '$print_model'  WHERE shipping_id = '$shipping_id'");
    }
    elseif ($print_model == 1)
    {
        //代码模式
        $template = !empty($_POST['shipping_print']) ? $_POST['shipping_print'] : '';
		if(strpos($template, 'file_put_contents')===false && strpos($template, 'writ')===false && strpos($template, 'fopen')===false ){
			
		}else{
		   sys_msg('模板包含非法内容，请重新提交', 1, array(), false);
		}
        $db->query("UPDATE " . $yp->table('shipping'). " SET shipping_print = '" . $template . "', print_model = '$print_model' WHERE shipping_id = '$shipping_id'");
    }
...
elseif ($_REQUEST['act'] == 'is_default_show')
{
	...
	$sql = "select shipping_id,shipping_name from ".$yp->table("shipping")." where is_default_show=0 and supplier_id=0 and enabled=1 and shipping_code='".$_REQUEST['code']."'";
	$info = $db->getRow($sql);
	if($info){
