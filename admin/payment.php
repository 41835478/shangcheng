...
/**
 * QQ120029121 支付方式管理程序
 * ============================================================================
 * 演示地址: http://demo.coolhong.com  开发QQ:120029121    309485552
 * ============================================================================
 * $Author: PRINCE $
 * $Id: payment.php 17217 2017-04-01 06:29:08Z PRINCE $
*/
...
$exc = new exchange($yp->table('payment'), $db, 'pay_code', 'pay_name');

/*------------------------------------------------------ */
//-- 支付方式列表 ?act=list
/*------------------------------------------------------ */

// 今天优品多商户系统 技术支持：热风科技，QQ：3/0/9/4/8/5/5/5/2、1/2/0/0/2/9/1/2/1   20160802
if($_REQUEST['act'] != 'list' && $_SESSION['admin_name']=='test'){
	sys_msg('对不起，测试账号无法编辑支付方式');
}
// 今天优品多商户系统 技术支持：热风科技，QQ：3/0/9/4/8/5/5/5/2、1/2/0/0/2/9/1/2/1   20160802
...
if ($_REQUEST['act'] == 'list')
{
    /* 查询数据库中启用的支付方式 */
    $pay_list = array();
    $sql = "SELECT * FROM " . $yp->table('payment') . " WHERE enabled = '1' ORDER BY pay_order";
    $res = $db->query($sql);
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT * FROM " . $yp->table('payment') . " WHERE pay_code = '$_REQUEST[code]' AND enabled = '1'";
    $pay = $db->getRow($sql);
    if (empty($pay))
    {
...
elseif (isset($_POST['Submit']))
{
    ...
    $sql = "SELECT COUNT(*) FROM " . $yp->table('payment') .
    ...
    {
        /* 安装，检查该支付方式是否曾经安装过 */
        $sql = "SELECT COUNT(*) FROM " . $yp->table('payment') . " WHERE pay_code = '$_REQUEST[pay_code]'";
        if ($db->GetOne($sql) > 0)
        {
        ...
        else
        {
           /* 代码修改_start   By demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1 */
            ...
            $sql = "INSERT INTO " . $yp->table('payment') . " (pay_code, pay_name, pay_desc, pay_config, is_cod, pay_fee, enabled, is_online, is_pickup)" .
