...
require_once(ROOT_PATH . '/' . ADMIN_PATH. "/nusoap/nusoap.php");   //代码增加  By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121
include_once(ROOT_PATH . '/includes/cls_image.php');
$image = new cls_image($_CFG['bgcolor']);
$exc = new exchange($yp->table('goods'), $db, 'goods_id', 'goods_name');
...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
    ...
    if(intval($_REQUEST['supp'])>0){
		...
		$sql_supplier = "select supplier_id, supplier_name FROM " . $GLOBALS['yp']->table("supplier") . " where status='1' order by supplier_id ";
	...
	$supplier_list = supplier_list_info(' status = 1 ');
	$supplier_list_count = count($supplier_list);
    $smarty->assign('supplier_list', ($supplier_list_count == 0 ? 0 : $supplier_list)); // 取入驻商商列表   寒冰  qq309    485552   平台转移商品到入驻商   20161206
...
//            'promote_end_date'   => local_date('Y-m-d', local_strtotime('+1 month')),
        ...
        $sql = "DELETE FROM " . $yp->table('link_goods') .
        ...
        $sql = "DELETE FROM " . $yp->table('group_goods') .
        ...
        /* 属性 */
        $sql = "DELETE FROM " . $yp->table('goods_attr') . " WHERE goods_id = 0";
        $db->query($sql);
    ...
    {
		/*查询输出条形码*/
		$sql = "SELECT * FROM". $yp->table('bar_code') ."WHERE goods_id ='$_REQUEST[goods_id]'";
		$bar_code =$db->getAll($sql);
	
        /* 商品信息 */
        $sql = "SELECT * FROM " . $yp->table('goods') . " WHERE goods_id = '$_REQUEST[goods_id]'";
        $goods = $db->getRow($sql);

		// 代码增加_start_prince20150129admin_goods  demo.coolhong.com
		
		$r_b_id = $db->getOne("select brand_name from ".$yp->table('brand')." where brand_id=".$goods['brand_id']);
...
//            $goods['promote_end_date'] = local_date('Y-m-d', $goods['promote_end_date']);
        ...
        if ($_REQUEST['act'] == 'copy')
        {
            ...
            $sql = "SELECT '0' AS goods_id, link_goods_id, is_double, '$_SESSION[admin_id]' AS admin_id" .
                    " FROM " . $yp->table('link_goods') .
                    " WHERE goods_id = '$_REQUEST[goods_id]' ";
            $res = $db->query($sql);
            while ($row = $db->fetchRow($res))
            {
                $db->autoExecute($yp->table('link_goods'), $row, 'INSERT');
            }
            ...
            $sql = "SELECT 0 AS parent_id, goods_id, goods_price, '$_SESSION[admin_id]' AS admin_id " .
                    "FROM " . $yp->table('group_goods') .
                    " WHERE parent_id = '$_REQUEST[goods_id]' ";
            $res = $db->query($sql);
            while ($row = $db->fetchRow($res))
            {
                $db->autoExecute($yp->table('group_goods'), $row, 'INSERT');
            }
            ...
            $sql = "SELECT 0 AS goods_id, article_id, '$_SESSION[admin_id]' AS admin_id " .
                    "FROM " . $yp->table('goods_article') .
                    " WHERE goods_id = '$_REQUEST[goods_id]' ";
            $res = $db->query($sql);
            while ($row = $db->fetchRow($res))
            {
                $db->autoExecute($yp->table('goods_article'), $row, 'INSERT');
            }
            ...
            $sql = "SELECT 0 AS goods_id, attr_id, attr_value, attr_price " .
                    "FROM " . $yp->table('goods_attr') .
                    " WHERE goods_id = '$_REQUEST[goods_id]' ";
            $res = $db->query($sql);
            while ($row = $db->fetchRow($res))
            {
                $db->autoExecute($yp->table('goods_attr'), addslashes_deep($row), 'INSERT');
            }
        }

        // 扩展分类
        $other_cat_list = array();
        $sql = "SELECT cat_id FROM " . $yp->table('goods_cat') . " WHERE goods_id = '$_REQUEST[goods_id]'";
        ...
        /* 图片列表 */
        $sql = "SELECT * FROM " . $yp->table('goods_gallery') . " WHERE goods_id = '$goods[goods_id]'";
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($_POST['goods_sn'])
    {
        $sql = "SELECT COUNT(*) FROM " . $yp->table('goods') .
    ...
    {
        $max_id     = $is_insert ? $db->getOne("SELECT MAX(goods_id) + 1 FROM ".$yp->table('goods')) : $_REQUEST['goods_id'];
    ...
    $click_count = isset($_POST['click_count']) ? intval($_POST['click_count']) : '0';// add by qq120029121 20170907
   //早市
	...
	//定制
	$is_custom = $_REQUEST['is_custom'];
	$custom_stye = $_REQUEST['custom_stye'];
	//手机专享价格 app   jx 
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $yp->table('goods') . " (goods_name, goods_name_style, ghost_count,click_count,  goods_sn,morning_market_price,is_morning_market, " .
                    "cat_id, brand_id, shop_price, market_price, is_promote, zhekou, promote_price,is_custom, custom_stye, " .
                ...
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$ghost_count',  '$click_count','$goods_sn','$morning_market_price','$is_morning_market', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote', '$zhekou', '$promote_price', '$is_custom', '$custom_stye',".
        ...
        {
            $sql = "INSERT INTO " . $yp->table('goods') . " (goods_name, goods_name_style, ghost_count,click_count,  goods_sn, " .
                ...
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$ghost_count',  '$click_count','$goods_sn', '$catgory_id', " .
    ...
    $db->query($sql);
            ...
            if ($cost_price > 0 and $is_distrib_goods == 1 )     
                 ...
                 {
				 $is_distrib_goods_id = $db->getOne('SELECT MAX(goods_id) AS is_distrib_goods_id FROM ' .$yp->table('goods'));
				
				  
				$sql = "insert into ".$GLOBALS['yp']->table('ypmart_distrib_goods')." (`distrib_time`,`start_time`,`end_time`,`distrib_money`,`distrib_type`,`goods_id`) 
...
/*------------------------------*/ 
	...
	if($goods_number>0)
	{
		...
		$dispose = "SELECT email, tel, is_dispose FROM " .$yp->table('booking_goods'). " WHERE  goods_id = '$_REQUEST[goods_id]'";
	    ...
	    $msg_goods_url = $GLOBALS['yp']->url(). $msg_goods_url;
		 ...
		 for($i=0;$i<count($email);$i++)
		{
			if($email[$i]['is_dispose']==0)
			{
				...
				$sql = "UPDATE " . $yp->table('booking_goods') . " SET is_dispose = 1 WHERE  goods_id = '$_REQUEST[goods_id]'";
                $db->query($sql);
			}
		}
		...
		if($sms_on>0)
		{
			...
			for($i=0;$i<count($email);$i++)
			{
				if($email[$i]['is_dispose']==0)
				{
					$templateParam=Array("code"=>mb_substr($msg_goods_name, 0,12,"UTF8").'...');
					$templateCode=$GLOBALS['_CFG']['sms_goods_stockout_dayu'];
					$r=sendSMS($email[$i]['tel'], $sms_content,'', '',$templateParam , $templateCode);//已支持阿里大于 QQ 120029121
					if($r==true)
					{
						$sql = "UPDATE " . $yp->table('booking_goods') . " SET is_dispose = 2 WHERE  goods_id = '$_REQUEST[goods_id]'";
						$db->query($sql);
					}	
					else
					{
						$sql = "UPDATE " . $yp->table('booking_goods') . " SET is_dispose = 3 WHERE  goods_id = '$_REQUEST[goods_id]'";
						$db->query($sql);
					}
				}			    
	...
	/* 代码增加_start  Byjdy    为了便于新手朋友修改，这里采用增加代码的方法来修改，没有采用修改代码的方法 */
	 $sql = "UPDATE " .$yp->table('goods'). " SET is_catindex = '$_REQUEST[is_catindex]' WHERE goods_id = '$goods_id' LIMIT 1";
	 $db->query($sql);
	 /*存入条形码*/
	if($_POST['txm_shu'] && $_POST['tiaoxingm']){//如果txm_shu 和 tiaoxingm存在 就存入  不存在就不执行
		if(isset($_POST['txm_shu']) && isset($_POST['tiaoxingm']) || (empty($_POST['txm_shu'])) && (empty($_POST['tiaoxingm'])) ){
			...
			$db->query("DELETE FROM" .$yp->table('bar_code')."WHERE goods_id ='$goods_id'");//根据商品ID清空数据
			foreach($type as $key=>$value){
				foreach($bar_code as $k=>$v){
					...
					if($key == $k){
						$sql = "INSERT INTO " . $yp->table('bar_code') . " (goods_id, taypes, bar_code) " .
    ...
    if ((isset($_POST['attr_id_list']) && isset($_POST['attr_value_list'])) || (empty($_POST['attr_id_list']) && empty($_POST['attr_value_list'])))
    {
        ...
        $sql = "SELECT attr_id, attr_index FROM " . $yp->table('attribute') . " WHERE cat_id = '$goods_type'";
        ...
        $sql = "SELECT g.*, a.attr_type
                FROM " . $yp->table('goods_attr') . " AS g
                    LEFT JOIN " . $yp->table('attribute') . " AS a
        ...
        $sql = "UPDATE " .$yp->table('goods'). " SET keywords = '$keywords' WHERE goods_id = '$goods_id' LIMIT 1";
        ...
        foreach ($goods_attr_list as $attr_id => $attr_value_list)
        {
            foreach ($attr_value_list as $attr_value => $info)
            {
                if ($info['sign'] == 'insert')
                {
                    $sql = "INSERT INTO " .$yp->table('goods_attr'). " (attr_id, goods_id, attr_value, attr_price)".
                            "VALUES ('$attr_id', '$goods_id', '$attr_value', '$info[attr_price]')";
                }
                elseif ($info['sign'] == 'update')
                {
                    $sql = "UPDATE " .$yp->table('goods_attr'). " SET attr_price = '$info[attr_price]' WHERE goods_attr_id = '$info[goods_attr_id]' LIMIT 1";
                }
                else
                {
                	//删除商品属性
                    $sql = "DELETE FROM " .$yp->table('goods_attr'). " WHERE goods_attr_id = '$info[goods_attr_id]' LIMIT 1";
                    $db->query($sql);
                    //删除商品属性对应的货品信息
                    $sql = "DELETE FROM " .$yp->table('products'). " WHERE goods_id = '$goods_id' and (goods_attr = '".$info['goods_attr_id']."' or goods_attr like '%|".$info['goods_attr_id']."' or goods_attr like '".$info['goods_attr_id']."|%' or goods_attr like '%|".$info['goods_attr_id']."|%')";
                    $db->query($sql);
                    continue;
                }
                $db->query($sql);
   ...
   /*编辑商品分成金额成为分销商品*/
    if ($cost_price > 0){
        ...
        if ($cost_price <= $shop_price )     
				...
				$sql = "SELECT goods_id FROM " .$GLOBALS['yp']->table('ypmart_distrib_goods'). " WHERE goods_id = $goods_id";
			...
			$sql = "insert into ".$GLOBALS['yp']->table('ypmart_distrib_goods')." (`distrib_time`,`start_time`,`end_time`,`distrib_money`,`distrib_type`,`goods_id`) 
...

                {
                    ...
                    $sql = "UPDATE " .$yp->table('ypmart_distrib_goods'). " SET distrib_money = '$cost_price' WHERE goods_id = $goods_id";
    ...
    if ($goods_img !== false)
    {
        $db->query("UPDATE " . $yp->table('goods') . " SET goods_img = '$goods_img' WHERE goods_id='$goods_id'");
    }

    if ($original_img !== false)
    {
        $db->query("UPDATE " . $yp->table('goods') . " SET original_img = '$original_img' WHERE goods_id='$goods_id'");
    }

    if ($goods_thumb !== false)
    {
        $db->query("UPDATE " . $yp->table('goods') . " SET goods_thumb = '$goods_thumb' WHERE goods_id='$goods_id'");
    }
    ...
    if (isset($img))
    {
        ...
        $sql = "INSERT INTO " . $yp->table('goods_gallery') . " (goods_id, img_url, img_desc, thumb_url, img_original) " .
    ...
    if (!$is_insert && isset($_POST['old_img_desc']))
    {
        ...
        {
            $sql = "UPDATE " . $yp->table('goods_gallery') . " SET img_desc = '$img_desc' WHERE img_id = '$img_id' LIMIT 1";
            $db->query($sql);
        }
    }
    ...
    {
        $db->query("UPDATE " . $yp->table('goods') . " SET original_img='' WHERE `goods_id`='{$goods_id}'");
        $db->query("UPDATE " . $yp->table('goods_gallery') . " SET img_original='' WHERE `goods_id`='{$goods_id}'");
        @unlink('../' . $original_img);
        @unlink('../' . $img);
    }
    ...
    {
        $link[0] = array('href' => 'goods.php?act=product_list&goods_id=' . $goods_id, 'text' => '设置货品/规格价格');//20170907 mod q120029121
    }
    if ($code == 'virtual_card')
    {
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['type']))
    {
        ...
        if ($_POST['type'] == 'trash')
        {
            ...
            /* 代码增加 By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121 Start */
            $sql = 'SELECT bonus_type_id FROM ' . $GLOBALS['yp']->table('goods') . " WHERE goods_id in ($goods_id)";
            $bonus_type_id =  $GLOBALS['db']->getOne($sql);
            if(!empty($bonus_type_id))
            {
                $sales_pro_list = ' [商品红包] ';
            }
            $sql = 'SELECT is_exchange FROM ' . $GLOBALS['yp']->table('exchange_goods') . " WHERE goods_id in ($goods_id)";
            $is_exchange =  $GLOBALS['db']->getOne($sql);
            if(!empty($is_exchange))
            {
                $sales_pro_list .= ' [积分商城] ';
            }
            $sql = 'SELECT package_id FROM ' . $GLOBALS['yp']->table('package_goods') . " WHERE goods_id in ($goods_id)";
            $package_id =  $GLOBALS['db']->getOne($sql);
            if(!empty($package_id))
            {
                $sales_pro_list .= ' [超值礼包] ';
            }
            $sql = 'SELECT act_id FROM ' . $GLOBALS['yp']->table('goods_activity') . " WHERE goods_id in ($goods_id) AND act_type = 2";
            $goods_activity =  $GLOBALS['db']->getOne($sql);
            if(!empty($goods_activity))
            {
                $sales_pro_list .= ' [拍卖] ';
            }
            $sql = 'SELECT data FROM ' . $GLOBALS['yp']->table('topic');
        ...
        elseif ($_POST['type'] == 'suppliers_move_to')
        {
          ...
          /* 转移到入驻商 */ // 寒冰 20161206   qq  309485552
        elseif ($_POST['type'] == 'supplier_move_to')
        {
            /* 检查权限 */
            admin_priv('goods_manage');
            update_goods($goods_id, 'supplier_id', $_POST['supplier_id']);
			update_goods($goods_id, 'supplier_status', '2');//状态更改为  平台转移商品
			update_goods($goods_id, 'is_on_sale', '0');//下架商品
			$cat_id =  $GLOBALS['db'] -> getOne("SELECT cat_id FROM " . $GLOBALS['yp']->table('supplier_category') . "  WHERE supplier_id=".$_POST['supplier_id']." ORDER by `cat_id` ASC limit 1 ");//取出一个默认分类
			
			$goods_id = str_replace(',',' ',$goods_id,$i);
			$goods_id = explode(' ',$goods_id);
			
		foreach ($goods_id as $goods_id){
		     $old_supplier_id =  $GLOBALS['db'] -> getOne("SELECT supplier_id FROM " . $GLOBALS['yp']->table('goods') . "  WHERE goods_id = $goods_id");//是否已是商家商品
		   if ($old_supplier_id){
			$sql = "DELETE FROM " . $GLOBALS['yp']->table('supplier_goods_cat') . " WHERE goods_id =  $goods_id";
	        $GLOBALS['db']->query($sql);//删除原商家分类
		   }
		
			$sql = "INSERT INTO " . $yp->table('supplier_goods_cat') . " (goods_id, cat_id, supplier_id) " .
                "VALUES ($goods_id, '$cat_id', '$_POST[supplier_id]')";	
           $db->query($sql);//循环插入到商家默认分类
		  }
        }
        /* 还原 */
        ...
        {
            admin_priv('goods_manage');
			if(file_exists("prince_shops.php"))
			$url = 'prince_shops.php?act=img_local&goods_id=' . $goods_id;
			else
				$url = 'albbprince_shops.php?act=img_local&goods_id=' . $goods_id;
			//$aa = db_create_in($goods_id);
       		yp_header("Location: $url\n");
			exit;
         }
        ...
        elseif ($_POST['type'] == 'translate')
        {
			...
			$url = 'prince_shops.php?act=translate&goods_id=' . $goods_id;
				...
				$url = 'albbprince_shops.php?act=translate&goods_id=' . $goods_id;
       		yp_header("Location: $url\n");
			exit;
          }		
...
elseif ($_REQUEST['act'] == 'edit_goods_sn')
{
    ...
    $sql="SELECT goods_id FROM ". $yp->table('products')."WHERE product_sn='$goods_sn'";
    if($db->getOne($sql))
    {
        make_json_error($_LANG['goods_sn_exists']);
    }
...
elseif ($_REQUEST['act'] == 'check_products_goods_sn')
{
    ...
    else
    {
        foreach ($products_sn as $val)
        {
            ...
            $sql="SELECT goods_id FROM ". $yp->table('products')."WHERE product_sn='$val'";
            if($db->getOne($sql))
            {
                make_json_error($val.$_LANG['goods_sn_exists']);
            }
...
elseif ($_REQUEST['act'] == 'edit_goods_price')
{
	...
	$sql_zk = "SELECT promote_price FROM " . $yp->table('goods') . " WHERE goods_id = " . $goods_id;
...
elseif ($_REQUEST['act'] == 'edit_goods_number')
{
		...
		if($goods_num>0)
	{
		...
		$dispose = "SELECT email, tel, is_dispose FROM " .$yp->table('booking_goods'). " WHERE  goods_id = '$goods_id'";
        $email   = $db->getAll($dispose);
		$gname = "SELECT goods_name FROM " .$yp->table('goods'). " WHERE  goods_id = '$goods_id'";
		 ...
		 for($i=0;$i<count($email);$i++)
		{
			...
			{
				send_mail('', $email[$i]['email'], '您登记的商品'. $msg_goods_name .'已经到货', $email_content, 1);
				$sql = "UPDATE " . $yp->table('booking_goods') . " SET is_dispose = 1 WHERE  goods_id = '$goods_id'";
                $db->query($sql);
			}
		}
		...
		if($sms_on>0)
		{
			...
			for($i=0;$i<count($email);$i++)
			{
				if($email[$i]['is_dispose']==0)
				{
					...
					if($r==true)
					{
						$sql = "UPDATE " . $yp->table('booking_goods') . " SET is_dispose = 2 WHERE  goods_id = '$goods_id'";
						$db->query($sql);
					}	
					else
					{
						$sql = "UPDATE " . $yp->table('booking_goods') . " SET is_dispose = 3 WHERE  goods_id = '$goods_id'";
						$db->query($sql);
					}
				}			    					
			} 
...
elseif ($_REQUEST['act'] == 'toggle_on_sale')
{
	...
	/* 代码增加_start  By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121 */
	$sql="select supplier_id,supplier_status from ". $yp->table('goods') ." where goods_id='$goods_id' ";
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 代码增加 By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121 Start */
    $sql = 'SELECT bonus_type_id FROM ' . $GLOBALS['yp']->table('goods') . " WHERE goods_id = $goods_id";
    $bonus_type_id =  $GLOBALS['db']->getOne($sql);
    if(!empty($bonus_type_id))
    {
        $sales_pro_list = ' [商品红包] ';
    }
    $sql = 'SELECT is_exchange FROM ' . $GLOBALS['yp']->table('exchange_goods') . " WHERE goods_id = $goods_id";
    $is_exchange =  $GLOBALS['db']->getOne($sql);
    if(!empty($is_exchange))
    {
        $sales_pro_list .= ' [积分商城] ';
    }
    $sql = 'SELECT package_id FROM ' . $GLOBALS['yp']->table('package_goods') . " WHERE goods_id = $goods_id";
    $package_id =  $GLOBALS['db']->getOne($sql);
    if(!empty($package_id))
    {
        $sales_pro_list .= ' [超值礼包] ';
    }
    $sql = 'SELECT act_id FROM ' . $GLOBALS['yp']->table('goods_activity') . " WHERE goods_id = $goods_id AND act_type = 2";
    $goods_activity =  $GLOBALS['db']->getOne($sql);
    if(!empty($goods_activity))
    {
        $sales_pro_list .= ' [拍卖] ';
...
elseif ($_REQUEST['act'] == 'drop_goods')
{
    ...
    /* 取得商品信息 */
    $sql = "SELECT goods_id, goods_name, is_delete, is_real, goods_thumb, goods_desc, " . //代码修改  By  demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1 
	...
	$domain_qq_wx_120029121 = $yp->get_domain();
	$fullurl_qq_wx_120029121 = $yp->url();
	$mainurl_qq_wx_120029121 = str_replace($domain_qq_wx_120029121, "", $fullurl_qq_wx_120029121);
	$mainurl_qq_wx_120029121 = str_replace("/", "\/", $mainurl_qq_wx_120029121);
	//echo $mainurl_qq_wx_120029121;
	//echo "<pre>";
	//print_r($DesImgs[1] );
	//echo "</pre>";	
	foreach ($DesImgs[1] AS $img_del_qq_wx_120029121)
	{
		//echo $img_del_qq_wx_120029121."<br />";		
		//$final_img_del_qq_wx_120029121 = preg_replace("/". $mainurl_qq_wx_120029121 ."/i", "../", $img_del_qq_wx_120029121, 1);
		//echo $final_img_del_qq_wx_120029121."<br />";
		//删除商品描述的图片
		$stt = explode('../../',$img_del_qq_wx_120029121);
		foreach($stt as $str){
			}
		@unlink($str);

	//	@unlink($final_img_del_qq_wx_120029121);
	}
	//exit;
	/* 代码增加_end  By  demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1 */
    ...
    /* 删除商品的货品记录 */
    $sql = "DELETE FROM " . $yp->table('products') .
            " WHERE goods_id = '$goods_id'";
    $db->query($sql);
	
	 /* 删除条形码 */
    $sql = "DELETE FROM " . $GLOBALS['yp']->table('bar_code') .
            " WHERE goods_id = '$goods_id'" ;
    $db->query($sql);
    ...
    $sql = "DELETE FROM " . $yp->table('goods_gallery') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);

    /* 删除相关表记录 */
    $sql = "DELETE FROM " . $yp->table('collect_goods') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);
    $sql = "DELETE FROM " . $yp->table('goods_article') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);
    $sql = "DELETE FROM " . $yp->table('goods_attr') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);
    $sql = "DELETE FROM " . $yp->table('goods_cat') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);
    $sql = "DELETE FROM " . $yp->table('member_price') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);
    $sql = "DELETE FROM " . $yp->table('group_goods') . " WHERE parent_id = '$goods_id'";
    $db->query($sql);
    $sql = "DELETE FROM " . $yp->table('group_goods') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);
    $sql = "DELETE FROM " . $yp->table('link_goods') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);
    $sql = "DELETE FROM " . $yp->table('link_goods') . " WHERE link_goods_id = '$goods_id'";
    $db->query($sql);
    $sql = "DELETE FROM " . $yp->table('tag') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);
    $sql = "DELETE FROM " . $yp->table('comment') . " WHERE comment_type = 0 AND id_value = '$goods_id'";
    $db->query($sql);
    $sql = "DELETE FROM " . $yp->table('collect_goods') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);
    $sql = "DELETE FROM " . $yp->table('booking_goods') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);
    $sql = "DELETE FROM " . $yp->table('goods_activity') . " WHERE goods_id = '$goods_id'";
    $db->query($sql);

    /* 如果不是实体商品，删除相应虚拟商品记录 */
    if ($goods['is_real'] != 1)
    {
        $sql = "DELETE FROM " . $yp->table('virtual_card') . " WHERE goods_id = '$goods_id'";
...
elseif($_REQUEST['act'] == 'get_txm')
{
	...
	$sql = "SELECT attr_id FROM" . $yp->table('attribute') ."WHERE cat_id='$id' AND attr_txm=1";
	$con =	$db->getAll($sql);
	$array_txm = array();
	if(count($con)>0){
		foreach($con as $k => $v){
...
elseif ($_REQUEST['act'] == 'drop_image')
{
    ...
    /* 删除数据 */
    $sql = "DELETE FROM " . $GLOBALS['yp']->table('goods_gallery') . " WHERE img_id = '$img_id' LIMIT 1";
...
elseif ($_REQUEST['act'] == 'add_link_goods')
{
    ...
    foreach ($linked_array AS $val)
    {
        ...
        {
            /* 双向关联 */
            $sql = "INSERT INTO " . $yp->table('link_goods') . " (goods_id, link_goods_id, is_double, admin_id) " .
...
elseif ($_REQUEST['act'] == 'drop_link_goods')
{
    ...
    {
        $sql = "UPDATE " .$yp->table('link_goods') . " SET is_double = 0 ".
                " WHERE link_goods_id = '$goods_id' AND goods_id " . $drop_goods_ids;
    }
    if ($goods_id == 0)
    {
...
elseif ($_REQUEST['act'] == 'add_group_goods')
{
    ...
    {
        $sql = "INSERT INTO " . $yp->table('group_goods') . " (parent_id, goods_id, goods_price, admin_id) " .
...
elseif ($_REQUEST['act'] == 'get_article_list')
{
    ...
    $sql        = 'SELECT article_id, title FROM ' .$yp->table('article'). $where.
...
elseif ($_REQUEST['act'] == 'add_goods_article')
{
    ...
    {
        $sql = "INSERT INTO " . $yp->table('goods_article') . " (goods_id, article_id, admin_id) " .
...
elseif ($_REQUEST['act'] == 'drop_goods_article')
{
    ...
    $sql = "DELETE FROM " .$yp->table('goods_article') . " WHERE " . db_create_in($articles, "article_id") . " AND goods_id = '$goods_id'";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'product_list')
{
    ...
    /* 取出商品信息 */
    $sql = "SELECT goods_sn, goods_name, goods_type, shop_price FROM " . $yp->table('goods') . " WHERE goods_id = '$goods_id'";
    $goods = $db->getRow($sql);
    if (empty($goods))
    {
        $link[] = array('href' => 'goods.php?act=list', 'text' => $_LANG['01_goods_list']);
...
elseif ($_REQUEST['act'] == 'product_remove')
{
    ...
    /* 删除货品 */
    $sql = "DELETE FROM " . $yp->table('products') . " WHERE product_id = '$product_id'";
    $result = $db->query($sql);
    if ($result)
    {
        /* 修改商品库存 */
...
elseif ($_REQUEST['act'] == 'edit_product_sn')
{
    ...
    /* 修改 */
    $sql = "UPDATE " . $yp->table('products') . " SET product_sn = '$product_sn' WHERE product_id = '$product_id'";
    $result = $db->query($sql);
    if ($result)
    {
        clear_cache_files();
...
elseif ($_REQUEST['act'] == 'edit_pro_price')
{
    ...
    /* 修改货品价格 */
    $sql = "UPDATE " . $yp->table('products') . " SET pro_price = '$pro_price' WHERE product_id = '$product_id'";
    $result = $db->query($sql);
    if ($result)
    {
            clear_cache_files();
...
elseif ($_REQUEST['act'] == 'edit_product_number')
{
    ...
    /* 修改货品库存 */
    $sql = "UPDATE " . $yp->table('products') . " SET product_number = '$product_number' WHERE product_id = '$product_id'";
    $result = $db->query($sql);
    if ($result)
    {
        /* 修改商品库存 */
...
elseif ($_REQUEST['act'] == 'product_add_execute')
{
    ...
    $product['pro_price']      = $_POST['pro_price'];//今/天/优/品/多/商/户/系/统-采集 mod on 20160808 by prince  qq 1/2/0/0/2/9/1/2/1
    ...
    /* 取出商品信息 */
    $sql = "SELECT goods_sn, goods_name, goods_type, shop_price FROM " . $yp->table('goods') . " WHERE goods_id = '" . $product['goods_id'] . "'";
    $goods = $db->getRow($sql);
    if (empty($goods))
    {
        sys_msg($_LANG['sys']['wrong'] . $_LANG['cannot_found_goods'], 1, array(), false);
    ...
    foreach($product['product_sn'] as $key => $value)
    {
        ...
        /* 插入货品表 */
        $sql = "INSERT INTO " . $GLOBALS['yp']->table('products') . " (goods_id, goods_attr, product_sn, product_number, pro_price)  VALUES ('" . $product['goods_id'] . "', '$goods_attr', '$value', '" . $product['product_number'][$key] . "','". $product['pro_price'][$key] . "')";//今/天/优/品/多/商/户/系/统-采集 mod on 20160808 by prince  qq 1/2/0/0/2/9/1/2/1保存货品价格
    ...
    if ($insert)
    {
         ...
         $link[] = array('href' => 'goods.php?act=list', 'text' => $_LANG['01_goods_list']);
         $link[] = array('href' => 'goods.php?act=product_list&goods_id=' . $product['goods_id'],  'text' => '设置货品/规格价格');//20170907 mod q120029121
    }
    else
    {
...
elseif ($_REQUEST['act'] == 'batch_product')
{
    ...
    if ($_POST['type'] == 'drop')
    {
        ...
        $sum = 0;
        $goods_id = 0;
        $sql = "SELECT product_id, goods_id, product_number FROM  " . $GLOBALS['yp']->table('products') . " WHERE product_id $product_bound";
        ...
        if (!empty($product_array))
        {
            ...
            /* 删除货品 */
            $sql = "DELETE FROM " . $yp->table('products') . " WHERE product_id $product_bound";
            if ($db->query($sql))
            {
                //记录日志
...
elseif ($_REQUEST['act'] == 'search_suppliers')
{
    ...
    {
        $sql_supplier = "SELECT supplier_id, supplier_name FROM " . $GLOBALS['yp']
...
{
    $sql = "DELETE FROM " . $GLOBALS['yp']->table('volume_price') .
    ...
    foreach ($price_list AS $key => $price)
    {
        ...
        if (!empty($price))
        {
            $sql = "INSERT INTO " . $GLOBALS['yp']->table('volume_price') .
...
function sendsms_pricecut($goods_id)
{
	...
	if($min_price_arr['goods_name'] != '')
	{
		...
		$goods_url = $GLOBALS['yp']->url(). $goods_url;
		$sql="select pricecut_id, mobile,email from ". $GLOBALS['yp']->table('pricecut') ." where goods_id='$goods_id' and price >= '$min_price' and status=0 ";
		...
		if($list)
		{
			for($i = 0;$i < count($list); $i++)
			{
				...
				if($GLOBALS['_CFG']['sms_pricecut'] == 1)
				{
					...
					if($list[$i]['mobile'] != '')
					{
						$templateParam=Array("code"=>mb_substr($goods_name, 0,12,"UTF8").'...');
						$templateCode=$GLOBALS['_CFG']['sms_pricecut_dayu'];
						$res = sendSMS($list[$i]['mobile'],$pricecut_content,'', '',$templateParam , $templateCode);//已支持阿里大于 QQ 120029121
						if($res == true)
						{
							$sql = "UPDATE " . $GLOBALS['yp']->table('pricecut') . " SET status = 2 WHERE pricecut_id = '" . $list[$i]['pricecut_id'] . "'";
							$GLOBALS['db']->query($sql);
						}
						else
						{
							$sql = "UPDATE " . $GLOBALS['yp']->table('pricecut') . " SET status = 1 WHERE pricecut_id = '" . $list[$i]['pricecut_id'] . "'";
							$GLOBALS['db']->query($sql);
						}
					}
				}
...
function get_min_price ($goods_id)
{
     $sql="SELECT g.goods_name, g.goods_type, g.promote_price, g.promote_start_date, g.promote_end_date, ".
				" g.shop_price, a.attr_id,a.attr_name FROM ". $GLOBALS['yp']->table('goods') .
				" AS g left join ". $GLOBALS['yp']->table('attribute') ." AS a on g.cat_id=a.cat_id ".
				" WHERE g.goods_id=$goods_id";
	 ...
	 {      
			...
			$sql = "select min(attr_price) from ". $GLOBALS['yp']->table('goods_attr') ." where goods_id='$goods_id' and attr_id='$row[attr_id]' ";
...
function tongbu_cart_price($goods_id){
	...
	$sql = "select c.rec_id,c.goods_id,c.goods_attr_id,c.user_id,c.session_id,g.market_price from ".$yp->table('cart')." as c left join ".$yp->table('goods')." as g on c.goods_id=g.goods_id where c.goods_id=".$goods_id." and c.rec_type='".CART_GENERAL_GOODS."' AND c.extension_code <> 'package_buy'";
	$query = $db->query($sql);
...
//			$sql1 = "select u.user_rank,IFNULL(ur.discount,100) as discount from ".$yp->table('users')." as u left join ".$yp->table('user_rank')." as ur on u.user_rank=ur.rank_id where u.user_id=".$row['user_id'];
//
//			$data = $db->getRow($sql1);
            $user_info = $db->getRow('SELECT * FROM ' . $yp->table('users') . ' WHERE user_id = ' . $row['user_id']);
		...
		$db->query("update ".$yp->table('cart')." set market_price='".$row['market_price']."',goods_price='".$price."' where rec_id=".$row['rec_id']);
	}
}
...
function get_rank($user_rank, $rank_points)
{
    ...
    $row = $db->getRow(
        'SELECT discount, rank_id, special_rank FROM ' . $yp->table('user_rank') . " WHERE rank_id = '$user_rank'"
    );
    if (!empty($row))
    {
        if ($row['special_rank'])
        ...
        {
            return $db->getRow(
                'SELECT discount, rank_id FROM ' . $yp->table('user_rank') . " WHERE min_points > '$rank_points' ORDER BY min_points ASC LIMIT 1"
            );
        }
    }
    else
    {
        return $db->getRow(
            'SELECT discount, rank_id FROM ' . $yp->table('user_rank') . " WHERE '$rank_points' > min_points ORDER BY min_points DESC LIMIT 1"
        );
    }
}
/* 代码增加 By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121 End */
...
/**
 * 入驻商列表信息  寒冰  20161206   qq309485552
 *
 * @param       string      $conditions
 * @return      array
 */
function supplier_list_info($conditions = '')
{
    $where = '';
    if (!empty($conditions))
    {
