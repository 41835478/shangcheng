 ...
 * ============================================================================
 * $Author: Prince $
 * $Id: category.php 17063 2010-03-25 06:35:46Z Prince $
*/
...
$exc = new exchange($yp->table("category"), $db, 'cat_id', 'cat_name');
...
if($_REQUEST['cat_id']){
	$parent_id=$GLOBALS['db']->getOne("select parent_id from ". $GLOBALS['yp']->table('category') ." where cat_id='$_REQUEST[cat_id]' ");
...
if ($_REQUEST['act'] == 'drop_adimg_1')
{
    ...
    /* 取得 adimg 名称 */
    $sql = "SELECT cat_adimg_1 FROM " .$yp->table('category'). " WHERE cat_id = '$cat_id'";
    $adimg_name = $db->getOne($sql);

    if (!empty($adimg_name))
    {
        @unlink(ROOT_PATH .'/' .$adimg_name);
        $sql = "UPDATE " .$yp->table('category'). " SET cat_adimg_1 = '' WHERE cat_id = '$cat_id'";
        $db->query($sql);
    }
...
elseif ($_REQUEST['act'] == 'drop_adimg_2')
{
    ...
    /* 取得 adimg 名称 */
    $sql = "SELECT cat_adimg_2 FROM " .$yp->table('category'). " WHERE cat_id = '$cat_id'";
    $adimg_name = $db->getOne($sql);

    if (!empty($adimg_name))
    {
        @unlink(ROOT_PATH . DATA_DIR . '/'. $catimg_dir .'/' .$adimg_name);
        $sql = "UPDATE " .$yp->table('category'). " SET cat_adimg_2 = '' WHERE cat_id = '$cat_id'";
        $db->query($sql);
    }
    $link= array(array('text' => '返回商品分类编辑页', 'href' => 'category.php?act=edit&cat_id=' . $cat_id), array('text' => '返回商品分类列表页', 'href' => 'category.php?act=list'));
...
if ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('cat_info',     array('is_show' => 1,'is_xcx_show' => 1));//20170911 qq120029121 prince 
...
if ($_REQUEST['act'] == 'insert')
{
 ...
 $cat['is_xcx_show']  = !empty($_POST['show'])      ? intval($_POST['show'])    : 0;
 ...
 /* 代码增加_start Byjdy */
	...
	$cat['cat_adimg_1']        =   $image->upload_image($_FILES['cat_adimg_1']);
	$cat['cat_adimg_1']		=	 $cat['cat_adimg_1']  ?  $cat['cat_adimg_1']  :  '';
  ...
  /*  代码增加_start By demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1 */
   $cat['brand_qq']  = !empty($_POST['brand_wwwwx_120029121com']) ? $_POST['brand_wwwwx_120029121com'] : '';
   ...
   $cat['xcx_cat_name']     = !empty($_POST['xcx_cat_name'])     ? trim($_POST['xcx_cat_name'])     : '';//新增分类小程序缩略名 prince
    ...
    if (isset($_FILES['type_img']) && $_FILES['type_img']['tmp_name'] != '' && isset($_FILES['type_img']['tmp_name']) &&$_FILES['type_img']['tmp_name'] != 'none'){
	...
	if($cat['path_name'] != '')
	{
		$is_have = $db->getOne("select cat_id  from ". $yp->table('category') ." where path_name='$cat[path_name]' "); 
		if ($is_have)
		{
 ...
 foreach($arrCatName as $arrCatNameValue)
 {
  ...
  /* 入库的操作 */
  if ($db->autoExecute($yp->table('category'), $cat) !== false)
  {
   $cat_id = $db->insert_id();
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    //分类是否存在首页推荐
    $res = $db->getAll("SELECT recommend_type FROM " . $yp->table("cat_recommend") . " WHERE cat_id=" . $cat_id);
    if (!empty($res))
    {
        $cat_recommend = array();
        foreach($res as $data)
...
elseif($_REQUEST['act'] == 'add_category')
{
    ...
    {
        $sql = "INSERT INTO " . $yp->table('category') . "(cat_name, parent_id, is_show,is_xcx_show)" .
               "VALUES ( '$category', '$parent_id', 1,1)";
...
if ($_REQUEST['act'] == 'update')
{
    ...
    $cat['xcx_cat_name'] = !empty($_POST['xcx_cat_name'])     ? trim($_POST['xcx_cat_name'])     : '';//新增分类小程序缩略名 prince
    $cat['is_show']      = !empty($_POST['is_show'])      ? intval($_POST['is_show'])    : 0;
    $cat['is_xcx_show']  = !empty($_POST['is_xcx_show'])      ? intval($_POST['is_xcx_show'])    : 0;
    $cat['show_in_nav']  = !empty($_POST['show_in_nav'])  ? intval($_POST['show_in_nav']): 0;
    $cat['style']        = !empty($_POST['style'])        ? trim($_POST['style'])        : '';
    $cat['grade']        = !empty($_POST['grade'])        ? intval($_POST['grade'])      : 0;
    $cat['filter_attr']  = !empty($_POST['filter_attr'])  ? implode(',', array_unique(array_diff($_POST['filter_attr'],array(0)))) : 0;
    $cat['cat_recommend']  = !empty($_POST['cat_recommend'])  ? $_POST['cat_recommend'] : array();
	 /*  代码增加_srat By demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1 */
	$cat['brand_qq']  = !empty($_POST['brand_wwwwx_120029121com']) ? $_POST['brand_wwwwx_120029121com'] : '';
	$cat['attr_qq120029121']  = !empty($_POST['attr_qq']) ? $_POST['attr_qq'] : '';
	 /*  代码增加_end By demo.coolhong.com 今天优品多商户系统 Q Q 1200 2912 1 */
	/* 代码增加_start Byjdy */
	...
	if($cat['path_name'] != '')
	{
		$is_have = $db->getOne("select count(*)  from ". $yp->table('category') ." where cat_id !='$cat_id' and path_name='$cat[path_name]' "); 
		if ($is_have)
		{
		   $link[] = array('text' => $_LANG['go_back'], 'href' => 'javascript:history.back(-1)');
		   sys_msg('对不起，已经存在同名目录', 0, $link);
	...
	$cat_imgs=$db->getOne("select cat_nameimg, cat_index_rightad, cat_adimg_1, cat_adimg_2  from ".$yp->table("category")." where cat_id='$cat_id'");
	...
	if ($_FILES['cat_adimg_1']['tmp_name'] != '' && $_FILES['cat_adimg_1']['tmp_name'] != 'none')
	{
		...
		if (!empty($cat_imgs['cat_adimg_1'])){
			...
			@unlink(ROOT_PATH .'/' .$cat_imgs['cat_adimg_1']);
		}
	}
    ...
    $dat = $db->getRow("SELECT cat_name, show_in_nav FROM ". $yp->table('category') . " WHERE cat_id = '$cat_id'");

    if ($db->autoExecute($yp->table('category'), $cat, 'UPDATE', "cat_id='$cat_id'"))
    {
        if($cat['cat_name'] != $dat['cat_name'])
        {
            //如果分类名称发生了改变
            $sql = "UPDATE " . $yp->table('nav') . " SET name = '" . $cat['cat_name'] . "' WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'";
            $db->query($sql);
        }
        if($cat['show_in_nav'] != $dat['show_in_nav'])
        {
            ...
            {
                //显示
                $nid = $db->getOne("SELECT id FROM ". $yp->table('nav') . " WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'");
                if(empty($nid))
                {
                    //不存在
                    $vieworder = $db->getOne("SELECT max(vieworder) FROM ". $yp->table('nav') . " WHERE type = 'middle'");
                    ...
                    $sql = "INSERT INTO " . $yp->table('nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,type) VALUES('" . $cat['cat_name'] . "', 'c', '$cat_id','1','$vieworder','0', '" . $uri . "','middle')";
                }
                else
                {
                    $sql = "UPDATE " . $yp->table('nav') . " SET ifshow = 1 WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'";
                }
                $db->query($sql);
            }
            else
            {
                //去除
                $db->query("UPDATE " . $yp->table('nav') . " SET ifshow = 0 WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'");
            }
        }
...
if ($_REQUEST['act'] == 'toggle_show_in_nav')
{
    ...
    if (cat_update($id, array('show_in_nav' => $val)) != false)
    {
        if($val == 1)
        {
            ...
            $vieworder = $db->getOne("SELECT max(vieworder) FROM ". $yp->table('nav') . " WHERE type = 'middle'");
            $vieworder += 2;
            $catname = $db->getOne("SELECT cat_name FROM ". $yp->table('category') . " WHERE cat_id = '$id'");
            //显示在自定义导航栏中
            $_CFG['rewrite'] = 0;
            $uri = build_uri('category', array('cid'=> $id), $catname);

            $nid = $db->getOne("SELECT id FROM ". $yp->table('nav') . " WHERE ctype = 'c' AND cid = '" . $id . "' AND type = 'middle'");
            if(empty($nid))
            {
                //不存在
                $sql = "INSERT INTO " . $yp->table('nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,type) VALUES('" . $catname . "', 'c', '$id','1','$vieworder','0', '" . $uri . "','middle')";
            }
            else
            {
                $sql = "UPDATE " . $yp->table('nav') . " SET ifshow = 1 WHERE ctype = 'c' AND cid = '" . $id . "' AND type = 'middle'";
            }
            $db->query($sql);
        }
        else
        {
            //去除
            $db->query("UPDATE " . $yp->table('nav') . "SET ifshow = 0 WHERE ctype = 'c' AND cid = '" . $id . "' AND type = 'middle'");
        }
        clear_cache_files();
        $redis->delete('cat_list');
        make_json_result($val);
...
if ($_REQUEST['act'] == 'toggle_is_xcx_show')
{
    check_authz_json('cat_manage');
    ...
    if (cat_update($id, array('is_xcx_show' => $val)) != false)
    {
        clear_cache_files();
        $redis->delete('cat_list');
        make_json_result($val);
...
if ($_REQUEST['act'] == 'remove')
{
    ...
    $sql = "select is_virtual from ".$yp->table('category')." where cat_id = $cat_id";
    $is_virtual = $db -> getOne($sql);
    
    $cat_name = $db->getOne('SELECT cat_name FROM ' .$yp->table('category'). " WHERE cat_id='$cat_id'");

    /* 当前分类下是否有子分类 */
    $cat_count = $db->getOne('SELECT COUNT(*) FROM ' .$yp->table('category'). " WHERE parent_id='$cat_id'");

    /* 当前分类下是否存在商品 */
    $goods_count = $db->getOne('SELECT COUNT(*) FROM ' .$yp->table('goods'). " WHERE cat_id='$cat_id'");

    /* 如果不存在下级子分类和商品，则删除之 */
    if ($cat_count == 0 && $goods_count == 0)
    {
        /* 删除分类 */
        $sql = 'DELETE FROM ' .$yp->table('category'). " WHERE cat_id = '$cat_id'";
        if ($db->query($sql))
        {
            $db->query("DELETE FROM " . $yp->table('nav') . "WHERE ctype = 'c' AND cid = '" . $cat_id . "' AND type = 'middle'");
            clear_cache_files();
            $redis->delete('cat_list');
...
if ($_REQUEST['act'] == 'drop_thumb')
{
    ...
    /* 取得logo名称 */
    $sql = "SELECT type_img FROM " .$yp->table('category'). " WHERE cat_id = '$cat_id'";
    $thumb_name = $db->getOne($sql);
    if (!empty($thumb_name))
    {
        @unlink(ROOT_PATH . DATA_DIR . '/catthumb/' .$thumb_name);
        $sql = "UPDATE " .$yp->table('category'). " SET type_img = '' WHERE cat_id = '$cat_id'";
        $db->query($sql);
    }
...
{
    $sql = "SELECT * FROM " .$GLOBALS['yp']->table('category'). " WHERE cat_id='$cat_id' LIMIT 1";
...
function cat_update($cat_id, $args)
{
    ...
    return $GLOBALS['db']->autoExecute($GLOBALS['yp']->table('category'), $args, 'update', "cat_id='$cat_id'");
}
...
function insert_cat_recommend($recommend_type, $cat_id)
{
    ...
    {
        //取得之前的分类
        $recommend_res = $GLOBALS['db']->getAll("SELECT recommend_type FROM " . $GLOBALS['yp']->table("cat_recommend") . " WHERE cat_id=" . $cat_id);
        if (empty($recommend_res))
        {
            foreach($recommend_type as $data)
            {
                $data = intval($data);
                $GLOBALS['db']->query("INSERT INTO " . $GLOBALS['yp']->table("cat_recommend") . "(cat_id, recommend_type) VALUES ('$cat_id', '$data')");
            }
        }
        else
        {
            ...
            {
                $GLOBALS['db']->query("DELETE FROM " . $GLOBALS['yp']->table("cat_recommend") . " WHERE cat_id=$cat_id AND recommend_type " . db_create_in($delete_array));
            }
    ...
    {
        $GLOBALS['db']->query("DELETE FROM ". $GLOBALS['yp']->table("cat_recommend") . " WHERE cat_id=" . $cat_id);
    }
}
...
/* 代码修改 By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121 End */
	...
	// 根据类别名称进行模糊查询
	$sql = "SELECT c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.is_xcx_show, c.show_in_nav, c.grade, c.sort_order, COUNT(s.cat_id) AS has_children, 1 AS result ".
...
function get_virtual_cat_select(){
    $sql = "select cat_id,cat_name from ".$GLOBALS['yp']->table('category')." where is_virtual = 1 and parent_id = 0";
    $res = $GLOBALS['db']->getAll($sql);
    $cat_select = '';
    foreach($res as $k=>$v){
