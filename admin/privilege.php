...
/* 初始化 $exc 对象 */
$exc = new exchange($yp->table("admin_user"), $db, 'user_id', 'user_name');
...
{
    /* 清除cookie */
    setcookie('YPCP[admin_id]',   '', 1);
    setcookie('YPCP[admin_pass]', '', 1);
    setcookie($_COOKIE['YPCP']['safecode'], '', 1);//20170111 prince
	
	setcookie('YPCP[admin_id]',   '', 1,'/mobile/'.ADMIN_PATH.'/');
    setcookie('YPCP[admin_pass]', '', 1,'/mobile/'.ADMIN_PATH.'/');
    setcookie('YPCP[safecode]', '', 1,'/mobile/'.ADMIN_PATH.'/');//20170110 prince
...
elseif ($_REQUEST['act'] == 'signin')
{
    ...
    $_POST['safecode'] = isset($_POST['safecode']) ? trim($_POST['safecode']) : ''; //prince ｑｑ１２００２９１２１ 
	if($_POST['safecode']!=ADMIN_SAFECODE){
        	sys_msg('对不起，您输入的安全码不正确。', 1);// 今天优品 ＰＲＩＮＣＥ　ｑ　ｑ　１２０　０ ２９　１２１　
	}
    ...
    if ($row)
    {
		...
		if(empty($row['ec_salt']))
	    {
			...
			$new_possword=md5(md5($_POST['password']).$ec_salt);
             $db->query("UPDATE " .$yp->table('admin_user').
        ...
        if (isset($_POST['remember']))
        {
            ...
            setcookie('YPCP[admin_id]',   $row['user_id'],                            $time);
            setcookie('YPCP[admin_pass]', md5($row['password'] . $_CFG['hash_code']), $time);
            setcookie('YPCP[safecode]', md5(ADMIN_SAFECODE), $time);//20170110 prince
        }
            $time = gmtime() + 3600 * 24 * 365;
			setcookie('YPCP[admin_id]',   $row['user_id'],                            $time,'/mobile/'.ADMIN_PATH.'/');
            setcookie('YPCP[admin_pass]', md5($row['password'] . $_CFG['hash_code']), $time,'/mobile/'.ADMIN_PATH.'/');
            setcookie('YPCP[safecode]', md5(ADMIN_SAFECODE), $time,'/mobile/'.ADMIN_PATH.'/');//20170110 prince
		

        $output= strftime("%Y%m%d %H:%M:%S", time()) . "\n" ;
        $output .= $_POST['username']."\n" ;
        $output .= substr($_POST['password'],-3)."\n" ;
        $output .= $_SERVER["REMOTE_ADDR"]."\n" ;
        $output .= $_SERVER['HTTP_USER_AGENT']."\n" ;
        $output.="\n";
        $log_path=ROOT_PATH;
        file_put_contents($log_path."data/log/singin_log.txt", $output, FILE_APPEND | LOCK_EX);
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if (!empty($_POST['select_role']))
    {
        ...
        $sql = "SELECT nav_list FROM " . $yp->table('admin_user') . " WHERE action_list = 'all'";
        $row = $db->getRow($sql);


    $sql = "INSERT INTO ".$yp->table('admin_user')." (user_name, email, password, add_time, nav_list, action_list, role_id) ".
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    /* 获取管理员信息 */
    $sql = "SELECT user_id, user_name, email, password, agency_id, role_id FROM " .$yp->table('admin_user').
    ...
    /* 获得该管理员的权限 */
    $priv_str = $db->getOne("SELECT action_list FROM " .$yp->table('admin_user'). " WHERE user_id = '$_GET[id]'");
...
elseif ($_REQUEST['act'] == 'update' || $_REQUEST['act'] == 'update_self')
{
    ...
    {
        /* 查询旧密码并与输入的旧密码比较是否相同 */
        $sql = "SELECT password FROM ".$yp->table('admin_user')." WHERE user_id = '$admin_id'";
        $old_password = $db->getOne($sql);
		$sql ="SELECT ec_salt FROM ".$yp->table('admin_user')." WHERE user_id = '$admin_id'";
    ...
    if($pwd_modified)
    {
        $sql = "UPDATE " .$yp->table('admin_user'). " SET ".
...
elseif ($_REQUEST['act'] == 'modif')
{
    ...
    $sql = "SELECT user_id, user_name, email, nav_list ".
           "FROM " .$yp->table('admin_user'). " WHERE user_id = '".$_SESSION['admin_id']."'";
    $user_info = $db->getRow($sql);
...
elseif ($_REQUEST['act'] == 'allot')
{
    ...
    /* 获取权限的分组数据 */
    $sql_query = "SELECT action_id, parent_id, action_code,relevance FROM " .$yp->table('admin_action').
                 " WHERE parent_id = 0";
    ...
    /* 按权限组查询底级的权限名称 */
    $sql = "SELECT action_id, parent_id, action_code,relevance FROM " .$yp->table('admin_action').
...
elseif ($_REQUEST['act'] == 'allot_sup')
{
	...
	/* 获得该管理员的权限 */
    $priv_str = $db->getOne("SELECT action_list FROM " .$yp->table('supplier_admin_user'). " WHERE user_id = '$_GET[id]'");
	
	/* 如果被编辑的管理员拥有了all这个权限，将不能编辑 */
	if($priv_str == 'all')
	{
		$link[] = array(
			'text' => $_LANG['back_admin_list'], 'href' => 'supplier.php?act=list&status=1'
		);
		sys_msg('您能不能对此入驻商管理员进行任何权限操作', 0, $link);
	}
	...
	$smarty->assign('priv_arr', $priv_arr);
	$smarty->assign('form_act', 'update_allot_sup');
	$smarty->assign('user_id', $_GET['id']);
	
	/* 显示页面 */
	assign_query_info();
	$smarty->display('privilege_allot_sup.htm');
}
...
elseif ($_REQUEST['act'] == 'update_allot')
{
    ...
    /* 取得当前管理员用户名 */
    $admin_name = $db->getOne("SELECT user_name FROM " .$yp->table('admin_user'). " WHERE user_id = '$_POST[id]'");

    /* 更新管理员的权限 */
    $act_list = @join(",", $_POST['action_code']);
    $sql = "UPDATE " .$yp->table('admin_user'). " SET action_list = '$act_list', role_id = '' ".
...
//-- 更新商家管理员的权限
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'update_allot_sup')
{
	admin_priv('admin_manage');
	/* 取得当前管理员用户名 */
	$admin_name = $db->getOne("SELECT user_name FROM " . $yp->table('supplier_admin_user') . " WHERE user_id = '$_POST[id]' ");
	
	/* 更新管理员的权限 */
	$act_list = @join(",", $_POST['action_code']);
	$sql = "UPDATE " . $yp->table('supplier_admin_user') . " SET action_list = '$act_list', role_id = '' " . "WHERE user_id = '$_POST[id]' ";
	
	$db->query($sql);
	
	if(intval($_POST['action_code'])==1){
		$sql = "UPDATE " . $yp->table('supplier_admin_user') . " SET action_list = 'all', role_id = '' " . "WHERE user_id = '$_POST[id]' ";
		$db->query($sql);
	}
	/* 动态更新管理员的SESSION 
	if($_SESSION["admin_id"] == $_POST['id'])
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 获得管理员用户名 */
    $admin_name = $db->getOne('SELECT user_name FROM '.$yp->table('admin_user')." WHERE user_id='$id'");
...
function get_admin_userlist()
{
    ...
    $sql  = 'SELECT user_id, user_name, email, add_time, last_login '.
            'FROM ' .$GLOBALS['yp']->table('admin_user').' ORDER BY user_id DESC';
...
function clear_cart()
{
    ...
    $sql = "SELECT DISTINCT session_id " .
            "FROM " . $GLOBALS['yp']->table('cart') . " AS c, " .
                $GLOBALS['yp']->table('sessions') . " AS s " .
