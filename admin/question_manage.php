...
/**
 * QQ120029121 用户咨询管理程序
 * ============================================================================
 * 演示地址: http://demo.coolhong.com  开发QQ:120029121    309485552
 * ============================================================================
 * $Author: Prince $
 * $Id: comment_manage.php 17123 2010-04-22 07:28:54Z Prince $
*/
...
if ($_REQUEST['act']=='reply')
{
    ...
    /* 获取评论详细信息并进行字符处理 */
    $sql = "SELECT * FROM " .$yp->table('question'). " WHERE question_id = '$_REQUEST[id]'";
    ...
    /* 获得评论回复内容 */
    $sql = "SELECT * FROM ".$yp->table('question'). " WHERE parent_id = '$_REQUEST[id]'";
...
if ($_REQUEST['act']=='action')
{
    ...
    /* 获得咨询是否有回复 */
    $sql = "SELECT question_id, content, parent_id FROM ".$yp->table('question').
    ...
    {
        /* 更新回复的内容 */
        $sql = "UPDATE ".$yp->table('question')." SET ".
               "email     = '$_POST[email]', ".
    ...
    {
        /* 插入回复的评论内容 */
        $sql = "INSERT INTO ".$yp->table('question')." (id_value, email, user_name , ".
    ...
    $sql = "UPDATE " .$yp->table('question'). " SET status = 1 WHERE question_id = '$_POST[question_id]'";
    $db->query($sql);
    ...
    yp_header("Location: question_manage.php?act=reply&id=$_REQUEST[question_id]&send_ok=$send_ok\n");
    exit;
}
/*------------------------------------------------------ */
//-- 更新咨询的状态为显示或者禁止
...
if ($_REQUEST['act'] == 'check')
{
    ...
    {
        /* 允许咨询显示 */
        $sql = "UPDATE " .$yp->table('question'). " SET status = 1 WHERE question_id = '$_REQUEST[id]'";
        $db->query($sql);

        /* 清除缓存 */
        clear_cache_files();

        yp_header("Location: question_manage.php?act=reply&id=$_REQUEST[id]\n");
        exit;
    }
    else
    {
        /* 禁止咨询显示 */
        $sql = "UPDATE " .$yp->table('question'). " SET status = 0 WHERE question_id = '$_REQUEST[id]'";
        $db->query($sql);
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $sql = "DELETE FROM " .$yp->table('question'). " WHERE question_id = '$id'";
    $res = $db->query($sql);
    if ($res)
    {
        $db->query("DELETE FROM " .$yp->table('question'). " WHERE parent_id = '$id'");
    }
...
if ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['checkboxes']))
    {
        ...
        {
            case 'remove':
                $db->query("DELETE FROM " . $yp->table('question') . " WHERE " . db_create_in($_POST['checkboxes'], 'question_id'));
                $db->query("DELETE FROM " . $yp->table('question') . " WHERE " . db_create_in($_POST['checkboxes'], 'parent_id'));
                break;

           case 'allow' :
               $db->query("UPDATE " . $yp->table('question') . " SET status = 1  WHERE " . db_create_in($_POST['checkboxes'], 'question_id'));
               break;

           case 'deny' :
               $db->query("UPDATE " . $yp->table('question') . " SET status = 0  WHERE " . db_create_in($_POST['checkboxes'], 'question_id'));
               break;
...
function get_question_list()
{
    ...
    $sql = "SELECT count(*) FROM " .$GLOBALS['yp']->table('question'). " WHERE parent_id = 0 $where";
    ...
    /* 获取评论数据 */
    $arr = array();
    $sql  = "SELECT * FROM " .$GLOBALS['yp']->table('question'). " WHERE parent_id = 0 $where " .
    ...
    {
        $sql = "SELECT goods_name FROM " .$GLOBALS['yp']->table('goods'). " WHERE goods_id='$row[id_value]' " ;
        $row['title'] = $GLOBALS['db']->getOne($sql);

        /* 标记是否回复过 */
		$sql = "SELECT COUNT(*) FROM " .$GLOBALS['yp']->table('question'). " WHERE parent_id = '$row[question_id]'";
