...
$exc = new exchange($yp->table("shop_grade"), $db, 'grade_id', 'is_comment');
/* act操作项的初始化 */
...
function get_order_comment_list()
{
    ...
    $sql = "SELECT count(*) FROM " .$GLOBALS['yp']->table('shop_grade'). " WHERE user_id > 0 $where";
    ...
    /* 获取评论数据 */
    $arr = array();
    $sql  = "SELECT * FROM " .$GLOBALS['yp']->table('shop_grade'). " WHERE user_id > 0  AND send >0$where " .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$sql = "SELECT AVG(comment_rank) FROM " . $GLOBALS['yp']->table('comment') . " c" . " LEFT JOIN " . $GLOBALS['yp']->table('order_goods') . " o"." ON o.goods_id = c.id_value"." WHERE  c.order_id = '$o_id' AND c.order_id = o.order_id";
