...
/**
 * QQ120029121 管理中心帐户变动记录
 * ============================================================================
 * 演示地址: http://demo.coolhong.com  开发QQ:120029121    309485552
 * ============================================================================
 * $Author: PRINCE $
 * $Id: account_log.php 17217 2017-04-01 06:29:08Z PRINCE $
 */
...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('action_link2',  array('text' => '会员列表', 'href' => 'users.php?act=list'));
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    if (empty($user))
    {
     ...
     $sql = "SELECT supplier_id FROM " .$yp->table('users'). " WHERE user_id = '$user_id'";
...
function get_accountlist($user_id, $account_type = '')
{
    ...
    /* 查询记录总数，计算分页数 */
    $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('account_log') . $where;
    $filter['record_count'] = $GLOBALS['db']->getOne($sql);
    $filter = page_and_size($filter);

    /* 查询记录 */
    $sql = "SELECT * FROM " . $GLOBALS['yp']->table('account_log') . $where .
