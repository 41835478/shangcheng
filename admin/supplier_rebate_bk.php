...
/**
 * QQ120029121 管理中心 返佣管理
 * ============================================================================
 * 演示地址: http://demo.coolhong.com；
 * ============================================================================
 * $Author: jtypmall120 $
 * $Id: suppliers.php 15013 2009-05-13 09:31:42Z jtypmall120 $
 */
...
elseif ($_REQUEST['act']== 'view')
{
    ...
    admin_priv('supplier_manage');
     ...
     $sql = "SELECT r.*, s.supplier_name, s.bank, s.supplier_rebate FROM " . $yp->table('supplier_rebate') . " AS r left join ". $yp->table('supplier'). 
	 ...
	 else
	{
		...
		$rebate['all_money'] = $GLOBALS['db']->getOne("select sum(money_paid) from ". $GLOBALS['yp']->table('order_info') ." where rebate_id=". $rebate['rebate_id'] ." and rebate_ispay=2");
		...
		$sql = "select s.*, r.rank_name, u.user_name from ".$yp->table('supplier')." AS s left join ".$yp->table('supplier_rank').
					" AS r on s.rank_id=r.rank_id left join ". $yp->table('users') ." AS u on s.user_id=u.user_id  where s.supplier_id='$rebate[supplier_id]' ";
		$supplier =$db->getRow($sql);
		if (!empty($supplier))
		{
			$supplier['province'] = $db->getOne("select region_name from ". $yp->table('region') ." where region_id='$supplier[province]' ");
			$supplier['city'] = $db->getOne("select region_name from ". $yp->table('region') ." where region_id='$supplier[city]' ");
			$supplier['district'] = $db->getOne("select region_name from ". $yp->table('region') ." where region_id='$supplier[district]' ");
		}
...
elseif ($_REQUEST['act']=='update')
{
  ...
  /* 取得供货商信息 */
  $sql = "SELECT * FROM " . $yp->table('supplier_rebate') . " WHERE rebate_id = '" . $rebate_id ."' ";
...
/* 保存返佣信息 */
$db->autoExecute($yp->table('supplier_rebate'), $rebate, 'UPDATE', "rebate_id = '" . $rebate_id . "'");
...
function rebate_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('supplier_rebate') ." AS sr  " . $where;
        $filter['record_count']   = $GLOBALS['db']->getOne($sql);
        $filter['page_count']     = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;

        /* 查询 */
        $sql = "SELECT sr.* , s.supplier_name, s.supplier_rebate ".
                "FROM " . $GLOBALS['yp']->table("supplier_rebate") . " AS  sr left join " .$GLOBALS['yp']->table("supplier") .  " AS s on sr.supplier_id=s.supplier_id 
                $where
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
	{
		...
		$row['all_money'] = $GLOBALS['db']->getOne("select sum(money_paid + surplus) from ". $GLOBALS['yp']->table('order_info') ." where rebate_id=". $row['rebate_id'] ." and rebate_ispay=2");
