...
/**
 * QQ120029121 配送区域管理程序
 * ============================================================================
 * 演示地址: http://demo.coolhong.com  开发QQ:120029121    309485552
 * ============================================================================
 * $Author: PRINCE $
 * $Id: shipping_area.php 17217 2017-04-01 06:29:08Z PRINCE $
*/
...
$exc = new exchange($yp->table('shipping_area'), $db, 'shipping_area_id', 'shipping_area_name');
...
elseif ($_REQUEST['act'] == 'add' && !empty($_REQUEST['shipping']))
{
    ...
    $shipping = $db->getRow("SELECT shipping_name, shipping_code FROM " .$yp->table('shipping'). " WHERE shipping_id='$_REQUEST[shipping]'");
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "SELECT COUNT(*) FROM " .$yp->table("shipping_area").
    ...
    {
        $shipping_code = $db->getOne("SELECT shipping_code FROM " .$yp->table('shipping').
        ...
        if (isset($_POST['regions']) && is_array($_POST['regions']))
        {
            ...
            {
                $sql = "INSERT INTO ".$yp->table('area_region')." (shipping_area_id, region_id) VALUES ('$new_id', '$val')";
                $db->query($sql);
            }
        }
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT a.shipping_name, a.shipping_code, a.support_cod, b.* ".
            "FROM " .$yp->table('shipping'). " AS a, " .$yp->table('shipping_area'). " AS b ".
...
//       }
       ...
       {
           $fields[$key]['value']= $val['value']?$val['value']:0;
       }
    ...
    $sql = "SELECT a.region_id, r.region_name ".
            "FROM ".$yp->table('area_region')." AS a, ".$yp->table('region'). " AS r ".
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    {
        $shipping_code = $db->getOne("SELECT shipping_code FROM " .$yp->table('shipping'). " WHERE shipping_id='$_POST[shipping]'");
        $plugin        = '../includes/modules/shipping/'. $shipping_code. ".php";
        ...
        $sql = "UPDATE " .$yp->table('shipping_area').
        ...
        $sql = "SELECT region_id, parent_id FROM " . $yp->table('region');
        $res = $db->query($sql);
        ...
        /* 清除原有的城市和地区 */
        $db->query("DELETE FROM ".$yp->table("area_region")." WHERE shipping_area_id='$_POST[id]'");

        /* 添加选定的城市和地区 */
        foreach ($selected_regions AS $key => $val)
        {
            $sql = "INSERT INTO ".$yp->table('area_region')." (shipping_area_id, region_id) VALUES ('$_POST[id]', '$val')";
            $db->query($sql);
        }
		if(!in_array($shipping_code,$not_set_default)){
			set_default_show($_POST['shipping'],0);//重新同步所有的非默认配送地址信息
...
elseif ($_REQUEST['act'] == 'multi_remove')
{
    ...
    if (isset($_POST['areas']) && count($_POST['areas']) > 0)
    {
        ...
        {
            $db->query("DELETE FROM " .$yp->table('shipping_area'). " WHERE shipping_area_id='$v'");
            $i++;
        }
...
elseif ($_REQUEST['act'] == 'remove_area')
{
    ...
    $exc->drop($id);
    $db->query('DELETE FROM '.$yp->table('area_region').' WHERE shipping_area_id='.$id);
...
{
    $sql = "SELECT * FROM " . $GLOBALS['yp']->table('shipping_area');
    if ($shipping_id > 0)
    {
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        $sql = "SELECT r.region_name " .
                "FROM " . $GLOBALS['yp']->table('area_region'). " AS a, " .
                    $GLOBALS['yp']->table('region') . " AS r ".
...
function is_set_area($shipping_id){
	...
	return $db->getOne("select is_default_show from ".$yp->table('shipping')." where shipping_id=".$shipping_id);
}
