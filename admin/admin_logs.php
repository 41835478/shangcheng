...
/**
 * QQ120029121 记录管理员操作日志
 * ============================================================================
 * 演示地址: http://demo.coolhong.com  开发QQ:120029121    309485552
 * ============================================================================
 * $Author: PRINCE $
 * $Id: admin_logs.php 17217 2017-04-01 06:29:08Z PRINCE $
*/
...
if ($_REQUEST['act'] == 'list')
{
	...
	/* 查询管理员列表 */
    $admin_list = array();
	$res = $db->query("SELECT DISTINCT au.user_id, au.user_name FROM ".$yp->table('admin_user')." as au INNER JOIN ".$yp->table('admin_log')." as al ON au.user_id = al.user_id");
    ...
    /* 查询IP地址列表 */
    $ip_list = array();
    $res = $db->query("SELECT DISTINCT ip_address FROM " .$yp->table('admin_log'));
    while ($row = $db->FetchRow($res))
    {
...
if ($_REQUEST['act'] == 'batch_drop')
{
    ...
    if ($drop_type_date)
    {
        if ($_POST['log_date'] == '0')
        {
            yp_header("Location: admin_logs.php?act=list\n");
            exit;
        }
        elseif ($_POST['log_date'] > '0')
        {
            ...
            $sql = "DELETE FROM " .$yp->table('admin_log').$where;
            $res = $db->query($sql);
            if ($res)
            {
    ...
    else
    {
        ...
        {
            $sql = "DELETE FROM " .$yp->table('admin_log'). " WHERE log_id = '$id'";
            $result = $db->query($sql);
...
function get_admin_logs()
{
	...
	{	
		...
		/* 获得总记录数据 */
		$sql = 'SELECT COUNT(*) FROM ' .$GLOBALS['yp']->table('admin_log'). ' AS al ' . $where;
		...
		/* 获取管理员日志记录 */
		$list = array();
		$sql  = 'SELECT al.*, u.user_name FROM ' .$GLOBALS['yp']->table('admin_log'). ' AS al '.
				'LEFT JOIN ' .$GLOBALS['yp']->table('admin_user'). ' AS u ON u.user_id = al.user_id '.
