...
/**
 * QQ120029121 店铺统计：销售统计
 * ============================================================================
 * 演示地址: http://demo.coolhong.com；
 * ============================================================================
 * $Author: jtypmall $
 * $Id: shop_sell_count_stats.php 17217 2015-10-29 15:13:08Z jtypmall $
 */
...
elseif ($_REQUEST['act'] == 'export')
{
    ...
    $sql = "SELECT IFNULL(s.supplier_id, 0) supplier_id, IFNULL(s.supplier_name, '平台自营') shop_name,"
        ...
        . $GLOBALS['yp']->table('order_info') . ' o LEFT JOIN ' . $GLOBALS['yp']->table('supplier')
    ...
    foreach($result as $key => $value)
    {
        ...
        $user_count = $GLOBALS['db']->getOne(
            'SELECT COUNT(*) FROM (SELECT DISTINCT o.user_id FROM ' . $GLOBALS['yp']->table('order_info')
...
function get_result_list ($start_date, $end_date)
{
    ...
    if($result === false)
    {
        ...
        $sql = 'SELECT COUNT(*) FROM (SELECT DISTINCT supplier_id FROM ' . $GLOBALS['yp']->table('order_info') . ' o ' . $where . ') t';
