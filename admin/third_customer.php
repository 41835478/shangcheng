...
/**
 * SZY QQ120029121 三方客服
 * ============================================================================
 * 演示地址: http://demo.coolhong.com；
 * ============================================================================
 * $Author: jtypmall $
 * $Id: third_customer.php 17217 2015-08-25 13:38:08Z jtypmall $
 */
...
/*初始化数据交换对象 */
$exc = new exchange($yp->table('chat_third_customer'), $db, 'cus_id', 'cus_name');
...
elseif ($_REQUEST['act'] == 'insert_update')
{
    ...
    if(empty($_POST['cus_id']))
    {
        ...
        /* 登录一条三方客服 */
        $db->autoExecute($yp->table('chat_third_customer'), $third_customer, 'INSERT');
    ...
    {
        /* 更新一条三方客服 */
        $db->autoExecute($yp->table('chat_third_customer'), $third_customer, 'UPDATE', "cus_id = '$cus_id'");
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    if ($exc->drop($id))
    {
        $db->query("DELETE FROM " . $yp->table('chat_third_customer') . " WHERE cus_id = $id");
...
elseif ($_REQUEST['act'] == 'batch_drop')
{
    ...
    if (isset($_POST['checkboxes']))
    {
        ...
        {
            $sql = "DELETE FROM " .$yp->table('chat_third_customer'). " WHERE cus_id = $id";
            $db->query($sql);
...
function third_customer_info ($cus_id)
{
    ...
    $sql = "SELECT * FROM " . $GLOBALS['yp']->table('chat_third_customer') . "WHERE cus_id = '$cus_id' ";
...
function get_third_customer_list ()
{
    ...
    if($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['yp']->table('chat_third_customer') . " WHERE supplier_id = 0 $where";
        ...
        /* 查询 */
        $sql = "SELECT * FROM " . $GLOBALS['yp']->table('chat_third_customer') . 
...
{
	$db = $GLOBALS['db'];
	$sql = 'UPDATE ' . $GLOBALS['yp']->table('chat_third_customer') . " SET cus_name = '$name'" . " WHERE cus_id = '$id'";
...
{
	$db = $GLOBALS['db'];
	$sql = 'UPDATE ' . $GLOBALS['yp']->table('chat_third_customer') . " SET cus_no = '$name'" . " WHERE cus_id = '$id'";
	$GLOBALS['db']->query($sql);
