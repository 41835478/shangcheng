...
function get_goodstype()
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数以及页数 */
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['yp']->table('goods_type');
    ...
    foreach ($all AS $key=>$val)
    {
      ...
      if($all[$key]['supplier_id']){
           $sql = "select supplier_name from " . $GLOBALS['yp']->table('supplier') . "WHERE supplier_id = ".$val['supplier_id'];
           $all[$key]['supplier_name'] = $GLOBALS['db']->getOne($sql);//归属商家
        }else{

          $all[$key]['supplier_name']= '平台自有';
