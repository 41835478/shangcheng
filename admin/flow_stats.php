 ...
 * ============================================================================
 * $Author: PRINCE $
 * $Id: flow_stats.php 17217 2017-04-01 06:29:08Z PRINCE $
*/
...
if ($_REQUEST['act'] == 'view')
{
    ...
    if(!$is_multi)
    {
        ...
        $sql = "SELECT COUNT(*) AS access_count, area FROM " . $yp->table('stats') .
                " WHERE access_time >= '$start_date' AND access_time < " .($end_date + 86400).
    ...
    else
    {
        ...
        $sql = "SELECT access_time, area FROM " . $yp->table('stats') .
                " WHERE $where";
        $res = $db->query($sql);
        $area_arr = array();
        while ($val = $db->fetchRow($res))
    ...
    if(!$is_multi)
    {
        ...
        $sql = "SELECT COUNT(*) AS access_count, referer_domain FROM " . $yp->table('stats') .
                " WHERE access_time >= '$start_date' AND access_time <= " .($end_date + 86400).
    ...
    else
    {
        ...
        $sql = "SELECT access_time, referer_domain FROM " . $yp->table('stats') .
                " WHERE $where";
...
elseif ($act = 'download')
{
    ...
    $sql = "SELECT COUNT(*) AS access_count, area FROM " . $GLOBALS['yp']->table('stats') .
            " WHERE access_time >= '$start_date' AND access_time <= " .($end_date + 86400).
            " GROUP BY area ORDER BY access_count DESC LIMIT 20";
    ...
    $sql = "SELECT COUNT(*) AS access_count, referer_domain FROM " . $GLOBALS['yp']->table('stats') .
            " WHERE access_time >= '$start_date' AND access_time <= " .($end_date + 86400).
            " GROUP BY referer_domain ORDER BY access_count DESC LIMIT 20";
