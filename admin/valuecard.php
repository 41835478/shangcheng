...
/* 初始化$exc对象 */
$exc = new exchange($yp->table('valuecard_type'), $db, 'type_id', 'type_name');
...
if ($_REQUEST['act'] == 'remove')
{
	...
	$sql="select count(*) from ". $yp->table('valuecard') ." where vc_type_id='$id' ";
...
{  
    ...
    /* 检查类型是否有重复 */
    $sql = "SELECT COUNT(*) FROM " .$yp->table('valuecard_type'). " WHERE type_name='$type_name'";
    if ($db->getOne($sql) > 0)
    {
    ...
    /* 插入数据库。 */
    $sql = "INSERT INTO ".$yp->table('valuecard_type')." (type_name, type_money, use_start_date, use_end_date)
    VALUES ('$type_name',
            '$_POST[type_money]',
            '$use_startdate',
            '$use_enddate')";
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    $vtype_arr = $db->getRow("SELECT * FROM " .$yp->table('valuecard_type'). " WHERE type_id = '$type_id'");
...
if ($_REQUEST['act'] == 'update')
{
    ...
    $sql = "UPDATE " .$yp->table('valuecard_type'). " SET ".
           "type_name       = '$type_name', ".
...
//        $vc_sn = $num . str_pad(mt_rand(12345678, 99999999), 8, '0', STR_PAD_LEFT);
		...
		$vc_id = $db->getOne("select vc_id from ". $yp->table('valuecard') ." where vc_sn = '$vc_sn' ");
		if (!$vc_id)
		{
			$db->query("INSERT INTO ".$yp->table('valuecard')." (vc_type_id, vc_sn,vc_pwd, add_time) VALUES('$vc_type_id', '$vc_sn', '$vc_pwd', '$add_time')");
			$j++;
		}
    }
...
if ($_REQUEST['act'] == 'gen_excel')
{
    ...
    $type_name = $db->getOne("SELECT type_name FROM ".$yp->table('valuecard_type')." WHERE type_id = '$tid'");
    ...
    if (YP_CHARSET != 'gbk')
    {
        echo yp_iconv('UTF8', 'GB2312', $type_name."储值卡号码列表") . "\t\n";
        /* 红包序列号, 红包金额, 类型名称(红包名称), 使用结束日期 */
        echo yp_iconv('UTF8', 'GB2312', $_LANG['bonus_sn']) ."\t";
		echo yp_iconv('UTF8', 'GB2312', $_LANG['vc_pwd']) ."\t";
    ...
    $sql = "SELECT vc.vc_id, vc.vc_type_id, vc.vc_sn, vc.vc_pwd,  vt.type_name, vt.type_money, vt.use_start_date, vt.use_end_date ".
           "FROM ".$yp->table('valuecard')." AS vc, ".$yp->table('valuecard_type')." AS vt ".
    ...
    {
    	/* 修改 by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1 字串导出变成科学技术问题 begin */
        // echo " ".$val['vc_sn'] . "\t"; 
        echo chunk_split($val['vc_sn'] ,4," ") . " \t";
        /* 修改 by demo.coolhong.com 今 天 优 品 多 商 户 系 统 q q 1 2 0 0 2 9 1 2 1 字串导出变成科学技术问题 end */
		echo $val['vc_pwd'] . "\t";
        echo $val['type_money'] . "\t";
...
elseif ($_REQUEST['act'] == 'remove_bonus')
{
    ...
    $db->query("DELETE FROM " .$yp->table('valuecard'). " WHERE vc_id='$id'");
...
if ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['checkboxes']))
    {
        ...
        if (isset($_POST['drop']))
        {
            $sql = "DELETE FROM " . $yp->table('valuecard'). " WHERE vc_id " . db_create_in($vc_id_list);
            $db->query($sql);
...
function get_type_list()
{
    ...
    $sql = "SELECT vc_type_id, COUNT(*) AS sent_count".
            " FROM " .$GLOBALS['yp']->table('valuecard') .
            " GROUP BY vc_type_id";
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['yp']->table('valuecard_type');
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        /* 分页大小 */
        $filter = page_and_size($filter);

        $sql = "SELECT * FROM " .$GLOBALS['yp']->table('valuecard_type'). " ORDER BY $filter[sort_by] $filter[sort_order]";
...
function get_valuecard_list()
{
    ...
    $sql = "SELECT COUNT(*) FROM ".$GLOBALS['yp']->table('valuecard'). ' AS vc '.$where;
    $filter['record_count'] = $GLOBALS['db']->getOne($sql);
    ...
    $sql = "SELECT vc.*, u.user_name ".
          " FROM ".$GLOBALS['yp']->table('valuecard'). " AS vc ".
          " LEFT JOIN " .$GLOBALS['yp']->table('users'). " AS u ON vc.user_id=u.user_id  $where ".
...
{
    $sql = "SELECT * FROM " . $GLOBALS['yp']->table('valuecard_type') .
