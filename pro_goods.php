 ...
 * ============================================================================
 * $Author: prince $
 * $Id: pro_goods.php 17063 2010-03-25 06:35:46Z prince $
*/
...
function get_article_new( $id = array(0), $getwhat = 'art_id', $num = 0, $isrand = false, $showall = true, $start = 0 ) {
	...
	elseif( $getwhat == 'goods_cat' || $getwhat == 'goods_id' ) {
		...
		'LEFT JOIN ' . $GLOBALS['yp']->table('goods_article') . ' AS ga ON g.goods_id=ga.goods_id INNER JOIN ' . 
		$GLOBALS['yp']->table('article') . ' AS a ON ga.article_id = a.article_id, ' .
		$GLOBALS['yp']->table('article_cat') . 'AS ac ' .
		'WHERE (a.cat_id = ac.cat_id) AND ' . $wherestr;	
	}
