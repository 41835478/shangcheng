...
// START  safety optimization by prince  2015-11-23
...
// END  safety optimization by prince  2015-11-23
...
// 读取频道页模版
$index_file_sql = "select * from " . $yp->table('category') . " where cat_id = " . $cat_id;
...
$display  = (isset($_REQUEST['display']) && in_array(trim(strtolower($_REQUEST['display'])), array('list', 'grid', 'text'))) ? trim($_REQUEST['display'])  : (isset($_COOKIE['YP']['display']) ? $_COOKIE['YP']['display'] : $default_display_type);
	...
	if (!$smarty->is_cached($index_dwt_files, $cache_id))
	{
		...
		if ($brand > 0)
		{
			if (strstr($brand,'_'))
			{
				...
				$sql = "SELECT brand_name FROM " .$GLOBALS['yp']->table('brand'). " WHERE brand_id in ($brand_sql) ";
			...
			{
				$sql = "SELECT brand_name FROM " .$GLOBALS['yp']->table('brand'). " WHERE brand_id = '$brand'";
		...
		$sql = "SELECT b.brand_id, b.brand_name, b.brand_logo, COUNT(*) AS goods_num ".
				"FROM " . $GLOBALS['yp']->table('brand') . "AS b, ".
					$GLOBALS['yp']->table('goods') . " AS g LEFT JOIN ". $GLOBALS['yp']->table('goods_cat') . " AS gc ON g.goods_id = gc.goods_id " .
				"WHERE g.brand_id = b.brand_id AND ($children OR " . 'gc.cat_id ' . db_create_in(array_unique(array_merge(array($cat_id), array_keys(cat_list($cat_id, 0, false))))) . ") AND b.is_show = 1 " .
				" AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ".
				"GROUP BY b.brand_id HAVING goods_num > 0 ORDER BY b.sort_order, b.brand_id ASC"; //此SQL语句增加字段 b.brand_logo,  By  demo.coolhong.com 今天优品 多商户系统 QQ 120-029-121
		...
		$brand_have_logo = 0;  //代码增加   By    demo.coolhong.com
		...
		foreach ($brands AS $key => $val)
		{
			...
			$brands[$temp_key]['brand_id_qq120029121'] = $val['brand_id'];
		...
		$smarty->assign('brand_zimu_qq120029121', $brand_zimu);
		...
		{
		$brands_wwwwx_120029121Com=array();
		$brands_wwwwx_120029121Com[0] = $brands[0];
		$brands_qq=explode(',' , $cat['brand_qq']);
		foreach($brands_qq as $key_qq => $brand_qq)
		{
			foreach($brands as $key_wwwwx_120029121com=>$brand_wx_120029121com)
			{
				if($brand_wx_120029121com['brand_name']==$brand_qq)
				{
					$brands_wwwwx_120029121Com[]=$brand_wx_120029121com;
					break;
				}
			}
		}
		$brands=$brands_wwwwx_120029121Com;
		$smarty->assign('brands', $brands);
		}
		...
		if ($cat['filter_attr'] > 0)
		{
			...
			{
				$sql = "SELECT a.attr_name FROM " . $yp->table('attribute') . " AS a, " . $yp->table('goods_attr') . " AS ga, " . $yp->table('goods') . " AS g WHERE ($children OR " . get_extension_goods($children) . ") AND a.attr_id = ga.attr_id AND g.goods_id = ga.goods_id AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND a.attr_id='$value'";
				if($temp_name = $db->getOne($sql))
				{
					$all_attr_list[$key]['filter_attr_name'] = $temp_name;

					$sql = "SELECT a.attr_id, MIN(a.goods_attr_id ) AS goods_id, a.attr_value AS attr_value FROM " . $yp->table('goods_attr') . " AS a, " . $yp->table('goods') .
						   " AS g" .
					...
					foreach ($attr_list as $k => $v)
					{
						...
						if($temp_name=='颜色')
						{
							$all_attr_list[$key]['attr_list'][$temp_key]['color_code']=$db->getOne("select color_code from ". $yp->table('attribute_color') ." where color_name='$v[attr_value]' and attr_id='$v[attr_id]' ");
						}
			...
			foreach ($filter_attr AS $fkey=>$fval)
			{
				if ($fval)
				{
					...
					$temp_arrt_url_qq120029121 =implode(".", $filter_attr_temp);
					if (strstr($fval, "_"))
					{
						$fval_array = explode("_", $fval);
						foreach ($fval_array AS $fval_key=> $fval_qq120029121)
						{
							$cond_name .= ($fval_key ? "，" : "") . $filter_attr_name[$fkey][$fval_qq120029121];
						}
					}
					else
					{
					...
					$condition[]=array(
							...
							'cond_name' =>$cond_name,
							'cond_url' =>build_uri('category', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>$price_min, 'price_max'=>$price_max, 'filter_attr'=>$temp_arrt_url_qq120029121, 'filter'=>$filter), $cat['cat_name'])
						);
				}
			}
			$attr_group_more_txt = "";
			$attr_group_more_id =0;
			foreach ($all_attr_list AS $k_qq120029121=>$k_value)
			{
				if ($k_qq120029121 >1)
				{
			...
			{
				$ext_sql = "SELECT DISTINCT(b.goods_id) FROM " . $yp->table('goods_attr') . " AS a, " . $yp->table('goods_attr') . " AS b " .  "WHERE ";
		 ...
		 if($cat['attr_qq120029121'])
		 {
			 ...
			 $attr_def_wwwwx_120029121com=array();
			 foreach($attr_qq120029121 as $attr_wx_120029121com)
			 {
				if ($attr_wx_120029121com)
				 {
						$attr_qq = explode(":", $attr_wx_120029121com);
						$attr_def_wwwwx_120029121com[$attr_qq[0]]=explode(",", $attr_qq[1]);
				}
			 }

			foreach ($all_attr_list as $key_attr_qq => $attr_old_wx_120029121com)
			{
				foreach($attr_def_wwwwx_120029121com as $key_def_wx_120029121com => $attr_def_wx_120029121com)
				{
					if ( $attr_old_wx_120029121com['filter_attr_name'] == $key_def_wx_120029121com)
					{
						 $attr_list_qq = array();
						 $attr_list_qq[0]= $attr_old_wx_120029121com['attr_list'][0];
						 foreach($attr_def_wx_120029121com as  $attr_new_wx_120029121com)
						 {
								foreach($attr_old_wx_120029121com['attr_list'] as $attr_temp_qq)
								{
									if($attr_temp_qq['attr_value'] == $attr_new_wx_120029121com )
									{
    ...
    $smarty->assign('current_cat_pr2_id',$catlist[count($catlist)-2]);/*取得二级分类ID*/
        ...
        //file_put_contents('./33$goodslist$goodslist$goodslist.txt',var_export($goodslist,true));//prince 
...
function get_categories($cat_id = 0)
{
    ...
    /*
     ...
     如果不是取当前分类及其下的子分类
     */
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['yp']->table('category') . " WHERE parent_id = '$cat_id' AND is_show = 1 ";
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        /* 获取当前分类及其子分类 */
        $sql = 'SELECT a.cat_id, a.cat_name, a.sort_order AS parent_order, a.cat_id, a.is_show,' .
            'b.cat_id AS child_id, b.cat_name AS child_name, b.sort_order AS child_order ' .
            'FROM ' . $GLOBALS['yp']->table('category') . ' AS a ' .
            'LEFT JOIN ' . $GLOBALS['yp']->table('category') . ' AS b ON b.parent_id = a.cat_id AND b.is_show = 1 ' .
...
{
    $sql = 'SELECT cat_name ' . 'FROM ' . $GLOBALS['yp']->table('category')  . "WHERE cat_id =$id " ;
...
function get_parent($value,$id='')
{
    ...
    if($value!=0)
    {
        $sql = 'SELECT parent_id FROM ' . $GLOBALS['yp']->table('category') . " WHERE cat_id = '$value'";
...
{
    return $GLOBALS['db']->getRow('SELECT cat_name, keywords, cat_desc, style, grade, filter_attr, parent_id FROM ' . $GLOBALS['yp']->table('category') .
        " WHERE cat_id = '$cat_id'");
}
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order, $is_stock = 0)
{
    ...
    $sql = "SELECT g.goods_id, g.goods_name, g.goods_name_style, g.click_count, g.goods_number, g.market_price, " .
		   ...
		   " AS g " .
           " LEFT JOIN " . $GLOBALS['yp']->table('member_price') .
		   " AS mp " .
	...
	if ($sort=='salenum')
	{
		$sql = "SELECT IFNULL(o.num,0) AS salenum, g.goods_id, g.goods_name, g.click_count, g.goods_number, g.goods_name_style, " .
			   ...
			   " (SELECT " .
				   ...
				   " AS num,og.goods_id " .
				   " FROM " .
				   $GLOBALS['yp']->table('order_goods') . " AS og, " .
				   $GLOBALS['yp']->table('order_info') . " AS oi " .
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT parent_id, cat_id, grade ".
                   " FROM " . $GLOBALS['yp']->table('category');
            $res = $GLOBALS['db']->getAll($sql);
